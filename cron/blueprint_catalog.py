...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``opencodon://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``opencodon://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"opencodon://blueprint/{quote(blueprint.key)}{qs}"
