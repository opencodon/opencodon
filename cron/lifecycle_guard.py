...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<OPENCODON_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.opencodon/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_opencodon_home() / "scripts" / raw
