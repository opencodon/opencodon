"""Shared transport for the scholarly APIs.

Every literature source in this package talks to a public, unauthenticated
REST API run by a non-profit or a public agency. That shapes the design more
...
class ScholarlyClient:
    """A polite, bounded, retrying JSON client for one scholarly API."""
    ...
    def get_json(
        ...
        Raises :class:`ScholarlyError` when the body is not JSON, so a caller
    ...
    def get(
        ...
        *,
        ...
        Raises :class:`ScholarlyError` on a non-retryable status and on
        ...
        for attempt in range(1, self.max_attempts + 1):
            try:
                with httpx.Client(
                    ...
                    response = client.get(url, params=merged, headers=headers)
