...
class ScienceRuntime:
    ...
    # ── ingest a local file as a tracked upload ─────────────────────
    ...
    def ingest_file(
        ...
        *,
        ...
        """Register a local file as a user-upload artifact version.

        Brings an on-disk file into the tracked world so it can be declared
        as a run_code input, read via load_artifact(), and appear in lineage
        / reproduction with a checksum — the principled alternative to a raw
        absolute-path read from an isolated kernel workspace.

        Idempotent on content: re-ingesting an unchanged file returns the
        existing latest version rather than churning a new one.
        """
        src = Path(path).expanduser()
        if not src.is_file():
            raise FileNotFoundError(f"no such file to ingest: {src}")
        filename = filename or src.name
        root_session_id = self.root_for(session_id)
        ref = self._blobs.put_path(src)
        ...
        if artifact is None:
            ...
            latest = self._store.latest_version(artifact_id)
            if latest and latest["checksum"] == ref.sha256:
                # Unchanged upload — reuse the existing version.
                return {
