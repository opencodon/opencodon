...
#
# mkNpmPassthru returns packageJsonPath (e.g. "apps/tui/package.json")
...
let
  ...
  # that's all npm uses here.  Literal patterns (e.g. "apps/tui") pass
  # through unchanged.
  expandWorkspace =
    pattern:
    let
  ...
  # apps/desktop + apps/shared + apps/tui + apps/web → [ "apps" ].
      ...
      let
        ...
        excludedDirs =
          ...
          ++ [
            ...
            "nix"
            # Skills are shipped via OPENCODON_BUNDLED_SKILLS /
            # OPENCODON_OPTIONAL_SKILLS (see opencodon.nix), not via the
            ...
            # __init__.py) shipped via symlinks + OPENCODON_BUNDLED_LOCALES
            # / OPENCODON_OPTIONAL_MCPS, not via the wheel. Excluding them
...
in
{
  ...
  #
  # Usage:
  #   npm = opencodonNpmLib.mkNpmPassthru { dirs = [ "apps/tui" ]; };
