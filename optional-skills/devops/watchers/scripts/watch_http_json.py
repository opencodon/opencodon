...
Usage (via cron with --no-agent):
    ...
    opencodon cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$OPENCODON_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "opencodon-Watcher/1.0"})
