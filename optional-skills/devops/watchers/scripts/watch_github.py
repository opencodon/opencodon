...
Usage (via cron with --no-agent):
    ...
    opencodon cron create opencodon-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$OPENCODON_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name opencodon-issues --repo opencodon/opencodon --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the opencodon .env file
(``${OPENCODON_HOME:-~/.opencodon}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "opencodon-Watcher/1.0",
    }
