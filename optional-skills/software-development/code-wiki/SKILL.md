...
version: 0.1.0
author: Teknium (teknium1), opencodon
license: MIT
...
Invoke through the `terminal` tool from the target repo's root, then use `read_file` / `search_files` / `write_file` to produce the wiki. Default output location is `~/.opencodon/wikis/<repo-name>/`. Only write into the repo (`docs/wiki/`) when the user explicitly requests it.
...
```bash
OUTPUT_DIR="$HOME/.opencodon/wikis/$REPO_NAME"
...
1. **User input** — [`cli.py:OpencodonCLI.run_session`](<link>)
...
cat > "$OUTPUT_DIR/.codewiki-state.json" <<EOF
{
  ...
  "generator": "opencodon code-wiki skill v0.1.0",
  "modules_documented": []
}
EOF
```
...
```
Generated wiki at ~/.opencodon/wikis/<repo-name>/:
...
- **In-repo output without asking.** Default is `~/.opencodon/wikis/`. Only write into the repo when the user explicitly requests it.
