...
$OPENCODON_OSINT_CACHE/icij/ (default: ~/.cache/opencodon-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("OPENCODON_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "opencodon-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "opencodon osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $OPENCODON_OSINT_CACHE/icij or ~/.cache/opencodon-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
