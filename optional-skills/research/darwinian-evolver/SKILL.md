...
version: 0.1.0
author: Bihruze (Asahi0x), opencodon
license: MIT
...
import upstream classes into opencodon itself.
...
```bash
mkdir -p ~/.opencodon/cache/darwinian-evolver && cd ~/.opencodon/cache/darwinian-evolver
...
```bash
cd ~/.opencodon/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.opencodon/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.opencodon/skills/research/darwinian-evolver
DE_DIR=~/.opencodon/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside opencodon core.
   Custom driver scripts under `~/.opencodon/skills/...` are user-side and fine.
