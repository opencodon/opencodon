...
- ``list_authenticated_providers`` already populates each row's
  ...
  curation and pulls in non-agentic models (an aggregator's /models can
  return hundreds of IDs including TTS, embeddings, rerankers, image and
  video generators).
"""
...
def build_models_payload(
    ...
    *,
    ...
    try:
        from opencodon_cli.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
...
def _append_unconfigured_rows(
    ...
    *,
    ...
    from opencodon_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`opencodon_cli.model_switch.list_picker_providers`) so the
