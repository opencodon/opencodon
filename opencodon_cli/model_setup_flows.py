"""Per-provider model-selection wizard flows for ``opencodon setup`` / ``opencodon model``.

Extracted from ``opencodon_cli/main.py`` as part of the god-file decomposition
campaign (``~/.opencodon/plans/god-file-decomposition.md``, Phase 2 — splitting
...
in main.py re-imports them (``from opencodon_cli.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``opencodon_cli.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from opencodon_cli.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``opencodon_cli.main`` at import time -> no import cycle.
"""
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from opencodon_cli.main import _prompt_api_key
    ...
    # bad paste short of re-running `opencodon setup` from scratch. OpenRouter
    ...
    from opencodon_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    if not presets:
        print("No MoA presets configured. Run `opencodon moa configure <name>` first.")
        return
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from opencodon_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `opencodon auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    # live only in the pool (e.g. after ``opencodon auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from opencodon_cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from opencodon_cli.auth import (
...
def _model_flow_custom(config):
    ...
    from opencodon_cli.models import probe_api_models
    ...
    if probe.get("used_fallback") and probe.get("resolved_base_url"):
        ...
        print(
            ...
            f"opencodon will still save it."
        )
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `opencodon model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    from opencodon_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  opencodon will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  opencodon will install it now (the preflight below "
...
def _model_flow_copilot(config, current_model=""):
    ...
    from opencodon_cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from opencodon_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from opencodon_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from opencodon_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates opencodon turns to `copilot --acp`.")
    print("  opencodon currently starts its own ACP subprocess for each request.")
    print("  opencodon uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set OPENCODON_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from opencodon_cli.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from opencodon_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from opencodon_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so opencodon knows where to find it
...
def _model_flow_vertex(config, current_model=""):
    ...
    *path* lives in .env (VERTEX_CREDENTIALS_PATH / GOOGLE_APPLICATION_CREDENTIALS);
    ...
    if sa_path:
        ...
        print("      • set VERTEX_CREDENTIALS_PATH in ~/.opencodon/.env to a service account JSON")
    print()
...
def _select_zai_endpoint(current_base: str) -> str:
    ...
    sourced from ``ZAI_ENDPOINTS`` in ``opencodon_cli.auth`` so it stays in
    sync with the probe list.

    Returns the selected base URL.  Falls back to *current_base* on cancel
    or error.
    """
    from opencodon_cli.main import _prompt_provider_choice
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   opencodon typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with opencodon, enable billing on your "
    ...
    if provider_id == "lmstudio":
        ...
        from opencodon_cli.models import fetch_lmstudio_models
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from opencodon_cli.main import _run_anthropic_oauth_flow
    from opencodon_cli.auth import (
        _prompt_model_selection,
        _save_model_choice,
        deactivate_provider,
    ...
    if selected:
        ...
        # providers if the user switches without running 'opencodon model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
