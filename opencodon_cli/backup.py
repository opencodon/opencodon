...
# OPENCODON_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    ...
    # Python dependency trees (plugin / MCP-server venvs under OPENCODON_HOME) —
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside OPENCODON_HOME, resolved from config only (no network, no init).
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside OPENCODON_HOME.
...
def run_backup(args) -> None:
    ...
    # Collect files
    print(f"Scanning {display_opencodon_home()} ...")
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_opencodon_home()}."
        )
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.opencodon/config.yaml` instead of `config.yaml`.
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_opencodon_home()}")
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_opencodon_home()}."
            )
...
_QUICK_STATE_FILES = (
    ...
    # are at $OPENCODON_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside OPENCODON_HOME and the entry is silently
...
def _quick_snapshot_root(opencodon_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(opencodon_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside OPENCODON_HOME. Kept in sync with
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(opencodon_home), keep=keep)
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        ...
        print(f"  {len(snaps)} snapshot(s) stored in {display_opencodon_home()}/state-snapshots/")
...
def _pre_update_backup_dir(opencodon_home: Optional[Path] = None) -> Path:
    home = opencodon_home or get_opencodon_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of OPENCODON_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<OPENCODON_HOME>/backups/pre-update-<timestamp>.zip`` and
