"""``opencodon login`` subcommand parser.
...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``opencodon login`` was removed in favor of ``opencodon auth`` / ``opencodon model``
    (the runtime handler in ``opencodon_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``opencodon login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``opencodon --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``opencodon login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``opencodon model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `opencodon auth` to manage credentials, "
            "`opencodon model` to select a provider, or `opencodon setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `opencodon model`",
    )
    login_parser.add_argument(
        "--no-browser",
        action="store_true",
