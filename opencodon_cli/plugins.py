"""
opencodon Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with opencodon;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.opencodon/plugins/<name>/``
3. **Project plugins** – ``./.opencodon/plugins/<name>/`` (opt-in via
   ``OPENCODON_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``opencodon_agent.plugins``
   entry-point group.
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``OPENCODON_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
...
# ---------------------------------------------------------------------------
#
# Set ``OPENCODON_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.opencodon/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("OPENCODON_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When OPENCODON_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other opencodon loggers.
    """
    ...
    logger.debug(
        "OPENCODON_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. opencodon' shipped guidance lives in the evidence-based
    ...
    # Kanban task lifecycle hooks. Fired by opencodon_cli.kanban_db when a task
    ...
    # separate `opencodon -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `opencodon kanban dispatch`),
...
ENTRY_POINTS_GROUP = "opencodon_agent.plugins"

_NS_PARENT = "opencodon_plugins"
...
class PluginManifest:
    ...
    #              in ~/.opencodon/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``opencodon plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        ...
        running under ``~/.opencodon/profiles/<name>``, or ``"custom"`` when
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with opencodon core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``opencodon <plugin> ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``opencodon <subcommand>``
    ...
    def register_secret_source(self, source) -> None:
        ...
        sources run during ``load_opencodon_dotenv()`` startup — after
        ``~/.opencodon/.env`` loads, before opencodon reads credentials — when
    ...
    def register_slack_action_handler(
        ...
        opencodon' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``opencodon model → Configure auxiliary models`` picker
    ...
    def register_skill(
        ...
        ``~/.opencodon/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if env_var_enabled("OPENCODON_SAFE_MODE"):
            logger.info("OPENCODON_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to opencodon_cli/. Two layouts are
        ...
        # 2. User plugins (~/.opencodon/plugins/)
        user_dir = get_opencodon_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.opencodon/plugins/)
        if _env_enabled("OPENCODON_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".opencodon" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set OPENCODON_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with opencodon and must
            ...
            # invocation — including plain `opencodon chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform opencodon ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `opencodon plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``opencodon plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`opencodon plugins
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `opencodon plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``opencodon_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``opencodon_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="opencodon-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``opencodon tools`` TUI so plugin-provided toolsets appear
