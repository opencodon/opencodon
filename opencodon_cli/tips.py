...
TIPS = [
    ...
    "hermes memory setup lets you configure an external memory provider (e.g. Honcho).",
    ...
    "OPENCODON_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    "The plan skill saves markdown plans under .opencodon/plans/ in the active workspace.",
    ...
    "Context files (.opencodon.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Hermes loads project context from .opencodon.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "Checkpoints use shadow repos in ~/.opencodon/checkpoints/ — your project's .git is never touched.",
    ...
    "OPENCODON_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    ...
    "Cron scripts live in ~/.opencodon/scripts/ and run before the agent — perfect for data collection pipelines.",
    ...
    "The skills audit log at ~/.opencodon/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Hermes state via OPENCODON_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "OPENCODON_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in OPENCODON_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.opencodon/interrupt_debug.log with timestamps.",
    ...
    "OPENCODON_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via hermes tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.opencodon/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.opencodon/skills/.hub/quarantine/ holds skills pending security review.",
    ...
    '/reload reloads ~/.opencodon/.env into the running session — pick up new API keys without restarting.',
    ...
    "OPENCODON_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.opencodon/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.opencodon/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    ...
    # --- TUI & Dashboard ---
    'OPENCODON_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "OPENCODON_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'Drop a YAML in ~/.opencodon/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.opencodon/dashboard-plugins/ — no npm build required.',
    ...
    'OPENCODON_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'OPENCODON_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'OPENCODON_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'OPENCODON_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.opencodon/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.opencodon/skills/ so drop-in skills appear without restarting the session.',
    ...
    'OPENCODON_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'OPENCODON_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.opencodon/plugins/ — trust-gated by design.',
    "OPENCODON_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'OPENCODON_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'OPENCODON_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'OPENCODON_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'OPENCODON_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'OPENCODON_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'OPENCODON_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'OPENCODON_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'OPENCODON_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'OPENCODON_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'OPENCODON_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.opencodon/dashboard-plugins/.',
]
