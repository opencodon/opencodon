"""opencodon hooks — inspect and manage shell-script hooks.

Usage::

    opencodon hooks list
    opencodon hooks test <event> [--for-tool X] [--payload-file F]
    opencodon hooks revoke <command>
    opencodon hooks doctor

Consent records live under ``~/.opencodon/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.opencodon/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``opencodon hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: opencodon hooks {list|test|revoke|doctor}")
        print("Run 'opencodon hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.opencodon/config.yaml.")
        print("See `opencodon hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `opencodon hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `opencodon hooks test` and `opencodon hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (opencodon wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `opencodon hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `opencodon hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `opencodon hooks doctor`.")
