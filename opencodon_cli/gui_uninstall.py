"""
opencodon Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``opencodon desktop`` / ``opencodon gui``)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/opencodon.app`` or ``~/Applications/opencodon.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\opencodon`` (NSIS per-user)
...
the app name ("opencodon"), separate from ``$OPENCODON_HOME``:
  - macOS:   ``~/Library/Application Support/opencodon``
  - Windows: ``%APPDATA%\\opencodon``
  - Linux:   ``$XDG_CONFIG_HOME/opencodon`` (default ``~/.config/opencodon``)
...
``opencodon uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def desktop_userdata_dir() -> Path:
    ...
    Mirrors Electron's ``app.getPath('userData')`` for an app named "opencodon"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "opencodon"
...
def source_built_gui_artifacts(opencodon_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``opencodon desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``opencodon/`` source + ``venv/`` and never
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "opencodon" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/opencodon.app"),
            home / "Applications" / "opencodon.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\opencodon).
            local_base / "Programs" / "opencodon",
            # Older / alternate layout some builds used.
            local_base / "opencodon-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "opencodon")
    else:
        ...
        paths += [
            data_base / "applications" / "opencodon.desktop",
            data_base / "applications" / "opencodon.desktop",
        ]
    return paths
...
def uninstall_gui(opencodon_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``opencodon/opencodon_cli`` (agent source), ``venv/``, or any
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove opencodon' or "
            "'sudo dnf remove opencodon'). AppImage builds are a single file you "
