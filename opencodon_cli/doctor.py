...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from opencodon_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    ...
    """
    try:
        from opencodon_cli import __version__ as init_version
    except Exception:
        return
...
def run_doctor(args):
    ...
    if ack_target:
        from opencodon_cli.security_advisories import (
            ADVISORIES,
            ack_advisory,
        )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from opencodon_cli.auth import PROVIDER_REGISTRY, get_auth_status
    ...
    try:
        from opencodon_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
