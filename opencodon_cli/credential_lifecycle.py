"""Unified provider-credential lifecycle across every store opencodon reads.

A provider API key can live in up to THREE stores at once:

    1. ``~/.opencodon/.env``                     — the canonical secret store
    2. ``~/.opencodon/auth.json`` →
       ``credential_pool.<provider>[*]``      — env-seeded pool entries
       (``source == "env:<VAR>"``) persisted by the pool loader
    3. ``~/.opencodon/config.yaml``              — inline mirrors written by the
...
def _prune_env_pool_entries(env_var: str) -> List[str]:
    ...
    """
    from opencodon_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `opencodon auth remove` does: a
