...
def find_profile_gateway_processes(
    ...
    try:
        from gateway.status import get_running_pid
        from opencodon_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "opencodon_cli.main"]
    if profile != "default":
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    if sys.platform == "win32":
        try:
            from opencodon_cli.gateway_windows import (
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from opencodon_cli.service_manager import detect_service_manager, get_service_manager
...
def _gateway_list() -> None:
    ...
    """
    try:
        from opencodon_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _gateway_run_command() -> list[str]:
    ...
    """
    cmd = [get_python_path(), "-m", "opencodon_cli.main"]
...
def _builtin_setup_fn(key: str):
    ...
    """
    from opencodon_cli import setup as _s
...
def _dispatch_via_service_manager_if_s6(
    ...
    """
    from opencodon_cli.service_manager import (
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            from opencodon_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
