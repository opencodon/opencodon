"""CLI for the opencodon Kanban board — ``opencodon kanban …`` subcommand.

Exposes the full Kanban command surface documented in the design spec
(``docs/opencodon-kanban-v1-spec.pdf``).  All DB work is delegated to
...
from opencodon_cli import kanban_swarm as ks
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    Used by ``opencodon kanban create`` (and callers) to warn when a task
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`opencodon kanban daemon --force`)."
        )
    return (
        False,
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Durable SQLite-backed task board shared across opencodon profiles. "
            "Tasks are claimed atomically, can depend on other tasks, and "
            "are executed by a named profile in an isolated workspace. "
            "See https://github.com/opencodon/opencodon/tree/main/docs "
            "or docs/opencodon-kanban-v1-spec.pdf for the full design."
        ),
    )
    # --- global --board flag ---
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `opencodon kanban boards switch <slug>` or the "
            "OPENCODON_KANBAN_BOARD env var). Use `opencodon kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_create.add_argument("--project", default=None,
                          help="Link to a project (id or slug). Anchors the task's "
                               ...
                               "deterministic branch. See `opencodon project list`.")
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `opencodon gateway start`.",
    )
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``opencodon kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: opencodon kanban <action> [options]\n"
                "Run 'opencodon kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `opencodon kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
    ...
    with board_scope:
        ...
        # every `opencodon kanban repair` into "could not initialize database"
...
# ---------------------------------------------------------------------------
# Boards management (opencodon kanban boards …)
# ---------------------------------------------------------------------------

def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``opencodon kanban boards <action>``.
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print("(no boards — create one with `opencodon kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `opencodon kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `opencodon kanban boards switch {meta['slug']}` to make it current.")
    return 0


def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `opencodon kanban boards delete <slug>` (alias), the
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("Create one with `opencodon -p <name> setup` before assigning tasks.")
    print()
    print("Next step: start the gateway so ready tasks actually get picked up.")
    print("  opencodon gateway start")
    print()
    print(
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — create a profile with `opencodon -p <name> setup`)")
        return 0
    # Header
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if len(all_boards) > 1:
        ...
        print(
            ...
            f"`opencodon kanban boards list`)\n"
        )
    if not tasks:
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    # comments / runs.
    from opencodon_cli import kanban_diagnostics as kd
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``opencodon kanban show <id>``
...
def _cmd_complete(args: argparse.Namespace) -> int:
    ...
    with kb.connect_closing() as conn:
        for tid in ids:
            ...
            # `opencodon kanban complete <id>` from the terminal tool and
            ...
            if task and task.goal_mode:
                ...
                if judge_available:
                    ...
                    try:
                        ...
                        # opencodon_cli/goals.py. Unpacking fewer raises
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "force", False):
        print(
            "opencodon kanban daemon: DEPRECATED — the dispatcher now runs\n"
            "inside the gateway. To use kanban:\n"
            "\n"
            "    opencodon gateway start       # starts the gateway + embedded dispatcher\n"
            "\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`opencodon kanban list --status ready` / "
                    f"`opencodon kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real opencodon profile (i.e. one the
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from opencodon_cli import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from opencodon_cli import kanban_decompose as decomp
