"""Shared curses-based UI components for opencodon CLI.

Used by `opencodon tools` and `opencodon skills` for interactive checklists.
...
def _token_score(orig: str, lower: str, token: str) -> float | None:
    ...
    A faithful port of ``fuzzyScore`` in apps/tui/src/lib/fuzzy.ts so both
    surfaces rank model ids identically:
