...
def _resolve_restart_drain_timeout() -> float:
    try:
        from opencodon_cli.gateway import _get_restart_drain_timeout
...
from opencodon_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# ---------------------------------------------------------------------------
from opencodon_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from opencodon_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from opencodon_cli.dashboard_auth.token_auth import token_auth_middleware
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from opencodon_cli.models import _AGGREGATOR_PROVIDERS
...
from opencodon_cli import web_git as _web_git  # noqa: E402
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``gateway_mode`` — ``"multiplex"`` when the default gateway serves
    ...
    """
    try:
        from opencodon_cli.profiles import _check_gateway_running, profiles_to_serve
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from opencodon_cli.debug import build_debug_share
...
def get_recommended_default_model(provider: str = ""):
    ...
    try:
        from opencodon_cli.inventory import build_models_payload, load_picker_context
        from opencodon_cli.models import pick_silent_default_model
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from opencodon_cli.main import _auto_provider_name, _save_custom_provider
...
async def _start_device_code_flow(
    ...
    if provider_id == "xai-oauth":
        from opencodon_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from opencodon_cli.auth import _save_codex_tokens
...
async def get_logs(
    ...
    from opencodon_cli.logs import _read_tail, LOG_FILES
...
def _normalize_mcp_server_create(
    ...
    """
    from opencodon_cli.mcp_config import (
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from opencodon_cli.mcp_config import _replace_mcp_servers
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from opencodon_cli.mcp_config import _remove_mcp_server
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from opencodon_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
async def list_webhooks():
    import opencodon_cli.webhook as wh
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    _resolve_profile_dir(name)
    try:
        from opencodon_cli import profile_describer
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from opencodon_cli.platforms import platform_label
...
def _toolset_model_catalog(ts_key: str, plugin_name: str):
    ...
    from opencodon_cli.tools_config import _plugin_image_gen_catalog
...
if sys.platform.startswith("win"):
    try:
        from opencodon_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from opencodon_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from opencodon_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from opencodon_cli.dashboard_auth.audit import AuditEvent, audit_log
        from opencodon_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    """
    from opencodon_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from opencodon_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from opencodon_cli.console_engine import OpencodonConsoleEngine
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    """
    from opencodon_cli.dashboard_auth.prefix import normalise_prefix
...
async def get_dashboard_plugins():
    ...
    try:
        from opencodon_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
        enabled_set = _get_enabled_set()
        disabled_set = _get_disabled_set()
    except Exception:
        enabled_set = set()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from opencodon_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from opencodon_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from opencodon_cli.plugins_cmd import dashboard_remove_user_plugin
...
from opencodon_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
