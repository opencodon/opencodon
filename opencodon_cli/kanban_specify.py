...
Used by ``opencodon kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``opencodon_cli/goals.py`` — same aux
...
OPENCODON_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("OPENCODON_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for opencodon board.
...
def _profile_author() -> str:
    """Mirror of ``opencodon_cli.kanban._profile_author``. Kept local to
    ...
    *,
    ...
    try:
        ...
        resp = call_llm(
            ...
            temperature=0.3,
            max_tokens=OPENCODON_KANBAN_SPECIFY_MAX_TOKENS,
