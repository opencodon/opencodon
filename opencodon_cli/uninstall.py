    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                ...
                if 'opencodon_cli' in content or 'hermes-agent' in content:
                    wrapper.unlink()
...
def _uninstall_profile(profile) -> None:
    ...
    #    Use `python -m opencodon_cli.main` so we don't depend on a `hermes`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "opencodon_cli.main", "--profile", name]
