"""Safe opencodon Console command engine.

This module backs ``opencodon console`` and is intentionally narrower than the
full opencodon CLI. It exposes a curated set of native adapters that can later be
...
class ConsoleCommand:
    ...
    usage: str
    summary: str
    handler: Callable[["OpencodonConsoleEngine", list[str]], str]
    mutating: bool = False
    confirmation: str = ""
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'opencodon doctor'")
        and last.startswith("Run 'opencodon setup'")
    ):
        return text.rstrip()
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="opencodon", add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `opencodon "):
        return ""
    return summary
...
# dashboard opens a fresh OpencodonConsoleEngine per /api/console connection, so
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("opencodon_cli.main")
    builder = getattr(module, builder_name)
...
def _extracted_handler(
    ...
    main_handler_name: str,
    namespace_update: Callable[[argparse.Namespace], None] | None = None,
) -> Callable[["OpencodonConsoleEngine", list[str]], str]:
    def handler(_engine: OpencodonConsoleEngine, args: list[str]) -> str:
...
def _register_command_family(
    ...
    *,
    ...
    handler_factory: Callable[[Sequence[str]], Callable[["OpencodonConsoleEngine", list[str]], str]],
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `opencodon {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `opencodon {usage}`?",
        )


class OpencodonConsoleEngine:
    """Curated line-command executor for opencodon Console."""
    ...
    def execute(self, line: str, *, confirmed: bool = False) -> ConsoleResult:
        ...
        try:
            ...
            if tokens and tokens[0] == "opencodon":
                tokens = tokens[1:]
            if not tokens:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "opencodon Console does not run shell syntax. Use one supported "
                    "opencodon command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show opencodon component status.", _status)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor)
        self.register(("logs",), "logs [name] [-n N]", "Show recent opencodon logs.", _logs)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update opencodon configuration?",
        )
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for opencodon Console."""

        extracted = {
            "version": (
                "opencodon_cli.subcommands.version",
            ...
            "dump": (
                "opencodon_cli.subcommands.dump",
                "build_dump_parser",
                "cmd_dump",
                [()],
                set(),
            ...
            "debug": (
                "opencodon_cli.subcommands.debug",
                "build_debug_parser",
                "cmd_debug",
                [("share",), ("delete",)],
                {("share",), ("delete",)},
            ...
            "prompt-size": (
                "opencodon_cli.subcommands.prompt_size",
            ...
            "insights": (
                "opencodon_cli.subcommands.insights",
            ...
            "security": (
                "opencodon_cli.subcommands.security",
            ...
            "backup": (
                "opencodon_cli.subcommands.backup",
                "build_backup_parser",
                "cmd_backup",
                [()],
                {()},
            ...
            "import": (
                "opencodon_cli.subcommands.import_cmd",
            ...
            "config": (
                "opencodon_cli.subcommands.config",
                "build_config_parser",
                "cmd_config",
                [("env-path",), ("check",)],
                set(),
            ...
            "tools": (
                "opencodon_cli.subcommands.tools",
                "build_tools_parser",
                "cmd_tools",
            ...
            "plugins": (
                "opencodon_cli.subcommands.plugins",
            ...
            "skills": (
                "opencodon_cli.subcommands.skills",
                "build_skills_parser",
                "cmd_skills",
                [
                    ("browse",),
            ...
            "memory": (
                "opencodon_cli.subcommands.memory",
                "build_memory_parser",
                "cmd_memory",
                [("status",), ("off",), ("reset",)],
                {("off",), ("reset",)},
            ...
            "auth": (
                "opencodon_cli.subcommands.auth",
                "build_auth_parser",
                "cmd_auth",
                [
                    ("list",),
            ...
            "pairing": (
                "opencodon_cli.subcommands.pairing",
            ...
            "webhook": (
                "opencodon_cli.subcommands.webhook",
            ...
            "hooks": (
                "opencodon_cli.subcommands.hooks",
                "build_hooks_parser",
                "cmd_hooks",
            ...
            "slack": (
                "opencodon_cli.subcommands.slack",
                "build_slack_parser",
                "cmd_slack",
                [("manifest",)],
                set(),
            ...
            "profile": (
                "opencodon_cli.subcommands.profile",
            ...
            "cron": (
                "opencodon_cli.subcommands.cron",
                "build_cron_parser",
                "cmd_cron",
        ...
        self.register(
            ...
            _adder_handler("send", (), "opencodon_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("opencodon_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("opencodon_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported opencodon Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in opencodon Console."
        blocked_top = {
            "acp",
            "chat",
            "completion",
        ...
        if first in blocked_top:
            return f"`opencodon {first}` is not available in opencodon Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in opencodon Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in opencodon Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in opencodon Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in opencodon Console.",
            ("skills", "publish"): "`skills publish` is not available in opencodon Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in opencodon Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in opencodon Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in opencodon Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in opencodon Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in opencodon Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in opencodon Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in opencodon Console.",
            ("kanban", "gc"): "`kanban gc` is not available in opencodon Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in opencodon Console."
        return ""
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in opencodon Console.")
...
def _status(_engine: OpencodonConsoleEngine, args: list[str]) -> str:
...
def _doctor(_engine: OpencodonConsoleEngine, args: list[str]) -> str:
...
def _logs(_engine: OpencodonConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in opencodon Console.")
    ...
    from opencodon_cli.logs import list_logs, tail_log
...
def _sessions_list(_engine: OpencodonConsoleEngine, args: list[str]) -> str:
...
def _config_set(_engine: OpencodonConsoleEngine, args: list[str]) -> str:
    if len(args) < 2:
...
def _sessions_export(_engine: OpencodonConsoleEngine, args: list[str]) -> str:
...
def _sessions_rename(_engine: OpencodonConsoleEngine, args: list[str]) -> str:
...
def _sessions_optimize(_engine: OpencodonConsoleEngine, args: list[str]) -> str:
...
def _sessions_repair(_engine: OpencodonConsoleEngine, args: list[str]) -> str:
...
def _profile_status(_engine: OpencodonConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "profile")
    return _dispatch_extracted_subcommand(
        root="profile",
        fixed=(),
        args=(),
        module_name="opencodon_cli.subcommands.profile",
        builder_name="build_profile_parser",
        main_handler_name="cmd_profile",
    )


def _cron_list(_engine: OpencodonConsoleEngine, args: list[str]) -> str:
    parser = _ArgumentParser(prog="cron list", add_help=False)
    parser.add_argument("--all", action="store_true")
    ns = parser.parse_args(args)
    from opencodon_cli.cron import cron_list

    return _capture_output(lambda: cron_list(show_all=ns.all))


def _cron_status(_engine: OpencodonConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "cron status")
    from opencodon_cli.cron import cron_status
...
def run_console_repl(
    *,
    ...
    """Run the local ``opencodon console`` REPL."""
    ...
    engine = OpencodonConsoleEngine()
    if interactive:
        print("opencodon Console. Type `help` for commands, `exit` to quit.", file=stdout)

    while True:
        if interactive:
            print("opencodon> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
