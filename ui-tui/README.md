...
React + Ink terminal UI for opencodon. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
- `src/app/setupHandoff.ts` — launches external `opencodon setup`, suspends Ink while it runs, opens a new session on success
...
### Setup (`setup.ts`)
`/setup` — launches external `opencodon setup` wizard, suspends Ink while it runs
...
ui-tui/
  ...
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external opencodon setup, suspends Ink while it runs
    ...
    types/
      opencodon-ink.d.ts            type declarations for @opencodon/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent OpencodonCLI subprocess for slash commands
```
