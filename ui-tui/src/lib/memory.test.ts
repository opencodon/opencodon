...
const ENV_KEYS = ['OPENCODON_AUTO_HEAPDUMP', 'OPENCODON_HEAPDUMP_DIR', 'OPENCODON_HEAPDUMP_MAX_BYTES'] as const
...
describe('performHeapDump auto opt-in gate (#21767)', () => {
  ...
  beforeEach(() => {
    ...
    dir = mkdtempSync(join(tmpdir(), 'opencodon-heapdump-test-'))
    process.env.OPENCODON_HEAPDUMP_DIR = dir
  })
  ...
  it('writes diagnostics only for auto-high without OPENCODON_AUTO_HEAPDUMP', async () => {
  ...
  it('writes diagnostics only for auto-critical without OPENCODON_AUTO_HEAPDUMP', async () => {
  ...
  it('writes both diagnostics and snapshot for auto-high when OPENCODON_AUTO_HEAPDUMP=1', async () => {
    process.env.OPENCODON_AUTO_HEAPDUMP = '1'
  ...
  it('accepts truthy spellings (true|yes|on, case-insensitive) as opt-in', async () => {
    for (const value of ['true', 'YES', 'On']) {
      process.env.OPENCODON_AUTO_HEAPDUMP = value
      const result = await performHeapDump('auto-high')
  ...
  it('writes both for manual triggers regardless of OPENCODON_AUTO_HEAPDUMP', async () => {
...
describe('heapdump retention guard (#21767)', () => {
  ...
  beforeEach(() => {
    savedDir = process.env.OPENCODON_HEAPDUMP_DIR
    savedMax = process.env.OPENCODON_HEAPDUMP_MAX_BYTES
    delete process.env.OPENCODON_AUTO_HEAPDUMP
    dir = mkdtempSync(join(tmpdir(), 'opencodon-heapdump-prune-'))
    process.env.OPENCODON_HEAPDUMP_DIR = dir
  })

  afterEach(() => {
    if (savedDir === undefined) {
      delete process.env.OPENCODON_HEAPDUMP_DIR
    } else {
      process.env.OPENCODON_HEAPDUMP_DIR = savedDir
    }

    if (savedMax === undefined) {
      delete process.env.OPENCODON_HEAPDUMP_MAX_BYTES
    } else {
      process.env.OPENCODON_HEAPDUMP_MAX_BYTES = savedMax
    }
  ...
  it('evicts oldest files when total bytes exceed the cap, retaining the newest', async () => {
    ...
    process.env.OPENCODON_HEAPDUMP_MAX_BYTES = String(2 * 1024)
