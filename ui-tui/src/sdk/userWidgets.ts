...
import { Box, Text } from '@opencodon/ink'
...
/**
 * User widget apps — Opencodon authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$OPENCODON_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.opencodon/plugins/`: files under OPENCODON_HOME execute
...
const widgetsDir = () => join(process.env.OPENCODON_HOME?.trim() || join(homedir(), '.opencodon'), 'tui-widgets')
 ...
 *  every change, so a widget Opencodon writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Opencodon) ever writes must hot-load too — a 10s poll here read
