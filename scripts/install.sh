...
# Python-driven tool session (e.g. opencodon terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:opencodon/opencodon.git"
REPO_URL_HTTPS="https://github.com/opencodon/opencodon.git"
OPENCODON_HOME="${OPENCODON_HOME:-$HOME/.opencodon}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${OPENCODON_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$OPENCODON_INSTALL_DIR"
...
#   code at /usr/local/lib/opencodon, command at /usr/local/bin/opencodon,
#   data still at /root/.opencodon (OPENCODON_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$OPENCODON_HOME/.no-bundled-skills so future"
            echo "                   'opencodon update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Opencodon.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.opencodon/opencodon"
            echo "                   default (root, Linux): /usr/local/lib/opencodon"
            echo "  --opencodon-home PATH  Data directory (default: ~/.opencodon, or \$OPENCODON_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, opencodon installs the code under"
            echo "  /usr/local/lib/opencodon and links the command into"
            echo "  /usr/local/bin/opencodon (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$OPENCODON_HOME"
            echo "  (default /root/.opencodon).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$OPENCODON_HOME/opencodon are preserved in-place."
...
print_banner() {
    ...
    echo "│             🧬 opencodon installer                     │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  The open-science AI agent.                             │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `opencodon update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Opencodon-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download opencodon","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install opencodon command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `opencodon` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $OPENCODON_HOME/opencodon
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $OPENCODON_HOME/opencodon
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/opencodon
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $OPENCODON_HOME/opencodon — then preserve it)
#
# Always no-op when the user set --dir or $OPENCODON_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in OPENCODON_HOME.
    if is_termux; then
        INSTALL_DIR="$OPENCODON_HOME/opencodon"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$OPENCODON_HOME/opencodon/.git" ]; then
            INSTALL_DIR="$OPENCODON_HOME/opencodon"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/opencodon)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/opencodon"
        ...
        # /usr/local/bin/opencodon wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/opencodon"
        log_info "  Data:    $OPENCODON_HOME (unchanged)"
...
# Point a opencodon-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $OPENCODON_HOME/node/bin — which is NOT on
...
# opencodon's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no opencodon-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$OPENCODON_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$OPENCODON_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$OPENCODON_HOME/node/etc/npmrc"
}

get_opencodon_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/opencodon" ]; then
        echo "$link_dir/opencodon"
    else
        echo "opencodon"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/opencodon/opencodon/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # opencodon owns its own uv at $OPENCODON_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (opencodon_cli/managed_uv.py) looks in the same
    # place, so install.sh and `opencodon update` stay in sync.
    local _managed_uv="$OPENCODON_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $OPENCODON_HOME/bin ..."
    mkdir -p "$OPENCODON_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/opencodon-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/opencodon-uv-installer.$$.sh")"
    ...
    # directly into $OPENCODON_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$OPENCODON_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the opencodon-
...
check_node() {
    ...
    # Repair pre-existing opencodon-managed installs where `npm install -g` lands
    ...
    if [ -x "$OPENCODON_HOME/node/bin/node" ] && node_satisfies_build "$("$OPENCODON_HOME/node/bin/node" --version)"; then
        export PATH="$OPENCODON_HOME/node/bin:$PATH"
        log_success "Node.js $("$OPENCODON_HOME/node/bin/node" --version) found (opencodon-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing opencodon-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.opencodon/node/..."
    ...
    # Place into ~/.opencodon/node/ and symlink binaries into the same bin dir
    # the opencodon command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$OPENCODON_HOME/node"
    mkdir -p "$OPENCODON_HOME"
    mv "$extracted_dir" "$OPENCODON_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$OPENCODON_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$OPENCODON_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$OPENCODON_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$OPENCODON_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.opencodon/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. opencodon install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "opencodon itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `opencodon update` path
                # (#4735).
                ...
                local stash_name
                stash_name="opencodon-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``opencodon update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if opencodon behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up opencodon command..."

    if [ "$USE_VENV" = true ]; then
        OPENCODON_BIN="$INSTALL_DIR/venv/bin/opencodon"
    else
        OPENCODON_BIN="$(which opencodon 2>/dev/null || echo "")"
        if [ -z "$OPENCODON_BIN" ]; then
            log_warn "opencodon not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$OPENCODON_BIN" ]; then
        log_warn "opencodon entry point not found at $OPENCODON_BIN"
...
EOF
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_success "opencodon command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v opencodon' \
                >/dev/null 2>&1; then
        ...
        log_info "opencodon not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# opencodon — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# opencodon — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# opencodon — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so opencodon works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.opencodon directory structure (config at top level, code in subdir)
    mkdir -p "$OPENCODON_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.opencodon/.env (top level, easy to find)
    if [ ! -f "$OPENCODON_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$OPENCODON_HOME/.env"
            log_success "Created ~/.opencodon/.env from template"
        else
            touch "$OPENCODON_HOME/.env"
            log_success "Created ~/.opencodon/.env"
        fi
    else
        log_info "~/.opencodon/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$OPENCODON_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.opencodon/config.yaml (top level, easy to find)
    if [ ! -f "$OPENCODON_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$OPENCODON_HOME/config.yaml"
            log_success "Created ~/.opencodon/config.yaml from template"
        fi
    else
        log_info "~/.opencodon/config.yaml already exists, keeping it"
    fi
    ...
    if [ ! -f "$OPENCODON_HOME/SOUL.md" ]; then
        cat > "$OPENCODON_HOME/SOUL.md" << 'SOUL_EOF'
You are opencodon, an open-science AI agent. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
SOUL_EOF
        log_success "Created ~/.opencodon/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.opencodon/"

    # Seed bundled skills into ~/.opencodon/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `opencodon update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'opencodon update'." \
            > "$OPENCODON_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $OPENCODON_HOME/.no-bundled-skills"
        log_info "  Future 'opencodon update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.opencodon/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.opencodon/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$OPENCODON_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$OPENCODON_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.opencodon/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$OPENCODON_HOME/.env"
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# opencodon browser tools — explicit browser override."
...
install_node_deps() {
    ...
    # Keep the checkout clean so `opencodon update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'opencodon setup' after install."
        return 0
    fi
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for opencodon to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$OPENCODON_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'opencodon whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                OPENCODON_CMD="$(get_opencodon_command_path)"
                $OPENCODON_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'opencodon whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'opencodon gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $OPENCODON_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $OPENCODON_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: opencodon gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: opencodon gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $OPENCODON_CMD gateway > "$OPENCODON_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.opencodon/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: opencodon gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: opencodon gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $OPENCODON_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $OPENCODON_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $OPENCODON_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}opencodon${NC}              Start chatting"
    echo -e "   ${GREEN}opencodon setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}opencodon config${NC}       View/edit configuration"
    echo -e "   ${GREEN}opencodon config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}opencodon gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}opencodon update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'opencodon' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'opencodon' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'opencodon' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$OPENCODON_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$OPENCODON_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$OPENCODON_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$OPENCODON_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$OPENCODON_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
# extract a tree MISSING the electron binary, so the `electron`->`Opencodon` rename
...
# _purge_electron_build_cache() used by `opencodon desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the opencodon-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `opencodon desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Opencodon" ]; then
            app="$desktop_dir/release/linux-unpacked/Opencodon"
        elif [ -x "$desktop_dir/release/linux-unpacked/opencodon" ]; then
            app="$desktop_dir/release/linux-unpacked/opencodon"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Opencodon.app" \
            "$desktop_dir/release/mac/Opencodon.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Opencodon is damaged and can't be opened").
    ...
    # checkout stays clean for the next `opencodon update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the opencodon-managed Node
            # provisioned during prerequisites/node-deps (at $OPENCODON_HOME/node/bin)
        ...
        complete)
            ...
            # $OPENCODON_HOME. $OPENCODON_HOME is a shared data dir (it can be
            # bind-mounted into a Docker gateway too), so a stamp there gets
            # clobbered by the container's 'docker' stamp and wrongly blocks
            # 'opencodon update' on this host install. See detect_install_method().
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $OPENCODON_HOME.
    # $OPENCODON_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'opencodon update' on this host install.
