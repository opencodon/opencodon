...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --hermes-home PATH  Data directory (default: ~/.opencodon, or \$OPENCODON_HOME)"
...
install_uv() {
    ...
    # Hermes owns its own uv at $OPENCODON_HOME/bin/uv.  Always install there —
...
check_node() {
    ...
    if [ -x "$OPENCODON_HOME/node/bin/node" ] && node_satisfies_build "$("$OPENCODON_HOME/node/bin/node" --version)"; then
        ...
        log_success "Node.js $("$OPENCODON_HOME/node/bin/node" --version) found (Hermes-managed)"
        HAS_NODE=true
        return 0
    fi
...
setup_path() {
    ...
    if [ "$USE_VENV" = true ]; then
        OPENCODON_BIN="$INSTALL_DIR/venv/bin/hermes"
    else
        OPENCODON_BIN="$(which hermes 2>/dev/null || echo "")"
    ...
    if [ ! -x "$OPENCODON_BIN" ]; then
        log_warn "hermes entry point not found at $OPENCODON_BIN"
    ...
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                OPENCODON_CMD="$(get_hermes_command_path)"
