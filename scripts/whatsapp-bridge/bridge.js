 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.opencodon/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.opencodon', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (OPENCODON_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.OPENCODON_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.opencodon', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.OPENCODON_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.opencodon', 'document_cache');
const AUDIO_CACHE_DIR = process.env.OPENCODON_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.opencodon', 'audio_cache');
