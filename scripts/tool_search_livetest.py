...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.opencodon/ for one test, copying minimal credentials.
    ...
    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, opencodon_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".opencodon" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, opencodon_home / ".env")
    ...
    (opencodon_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
...
def reset_module_state():
    """Drop cached modules so the new OPENCODON_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "opencodon_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["OPENCODON_HOME"] = str(home)
...
def main():
    ...
    # Restore original OPENCODON_HOME
    if ORIGINAL_HOME is not None:
        os.environ["OPENCODON_HOME"] = ORIGINAL_HOME
    else:
