...
function Install-Venv {
    ...
    if (Test-Path "venv") {
        ...
        if ($env:OS -eq "Windows_NT") {
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Opencodon_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (opencodon.exe) plus its child tree.
            & taskkill /F /T /IM opencodon.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM opencodon.exe is NOT enough: the gateway/agent that a
...
"@ 2>$null
    ...
    if (-not $NoVenv) {
        ...
        # `opencodon` command while opencodon.exe / opencodon-acp.exe exist
...
function Set-PathVariable {
    ...
    # On Windows, the opencodon.exe in venv\Scripts\ has the venv Python baked in
    ...
    Write-Success "opencodon command ready"
}
...
function Install-Desktop {
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, opencodon#38216).
