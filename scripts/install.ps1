# ============================================================================
# opencodon installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    # -HermesHome is a deprecated alias from the hermes-agent lineage; kept
    # for one release so existing drivers (dep_ensure.py, older bootstrap
    # installers) keep working.
    [Alias("HermesHome")]
    [string]$OpencodonHome = $(if ($env:OPENCODON_HOME) { $env:OPENCODON_HOME } else { "$env:LOCALAPPDATA\opencodon" }),
    [string]$InstallDir = $(if ($env:OPENCODON_HOME) { "$env:OPENCODON_HOME\opencodon" } else { "$env:LOCALAPPDATA\opencodon\opencodon" }),
    ...
    # builds apps/desktop into a launchable Opencodon.exe.
    #
    # Why opt-in:
    #   * Opencodon-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Opencodon.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Opencodon.exe
    ...
    #     `opencodon desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:opencodon/opencodon.git"
$RepoUrlHttps = "https://github.com/opencodon/opencodon.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * opencodon installer                       |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  The open-science AI agent.                             |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $OpencodonHome)) {
        New-Item -ItemType Directory -Force -Path $OpencodonHome | Out-Null
    }
    $envFile = Join-Path $OpencodonHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $OpencodonHome "node"
...
function Install-Uv {
    # opencodon owns its own uv at $OpencodonHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (opencodon_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `opencodon update` stay in sync.
    $managedUv = Join-Path $OpencodonHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $OpencodonHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $OpencodonHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $OpencodonHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $OpencodonHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and OPENCODON_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Opencodon-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun opencodon setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which opencodon uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\opencodon\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    opencodon needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the opencodon copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\opencodon\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``OPENCODON_GIT_BASH_PATH`` (User scope) so opencodon can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a opencodon-managed PortableGit install instead..."
    }

    # Download PortableGit into $OpencodonHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $OpencodonHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent opencodon features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  opencodon needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``OPENCODON_GIT_BASH_PATH`` (User env scope) so opencodon can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $OpencodonHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $OpencodonHome\git\usr\bin\bash.exe
    $candidates += "$OpencodonHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$OpencodonHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("OPENCODON_GIT_BASH_PATH", $candidate, "User")
            $env:OPENCODON_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set OPENCODON_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- opencodon may not find Git Bash."
    Write-Info "If needed, set OPENCODON_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    # Prefer a opencodon-managed Node from a previous run over a too-old system one.
    $managedNode = "$OpencodonHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$OpencodonHome\node;$env:Path"
        Write-Success "Node.js $version found (opencodon-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing opencodon-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $OpencodonHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $OpencodonHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\opencodon-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$OpencodonHome\node") { Remove-Item -Recurse -Force "$OpencodonHome\node" }
                Move-Item $extractedDir.FullName "$OpencodonHome\node"
                ...
                $version = & "$OpencodonHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $OpencodonHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\opencodon-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `opencodon
                    # update` path (#4735).
                    ...
                    $stashName = "opencodon-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # ff-only pull cannot succeed -- mirror ``opencodon update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if opencodon behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running opencodon processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/opencodon/opencodon/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/opencodon/opencodon/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/opencodon/opencodon/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\opencodon-$zipLabel.zip"
                $extractPath = "$env:TEMP\opencodon-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `opencodon update` (see the notes at the shared clone-path
    ...
    # next `opencodon update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Opencodon-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running opencodon process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running opencodon processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Opencodon_Gateway*' -or $_.TaskName -like '*Hermes_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (opencodon.exe, plus the deprecated hermes.exe
            # alias shipped for one release) and its child tree.
            & taskkill /F /T /IM opencodon.exe /FI "PID ne $myPid" 2>$null | Out-Null
            & taskkill /F /T /IM hermes.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill by image name is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m opencodon_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not opencodon.exe.
    ...
    if (Test-Path $venvPythonExe) {
        ...
        # the task fires normally on next logon and `opencodon update` / the
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `opencodon.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install opencodon package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and opencodon.exe broken with
    ...
    # here, not 30 seconds later when the user runs `opencodon`.
    if (-not $NoVenv) {
    ...
    if (-not $NoVenv) {
        # uv on Windows can register opencodon.exe in dist-info/RECORD but fail to
        ...
        # `opencodon` command while opencodon-agent.exe / opencodon-acp.exe exist
...
"@ 2>$null
    ...
    # users hit and lazy-import errors from `opencodon dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `opencodon dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `opencodon dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up opencodon command..."

    if ($NoVenv) {
        $opencodonBin = "$InstallDir"
    } else {
        $opencodonBin = "$InstallDir\venv\Scripts"
    }

    # Add the venv Scripts dir to user PATH so opencodon is globally available.
    # On Windows, the opencodon.exe in venv\Scripts\ has the venv Python baked
    # in; the same dir also ships hermes.exe, a deprecated alias kept for one
    # release (see [project.scripts] in pyproject.toml).
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")

    if ($currentPath -notlike "*$opencodonBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$opencodonBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $opencodonBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set OPENCODON_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\opencodon instead
    # of the Unix default ~/.opencodon
    $currentOpencodonHome = [Environment]::GetEnvironmentVariable("OPENCODON_HOME", "User")
    if (-not $currentOpencodonHome -or $currentOpencodonHome -ne $OpencodonHome) {
        [Environment]::SetEnvironmentVariable("OPENCODON_HOME", $OpencodonHome, "User")
        Write-Success "Set OPENCODON_HOME=$OpencodonHome"
    }
    $env:OPENCODON_HOME = $OpencodonHome

    # Update current session
    $env:Path = "$opencodonBin;$env:Path"

    Write-Success "opencodon command ready (plus a hermes alias for one release)"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.opencodon-bootstrap-complete which tells the Opencodon
    ...
    # Opencodon-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `opencodon update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".opencodon-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the OPENCODON_HOME directory structure ($OpencodonHome, default %LOCALAPPDATA%\opencodon)
    New-Item -ItemType Directory -Force -Path "$OpencodonHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OpencodonHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OpencodonHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OpencodonHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OpencodonHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OpencodonHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OpencodonHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OpencodonHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OpencodonHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$OpencodonHome\.env"
    ...
    # Create config.yaml
    $configPath = "$OpencodonHome\config.yaml"
    ...
    # (the default PS5 behaviour), and opencodon's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$OpencodonHome\SOUL.md"
...
"@
    ...
    Write-Success "Configuration directory ready: $OpencodonHome"
    
    # Seed bundled skills into $OpencodonHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $OpencodonHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $OpencodonHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$OpencodonHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $OpencodonHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Opencodon-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'opencodon setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\opencodon-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\opencodon-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
...
# the final `electron` -> `Opencodon` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Opencodon.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_opencodon_desktop command
    # resolves apps/desktop/release/win-unpacked/Opencodon.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the opencodon-managed
    ...
    # The --dir mode produces an unpacked Opencodon.exe in
    ...
    # are NOT trying to work around). The Opencodon icon + product name are
    # stamped onto Opencodon.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\opencodon-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Opencodon.exe",
        "$desktopDir\release\win-arm64-unpacked\Opencodon.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Opencodon.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Opencodon icon + identity are stamped onto Opencodon.exe by the
    ...
    #    Opencodon.exe. We deliberately do NOT point them at `opencodon desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Opencodon.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Opencodon.lnk')
        )

        # Drop stale hermes-agent-era shortcuts: after this rebuild the release
        # dir only contains Opencodon.exe, so a leftover Hermes.lnk would point
        # at a binary that no longer exists. Best-effort.
        foreach ($legacyLnk in @(
        ...
        # with the Opencodon icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.opencodon/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'opencodon setup'."
        return
    }
    ...
    # Run opencodon setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
...
function Start-GatewayIfConfigured {
    ...
    $opencodonCmd = "$InstallDir\venv\Scripts\opencodon.exe"
    if (-not (Test-Path $opencodonCmd)) {
        $opencodonCmd = "opencodon"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$OpencodonHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'opencodon whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: opencodon gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$OpencodonHome\logs\gateway.log"
            Start-Process -FilePath $opencodonCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$OpencodonHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: opencodon gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: opencodon gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$OpencodonHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$OpencodonHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$OpencodonHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$OpencodonHome\opencodon\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   opencodon              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   opencodon setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   opencodon config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   opencodon config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   opencodon gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   opencodon update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning opencodon repository";         Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Opencodon-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding opencodon to PATH";             Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/opencodon/opencodon/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
