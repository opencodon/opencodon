...
2. Arms the synthetic GIL-heavy turn seam (``OPENCODON_ISO_CERTIFY_SYNTH_TURN=1``,
...
the scratch OPENCODON_HOME; ``--isolation on|off`` sets it. Run BOTH:
...
_READY_RE = re.compile(r"OPENCODON_(?:DASHBOARD|BACKEND)_READY port=(\d+)")
...
class ScratchDashboard:
    ...
    def __enter__(self) -> "ScratchDashboard":
        ...
        env["OPENCODON_HOME"] = str(self.home)
        env["HOME"] = str(self.home.parent) if str(self.home.parent) else env.get("HOME", "")
        env["OPENCODON_HOME"] = str(self.home)
        env["PYTHONPATH"] = str(REPO_ROOT) + os.pathsep + env.get("PYTHONPATH", "")
        env["OPENCODON_ISO_CERTIFY_SYNTH_TURN"] = "1"
        cmd = [
            python, "-m", "opencodon_cli.main", "dashboard",
...
def run_certify(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        with ScratchDashboard(
            ...
            env_extra={"OPENCODON_DASHBOARD_SESSION_TOKEN": token},
        ) as dash:
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--isolation", choices=["on", "off"], default="on",
                   help="set dashboard.turn_isolation in the scratch OPENCODON_HOME")
    ...
    p.add_argument("--keep-home", action="store_true", help="do not delete the scratch OPENCODON_HOME on exit")
