...
# .opencodon-sandbox/ in the worktree git root).
#
# Usage:
...
Options:
  --persistent    Keep the sandbox dir across restarts (under the worktree
                  git root, in .opencodon-sandbox/). Without this flag the
                  sandbox is a temp dir that is removed on exit.
  --from DIR      Copy DIR into the sandbox OPENCODON_HOME as the starting
                  point (config, sessions, skills, etc.).
                  Ignored if the sandbox OPENCODON_HOME already has content
                  (e.g. reusing a --persistent sandbox) to avoid clobbering.
  --delete        Delete the existing persistent sandbox in .opencodon-sandbox.
...
SANDBOX_DIR_NAME="${OPENCODON_DEV_SANDBOX_DIR:-.opencodon-sandbox}"
...
mkdir -p "$OPENCODON_HOME" "$OPENCODON_DESKTOP_USER_DATA_DIR"

if [ -n "$SEED_DIR" ]; then
  # Only seed when the sandbox OPENCODON_HOME is empty — avoids clobbering an
  # existing persistent sandbox on re-run.
  if [ -z "$(ls -A "$OPENCODON_HOME" 2>/dev/null)" ]; then
    echo "[sandbox] seeding OPENCODON_HOME from $SEED_DIR" >&2
    cp -a "$SEED_DIR/." "$OPENCODON_HOME/"
  else
    echo "[sandbox] --from ignored: $OPENCODON_HOME already has content" >&2
  fi
fi

echo "[sandbox] OPENCODON_HOME=$OPENCODON_HOME" >&2
echo "[sandbox] userData=$OPENCODON_DESKTOP_USER_DATA_DIR" >&2
echo "[sandbox] appName=$OPENCODON_DESKTOP_APP_NAME" >&2
if [ "$PERSISTENT" = true ]; then
