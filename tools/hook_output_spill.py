...
Config (``config.yaml``)::
    ...
    hooks:
      output_spill:
        ...
        directory: null        # default: <OPENCODON_HOME>/hook_outputs
...
* Never raises. Any I/O error (disk full, permission denied, missing
  OPENCODON_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: Optional[str], session_id: Optional[str]) -> Path:
    ...
    if directory_override:
        ...
        try:
            ...
            base = Path(get_opencodon_home()) / "hook_outputs"
        except Exception:
            # Last-resort fallback: OPENCODON_HOME env var, then ~/.opencodon
            home = os.environ.get("OPENCODON_HOME") or os.path.expanduser("~/.opencodon")
