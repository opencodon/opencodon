...
One ``CDPSupervisor`` runs per opencodon ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach opencodon logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "opencodon-dialog-bridge.invalid"
...
(() => {
  if (window.__opencodonDialogBridgeInstalled) return;
  window.__opencodonDialogBridgeInstalled = true;
  const ENDPOINT = "http://opencodon-dialog-bridge.invalid/";
