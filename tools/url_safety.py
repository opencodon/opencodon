...
Limitations:
  - DNS rebinding (TOCTOU): an attacker-controlled DNS server with TTL=0
    ...
    connection. opencodon-owned direct httpx request paths should use
...
def normalize_url_for_request(url: str) -> str:
    """Return an ASCII-safe HTTP URL for opencodon-owned URL tools.
...
def _global_allow_private_urls() -> bool:
    ...
    1. ``OPENCODON_ALLOW_PRIVATE_URLS`` env var  (``true``/``1``/``yes``)
    ...
    env_val = os.getenv("OPENCODON_ALLOW_PRIVATE_URLS", "").strip().lower()
...
def is_safe_url(url: str) -> bool:
    ...
    ``OPENCODON_ALLOW_PRIVATE_URLS=true``), private-IP blocking is skipped.
