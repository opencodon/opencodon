...
def _check_kanban_mode() -> bool:
    ...
    1. ``OPENCODON_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if arg:
        return arg
    env_tid = os.environ.get("OPENCODON_KANBAN_TASK")
...
def _stamp_worker_session_metadata(
    ...
    if os.environ.get("OPENCODON_KANBAN_TASK") != task_id:
        return metadata
    session_id = os.environ.get("OPENCODON_SESSION_ID")
    if not session_id:
        return metadata
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``OPENCODON_KANBAN_TASK`` set
    ...
    ``OPENCODON_KANBAN_TASK`` in env) aren't subject to this check — their
...
#   - No-op outside dispatcher-spawned worker context (no ``OPENCODON_KANBAN_TASK``).
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    Identity comes from:
      * ``OPENCODON_KANBAN_TASK`` — task id (required; absence means no-op)
      * ``OPENCODON_KANBAN_RUN_ID`` — pins the run row so we don't heartbeat
        a stale run that may have already been reclaimed
      * ``OPENCODON_KANBAN_CLAIM_LOCK`` — claim lock for ``heartbeat_claim``;
    ...
    tid = os.environ.get("OPENCODON_KANBAN_TASK")
    if not tid:
        return False
    ...
    try:
        kb, conn = _connect()
        try:
            claim_lock = os.environ.get("OPENCODON_KANBAN_CLAIM_LOCK")
            try:
                kb.heartbeat_claim(conn, tid, claimer=claim_lock)
            except Exception:
                logger.debug("auto-heartbeat: heartbeat_claim failed", exc_info=True)
            run_id_raw = os.environ.get("OPENCODON_KANBAN_RUN_ID")
            run_id: Optional[int]
            try:
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set OPENCODON_KANBAN_TASK in the env)"
        )
        ...
        try:
            ...
            if task and task.goal_mode and _goal_judge_available():
                ...
                try:
                    ...
                    # opencodon_cli/goals.py. Unpacking fewer raises ValueError,
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            # OPENCODON_KANBAN_CLAIM_LOCK in the worker env at spawn time
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    author = os.environ.get("OPENCODON_PROFILE") or "worker"
    board = args.get("board")
    try:
        kb, conn = _connect(board=board)
        try:
...
def _handle_create(args: dict, **kw) -> str:
    ...
    tenant = args.get("tenant") or os.environ.get("OPENCODON_TENANT")
    # Stamp the originating session id when the agent loop runs under
    # ACP (which sets OPENCODON_SESSION_ID before invoking tools). NULL on
    # CLI / dashboard paths and on legacy hosts that don't set the env.
    session_id = args.get("session_id") or os.environ.get("OPENCODON_SESSION_ID")
    priority = args.get("priority")
    # Resolve workspace. If the caller passed one explicitly, honor it.
    # Otherwise, a dispatcher-spawned worker (OPENCODON_KANBAN_TASK set)
    ...
    # (kanban toolset, no OPENCODON_KANBAN_TASK) and CLI/dashboard callers
    ...
    try:
        ...
        try:
            ...
            if _inherit_workspace:
                _self_tid = os.environ.get("OPENCODON_KANBAN_TASK")
                if _self_tid:
            ...
            new_tid = kb.create_task(
                ...
                created_by=os.environ.get("OPENCODON_PROFILE") or "worker",
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    - **Gateway** (telegram/discord/slack/etc): ``OPENCODON_SESSION_PLATFORM``
      and ``OPENCODON_SESSION_CHAT_ID`` are set in ContextVars by the
    ...
    - **TUI** (herm desktop / herm TUI): the platform/chat_id ContextVars
      ...
      ``OPENCODON_SESSION_KEY`` from the parent session. We subscribe with
    ...
    try:
        ...
        if not platform or not chat_id:
            ...
            # OPENCODON_SESSION_KEY into the subprocess env. Treat that
            # as a "tui" subscription so the TUI notification poller
            # (tui_gateway/server.py) can pick it up.
            #
            # OPENCODON_SESSION_ID is intentionally NOT a fallback here:
            ...
            # poller keys on OPENCODON_SESSION_KEY.
            session_key = (
                get_session_env("OPENCODON_SESSION_KEY", "")
                or os.environ.get("OPENCODON_SESSION_KEY", "")
            )
            if not session_key:
                return False  # CLI / cron / test — no persistent channel
            platform = "tui"
            chat_id = session_key
        thread_id = get_session_env("OPENCODON_SESSION_THREAD_ID", "") or None
        user_id = get_session_env("OPENCODON_SESSION_USER_ID", "") or None
        notifier_profile = (
            get_session_env("OPENCODON_SESSION_PROFILE", "")
            or os.environ.get("OPENCODON_PROFILE")
        )
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to OPENCODON_KANBAN_TASK from the env "
    "(the task the dispatcher spawned you to work on)."
)

_DESC_BOARD = (
    "Kanban board slug to target. When omitted, the call resolves the "
    "active board the usual way: OPENCODON_KANBAN_DB env → "
    "OPENCODON_KANBAN_BOARD env → the 'current' symlink under the kanban "
        ...
        "properties": {
            ...
            "tenant": {
                ...
                "description": (
                    ...
                    "Defaults to OPENCODON_TENANT env if set."
                ),
            },
            "priority": {
                "type": "integer",
