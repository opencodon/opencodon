...
-----------------------------------------------------------------------------
    ...
    ~/.opencodon/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/opencodon/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_opencodon_home() / "checkpoints"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (opencodon convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("OPENCODON_CHECKPOINT_TIMEOUT", 30)))
...
def _git_env(
    ...
    The shared store is internal opencodon infrastructure — it must NOT inherit
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "opencodon@local"], store, cfg_wd)
    _run_git(["config", "user.name", "opencodon Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``OPENCODON_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at OPENCODON_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "OPENCODON_WORKDIR").write_text(
...
# ---------------------------------------------------------------------------
# Public helpers for `opencodon checkpoints` CLI
# ---------------------------------------------------------------------------
