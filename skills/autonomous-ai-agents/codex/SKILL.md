...
metadata:
  opencodon:
    ...
    related_skills: [claude-code, opencodon]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the opencodon terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For opencodon itself, `model.provider: openai-codex` uses opencodon-managed Codex
OAuth from `~/.opencodon/auth.json` after `opencodon auth add openai-codex`. For the
...
## opencodon Gateway Caveat

When invoking the Codex CLI from a opencodon gateway/service context (for example,
