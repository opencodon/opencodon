...
| Change | Upstream | opencodon |
|--------|----------|--------|
| Metadata namespace | `openclaw` | `opencodon` |
| Trigger | `/baoyu-infographic` slash command | Natural language skill matching |
| User config | EXTEND.md file (project/user/XDG paths) | Removed — not part of opencodon infra |
...
| File operations | Bash commands | opencodon file tools (write_file, read_file) |
...
Reference files can be overwritten directly (they're unchanged from upstream). SKILL.md must be manually merged since it contains opencodon-specific adaptations.
