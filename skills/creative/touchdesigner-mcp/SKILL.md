...
metadata:
  opencodon:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, opencodon-video]
...
```
opencodon -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${OPENCODON_HOME:-$HOME/.opencodon}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to opencodon config (if missing)
...
3. **Restart opencodon session** to pick up the new MCP server
