...
## 5. opencodon Configuration

### Config location

`$OPENCODON_HOME/config.yaml` (defaults to `~/.opencodon/config.yaml` when `OPENCODON_HOME` is unset)
...
Restart the opencodon session for changes to take effect. The MCP connection is
