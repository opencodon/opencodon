...
version: 1.1.1
author: xdevplatform + openclaw + opencodon
license: MIT
...
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by opencodon tool subprocesses; see the Docker note below.
...
> **Docker HOME pitfall:** In the official opencodon Docker layout, `/opt/data` is `OPENCODON_HOME`, but opencodon tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for opencodon-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, opencodon tool calls will not see the credentials.
...
- **Token storage:** `~/.xurl` is YAML. In Docker, use the opencodon subprocess HOME (`/opt/data/home` in the official image) so tokens land under `/opt/data/home/.xurl`. Never read or send this file to LLM context.
...
- opencodon adaptation: reformatted for opencodon skill conventions; safety guardrails preserved verbatim.
