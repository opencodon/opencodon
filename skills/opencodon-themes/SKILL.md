---
name: opencodon-themes
description: "Author a opencodon color theme that skins every surface."
version: 1.0.0
...
# opencodon Themes Skill

Author a opencodon **skin** — one YAML file that themes the CLI, the TUI, and the
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for opencodon itself.
...
- Write access to the opencodon home dir — `~/.opencodon` by default, or `$OPENCODON_HOME`
  / the active profile's dir. Skins live in `<opencodon-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `opencodon config set`).
...
3. `write_file` it to `<opencodon-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `opencodon config set display.skin default` (or they can `/skin default`).
...
```
opencodon skin set <key> <hex>      # e.g. opencodon skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `opencodon skin set background "#08201f"` changes
only the background; `opencodon skin use <name>` / `opencodon skin list` switch and
enumerate.
...
- **Never hand-edit `config.yaml` to activate.** Use `opencodon config set
...
- **You apply it, not the user.** `opencodon config set display.skin <name>` is
...
- `read_file` the written `<opencodon-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `opencodon config get display.skin` and confirm it reports `<name>`.
