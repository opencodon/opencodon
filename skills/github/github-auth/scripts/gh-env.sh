#!/usr/bin/env bash
# GitHub environment detection helper for opencodon skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _opencodon_env="${OPENCODON_HOME:-$HOME/.opencodon}/.env"; [ -f "$_opencodon_env" ] && grep -q "^GITHUB_TOKEN=" "$_opencodon_env" 2>/dev/null; then
