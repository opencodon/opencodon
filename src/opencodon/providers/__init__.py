...
from opencodon.providers.base import OMIT_TEMPERATURE, ProviderProfile  # noqa: F401
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$OPENCODON_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$OPENCODON_HOME/plugins/model-providers/`` if it exists."""
    try:
        from opencodon_constants import get_opencodon_home

        d = get_opencodon_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple OPENCODON_HOME profiles don't alias each other.
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$OPENCODON_HOME/plugins/model-providers/<name>/``
    ...
    # 2. User plugins — under $OPENCODON_HOME/plugins/model-providers/<name>/.
