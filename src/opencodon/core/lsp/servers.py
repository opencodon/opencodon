...
from opencodon.core.lsp.workspace import nearest_root

logger = logging.getLogger("opencodon.core.lsp.servers")
...
def _spawn_pyright(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bin_path is None:
        from opencodon.core.lsp.install import try_install
...
def _find_pses_bundle(ctx: ServerContext) -> Optional[str]:
    ...
    4. ``<OPENCODON_HOME>/lsp/PowerShellEditorServices`` staging dir (where a
    ...
    if bundle is None:
        ...
        if not _PSES_BUNDLE_WARNED:
            ...
            logger.warning(
                ...
                "<OPENCODON_HOME>/lsp/PowerShellEditorServices."
            )
        return None
    start_script = os.path.join(
