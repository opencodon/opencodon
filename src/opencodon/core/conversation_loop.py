...
from opencodon.core.turn_context import (
...
def _apply_active_turn_redirect(agent: Any, messages: List[Dict[str, Any]], text: str) -> None:
    ...
    Preserve only what opencodon actually displayed, demoted to ordinary text,
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for opencodon tool use: "
    ...
    return (
        ...
        f"context, but opencodon needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In opencodon config, set `model.ollama_num_ctx: 65536` "
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `opencodon fallback add`."
)
...
def run_conversation(
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for opencodon internals.
        #
        # opencodon invariant: the system prompt is built ONCE per session
        ...
        if moa_config:
            try:
                from opencodon.core.message_content import flatten_message_text as _flatten_mt
                from opencodon.core.moa_loop import _preset_temperature, aggregate_moa_context
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for opencodon tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
            ...
            try:
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a opencodon/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `opencodon doctor` for credential-chain diagnostics, or")
                    ...
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for opencodon-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: opencodon config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: opencodon config set ANTHROPIC_API_KEY \"\"")
                ...
                # which opencodon' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. opencodon' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`opencodon",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # opencodon may add API-only content not present in persisted
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `opencodon auth` to re-authenticate.", force=True)
                            else:  # xai-oauth
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `opencodon model`.", force=True)
                        else:
                            agent._vprint(f"{agent.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: opencodon setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        opencodon fallback add   (interactive picker — same as `opencodon model`)",
                            force=True,
                        )
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        _policy_response = (
                            ...
                            "(not a opencodon/gateway failure).\n\n"
                ...
                if retry_count >= max_retries:
                    ...
                    from opencodon.core.thinking_timeout_guidance import (
                        is_thinking_timeout,
                    )
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"timeout. (opencodon's built-in floor is 600s for "
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates opencodon mid-turn, resume logic still sees the
                ...
                from opencodon.core.agent_runtime_helpers import (
                ...
                try:
                    from opencodon.core.verification_stop import (
                ...
                try:
                    from opencodon.core.verify_hooks import max_verify_nudges
    ...
    from opencodon.core.turn_finalizer import finalize_turn
