...
def _opencodon_meta(fm: dict[str, Any]) -> dict[str, Any]:
    ...
    return opencodon if isinstance(opencodon, dict) else {}


def _related(fm: dict[str, Any]) -> list[str]:
    raw = fm.get("related_skills") or _opencodon_meta(fm).get("related_skills")
...
def _category(fm: dict[str, Any], skill_md: Path) -> str:
    cat = fm.get("category") or _opencodon_meta(fm).get("category")
    if cat:
        return str(cat)
