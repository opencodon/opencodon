...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_OPENCODON_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, opencodon expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``opencodon_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_opencodon_home()`` themselves.
        """
        if "opencodon_home" not in kwargs:
            from opencodon_constants import get_opencodon_home
            kwargs["opencodon_home"] = str(get_opencodon_home())
        for provider in self._providers:
            try:
