...
- **skills** → the skill name (e.g. ``"debugging-opencodon-desktop"``)
...
shared by the CLI (``opencodon journey delete|edit``), the TUI ``/journey`` overlay
(gateway RPCs), and the desktop GUI (REST). Deleting a skill *archives* it
(recoverable via ``opencodon curator restore``); deleting a memory rewrites its
...
def _memory_local_index(source: str, global_index: int) -> int:
    ...
    """
    from opencodon.core.memory.learning_graph import _memory_cards
...
def _delete_skill(name: str) -> dict[str, Any]:
    ...
    return {"ok": ok, "message": f"archived '{name}' — restore with: opencodon curator restore {name}" if ok else message}
