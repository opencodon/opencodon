...
from opencodon.core.runtime_cwd import resolve_context_cwd
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``OPENCODON_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("OPENCODON_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # Some execution modes (cron) still want OPENCODON_HOME persona while keeping
    ...
    if has_skills_tools:
        ...
        try:
            from opencodon.core.context.coding_context import coding_compact_skill_categories
    ...
    if agent.valid_tool_names:
        try:
            from opencodon.core.context.coding_context import coding_system_blocks
