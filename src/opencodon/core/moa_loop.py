...
MoA-enabled; the normal opencodon agent loop still owns tool calling and turn
...
def _reference_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    conversation flow, but flatten it into clean user/assistant *text* turns:
      ...
      - system prompt: dropped (8K of opencodon boilerplate, not advisory signal).
...
def _preset_temperature(preset: dict[str, Any], key: str) -> float | None:
    ...
    single-model opencodon agent (which never sends temperature unless
...
def aggregate_moa_context(
    *,
    ...
    synth_prompt = (
        ...
        "opencodon agent. Focus on next steps, tool-use strategy, risks, and any "
    ...
    return (
        ...
        "normal opencodon agent loop. You may call tools, continue reasoning, or "
        "finish normally.]\n"
...
class MoAChatCompletions:
    ...
    def consume_and_save_trace(
        ...
        if not pending or "aggregator_input_messages" not in pending:
            return
        try:
            from opencodon.core.moa_trace import save_moa_turn
