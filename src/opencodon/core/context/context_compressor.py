...
from opencodon.core.auxiliary_client import call_llm, _is_connection_error, aux_interrupt_protection
from opencodon.core.context.context_engine import ContextEngine, sanitize_memory_context
...
def _strip_historical_media(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    shape the opencodon compressor emits).
    """
    if not messages:
        return messages
...
class ContextCompressor(ContextEngine):
    ...
    def should_defer_preflight_to_real_usage(self, rough_tokens: int) -> bool:
        ...
        overestimates schema-heavy requests so opencodon compresses before a
...
Summary generation was unavailable, so this is a best-effort deterministic fallback for {len(turns_to_summarize)} compacted message(s).{reason_text}"""
    ...
    def _generate_summary(
        ...
        # user's configured timezone via opencodon_time.now(). The compaction summary
        ...
        try:
            ...
            _today_str = _opencodon_now().strftime("%Y-%m-%d")
...
This compaction should PRIORITISE preserving all information related to the focus topic above. For content related to "{focus_topic}", include full detail — exact values, file paths, command outputs, error messages, and decisions. For content NOT related to the focus topic, summarise more aggressively (brief one-liners or omit if truly irrelevant). The focus topic sections should receive roughly 60-70% of the summary token budget. Even for the focus topic, NEVER preserve API keys, tokens, passwords, or credentials — use [REDACTED]."""
    ...
    def _latest_user_task_snapshot(
        ...
        from opencodon.core.context.conversation_compression import _is_real_user_message
    ...
    def compress(
        ...
        # headless ``opencodon chat -q`` run seeded with a single short
        # prompt followed by nothing but assistant/tool turns — that
        # leaves the compressed transcript with
