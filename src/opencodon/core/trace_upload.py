"""Upload a opencodon session transcript to Hugging Face as an agent trace.

opencodon stores sessions in its own SQLite store (``opencodon_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``opencodon trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through opencodon' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "opencodon-traces"
_OPENCODON_VERSION = "opencodon"
...
# ---------------------------------------------------------------------------
# Conversion: opencodon OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses opencodon' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render opencodon conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.opencodon/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `opencodon trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/opencodon-traces`` dataset. Returns a
