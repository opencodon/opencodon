...
# output items.  opencodon injects xAI's native ``web_search`` for the xAI
...
# server-assigned base64 blobs that can run 400+ chars, while opencodon-minted
...
def _chat_messages_to_responses_input(
    ...
    *,
    ...
    explicitly relies on opencodon threading encrypted reasoning back across
                ...
                if isinstance(codex_reasoning, list):
                    for ri in codex_reasoning:
                        if isinstance(ri, dict) and ri.get("encrypted_content"):
                            ...
                            # it is a opencodon-side metadata key and not part
...
def _preflight_codex_api_kwargs(
    ...
    *,
    ...
    if is_xai_model and normalized.get("tools"):
        try:
            from opencodon.tools.schema_sanitizer import strip_slash_enum
