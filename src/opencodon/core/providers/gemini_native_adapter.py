...
opencodon keeps ``api_mode='chat_completions'`` for the ``gemini`` provider so the
...
Why this exists
---------------
Google's OpenAI-compatible endpoint has been brittle for opencodon's multi-turn
...
from opencodon.core.bounded_response import read_streaming_error_body
from opencodon.core.providers.gemini_schema import sanitize_gemini_tool_parameters
...
def probe_gemini_tier(
    ...
    *,
    ...
    - ``"free"``    -- key is on the free tier (unusable with opencodon)
        ...
        with httpx.Client(timeout=timeout) as client:
            resp = client.post(
                ...
                headers={
                    ...
                    "X-Goog-Api-Client": f"opencodon/{_OPENCODON_VERSION}",
                },
            )
...
_FREE_TIER_GUIDANCE = (
    ...
    "gemini-2.5-flash). opencodon typically makes 3-10 API calls per user turn, "
...
class GeminiAPIError(Exception):
    """Error shape compatible with opencodon retry/error classification."""
...
def build_gemini_request(
    *,
    ...
    if max_tokens is not None:
        ...
        # finishReason=MAX_TOKENS, truncating tool calls mid-stream (opencodon
...
class GeminiNativeClient:
        ...
        **_: Any,
        ...
        if not (api_key or "").strip():
            raise RuntimeError(
                ...
                "Set GOOGLE_API_KEY or GEMINI_API_KEY in your environment / ~/.opencodon/.env "
                "(get one at https://aistudio.google.com/app/apikey), or run `opencodon setup` "
    ...
    def _headers(self) -> Dict[str, str]:
        headers = {
            ...
            "User-Agent": f"opencodon/{_OPENCODON_VERSION} (gemini-native)",
            "X-Goog-Api-Client": f"opencodon/{_OPENCODON_VERSION}",
        }
