...
from opencodon.common.version import __version__ as _OPENCODON_VERSION
...
def _codex_curated_models() -> list[str]:
    ...
    """
    from opencodon.core.providers.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $OPENCODON_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    """
    try:
        from opencodon.core.providers.models_dev import _load_disk_cache
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # endpoint expects (see opencodon_cli/model_setup_flows.py).
    "vertex": [
        "google/gemini-3.1-pro-preview",
        "google/gemini-3-pro-preview",
        "google/gemini-3.5-flash",
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from opencodon.core.providers.model_catalog import get_default_model_from_cache
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from opencodon.core.providers.model_catalog import get_curated_openrouter_models
...
def _fireworks_pricing_from_models_dev(
    *,
    ...
    try:
        from opencodon.core.providers.models_dev import _get_provider_models
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from opencodon.core.credentials.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from opencodon.frontends.cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``opencodon.core.credentials.auth.resolve_provider()`` to resolve it to a concrete
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "bedrock":
        try:
            from opencodon.core.providers.bedrock_adapter import bedrock_model_ids_or_none
...
#
# Cache strategy:
#   - One JSON file at $OPENCODON_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    ...
    return get_opencodon_home() / "provider_models_cache.json"
...
def _save_provider_models_cache(data: dict) -> None:
    ...
    try:
        from opencodon.common.utils import atomic_json_write
        path = _provider_models_cache_path()
...
def _fetch_anthropic_models(
    ...
    *,
    ...
    """
    try:
        from opencodon.core.providers.anthropic_adapter import resolve_anthropic_token, _is_oauth_token
    except ImportError:
        return None
    ...
    if is_oauth:
        ...
        from opencodon.core.providers.anthropic_adapter import _COMMON_BETAS, _OAUTH_ONLY_BETAS, _CONTEXT_1M_BETA
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _OPENCODON_USER_AGENT}
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through our native Anthropic adapter. The live catalog may
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from opencodon_constants import get_opencodon_home
    return get_opencodon_home() / "ollama_cloud_models_cache.json"
...
def validate_requested_model(
    ...
    *,
    ...
    if normalized == "bedrock":
        try:
            from opencodon.core.providers.bedrock_adapter import discover_bedrock_models, resolve_bedrock_region
