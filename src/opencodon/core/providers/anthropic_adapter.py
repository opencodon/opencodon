"""Anthropic Messages API adapter for opencodon.

Translates between opencodon's internal OpenAI-style message format and
...
# opencodon effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    from opencodon.core.providers.azure_identity_adapter import build_bearer_http_client
    ...
    kwargs = {
        ...
        # Delegate retry to opencodon's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to opencodon's outer conversation
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    opencodon notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    opencodon historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``opencodon models``) never mutates ``~/.opencodon/auth.json``
    or makes a network refresh call. Refresh-on-expiry is owned by the API call
    path's pool recovery, not the resolver.
    """
    try:
        from opencodon.core.credentials.credential_pool import AUTH_TYPE_OAUTH, load_pool
    except Exception:
        return None
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by opencodon)
    ...
    # 1. opencodon-managed OAuth/setup token env var
    ...
    # 4. opencodon credential_pool OAuth entry.
    resolved_pool_token = _resolve_anthropic_pool_token()
    if resolved_pool_token:
        return resolved_pool_token

    # 5. Regular API key, or a legacy OAuth token saved in ANTHROPIC_API_KEY.
    # This remains as a compatibility fallback for pre-migration opencodon configs.
...
# ── opencodon-native PKCE OAuth flow ────────────────────────────────────────
...
def _get_opencodon_oauth_file() -> Path:
...
def run_opencodon_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run opencodon-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize opencodon with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from opencodon.core.credentials.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_opencodon_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read opencodon-managed OAuth credentials from ~/.opencodon/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read opencodon OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See opencodon#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See opencodon: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See opencodon#13848 (Kimi) and
    opencodon#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        #      a) bare opencodon-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that opencodon surfaces in its CLI. We
...
def _is_stream_unavailable_error(exc: Exception) -> bool:
    ...
    if "invokemodelwithresponsestream" in err_lower:
        from opencodon.core.providers.bedrock_adapter import is_streaming_access_denied_error
