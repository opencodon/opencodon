...
from opencodon.core.credentials.secret_scope import get_secret as _get_secret
from opencodon.core.credentials.credential_persistence import (
...
# un-throttled log fires on *every* selection. On Windows several opencodon
...
# readiness handshake ("Timed out connecting to opencodon backend after
...
def _write_through_provider_state_to_global_root(
    ...
    (#48415 / #43589): openai-codex and xai-oauth rotate the
    ...
    ``opencodon.core.credentials.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``opencodon model`` / ``opencodon auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another opencodon process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (currently OpenAI Codex and xAI Grok OAuth).
        ...
        here would mean every Codex/xAI refresh in a multi-provider
        setup silently flips the ``active_provider`` flag — the next
        ``opencodon`` invocation that defaults to the active provider
        (e.g. setup wizard, ``opencodon auth status``) would land on
        ...
        # device-code sources (openai-codex, xAI) use ``device_code``.
        ...
        try:
            with _auth_store_lock():
                ...
                # opencodon.core.credentials.auth._save_xai_oauth_tokens (#43589); the pool
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across opencodon
    ...
    def _refresh_entry_impl(
        ...
        try:
            if self.provider == "anthropic":
                from opencodon.core.providers.anthropic_adapter import refresh_anthropic_oauth_pure
                ...
                if entry.source == "claude_code":
                    try:
                        from opencodon.core.providers.anthropic_adapter import _write_claude_code_credentials
                ...
                # refresh_token — single-use tokens consumed by another opencodon
            ...
            if self.provider == "anthropic" and entry.source == "claude_code":
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    return synced
            # For xai-oauth: another process may have
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI — another agent process
            ...
            if self.provider == "openai-codex":
                ...
                # in-memory pool.  Mirrors the xAI quarantine path.
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other opencodon profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `opencodon model` / `opencodon auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `opencodon model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``opencodon auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `opencodon auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    try:
        from opencodon.core.credentials.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, opencodon PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `opencodon setup` ("Claude
        ...
        from opencodon.core.providers.anthropic_adapter import read_claude_code_credentials, read_opencodon_oauth_credentials

        for source_name, creds in (
            ("hermes_pkce", read_opencodon_oauth_credentials()),
        ...
        # opencodon auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from opencodon.core.credentials.auth import resolve_qwen_runtime_credentials
        ...
        # standard `opencodon auth remove minimax-oauth <N>` flow works.
        ...
        try:
            from opencodon.core.credentials.auth import get_provider_auth_state
        ...
        # Respect user suppression — `opencodon auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the opencodon auth store.  Without this gate the removal is instantly
        ...
        # opencodon owns its own Codex auth state — we do NOT auto-import from
        ...
        # via `opencodon auth openai-codex`.
        ...
        # When the user logs in via ``opencodon model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``opencodon auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            # always surfaced as ``device_code`` (consistent with codex).
            source = "device_code"
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # authoritative source for opencodon credentials. Stale env vars from parent
    ...
    def _get_env_prefer_dotenv(key: str) -> str:
        ...
        # load_opencodon_dotenv() -> apply_onepassword_secrets()).  The raw
    ...
    # Honour user suppression — `opencodon auth remove <provider> <N>` for an
    ...
    try:
        from opencodon.core.credentials.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `opencodon auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and opencodon
