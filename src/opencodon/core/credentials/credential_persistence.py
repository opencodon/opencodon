...
``auth.json``.  They intentionally have no dependency on ``opencodon.core.credentials.auth`` so
...
# Sources opencodon owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and opencodon-owned OAuth/device-code state)
