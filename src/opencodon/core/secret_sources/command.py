...
``CommandSecretsProvider`` (opencodon-desktop ``src/main/secrets/commandProvider.ts``)
...
# opencodon.config.env_loader can consume both providers identically.
from opencodon.core.secret_sources.base import ErrorKind, SecretSource
from opencodon.core.secret_sources.bitwarden import FetchResult
...
class CommandSource(SecretSource):
    ...
    Bulk shape: the helper enumerates a KEY=VALUE blob in one run.  Config::
        ...
        secrets:
          command:
            enabled: true
            command: "cat /run/user/1000/opencodon-secrets.env"
    ...
    def remediation(self, kind, cfg: dict) -> str:
        ...
        if kind == ErrorKind.INTERNAL:
            return (
                ...
                "opencodon discards helper stderr so diagnostics can't leak "
                "secret material."
            )
