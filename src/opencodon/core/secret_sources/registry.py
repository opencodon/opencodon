...
def _ensure_builtin_sources() -> None:
    ...
    try:
        from opencodon.core.secret_sources.bitwarden import BitwardenSource
    ...
    try:
        ...
        logger.warning("Failed to register bundled 1Password secret source",
                       exc_info=True)
    try:
        from opencodon.core.secret_sources.command import CommandSource
...
def _fetch_with_timeout(
    ...
    an unbounded hang on every ``opencodon`` invocation.
    """
...
def _active_profile_name(home_path: Optional[Path]) -> str:
    ...
    A named profile's OPENCODON_HOME is ``~/.opencodon/profiles/<name>``; the
    default profile (``~/.opencodon``) returns "".
    """
    if home_path is not None:
        resolved = Path(home_path)
        if resolved.parent.name == "profiles" and resolved.name:
            return resolved.name
    for env_name in ("OPENCODON_PROFILE_NAME", "OPENCODON_PROFILE"):
