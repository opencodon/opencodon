"""opencodon middleware contract helpers.
...
OBSERVER_SCHEMA_VERSION = "opencodon.observer.v1"
MIDDLEWARE_SCHEMA_VERSION = "opencodon.middleware.v1"
...
def apply_llm_request_middleware(
    ...
    **context: Any,
    ...
    provider kwargs before opencodon sends them.
    """
...
def _invoke_middleware(kind: str, **kwargs: Any) -> List[Any]:
    from opencodon.plugins_runtime import invoke_middleware

    return invoke_middleware(kind, **middleware_payload(**kwargs))


def _has_middleware(kind: str) -> bool:
    from opencodon.plugins_runtime import has_middleware
