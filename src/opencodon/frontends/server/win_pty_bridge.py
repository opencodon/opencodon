"""Windows ConPTY bridge for the `opencodon dashboard` chat tab.

Drop-in counterpart to ``opencodon.frontends.server.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``opencodon.frontends.server.web_server`` consumes: ``spawn``, ``read``,
