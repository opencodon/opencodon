...
# xAI's public API is OpenAI-compatible for the endpoints opencodon commonly
# uses. The Responses endpoint is included because opencodon' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via opencodon-managed OAuth credentials."""

    auth_hint = "opencodon auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`opencodon auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`opencodon auth reset xai-oauth` or re-authenticate with "
                    "`opencodon auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `opencodon auth add xai-oauth --type oauth`."
            )
