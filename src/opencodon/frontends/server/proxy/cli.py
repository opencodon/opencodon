"""CLI handlers for the ``opencodon proxy`` subcommand."""
...
from opencodon.frontends.server.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "opencodon proxy requires aiohttp. Run `opencodon setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"opencodon auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting opencodon proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("opencodon proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: opencodon proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``opencodon proxy <subcommand>``."""
    ...
    print(
        "opencodon proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  opencodon proxy start [--provider xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  opencodon proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  opencodon proxy providers\n"
