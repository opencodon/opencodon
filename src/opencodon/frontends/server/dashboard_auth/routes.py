...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``OPENCODON_DASHBOARD_PUBLIC_URL`` env var or
      ...
      2. ``X-Forwarded-Prefix: /opencodon`` (Mission Control deploys) — we
    ...
    from opencodon.frontends.server.dashboard_auth.prefix import (
        prefix_from_request,
        resolve_public_url,
    )
...
def _prefix(request: Request) -> str:
    ...
    ``opencodon.frontends.server.dashboard_auth.prefix`` for the normalisation rules.
    """
...
async def auth_native_authorize(
    ...
    from opencodon.frontends.server.dashboard_auth import native_flow
...
async def api_auth_ws_ticket(request: Request):
    ...
    from opencodon.frontends.server.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket
...
async def auth_native_refresh(request: Request, body: _NativeRefreshBody):
    ...
    from opencodon.frontends.server.dashboard_auth.base import RefreshExpiredError
