...
from opencodon.frontends.server.dashboard_auth import list_session_providers
from opencodon.frontends.server.dashboard_auth.audit import AuditEvent, audit_log
from opencodon.frontends.server.dashboard_auth.base import (
...
from opencodon.frontends.server.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /opencodon``, the
    ``login_url`` is prefixed (``/opencodon/login?next=...``) so the
    ...
    """
    from opencodon.frontends.server.dashboard_auth.prefix import prefix_from_request
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    from opencodon.frontends.server.dashboard_auth.cookies import detect_https
    set_sso_attempt_cookie(
...
async def gated_auth_middleware(
    ...
    # ``opencodon_session_at``. Verify it with the identical ``verify_session``
    ...
    # sends only ``opencodon_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if at:
        ...
        # a self-hosted-OIDC session may hit another provider first, which
        # tries to reach that provider's JWKS; if that's unreachable it raises,
    ...
    if session is None:
        ...
        from opencodon.frontends.server.dashboard_auth.cookies import clear_session_cookies
        from opencodon.frontends.server.dashboard_auth.prefix import prefix_from_request
