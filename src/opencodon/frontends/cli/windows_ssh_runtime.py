...
def process_state(pid: int, creation_time_ns: int, opencodon_path: str, spawn_nonce: str) -> dict[str, Any]:
    ...
    launches_module = (
        argv[1:3] in (["-m", "opencodon_cli.main"], ["-m", "opencodon.frontends.cli.main"])
        or (len(argv) > 2 and argv[1] == "-c" and ("opencodon_cli.main" in argv[2] or "opencodon.frontends.cli.main" in argv[2]))
    )
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    Also resolves opencodon_cli's own location and prepends its parent, because the
    ...
    # the opencodon_cli package parent must be explicit -- the launcher resolves it via
    ...
    if not root or not os.path.isdir(root):
        raise ValueError("could not locate the opencodon_cli package")
    return base, py_path
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    bootstrap = (
        ...
        "runpy.run_module('opencodon_cli.main',run_name='__main__',alter_sys=True)"
    )
