...
# lookup, and opencodon-ink reports Alt as ``key.meta`` on many terminals.
...
def normalize_voice_record_key_for_prompt_toolkit(raw: Any) -> str:
    ...
    * ``super`` / ``win`` / ``windows`` → ``c-b`` (TUI-only modifiers —
    ...
    # in prompt_toolkit (a-c-r form) and opencodon-ink rejects them; collapse
...
from opencodon.tools.voice_mode import (
...
def _debug(msg: str) -> None:
    """Emit a debug breadcrumb when OPENCODON_VOICE_DEBUG=1.
    ...
    """
    if os.environ.get("OPENCODON_VOICE_DEBUG", "").strip() != "1":
        return
    try:
