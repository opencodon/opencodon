...
the existing module-level functions in opencodon_cli.gateway and
opencodon_cli.gateway_windows directly. This protocol is a thin facade
...
``opencodon gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``opencodon gateway start/stop/restart``).
    """
    ...
    # `opencodon gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged opencodon
    ...
    detection always fails. Since every opencodon runtime call inside the
    container drops to opencodon via ``s6-setuidgid``, that silent failure
...
# in ``opencodon_cli.gateway`` (systemd/launchd) and ``opencodon_cli.gateway_windows``
...
# active (set via the ``opencodon -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in opencodon_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from opencodon.frontends.cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from opencodon.frontends.cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from opencodon.frontends.cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from opencodon.frontends.cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in opencodon_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from opencodon.frontends.cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from opencodon.frontends.cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from opencodon.frontends.cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from opencodon.frontends.cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``opencodon_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `opencodon profile create`
# runs inside the container (Phase 4). Static services (main-opencodon, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's OPENCODON_HOME blindly;
    derive the shared profile root from the current OPENCODON_HOME and map the
    ...
    if opencodon_home.parent.name == "profiles":
        root = opencodon_home.parent.parent
    else:
        root = opencodon_home
...
# UID/GID of the in-image ``opencodon`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    (the ``opencodon`` user — UID 10000 — running every opencodon runtime
    ...
    is dispatched as the opencodon user.
    ...
    with opencodon ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           opencodon:opencodon, 0755 (parent must already exist)
    ``svc_dir/event/``                     opencodon:opencodon, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 opencodon:opencodon, 0755
    ``svc_dir/supervise/event/``           opencodon:opencodon, 03730
    ``svc_dir/supervise/control``          opencodon:opencodon, 0660    (FIFO)
    ...
    opencodon user reads them fine.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _OPENCODON_UID, _OPENCODON_GID)
        except PermissionError:
            # Running as the opencodon user already — directory is opencodon-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _OPENCODON_UID, _OPENCODON_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is opencodon-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _OPENCODON_UID, _OPENCODON_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``opencodon -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`opencodon profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-opencodon, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources OPENCODON_HOME (and any extra env) via with-contenv —
             so e.g. ``-e OPENCODON_HOME=/data/opencodon`` is honored at run
          ...
          4. Drops to the opencodon user and exec's
             ``opencodon -p <profile> gateway run`` (or just ``opencodon
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``opencodon gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root OPENCODON_HOME profile" (the implicit profile that exists at
        the top of $OPENCODON_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``opencodon_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$OPENCODON_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``OPENCODON_HOME``,
        ...
        lines = [
            ...
            "cd /opt/data",
            ". /opt/opencodon/.venv/bin/activate",
        ]
        ...
        # guard.
        lines.append("export OPENCODON_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's OPENCODON_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``opencodon gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-OPENCODON_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The OPENCODON_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own OPENCODON_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "opencodon gateway run --replace"
        else:
            gateway_cmd = f"opencodon -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid opencodon {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_finish_script() -> str:
        ...
        normally.  See #51228.
        """
        from opencodon.frontends.gateway.restart import GATEWAY_FATAL_CONFIG_EXIT_CODE
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${OPENCODON_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the OPENCODON_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e OPENCODON_HOME=/data/opencodon``
        gets its logs under /data/opencodon/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``opencodon logs`` reads and what
        ...
        return (
            ...
            f': "${{OPENCODON_HOME:=/opt/data}}"\n'
            f'log_dir="$OPENCODON_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            ...
            # later then runs its log service as opencodon and crash-loops on
            ...
            f'chown opencodon:opencodon "$OPENCODON_HOME/logs/gateways" 2>/dev/null || true\n'
            f'chown -R opencodon:opencodon "$log_dir" 2>/dev/null || true\n'
            f'rm -f "$log_dir/lock"\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid opencodon s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``opencodon gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``opencodon -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with opencodon ownership
            ...
            # the opencodon user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is opencodon-owned (see ``_seed_supervise_skeleton``), and on
