"""
MCP Server Management CLI — ``opencodon mcp`` subcommand.

Implements ``opencodon mcp add/remove/list/test/configure`` for interactive
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from opencodon.frontends.cli.cli_output import prompt as _shared_prompt
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``opencodon mcp login`` to distinguish a genuine authentication
...
def cmd_mcp_add(args):
    ...
    # opencodon_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  opencodon mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  opencodon mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  opencodon mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: opencodon mcp test " + name)
        return
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  opencodon mcp add <name> --url <endpoint>')
        _info('  opencodon mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``opencodon mcp login`` and
    ``opencodon mcp reauth`` so both behave identically for a single server.
    """
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `opencodon mcp remove` + `opencodon mcp add` to reconfigure auth.")
        return False
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `opencodon mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    ``opencodon mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``opencodon mcp reauth --all`` discovers every ``auth: oauth`` server in
    ...
    if not name:
        ...
        _info("Usage: opencodon mcp reauth <name>   |   opencodon mcp reauth --all")
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
# ─── opencodon mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'opencodon mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``opencodon mcp`` subcommands."""
    action = getattr(args, "mcp_action", None)

    if action == "serve":
        from opencodon.frontends.mcp import run_mcp_server
    ...
    if action == "picker":
        from opencodon.frontends.cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from opencodon.frontends.cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from opencodon.frontends.cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `opencodon plugin`.
        from opencodon.frontends.cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("opencodon mcp                                    Open the catalog picker (default)")
        _info("opencodon mcp catalog                            List curated MCPs")
        _info("opencodon mcp install <name>                     Install a catalog MCP")
        _info("opencodon mcp serve                              Run as MCP server")
        _info("opencodon mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("opencodon mcp add <name> --command <cmd>         Add a stdio server")
        _info("opencodon mcp add <name> --preset <preset>       Add from a known preset")
        _info("opencodon mcp remove <name>                      Remove a server")
        _info("opencodon mcp list                               List configured servers")
        _info("opencodon mcp test <name>                        Test connection")
        _info("opencodon mcp configure <name>                   Toggle tools")
        _info("opencodon mcp login <name>                       Re-authenticate OAuth")
        _info("opencodon mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
