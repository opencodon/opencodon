...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"opencodon send: {file_path} is not a text file. --file reads the "
                ...
                f'  opencodon send --to telegram "MEDIA:{file_path}"\n'
                f'  opencodon send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  opencodon send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"opencodon send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"opencodon send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    """
    try:
        from opencodon.frontends.gateway.channel_directory import (
            format_directory_for_display,
            load_directory,
        )
    except Exception as exc:
        print(f"opencodon send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"opencodon send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"opencodon send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `opencodon gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.opencodon/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_opencodon_env() -> None:
    """Populate ``os.environ`` from ``~/.opencodon/.env`` AND bridge top-level
    ...
    startup that ``opencodon send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.opencodon/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.opencodon/config.yaml`` into
       ...
       via ``opencodon config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``opencodon send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.opencodon/.env and ~/.opencodon/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "opencodon send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  opencodon send --to telegram \"hello\"\n"
            "  opencodon send --to discord:#ops --file report.md\n"
            "  opencodon send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "opencodon send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `opencodon send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform opencodon "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.opencodon/.env + ~/.opencodon/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  opencodon send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | opencodon send --to telegram:-1001234567890\n"
            "  opencodon send --to discord:#ops --file /tmp/report.md\n"
            "  opencodon send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  opencodon send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
            "  opencodon send --list                  # all platforms\n"
            "  opencodon send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `opencodon send --list telegram`.",
    )
