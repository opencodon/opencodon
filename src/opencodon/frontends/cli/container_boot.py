...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-opencodon-setup
...
def reconcile_profile_gateways(
    *,
    ...
    is what ``opencodon gateway start`` (no ``-p``) targets. Without it,
    bare ``opencodon gateway start`` inside the container would land on
    ...
    # ``opencodon gateway start`` (no ``-p``) has somewhere to land;
    # auto-up only when the prior state was "running" (same rule as
    # named profiles).
    default_prior_state = _read_desired_state(opencodon_home)
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `opencodon profile create` (config.yaml
            # is not — that comes later via `opencodon setup`). Use it as the
            ...
            # via ``opencodon -p default-named gateway start`` if they
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the opencodon args.

    Two container-command argv shapes are handled:

    * **s6-overlay v2 / tini:** PID 1 argv is
      ``/init /opt/opencodon/docker/main-wrapper.sh <subcommand> [args...]``.
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
      rc.init-launched process as ``/bin/sh -e
      /run/s6/basedir/scripts/rc.init top /opt/opencodon/docker/main-wrapper.sh
    ...
    ``opencodon`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``opencodon <subcommand>``, so an
    explicit leading ``opencodon`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `opencodon <subcommand>`; peel an explicit opencodon.
    if args and Path(args[0]).name == "opencodon":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``opencodon dashboard ...``) never spawns or
    ...
    reintroducing the exact storm this prevents.
    """
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `opencodon -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit opencodon
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the opencodon user) won't hit EACCES. See
