"""Implementation of the ``opencodon bundles`` CLI subcommand.

Mirrors the structure of ``opencodon_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `opencodon bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  opencodon bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``opencodon bundles`` argparse tree.

    Called from ``opencodon_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `opencodon bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``opencodon bundles <subcommand>`` to the right handler."""
