"""
Dump command for opencodon CLI.

Outputs a compact, plain-text summary of the user's opencodon setup
...
def _dotenv_key_names() -> set[str]:
    ...
    exports. ``opencodon debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.opencodon_build_sha`` by the Dockerfile's
...
def _redact(value: str) -> str:
    ...
    ``opencodon dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from opencodon.frontends.cli.gateway import get_gateway_runtime_snapshot
...
def run_dump(args):
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `opencodon auth add openrouter` lives in the
