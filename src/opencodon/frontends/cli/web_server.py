...
#
# This list is defined in ``opencodon_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from opencodon.frontends.cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from opencodon.frontends.cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from opencodon.frontends.cli.dashboard_auth.token_auth import token_auth_middleware
...
# gate + path hardening as /api/fs). Logic lives in ``opencodon_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from opencodon.frontends.cli import web_git as _web_git  # noqa: E402
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from opencodon.frontends.cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
#
# Wraps the same JSON store the CLI uses (opencodon_cli.webhook); the webhook
...
async def list_webhooks():
    import opencodon.frontends.cli.webhook as wh
...
if sys.platform.startswith("win"):
    try:
        from opencodon.frontends.cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from opencodon.frontends.cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from opencodon.frontends.cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from opencodon.frontends.cli.dashboard_auth.audit import AuditEvent, audit_log
        from opencodon.frontends.cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from opencodon.frontends.cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`opencodon_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from opencodon.frontends.cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    from opencodon.frontends.cli import web_app as _web_app
    from opencodon.frontends.cli.web_app import mount_root_spa
...
from opencodon.frontends.cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
from opencodon.frontends.cli.web_app import mount_web_app as _mount_web_app  # noqa: E402
