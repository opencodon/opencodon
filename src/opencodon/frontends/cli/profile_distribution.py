"""Profile distributions — shareable, packaged opencodon profiles via git.

A distribution is a opencodon profile published as a git repository (or
...
* ``opencodon profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``opencodon skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``opencodon profile``, not a parallel tree):

    opencodon profile install <source> [--name N] [--alias] [--force] [--yes]
    opencodon profile update  <name>  [--force-config] [--yes]
    opencodon profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "opencodon.state.db", "response_store.db",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            opencodon_requires=str(data.get("opencodon_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.opencodon_requires:
            out["opencodon_requires"] = self.opencodon_requires
        if self.author:
...
def check_opencodon_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires opencodon {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this opencodon distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``opencodon profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a opencodon profile distribution."
            )
...
def plan_install(
    ...
    from opencodon.frontends.cli import __version__ as opencodon_version
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a opencodon distribution."
        )

    # Version check up-front so we fail fast
    check_opencodon_requires(manifest.opencodon_requires, opencodon_version)
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="opencodon_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `opencodon profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`opencodon profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="opencodon_dist_update_") as tmp:
        plan = plan_install(
