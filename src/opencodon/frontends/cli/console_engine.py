...
from opencodon.common.ansi_strip import strip_ansi as _strip_ansi
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("opencodon.frontends.cli.main")
    builder = getattr(module, builder_name)
...
def _logs(_engine: OpencodonConsoleEngine, args: list[str]) -> str:
    ...
    from opencodon.frontends.cli.logs import list_logs, tail_log
...
def _cron_list(_engine: OpencodonConsoleEngine, args: list[str]) -> str:
    ...
    ns = parser.parse_args(args)
    from opencodon.frontends.cli.cron import cron_list
...
def _cron_status(_engine: OpencodonConsoleEngine, args: list[str]) -> str:
    ...
    from opencodon.frontends.cli.cron import cron_status
