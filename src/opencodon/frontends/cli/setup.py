...
from opencodon.frontends.cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from opencodon.frontends.cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def _install_neutts_deps() -> bool:
    ...
    from opencodon.frontends.cli.tools_config import _pip_install
...
def setup_gateway(config: dict):
    ...
    from opencodon.frontends.cli.gateway import _all_platforms, _platform_status, _configure_platform
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``opencodon.core.credentials.auth`` — lists every supported
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    Used to detect which setup sections can be skipped.
    ``get_env_value`` is the module-level import from opencodon_cli.config
    ...
    if section_key == "model":
        ...
        from opencodon.frontends.cli.gateway import _all_platforms, _platform_status
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    try:
        ...
        from opencodon.frontends.cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
