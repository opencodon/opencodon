...
from opencodon.frontends.acp.auth import TERMINAL_SETUP_AUTH_METHOD_ID, build_auth_methods, detect_provider
from opencodon.frontends.acp.events import (
...
class OpencodonACPAgent(acp.Agent):
    ...
    async def _send_session_info_update(
        ...
        *,
        ...
        # opencodon_state.py schema — only started_at/ended_at). Use "now" as
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="opencodon", version=OPENCODON_VERSION),
    ...
    def _history_summary_meta(message: dict[str, Any], text: str) -> dict[str, Any] | None:
        ...
        Two distinct keys under ``_meta.opencodon`` (ACP's extensibility
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        if conn:
            ...
            try:
                from opencodon.frontends.acp.edit_approval import make_acp_edit_approval_requester
        ...
        # callback shape — not the gateway-queue OPENCODON_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind OPENCODON_SESSION_KEY for this session so per-session caches
            ...
            if edit_approval_requester:
                try:
                    from opencodon.frontends.acp.edit_approval import set_edit_approval_requester
            ...
            previous_session_id = os.environ.get("OPENCODON_SESSION_ID")
            ...
            try:
                ...
                if edit_approval_token is not None:
                    try:
                        from opencodon.frontends.acp.edit_approval import reset_edit_approval_requester
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"opencodon v{OPENCODON_VERSION}"
