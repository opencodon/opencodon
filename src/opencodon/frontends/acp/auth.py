"""ACP auth helpers — detect and advertise opencodon authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "opencodon-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active opencodon runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if opencodon can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for opencodon.
    ...
    not have opencodon provider credentials configured yet, so opencodon always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate opencodon using the currently configured "
