...
def _json_loads_maybe_prefix(value: str) -> Any:
    """Parse a JSON object even when opencodon appended a human hint after it."""
    text = value.strip()
    try:
...
def _build_plan_update_from_todo_result(result: Any) -> AgentPlanUpdate | None:
    """Translate opencodon' todo tool result into ACP's native plan update.

    Zed renders ``sessionUpdate: plan`` as its first-class task/todo panel. The
    opencodon agent already maintains task state through the ``todo`` tool, so the
...
def make_tool_progress_cb(
    ...
    def _tool_progress(event_type: str, name: str = None, preview: str = None, args: Any = None, **kwargs) -> None:
        ...
        if name in {"write_file", "patch"} and edit_approval_policy_getter is not None:
            try:
                from opencodon.frontends.acp.edit_approval import build_edit_proposal, should_auto_approve_edit
