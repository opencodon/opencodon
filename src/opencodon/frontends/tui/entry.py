...
from opencodon.frontends.tui.server import _CRASH_LOG, dispatch, resolve_skin, write_json
from opencodon.frontends.tui.transport import TeeTransport
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `OPENCODON_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("OPENCODON_TUI_SIDECAR_URL")

    if not url:
        return

    from opencodon.frontends.tui.event_publisher import WsPublisherTransport
...
# tested; override via ``OPENCODON_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("OPENCODON_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``OPENCODON_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
    ...
    try:
        from opencodon.frontends.tui.server import _shutdown_sessions
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    if thread is not None and thread.is_alive():
        try:
            from opencodon.frontends.cli.mcp_startup import _resolve_discovery_timeout
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    try:
        from opencodon.frontends.cli.mcp_startup import join_mcp_discovery as _startup_join
