...
Create a plugin directory in `~/.opencodon/plugins/` (or under `plugins/platforms/`
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `opencodon_cli/config.py` so the setup
...
| `send_choice_picker(...)` | Flat single-level picker for finite-choice commands (`/reasoning`, `/fast`). Implemented by Telegram (inline keyboard) and Discord (select menu). Platforms without it fall back to the text status card automatically. |
...
And add it to the `opencodon-gateway` composite:
...
```python
for plat_name in ("telegram", "whatsapp", "your_platform"):
```

---

## 12. Status Display (`opencodon_cli/status.py`)
...
## 13. Gateway Setup Wizard (`opencodon_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ opencodon_cli/ toolsets.py \
  --include="*.py" -l | sort -u
