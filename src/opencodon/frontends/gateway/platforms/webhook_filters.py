...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.opencodon to the active profile home."""
    ...
    if raw.startswith("~/.opencodon/"):
        return opencodon_home / raw.removeprefix("~/.opencodon/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return opencodon_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under OPENCODON_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from opencodon_constants import get_opencodon_home

    scripts_root = (get_opencodon_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.opencodon" or raw_text.startswith("~/.opencodon/"):
