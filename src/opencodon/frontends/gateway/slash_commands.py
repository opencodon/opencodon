...
from opencodon.frontends.gateway.platforms.base import EphemeralReply, MessageEvent, MessageType
from opencodon.frontends.gateway.session import (
    AsyncSessionStore,
    SessionSource,
    build_session_key,
...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    """
    from opencodon.frontends.gateway.code_skew import detect_code_skew
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: opencodon gateway restart"
        ),
    )
...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_whoami_command(self, event: MessageEvent) -> str:
        ...
        """
        from opencodon.frontends.gateway.slash_access import policy_for_source as _policy_for_source
    ...
    async def _handle_status_command(self, event: MessageEvent) -> str:
        ...
        from opencodon.frontends.gateway.run import _AGENT_PENDING_SENTINEL, _load_gateway_config, _resolve_gateway_model
    ...
    def _resume_caller_is_admin(self, source: SessionSource) -> bool:
        ...
        """
        try:
            from opencodon.frontends.gateway.slash_access import policy_for_source
    ...
    async def _handle_agents_command(self, event: MessageEvent) -> str:
        ...
        from opencodon.frontends.gateway.run import _AGENT_PENDING_SENTINEL
    ...
    async def _handle_stop_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        """
        from opencodon.frontends.gateway.run import _AGENT_PENDING_SENTINEL, _INTERRUPT_REASON_STOP
        source = event.source
        session_entry = await self.async_session_store.get_or_create_session(source)
        session_key = session_entry.session_key
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`opencodon gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from opencodon.frontends.gateway.run import _opencodon_home
        ...
        from opencodon.frontends.gateway.restart import is_gateway_supervisor_process
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running opencodon version."""
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from opencodon.frontends.gateway.run import _telegramize_command_mentions
        from opencodon.frontends.cli.commands import gateway_help_lines
        lines = [
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from opencodon.frontends.gateway.run import _opencodon_home, _load_gateway_config
        import yaml
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected_scoped(
                        ...
                        try:
                            from opencodon.frontends.cli.context_switch_guard import (
    ...
    async def _handle_goal_command(self, event: "MessageEvent") -> str:
        ...
        if lower.startswith("draft"):
            ...
            try:
                import asyncio
                from opencodon.frontends.cli.goals import draft_contract
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        from opencodon.frontends.gateway.run import _checkpoint_agent_kwargs, _load_gateway_config
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from opencodon.frontends.gateway.run import _platform_config_key
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        ``opencodon skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from opencodon.frontends.gateway.run import _opencodon_home
        from opencodon.frontends.cli.write_approval_commands import handle_pending_subcommand
        from opencodon.tools import write_approval as wa
        ...
        # the real review surface. (Note: `opencodon skills diff <name>` is a
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from opencodon.frontends.gateway.run import _opencodon_home, _load_gateway_config, _platform_config_key
        ...
        from opencodon.frontends.gateway.display_config import resolve_display_setting
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from opencodon.frontends.gateway.run import _opencodon_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
        from opencodon.frontends.gateway.runtime_footer import resolve_footer_config
        ...
        if new_state:
            ...
            from opencodon.frontends.gateway.runtime_footer import format_runtime_footer
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from opencodon.frontends.cli.session_listing import (
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `opencodon bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``opencodon debug share`` from the CLI.
        """
        import asyncio
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update opencodon to the latest version.

        Spawns ``opencodon update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``opencodon update`` may trigger. Marker
        files are written so either the current gateway process or the next one
        can notify the user when the update finishes.
        """
        from opencodon.frontends.gateway.run import _opencodon_home, _resolve_opencodon_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        ...
        opencodon_cmd = _resolve_opencodon_bin()
        if not opencodon_cmd:
            return t("gateway.update.opencodon_cmd_not_found")
        ...
        # Spawn `opencodon update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `opencodon update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                opencodon_cmd_str = " ".join(shlex.quote(part) for part in opencodon_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {opencodon_cmd_str} update --gateway"
