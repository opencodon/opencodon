...
messages arrive at the same time the old code did:
    ...
    os.environ["OPENCODON_SESSION_THREAD_ID"] = str(context.source.thread_id)
...
``os.getenv("OPENCODON_SESSION_*", ...)`` calls.  Existing tool code only
needs to replace the import + call site:

    # before
    import os
    platform = os.getenv("OPENCODON_SESSION_PLATFORM", "")
...
_SESSION_PLATFORM: ContextVar = ContextVar("OPENCODON_SESSION_PLATFORM", default=_UNSET)
_SESSION_SOURCE: ContextVar = ContextVar("OPENCODON_SESSION_SOURCE", default=_UNSET)
_SESSION_CHAT_ID: ContextVar = ContextVar("OPENCODON_SESSION_CHAT_ID", default=_UNSET)
_SESSION_CHAT_NAME: ContextVar = ContextVar("OPENCODON_SESSION_CHAT_NAME", default=_UNSET)
_SESSION_THREAD_ID: ContextVar = ContextVar("OPENCODON_SESSION_THREAD_ID", default=_UNSET)
_SESSION_USER_ID: ContextVar = ContextVar("OPENCODON_SESSION_USER_ID", default=_UNSET)
_SESSION_USER_NAME: ContextVar = ContextVar("OPENCODON_SESSION_USER_NAME", default=_UNSET)
_SESSION_KEY: ContextVar = ContextVar("OPENCODON_SESSION_KEY", default=_UNSET)
_SESSION_ID: ContextVar = ContextVar("OPENCODON_SESSION_ID", default=_UNSET)
# In-process UI session/window id for multi-session desktop/TUI hosts. This is
# intentionally separate from OPENCODON_SESSION_ID: the latter is the durable
...
_SESSION_UI_SESSION_ID: ContextVar = ContextVar("OPENCODON_UI_SESSION_ID", default=_UNSET)
# ID of the message that triggered the current turn. Used as a reply anchor
# so background-process notifications stay inside the originating Telegram
# private-chat topic (those lanes route only with thread id + reply anchor).
_SESSION_MESSAGE_ID: ContextVar = ContextVar("OPENCODON_SESSION_MESSAGE_ID", default=_UNSET)

_SESSION_PROFILE: ContextVar = ContextVar("OPENCODON_SESSION_PROFILE", default=_UNSET)
...
_SESSION_ASYNC_DELIVERY: ContextVar = ContextVar("OPENCODON_SESSION_ASYNC_DELIVERY", default=_UNSET)

# Cron auto-delivery vars — set per-job in run_job() so concurrent jobs
# don't clobber each other's delivery targets.
_CRON_AUTO_DELIVER_PLATFORM: ContextVar = ContextVar("OPENCODON_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID: ContextVar = ContextVar("OPENCODON_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID: ContextVar = ContextVar("OPENCODON_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)

_VAR_MAP = {
    "OPENCODON_SESSION_PLATFORM": _SESSION_PLATFORM,
    "OPENCODON_SESSION_SOURCE": _SESSION_SOURCE,
    "OPENCODON_SESSION_CHAT_ID": _SESSION_CHAT_ID,
    "OPENCODON_SESSION_CHAT_NAME": _SESSION_CHAT_NAME,
    "OPENCODON_SESSION_THREAD_ID": _SESSION_THREAD_ID,
    "OPENCODON_SESSION_USER_ID": _SESSION_USER_ID,
    "OPENCODON_SESSION_USER_NAME": _SESSION_USER_NAME,
    "OPENCODON_SESSION_KEY": _SESSION_KEY,
    "OPENCODON_SESSION_ID": _SESSION_ID,
    "OPENCODON_UI_SESSION_ID": _SESSION_UI_SESSION_ID,
    "OPENCODON_SESSION_MESSAGE_ID": _SESSION_MESSAGE_ID,
    "OPENCODON_SESSION_PROFILE": _SESSION_PROFILE,
    "OPENCODON_CRON_AUTO_DELIVER_PLATFORM": _CRON_AUTO_DELIVER_PLATFORM,
    "OPENCODON_CRON_AUTO_DELIVER_CHAT_ID": _CRON_AUTO_DELIVER_CHAT_ID,
    "OPENCODON_CRON_AUTO_DELIVER_THREAD_ID": _CRON_AUTO_DELIVER_THREAD_ID,
}


def set_current_session_id(session_id: str) -> None:
    """Synchronize ``OPENCODON_SESSION_ID`` across ContextVar and ``os.environ``.
    ...
    ``get_session_env("OPENCODON_SESSION_ID")`` with an ``os.environ`` fallback,
    so both storage paths must move together when the active session changes.
    """
    import os

    os.environ["OPENCODON_SESSION_ID"] = session_id
...
def set_session_vars(
    ...
    try:
        from opencodon.core.runtime_cwd import set_session_cwd
...
def clear_session_vars(tokens: list) -> None:
    ...
    try:
        from opencodon.core.runtime_cwd import clear_session_cwd
...
def reset_session_vars() -> None:
    ...
    *A's* ``OPENCODON_SESSION_*`` identity via the subprocess-env bridge.  The
    ...
    ``OPENCODON_SESSION_*`` env var read via :func:`get_session_env`), so it is
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session context variable by its legacy ``OPENCODON_SESSION_*`` name.

    Drop-in replacement for ``os.getenv("OPENCODON_SESSION_*", default)``.
