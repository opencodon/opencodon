...
def _get_lock_dir() -> Path:
    ...
    return state_home / "opencodon" / _LOCKS_DIRNAME
...
# Reject epoch values before 2000-01-01T00:00:00Z: nothing in opencodon' lifetime
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the opencodon gateway lifecycle subcommand from a command line.
    ...
    test also matched ``opencodon_cli.main gateway status`` and even unrelated
    ...
    ``--profile``/``-p`` selectors from anywhere in argv -- opencodon's
    ...
    joined = " ".join(tokens)
    has_gateway_entry = (
        "opencodon_cli.main" in joined
        or "opencodon_cli/main.py" in joined
        or "opencodon.frontends.cli.main" in joined
        or "opencodon/frontends/cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("opencodon", "opencodon.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `opencodon gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating opencodon-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the opencodon gateway."""
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``opencodon_cli.gateway._matches_current_profile`` so the dashboard's
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `opencodon status` show per-profile
...
# enabled in the user's systemd (e.g. ``opencodon.service`` + ``opencodon-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``opencodon gateway stop`` on Windows would be misclassified as an
