...
def relay_url() -> Optional[str]:
    ...
    if url:
        return url.rstrip("/")
    try:
        from opencodon.frontends.gateway.run import _load_gateway_config  # late import to avoid cycle
...
def relay_endpoint() -> Optional[str]:
    ...
    ``OPENCODON_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def self_provision_relay() -> bool:
    ...
    up. The creds live ONLY in process memory — never written to ``~/.opencodon/.env``.
    ...
    sets neither ``OPENCODON_MANAGED`` nor a ``.managed`` marker), so gating on it
...
def send_relay_policy() -> bool:
    ...
    try:
        from opencodon.frontends.gateway.relay.auth import make_upgrade_token
...
def register_relay_adapter(force: bool = False, url: Optional[str] = None) -> bool:
    ...
    from opencodon.frontends.gateway.relay.adapter import RelayAdapter
    from opencodon.frontends.gateway.relay.descriptor import CONTRACT_VERSION, CapabilityDescriptor
    ...
    def _factory(config):
        ...
        transport = None
        if resolved_url:
            from opencodon.frontends.gateway.relay.ws_transport import WebSocketRelayTransport
