...
class CronScheduler(ABC):
    ...
    def recover_interrupted(self) -> int:
        ...
        from opencodon.cron.executions import recover_interrupted_executions
    ...
    def fire_due(self, job_id: str, *, adapters: Any = None, loop: Any = None) -> bool:
        ...
        """
        from opencodon.cron.jobs import claim_job_for_fire, get_job
        from opencodon.cron.executions import create_execution
...
def resolve_cron_scheduler() -> "CronScheduler":
    ...
    logger = logging.getLogger("opencodon.cron.scheduler_provider")
...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(self, stop_event, *, adapters=None, loop=None, interval=60, can_dispatch=None):
        import logging
        from opencodon.cron.scheduler import tick as cron_tick
        from opencodon.cron.jobs import record_ticker_heartbeat
        ...
        # Heartbeat once before the first sleep so `opencodon cron status` sees a
