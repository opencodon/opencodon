...
Usage:
    ...
    from opencodon.tools.terminal_tool import _active_environments
...
from opencodon.tools.binary_extensions import BINARY_EXTENSIONS
...
_FENCE_MARKER_RE = re.compile(r"'?\x07?__OPENCODON_FENCE_[A-Za-z0-9]+__\x07?'?")
...
def _strip_terminal_fence_leaks(text: str) -> str:
    ...
    for line in text.splitlines(keepends=True):
        had_terminal_wrapper = "__OPENCODON_FENCE_" in line or "\x1b]" in line
...
def normalize_read_pagination(offset: Any = DEFAULT_READ_OFFSET,
    ...
    """
    from opencodon.tools.tool_output_limits import get_max_lines
...
class ShellFileOperations(FileOperations):
    ...
    def _add_line_numbers(self, content: str, start_line: int = 1) -> str:
        ...
        """
        from opencodon.tools.tool_output_limits import get_max_line_length
    ...
    def _escape_shell_arg(self, arg: str) -> str:
        ...
        """
        from opencodon.tools.environments.local import _bash_safe_path
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        ``.opencodon-tmp`` file next to the user's data, and the original file
        ...
        tmpl = self._escape_shell_arg(".opencodon-tmp.XXXXXX")
        ...
        script = (
            ...
            '|| mktemp "$d/.opencodon-tmp.$$.XXXXXX" 2>/dev/null '
            '|| { tmp="$d/.opencodon-tmp.$$"; : > "$tmp" && echo "$tmp"; })"; '
    ...
    def write_file(self, path: str, content: str) -> WriteResult:
        ...
        # clean the temp up on any failure so we never leak a ``.opencodon-tmp``
    ...
    def patch_v4a(self, patch_content: str) -> PatchResult:
        ...
        """
        # Import patch parser
        from opencodon.tools.patch_parser import parse_v4a_patch, apply_v4a_operations
