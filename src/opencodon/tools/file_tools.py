...
from opencodon.tools.binary_extensions import has_binary_extension
from opencodon.tools.file_operations import (
    ShellFileOperations,
    normalize_read_pagination,
    normalize_search_pagination,
)
from opencodon.tools import file_state
...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``opencodon_constants.get_subprocess_home()`` so that ``~`` resolves
    ...
    if not path or "~" not in path:
        return path
    try:
        from opencodon_constants import get_subprocess_home
...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where opencodon previously hard-rejected an oversized read
...
def _uses_container_paths(task_id: str = "default") -> bool:
    try:
        from opencodon.tools.terminal_tool import _CONTAINER_BACKENDS
...
def _registered_task_cwd_override(task_id: str = "default") -> str | None:
    ...
    """
    try:
        from opencodon.tools.terminal_tool import resolve_task_overrides
...
def _resolve_base_dir(
    ...
    *,
    ...
    from opencodon.tools.environments.local import _msys_to_windows_path
...
_opencodon_config_resolved: str | None = None
_opencodon_config_resolved_loaded = False


def _get_opencodon_config_resolved() -> str | None:
    """Return the resolved absolute path of the opencodon config file (cached)."""
    global _opencodon_config_resolved, _opencodon_config_resolved_loaded
    if _opencodon_config_resolved_loaded:
    ...
    try:
        ...
        try:
            _opencodon_config_resolved = str(Path(_expand_tilde("~/.opencodon/config.yaml")).resolve())
        except Exception:
            _opencodon_config_resolved = None
    return _opencodon_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the opencodon config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    opencodon_config = _get_opencodon_config_resolved()
    if opencodon_config and (resolved == opencodon_config or normalized == opencodon_config):
        return (
            f"Refusing to write to opencodon config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.opencodon/config.yaml directly or use 'opencodon config' instead."
        )
    return None


def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side opencodon mirror prefix for Docker file tools."""
    try:
        from opencodon.tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another opencodon
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of opencodon state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.opencodon/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      opencodon process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.opencodon/…`` path.

    Returns ``None`` when the write is in-scope or outside opencodon scope.
    ...
    # in a session that cd'd into ``~/.opencodon/profiles/other/`` is
    ...
    if cached is not None:
        with _env_lock:
            if task_id in _active_environments:
                ...
                if old_cwd:
                    try:
                        from opencodon.tools.terminal_tool import record_session_cwd
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        from opencodon.tools.read_extract import ExtractionError, extract_document_text, is_extractable_document
        ...
        # ── opencodon internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under OPENCODON_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == OPENCODON_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-opencodon-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-opencodon-profile guard for
    ...
    if mode == "patch" and patch:
        import re as _re
        from opencodon.tools.path_security import has_traversal_component
...
def _check_file_reqs():
    ...
    from opencodon.tools import check_file_requirements
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another opencodon profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another opencodon profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with opencodon_tools.write_file() for very "
            "large files."
        )
