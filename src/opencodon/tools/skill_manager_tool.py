...
~/.opencodon/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def mark_background_review_skill_read(path: Path) -> None:
    ...
    """
    try:
        from opencodon.tools.skill_provenance import is_background_review
...
try:
    from opencodon.tools.skills_guard import scan_skill, should_allow_install, format_scan_report
...
# All skills live in ~/.opencodon/skills/ (single source of truth)
...
def _skills_dir() -> Path:
    ...
    spawned workers) import this module once under the launch OPENCODON_HOME and
    ...
    live profile-scoped OPENCODON_HOME on every call.
    """
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`opencodon curator unpin {name}` if they want to delete it. "
        ...
        if skill_usage.get_record(name).get("pinned"):
            return {
                ...
                "error": (
                    ...
                    f"`opencodon curator unpin {name}` if they want it changed."
                ),
            }
    except Exception:
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.opencodon/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER opencodon profiles.
    ...
    try:
        root = get_default_opencodon_root()
    except Exception:
        return matches
    ...
    # Default profile (~/.opencodon/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.opencodon/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`opencodon -p "
            ...
            base += (
                ...
                f"Switch profiles (`opencodon -p <name>`) to edit there, or "
...
def _resolve_skill_target(skill_dir: Path, file_path: str) -> Tuple[Optional[Path], Optional[str]]:
    ...
    from opencodon.tools.path_security import validate_within_dir
...
def _patch_skill(
    ...
    if match_error:
        ...
        err_msg = match_error
        try:
            from opencodon.tools.fuzzy_match import format_no_match_hint
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    # RECOVERABLE: archival into ~/.opencodon/skills/.archive/ is documented as
    # the maximum destructive action the curator may take, and
    # `opencodon curator restore` promises the skill can be brought back. Route
...
def skill_manage(
    ...
    if result.get("success"):
        ...
        try:
            from opencodon.tools.skill_usage import bump_patch, forget, mark_agent_created
            from opencodon.tools.skill_provenance import is_background_review
            if action == "create":
                ...
                # keeps its usage record as STATE_ARCHIVED so `opencodon curator
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_opencodon_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "will refuse with a message pointing the user to `opencodon curator unpin <name>`. "
