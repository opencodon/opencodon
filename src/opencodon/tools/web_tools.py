...
from plugins.web.firecrawl.provider import (
    Firecrawl,  # noqa: F401  # re-exported for tests that mock.patch("opencodon.tools.web_tools.Firecrawl")
...
from opencodon.tools.url_safety import async_is_safe_url, normalize_url_for_request, sensitive_query_param_name
import sys
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.opencodon/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    # pre-empted by an OAuth token whose tier may not
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        try:
            from opencodon.tools.xai_http import has_xai_credentials
...
if __name__ == "__main__":
    ...
    from opencodon.config import get_env_value as _gev
