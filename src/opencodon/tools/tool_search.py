"""Progressive tool disclosure ("tool search") for opencodon.
...
``tool_call`` — and surfaced on demand. Core opencodon tools never defer.

Design constraints this module is built around (see ``openclaw-tool-search-report``
for the full rationale):

* Core tools defined in ``toolsets._OPENCODON_CORE_TOOLS`` are *never* deferred.
...
logger = logging.getLogger("opencodon.tools.tool_search")
...
def _core_tool_names() -> frozenset[str]:
    ...
    try:
        ...
        return frozenset(_OPENCODON_CORE_TOOLS)
    except Exception:
        return frozenset()
...
def is_deferrable_tool_name(name: str) -> bool:
    ...
    OR it is not in ``_OPENCODON_CORE_TOOLS``. Core tools are never deferred
...
class CatalogEntry:
    ...
    source_name: str  # Toolset name, e.g. "mcp-github" or "browser"
