...
etc.) surface as specific blocked checks via `opencodon computer-use doctor`
...
from opencodon.tools.computer_use.backend import (
    ActionResult,
    CaptureResult,
    ComputerUseBackend,
    UIElement,
...
def _action_result_from(
    ...
    *,
    ...
    cua-driver's mcp-tool-notes and upstream#67052.
    """
...
def _cua_no_overlay() -> bool:
    """True when opencodon should pass ``--no-overlay`` to cua-driver.
...
def _cua_telemetry_disabled() -> bool:
    """True when opencodon should disable cua-driver telemetry for this user.
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    Surface 8 of upstream#47072: instead of hardcoding
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    opencodon Desktop/TUI session can otherwise filter out the `computer_use`
    tool even though `opencodon computer-use doctor` succeeds from a login shell.
    """
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `opencodon computer-use install --upgrade`."
    )
...
def cua_driver_install_hint() -> str:
    ...
    return (
        ...
        "  opencodon computer-use install\n"
        "Or run the upstream installer directly:\n"
        f"{installer}\n"
        "Or run `opencodon tools` and enable the Computer Use toolset to install it automatically."
    )
...
def _parse_elements_from_structured(raw_elements: List[Dict[str, Any]]) -> List[UIElement]:
    """Surface 2 of upstream#47072: read the canonical
...
class _CuaDriverSession:
    ...
    def __init__(self, bridge: _AsyncBridge) -> None:
        ...
        self._started = False
        # Surface 4 of upstream#47072: per-tool
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize opencodon-managed secrets out of the child env.
                env=_sanitize_subprocess_env(cua_driver_child_env()),
            )
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        if not self._ready_event.wait(timeout=30.0):
            ...
            raise RuntimeError(
                ...
                "Run `opencodon computer-use doctor` and check "
...
def _extract_tool_result(mcp_result: Any) -> Dict[str, Any]:
    ...
    upstream#47072). Each entry corresponds index-for-index
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self) -> None:
        ...
        # Surface 6 of upstream#47072: per-snapshot
        ...
        #   - Gets a distinct agent-cursor color per opencodon run, with
        ...
        #     concurrent opencodon runs / subagents don't step on each
        #     other.
        # We mint a UUID4-based id once per CuaDriverBackend instance —
        # one opencodon run = one backend = one session — and pass it as
        ...
        self._session_id: str = f"opencodon-{uuid.uuid4().hex[:12]}"

    # ── Lifecycle ──────────────────────────────────────────────────
    def start(self) -> None:
        _maybe_nudge_update()
        # The MCP client SDK (`mcp`) is an optional dependency (the
        # `computer-use` / `mcp` extras), not part of opencodon' minimal core.
    ...
    def capture(
        ...
        Maps opencodon `capture(mode, app)` → cua-driver `list_windows` +
        `get_window_state` (ax/som) or `screenshot` (vision).
        """
        # Step 1: enumerate on-screen windows to find target pid/window_id.
        # Surface 3 of upstream#47072: read the canonical
        ...
        if mode == "vision":
            ...
            # Surface 2 of upstream#47072: prefer the
    ...
    def _apply_delivery(
        ...
        to proceed. See upstream#67052 phase B.
        """
    ...
    def click(
        ...
        *,
        ...
        # upstream#47072). cua-driver-rs gained an explicit
