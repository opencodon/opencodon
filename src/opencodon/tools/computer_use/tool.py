...
`health_report` (surfaced by `opencodon computer-use doctor`) reports the
...
# regardless of approval level (e.g. logout kills the session opencodon runs in).
_BLOCKED_KEY_COMBOS = {
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "If the cua-driver binary is missing, run `opencodon computer-use install`. "
...
def _request_approval(action: str, args: Dict[str, Any],
    ...
    silently authorize it (upstream#67052).
...
def _capture_response(cap: CaptureResult, max_elements: int = _DEFAULT_MAX_ELEMENTS) -> Any:
    ...
    if cap.png_b64 and cap.mode != "ax" and not image_too_small:
        ...
        # parts (Surface 7 of upstream#47072 — trycua/cua#1961
...
def _should_route_through_aux_vision() -> bool:
    ...
    try:
        ...
        from opencodon.tools.computer_use.vision_routing import (
...
def _route_capture_through_aux_vision(
    ...
    try:
        ...
        if _mime_for_ext == "image/jpeg" or (not _mime_for_ext and cap.png_b64[:8].startswith("/9j/")):
            ...
            ext = ".png"
        cache_dir = get_opencodon_dir("cache/vision", "temp_vision_images")
...
def check_computer_use_requirements() -> bool:
    ...
    `opencodon computer-use doctor` if their session is incomplete (e.g. no
    DISPLAY set).
    """
    if sys.platform not in ("darwin", "win32", "linux"):
        return False
    from opencodon.tools.computer_use.cua_backend import cua_driver_binary_available
