...
supports multiple backends — **Browser Use** (cloud, default for
...
Usage:
    from opencodon.tools.browser_tool import browser_navigate, browser_snapshot, browser_click
...
# means a compromised transitive dependency could read every opencodon secret
...
def _build_browser_env() -> dict:
    ...
    Strips opencodon-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``opencodon_subprocess_env`` import is deferred to keep ``browser_tool``
...
from opencodon.tools.tool_backend_helpers import normalize_browser_cloud_provider
# Camofox local anti-detection browser backend (optional).
# When CAMOFOX_URL is set, all browser operations route through the
# camofox REST API instead of the agent-browser CLI.
try:
    from opencodon.tools.browser_camofox import is_camofox_mode as _is_camofox_mode
except ImportError:
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    opencodon_node_bin = str(opencodon_home / "node" / "bin")
    opencodon_node_root = str(opencodon_home / "node")
    opencodon_nm_bin = str(opencodon_home / "node_modules" / ".bin")
    return [opencodon_node_bin, opencodon_node_root, opencodon_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _get_dialog_policy_config() -> Tuple[str, float]:
    ...
    from opencodon.tools.browser_supervisor import (
...
def _ensure_cdp_supervisor(task_id: str) -> None:
    ...
    if not cdp_url:
        return
    try:
        from opencodon.tools.browser_supervisor import SUPERVISOR_REGISTRY  # type: ignore[import-not-found]
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    fallback. If unset, fall back to Browser Use (
    ...
    if resolved is None:
        # Auto-detect path: Browser Use first (
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when opencodon silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the opencodon CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-opencodon_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed opencodon processes — this way every clean opencodon exit sweeps
    ...
    # Sweep orphans from other crashed opencodon processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live opencodon processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current opencodon PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live opencodon process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning opencodon process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced opencodon PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent opencodon instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-opencodon_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live opencodon process.
            continue
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, opencodon-managed
    ...
    # node_modules binary, which disappears on the next ``opencodon update`` and
    ...
    # Build an extended search PATH including opencodon-managed Node, macOS
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this opencodon PID as the session owner (cross-process safe
...
def browser_navigate(url: str, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from opencodon.tools.browser_camofox import camofox_navigate
...
def browser_snapshot(
    ...
    """
    if _is_camofox_mode():
        from opencodon.tools.browser_camofox import camofox_snapshot
...
def browser_click(ref: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from opencodon.tools.browser_camofox import camofox_click
...
def browser_type(ref: str, text: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from opencodon.tools.browser_camofox import camofox_type
...
def browser_scroll(direction: str, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from opencodon.tools.browser_camofox import camofox_scroll
...
def browser_back(task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from opencodon.tools.browser_camofox import camofox_back
...
def browser_press(key: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from opencodon.tools.browser_camofox import camofox_press
...
def browser_console(clear: bool = False, expression: Optional[str] = None, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from opencodon.tools.browser_camofox import camofox_console
...
def _camofox_current_page_private_url(tab_id: str, user_id: str) -> Optional[str]:
    ...
    """
    try:
        from opencodon.tools.browser_camofox import _post
...
def _camofox_eval(expression: str, task_id: Optional[str] = None) -> str:
    ...
    from opencodon.tools.browser_camofox import _ensure_tab, _post
    try:
...
def browser_get_images(task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from opencodon.tools.browser_camofox import camofox_get_images
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise opencodon
    ...
    """
    if _is_camofox_mode():
        from opencodon.tools.browser_camofox import camofox_vision
        return camofox_vision(question, annotate, task_id)

    import base64
    import uuid as uuid_mod
    from opencodon_constants import get_opencodon_dir
    screenshots_dir = get_opencodon_dir("cache/screenshots", "browser_screenshots")
    ...
    try:
        ...
        from opencodon.tools.vision_tools import (
...
def _cleanup_single_browser_session(task_id: str) -> None:
    ...
    if _is_camofox_mode():
        try:
            from opencodon.tools.browser_camofox import camofox_close, camofox_soft_cleanup
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/opencodon/opencodon:latest")
                else:
