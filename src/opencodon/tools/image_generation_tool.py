...
selectable via ``opencodon tools`` → Image Generation; the active model is
...
def _load_fal_client() -> Any:
    ...
    if fal_client is not None:
        return fal_client
    from opencodon.tools.fal_common import import_fal_client
    fal_client = import_fal_client()
    return fal_client


from opencodon.tools.debug_helpers import DebugSession
from opencodon.tools.fal_common import _extract_http_status  # noqa: F401 — re-exported for tests
...
def _active_terminal_env(task_id: str | None):
    try:
        from opencodon.tools.terminal_tool import get_active_env
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    ...
    # opencodon cache roots can be translated without side effects. SSH can still
...
def _agent_visible_cache_path(host_path: str, env: Any) -> str | None:
    ...
    try:
        from opencodon.tools.credential_files import map_cache_path_to_container
    ...
    try:
        ...
        if source_images and not edit_endpoint:
            raise ValueError(
                ...
                f"via `opencodon tools` → Image Generation."
            )
...
def _build_no_backend_setup_message() -> str:
    ...
    Used by the in-tree FAL path. Mentions the FAL_KEY signup link and a
    pointer to the plugin registry, so users on a stale ``image_gen.provider``
    know other backends exist and how to inspect them.
    """
    ...
    lines.append(
        "  2. Configure a different image_gen provider via `opencodon tools` "
        "→ Image Generation (run `opencodon plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `opencodon plugins list` to see "
        ...
        if "image_url" in kwargs or "reference_image_urls" in kwargs:
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    f"supports editing via `opencodon tools` → Image Generation."
                ),
...
# model/provider via `opencodon tools` or `/skills`.
