"""Central registry for all opencodon tools.
...
# timescales. Cache results for ~30 s so env-var flips via ``opencodon tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``opencodon tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def _plugin_owner_of(self, handler: Callable) -> Optional[str]:
        ...
        if isinstance(mod, str) and mod.startswith("opencodon_plugins."):
            return mod
        return None
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``opencodon_plugins.{name}``), not the exact module string.
                # A handler defined in ``opencodon_plugins.pkg.handlers`` is
                # still owned by the ``opencodon_plugins.pkg`` package — exact
                ...
                if (
                    caller_mod.startswith("opencodon_plugins.")
                    and not same_plugin
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``opencodon tools enable foo``)
    ...
    def dispatch(self, name: str, args: dict, **kwargs) -> str | dict:
        ...
        * All exceptions are caught and returned as ``{"error": "..."}``
        ...
        try:
            ...
            try:
                from opencodon.tools.model_tools import _sanitize_tool_error
    ...
    def get_max_result_size(self, name: str, default: int | float | None = None) -> int | float:
        ...
        if default is not None:
            return default
        from opencodon.tools.budget_config import DEFAULT_RESULT_SIZE_CHARS
