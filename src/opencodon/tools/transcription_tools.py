...
LOCAL_STT_COMMAND_ENV = "OPENCODON_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "OPENCODON_LOCAL_STT_LANGUAGE"
...
# DeepInfra STT base URL now resolved via opencodon.core.models.deepinfra_base_url (shared).
...
#
# The single-env-var ``OPENCODON_LOCAL_STT_COMMAND`` escape hatch is preserved
...
def _render_command_stt_template(
    ...
    def replace_match(match: "re.Match[str]") -> str:
        ...
        token = f"__OPENCODON_STT_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set OPENCODON_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _transcribe_xai(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    language = str(
        ...
        or os.getenv("OPENCODON_LOCAL_STT_LANGUAGE")
        or DEFAULT_LOCAL_STT_LANGUAGE
    ).strip()
...
def _transcribe_deepinfra(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    ``opencodon.core.models`` helpers so every DeepInfra surface resolves the
