"""Base class for all opencodon execution environment backends.
...
# OPENCODON_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("OPENCODON_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {OPENCODON_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_opencodon_home() / "sandboxes"
...
def _cwd_marker(session_id: str) -> str:
    return f"__OPENCODON_CWD_{session_id}__"
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all opencodon backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/opencodon-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/opencodon-cwd-{self._session_id}.txt"
    ...
    def init_session(self):
        ...
        bootstrap = (
            ...
            f"__opencodon_fns=$(declare -F | awk '{{print $3}}' | grep -vE '^_[^_]') || true\n"
            f"[ -n \"$__opencodon_fns\" ] && declare -f $__opencodon_fns "
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        parts.append("__opencodon_ec=$?")
        # Restrict opencodon metadata files without changing the user's command
        ...
        parts.append("exit $__opencodon_ec")
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"OPENCODON_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(
        ...
        # --- Debug tracing (opt-in via OPENCODON_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __OPENCODON_CWD_{session}__ marker from stdout output.
    ...
    def execute(
        ...
        *,
        ...
        if rewrite_compound_background:
            from opencodon.tools.terminal_tool import _rewrite_compound_background
    ...
    def _prepare_command(self, command: str) -> tuple[str, str | None]:
        ...
        from opencodon.tools.terminal_tool import _transform_sudo_command
