...
from opencodon.tools.environments.base import _file_mtime_key
...
_FORCE_SYNC_ENV = "OPENCODON_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.opencodon") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.opencodon to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.opencodon", container_base, 1
        )
...
def _credential_host_paths() -> set[str]:
    ...
    try:
        from opencodon.tools.credential_files import get_credential_file_mounts
    except Exception:
        return set()
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``OPENCODON_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, opencodon_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.opencodon/`` directory as a tar archive,
        ...
        # against an uninitialized remote .opencodon/ directory.
        if not self._pushed_hashes and not self._synced_files:
            logger.debug("sync_back: no prior push state — skipping")
            return

        lock_path = (opencodon_home or get_opencodon_home()) / ".sync.lock"
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="opencodon-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.opencodon/skills/a.md`` →
        ``~/.opencodon/skills/a.md``, a new remote file at
        ``/root/.opencodon/skills/b.md`` maps to ``~/.opencodon/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
