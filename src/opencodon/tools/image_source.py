...
def _media_cache_roots() -> list:
    """Agent-managed media cache directories under OPENCODON_HOME (host side).
...
def _permitted_host_read_target(p: Path, ctx: ResolveContext) -> Optional[Path]:
    ...
    - Non-local backend: permitted only if the path resolves inside a media
      cache root. A container-visible cache path (e.g. ``/root/.opencodon/cache/
    ...
    from opencodon.tools.credential_files import from_agent_visible_cache_path
...
def _finalize(data: bytes, declared_mime: str, origin: str, src: str) -> ResolvedImage:
    ...
    """
    from opencodon.tools.vision_tools import _detect_image_mime_type_from_bytes
