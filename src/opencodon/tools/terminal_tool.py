...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or opencodon exit
...
# ---------------------------------------------------------------------------
from opencodon.tools.interrupt import is_interrupted, _interrupt_event  # noqa: F401 — re-exported
# display_opencodon_home imported lazily at call site (stale-module safety during opencodon update)
...
from opencodon.tools.environments.singularity import _get_scratch_dir
from opencodon.tools.tool_backend_helpers import (
    coerce_modal_mode,
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of opencodon directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "opencodon-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = os.getenv("OPENCODON_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("OPENCODON_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (OPENCODON_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (OPENCODON_INTERACTIVE=1 or a registered sudo password callback):
    ...
    # interactive opencodon password prompt or the sudo -S password-pipe path.
    ...
    # silently without opencodon prompting.
    if not has_configured_password and not sudo_password and _sudo_nopasswd_works():
        return command, None

    has_sudo_prompt_callback = _get_sudo_password_callback() is not None
    should_prompt_for_sudo = (
        env_var_enabled("OPENCODON_INTERACTIVE") or has_sudo_prompt_callback
    )
...
from opencodon.tools.environments.local import LocalEnvironment as _LocalEnvironment
from opencodon.tools.environments.singularity import SingularityEnvironment as _SingularityEnvironment
from opencodon.tools.environments.ssh import SSHEnvironment as _SSHEnvironment
from opencodon.tools.environments.docker import DockerEnvironment as _DockerEnvironment
from opencodon.tools.environments.modal import ModalEnvironment as _ModalEnvironment
import sys
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so opencodon can track lifecycle and output.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``opencodon=1`` for the current
    profile that match the issue #20561 leak class — containers left behind
    by opencodon processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      opencodon processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling opencodon processes a generous grace
    ...
    try:
        from opencodon.tools.environments.docker import (
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, OpencodonSweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter: Any = int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.opencodon/.env or environment variables."
        )
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    expand itself, so the opencodon host/container must NOT ``expanduser`` it.

    SSH ``cwd`` is interpreted by the *remote* shell (``cd ~`` / ``cd ~/x``
    over ``ssh ... bash -c``). Expanding ``~`` locally would rewrite it to the
    opencodon host HOME (often ``/opt/data`` under Docker) and inject a
...
def _ensure_terminal_env_bridged() -> None:
    ...
    vars at startup — but processes that skip all of those paths (``opencodon
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        # Startup orphan reaper for opencodon-tagged containers left behind by
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior opencodon processes that hit SIGKILL / OOM / a closed terminal
        ...
        if modal_state["selected_backend"] != "direct":
            raise ValueError(
                "Modal backend selected but no direct Modal "
                "credentials/config were found. Set MODAL_TOKEN_ID and "
        ...
        from opencodon.tools.environments.daytona import DaytonaEnvironment as _DaytonaEnvironment
...
def _cleanup_inactive_envs(lifetime_seconds: int = 300):
    ...
    for task_id, env in envs_to_stop:
        ...
        try:
            from opencodon.tools.file_tools import clear_file_ops_cache
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so opencodon can track the process, then run "
...
def terminal_tool(
    ...
    try:
        ...
        # Hard-block: gateway lifecycle commands (systemctl/launchctl/opencodon
        # restart|stop targeting opencodon-gateway) must never run inside the
        ...
        # never restart. This mirrors the `opencodon gateway restart` guard in
            ...
            if _contains_gateway_lifecycle_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Run `opencodon gateway restart` from a separate shell outside "
                        "the running gateway."
                    ),
                    "status": "error",
                }, ensure_ascii=False)
        ...
        if background:
            ...
            try:
                ...
                # opencodon dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in opencodon dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if not _async_ok():
                        ...
                        result_data["notify_unsupported"] = (
                            ...
                            "the turn ends (a one-shot runner such as `opencodon -z` or a "
                        ...
                        _gw_platform = _gse("OPENCODON_SESSION_PLATFORM", "")
                        if _gw_platform:
                            _gw_chat_id = _gse("OPENCODON_SESSION_CHAT_ID", "")
                            _gw_thread_id = _gse("OPENCODON_SESSION_THREAD_ID", "")
                            _gw_user_id = _gse("OPENCODON_SESSION_USER_ID", "")
                            _gw_user_name = _gse("OPENCODON_SESSION_USER_NAME", "")
                            _gw_message_id = _gse("OPENCODON_SESSION_MESSAGE_ID", "")
            ...
            if _approved_run:
                from opencodon.tools.interrupt import clear_current_thread_interrupt
            ...
            if sudo_cache_cleared:
                has_sudo_prompt_callback = _get_sudo_password_callback() is not None
                if has_sudo_prompt_callback or env_var_enabled("OPENCODON_INTERACTIVE"):
                    output += (
            ...
            from opencodon.tools.tool_output_limits import get_max_bytes
...
def check_terminal_requirements() -> bool:
    ...
    try:
        ...
        if env_type == "local":
            ...
            from opencodon.tools.environments.docker import find_docker
            docker = find_docker()
            if not docker:
            ...
            if modal_state["selected_backend"] != "direct":
                logger.error(
                    ...
                    "were found. Set MODAL_TOKEN_ID and MODAL_TOKEN_SECRET (or run "
                    "`modal token new`), or choose a different TERMINAL_ENV."
                )
                return False
