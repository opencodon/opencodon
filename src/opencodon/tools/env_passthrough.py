...
def _is_opencodon_provider_credential(name: str) -> bool:
    """True if ``name`` is a opencodon-managed provider credential (API key,
    ...
    Non-opencodon API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a opencodon credential into the execute_code child.
    """
    try:
        ...
        return True
    # Dynamically-generated opencodon-internal secrets (AUXILIARY_*_API_KEY /
    ...
    if _is_opencodon_internal_secret(name):
        return True
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are opencodon-managed provider credentials (from
    ...
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a opencodon-managed
    ...
    Non-opencodon third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_opencodon_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register opencodon provider "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # opencodon-managed provider credentials must not be passed
                ...
                if _is_opencodon_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register opencodon "
