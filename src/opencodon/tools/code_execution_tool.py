...
Lets the LLM write a Python script that calls opencodon tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `opencodon_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `opencodon_tools.py` with file-based RPC stubs
...
# available on every platform opencodon itself runs on.
...
# match a safe prefix, the operational OPENCODON_ allowlist, or (on Windows) an
# OS-essential name.
#
...
# Operational OPENCODON_* vars the child legitimately needs by exact name — these
...
_OPENCODON_CHILD_ALLOWED = frozenset({
    "OPENCODON_HOME",
    "OPENCODON_PROFILE",
    "OPENCODON_CONFIG",
    "OPENCODON_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational OPENCODON_* vars (_OPENCODON_CHILD_ALLOWED) pass by exact name.
    ...
    if is_passthrough is None:
        try:
            from opencodon.tools.env_passthrough import is_env_passthrough as _ep
        except Exception:
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _OPENCODON_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("OPENCODON_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped OPENCODON_* var.
            _dropped_opencodon.append(k)
    if _dropped_opencodon:
        logger.debug(
            "execute_code: dropped %d non-allowlisted OPENCODON_* var(s) from the "
            ...
            len(_dropped_opencodon),
            ", ".join(sorted(_dropped_opencodon)),
        )
    return scrubbed
...
# ---------------------------------------------------------------------------
# opencodon_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-opencodon-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-opencodon-profile soft guard."""',
...
def generate_opencodon_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the opencodon_tools.py stub module.
...
"""Auto-generated opencodon tools RPC stubs."""
...
def _connect():
    ...
    OPENCODON_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["OPENCODON_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("OPENCODON_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
"""Auto-generated opencodon tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("OPENCODON_RPC_DIR") or os.path.join(tempfile.gettempdir(), "opencodon_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated opencodon_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/opencodon_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_opencodon_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/opencodon_tools.py", tools_src)
        ...
        env_prefix = (
            f"OPENCODON_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"OPENCODON_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("OPENCODON_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    to a subset of opencodon tools.
    ...
    from opencodon.tools.terminal_tool import _get_env_config, _docker_has_host_access
    ...
    from opencodon.tools.approval import check_execute_code_guard
    ...
    # --- Set up temp directory with opencodon_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="opencodon_sandbox_")
    ...
    # selector from OPENCODON_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"opencodon_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated opencodon_tools module.
        ...
        tools_src = generate_opencodon_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "opencodon_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  OPENCODON_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["OPENCODON_RPC_SOCKET"] = rpc_endpoint
        child_env["OPENCODON_RPC_TOKEN"] = rpc_token
        ...
        # OPENCODON_TIMEZONE is an internal opencodon setting and must not leak
        # into child processes.
        _tz_name = os.getenv("OPENCODON_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("OPENCODON_TIMEZONE", None)
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.opencodon/.env')).
...
def _load_config() -> dict:
    ...
    ``opencodon --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (opencodon's python). Reproducible and the
...
def _resolve_child_cwd(mode: str, staging_dir: str, task_id: str = "") -> str:
    ...
    if task_id:
        ...
        try:
            from opencodon.tools.terminal_tool import get_session_cwd
        ...
        try:
            from opencodon.tools.file_tools import _registered_task_cwd_override
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``opencodon tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and opencodon's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.opencodon/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call opencodon tools programmatically. "
        ...
        f"Available via `from opencodon_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into opencodon_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from opencodon_tools import {import_str}` "
