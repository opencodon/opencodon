#!/usr/bin/env python3
"""Reveal/focus a pane in the opencodon desktop GUI.

Gated on ``OPENCODON_DESKTOP`` (like the other GUI affordances). Emits
...
def focus_pane_tool(pane: str) -> str:
    ...
    if not ok:
        return tool_error("Pane focus is only available in the opencodon desktop app.")
...
FOCUS_PANE_SCHEMA = {
    "name": "focus_pane",
    "description": (
        "Reveal and focus a pane in the opencodon desktop app when the user asks to "
