...
# Most platforms read their home channel from "<PLATFORM>_HOME_CHANNEL"; keep
# the override map for any future divergent platforms.
...
def _media_caption_split(text, media_files, *, max_caption_len):
    ...
    across every standalone sender. ``opencodon send`` (and the send_message
    ...
    ``opencodon send --to whatsapp "MEDIA:/x.png This Caption"`` into two parts.
...
SEND_MESSAGE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "target": {
                "type": "string",
                "description": "Delivery target. Format: 'platform' (uses home channel), 'platform:#channel-name', 'platform:chat_id', or 'platform:chat_id:thread_id' for Telegram topics and Discord threads. Examples: 'telegram', 'telegram:-1001234567890:17585', 'discord:999888777:555444333', 'discord:#bot-home', 'slack:#engineering'"
            },
            "message": {
                "type": "string",
...
def _handle_react(args, remove=False):
    ...
    tapbacks). Requires the gateway to be running in this
    ...
    if target_ref:
        ...
        if not chat_id:
            try:
                ...
                resolved = None
            # Opaque platform-native ids (e.g. space GUIDs like
    ...
    try:
        from opencodon.tools.model_tools import _run_async
        if remove:
            result = _run_async(
    ...
    if not chat_id:
        ...
        if home:
            ...
            return json.dumps({
                ...
                f"or set a home channel via: opencodon config set {home_env} <channel_id>"
            })
...
def _get_cron_auto_delivery_target():
    ...
    if not platform or not chat_id:
        return None
    thread_id = get_session_env("OPENCODON_CRON_AUTO_DELIVER_THREAD_ID", "").strip() or None
    return {
        "platform": platform,
        "chat_id": chat_id,
        "thread_id": thread_id,
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None, force_document=False):
    ...
    if media_files and not message.strip():
        return {
            "error": (
                f"send_message MEDIA delivery is currently only supported for telegram, discord, whatsapp and slack; "
    ...
    if media_files:
        warning = (
            ...
            "native send_message media delivery is currently only supported for telegram, discord, whatsapp and slack"
        )
