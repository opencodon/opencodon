#!/usr/bin/env python3
"""Close a read-only agent terminal tab in the opencodon desktop GUI.
...
``read_terminal`` it is gated on ``OPENCODON_DESKTOP`` so it never appears outside
the GUI.
"""
...
CLOSE_TERMINAL_SCHEMA = {
    ...
    "description": (
        ...
        "the opencodon desktop GUI (the tabs mirroring terminal(background=true) runs). "
