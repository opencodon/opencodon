...
from opencodon.tools.registry import discover_builtin_tools, registry
...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_opencodon_home() under the active profile.
...
#   - cli.py, opencodon_cli/*      -> inline on startup (no event loop)
...
def _compute_tool_definitions(
    ...
    # This ensures that even if a composite toolset (like opencodon-cli)
    ...
    if "execute_code" in available_tool_names:
        from opencodon.tools.code_execution_tool import SANDBOX_ALLOWED_TOOLS, build_execute_code_schema, _get_execution_mode
    ...
    for discord_tool_name in _discord_schema_fns:
        if discord_tool_name in available_tool_names:
            try:
                from opencodon.tools import discord_tool as _dt
    ...
    try:
        from opencodon.tools.schema_sanitizer import sanitize_tool_schemas
    ...
    try:
        from opencodon.tools.tool_search import assemble_tool_defs, load_config as _load_ts_config
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from opencodon.plugins_runtime import has_hook, invoke_hook
...
def handle_function_call(
    ...
    _ts_mod = None
    try:
        from opencodon.tools import tool_search as _ts_mod  # noqa: F401
    except Exception:
        _ts_mod = None
    ...
    try:
        ...
        if function_name not in _READ_SEARCH_TOOLS:
            try:
                from opencodon.tools.file_tools import notify_other_tool_call
