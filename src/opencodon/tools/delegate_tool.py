...
# Must match opencodon.core.providers.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
_RUNTIME_PROVIDER_CUSTOM = "custom"
from opencodon.tools import file_state
from opencodon.tools.terminal_tool import set_approval_callback as _set_subagent_approval_cb
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``opencodon-cli`` (which bundles
...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``opencodon-cli`` because those also contain
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs elsewhere).
    """
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. opencodon-cli) so that individual
    ...
    # Blocked tools also live inside mixed platform bundles (opencodon-cli,
    # opencodon-telegram, etc.) that _strip_blocked_tools must keep because they
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.opencodon/logs/subagent-<sid>-<ts>.log``
...
def _run_single_child(
    ...
    **_kwargs,
    ...
    try:
        ...
        try:
            from opencodon.tools.terminal_tool import get_session_cwd, record_session_cwd
...
def delegate_task(
    ...
    from opencodon.tools.delegation_live_log import (
    ...
    from opencodon.tools import model_tools as _model_tools
    ...
    if background:
        from opencodon.tools.async_delegation import dispatch_async_delegation_batch
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and OPENCODON_SESSION_KEY is not in the CLI
...
def _resolve_child_credential_pool(
    ...
    if effective_provider == "custom":
        try:
            from opencodon.core.credentials.credential_pool import get_custom_provider_pool_key, load_pool
