...
This module provides the glue:
    - ``OpencodonTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "opencodon"
"""
...
from opencodon_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__opencodon_user_skipped__"
...
def _get_token_dir(opencodon_home: str | Path | None = None) -> Path:
    """Return the directory for MCP OAuth token files.

    Uses OPENCODON_HOME so each profile gets its own OAuth tokens.
    Layout: ``OPENCODON_HOME/mcp-tokens/``
    """
    try:
        from opencodon_constants import get_opencodon_home
        base = Path(opencodon_home) if opencodon_home is not None else Path(get_opencodon_home())
    except ImportError:
        base = Path(os.environ.get("OPENCODON_HOME", str(Path.home() / ".opencodon")))
...
def _cached_redirect_port(storage: "OpencodonTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If opencodon restarts and chooses a
...
def _cached_redirect_uri(storage: "OpencodonTokenStorage | None") -> str | None:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``opencodon mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `opencodon mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# OpencodonTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class OpencodonTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        OPENCODON_HOME/mcp-tokens/<server_name>.json         -- tokens
        OPENCODON_HOME/mcp-tokens/<server_name>.client.json   -- client info
        OPENCODON_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """

    def __init__(self, server_name: str, *, opencodon_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._opencodon_home = Path(opencodon_home) if opencodon_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._opencodon_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._opencodon_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._opencodon_home) / f"{self._server_name}.meta.json"
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # opencodon records an absolute wall-clock ``expires_at`` alongside the
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._opencodon_home)
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to opencodon.</p></body></html>"
            ) if code else (
...
def _make_redirect_handler(port: int, redirect_uri: str | None = None):
    ...
    async def _redirect_handler(authorization_url: str) -> None:
        ...
        """
        from opencodon.tools.mcp_dashboard_oauth import get_dashboard_oauth_flow
        ...
        if on_ssh and redirect_uri:
            ...
            print(
                ...
                f"\n"
                f"  See: https://github.com/opencodon/opencodon/tree/main/docs\n",
                file=sys.stderr,
            )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `opencodon mcp login <server>` later to "
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = OpencodonTokenStorage(server_name, opencodon_home=opencodon_home)
    storage.remove()
...
def _configure_callback_port(
    cfg: dict,
    storage: "OpencodonTokenStorage | None" = None,
) -> int:
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "opencodon")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    storage = OpencodonTokenStorage(server_name)
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`opencodon mcp login {server_name}` interactively first to complete "
