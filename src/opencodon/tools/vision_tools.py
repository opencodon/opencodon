...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("OPENCODON_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
...
def _resolve_vision_cpu_workers() -> int:
    ...
    Resolution order: OPENCODON_VISION_MAX_CONCURRENCY env →
    ...
    """
    env_val = os.getenv("OPENCODON_VISION_MAX_CONCURRENCY", "").strip()
    if env_val:
        try:
            parsed = int(env_val)
            if parsed >= 1:
...
async def _validate_image_url_async(url: str) -> bool:
    ...
    if not _image_url_shape_ok(url):
        return False
    from opencodon.tools.url_safety import async_is_safe_url
...
async def _download_image(image_url: str, destination: Path, max_retries: int = 3) -> Path:
    ...
    async def _ssrf_redirect_guard(response):
        ...
        """
        from opencodon.tools.url_safety import async_is_safe_url, redirect_target_from_response
...
def _resize_image_for_vision(image_path: Path, mime_type: Optional[str] = None,
    ...
    try:
        ...
        try:
            from opencodon.tools.lazy_deps import ensure as _ensure_dep
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $OPENCODON_HOME/cache/vision/ and cleaned up
