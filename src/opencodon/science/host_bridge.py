...
class HostBridge:
    ...
    def _llm_client(self, model: Optional[str]):
        from opencodon.science.hostinfra import get_llm_client

        client, default_model = get_llm_client("science_llm")
        if client is None:
            raise RuntimeError(
    ...
    def _do_tool(self, params: dict):
        ...
        from opencodon.science.hostinfra import dispatch_tool

        raw = dispatch_tool(name, dict(args), task_id=None)
        try:
            data = json.loads(raw)
...
def _configured_model(key: str) -> Optional[str]:
    ...
    try:
        from opencodon.science.hostinfra import get_task_config

        value = get_task_config("science_llm").get(key)
    except Exception:
        return None
...
def _provider_is_anthropic() -> bool:
    ...
    try:
        from opencodon.science.hostinfra import resolve_task_provider_model

        provider, _model, base_url, _key, _mode = resolve_task_provider_model(
            "science_llm"
        )
    except Exception:
        return False
