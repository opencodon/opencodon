...
def kernels_available() -> bool:
    ...
    if kernels_installed():
        return True
    try:
        from opencodon.science.hostinfra import allow_lazy_installs

        return allow_lazy_installs()
    except Exception:
        # Includes LookupError from an unregistered seam: without the tools
        # layer wired up there is nothing that could lazy-install anyway.
        return False
...
def ensure_kernels() -> None:
    ...
    if kernels_installed():
        return
    from opencodon.science.hostinfra import ensure_deps

    ensure_deps("tool.science", prompt=False)
