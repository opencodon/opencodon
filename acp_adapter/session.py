"""ACP session manager — maps ACP sessions to opencodon AIAgent instances.
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when opencodon itself is running in WSL.

    Windows ACP clients can launch ``opencodon acp`` inside WSL while still sending
...
def _normalize_cwd_for_compare(cwd: str | None) -> str:
    ...
    from opencodon_constants import windows_path_to_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch opencodon from a Windows workspace while the ACP process runs
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed opencodon agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by opencodon AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current opencodon runtime provider configuration.
    ...
    def _restore(self, session_id: str) -> Optional[SessionState]:
        ...
        # for the rest of the session (see opencodon_state.get_messages_as_conversation).
        try:
    ...
    def _make_agent(
        ...
        *,
        ...
        # Stamp the ACP workspace onto the agent so the runtime starts from
        # the editor/session cwd instead of the opencodon daemon's process cwd.
