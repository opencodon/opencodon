...
This is an additive opencodon extension surfaced under ACP ``_meta.opencodon`` so
...
compression rotates the internal opencodon head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    Args:
        ...
        current_opencodon_session_id: The live internal opencodon DB session id
            (``state.agent.session_id``).
        previous_opencodon_session_id: The internal id from before the most recent
            turn, when known. Supplied by ``prompt()`` to flag a rotation.

    Returns:
        A dict suitable for ``{"opencodon": {"sessionProvenance": <dict>}}`` under
        ACP ``_meta``, or ``None`` if the session can't be read.
    """
    try:
        row = db.get_session(current_opencodon_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_opencodon_session_id
    compression_depth = 0
    cursor_parent = parent_id
    seen = {current_opencodon_session_id}
    ...
    rotated = bool(
        ...
        and previous_opencodon_session_id != current_opencodon_session_id
    )

    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id,
        "currentOpencodonSessionId": current_opencodon_session_id,
        "rootOpencodonSessionId": root_id,
        "parentOpencodonSessionId": parent_id,
        "sessionKind": "continuation" if is_continuation else "root",
        "compressionDepth": compression_depth,
    }
    if previous_opencodon_session_id:
        provenance["previousOpencodonSessionId"] = previous_opencodon_session_id
    if rotated:
...
def session_provenance_meta(
    ...
    *,
    ...
    """Return a ready ``_meta`` payload: ``{"opencodon": {"sessionProvenance": ...}}``."""
    ...
    if prov is None:
        return None
    return {"opencodon": {"sessionProvenance": prov}}
