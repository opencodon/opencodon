...
Defenses 1 and 2 (the ``_OPENCODON_GATEWAY`` guard on ``opencodon gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.opencodon.gateway")``,
...
State lives in ``<OPENCODON_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_opencodon_home() / "gateway" / "restart_loop.json"
