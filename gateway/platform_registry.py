...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``opencodon setup`` display).
    ...
    # built-ins).  Used by ``opencodon gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `opencodon`
        # invocation -- including plain `opencodon chat`, which never touches any
        ...
        # `opencodon setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``opencodon setup``/``gateway status``, channel
