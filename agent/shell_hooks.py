...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # opencodon-canonical
...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    ...
    registering anything.  Used by ``opencodon hooks list`` and ``doctor``."""
    if not isinstance(cfg, dict):
        return []
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a opencodon wire-shape dict.
    ...
    if event == "pre_verify":
        # "continue" (opencodon) / "block" (Claude-Code Stop: block the stop) both
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ opencodon is about to register a shell hook that will run a\n"
...
# ---------------------------------------------------------------------------
# Introspection (used by `opencodon hooks` CLI)
# ---------------------------------------------------------------------------
...
def run_once(
    ...
    holding the canonical opencodon-wire-shape response."""
