...
The version is sourced live from ``opencodon_cli.__version__`` so it auto-aligns
...
``opencodon_cli.__version__`` is the canonical source of truth.
"""
...
def _hermes_version() -> str:
    ...
    Falls back to ``"unknown"`` if ``opencodon_cli`` cannot be imported (should
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    """
    tags = ["product=hermes-agent", opencodon_client_tag()]
