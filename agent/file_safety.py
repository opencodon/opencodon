...
def _opencodon_root_path() -> Path:
    """Resolve the opencodon root dir (always the parent of any profile, never per-profile)."""
    try:
        from opencodon_constants import get_default_opencodon_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    opencodon_root = _opencodon_root_path()
    return {
        os.path.realpath(p)
        for p in [
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    opencodon_dirs = []
    for base in (_opencodon_home_path(), _opencodon_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in opencodon_dirs:
                opencodon_dirs.append(real)
        except Exception:
            continue

    for base_real in opencodon_dirs:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied opencodon path.

    Three categories are blocked:

      * Internal opencodon cache files under ``OPENCODON_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under OPENCODON_HOME and the global opencodon
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    # opencodon root so credential stores at <root>/auth.json etc. are also
    ...
    for hd in opencodon_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal opencodon cache file "
    ...
    for hd in opencodon_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a opencodon credential store "
    ...
    for hd in opencodon_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the opencodon MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a opencodon MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied opencodon read (see
...
#
# opencodon profiles are separate OPENCODON_HOME dirs under
...
#
# Reference: May 2026 incident where a opencodon-security profile session
# edited skills under both ``~/.opencodon/profiles/opencodon-security/skills/``
...
def _resolve_active_profile_name() -> str:
    ...
    try:
        ...
        root_real = _opencodon_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside opencodon scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    opencodon entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to opencodon profile {info['target_profile']!r}, but the "
...
# regardless of which opencodon profile is active. It does NOT cover the
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    ...
    index where the inner opencodon-state portion starts. Returns ``None`` for
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative opencodon state.
    ...
    Detection is path-shape-only — does not require any opencodon resolver to
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host opencodon process never "
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative opencodon state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host opencodon "
