...
Owns one Codex thread per opencodon session. Drives `turn/start`, consumes
...
# opencodon' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_OPENCODON_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse opencodon/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per opencodon session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _OPENCODON_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("OPENCODON_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="opencodon",
            client_title="opencodon",
            client_version=_get_opencodon_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but opencodon' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into opencodon' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own opencodon-tools callback we
            # auto-accept — the user already approved opencodon' tools
            ...
            if server_name in ("opencodon-tools", "opencodon-tools"):
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map opencodon approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    opencodon returns 'once', 'session', 'always', or 'deny'.
...
def _get_opencodon_version() -> str:
    """Best-effort opencodon version string for codex's userAgent line."""
    try:
