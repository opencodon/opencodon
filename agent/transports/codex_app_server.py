...
projection into opencodon' display, approval bridging, transcript projection into
...
"codex_app_server"`. opencodon' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 opencodon secret — gateway
        ...
        spawn_env = opencodon_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("OPENCODON_KANBAN_TASK"):
            kanban_db = spawn_env.get("OPENCODON_KANBAN_DB")
            kanban_root = (
                os.path.dirname(kanban_db)
                if kanban_db
                else spawn_env.get(
                    "OPENCODON_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("OPENCODON_HOME", os.path.expanduser("~/.opencodon")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "opencodon",
        client_title: str = "opencodon",
