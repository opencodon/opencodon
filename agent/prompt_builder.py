...
from opencodon_constants import get_opencodon_home, get_skills_dir, is_wsl
...
_OPENCODON_MD_NAMES = (".opencodon.md", "OPENCODON.md")


def _find_opencodon_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.opencodon.md`` or ``OPENCODON.md``.
    ...
    # could pick up a .opencodon.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _OPENCODON_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are opencodon, an open-science AI agent. "
...
OPENCODON_AGENT_HELP_GUIDANCE = (
    "You run on opencodon (a hard fork of Nous Research's hermes-agent). When "
    "the user needs help with opencodon itself — configuring, setting up, "
    "using, extending, or troubleshooting it — or when you need to understand "
    "your own features, tools, or capabilities, the project README and docs at "
    "https://github.com/opencodon/opencodon are your authoritative reference "
    "and always hold the latest, most up-to-date information."
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.opencodon/kanban.db`. Your task id is in "
    "`$OPENCODON_KANBAN_TASK`; your workspace is `$OPENCODON_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $OPENCODON_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $OPENCODON_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${OPENCODON_KANBAN_BRANCH:-wt/$OPENCODON_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$OPENCODON_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`opencodon profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `opencodon kanban <verb>` for board operations. Use "
...
#
# The opencodon runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to opencodon's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `opencodon computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that opencodon "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the opencodon terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the opencodon desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the opencodon WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where opencodon itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host opencodon runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where opencodon itself is running. The host OS, home, and cwd "
                f"of the opencodon process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where opencodon "
    ...
    # Embedder-supplied environment description. Lets a host that wraps opencodon
...
def _skills_prompt_snapshot_path() -> Path:
    return get_opencodon_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("OPENCODON_PLATFORM") or os.environ.get("OPENCODON_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.opencodon/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot opencodon itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — use the real `opencodon` CLI "
            "commands (e.g. `opencodon config set …`, `opencodon tools`, `opencodon setup`) "
            "rather than guessing or inventing workarounds.\n"
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from OPENCODON_HOME and return its content, or None.
    ...
    soul_path = get_opencodon_home() / "SOUL.md"
...
def _load_opencodon_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.opencodon.md / OPENCODON.md — walk to git root."""
    opencodon_md_path = _find_opencodon_md(cwd_path)
    if not opencodon_md_path:
        return ""
    try:
        content = opencodon_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = opencodon_md_path.name
        try:
            rel = str(opencodon_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".opencodon.md", context_length=context_length,
            read_path=str(opencodon_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", opencodon_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .opencodon.md / OPENCODON.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from OPENCODON_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the opencodon install/source
    ...
    # explicitly configured cwd is honored verbatim — the opencodon tree is a
    ...
    # their launch dir IS the user's shell cwd (developing opencodon in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the opencodon install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_opencodon_md(cwd_path, context_length)
    ...
    # SOUL.md from OPENCODON_HOME only — skip when already loaded as identity
    if not skip_soul:
