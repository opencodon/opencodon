...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.opencodon/.env  → clear it
    ...
    """
    from opencodon_cli.config import get_env_path, remove_env_value
...
def _remove_hermes_pkce(provider: str, removed) -> RemovalResult:
    """~/.opencodon/.anthropic_oauth.json is ours — delete it outright."""
    from opencodon_constants import get_opencodon_home

    result = RemovalResult()
    oauth_file = get_opencodon_home() / ".anthropic_oauth.json"
