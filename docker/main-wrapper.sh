...
# shellcheck shell=sh
# /opt/opencodon/docker/main-wrapper.sh — wraps the container's CMD with
...
# from the Dockerfile never reaches `opencodon`. with-contenv repopulates
...
#
# Routing:
#   no args                       → exec `opencodon` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `opencodon <args>` (subcommand passthrough)
#
# Drop to opencodon via s6-setuidgid, but skip it when already non-root.
set -e

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid opencodon "$@"; exec "$@"; }
...
# an arbitrary non-root, non-opencodon UID, the bootstrap was skipped and the
# baked image dirs (owned by the opencodon build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u opencodon)" ]; then
    cat >&2 <<EOF
[opencodon] ERROR: container started with --user $cur_uid (an arbitrary, non-opencodon UID) — not supported.
...
# to the opencodon user's home before dropping privileges so libraries that
...
_opencodon_orig_cwd="${OPENCODON_ORIG_CWD:-$PWD}"
...
# the user's command so `opencodon chat` starts in the Docker -w
# directory, not /opt/data.
cd "$_opencodon_orig_cwd"
...
# opencodon subcommand pass-through.
drop opencodon "$@"
