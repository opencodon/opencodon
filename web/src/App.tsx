...
const SIDEBAR_COLLAPSED_KEY = "opencodon-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="opencodon-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="opencodon-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateOpencodon = status?.can_update_opencodon === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'opencodon update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "opencodon update";
    return (
      t.status.updateOpencodonConfirmMessage ??
      `This will run 'opencodon update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateOpencodonConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateOpencodon) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateOpencodon,
      runningLabel: t.status.updatingOpencodon,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Opencodon gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateOpencodonConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateOpencodonConfirmTitle ?? `${t.status.updateOpencodon}?`}
    />
    </>
  );
}
