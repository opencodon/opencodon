...
function readBasePath(): string {
  ...
  const raw = window.__OPENCODON_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const OPENCODON_BASE_PATH = readBasePath();
const BASE = OPENCODON_BASE_PATH;
...
const SESSION_HEADER = "X-Opencodon-Session-Token";
...
export async function fetchJSON<T>(
  ...
  if (res.status === 401) {
    ...
    // (``opencodon update``, ``opencodon gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__OPENCODON_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__OPENCODON_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("opencodon.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("opencodon.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__OPENCODON_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("opencodon.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
 ...
 *
 * The dashboard's gated-mode WS auth (``opencodon_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__OPENCODON_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Opencodon-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``opencodon_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__OPENCODON_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  return buildOpencodonWebSocketUrl({
...
export const api = {
  ...
  updateOpencodon: () =>
    fetchJSON<ActionResponse>("/api/opencodon/update", { method: "POST" }),
  checkOpencodonUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/opencodon/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** opencodon-index only: whether the centralized index loaded. */
  available?: boolean;
}
