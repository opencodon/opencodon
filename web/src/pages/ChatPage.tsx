/**
 * ChatPage — embeds `opencodon --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (opencodon_cli/web_server.py)                          .
 *          ▼                                                              .
 *     POSIX PTY → `node apps/tui/dist/entry.js` → tui_gateway + AIAgent     .
 */
...
const PTY_ATTACH_TOKEN_KEY = "opencodon.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__OPENCODON_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `opencodon dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      // even when the inner Opencodon TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[opencodon-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // forwarding them into Opencodon. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="opencodon-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
