...
Auto-tracks and cleans up ephemeral files created during opencodon
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$OPENCODON_HOME` and `/tmp/opencodon-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `OPENCODON_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under OPENCODON_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `OPENCODON_HOME` or `/tmp/opencodon-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$OPENCODON_HOME/disk-cleanup/` is itself excluded
- `$OPENCODON_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
