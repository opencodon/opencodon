...
same precedence convention as the other auth plugins)::
    ...
    OPENCODON_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
    OPENCODON_DASHBOARD_OIDC_CLIENT_SECRET # optional; set for a confidential client
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``OPENCODON_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "OPENCODON_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "OPENCODON_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("OPENCODON_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    # Optional — set only for a confidential client. A credential, so the
    # canonical home is the env var / ~/.opencodon/.env; config.yaml is supported
    # for precedence symmetry. Empty ⇒ public client (unchanged behaviour).
    client_secret = _resolve_setting(
        "OPENCODON_DASHBOARD_OIDC_CLIENT_SECRET", oidc_cfg.get("client_secret")
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(OPENCODON_DASHBOARD_OIDC_ISSUER + OPENCODON_DASHBOARD_OIDC_CLIENT_ID) "
