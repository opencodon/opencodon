...
Config: Uses the existing Honcho config chain:
  1. $OPENCODON_HOME/honcho.json (profile-scoped)
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, opencodon_home):
        """Write config to $OPENCODON_HOME/honcho.json (Honcho SDK native format)."""
        import json
        import os
        from pathlib import Path
        config_path = Path(opencodon_home) / "honcho.json"
        existing = {}
    ...
    def post_setup(self, opencodon_home: str, config: dict) -> None:
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        try:
            if not session.messages and cfg.session_strategy != "per-session":
                ...
                mem_dir = str(get_opencodon_home() / "memories")
