...
# trigger its own recovery path. Refs: upstream#59614
...
# _UPDATER_STOP_TIMEOUT. Refs: upstream#66377
_DRAIN_TIMEOUT = 15.0
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = env_float("OPENCODON_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", 0.8)
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "OPENCODON_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "OPENCODON_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. opencodon-created private-chat topic lanes are marked with
    ...
    # older PTB/clients. Streaming edits stay on opencodon' existing MarkdownV2
    ...
    def _has_telegram_desktop_details_math_crash_shape(self, content: str) -> bool:
        ...
        opencodon must skip rich delivery up front and use the legacy MarkdownV2
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `opencodon update` or `--replace` handoffs)
        ...
        message = (
            ...
            "To recover: ensure no other opencodon or OpenClaw instance is running "
            "with this token, then restart the gateway with 'opencodon gateway restart'."
    ...
    async def _run_post_connect_housekeeping(self) -> None:
        ...
        try:
            ...
            try:
                ...
                # payload size limit (~4KB total).  opencodon defaults to 60 to
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # readable by the opencodon process (shared mount, same machine, etc.).
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("OPENCODON_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("OPENCODON_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("OPENCODON_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("OPENCODON_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("OPENCODON_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            }
            ...
            disable_fallback = (os.getenv("OPENCODON_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in {"1", "true", "yes", "on"})
            ...
            _max_connect = 8
            _init_timeout = _env_float("OPENCODON_TELEGRAM_INIT_TIMEOUT", 30.0)
                ...
                try:
                    ...
                    if _attempt < _max_connect - 1:
                        ...
                        raise OSError(
                            ...
                            f"or set OPENCODON_TELEGRAM_HTTP_CONNECT_TIMEOUT to a lower value."
                        )
    ...
    def _build_provider_keyboard(self, providers: list, page: int = 0) -> tuple:
        ...
        CLI ``opencodon model`` picker via the shared ``group_providers`` fold,
        so all surfaces stay consistent.
        """
        try:
            from opencodon.core.models import group_providers
        except Exception:
    ...
    async def _handle_model_picker_callback(
        ...
        if data.startswith("mp:"):
            ...
            group_id = data[4:]
            try:
                from opencodon.core.models import PROVIDER_GROUPS
    ...
    # Scripts live in ~/.opencodon/scripts/gmail-triage/. `arg` from the callback
    ...
    async def _handle_gmail_triage_callback(
        ...
        *,
        ...
        script_path = _Path.home() / ".opencodon" / "scripts" / "gmail-triage" / script_name
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@opencodon_bot.example" are not mentions
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several opencodon bot profiles. A message like
    ...
    def _is_own_message(self, message: Message) -> bool:
        ...
        counted as incoming unread messages in the opencodon inbox.
        """
        if not self._bot:
            return False
    ...
    def _should_process_message(self, message: Message, *, is_command: bool = False) -> bool:
        ...
        # unread in the opencodon inbox (#52363).
        if self._is_own_message(message):
            return False
...
# opencodon_cli/{setup,gateway}.py, and the _send_telegram dispatch in
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `opencodon setup` to install Telegram support.",
