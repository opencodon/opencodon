...
``{title, url, description, position}`` rows every other opencodon web
provider produces.
...
prefers opencodon-managed xAI Grok OAuth (via ``opencodon auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.opencodon/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``opencodon tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `opencodon auth` to sign in with "
