...
``opencodon_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
from opencodon_cli import kanban_db
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    WS auth gate (``opencodon_cli.web_server._ws_auth_ok``).
    ...
    """
    try:
        from opencodon_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``opencodon_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from opencodon_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from opencodon_cli.kanban_diagnostics import SEVERITY_ORDER
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from opencodon_cli.kanban import _check_dispatcher_presence
...
from opencodon_cli.kanban_db import (  # noqa: E402
...
# spawn failures, stuck-blocked). See opencodon_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def specify_task_endpoint(
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        ...
        from opencodon_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    """
    try:
        from opencodon_cli import profile_describer  # noqa: WPS433 (intentional)
