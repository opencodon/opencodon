/**
 * Opencodon Kanban — Dashboard Plugin
 *
...
(function () {
  ...
  const COLUMN_DOT = {
    triage: "opencodon-kanban-dot-triage",
    todo: "opencodon-kanban-dot-todo",
    ready: "opencodon-kanban-dot-ready",
    running: "opencodon-kanban-dot-running",
    blocked: "opencodon-kanban-dot-blocked",
    done: "opencodon-kanban-dot-done",
    archived: "opencodon-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-opencodon-task";
  ...
  const DOCS_URL = "https://github.com/opencodon/opencodon/tree/main/docs";
  const DOCS_TUTORIAL_URL = "https://github.com/opencodon/opencodon/tree/main/docs";
  ...
  const LS_BOARD_KEY = "opencodon.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="opencodon-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])opencodon-kanban-md-code\1/i.test(attrs)) {
      return ' class="opencodon-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "opencodon-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "opencodon-kanban-md",
  ...
  // standard `drop` event and our `opencodon-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("opencodon-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("opencodon-kanban-column--drop");
          if (target) target.classList.add("opencodon-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("opencodon-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("opencodon-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("opencodon-kanban:drop", {
  ...
  function KanbanPage() {
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "opencodon-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "opencodon-kanban-attention",
        "opencodon-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "opencodon-kanban-attention-bar" },
        h("span", { className: "opencodon-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "opencodon-kanban-attention-text" },
          diagTasks.length === 1
        ...
        h("button", {
          className: "opencodon-kanban-attention-toggle",
          onClick: function () { setExpanded(function (x) { return !x; }); },
          type: "button",
        }, expanded ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
        h("button", {
          className: "opencodon-kanban-attention-dismiss",
      ...
      expanded
        ? h("div", { className: "opencodon-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "opencodon-kanban-attention-row",
                  "opencodon-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "opencodon-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "opencodon-kanban-attention-row-id" }, task.id),
                h("span", { className: "opencodon-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "opencodon-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "opencodon-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "opencodon-kanban-diag-action-btn",
      action.suggested ? "opencodon-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " opencodon-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const sevClass = "opencodon-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("opencodon-kanban-diag", sevClass) },
      h("div", { className: "opencodon-kanban-diag-header" },
        h("span", { className: "opencodon-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "opencodon-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "opencodon-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "opencodon-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "opencodon-kanban-diag-data-row" },
                  h("span", { className: "opencodon-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "opencodon-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "opencodon-kanban-diag-data-row" },
                h("span", { className: "opencodon-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "opencodon-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "opencodon-kanban-diag-reassign-row" },
            h("span", { className: "opencodon-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "opencodon-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "opencodon-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "opencodon-kanban-diag-msg",
              msg.ok ? "opencodon-kanban-diag-msg--ok" : "opencodon-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "opencodon-kanban-section" },
      h("div", { className: "opencodon-kanban-section-head-row" },
        h("span", { className: "opencodon-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "opencodon-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "opencodon-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "opencodon-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "opencodon-kanban-docs-link",
      title: "Open Opencodon Kanban docs in a new tab",
      "aria-label": "Opencodon Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `opencodon kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "opencodon-kanban-msg-ok" : "opencodon-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "opencodon-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "opencodon-kanban-boardswitcher" },
      h("div", { className: "opencodon-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "opencodon-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "opencodon-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "opencodon-kanban-dialog-title" },
        ...
        h("div", { className: "opencodon-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Opencodon profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "opencodon-kanban-bulk" },
      h("span", { className: "opencodon-kanban-bulk-count" },
      ...
      h("div", { className: "opencodon-kanban-bulk-priority",
      ...
      h("div", { className: "opencodon-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Opencodon profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "opencodon-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("opencodon-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("opencodon-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "opencodon-kanban-trash",
        dragOver ? "opencodon-kanban-trash--drop" : "",
        props.draggingTaskId ? "opencodon-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "opencodon-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "opencodon-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("opencodon-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("opencodon-kanban-columns--panning");
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "opencodon-kanban-columns",
        isScrollable ? "opencodon-kanban-columns--scrollable" : "",
        isPanning ? "opencodon-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("opencodon-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("opencodon-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "opencodon-kanban-column",
        dragOver ? "opencodon-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "opencodon-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "opencodon-kanban-col-check",
        ...
        h("span", { className: cn("opencodon-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "opencodon-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "opencodon-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "opencodon-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "opencodon-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "opencodon-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "opencodon-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "opencodon-kanban-lane" },
                  h("div", { className: "opencodon-kanban-lane-head" },
                    h("span", { className: "opencodon-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "opencodon-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "opencodon-kanban-card--stale-red";
    if (age >= tier.amber) return "opencodon-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      if (selectedCards.length > 1 && props.selected) {
        ...
        ghost.className = "opencodon-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      "data-task-id": t.id,
      className: cn(
        "opencodon-kanban-card",
        props.selected ? "opencodon-kanban-card--selected" : "",
        props.failed ? "opencodon-kanban-card--failed" : "",
        props.draggingSource ? "opencodon-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "opencodon-kanban-card-content" },
          h("div", { className: "opencodon-kanban-card-row" },
            h("label", {
              className: "opencodon-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "opencodon-kanban-card-check",
            ...
            h("span", { className: "opencodon-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or opencodon kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "opencodon-kanban-warning-badge",
                    "opencodon-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
              ...
              : null,
            t.priority > 0
              ? h(Badge, { className: "opencodon-kanban-priority",
                           title: `Priority ${t.priority}. Higher-priority tasks are claimed first by the dispatcher.` }, `P${t.priority}`)
              : null,
            t.tenant
              ? h(Badge, { variant: "outline", className: "opencodon-kanban-tag",
                           title: `Tenant: ${t.tenant}. Free-form tag for grouping tasks (customer, project, team).` }, t.tenant)
              : null,
            progress
              ? h("span", {
                  className: cn(
                    "opencodon-kanban-progress",
                    progress.done === progress.total ? "opencodon-kanban-progress--full" : "",
                  ),
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "opencodon-kanban-needs-assignee",
          ...
          h("div", { className: "opencodon-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "opencodon-kanban-card-row opencodon-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "opencodon-kanban-assignee",
                            title: `Assigned to Opencodon profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "opencodon-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "opencodon-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "opencodon-kanban-ago",
  ...
  // centered modal reusing the opencodon-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "opencodon-kanban-dialog opencodon-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Opencodon profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Opencodon profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "opencodon-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "opencodon-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "opencodon-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "opencodon-kanban-drawer-close",
        ...
        data ? h("div", { className: "opencodon-kanban-drawer-comment-foot" },
          h("div", {
            className: "opencodon-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "opencodon-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "opencodon-kanban-section" },
      h("div", { className: "opencodon-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "opencodon-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "opencodon-kanban-drawer-body" },
      h("div", { className: "opencodon-kanban-drawer-title" },
        h("span", { className: cn("opencodon-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "opencodon-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "opencodon-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "opencodon-kanban-section" },
            h("div", { className: "opencodon-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "opencodon-kanban-section" },
            h("div", { className: "opencodon-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "opencodon-kanban-done-no-result opencodon-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "opencodon-kanban-section" },
            h("div", { className: "opencodon-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "opencodon-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "opencodon-kanban-section" },
        h("div", { className: "opencodon-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "opencodon-kanban-comment" },
            h("div", { className: "opencodon-kanban-comment-head" },
              h("span", { className: "opencodon-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "opencodon-kanban-diag-action-btn",
      ...
      h("div", { className: "opencodon-kanban-section" },
        h("div", { className: "opencodon-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "opencodon-kanban-comment" },
                h("div", { className: "opencodon-kanban-comment-head" },
                  h("span", { className: "opencodon-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "opencodon-kanban-comment-ago" },
      ...
      h("div", { className: "opencodon-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "opencodon-kanban-event",
              isDiag ? "opencodon-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "opencodon-kanban-event-header" },
                  h("span", { className: "opencodon-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "opencodon-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "opencodon-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "opencodon-kanban-event-header-plain" },
                  h("span", { className: "opencodon-kanban-event-kind" }, e.kind),
                  h("span", { className: "opencodon-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "opencodon-kanban-event-phantom-row" },
                  h("span", { className: "opencodon-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "opencodon-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "opencodon-kanban-event-payload" },
    ...
    return h("div", { className: "opencodon-kanban-section" },
      h("div", { className: "opencodon-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "opencodon-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `opencodon-kanban-run--${r.outcome || r.status || "ended"}`
          : "opencodon-kanban-run--active";
        return h("div", { key: r.id, className: cn("opencodon-kanban-run", outcomeClass) },
          h("div", { className: "opencodon-kanban-run-head" },
            h("span", { className: "opencodon-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "opencodon-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "opencodon-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "opencodon-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "opencodon-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "opencodon-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "opencodon-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "opencodon-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "opencodon-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "opencodon-kanban-pre opencodon-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "opencodon-kanban-meta-row" },
      h("span", { className: "opencodon-kanban-meta-label" }, props.label),
      h("span", { className: "opencodon-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "opencodon-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "opencodon-kanban-meta-row" },
        h("span", { className: "opencodon-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "opencodon-kanban-meta-value opencodon-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "opencodon-kanban-meta-row" },
        h("span", { className: "opencodon-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "opencodon-kanban-meta-value opencodon-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "opencodon-kanban-meta-row" },
        h("span", { className: "opencodon-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "opencodon-kanban-meta-value opencodon-kanban-editable",
    ...
    return h("div", { className: "opencodon-kanban-meta-row" },
      h("span", { className: "opencodon-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "opencodon-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "opencodon-kanban-section" },
      h("div", { className: "opencodon-kanban-section-head-row" },
        h("span", { className: "opencodon-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "opencodon-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "opencodon-kanban-section" },
      h("div", { className: "opencodon-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "opencodon-kanban-deps-row" },
        h("span", { className: "opencodon-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "opencodon-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "opencodon-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "opencodon-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "opencodon-kanban-dep-chip-x",
      ...
      h("div", { className: "opencodon-kanban-deps-row" },
        h("span", { className: "opencodon-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "opencodon-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "opencodon-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "opencodon-kanban-msg-ok"
          : "opencodon-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "opencodon-kanban-section" },
      ...
      h("div", { className: "opencodon-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "opencodon-kanban-home-sub opencodon-kanban-home-sub--on"
              : "opencodon-kanban-home-sub",
          }, label);
        })
      )
    );
