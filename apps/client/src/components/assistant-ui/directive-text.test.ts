...
import { formatRefValue, opencodonDirectiveFormatter } from './directive-text'
...
describe('opencodonDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = opencodonDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = opencodonDirectiveFormatter.parse('@file:src/main.tsx the entry point')
