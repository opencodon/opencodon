...
const OPENCODON_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type OpencodonRefType = (typeof OPENCODON_REF_TYPES)[number]
...
const ICON_PATHS: Record<OpencodonRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as OpencodonRefType] ?? ICON_FALLBACK
...
const OPENCODON_DIRECTIVE_RE = new RegExp(
...
export const opencodonDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(OPENCODON_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as OpencodonRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: OpencodonRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...opencodonDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Opencodon directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.opencodonDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.opencodonDesktop?.readFileDataUrl(id)
