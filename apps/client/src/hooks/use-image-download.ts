...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'opencodon:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.opencodonDesktop?.saveImageFromUrl) {
        if (await window.opencodonDesktop.saveImageFromUrl(src)) {
