 ...
 * `OpencodonPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/opencodon'
...
export type { PluginRestOptions } from '@/opencodon'
 ...
 *  under `opencodon.plugin.<id>.` — plugins can't read or clobber each other. */
...
export interface OpencodonPlugin {
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `opencodon.plugin.${pluginId}.${key}`
