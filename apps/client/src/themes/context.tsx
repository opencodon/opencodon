...
import { BUILTIN_THEME_LIST, DEFAULT_SKIN_NAME, DEFAULT_TYPOGRAPHY, opencodonTheme } from './presets'
...
const SKIN_KEY = 'opencodon-desktop-theme-v2'
const MODE_KEY = 'opencodon-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'opencodon-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'opencodon-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'opencodon-desktop-active-profile-v1'
...
export function getBaseColors(skinName: string, mode: 'light' | 'dark'): DesktopThemeColors {
  const seed = resolveTheme(skinName) ?? opencodonTheme
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  const typo = { ...DEFAULT_TYPOGRAPHY, ...opencodonTheme.typography, ...theme.typography }
  ...
  root.dataset.opencodonTheme = skinName
  root.dataset.opencodonMode = rendered
  ...
  window.opencodonDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('opencodon-boot-background', chromeBg)
    window.localStorage.setItem('opencodon-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.opencodonThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.opencodonDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Opencodon authoring/activating a skin from a
