/**
 * Live skin sync from the Opencodon backend.
 *
 * The backend resolves the active skin (built-in or `$OPENCODON_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Opencodon
 ...
 */
export function ingestBackendSkin(skin: OpencodonSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as OpencodonSkin)
