...
export async function resolveMediaDisplaySrc(path: string): Promise<string> {
  ...
  if (window.opencodonDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.opencodonDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.opencodonDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
export function mediaStreamUrl(path: string): string {
  return `opencodon-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// (workspace, skills, ~/.opencodon/cache, etc.); /api/media is intentionally
