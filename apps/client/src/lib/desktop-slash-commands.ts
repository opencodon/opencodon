...
/** Why a known Opencodon command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Opencodon profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Opencodon Agent version', surface: exec() },
...
function isKnownOpencodonSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Opencodon' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownOpencodonSlashCommand(normalized)
}
