...
import type { OpencodonReadDirEntry, OpencodonReadDirResult } from '@/global'
...
export type ProjectTreeEntry = OpencodonReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: OpencodonReadDirEntry) {
...
async function filterIgnored(entries: OpencodonReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<OpencodonReadDirResult> {
  if (!window.opencodonDesktop) {
