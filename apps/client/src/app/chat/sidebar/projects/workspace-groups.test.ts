...
import type { ProjectInfo, SessionInfo } from '@/types/opencodon'
...
describe('baseName', () => {
  it('returns the final path segment, ignoring trailing slashes and separators', () => {
    expect(baseName('/www/opencodon-agent/')).toBe('opencodon-agent')
...
describe('mergeRepoWorktreeGroups (visual enhancer)', () => {
  ...
  it('surfaces a user-named "New worktree" under .worktrees/ as its own lane', () => {
    const discovered: OpencodonGitWorktree[] = [
      {
        branch: 'opencodon/test-gui-stuff',
        detached: false,
        isMain: false,
        locked: false,
    ...
    expect(merged.map(g => g.label)).toContain('opencodon/test-gui-stuff')
  })

  it('relabels a dir-named linked worktree lane to its live checked-out branch', () => {
    // Backend labels the lane by the worktree dir (`opencodon-agent-ci`); the live
...
describe('overlayLiveLanes', () => {
  ...
  it('places a session into an out-of-tree (sibling) worktree lane by its path', () => {
    // `opencodon-agent-ci` is a linked worktree living BESIDE the repo, not under
