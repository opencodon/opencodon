...
import type { ProjectInfo, ProjectsPayload } from '@/types/opencodon'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Opencodon gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== ($activeGatewayProfile.get() || 'default')) {
    throw new Error('Active Opencodon profile changed while connecting')
  }
...
const repoScanStates = new WeakMap<OpencodonGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<OpencodonGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getOpencodonConfig(context.profile))
...
// repo, returning where Opencodon should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<OpencodonGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<OpencodonGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.opencodonDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.opencodonDesktop?.writeClipboard?.(path)
  }
}
