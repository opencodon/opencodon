...
const PROFILE_ORDER_STORAGE_KEY = 'opencodon.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'opencodon.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.opencodonDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new OPENCODON_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.opencodonDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.opencodonDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'opencodon.desktop.showAllProfiles'
...
// Switch to the default (root ~/.opencodon) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.opencodonDesktop?.touchBackend?.(target).catch(() => undefined)
}
