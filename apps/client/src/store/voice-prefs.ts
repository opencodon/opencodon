...
import { getOpencodonConfigRecord, saveOpencodonConfig } from '@/opencodon'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getOpencodonConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveOpencodonConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
