...
How to build opencodon Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another opencodon surface can also
...
- A **runtime home change** (switching the underlying `OPENCODON_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across opencodon — do not assume
