 ...
 * packaged binary) with a real `opencodon serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → opencodon serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Opencodon title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Opencodon')
  })
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → opencodon serve is spawned → WS connects → config
