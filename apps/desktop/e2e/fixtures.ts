/**
 * Shared E2E fixtures for the Opencodon desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → opencodon serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `opencodon desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `opencodon-e2e-${prefix}-${Math.random()}`))
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(opencodonHome, 'config.yaml')
 ...
 */
export function writeEnvFile(opencodonHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(opencodonHome, '.env')
 ...
 */
function writeEmptyConfig(opencodonHome: string): void {
  const configPath = path.join(opencodonHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - OPENCODON_HOME → sandbox opencodon-home (isolated config/sessions)
 *  - OPENCODON_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - OPENCODON_DESKTOP_IGNORE_EXISTING=1 → don't pick up `opencodon` from PATH
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...
    OPENCODON_DESKTOP_APP_NAME: `OpencodonE2E-${Date.now()}`,
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.opencodonHome)
 ...
 * nothing listens on). By default the backend still boots (`opencodon serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.opencodonHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { OPENCODON_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Opencodon backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Opencodon.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Opencodon.app', 'Contents', 'MacOS', 'Opencodon')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'opencodon')
}
 ...
 * progress without spawning a real Opencodon backend.
 *
