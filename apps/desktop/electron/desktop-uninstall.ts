 ...
 * the three user-facing uninstall modes to the `opencodon uninstall` CLI flags,
 ...
 *              `opencodon uninstall --gui --yes`
 *   - 'lite' → remove the GUI + agent code, KEEP user data (config / sessions
 *              / .env) for a future reinstall. `opencodon uninstall --yes`
 *   - 'full' → remove everything: GUI + agent + all user data.
 *              `opencodon uninstall --full --yes`
 *
 * Why a detached cleanup script: 'lite'/'full' delete the very venv the
 * `opencodon` command runs from, and every mode may need to delete the running
...
/**
 * Map an uninstall mode to the `python -m opencodon_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `opencodon_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'opencodon_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Opencodon.app/Contents/MacOS/Opencodon  → …/Opencodon.app
 *   Windows: …\Opencodon\Opencodon.exe                 → …\Opencodon  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Opencodon.app/Contents/MacOS/Opencodon → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Opencodon.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Opencodon.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Opencodon$/i.test(dir) || /[\\/]opencodon-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/opencodon
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import opencodon_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, opencodonHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export OPENCODON_HOME=${q(opencodonHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import opencodon_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Opencodon install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "OPENCODON_HOME=${String(opencodonHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
