 ...
 * null to keep it enabled. `OPENCODON_DESKTOP_DISABLE_GPU` overrides detection
...
function detectRemoteDisplay(options: { env?: NodeJS.ProcessEnv; platform?: NodeJS.Platform } = {}) {
  ...
  const override = String(env.OPENCODON_DESKTOP_DISABLE_GPU || '')
    .trim()
    .toLowerCase()

  if (GPU_OVERRIDE_ON.has(override)) {
    return 'override (OPENCODON_DESKTOP_DISABLE_GPU)'
  }
