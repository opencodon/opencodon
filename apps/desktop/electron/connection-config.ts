 ...
 *     `X-Opencodon-Session-Token` header; WS uses `?token=`.
...
// opencodon_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - opencodon_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - opencodon_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / opencodon #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-opencodon_session_at', '__Secure-opencodon_session_at', 'opencodon_session_at']
const RT_COOKIE_VARIANTS = ['__Host-opencodon_session_rt', '__Secure-opencodon_session_rt', 'opencodon_session_rt']

// The Nous portal (NAS) does NOT use Opencodon gateway session cookies — it is a
...
// Opencodon Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteOpencodonPath = String(entry.remoteOpencodonPath || '').trim()

  if (remoteOpencodonPath) {
    out.remoteOpencodonPath = remoteOpencodonPath
  }
...
function pathWithGlobalRemoteProfile(path, profile, opts: any = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://opencodon.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a opencodon session ACCESS-token cookie
 ...
 * the Opencodon gateway session cookies, so cloud sign-in / discovery liveness
