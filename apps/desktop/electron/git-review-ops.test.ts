...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'opencodon-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'opencodon-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Opencodon Test'], { cwd: dir })
