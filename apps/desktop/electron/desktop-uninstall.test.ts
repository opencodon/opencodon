...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'opencodon_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'opencodon_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'opencodon_cli.uninstall', '--mode', 'full'])
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  ...
  assert.match(script, /'-m' 'opencodon_cli\.uninstall' '--mode' 'lite'/)
})
