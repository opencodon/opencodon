  ...
  const result = await detectRemotePlatform(
    sshWith(async command => {
      ...
      return JSON.stringify({
        ...
        arch: 'ARM64',
        opencodonHome: 'C:\\h',
        opencodonPath: 'C:\\h\\opencodon.exe',
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('Opencodon is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /Opencodon is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\Opencodon's\\python.exe" }, 'inspect', [
    'C:\\x y\\opencodon.exe'
  ])
  ...
  assert.match(script, /Opencodon''s/)
  assert.match(script, /C:\\x y\\opencodon\.exe/)
})
