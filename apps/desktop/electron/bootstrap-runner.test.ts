...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'opencodon-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/opencodon-runner-test',
    sourceRepoRoot: null,
    opencodonHome: '/tmp/opencodon-runner-test',
    logRoot: '/tmp/opencodon-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'opencodon-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'opencodon-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/opencodon-agent',
      opencodonHome: '/tmp/opencodon'
    }),
    ['--dir', '/tmp/opencodon-agent', '--opencodon-home', '/tmp/opencodon', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/opencodon-agent', '--opencodon-home', '/tmp/opencodon', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/opencodon',
      opencodonHome: '/tmp/home'
    }),
    ['--dir', '/tmp/opencodon', '--opencodon-home', '/tmp/home', '--branch', 'main']
  )
})
