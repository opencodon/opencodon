...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line = 'setsid env OPENCODON_DASHBOARD_SESSION_TOKEN=abc123deadbeef OPENCODON_DESKTOP=1 opencodon dashboard'
  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /OPENCODON_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /OPENCODON_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-Opencodon-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-Opencodon-Session-Token: hdr_888'), /X-Opencodon-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-Opencodon-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('opencodon', 'remote-build-server', 22) // no baseDir → default
...
test('buildExecArgs ends with host then the remote command', () => {
  ...
  const args = buildExecArgs(conn, 'command -v opencodon', 15000)
  assert.equal(args[args.length - 1], 'command -v opencodon')
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `opencodon-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `opencodon-ssh-test-${process.pid}-${Date.now()}`)
