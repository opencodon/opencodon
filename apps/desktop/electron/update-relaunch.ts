 ...
 * Background
 * ----------
 * After `opencodon update` + `opencodon desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything OPENCODON_DESKTOP_* is preserved
// plus OPENCODON_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['OPENCODON_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['OPENCODON_DESKTOP_']
