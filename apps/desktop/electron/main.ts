...
import { buildDesktopBackendEnv, normalizeOpencodonHomeRoot } from './backend-env'
import { canImportOpencodonCli, shouldTrustOpencodonOverride, verifyOpencodonCli } from './backend-probes'
...
const USER_DATA_OVERRIDE = process.env.OPENCODON_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.OPENCODON_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.OPENCODON_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with OPENCODON_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[opencodon] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[opencodon] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// never go through `opencodon desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Opencodon sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[opencodon] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[opencodon] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[opencodon] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[opencodon] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[opencodon] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('opencodon:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[opencodon] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[opencodon] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[opencodon] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[opencodon] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// OPENCODON_HOME — the user-facing root for everything Opencodon-related. Mirrors
// scripts/install.ps1's $OpencodonHome and scripts/install.sh's $OPENCODON_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\opencodon (matches install.ps1)
//   macOS / Linux: ~/.opencodon (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.opencodon directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\opencodon yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// OPENCODON_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// OPENCODON_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.opencodon / %LOCALAPPDATA%\opencodon.
function resolveOpencodonHome() {
  if (process.env.OPENCODON_HOME) {
    return normalizeOpencodonHomeRoot(process.env.OPENCODON_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'opencodon-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a OPENCODON_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\opencodon and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('OPENCODON_HOME')

    if (fromRegistry) {
      return normalizeOpencodonHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'opencodon')
    const legacy = path.join(app.getPath('home'), '.opencodon')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.opencodon setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.opencodon')
}

const OPENCODON_HOME = resolveOpencodonHome()

function opencodonManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_opencodon_node_dirs() in
  // opencodon_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(OPENCODON_HOME, 'node')
...
function pathWithOpencodonManagedNode(...entries) {
  return [...opencodonManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}
...
// boots, going straight to `resolveOpencodonBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_OPENCODON_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_OPENCODON_ROOT, '.opencodon-bootstrap-complete')
...
// active-profile.json records which Opencodon profile the desktop launches its
// local backend as. When set, startOpencodon() passes `opencodon --profile <name>
// dashboard …`, which deterministically pins OPENCODON_HOME (see
// _apply_profile_override in opencodon_cli/main.py) and bypasses the sticky
// ~/.opencodon/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors opencodon_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// opencodonDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under OPENCODON_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by opencodon_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(OPENCODON_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (opencodon_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.OPENCODON_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.OPENCODON_DESKTOP_BOOT_FAKE_ERROR || ''

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.OPENCODON_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.OPENCODON_DESKTOP_APP_NAME || 'Opencodon'
...
// 'opencodon:native-theme' ('dark' | 'light' | 'system'); we pin
...
// an in-place `opencodon update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveOpencodonVersion(),
...
const MEDIA_PROTOCOL = 'opencodon-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(opencodon-media://…)`.
...
// startOpencodon(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.OPENCODON_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.OPENCODON_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startOpencodon() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning opencodon serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Opencodon backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  const lines = text.split(/\r?\n/).map(line => `[opencodon] ${line}`)
  opencodonLog.push(...lines)

  if (opencodonLog.length > 300) {
    opencodonLog.splice(0, opencodonLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-opencodon-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('opencodon:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the opencodon:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('opencodon:bootstrap:event', ev)
}
...
// (`force_kill_other_opencodon`, taskkill /IM opencodon.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(OPENCODON_HOME)
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Opencodon will start automatically when it completes…',
      12
    )
    await new Promise(r => setTimeout(r, UPDATE_WAIT_POLL_MS))
    marker = readLiveUpdateMarker(OPENCODON_HOME)
  }
...
function findOnPath(command) {
  ...
  // shell-script shim named `opencodon` — must not shadow `opencodon.cmd`/`opencodon.exe`.
...
function unwrapWindowsVenvOpencodonCommand(command, backendArgs) {
  return resolveVenvOpencodonCommand(command, backendArgs, {
...
// spawns `opencodon serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `opencodon` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'opencodon_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      execFileSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, OPENCODON_HOME, ...(backend.env || {}) },
        timeout: 15000,
        stdio: 'ignore',
        windowsHide: true
      })
...
function isOpencodonSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'opencodon_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.OPENCODON_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Opencodon deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Opencodon' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\opencodon\git\... — checked
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'opencodon', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'opencodon', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// normal OPENCODON_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\opencodon\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'opencodon', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'opencodon', 'git', 'bin', 'git.exe'))
  }
...
function recentOpencodonLog() {
  return opencodonLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's opencodon root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_OPENCODON_ROOT.
// OPENCODON_DESKTOP_OPENCODON_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.OPENCODON_DESKTOP_OPENCODON_ROOT && path.resolve(process.env.OPENCODON_DESKTOP_OPENCODON_ROOT),
    !IS_PACKAGED && isOpencodonSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isOpencodonSourceRoot(ACTIVE_OPENCODON_ROOT) ? ACTIVE_OPENCODON_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_OPENCODON_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('opencodon:updates:progress', merged)
  }
}
...
// OPENCODON_HOME/opencodon-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_opencodon_home). That binary owns
// ALL repo mutation — running `opencodon update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'opencodon-setup.exe' : 'opencodon-setup'
  const candidate = path.join(OPENCODON_HOME, name)
...
// Path to the venv shim whose lock decides whether `opencodon update` can write
// fresh entry points. On Windows this is the file the running backend
// `opencodon.exe` holds open; on POSIX it's never mandatory-locked.
function venvOpencodonShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'opencodon.exe')
    : path.join(updateRoot, 'venv', 'bin', 'opencodon')
}
...
// only signals the direct child, so on Windows a backend `opencodon.exe` that
// spawned its own grandchildren (a `opencodon` REPL, a pty terminal session, the
...
// `opencodonProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `opencodon.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a opencodon
// REPL, a pty terminal, the gateway) keep `opencodon.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  const opencodonProcess = backendConnectionState.getProcess()

  if (opencodonProcess && Number.isInteger(opencodonProcess.pid)) {
    pids.push(opencodonProcess.pid)
  }
  ...
  if (opencodonProcess && !opencodonProcess.killed) {
    try {
      opencodonProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvOpencodonShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const currentOpencodonProcess = backendConnectionState.getProcess()

    if (currentOpencodonProcess && Number.isInteger(currentOpencodonProcess.pid)) {
      stragglers.push(currentOpencodonProcess.pid)
    }
...
// `opencodon update`). Instead we spawn the staged Opencodon-Setup binary with
// --update and quit, so it can run `opencodon update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri opencodon-setup. Unlike Windows
      ...
      // whole update itself: `opencodon update` (backend) + `opencodon desktop
    ...
    if (!updater) {
      ...
      // `opencodon desktop`, never the Tauri installer that self-copies
      // opencodon-setup.exe into OPENCODON_HOME). They DO have a working `opencodon`
      ...
      // checkout they're on — bare `opencodon update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') {
            command = `opencodon update --branch ${branch}`
          }
        }
      } catch {
        // Best-effort: fall back to bare `opencodon update` if branch detection fails.
      }

      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })

      return { ok: true, manual: true, command, opencodonRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Opencodon — this window will close and the updater will open. Don’t reopen Opencodon yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // opencodon.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running opencodon, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Opencodon install open ' +
        '(a second Opencodon window or a terminal running opencodon?). Close it and retry.'

      emitUpdateProgress({ stage: 'error', message, percent: null })
      startOpencodon().catch(() => {})

      return { ok: false, error: message }
    }

    // Detached so the updater outlives this process — it needs us GONE before
    // `opencodon update` will run (the venv shim is locked while we live).
    const child = spawnUpdaterProcess(updater, updaterArgs, {
      cwd: OPENCODON_HOME,
      env: {
        ...process.env,
        OPENCODON_HOME,
        PATH: pathWithOpencodonManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore'
    })
    ...
    if (Number.isInteger(child.pid)) {
      writeUpdateMarker(OPENCODON_HOME, child.pid)
    }
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvOpencodon = path.join(venvBin, IS_WINDOWS ? 'opencodon.exe' : 'opencodon')
  ...
  // not just the `opencodon.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvOpencodon) || fileExists(path.join(updateRoot, '.opencodon-bootstrap-complete'))
...
// Resolve the opencodon CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `opencodon` on PATH.
function resolveOpencodonCliBinary(updateRoot) {
  const venvOpencodon = path.join(updateRoot, 'venv', 'bin', 'opencodon')

  if (fileExists(venvOpencodon)) {
    return venvOpencodon
  }

  return findOnPath('opencodon') || null
}
...
// macOS/Linux in-app update: backend (`opencodon update`) + OS-aware GUI rebuild
// (`opencodon desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const opencodon = resolveOpencodonCliBinary(updateRoot)

  if (!opencodon) {
    emitUpdateProgress({ stage: 'manual', message: 'opencodon update', percent: null })

    return { ok: true, manual: true, command: 'opencodon update', opencodonRoot: updateRoot }
  }

  // Put the Opencodon-managed Node and the venv on PATH so `opencodon desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\opencodon\node, not node\bin.
  // PYTHONUNBUFFERED: `opencodon update` writes to a pipe here, so CPython
  ...
  const env: Record<string, string> = {
    OPENCODON_HOME,
    PYTHONUNBUFFERED: '1',
    PATH: pathWithOpencodonManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `opencodon update` reaps stale `opencodon serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads OPENCODON_DESKTOP_CHILD_PID and excludes
  ...
  if (opencodonProcess && Number.isInteger(opencodonProcess.pid)) {
    desktopChildPids.push(opencodonProcess.pid)
  }
  ...
  if (desktopChildPids.length) {
    env.OPENCODON_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Opencodon (git + dependencies)…', percent: 10 })

  const updated = (await runStreamedUpdate(opencodon, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })) as any
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'opencodon update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'opencodon update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(opencodon, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })

  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Opencodon to retry.',
  ...
  // Linux in-app update terminal state (#45205). `opencodon desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Opencodon…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `opencodon-desktop-update-${Date.now()}.sh`)
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Opencodon to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Opencodon desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Opencodon to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Opencodon.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Opencodon.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Opencodon to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.opencodon-update-new"; then
    rm -rf "$DST.opencodon-update-old" 2>/dev/null || true
    mv "$DST" "$DST.opencodon-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.opencodon-update-new" "$DST"
    rm -rf "$DST.opencodon-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Opencodon to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_OPENCODON_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isOpencodonSourceRoot(ACTIVE_OPENCODON_ROOT) &&
    fileExists(venvPython) &&
    canImportOpencodonCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_OPENCODON_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `opencodon
...
function resolveWebDist() {
  const override = process.env.OPENCODON_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set OPENCODON_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: opencodon desktop --force-build`
  )
...
function resolveOpencodonCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Opencodon.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.OPENCODON_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveOpencodonCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'opencodon_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_OPENCODON_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Opencodon at ${ACTIVE_OPENCODON_ROOT}`,
    command,
    args: ['-m', 'opencodon_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      opencodonHome: OPENCODON_HOME,
      pythonPathEntries: [ACTIVE_OPENCODON_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_OPENCODON_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveOpencodonBackend(backendArgs) {
  // 1. Explicit override -- OPENCODON_DESKTOP_OPENCODON_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.OPENCODON_DESKTOP_OPENCODON_ROOT && path.resolve(process.env.OPENCODON_DESKTOP_OPENCODON_ROOT)

  if (overrideRoot && isOpencodonSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Opencodon source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `opencodon` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isOpencodonSourceRoot.)
  if (!IS_PACKAGED && isOpencodonSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Opencodon source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  //    (applyUpdates -> git pull) or `opencodon update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(backendArgs)
  }

  // 4. Existing `opencodon` on PATH -- installed via install.ps1 / install.sh from
  ...
  if (process.env.OPENCODON_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (opencodonOverride) {
      ...
      if (resolvedOverride) {
        opencodonCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(opencodonOverride, { isWsl: IS_WSL })) {
        opencodonCommand = opencodonOverride
      } else {
        rememberLog(`Ignoring Windows Opencodon override under WSL: ${opencodonOverride}`)
      }
    } else {
      opencodonCommand = findOnPath('opencodon')
    }

    if (opencodonCommand) {
      if (looksLikeDesktopAppBinary(opencodonCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Opencodon CLI: ${opencodonCommand}`)
        opencodonCommand = null
      }
    }

    if (opencodonCommand) {
      const unwrapped = unwrapWindowsVenvOpencodonCommand(opencodonCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `opencodon` shim
      ...
      if (shouldTrustOpencodonOverride(opencodonOverride) || verifyOpencodonCli(opencodonCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvOpencodonCommand(opencodonCommand, backendArgs) || {
            label: `existing Opencodon CLI at ${opencodonCommand}`,
            command: opencodonCommand,
            args: backendArgs,
            bootstrap: false,
            env: {},
            kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Opencodon CLI at ${opencodonCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed opencodon_cli module via system Python.
  ...
  if (python) {
    ...
    // having opencodon_cli installed -- common on dev boxes that have
    ...
    if (canImportOpencodonCli(python)) {
      return {
        kind: 'python',
        label: `installed opencodon_cli module via ${python}`,
        command: python,
        args: ['-m', 'opencodon_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: opencodon_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveOpencodonBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Opencodon Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_OPENCODON_ROOT,
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveOpencodonBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Opencodon install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Opencodon recovery was handed off to Opencodon Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      opencodonHome: OPENCODON_HOME,
      logRoot: path.join(OPENCODON_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Opencodon install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Opencodon bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(OPENCODON_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startOpencodon() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // opencodon:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveOpencodonBackend(backend.args))
  }
  ...
  if (!isOpencodonSourceRoot(ACTIVE_OPENCODON_ROOT)) {
    throw new Error(
      `Opencodon install at ${ACTIVE_OPENCODON_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Opencodon' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\opencodon\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `opencodon` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Opencodon on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Opencodon.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // isOpencodonSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Opencodon venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Opencodon at ${ACTIVE_OPENCODON_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Opencodon runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Opencodon backend URL protocol: ${parsed.protocol}`))
    ...
    const req = client.request(
      parsed,
      {
        ...
        headers: {
          ...
          'X-Opencodon-Session-Token': token,
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Opencodon backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Opencodon backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Opencodon-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('opencodon:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveOpencodonCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('opencodon:preview-file-changed', payload)
}
...
async function waitForOpencodon(baseUrl, token, signal?) {
  ...
  throw new Error(`Opencodon backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  webContents.send('opencodon:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('opencodon:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Opencodon...".
function sendPowerResume() {
  ...
  webContents.send('opencodon:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('opencodon:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('opencodon:window-state-changed', state)
}
...
//
// Hosted Opencodon gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``opencodon_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / opencodon #37247).
//     Both are set as HttpOnly cookies (``opencodon_session_at`` ~15 min,
//     ``opencodon_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:opencodon-remote-oauth'
...
// "not signed in", which on the initial boot path (startOpencodon → the renderer's
// single-shot boot() with no retry) surfaced as the "Opencodon couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Opencodon
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Opencodon Cloud agent…' : 'Sign in to Opencodon gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Opencodon backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in opencodon_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Opencodon Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:opencodon-remote-oauth`:
...
// authenticates via PRIVY, not the Opencodon gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for opencodon_session_at/rt that the portal
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Opencodon Cloud sign-in.'))
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Opencodon Cloud',
        autoHideMenuBar: true,
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
...
// Discover the hosted (Opencodon Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Opencodon Cloud. Open Settings → Gateway, choose Opencodon Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your Opencodon Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Opencodon Cloud session has expired. Sign in to Opencodon Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Opencodon Cloud org tag on cloud-mode entries so Settings can
...
// a valid stored value (pins the root OPENCODON_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.OPENCODON_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.OPENCODON_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Opencodon Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteOpencodonPath: (ssh || savedSsh)?.remoteOpencodonPath || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by OPENCODON_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Opencodon Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Opencodon Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteOpencodonPath: input.sshRemoteOpencodonPath ?? existingBlock.remoteOpencodonPath
  })
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (opencodon #37247), and the gateway middleware transparently
    ...
    if (!oauthSessionIsLive(hasNativeSession(baseUrl), await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Opencodon gateway uses OAuth, but you are not signed in. ' +
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Opencodon gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Opencodon gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget() {
  ...
  if (process.env.OPENCODON_DESKTOP_REMOTE_URL) {
    return null
  }
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  try {
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteOpencodonPath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      profile: connectionScopeKey(profile) || '',
      remoteOpencodonPath: sshConfig.remoteOpencodonPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForOpencodon: (baseUrl, token) => waitForOpencodon(baseUrl, token, lease.signal),
  ...
  sshConnections.set(scope, {
    ...
    host: sshConfig.host,
    hostLabel,
    opencodonVersion: result.opencodonVersion || '',
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.opencodonVersion || 'opencodon (version unknown)'} at ${result.opencodonPath || '?'}`
  )
  ...
  return { ...connection, remoteOpencodonVersion: result.opencodonVersion || '' }
}
...
//   2. env override (OPENCODON_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.OPENCODON_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.OPENCODON_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'OPENCODON_DESKTOP_REMOTE_URL is set but OPENCODON_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Opencodon backend.'
      )
    }
...
// env → global) so a startOpencodon() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Opencodon
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteOpencodonPath: input.sshRemoteOpencodonPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteOpencodonPath || '')
          let opencodonPath
          let opencodonVersion
          let supported

          if (platform.os === 'Windows') {
            const runtime = platform
            opencodonPath = runtime.opencodonPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.opencodonPath])
            opencodonVersion = inspection.version
            supported = inspection.supported
          } else {
            opencodonPath = await remoteLifecycle.locateOpencodon(ssh, sshConfig.remoteOpencodonPath || '')
            opencodonVersion = await remoteLifecycle.probeOpencodonVersion(ssh, opencodonPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, opencodonPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Opencodon on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteOpencodonPath: opencodonPath,
            remoteOpencodonVersion: opencodonVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startOpencodon())
  ...
  // connect to Opencodon gateway". Mirror the renderer's connect here so the test
...
function resetOpencodonConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteLiveness.clear()
  const opencodonProcess = backendConnectionState.invalidate()
  stopBackendChild(opencodonProcess)
...
// startOpencodon() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetOpencodonConnection() invalidates it.
  const opencodonProcess = backendConnectionState.getProcess()
  const dying = opencodonProcess && !opencodonProcess.killed ? opencodonProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('opencodon:connection:applied')
}
...
// profile to startOpencodon() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startOpencodon() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForOpencodon(remote.baseUrl, remote.token)

    return {
      ...remote,
      profile,
      logs: opencodonLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited OPENCODON_HOME env (see _apply_profile_override
  // step 3 in opencodon_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveOpencodonBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const opencodonCwd = resolveOpencodonCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Opencodon backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: opencodonCwd,
        OPENCODON_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        OPENCODON_DESKTOP: '1',
        OPENCODON_WEB_DIST: webDist,
        ...(readyFile ? { OPENCODON_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Opencodon backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Opencodon backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`Opencodon backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForOpencodon(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Opencodon backend for profile "${profile}"`,
    rememberLog
  })
...
// backend whose ensure_opencodon_home() recreates the deleted profile directory.
//
...
async function startOpencodon() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startOpencodon() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Opencodon backend', 8)
  ...
  const connectionPromise = (async () => {
    ...
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Opencodon backend at ${remote.baseUrl}`, 24)
      await waitForOpencodon(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Opencodon backend is ready',
        progress: 94,
        running: true,
        error: null
      })
      ...
      return {
        ...
        remoteOpencodonVersion: remote.remoteOpencodonVersion,
        token: remote.token,
        wsUrl: remote.wsUrl,
        logs: opencodonLog.slice(-80),
        ...getWindowState()
    ...
    // deterministic (it wins over the sticky ~/.opencodon/active_profile file) and
    // resolves OPENCODON_HOME the same way `opencodon -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Opencodon runtime', 28)
    const backend = await ensureRuntime(resolveOpencodonBackend(backendArgs))
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    const opencodonCwd = resolveOpencodonCwd()
    ...
    await advanceBootProgress('backend.spawn', `Starting Opencodon backend via ${backend.label}`, 84)
    rememberLog(`Starting Opencodon backend via ${backend.label}`)

    const opencodonProcess = spawn(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: opencodonCwd,
        env: {
          ...process.env,
          // Explicitly pin OPENCODON_HOME for the child so Python's get_opencodon_home()
          // resolves to the SAME location our resolveOpencodonHome() picked. Without
          // this pin, Python falls back to ~/.opencodon on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\opencodon, which differs from C:\Users\<u>\.opencodon.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets OPENCODON_HOME via setx; the desktop
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, opencodonProcess)

    if (!processOwner) {
      stopBackendChild(opencodonProcess)
      throw new Error('Opencodon backend start was superseded by a newer connection attempt.')
    }

    opencodonProcess.stdout.on('data', rememberLog)
    opencodonProcess.stderr.on('data', rememberLog)
    ...
    opencodonProcess.once('error', error => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Opencodon backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Opencodon backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Opencodon backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Opencodon backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    opencodonProcess.once('exit', (code, signal) => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Opencodon backend exit (${signal || code})`)
      ...
      rememberLog(`Opencodon backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Opencodon backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Opencodon backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentOpencodonLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Opencodon backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(opencodonProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Opencodon backend to become ready', 90)
    await Promise.race([waitForOpencodon(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => opencodonProcess.exitCode === null && !opencodonProcess.killed,
      rememberLog
    })

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Opencodon backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Opencodon is
...
// pushes pet state over IPC (opencodon:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via opencodon:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Opencodon app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Opencodon icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see opencodon:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Opencodon in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('opencodon:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function createWindow() {
  ...
  startOpencodon().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('opencodon:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Opencodon…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('opencodon:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const result = await revalidateRemoteConnection({
      ...
      resetConnection: resetOpencodonConnection,
...
ipcMain.handle('opencodon:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('opencodon:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('opencodon:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('opencodon:window:openInstance', async () => {
...
ipcMain.handle('opencodon:zoom:get', event => {
...
ipcMain.on('opencodon:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('opencodon:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('opencodon:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('opencodon:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('opencodon:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('opencodon:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('opencodon:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('opencodon:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('opencodon:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('opencodon:pet-overlay:control', payload)
})
ipcMain.handle('opencodon:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startOpencodon() call restarts the
...
ipcMain.handle('opencodon:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startOpencodon() re-runs the full installer (refreshing a broken/partial
  ...
  bootstrapFailure = null
  backendStartFailure = null
  resetOpencodonConnection()

  return { ok: true }
})
ipcMain.handle('opencodon:bootstrap:cancel', async () => {
...
ipcMain.handle('opencodon:boot-progress:get', async () => bootProgressState)
ipcMain.handle('opencodon:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('opencodon:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('opencodon:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('opencodon:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('opencodon:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('opencodon:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('opencodon:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('opencodon:connection-config:oauth-logout', async (_event, rawUrl) => {
...
// --- Opencodon Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('opencodon:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('opencodon:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('opencodon:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('opencodon:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('opencodon:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new OPENCODON_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('opencodon:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('opencodon:requestMicrophoneAccess', async () => {
...
ipcMain.handle('opencodon:api', async (_event, request) => {
  ...
  // backend calls ensure_opencodon_home() which recreates the profile directory,
...
ipcMain.handle('opencodon:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('opencodon:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('opencodon:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('opencodon:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
...
ipcMain.handle('opencodon:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('opencodon:readFileText', async (_event, filePath) => {
...
ipcMain.handle('opencodon:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('opencodon:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))

  return true
})

ipcMain.handle('opencodon:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('opencodon:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('opencodon:saveClipboardImage', async () => {
...
ipcMain.handle('opencodon:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('opencodon:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('opencodon:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('opencodon:titlebar-theme', (_event, payload) => {
...
ipcMain.on('opencodon:native-theme', (_event, mode) => {
...
ipcMain.on('opencodon:translucency', (_event, payload) => {
...
ipcMain.on('opencodon:keep-awake', (_event, on) => {
...
ipcMain.handle('opencodon:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('opencodon:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveOpencodonCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('opencodon:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveOpencodonCwd()
}))

ipcMain.handle('opencodon:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('opencodon:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('opencodon:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('opencodon:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('opencodon:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('opencodon:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: opencodonLog.slice(-200) }))
...
function terminalShellCommand() {
  // OPENCODON_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.OPENCODON_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Opencodon' TUI into
  ...
  env.TERM_PROGRAM = 'Opencodon'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a opencodon/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from OPENCODON_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.OPENCODON_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `opencodon:terminal:${id}:${suffix}`
}
...
ipcMain.handle('opencodon:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('opencodon:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('opencodon:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('opencodon:fs:openDir', async (_event, dirPath) => {
...
ipcMain.handle('opencodon:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('opencodon:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('opencodon:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('opencodon:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('opencodon:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('opencodon:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('opencodon:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('opencodon:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))

ipcMain.handle('opencodon:git:baseBranchList', async (_event, repoPath) => listBaseBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('opencodon:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('opencodon:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('opencodon:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('opencodon:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('opencodon:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('opencodon:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('opencodon:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('opencodon:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('opencodon:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('opencodon:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('opencodon:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('opencodon:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('opencodon:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('opencodon:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('opencodon:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('opencodon:terminal:write', (_event, id, data) => {
...
ipcMain.handle('opencodon:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('opencodon:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('opencodon:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('opencodon:updates:check', async () =>
...
ipcMain.handle('opencodon:updates:apply', async (_event, payload) =>
...
ipcMain.handle('opencodon:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('opencodon:updates:branch:set', async (_event, name) => {
...
function resolveOpencodonVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'opencodon_cli', '__init__.py')
...
// Re-resolve the live Opencodon version and push it into the native About panel
// just before showing it, so an in-place `opencodon update` is reflected without
...
ipcMain.handle('opencodon:version', async () => ({
  appVersion: resolveOpencodonVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  opencodonRoot: resolveUpdateRoot()
}))
...
// the actual removal via `opencodon uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (opencodon_cli/uninstall.py + opencodon_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_OPENCODON_ROOT
  ...
  const fallback = () => ({
    ...
    agent_installed: isOpencodonSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'opencodon_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, OPENCODON_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Opencodon agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import opencodon_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_OPENCODON_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_OPENCODON_ROOT, 'uninstall')
  } catch (error) {
  ...
  const scriptArgs = {
    ...
    pythonExe: py,
    pythonPath,
    agentRoot: ACTIVE_OPENCODON_ROOT,
    uninstallArgs,
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `opencodon-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `opencodon-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('opencodon:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('opencodon:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('opencodon:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('opencodon:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// opencodon:// deep links (e.g. opencodon://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const OPENCODON_PROTOCOL = 'opencodon'
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${OPENCODON_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // opencodon://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('opencodon:deep-link', payload)
...
ipcMain.handle('opencodon:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${OPENCODON_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(OPENCODON_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(OPENCODON_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `opencodon://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching opencodon:// URL is in our own argv.
