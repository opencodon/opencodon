// Profile-delete routing logic for the `opencodon:api` IPC handler.
//
...
// pool backend -- spawning a fresh one would call ensure_opencodon_home() and
...
/**
 * Parse a `opencodon:api` request into the profile name a DELETE targets, or
...
/**
 * Route the next `opencodon:api` request away from the primary/window backend
 ...
 * ensure_opencodon_home() recreates the directory the delete just removed.
 */
