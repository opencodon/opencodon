...
export const ZOOM_STORAGE_KEY = 'opencodon:desktop:zoomLevel'
 ...
 * but forgot to emit 'opencodon:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('opencodon:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * global UI zoom; unknown kinds fall back to the chat wiring.
 *
 * Extracted so the per-kind contract is unit-testable without booting a
 * BrowserWindow or reading source.
 */
