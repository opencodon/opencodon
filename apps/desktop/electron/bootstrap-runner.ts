 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Opencodon Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.opencodon-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(opencodonHome) {
  return path.join(opencodonHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.opencodon/opencodon. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(opencodonHome) {
  if (!opencodonHome) {
    return null
  }
...
function cachedScriptPath(opencodonHome, commit) {
  return path.join(bootstrapCacheDir(opencodonHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/opencodon/opencodon/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(opencodonHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(opencodonHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, opencodonHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass OPENCODON_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          OPENCODON_HOME: opencodonHome || process.env.OPENCODON_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, opencodonHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, opencodonHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--opencodon-home', opencodonHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, opencodonHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, opencodonHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, opencodonHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(opencodonHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, opencodonHome, emit })
