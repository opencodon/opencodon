...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/opencodon', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/opencodon/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/opencodon'), 'https://gw.example.com/opencodon/auth/native/refresh')
})
