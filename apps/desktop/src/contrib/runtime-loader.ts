 ...
 *   (`@opencodon/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default OpencodonPlugin -> register(ctx)
 *
 ...
 * and `<opencodon home>/desktop-plugins/<name>/plugin.js` on disk — the door the
...
import { getStatus } from '@/opencodon'
import { installPluginSdk, sdkImportMap } from '@/sdk/runtime'
import { notifyError } from '@/store/notifications'

import { createPluginContext, type OpencodonPlugin } from './plugin'
...
/** Rewrite ONLY mapped import specifiers (@opencodon/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @opencodon/plugin-sdk and react`
      )
    }

    const url = URL.createObjectURL(new Blob([rewriteSpecifiers(source)], { type: 'text/javascript' }))

    let mod: { default?: OpencodonPlugin }
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default OpencodonPlugin export`)
    }
...
// ---------------------------------------------------------------------------
// The on-disk plugin door: `<opencodon home>/desktop-plugins/<name>/plugin.js`
...
async function scanDiskPlugins(): Promise<void> {
  ...
  try {
    ...
    const { entries } = await desktop.readDir(`${opencodon_home}/desktop-plugins`)
