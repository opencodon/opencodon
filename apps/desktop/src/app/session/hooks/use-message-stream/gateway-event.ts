...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  ...
  // refreshOpencodonConfig — two REST calls (config + defaults) per event, per
  ...
  return useCallback(
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        ingestBackendSkin((payload as { skin?: OpencodonSkin } | undefined)?.skin, { apply: false })

        return
      } else if (event.type === 'skin.changed') {
        // A runtime skin switch (Opencodon activating an authored skin, or `/skin`
        ...
        if (fromActiveProfile) {
          ingestBackendSkin(payload as OpencodonSkin | undefined, { apply: true })
        }
        ...
        if (apply) {
          ...
          // everything refreshOpencodonConfig applies is either active-session
        ...
        // native OS notification when Opencodon is backgrounded; dispatch is gated
        ...
        const errorMessage = payload?.message || 'Opencodon reported an error'
