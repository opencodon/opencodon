...
import { getActionStatus, getLogs, getStatus, getUsageAnalytics, restartGateway, updateOpencodon } from '@/opencodon'
import type { ActionStatusResponse, AnalyticsResponse, StatusResponse } from '@/opencodon'
...
export function CommandCenterView({ initialSection, onClose, onDeleteSession, onOpenSession }: CommandCenterViewProps) {
  ...
  // Client-side substring filter over the fetched tail (matches `opencodon logs --search`).
  ...
  const runSystemAction = useCallback(
    async (kind: 'restart' | 'update') => {
      ...
      try {
        const started = kind === 'restart' ? await restartGateway() : await updateOpencodon()
                    ...
                    <div className="flex items-start justify-between gap-3 max-[47.5rem]:flex-col max-[47.5rem]:gap-2">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">
                          {cc.opencodonActiveSessions(status.version, status.active_sessions)}
                        </div>
                      </div>
