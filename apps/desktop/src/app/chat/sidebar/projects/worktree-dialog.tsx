...
import type { OpencodonGitBranch } from '@/global'
...
const branchActionLabel = (branch: OpencodonGitBranch, copy: BranchActionCopy) => {
...
export function WorktreeDialog({ repoPath, onStarted, open, onOpenChange, initialBase }: WorktreeDialogProps) {
  ...
  const [branches, setBranches] = useState<OpencodonGitBranch[]>([])
  ...
  const convert = async (branch: OpencodonGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
