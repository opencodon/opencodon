...
import { getOpencodonConfigDefaults, getOpencodonConfigRecord, saveOpencodonConfig } from '@/opencodon'
...
export function SettingsView({ onClose, onConfigSaved, onMainModelChanged }: SettingsPageProps) {
  ...
  const exportConfig = async () => {
    try {
      const cfg = await getOpencodonConfigRecord()
      const blob = new Blob([JSON.stringify(cfg, null, 2)], { type: 'application/json' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = 'opencodon-config.json'
      a.click()
  ...
  const resetConfig = async () => {
    ...
    try {
      await saveOpencodonConfig(await getOpencodonConfigDefaults())
      triggerHaptic('success')
      onConfigSaved?.()
    } catch (err) {
