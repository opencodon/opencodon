...
import { getElevenLabsVoices, getOpencodonConfigSchema, saveOpencodonConfig } from '@/opencodon'
import { useI18n } from '@/i18n'
import { $keepAwake, setKeepAwake } from '@/store/keep-awake'
import { notify, notifyError } from '@/store/notifications'
import { repoDiscoveryPolicyFromConfig, repoDiscoveryPolicySignature, scanAndRecordRepos } from '@/store/projects'
import type { ConfigFieldSchema, OpencodonConfigRecord } from '@/types/opencodon'

import { setOpencodonConfigCache, useOpencodonConfigRecord } from '../hooks/use-config-record'
...
export function voiceFieldVisible(key: string, config: OpencodonConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<OpencodonConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useOpencodonConfigRecord()
  ...
  const {
    ...
    queryKey: ['opencodon-config-schema'],
    queryFn: getOpencodonConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const result = await saveOpencodonConfig(config)
          ...
          setOpencodonConfigCache(config)
  ...
  const updateConfig = (next: OpencodonConfigRecord) => {
