...
import { invalidateOpencodonConfig, setOpencodonConfigCache, useOpencodonConfigRecord } from '../hooks/use-config-record'
...
// Opencodon' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Opencodon default (medium), shown as Medium.
...
// backend now surfaces the full `opencodon model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in opencodon_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useOpencodonConfigRecord()
  const setConfig = setOpencodonConfigCache
  ...
  const refresh = useCallback(async ({ replaceSelection = false }: { replaceSelection?: boolean } = {}) => {
    ...
    try {
      ...
      void invalidateOpencodonConfig()
    } catch (err) {
      if (profileEpoch.current === epoch) {
  ...
  const writeAgentDefault = useCallback(
    async (key: string, value: string) => {
      ...
      try {
        await saveOpencodonConfig(next)
      } catch (err) {
        setConfig(prev)
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `opencodon model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Opencodon runs the flow for you.`}
          </p>
        )}
