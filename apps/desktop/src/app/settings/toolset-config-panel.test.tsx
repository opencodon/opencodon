...
import type { ToolsetConfig } from '@/types/opencodon'
...
const getOpencodonConfigRecord = vi.fn()
const getOpencodonConfigSchema = vi.fn()
const saveOpencodonConfig = vi.fn()
...
vi.mock('@/opencodon', () => ({
  ...
  getOpencodonConfigRecord: () => getOpencodonConfigRecord(),
  getOpencodonConfigSchema: () => getOpencodonConfigSchema(),
  saveOpencodonConfig: (config: unknown) => saveOpencodonConfig(config),
...
beforeEach(() => {
  ...
  getOpencodonConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getOpencodonConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
  saveOpencodonConfig.mockResolvedValue({ ok: true })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveOpencodonConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveOpencodonConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
