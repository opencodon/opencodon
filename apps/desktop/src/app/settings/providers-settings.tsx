...
import { disconnectOAuthProvider, listOAuthProviders } from '@/opencodon'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/opencodon'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.opencodonDesktop?.terminal)
...
//      in opencodon_cli/provider_catalog.py) — the SAME provider identity
//      `opencodon model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Opencodon can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Opencodon can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Opencodon never deletes creds another tool owns behind a silent API call.
