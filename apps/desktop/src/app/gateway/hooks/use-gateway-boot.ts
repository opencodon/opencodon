import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@opencodon/shared'
import { useEffect, useRef } from 'react'

import type { OpencodonConnection } from '@/global'
import { OpencodonGateway } from '@/opencodon'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.opencodonDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: OpencodonGateway | null) => void
  refreshOpencodonConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: OpencodonConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Opencodon...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Opencodon…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Opencodon gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshOpencodonConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshOpencodonConfig().catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startOpencodon re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new OpencodonGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshOpencodonConfig(),
          ...
          // retry it), not brick boot behind the "Opencodon couldn't start"
