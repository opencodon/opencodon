...
import type { OpencodonReadDirResult } from '@/global'
import { $connection } from '@/store/session'

import { clearProjectDirCache, readProjectDir } from './ipc'
import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<OpencodonReadDirResult>>()

beforeEach(() => {
  $connection.set(null)
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { opencodonDesktop: { readDir: typeof readDir } }).opencodonDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): OpencodonReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { opencodonDesktop: unknown }).opencodonDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: OpencodonReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<OpencodonReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { opencodonDesktop: unknown }).opencodonDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.opencodonDesktop is missing', async () => {
    delete (window as unknown as { opencodonDesktop?: unknown }).opencodonDesktop
