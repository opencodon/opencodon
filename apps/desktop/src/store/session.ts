import type { ConnectionState } from '@opencodon/shared'
...
import type { SessionInfo, UsageStats } from '@/types/opencodon'
...
const COMPOSER_MODEL_KEY = 'opencodon.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'opencodon.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'opencodon.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'opencodon.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'opencodon.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'opencodon.desktop.lastSessionId'
...
const LAST_ROUTE_KEY = 'opencodon.desktop.lastRoute'
...
function workspaceCwdKey(connection: OpencodonConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.opencodonDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.opencodonDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<OpencodonConnection | null>(null)
...
export const setConnection = (next: Updater<OpencodonConnection | null>) => updateAtom($connection, next)
