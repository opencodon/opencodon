...
import type { SessionInfo } from '@/types/opencodon'
...
describe('workspaceCwdForNewSession', () => {
  afterEach(() => {
    ...
    window.localStorage.removeItem('opencodon.desktop.workspace-cwd')
    window.localStorage.removeItem('opencodon.desktop.workspace-cwd.remote.http%3A%2F%2Fbackend-a.default')
    window.localStorage.removeItem('opencodon.desktop.workspace-cwd.remote.http%3A%2F%2Fbackend-b.default')
  })

  it('prefers the configured default over the sticky remembered workspace', () => {
    window.localStorage.setItem('opencodon.desktop.workspace-cwd', '/home/user/sticky')
  ...
  it('keeps remote workspace memory separate from local and other remotes', () => {
    window.localStorage.setItem('opencodon.desktop.workspace-cwd', '/local/project')
    $currentCwd.set('/live/session/path')
...
describe('remembered session id (per profile)', () => {
  ...
  it('keeps the default profile on the legacy unsuffixed key for back-compat', () => {
    ...
    localStorage.setItem('opencodon.desktop.lastSessionId', 'legacy-session')
