...
import type { OpencodonReviewFile, OpencodonReviewShipInfo } from '@/global'
...
// reaches the gateway); everything else routes through window.opencodonDesktop.git,
...
function file(path: string, over: Partial<OpencodonReviewFile> = {}): OpencodonReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as OpencodonReviewFile
}

type ReviewStub = Record<string, ReturnType<typeof vi.fn>>

// Install a review bridge on window.opencodonDesktop. Any op not supplied defaults
...
function stubReview(over: ReviewStub = {}) {
  ...
  ;(window as unknown as { opencodonDesktop?: unknown }).opencodonDesktop = {
    git: { review },
    openExternal: vi.fn()
  }
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as OpencodonReviewShipInfo)

    await createOrOpenPr()

    expect(review.createPr).not.toHaveBeenCalled()
    expect(
      (window.opencodonDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: OpencodonReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as OpencodonReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { opencodonDesktop?: unknown }).opencodonDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as OpencodonReviewShipInfo)
