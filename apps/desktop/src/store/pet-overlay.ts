 ...
 * app's bounds and stays visible while Opencodon is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'opencodon.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'opencodon.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.opencodonDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.opencodonDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.opencodonDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.opencodonDesktop?.petOverlay?.close()
}
