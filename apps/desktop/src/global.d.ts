...
declare global {
  interface Window {
    opencodonDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<OpencodonConnection>
      ...
      // Opencodon Cloud: one portal login powers discovery + silent per-agent
      ...
      api: <T>(request: OpencodonApiRequest) => Promise<T>
      notify: (payload: OpencodonNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<OpencodonReadFileTextResult>
      selectPaths: (options?: OpencodonSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<OpencodonPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<OpencodonPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: OpencodonTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<OpencodonReadDirResult>
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<OpencodonGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<OpencodonGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<OpencodonGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<OpencodonRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: OpencodonReviewScope, baseRef?: null | string) => Promise<OpencodonReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: OpencodonReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<OpencodonReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: OpencodonTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<OpencodonTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: OpencodonWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: OpencodonPreviewFileChanged) => void) => () => void
...
export interface OpencodonTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface OpencodonTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `opencodon update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  opencodonRoot?: string
...
export interface OpencodonConnection {
  baseUrl: string
...
export interface OpencodonTitleBarTheme {
  background: string
  foreground: string
}

export interface OpencodonWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Opencodon Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Opencodon Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteToken?: string
  remoteUrl?: string
  // For a 'cloud' connection: the selected Opencodon Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the opencodon gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Opencodon Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface OpencodonApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface OpencodonNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface OpencodonPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface OpencodonReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface OpencodonPreviewWatch {
  id: string
  path: string
}
...
export interface OpencodonGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface OpencodonGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
...
// preselect it.
export interface OpencodonGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface OpencodonRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface OpencodonRepoStatus {
  branch: null | string
...
// turn began.
export type OpencodonReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface OpencodonReviewFile {
  path: string
  added: number
  removed: number
...
export interface OpencodonReviewList {
  files: OpencodonReviewFile[]
...
export interface OpencodonReviewPr {
  url: string
  state: string
  number: number
}
...
export interface OpencodonReviewShipInfo {
  ghReady: boolean
  pr: OpencodonReviewPr | null
}

export interface OpencodonReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface OpencodonReadDirResult {
  entries: OpencodonReadDirEntry[]
  error?: string
}

export interface OpencodonPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface OpencodonSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
