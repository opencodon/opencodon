...
import { getOpencodonConfigRecord, type OpencodonConfigRecord, saveOpencodonConfig } from '@/opencodon'
...
export interface I18nConfigClient {
  getConfig: () => Promise<OpencodonConfigRecord>
  saveConfig: (config: OpencodonConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.opencodonDesktop?.api) {
      return Promise.resolve({})
    }

    return getOpencodonConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.opencodonDesktop?.api) {
    ...
    return saveOpencodonConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: OpencodonConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: OpencodonConfigRecord, locale: Locale): OpencodonConfigRecord {
