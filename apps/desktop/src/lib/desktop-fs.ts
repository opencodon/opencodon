...
function connectionCacheKey(connection: OpencodonConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export async function readDesktopDir(path: string): Promise<OpencodonReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<OpencodonReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<OpencodonReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<OpencodonReadFileTextResult>(fsPath('read-text', path))
}
...
export async function selectDesktopPaths(options?: OpencodonSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
