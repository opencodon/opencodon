...
//   · its own OPENCODON_HOME      → its own backend + sessions, no shared state.
//   · its own --remote-debugging-port → a private CDP endpoint.
//   · OPENCODON_DESKTOP_BOOT_FAKE=1 → deterministic boot overlay.
...
// Seed an isolated OPENCODON_HOME with just enough config (NOT sessions) so the
// spawned instance reaches an empty chat view instead of the onboarding wizard.
// A separate OPENCODON_HOME dir means a separate gateway lock — no collision with
...
export async function startIsolatedInstance({
  ...
  try {
    ...
    // OPENCODON_HOME (backend + sessions). No DEV_SERVER env in prod → dist load.
    const electronBin = require('electron')
    // NB: do NOT set OPENCODON_DESKTOP_BOOT_FAKE here — it injects artificial
    ...
    if (devUrl) {
      env.OPENCODON_DESKTOP_DEV_SERVER = devUrl
    }
