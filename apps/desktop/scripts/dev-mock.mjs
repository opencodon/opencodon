...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `opencodon-dev-mock-${Date.now()}`))
  const opencodonHome = path.join(root, 'opencodon-home')
  const userDataDir = path.join(root, 'electron-user-data')
  fs.mkdirSync(opencodonHome, { recursive: true })
  fs.mkdirSync(userDataDir, { recursive: true })
  return { root, opencodonHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(opencodonHome, mockUrl) {
  fs.writeFileSync(
    path.join(opencodonHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(opencodonHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.opencodonHome, mock.url)
  console.log(`  OPENCODON_HOME: ${sandbox.opencodonHome}`)

  const electronBin = findElectron()

  const env = {
    ...process.env,
    OPENCODON_HOME: sandbox.opencodonHome,
    ...
    OPENCODON_DESKTOP_APP_NAME: `OpencodonDevMock-${Date.now()}`,
  }
