#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Opencodon icon + version metadata onto the
// built Opencodon.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Opencodon.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `opencodon desktop`, the installer's
...
// path (which rebuilds via `opencodon desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Opencodon.exe>
//
...
// Stamp the Opencodon icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    icon,
    'version-string': {
      ProductName: 'Opencodon',
      FileDescription: 'Opencodon',
      CompanyName: 'Nous Research',
      LegalCopyright: 'Copyright (c) 2026 Nous Research'
    }
  })

  console.log('[set-exe-identity] done — Opencodon icon + identity stamped')
}
