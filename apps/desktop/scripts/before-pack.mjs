 ...
 * product name (`Opencodon`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Opencodon'
 *
 * This is a hard failure with no obvious cause for the user — `opencodon desktop`
 ...
 * (the mac-unpacked Opencodon.app bundle) and Windows (win-unpacked), so we
