import type { RunExternalProcess } from '@opencodon/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`opencodon ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching opencodon: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`opencodon ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
