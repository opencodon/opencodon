...
import { terminalBackgroundHex } from '@opencodon/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $OPENCODON_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see OPENCODON_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['OPENCODON_TUI_BACKGROUND', process.env.OPENCODON_TUI_BACKGROUND ?? '(unset)'],
            ['OPENCODON_TUI_THEME', process.env.OPENCODON_TUI_THEME ?? '(unset)'],
