...
const logDir = join(process.env.OPENCODON_HOME?.trim() || join(homedir(), '.opencodon'), 'logs')
...
// a real ~/.opencodon (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('opencodon-tui: parent lifecycle log unavailable\n')
    }
  }
}
