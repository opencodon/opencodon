...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.OPENCODON_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Opencodon on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via OPENCODON_TUI_TRUECOLOR=1.
