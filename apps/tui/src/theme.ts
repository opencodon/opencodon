import type { SkinBranding, SkinColors } from '@opencodon/shared/skin'
...
// The classic light-mode Opencodon look was never hand-authored: for years the
 ...
 *  answer when known (cached in OPENCODON_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.OPENCODON_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit OPENCODON_TUI_THEME / COLORFGBG signals above still win,
...
// `OPENCODON_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `OPENCODON_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `OPENCODON_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `OPENCODON_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Opencodon palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.OPENCODON_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.OPENCODON_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.OPENCODON_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into OPENCODON_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into OPENCODON_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (Opencodon navy
  ...
  // the derived value already carries the identity seeds (e.g. Opencodon navy).
  const activeRow =
