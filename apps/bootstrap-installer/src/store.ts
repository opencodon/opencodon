...
export const $opencodonHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, opencodonHome, mode] = await Promise.all([
    ...
    $logPath.set(logPath)
    $opencodonHome.set(opencodonHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Opencodon" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Opencodon-Setup.exe --update);
...
export async function launchOpencodonDesktop(): Promise<void> {
  ...
  await invoke('launch_opencodon_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Opencodon repository' },
