...
pub fn opencodon_home() -> PathBuf {
    ...
    // doesn't panic.
    PathBuf::from(".opencodon")
}

pub fn log_dir() -> PathBuf {
    opencodon_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    opencodon_home().join("bootstrap-cache")
}
...
/// Electron desktop — which resolves OPENCODON_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    opencodon_home().join(".opencodon-update-in-progress")
}
...
pub fn copy_self_to_opencodon_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to OPENCODON_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_OPENCODON_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".opencodon-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under OPENCODON_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("OPENCODON_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_opencodon_home() -> String {
    opencodon_home().to_string_lossy().into_owned()
}
