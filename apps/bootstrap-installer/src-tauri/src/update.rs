...
//!
//! Driven when the installer is launched as `Opencodon-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Opencodon desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `opencodon update`
//!      or repair bootstrap can race locked files),
//!   2. run `opencodon update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in opencodon_cli/main.py),
//!   3. run `opencodon desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `opencodon update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_opencodon) and
...
/// `opencodon update` exit code meaning "another opencodon process is holding the
...
/// install tree before giving up and letting `opencodon update`'s own guard decide.
...
async fn run_update(app: AppHandle) -> Result<()> {
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_opencodon`
    ...
    let opencodon = resolve_opencodon(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the opencodon CLI under {}. Is Opencodon installed? \
    ...
    // async on Windows. If it still holds the venv shim, `opencodon update`
    ...
    // ---- stage 2: opencodon update -----------------------------------------
    // Pass --branch so `opencodon update` targets the branch this installer was
    ...
    // without --branch, `opencodon update` switches the checkout to `main` (a
    ...
    // --force skips `opencodon update`'s Windows running-exe guard (which would
    ...
    // time `opencodon update` runs there is no legitimate opencodon.exe to protect,
    // and the guard would only produce a false "Opencodon is still running" stop.
    //
    ...
    // probe only checks the opencodon.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all Opencodon windows" message.
    ...
    // Retry-once for the update-boundary crash. `opencodon update` lazily imports
    ...
    // second `opencodon update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Opencodon" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Opencodon is still running. Close all Opencodon windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "opencodon update failed (exit {:?}). See {} for details.",
                other,
    ...
    // ---- stage 3: opencodon desktop --build-only ----------------------------
    // `opencodon update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `opencodon update` already does above, and `opencodon update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `opencodon desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Opencodon manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_opencodon_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Opencodon to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend opencodon.exe (or the desktop Opencodon.exe
            ...
            // force-kill every Opencodon.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Opencodon still holding install files ({}); force-killing stragglers…",
                    format_locked_paths(&locked)
                ),
            );
            force_kill_other_opencodon();
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_opencodon(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Opencodon.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Opencodon.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `opencodon.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `opencodon.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\opencodon.exe update`. And a
/// desktop the user relaunches mid-update will NOT have spawned a backend —
/// `startOpencodon()` in the desktop gates local-backend startup on our
/// update-in-progress marker and parks until we finish (#50238). So the only
/// opencodon.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named opencodon.exe.)
fn force_kill_other_opencodon() {
    if !cfg!(target_os = "windows") {
        return;
    }
...
/// Spawn `opencodon <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv opencodon shim under an install root, regardless of existence.
fn venv_opencodon(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("opencodon.exe")
    } else {
        install_root.join("venv").join("bin").join("opencodon")
    }
}

/// Resolve the opencodon CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `opencodon` on PATH.
fn resolve_opencodon(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_opencodon(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "opencodon.exe" } else { "opencodon" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    ...
    // `opencodon update` is a Python CLI writing to a pipe here, so CPython
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_opencodon_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Opencodon.app was found under {}",
...
mod tests {
    ...
    #[test]
    fn venv_opencodon_is_under_install_root() {
        let root = Path::new("/x/opencodon-agent");
        let shim = venv_opencodon(root);
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/opencodon-agent"));
        assert!(
            envs.iter()
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_opencodon(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/opencodon-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Opencodon.app"]),
            Some(PathBuf::from("/Applications/Opencodon.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Opencodon.app");
        let tmp = base.join("Opencodon.app.opencodon-update-new");
        let old = base.join("Opencodon.app.opencodon-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Opencodon.app");
        let tmp = base.join("Opencodon.app.opencodon-update-new"); // intentionally absent
        let old = base.join("Opencodon.app.opencodon-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Opencodon.app");
        let tmp = base.join("Opencodon.app.opencodon-update-new"); // absent
        let old = base.join("Opencodon.app.opencodon-update-old");
        write_marker(&target, "OLD");
