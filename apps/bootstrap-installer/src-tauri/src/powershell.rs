...
///
/// `opencodon_home_override` propagates to the child as $OPENCODON_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, opencodon_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = opencodon_home_override {
        cmd.env("OPENCODON_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, opencodon_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = opencodon_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_opencodon_home() {
