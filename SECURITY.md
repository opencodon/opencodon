# opencodon Security Policy

This document describes opencodon's trust model, names the one
...
- Environment details (`opencodon version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted opencodon access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in opencodon's documentation
...
opencodon supports two OS-level isolation postures. They address
...
opencodon supports this in two ways:

- **opencodon's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, opencodon's in-process heuristics
...
opencodon filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in opencodon
itself. Bugs in opencodon's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   opencodon does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, opencodon's own documentation, or reasonable
  operator expectations would predict — including cases where
  opencodon has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  opencodon-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not opencodon's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in opencodon. Bugs in opencodon
...
- opencodon includes supply-chain guards for MCP server
