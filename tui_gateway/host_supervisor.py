...
def _default_registry_path() -> Path:
    return get_opencodon_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_opencodon_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_opencodon_home = expected_opencodon_home if expected_opencodon_home is not None else str(get_opencodon_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["OPENCODON_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("opencodon_home") or "")
        if got_home and got_home != self.expected_opencodon_home:
            raise RuntimeError(f"compute host OPENCODON_HOME mismatch: {got_home} != {self.expected_opencodon_home}")
