...
A profile = a full `~/.opencodon/profiles/<name>/` directory with its own:
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — OPENCODON_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with OPENCODON_HOME override |
...
### Seam #1 — hub-skill install cannot use the OPENCODON_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = OPENCODON_HOME / "skills"` at **module
import time**. The context-local `set_opencodon_home_override()` swap (which makes
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # synchronous, OPENCODON_HOME override
...
page afterward. Mirror `_write_profile_model`'s OPENCODON_HOME-override helper for
...
- Backend E2E with isolated OPENCODON_HOME: POST a full create body
...
- `npm run build --workspace @opencodon/web` (no JS test suite in apps/web/).
