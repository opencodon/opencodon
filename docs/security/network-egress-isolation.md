...
When running opencodon inside Docker, the default `network_mode: host` gives the
...
The opencodon [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   OPENCODON_UID=$(id -u) OPENCODON_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=opencodon,opencodon-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://opencodon-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — opencodon trust model and vulnerability reporting
