# opencodon Observer Hooks

opencodon observer hooks are the read-only telemetry contract for plugins that
...
They do not replace opencodon' planner, model providers, memory, tool registry,
...
```text
telemetry_schema_version = "opencodon.observer.v1"
```

Hook callbacks are fail-open. opencodon catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so opencodon only builds
