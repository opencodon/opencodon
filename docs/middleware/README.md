# opencodon Middleware

opencodon middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing opencodon'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before opencodon calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving opencodon retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving opencodon guardrails,
...
- `telemetry_schema_version`: currently `opencodon.observer.v1`
- `middleware_schema_version`: currently `opencodon.middleware.v1`
...
opencodon stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, opencodon runs
them as a nested chain in registration order. Middleware failures are fail-open:
opencodon logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, opencodon applies middleware in this order:
...
For each tool call, opencodon applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal opencodon pre-execution path against the effective arguments:
...
```bash
opencodon plugins enable <plugin-name>
```

For isolated local testing, use one `OPENCODON_HOME` for plugin enablement and the
agent run:

```bash
export OPENCODON_HOME=/tmp/opencodon-middleware-test
mkdir -p "$OPENCODON_HOME"
opencodon plugins enable <plugin-name>
opencodon chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run opencodon plugins enable <plugin-name>
uv run opencodon chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["opencodon_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/opencodon-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape opencodon expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, opencodon treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, opencodon preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. opencodon does not convert downstream
