 ...
 *
 * The root ``package.json`` is installed by ``opencodon update`` on every user,
 ...
 *   ``opencodon tools`` → Browser Automation → Camofox, and only used at
 *   runtime when ``CAMOFOX_URL`` is set. Its postinstall fetches a ~300MB
 *   Firefox-fork binary, which silently blocked ``opencodon update`` for
 ...
 * guidance in the ``opencodon-dev`` skill before changing the
 * expectations.
 */
...
test('camofox is not in root dependencies (must stay opt-in)', () => {
  ...
  assert.ok(
    ...
    'Camofox is a ~300MB binary-postinstall backend that must stay ' +
      ...
      'Camofox post_setup handler in opencodon_cli/tools_config.py so it ' +
      'only installs when the user explicitly selects Camofox via ' +
      '`opencodon tools` → Browser Automation → Camofox.'
  )
})
...
test('agent-browser stays eager (default backend)', () => {
  ...
  assert.ok(
    ...
    'agent-browser is the default browser-tool backend used by every ' +
      ...
      'after `opencodon setup` / `opencodon update` without an explicit ' +
      'post_setup step.'
  )
})
