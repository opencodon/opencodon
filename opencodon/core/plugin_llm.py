...
Plugins built on opencodon often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing opencodon
...
embedded slugs, no shorthands. This mirrors opencodon' main config
...
handles every provider, fallback chain, and per-task override opencodon
supports.
"""
...
def _resolve_attribution(
    *,
    ...
    if provider_override:
        ...
        try:
            from opencodon.core.auxiliary_client import _read_main_provider
    ...
    if isinstance(response_model, str) and response_model.strip():
        ...
        try:
            from opencodon.core.auxiliary_client import _read_main_model
...
class PluginLlm:
    ...
    async def _invoke_async(
        ...
        *,
        ...
        from opencodon.core.auxiliary_client import async_call_llm
        merged_extra = dict(extra_body or {})
        if profile_override:
            merged_extra.setdefault("metadata", {})["auth_profile"] = profile_override
