...
credentials at process startup, _after_ ~/.opencodon/.env has loaded.
...
Currently bundled:
  ...
  - ``bitwarden`` — Bitwarden Secrets Manager (`bws` CLI).  See
    ...
    ``opencodon_cli.secrets_cli`` for the user-facing setup wizard.
  - ``onepassword`` — 1Password ``op://`` secret references (`op` CLI).
    See ``agent.secret_sources.onepassword`` for the integration and
    ``opencodon_cli.onepassword_secrets_cli`` for the user-facing commands.
...
from opencodon.core.secret_sources.base import (  # noqa: F401
