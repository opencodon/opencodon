"""Lightweight internationalization (i18n) for opencodon static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by opencodon itself -- approval prompts, a handful of gateway slash
...
# Accept the common English locale tags; everything else falls back to "en".
...
def _locales_dir() -> Path:
    ...
    source_dir = Path(__file__).resolve().parents[2] / "locales"
    return source_dir
...
def _normalize_lang(value: Any) -> str:
    ...
    # Try stripping a region suffix (e.g. "en-GB" -> "en").
    base = key.split("-", 1)[0]
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a opencodon dependency
...
def reset_language_cache() -> None:
    ...
    Call after :func:`opencodon.config.save_config` if a running process
