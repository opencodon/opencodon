"""Language Server Protocol (LSP) integration for opencodon.

opencodon runs full language servers (pyright, gopls, rust-analyzer,
...
from opencodon.core.lsp.manager import LSPService

logger = logging.getLogger("opencodon.core.lsp")
...
def get_service() -> Optional[LSPService]:
    ...
    with _service_lock:
        ...
        if not _atexit_registered:
            ...
            # ``opencodon chat`` exit would leak pyright processes that
