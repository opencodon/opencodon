...
from opencodon_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "docker":
        ...
        # The opencodon Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``opencodon_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``OPENCODON_PLATFORM`` or
            ``OPENCODON_SESSION_PLATFORM`` env vars.  Returns the global
...
# which becomes the dominant cost of ``opencodon`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.opencodon/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``opencodon`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against OPENCODON_HOME, not cwd
        if not p.is_absolute():
            p = (opencodon_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.opencodon/skills/`` first, then external.
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.opencodon/skills/`` (including via symlinks) or configured
    ...
    # ~/.opencodon/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: opencodon can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    opencodon = metadata.get("opencodon") or {}
    if not isinstance(opencodon, dict):
        opencodon = {}
    return {
        "fallback_for_toolsets": opencodon.get("fallback_for_toolsets", []),
        "requires_toolsets": opencodon.get("requires_toolsets", []),
        "fallback_for_tools": opencodon.get("fallback_for_tools", []),
        "requires_tools": opencodon.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    opencodon = metadata.get("opencodon")
    if not isinstance(opencodon, dict):
        return []
    raw = opencodon.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes opencodon metadata, VCS, virtualenv/dependency, cache, and skill
