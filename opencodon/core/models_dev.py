...
# ---------------------------------------------------------------------------
# Provider ID mapping: opencodon ↔ models.dev
# ---------------------------------------------------------------------------

# opencodon provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → opencodon (built lazily)
...
def fetch_models_dev(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    When ``force_refresh=True`` (used by ``opencodon config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a opencodon provider ID to its models dict from models.dev.
...
def list_provider_models(provider: str) -> List[str]:
    ...
    """
    from opencodon.frontends.cli.models import normalize_provider
...
# Gemma models whose TPM quotas are too small for normal opencodon agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a opencodon provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve opencodon ID → models.dev ID
...
def get_model_info(
    ...
    Accepts opencodon or models.dev provider ID.  Tries exact match then
