...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into opencodon with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.opencodon/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of opencodon
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``opencodon tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``opencodon setup`` for
