...
def restore_primary_runtime(agent) -> bool:
    ...
    try:
        ...
        from opencodon.core.chat_completion_helpers import _reset_stale_streak
        _reset_stale_streak(agent)

        # Undo the fallback's identity rewrite so the prompt is
        # byte-identical to the stored copy again (prefix cache match).
        from opencodon.core.chat_completion_helpers import rewrite_prompt_model_identity
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    from opencodon.core.auxiliary_client import _validate_base_url, _validate_proxy_env_urls
    ...
    # Delegate all rate-limit / 5xx retry to opencodon's outer conversation loop,
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #
    # We do NOT drop the call: opencodon' own dispatch loop intentionally keeps an
...
def force_close_tcp_sockets(client: Any) -> int:
    ...
    different thread than the one driving the request:
      ...
      * BUT the SSL wrapper (``ssl.SSLSocket``'s underlying OpenSSL ``BIO``)
        ...
        another thread opening a SQLite database file.
