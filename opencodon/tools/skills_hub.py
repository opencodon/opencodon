#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the opencodon Skills Hub.
...
from opencodon.tools.skills_guard import (
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _opencodon_home() / "skills"
...
def _ssrf_safe_http_get(url: str, *, timeout: int = 20) -> httpx.Response:
    ...
    from opencodon.tools.url_safety import create_ssrf_safe_client
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            opencodon_meta = metadata.get("opencodon", {})
            if isinstance(opencodon_meta, dict):
                tags = opencodon_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            opencodon_meta = metadata.get("opencodon", {})
            if isinstance(opencodon_meta, dict):
                raw_tags = opencodon_meta.get("tags", [])
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                opencodon_meta = meta_block.get("opencodon", {})
                if isinstance(opencodon_meta, dict):
                    tags = opencodon_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# opencodon centralized index source
# ---------------------------------------------------------------------------
...
def _opencodon_index_cache_file() -> Path:
    return _index_cache_dir() / "opencodon-index.json"


def _load_opencodon_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for OPENCODON_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    opencodon_index_cache_file = _opencodon_index_cache_file()
    if opencodon_index_cache_file.exists():
        try:
            age = time.time() - opencodon_index_cache_file.stat().st_mtime
            if age < OPENCODON_INDEX_TTL:
                return json.loads(opencodon_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("opencodon index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "opencodon index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("opencodon index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        opencodon_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        opencodon_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class OpencodonIndexSource(SkillSource):
    """Skill source backed by the centralized opencodon Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_opencodon_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "opencodon-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "opencodon-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        OpencodonIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if _effective_filter == "all":
        for src in sources:
            if (src.source_id() == "opencodon-index"
