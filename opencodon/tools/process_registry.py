...
from opencodon.tools.environments.local import _find_shell, _resolve_safe_cwd, _sanitize_subprocess_env
...
CHECKPOINT_PATH = get_opencodon_home() / "processes.json"
...
class ProcessRegistry:
    ...
    def __init__(self):
        ...
        try:
            from opencodon.tools.async_delegation import restore_undelivered_completions
    ...
    def spawn_via_env(
        ...
        log_path = f"{temp_dir}/opencodon_bg_{session.id}.log"
        pid_path = f"{temp_dir}/opencodon_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/opencodon_bg_{session.id}.exit"
    ...
    def is_session_waiting(self, session_id: str) -> bool:
        ...
        Used by the goal-loop wait barrier (``opencodon_cli.goals``) to support
    ...
    def request_close_terminal(self, session_id: str) -> dict:
        ...
        if sink is None:
            return {
                "status": "error",
                "error": "close_terminal is only available in the opencodon desktop app.",
            }
...
def format_process_notification(evt: dict) -> "str | None":
    ...
    if _exit in {-15, 143, "-15", "143"}:
        _signal = ", SIGTERM"
    if _reason == "killed":
        _status = f"terminated by {_source or 'opencodon'}"
    elif _reason == "lost":
