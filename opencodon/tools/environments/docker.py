...
from opencodon.tools.environments.base import BaseEnvironment, _popen_bash
from opencodon.tools.environments.local import (
    _OPENCODON_PROVIDER_ENV_BLOCKLIST,
    _is_opencodon_internal_secret,
)
...
def _get_active_profile_name() -> str:
    """Return the active opencodon profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale opencodon-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=opencodon=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling opencodon process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=opencodon-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a opencodon process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed opencodon
    process leaves its container behind permanently because there's no
    subsequent opencodon process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=opencodon=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=opencodon-profile={_sanitize_label_value(profile_filter)}"])
...
#   SETUID/SETGID - the image's init drops from root to the 'opencodon'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``opencodon:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        from opencodon.tools.environments.base import get_sandbox_dir
        ...
        # s6-overlay images (e.g. opencodon:latest) already use /init as PID 1
        ...
        container_name = f"opencodon-{uuid.uuid4().hex[:8]}"
        # Labels make opencodon-created containers identifiable to:
        #   * the orphan reaper (`opencodon=1` for the global sweep filter)
        #   * future cross-process reuse (`opencodon-task-id`, `opencodon-profile`)
        #   * operators running `docker ps --filter label=opencodon=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active opencodon profile is captured at
        ...
        label_args = [
            "--label", "opencodon=1",
            "--label", f"opencodon-task-id={task_label}",
            "--label", f"opencodon-profile={profile_name}",
        ]
        ...
        self._labels = {
            "opencodon": "1",
            "opencodon-task-id": task_label,
            "opencodon-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior opencodon process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic opencodon secret blocklist. Only implicit passthrough
        # keys are filtered. Also strip opencodon-internal dynamic secrets
        # (AUXILIARY_*_API_KEY / _BASE_URL, GATEWAY_RELAY_* auth) that the
        # name-based blocklist doesn't cover — see _is_opencodon_internal_secret.
        _implicit_forward = {
            k for k in passthrough_keys if not _is_opencodon_internal_secret(k)
        }
        forward_keys = explicit_forward_keys | (_implicit_forward - _OPENCODON_PROVIDER_ENV_BLOCKLIST)
        opencodon_env = _load_opencodon_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = opencodon_env.get(key)
            if value:
                exec_env[key] = value
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("opencodon-task-id", "")
        profile_label = self._labels.get("opencodon-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"opencodon-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``opencodon-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=opencodon=1",
                    "--filter", f"label=opencodon-task-id={task_label}",
                    "--filter", f"label=opencodon-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
        ...
        # container) but can happen if a previous opencodon process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every opencodon exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        opencodon process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next opencodon startup. That covers the
        ...
        # outlives opencodon processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"opencodon-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``opencodon /quit``
