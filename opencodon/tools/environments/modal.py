...
wrapper, while preserving opencodon' persistent snapshot behavior across sessions.
"""
...
from opencodon.tools.environments.base import (
    BaseEnvironment,
...
_SNAPSHOT_STORE = get_opencodon_home() / "modal_snapshots.json"
...
class ModalEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        async def _create_sandbox(image_spec: Any):
            app = await _modal.App.lookup.aio("opencodon", create_if_missing=True)
    ...
    def _modal_bulk_download(self, dest: Path) -> None:
        """Download remote .opencodon/ as a tar archive.

        Modal sandboxes always run as root, so /root/.opencodon is hardcoded
        (consistent with iter_sync_files call on line 269).
        """
        async def _download():
            proc = await self._sandbox.exec.aio(
                "bash", "-c", "tar cf - -C / root/.opencodon"
            )
