...
# Audio file extensions opencodon recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through opencodon' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. opencodon-created Telegram private-chat
...
from opencodon_constants import get_default_opencodon_root, get_opencodon_dir, get_opencodon_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.opencodon/.env manually."
)
...
IMAGE_CACHE_DIR = get_opencodon_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_opencodon_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_opencodon_dir(new_subpath, old_name)
...
AUDIO_CACHE_DIR = get_opencodon_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_opencodon_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_opencodon_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_opencodon_dir("cache/screenshots", "browser_screenshots")
...
_OPENCODON_ROOT = get_default_opencodon_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "OPENCODON_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "OPENCODON_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "OPENCODON_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "OPENCODON_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _OPENCODON_HOME / "image_cache",
    _OPENCODON_HOME / "audio_cache",
    _OPENCODON_HOME / "video_cache",
    _OPENCODON_HOME / "document_cache",
    _OPENCODON_HOME / "browser_screenshots",
    ...
    _OPENCODON_HOME / "cache" / "images",
    _OPENCODON_HOME / "cache" / "audio",
    _OPENCODON_HOME / "cache" / "videos",
    _OPENCODON_HOME / "cache" / "documents",
    _OPENCODON_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared opencodon root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* OPENCODON_HOME's cache, so a gateway running at
    the root (e.g. ``OPENCODON_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when OPENCODON_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _OPENCODON_ROOT / "profiles"
    try:
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active opencodon profile and shared opencodon root both contain control
    ...
    # OPENCODON_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.opencodon
    ...
    for opencodon_root in (_OPENCODON_HOME, _OPENCODON_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(opencodon_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and opencodon
    secrets (``~/.opencodon/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``OPENCODON_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    opencodon-managed cache, under an operator-allowlisted root
    (``OPENCODON_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the opencodon root (~/.opencodon/.env,
    ...
    # ``MEDIA:~/.opencodon/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?opencodon\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+opencodon[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``OPENCODON_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Cross-OPENCODON_HOME token takeover is armed by GatewayRunner only for
        ...
        self._busy_text_mode: str = (
            os.environ.get("OPENCODON_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "OPENCODON_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "OPENCODON_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: opencodon sends
    ...
    def _write_runtime_status_safe(self, context: str, **kwargs) -> None:
        ...
        """
        try:
            from opencodon.frontends.gateway.status import write_runtime_status
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        A live cross-OPENCODON_HOME holder may be replaced only when the runner
    ...
    def _release_platform_lock(self) -> None:
        ...
        if not identity:
            return
        from opencodon.frontends.gateway.status import release_scoped_lock
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the opencodon home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the opencodon home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.opencodon/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from opencodon.frontends.cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          OPENCODON_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          OPENCODON_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          OPENCODON_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("OPENCODON_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("OPENCODON_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("OPENCODON_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if response:
                ...
                if text_content and not _tts_caption_delivered:
                    ...
                    if not is_ephemeral_response and not str(
                        ...
                        try:
                            from opencodon.frontends.gateway.delivery_ledger import (
            ...
            # not at the top of this task.  _opencodon_run_generation is set on
    ...
    def build_source(
        ...
        per-profile OPENCODON_HOME isolation.
        """
