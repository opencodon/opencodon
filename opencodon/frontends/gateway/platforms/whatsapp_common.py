...
class WhatsAppBehaviorMixin:
    ...
    def _matches_whatsapp_allowlist(candidate: str, allow_from) -> bool:
        ...
        from opencodon.frontends.gateway.whatsapp_identity import (
...
def resolve_whatsapp_bridge_dir() -> Path:
    ...
    install_bridge = _Path(__file__).resolve().parents[4] / "scripts" / "whatsapp-bridge"

    # Try OPENCODON_HOME location first
    opencodon_home = get_opencodon_home()
    opencodon_home_bridge = opencodon_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to OPENCODON_HOME if needed
    if opencodon_home_bridge.exists():
        return opencodon_home_bridge

    # Mirror the bridge source to OPENCODON_HOME
    try:
        opencodon_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            opencodon_home_bridge,
            dirs_exist_ok=False,
        )
        return opencodon_home_bridge
    except Exception:
