...
from opencodon.frontends.tui.transport import (
    StdioTransport,
    Transport,
    bind_transport,
    current_transport,
...
load_opencodon_dotenv(
    opencodon_home=_opencodon_home, project_env=Path(__file__).resolve().parents[3] / ".env"
)
...
from opencodon.frontends.tui.render import make_stream_renderer, render_diff, render_message
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from opencodon.frontends.cli.active_sessions import transfer_active_session
...
def _max_live_sessions() -> int:
    try:
        from opencodon.frontends.cli.active_sessions import coerce_max_concurrent_sessions
...
def _get_compute_host_supervisor(cfg: dict | None = None):
    ...
    with _compute_host_supervisor_lock:
        if _compute_host_supervisor is None:
            from opencodon.frontends.tui.host_supervisor import HostSupervisor
...
def _emit_approval_request(sid: str, data: dict | None) -> None:
    ...
    if "command" in payload:
        from opencodon.frontends.gateway.run import _redact_approval_command
...
def _clear_session_context(tokens: list) -> None:
    if not tokens:
        return
    try:
        from opencodon.frontends.gateway.session_context import clear_session_vars
...
def resolve_skin() -> dict:
    try:
        from opencodon.frontends.cli.skin_engine import init_skin_from_config, get_active_skin
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    if agent is not None and not (session or {}).get("_compute_host_active"):
        try:
            ...
            pass
        try:
            from opencodon.frontends.cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from opencodon.frontends.cli.banner import get_update_result
...
def _schedule_mcp_late_refresh(sid: str, agent) -> None:
    ...
    """
    try:
        from opencodon.frontends.tui.entry import mcp_discovery_in_flight, join_mcp_discovery
    except Exception:
        return
...
def _make_agent(
    ...
    from opencodon.frontends.tui.synthetic_turn import maybe_build_synthetic_agent
    ...
    try:
        ...
        pass
    try:
        from opencodon.frontends.tui.entry import wait_for_mcp_discovery
...
def _(rid, params: dict) -> dict:
    from opencodon.frontends.cli.input_sanitize import sanitize_user_prompt_text
    ...
    def run():
        ...
        try:
            ...
            if (
                ...
                try:
                    from opencodon.frontends.cli.voice import speak_text
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from opencodon.frontends.cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
...
def _allowed_image_extensions() -> frozenset[str]:
    try:
        from opencodon.frontends.cli.shell import _IMAGE_EXTENSIONS
...
def _(rid, params: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="pdf_attach_") as td:
        ...
        if raw_b64:
            ...
            try:
                from opencodon.frontends.cli.shell import _resolve_attachment_path
...
def _resolve_gateway_attachment_path(raw: str) -> Path | None:
    ...
    if not raw:
        return None
    try:
        from opencodon.frontends.cli.shell import _detect_file_drop, _resolve_attachment_path, _split_path_input
    except Exception:
        return None
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Opencodon Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Opencodon/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Opencodon Desktop. If it serves Opencodon/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from opencodon.frontends.cli.model_switch import parse_model_flags_detailed
...
def _build_project_tree(
    ...
    from opencodon.frontends.tui import project_tree
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from opencodon.frontends.cli.commands import (
            COMMAND_REGISTRY,
            SUBCOMMANDS,
            _build_description,
        )
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `opencodon` is interactive — use `/opencodon chat -q …` or run `opencodon` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`opencodon setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`opencodon gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`opencodon sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`opencodon config edit` needs $EDITOR in a real terminal"
    return None
...
def _(rid, params: dict) -> dict:
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "opencodon.frontends.cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from opencodon.frontends.cli.commands import SlashCommandCompleter
...
def _model_picker_context(agent):
    ...
    from opencodon.frontends.cli.inventory import load_picker_context
    ...
    try:
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `opencodon model` to configure",
            )
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "Opencodon" if role == "assistant" else role.title()
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        if not enabled:
            ...
            try:
                from opencodon.frontends.cli.voice import stop_continuous
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from opencodon.frontends.cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from opencodon.frontends.cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from opencodon.frontends.cli.browser_connect import DEFAULT_BROWSER_CDP_URL
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from opencodon.frontends.cli.skills_hub import do_install
        ...
        if action == "browse":
            from opencodon.frontends.cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from opencodon.frontends.cli.skills_hub import inspect_skill
