"""
opencodon MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (opencodon-specific extra)

Usage:
    opencodon mcp serve
    opencodon mcp serve --verbose
...
logger = logging.getLogger("opencodon.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using OPENCODON_HOME."""
    try:
        from opencodon_constants import get_opencodon_home
        return get_opencodon_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("OPENCODON_HOME", Path.home() / ".opencodon")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from opencodon_constants import get_opencodon_home
        directory_file = get_opencodon_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("OPENCODON_HOME", Path.home() / ".opencodon")
...
class EventBridge:
    ...
    This is the opencodon equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from opencodon_constants import get_opencodon_home
            db_file = get_opencodon_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("OPENCODON_HOME", Path.home() / ".opencodon")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the opencodon MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "opencodon",
        instructions=(
            "opencodon messaging bridge. Use these tools to interact with "
            "conversations across Telegram, Discord, Slack, WhatsApp, "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the opencodon MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
