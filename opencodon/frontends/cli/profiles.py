"""
Profile management for multiple isolated opencodon instances.

Each profile is a fully independent OPENCODON_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.opencodon/profiles/<name>/`` by default.

The "default" profile is ``~/.opencodon`` itself — backward compatible,
zero migration needed.

Usage::

    opencodon profile create coder          # fresh profile + bundled skills
    opencodon profile create coder --clone  # also copy config, .env, SOUL.md, skills
    opencodon profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    opencodon -p coder chat                 # or via flag
...
_PROFILE_DIRS = [
    ...
    # See opencodon_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.opencodon``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   opencodon  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `opencodon backup` archives
...
# Marker file written by `opencodon profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `opencodon
...
# `opencodon skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         opencodon infrastructure directories that only the default profile
         (``~/.opencodon``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_opencodon_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.opencodon) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "opencodon",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when OPENCODON_HOME equals the
...
# into the export. The set below identifies the *known opencodon profile
# artifacts* at the root of OPENCODON_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``opencodon_constants``.
...
_RESERVED_NAMES = frozenset({
    "opencodon", "default", "test", "tmp", "root", "sudo",
})

# opencodon subcommands that cannot be used as profile names/aliases
_OPENCODON_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the opencodon root, NOT to the current OPENCODON_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where OPENCODON_HOME points outside
    ``~/.opencodon``, profiles live under ``OPENCODON_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_opencodon_home() / "profiles"


def _get_default_opencodon_home() -> Path:
    """Return the default (pre-profile) OPENCODON_HOME path.

    In standard deployments this is ``~/.opencodon``.
    In Docker/custom deployments where OPENCODON_HOME is outside ``~/.opencodon``
    (e.g. ``/opt/data``), returns OPENCODON_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_opencodon_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the opencodon installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its OPENCODON_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_opencodon_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, opencodon subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _OPENCODON_SUBCOMMANDS:
        return f"'{canon}' conflicts with a opencodon subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            opencodon_exe = shutil.which("opencodon") or "opencodon"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(opencodon_exe)} -p {profile} "$@"\n')
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``opencodon -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``opencodon profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``opencodon -p X``)
...
class ProfileInfo:
    ...
    # installs). Lets other surfaces refer to a profile by role rather
    # than name alone.
    description: str = ""
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``opencodon profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``OPENCODON_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing opencodon config
...
# descriptions and just fall back to the profile name.
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_opencodon_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, opencodon_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own OPENCODON_HOME.
    ...
    The returned ``opencodon_home`` is the path to pass to
    ``set_opencodon_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_opencodon_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``opencodon update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.opencodon)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.opencodon/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `opencodon -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this opencodon profile.\n"
    ...
    # Write the opt-out marker so seed_profile_skills() and `opencodon update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`opencodon profile create --no-skills`).\n"
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `opencodon profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `opencodon -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches OPENCODON_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``opencodon profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "OPENCODON_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_opencodon_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running opencodon *backends* bound to this profile.
    ...
    names it, so find it by inspection: a opencodon backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``OPENCODON_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `opencodon -p <canon> profile
    ...
    opencodon_markers = ("opencodon_cli.main", "opencodon.frontends.cli.main", "opencodon-gateway", "opencodon.frontends.tui", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a opencodon process: either an entrypoint marker in argv, or
            # a resolved executable named `opencodon`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_opencodon = (
                any(marker in joined for marker in opencodon_markers)
                or exe_name == "opencodon"
                or exe_name.startswith("opencodon")
            )
            if not is_opencodon:
                continue
            ...
            # ...or by OPENCODON_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("OPENCODON_HOME", "")
...
def _stop_profile_backends(canon: str, profile_dir: Path) -> None:
    ...
    try:
        from opencodon.frontends.gateway.status import _pid_exists, terminate_pid as _terminate_pid
    except Exception:
        return
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.opencodon).\n"
            "To remove everything, use: opencodon uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_opencodon_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``OPENCODON_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from opencodon.frontends.cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set OPENCODON_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("OPENCODON_HOME")
    try:
        os.environ["OPENCODON_HOME"] = str(profile_dir)
        from opencodon.frontends.cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["OPENCODON_HOME"] = old_home
        elif "OPENCODON_HOME" in os.environ:
            del os.environ["OPENCODON_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.opencodon/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: opencodon profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from OPENCODON_HOME.

    Returns ``"default"`` if OPENCODON_HOME is not set or points to ``~/.opencodon``.
    Returns the profile name if OPENCODON_HOME points into ``~/.opencodon/profiles/<name>``.
    Returns ``"custom"`` if OPENCODON_HOME is set to an unrecognized path.
    """
    from opencodon_constants import get_opencodon_home
    opencodon_home = get_opencodon_home()
    resolved = opencodon_home.resolve()

    default_resolved = _get_default_opencodon_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      OPENCODON_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-opencodon file the
      user may have lying alongside OPENCODON_HOME (#58394).
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.opencodon itself — its parent is ~/ and its
        # directory name is ".opencodon", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: opencodon profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.opencodon itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.opencodon). "
            "Specify a different name: opencodon profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="opencodon_profile_import_") as tmpdir:
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a OPENCODON_HOME path string.

    Called early in the CLI entry point, before any opencodon modules
    are imported, to set the OPENCODON_HOME environment variable.
    """
