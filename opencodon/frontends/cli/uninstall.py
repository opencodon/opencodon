...
def remove_path_from_shell_configs():
    """Remove opencodon PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing opencodon or opencodon PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# opencodon" comment and following line
                if '# opencodon' in line or '# opencodon' in line:
                    skip_next = True
                    continue
                if skip_next and ('opencodon' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing opencodon
                if 'opencodon' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the opencodon wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "opencodon",
        Path("/usr/local/bin/opencodon"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains opencodon_cli reference)
                content = wrapper.read_text()
                if 'opencodon_cli' in content or 'opencodon' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(opencodon_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``opencodon`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this opencodon home's ``node``
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``opencodon gateway run`` processes
    ...
    try:
        from opencodon.frontends.cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from opencodon.frontends.cli.gateway import get_launchd_plist_path
...
#      like ``%LOCALAPPDATA%\opencodon\git\cmd``, ``%LOCALAPPDATA%\opencodon\git\bin``,
#      ``%LOCALAPPDATA%\opencodon\git\usr\bin``, ``%LOCALAPPDATA%\opencodon\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\opencodon\git\`` and Node to
#      ``%LOCALAPPDATA%\opencodon\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``opencodon dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\opencodon\gateway-service\`` and sometimes
...
def _opencodon_path_markers(opencodon_home: Path) -> list[str]:
    """Path-entry substrings that identify opencodon-owned User-PATH entries."""
    root = str(opencodon_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare opencodon install dir.
    markers = [root + "\\opencodon", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if OPENCODON_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "opencodon".  We don't want to catch
    # unrelated entries like "copencodon-foo" or "ephermeral", so we look for
    # backslash-opencodon as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(opencodon_home: Path) -> list[str]:
    """Strip opencodon-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _opencodon_path_markers(opencodon_home)
            kept: list[str] = []
...
def remove_opencodon_env_vars_windows() -> list[str]:
...
def remove_portable_tooling_windows(opencodon_home: Path) -> list[Path]:
    ...
    ``%LOCALAPPDATA%\\opencodon\\``.  Only called on full uninstall; they're
...
def _is_default_opencodon_home(opencodon_home: Path) -> bool:
    ...
    try:
        ...
        return opencodon_home.resolve() == get_default_opencodon_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    We shell out to ``opencodon -p <name> gateway stop|uninstall`` because
    ...
    #    Use `python -m opencodon_cli.main` so we don't depend on a `opencodon`
    #    wrapper that may be half-removed mid-uninstall.
    opencodon_invocation = [_sys.executable, "-m", "opencodon_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                opencodon_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
...
def run_gui_uninstall(args):
    ...
    Mirrors ``opencodon uninstall --gui``. Removes the desktop app's built
    ...
    """
    from opencodon.frontends.cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    print(color("│         ⚕ opencodon Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No opencodon Chat GUI installation was found.")
        print(f"  Checked: {opencodon_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The opencodon agent stays installed.", Colors.CYAN))
    print()
    ...
    if agent_is_installed(opencodon_home):
        ...
        print(f"  • The opencodon agent at {opencodon_home / 'opencodon'}")
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The opencodon agent is still installed. Run 'opencodon' to use the CLI,")
    print("or 'opencodon uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ opencodon Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL opencodon data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the opencodon code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, opencodon_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • opencodon PATH entries from shell configs / Windows User PATH")
    print("  • opencodon wrapper scripts and opencodon-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • opencodon config/data: {opencodon_home}")
        ...
        print(f"  • Keep opencodon config/data: {opencodon_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``opencodon`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    ...
    if _is_windows():
        ...
        # like C:\Users\<u>\AppData\Local\opencodon\git\cmd, not %LOCALAPPDATA%.
        ...
        if removed_path_entries:
            ...
            log_info("No opencodon-owned PATH entries in User environment")

        log_info("Removing OPENCODON_HOME / OPENCODON_GIT_BASH_PATH User env vars...")
        removed_env = remove_opencodon_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No opencodon-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing opencodon command...")
    ...
    #     (only when they still point into this opencodon home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing opencodon-managed node/npm/npx symlinks...")
    ...
    if removed_node_links:
        ...
        log_info("No opencodon-managed node/npm/npx symlinks found")
    ...
    try:
        from opencodon.frontends.cli.gui_uninstall import uninstall_gui
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.opencodon/, just remove the opencodon subdir
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/opencodon/opencodon/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/opencodon/opencodon/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using opencodon! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m opencodon_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import opencodon_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``opencodon_constants`` + ``opencodon_cli.colors``
    (and lazily ``opencodon_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m opencodon_cli.uninstall")
    parser.add_argument(
        "--mode",
