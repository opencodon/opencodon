...
_TASK_NAME_DEFAULT = "Opencodon_Gateway"
_TASK_DESCRIPTION = "opencodon Gateway - Messaging Platform Integration"
...
def _preserve_opencodon_home_path(path: str | Path) -> str:
    """Render opencodon-owned paths under the configured OPENCODON_HOME spelling.

    Windows installs may keep ``%LOCALAPPDATA%\\opencodon`` as a symlink/junction to
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current opencodon profile."""
    from opencodon.frontends.cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "opencodon.frontends.cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def get_task_name() -> str:
    ...
    Default profile: ``Opencodon_Gateway``
    Named profile X: ``Opencodon_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during opencodon_cli boot.
    from opencodon.frontends.cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\opencodon\\gateway-service\\<task_name>.cmd``
...
def _build_gateway_cmd_script(
    ...
    pythonpath_entries = [
        _preserve_opencodon_home_path(Path(__file__).resolve().parents[3]),
    ...
    prog_args = [pythonw_path, "-m", "opencodon.frontends.cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    repo_root = _preserve_opencodon_home_path(Path(__file__).resolve().parents[3])
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older opencodon builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "opencodon.frontends.cli.main"]
    if profile_arg:
...
def windowless_gateway_restart_spec(
    ...
    ``opencodon_cli`` package without the venv launcher's site config.

    Returns ``(new_argv, working_dir, env_overlay)``.  ``new_argv``
    preserves every argument after the interpreter (``-m opencodon_cli.main
    ...
    from opencodon.config import get_opencodon_home
    from opencodon.frontends.cli.gateway import PROJECT_ROOT
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m opencodon_cli.main gateway run``
...
def _prompt_install_choices(
    ...
    from opencodon.frontends.cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `opencodon -p <profile> gateway install` must be safe.
    ...
    from opencodon.frontends.cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"opencodon {profile_arg} gateway start" if profile_arg else "opencodon gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: opencodon gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated opencodon gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: opencodon gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: opencodon gateway start")
        _print_next_steps()
        return
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated opencodon gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: opencodon gateway status")
                    return
...
def _print_deep_probes() -> None:
    ...
    if lock_present:
        try:
            from opencodon.frontends.gateway.status import is_gateway_runtime_lock_active
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: opencodon gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: opencodon gateway start")
            return
...
def _drain_gateway_pid(pid: int, drain_timeout: float) -> bool:
    ...
    if pid <= 0:
        return False
    try:
        from opencodon.frontends.gateway.status import write_planned_stop_marker, _pid_exists
    except ImportError:
        return False
...
def _force_terminate_known_gateway_pids(pids: list[int]) -> int:
    ...
    try:
        from opencodon.frontends.gateway.status import _pid_exists, terminate_pid
    except ImportError:
        return 0
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `opencodon gateway status`."
        )
