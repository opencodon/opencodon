"""opencodon webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    opencodon webhook subscribe <name> [options]
...
def _setup_hint() -> str:
  ...
  Then start the gateway: opencodon gateway run
"""
...
def webhook_command(args):
    """Entry point for 'opencodon webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: opencodon webhook {subscribe|list|remove|test}")
        print("Run 'opencodon webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (opencodon gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: opencodon webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from opencodon webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (opencodon gateway run)")
