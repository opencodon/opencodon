"""
Baked-in build metadata for opencodon.

Source installs report their git revision live via ``git rev-parse`` (see
``opencodon_cli/dump.py`` and ``opencodon_cli/banner.py``).  That doesn't work inside
...
To make ``opencodon dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$OPENCODON_GIT_SHA``
arg into ``<project_root>/.opencodon_build_sha``.  This module is the single
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``OPENCODON_GIT_SHA`` build-arg fall through to live-git
...
_BUILD_SHA_FILE = Path(__file__).resolve().parents[3] / ".opencodon_build_sha"


def get_build_sha(short: int = 8) -> Optional[str]:
    """Return the baked-in build SHA, truncated to ``short`` chars, or None.

    Reads ``<project_root>/.opencodon_build_sha`` if present.  The file is
    written by the Dockerfile's ``OPENCODON_GIT_SHA`` build-arg and contains
