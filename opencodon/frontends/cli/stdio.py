...
on Japanese locales, etc.).  opencodon's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``opencodon_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. opencodon is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``OPENCODON_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("OPENCODON_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the opencodon-managed Git install directories so
    ...
    # already-running shells don't see the change — which means opencodon
    ...
    # install without opencodon-managed PortableGit).
    ...
    # stdin is re-configured for completeness; opencodon's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and opencodon's
    ``opencodon config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching opencodon (User env var in Windows Settings, or
    export in a PowerShell profile) and opencodon picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known opencodon-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\opencodon\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``opencodon`` from right after install) retain their old PATH.

    Any subprocess opencodon spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known opencodon-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "opencodon", "git", "cmd"),
        os.path.join(local_appdata, "opencodon", "git", "bin"),
        os.path.join(local_appdata, "opencodon", "git", "usr", "bin"),
        # opencodon venv Scripts directory — host of the opencodon.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes opencodon, but harmless to include.
        os.path.join(local_appdata, "opencodon", "opencodon", "venv", "Scripts"),
