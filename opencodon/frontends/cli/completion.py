"""Shell completion script generation for opencodon CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_opencodon_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# opencodon bash completion
# Add to ~/.bashrc:
#   eval "$(opencodon completion bash)"

_opencodon_profiles() {{
    local profiles_dir="$HOME/.opencodon/profiles"
...
_opencodon_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_opencodon_profiles)" -- "$cur"))
        return
    fi
...
complete -F _opencodon_completion opencodon
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef opencodon
# opencodon zsh completion
# Add to ~/.zshrc:
#   eval "$(opencodon completion zsh)"

_opencodon_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.opencodon/profiles" ]]; then
        profiles+=($HOME/.opencodon/profiles/*(N/:t))
    fi
...
_opencodon() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_opencodon_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'opencodon command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# opencodon fish completion",
        "# Add to your config:",
        "#   opencodon completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __opencodon_profiles",
        "    echo default",
        "    if test -d $HOME/.opencodon/profiles",
        "        for d in $HOME/.opencodon/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c opencodon -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c opencodon -f -s p -l profile"
        " -d 'Profile name' -xa '(__opencodon_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__opencodon_profiles)' -d 'Profile name'"
                )
