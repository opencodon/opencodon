"""MCP catalog — curated MCP servers shipped with the repo.
...
entries via ``opencodon mcp catalog`` or the interactive ``opencodon mcp picker``,
and install them with ``opencodon mcp install <name>`` (or by toggling in the
picker, which flows them through any required env/OAuth setup).

Catalog policy:
- Entries are added only by merging a PR into opencodon. Presence in the
  ``optional-mcps/`` directory = curated approval. No community tier, no trust
...
- Manifests pin transport details (commands, args, refs). Pins follow the
  ...
  auto-updated; users explicitly re-run ``opencodon mcp install <name>`` to
...
from opencodon.frontends.cli.cli_output import prompt as _prompt_input
...
def _catalog_root() -> Path:
    ...
    return get_optional_mcps_dir(Path(__file__).resolve().parents[3] / "optional-mcps")
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this opencodon understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their opencodon is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this opencodon
        understands. Update opencodon to install this entry.
...
def _build_server_config(
    ...
    by opencodon_cli/mcp_config.py."""
    cfg: dict = {}
    t = entry.transport
    if t.type == "stdio":
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    try:
        ...
        from opencodon.frontends.cli.mcp_config import _probe_single_server
...
def _apply_tool_selection(
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``opencodon mcp configure <name>``.
    """
    print()
    ...
    if probed is None:
        ...
        if manifest_default:
            ...
            print(color(
                ...
                f"Run `opencodon mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `opencodon mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `opencodon mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `opencodon mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `opencodon auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`opencodon auth {entry.auth.provider}` if you have not "
    ...
    from opencodon.frontends.cli.mcp_config import _save_mcp_server
    ...
    print(color(
        ...
        f"Start a new opencodon session to load its tools.",
        Colors.GREEN,
    ))
