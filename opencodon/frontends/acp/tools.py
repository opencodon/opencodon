"""ACP tool-call helpers for mapping opencodon tools to ACP ToolKind and building content."""
...
# ---------------------------------------------------------------------------
# Map opencodon tool names -> ACP ToolKind
# ---------------------------------------------------------------------------
...
def get_tool_kind(tool_name: str) -> ToolKind:
    """Return the ACP ToolKind for a opencodon tool, defaulting to 'other'."""
...
def _json_loads_maybe(value: Optional[str]) -> Any:
    ...
    # Some opencodon tools append a human hint after a JSON payload, e.g.
...
def _tool_result_failed(result: Optional[str], tool_name: str | None = None) -> bool:
    """Return True when a structured opencodon tool result clearly failed.
    ...
    # opencodon core/polished tools commonly report tool-level failures as a
...
def _format_read_file_result(result: Optional[str], args: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    # opencodon read_file output is line-numbered with `|`. If we send it as raw
...
def _build_tool_complete_content(
    ...
    *,
    ...
    if tool_name == "skill_manage":
        try:
            from opencodon.core.display import extract_edit_diff
...
def build_tool_start(
    ...
    *,
    ...
    """Create a ToolCallStart event for the given opencodon tool invocation.
