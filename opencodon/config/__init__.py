"""
Configuration management for opencodon.
...
- opencodon config          - Show current configuration
- opencodon config edit     - Open config in editor
- opencodon config get      - Print a resolved configuration value
- opencodon config set      - Set a specific value
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``opencodon config set`` (which rewrites
    ...
    ``config.yaml`` in place: opencodon never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``opencodon logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  opencodon config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` opencodon makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. opencodon itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``opencodon update``, the TUI build.
...
#   on every plugin install / ``opencodon update``.
...
#   ``OPENCODON_ENV`` — opencodon runtime location flags. Writing these into
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # opencodon runtime location — never via dashboard env writer.
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or opencodon runtime location "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `opencodon plugins enable
    # observability/langfuse` or `opencodon tools → Langfuse`); credentials gate
...
from opencodon.config.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def is_managed() -> bool:
    """Check if opencodon is running in package-manager-managed mode.
...
_NIX_UPDATE_MSG = (
    "Update opencodon through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    installs, ``/opt/opencodon`` inside the published image. It is a property of
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how opencodon was installed: 'docker', 'nix', 'nixos', 'git', or 'unknown'.
    ...
    ``docker`` and ``opencodon update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``opencodon/opencodon`` image bakes a ``docker``
        stamp into ``/opt/opencodon`` at build time.
    ...
    #    container, and honouring it wrongly blocks ``opencodon update``.
    try:
        method = (
...
def _running_in_container() -> bool:
    """Thin wrapper around ``opencodon_constants.is_container`` (import-safe)."""
    try:
        from opencodon.common.constants import is_container
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    ``/opt/opencodon`` in the published image, which instead bakes the stamp at
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method in {"nix", "nixos"}:
        return _NIX_UPDATE_MSG
    if method == "docker":
        return "docker pull opencodon/opencodon:latest"
    return "opencodon update"
...
# Long-form text for ``opencodon update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# opencodon_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side opencodon, it doesn't update
...
✗ ``opencodon update`` doesn't apply inside the Docker container.

opencodon runs as a published image (opencodon/opencodon), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull opencodon/opencodon:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate opencodon
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm opencodon/opencodon:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/opencodon/opencodon/tags
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``opencodon update`` inside Docker.
...
def format_managed_message(action: str = "modify this opencodon installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this opencodon installation is managed by NixOS "
    ...
    return (
        f"Cannot {action}: this opencodon installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall opencodon."
    )
...
def get_container_exec_info() -> Optional[dict]:
    ...
    Returns a dict with keys: backend, container_name, exec_user, opencodon_bin
    ...
    exec_user = info.get("exec_user", "opencodon")
    opencodon_bin = info.get("opencodon_bin", "/data/current-package/bin/opencodon")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "opencodon_bin": opencodon_bin,
    }
...
from opencodon.common.constants import get_opencodon_home, get_process_opencodon_home  # noqa: F811,E402
from opencodon.common.utils import atomic_replace, fast_safe_load
...
def _resolve_opencodon_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    Docker containers running opencodon commonly set these to map the in-container
...
def _chown_to_opencodon_uid(path) -> None:
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the opencodon group can
    ...
    created at runtime by spawned workers don't land as root:root and block
    subsequent uid-mapped runs).
    """
    if is_managed():
        return
    try:
    ...
    try:
        ...
        pass
    _chown_to_opencodon_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When opencodon runs in a container with volume-mounted config files, forcing
...
def ensure_opencodon_home():
    ...
    if key in _OPENCODON_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``opencodon profile create``).
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the opencodon-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps opencodon
        ...
        # app, ACP) in a code workspace, opencodon adds a coding operating brief
    ...
    "terminal": {
        ...
        # skipped. When empty, opencodon auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), opencodon sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when opencodon
        ...
        # bundled opencodon image, which drops to the `opencodon` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, opencodon sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and opencodon should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``opencodon chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: opencodon sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .opencodon.md, or .cursorrules before opencodon applies
    ...
    # single read_file page exceeds these limits, opencodon truncates the
    ...
    "compression": {
        ...
        "codex_gpt55_autoraise_notice": True,  # Display the one-time Codex gpt-5.4/5.5/5.6
                                      ...
                                      # to compact its own thread (default); opencodon =
                                      # opencodon' compression threshold triggers
    ...
    "auxiliary": {
        ...
        # ``opencodon profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `opencodon model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `opencodon` (and `opencodon chat`) launches by default:
        ...
        "interface": "cli",
        # When true, `opencodon --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `opencodon -c` muscle memory.  Default off so existing
        # users aren't surprised.  OPENCODON_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `opencodon --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/opencodon`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``--insecure`` is not). The bundled OAuth plugin reads
        ...
        "oauth": {
            "client_id": "",  # agent:{instance_id} — the IdP provisions this
            "portal_url": "",  # blank → use the plugin default
        },
        ...
        # the orchestrator at deploy time via the
        ...
        # optional path prefix (e.g. ``https://example.com/opencodon``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; opencodon appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
    ...
    # response. If not, opencodon feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before opencodon auto-pauses the goal and
    ...
    # cron daemon.
    #
    # See `opencodon curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `opencodon curator run --consolidate` overrides
        ...
        # normally restored on every `opencodon update`, so pruning them only
        ...
        # user can roll back with `opencodon curator rollback`.
        "backup": {
            "enabled": True,
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *opencodon*" header.
    ...
    # replace opencodon' built-in platform hint for a single messaging platform
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `opencodon doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``opencodon_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow opencodon to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $OPENCODON_HOME/plugins/<name>/) to relocate the trigger.
    ...
    "code_execution": {
        ...
        #     opencodon's own python (sys.executable). Maximum isolation
    ...
    #
    # Core opencodon tools (terminal, read_file, write_file, patch,
    ...
    # update model picker lists without shipping a opencodon release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://github.com/opencodon/opencodon/tree/main/docs",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `opencodon model` invocation; network failures
    ...
    "gateway": {
        ...
        # behavior: files must live under the opencodon cache, under
        ...
        # uploaded as native gateway attachments. Files inside the opencodon
    ...
    "sessions": {
        ...
        # default of ``opencodon sessions prune``.
        "retention_days": 90,
        ...
        # user runs `opencodon sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `opencodon update` prints a one-line notice with
    ...
    "updates": {
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``opencodon backup``-style zip
        #     of OPENCODON_HOME into <OPENCODON_HOME>/backups/, restorable with
        #     ``opencodon import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `opencodon update` does with uncommitted local changes to the
    ...
    # `opencodon tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # Bitwarden due to NETWORK/TIMEOUT, opencodon may use this encrypted
            ...
            # `opencodon secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
    ...
    "computer_use": {
        ...
        # by default upstream. opencodon disables it for our users unless they
        # explicitly opt in here. When false (default), opencodon sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; opencodon also calls
    ...
    # opencodon Desktop (Electron app) launch options. These only affect
    # `opencodon desktop`; they do not touch the CLI/gateway.
    "desktop": {
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials opencodon mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'opencodon model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit access token for tool-gateway requests (optional; otherwise read from the agent auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use opencodon, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'opencodon' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote opencodon API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote opencodon API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote opencodon API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `opencodon config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # should never break `opencodon update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but opencodon' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older opencodon versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``opencodon_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (opencodon_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (opencodon_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (opencodon_cli/setup.py)
    "known_plugin_toolsets", # written/read by opencodon_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'opencodon setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — opencodon won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, opencodon send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'opencodon doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_opencodon_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``opencodon config set``
    ...
    backends) — while display paths (``opencodon status``/``dump``) read ``name``
...
def atomic_config_write(config_path: Path, data: Any, **kwargs: Any) -> None:
    ...
    """
    from opencodon.common.utils import atomic_yaml_write
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/opencodon/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_opencodon_dotenv()
...
#   openai-codex (OAuth — opencodon auth) — OpenAI Codex
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset opencodon writes itself."""
...
def load_env() -> Dict[str, str]:
    ...
    interactive menu render (`opencodon tools`, `opencodon setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `opencodon tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real opencodon env var names, avoiding false positives from values
...
def reload_env() -> int:
    ...
    the .env file (but only vars known to opencodon — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known opencodon vars that are no longer in .env
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    ...
    Used for opencodon-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ opencodon Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                ...
                f"(run 'opencodon doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  opencodon config edit     # Edit config file", Colors.DIM))
    print(color("  opencodon config set <key> <value>", Colors.DIM))
    print(color("  opencodon setup           # Run setup wizard", Colors.DIM))
    print()
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``opencodon config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``opencodon config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            exposes this via ``opencodon config set --force``.
    """
    if is_managed():
    ...
    # so a fresh `opencodon config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `opencodon config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: opencodon config get <key> [--json]")
            print()
            print("Examples:")
            print("  opencodon config get model")
            print("  opencodon config get terminal.backend")
            print("  opencodon config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: opencodon config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  opencodon config set model anthropic/claude-sonnet-4")
            print("  opencodon config set terminal.backend docker")
            print("  opencodon config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: opencodon config unset <key>")
            print()
            print("Examples:")
            print("  opencodon config unset model")
            print("  opencodon config unset terminal.backend")
            print("  opencodon config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        print("  opencodon config           Show current configuration")
        print("  opencodon config edit      Open config in editor")
        print("  opencodon config get <key>          Print a resolved config value")
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    user_config_path = Path(get_opencodon_home()) / 'config.yaml'
    ...
    try:
        ...
        from opencodon.common.utils import atomic_roundtrip_yaml_update
...
# ``_inject_profile_env_vars`` surfaces them in ``opencodon config`` UI so users
