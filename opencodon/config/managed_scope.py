...
This is DISTINCT from ``opencodon.config.is_managed()`` / ``OPENCODON_MANAGED``,
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through opencodon.config.load_config) to honor managed
    scope. Mirrors opencodon.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from opencodon.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
