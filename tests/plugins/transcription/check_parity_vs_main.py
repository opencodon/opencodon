...
the legacy ``OPENCODON_LOCAL_STT_COMMAND`` escape hatch exists) or this PR's
...
def _resolve_main_dir() -> Path:
    ...
    if (candidate / "tools" / "transcription_tools.py").exists() and candidate != REPO_ROOT:
        return candidate
    sibling = REPO_ROOT.parent / "opencodon-main"
...
# Isolated OPENCODON_HOME so the config write is hermetic.
...
import opencodon.tools.transcription_tools as tt
...
def main() -> int:
    ...
    if MAIN_DIR == PR_DIR:
        print(
            ...
            "      Set up a sibling 'opencodon-main' checkout pinned to "
