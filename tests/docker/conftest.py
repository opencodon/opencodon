...
Override the image with ``OPENCODON_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("OPENCODON_TEST_IMAGE", "opencodon-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``OPENCODON_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("OPENCODON_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged opencodon user
# ---------------------------------------------------------------------------
#
# Background: every opencodon runtime path inside the container drops to UID
# 10000 (the ``opencodon`` user) via ``s6-setuidgid opencodon``. ``docker exec``
...
# fails (PermissionError swallowed) for opencodon, so a test that ran as root
...
# helpers below run every probe under ``-u opencodon`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: opencodon).
