...
Before the s6 migration, ``docker run opencodon/opencodon gateway
...
Every ``docker exec`` runs as ``opencodon`` per the conftest module
...
def _wait_for_gateway_or_exit(
    ...
    *,
    ...
    ``pgrep -f 'opencodon.*gateway'`` (the gateway is running) or
    ...
    while time.monotonic() < end:
        ...
        if status == "running":
            ...
            pgrep = docker_exec_sh(
                container, "pgrep -f 'opencodon.*gateway' >/dev/null 2>&1",
            )
...
def test_gateway_run_redirects_to_supervised(
    ...
    # process (main-wrapper.sh → python → opencodon gateway run) can take
    ...
    # (main-wrapper.sh → python → opencodon gateway run) can take well over
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``opencodon gateway run`` invocation
        (not ``sleep infinity``).
...
def test_supervised_gateway_does_not_recurse(
    ...
    supervised ``opencodon gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``opencodon gateway
    ...
    # (main-wrapper.sh → python → opencodon gateway run) can take well
    ...
    # started either — so we'd see the CMD's `opencodon gateway run`
    ...
    # `opencodon gateway run`. If the recursion guard fails, s6 would
    # respawn fresh `gateway run` processes on every cycle, leaving
    # multiple Python-process descendants under the gateway-default
    # supervise tree.
    r = docker_exec_sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*opencodon.*gateway run' | wc -l")
    assert r.returncode == 0
    n = int(r.stdout.strip() or 0)
    assert n <= 1, (
        f"expected at most one supervised python `opencodon gateway run` "
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    a poor user experience: the iconic "opencodon Gateway Starting…"
    ...
    # Poll docker logs for the banner glyph (⚕) or "opencodon Gateway
    ...
    assert "⚕" in combined or "opencodon Gateway Starting" in combined, (
    ...
    assert "⚕" in file_contents or "opencodon Gateway Starting" in file_contents, (
