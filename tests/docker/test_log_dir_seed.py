"""Runtime smoke test for Docker $OPENCODON_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the opencodon user after container boot.
...
registered later runs its log service as the dropped opencodon user and
...
def test_logs_gateways_seeded_and_opencodon_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by opencodon after boot."""
    ...
    # Both must be owned by opencodon
    r = docker_exec_sh(
        container_name,
    ...
    assert "logs=opencodon" in r.stdout, (
        f"logs/ not owned by opencodon: {r.stdout}"
    )
    assert "gateways=opencodon" in r.stdout, (
        f"logs/gateways/ not owned by opencodon: {r.stdout}"
    )
