...
only a subset of the npm monorepo workspaces (root/web/apps/tui, never apps/*),
so the actualized node_modules permanently disagrees with the canonical
package-lock.json. Without OPENCODON_TUI_DIR set, ``_make_tui_argv`` falls
...
The fix is ``ENV OPENCODON_TUI_DIR=/opt/opencodon/apps/tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the opencodon user, return stdout."""
    inner = (
        "source /opt/opencodon/.venv/bin/activate && "
        "cd /opt/opencodon && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the opencodon user (UID 10000) so we exercise the same path the
    # dashboard PTY child runs as — not root.
    cmd = [
        "docker", "run", "--rm", "--entrypoint", "su", image,
        "opencodon", "-s", "/bin/bash", "-c", inner,
    ]
...
def test_opencodon_tui_dir_env_is_set(built_image: str) -> None:
    """OPENCODON_TUI_DIR must point at the prebuilt bundle dir in the image."""
    r = subprocess.run(
        ["docker", "run", "--rm", "--entrypoint", "sh", built_image,
         "-c", 'printf "%s" "$OPENCODON_TUI_DIR"'],
        capture_output=True, text=True, timeout=60,
    )
    assert r.returncode == 0, r.stderr[-2000:]
    assert r.stdout.strip() == "/opt/opencodon/apps/tui", (
        f"OPENCODON_TUI_DIR={r.stdout.strip()!r} (expected /opt/opencodon/apps/tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    py = (
        ...
        "from opencodon.frontends.cli.main import _tui_need_npm_install, _find_bundled_tui, _make_tui_argv\n"
        "ui = Path('/opt/opencodon/apps/tui')\n"
    ...
    assert out["dist_entry_exists"], "prebuilt apps/tui/dist/entry.js missing from image"
    # With OPENCODON_TUI_DIR set, _make_tui_argv returns the prebuilt path BEFORE
