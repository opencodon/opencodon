...
is migrated by docker_config_migrate.py on boot, running as the opencodon
user.
"""
...
def test_config_migration_runs_on_boot(
    ...
    docker_config_migrate.py, running as the opencodon user."""
    # Start container
    ...
    r = docker_exec_sh(
        container_name,
        "test -f /opt/opencodon/scripts/docker_config_migrate.py && "
    ...
    # Verify config.yaml is owned by opencodon (migration ran as opencodon)
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/config.yaml',
        timeout=10,
    )
    assert r.stdout.strip() == "opencodon", (
        f"config.yaml not owned by opencodon (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
