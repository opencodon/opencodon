...
def test_model_flow_openrouter_clears_stale_custom_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon_cli.main._prompt_api_key",
        lambda *args, **kwargs: ("sk-openrouter", False),
    )
    monkeypatch.setattr(
        "opencodon_cli.models.model_ids",
        lambda **kwargs: ["anthropic/claude-sonnet-4.6"],
    )
    monkeypatch.setattr("opencodon_cli.models.get_pricing_for_provider", lambda *a, **k: {})
    monkeypatch.setattr(
        "opencodon_cli.auth._prompt_model_selection",
...
def test_model_flow_anthropic_clears_stale_custom_key_and_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon_cli.model_setup_flows._prompt_auth_credentials_choice",
        lambda title: "use",
    )
    monkeypatch.setattr(
        "opencodon_cli.auth._prompt_model_selection",
...
def test_auto_provider_name_localhost():
    from opencodon_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from opencodon_cli.main import _save_custom_provider
