...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. opencodon passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("OPENCODON_HOME", "/users/alice/.opencodon/profiles/backend-worker")
        monkeypatch.setenv("OPENCODON_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "OPENCODON_KANBAN_DB",
            "/users/alice/.opencodon/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.opencodon/kanban/boards/smoke"]'
            in cmd
        )
...
class TestSpawnEnvSecretStripping:
    """codex app-server routes its spawn env through opencodon_subprocess_env(
    ...
    credentials to authenticate, but it must NOT inherit Tier-1 opencodon secrets
    ...
    def test_tier1_and_internal_secrets_stripped_from_spawn_env(self, monkeypatch):
        ...
        for var in (
            ...
            "OPENCODON_DASHBOARD_SESSION_TOKEN", "AUXILIARY_VISION_API_KEY",
    ...
    def test_home_still_preserved_through_helper(self, monkeypatch):
        """Regression guard: routing through opencodon_subprocess_env must not
