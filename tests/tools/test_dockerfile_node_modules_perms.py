...
Older images made /opt/opencodon/ui-tui and /opt/opencodon/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV OPENCODON_TUI_DIR=/opt/opencodon/ui-tui" in text
    assert "cd ui-tui && npm run build" in text
    assert "chown -R opencodon:opencodon /opt/opencodon/ui-tui" not in text
