...
code paths opencodon takes on native Windows.
...
class TestConfigureWindowsStdio:
    """``opencodon_cli.stdio.configure_windows_stdio`` wiring.
...
class TestSigkillFallback:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("opencodon_cli.kanban_db", 'getattr(signal, "SIGKILL", signal.SIGTERM)'),
        ],
    )
...
class TestEntryPointsConfigureStdio:
    """cli.py, opencodon_cli/main.py, gateway/run.py must call configure_windows_stdio."""
    ...
    def test_entry_point_calls_configure_stdio(self, relpath):
        ...
        assert "configure_windows_stdio" in source, (
            f"{relpath} must call opencodon_cli.stdio.configure_windows_stdio() "
...
class TestSubprocessCompatHelpers:
    ...
    def test_windows_detach_flags_includes_breakaway_from_job(self, monkeypatch):
        ...
        Without it, the gateway-respawn watcher spawned by ``opencodon update``
        (which runs under opencodon-setup.exe, itself a grandchild of the
...
# ---------------------------------------------------------------------------
# opencodon_cli/kanban_db.py waitpid guard
# ---------------------------------------------------------------------------
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "opencodon_cli" / "kanban_db.py").read_text(encoding="utf-8")
