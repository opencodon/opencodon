...
SKILLS = pathlib.Path(__file__).resolve().parents[2] / "skills" / "science"
BRIDGE = pathlib.Path(__file__).resolve().parents[2] / "science" / "bridge.py"
...
def test_the_register_lists_every_guarded_entry_point():
    ...
    register = (
        pathlib.Path(__file__).resolve().parents[2] / "docs" / "science-skill-gaps.md"
    ).read_text()
