...
def _build_artifact(kind: str, tmp_path, *, nix_build: bool) -> subprocess.CompletedProcess[str]:
    ...
    if nix_build:
        env["OPENCODON_NIX_BUILD"] = "1"
    else:
        env.pop("OPENCODON_NIX_BUILD", None)
...
def test_artifact_build_rejects_nix_development_shell_environment(kind, tmp_path):
    ...
    assert "Building wheels or sdists for opencodon is not supported" in result.stderr


@pytest.mark.parametrize(
    ("kind", "artifact_glob"),
    [("sdist", "opencodon-*.tar.gz"), ("wheel", "opencodon-*.whl")],
)
