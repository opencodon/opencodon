...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"opencodon[termux]"' in text
    assert '"opencodon[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"opencodon[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
