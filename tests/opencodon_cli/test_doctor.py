"""Tests for opencodon_cli.doctor."""
...
import opencodon_cli.doctor as doctor
import opencodon_cli.gateway as gateway_cli
from opencodon_cli import doctor as doctor_mod
from opencodon_cli.doctor import _has_provider_env_config
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("OPENCODON_KANBAN_TASK", "probe")
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_opencodon_home(self, tmp_path, provider=""):
        """Create a minimal OPENCODON_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_opencodon_home(tmp_path, provider)
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp OPENCODON_HOME.

        ``run_doctor`` reads the module-level ``OPENCODON_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("OPENCODON_HOME")`` alone
        leaves doctor probing the REAL ~/.opencodon. On a dev machine with a
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from opencodon_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("opencodon_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "opencodon_cli.doctor", raising=False)
        from opencodon_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale OPENCODON_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        if ghost is not None:
            env_lines.append(f"OPENCODON_MAX_ITERATIONS={ghost}\n")
        (opencodon_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "OPENCODON_HOME", opencodon_home)
        monkeypatch.setattr(doctor_mod, "get_opencodon_home", lambda: opencodon_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("OPENCODON_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("OPENCODON_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale OPENCODON_MAX_ITERATIONS" in out
        env_after = (opencodon_home / ".env").read_text(encoding="utf-8")
        assert "OPENCODON_MAX_ITERATIONS" not in env_after
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_collect_deprecated_env_vars(self):
        ...
        assert "OPENCODON_TOOL_PROGRESS" in names
        assert "TERMINAL_CWD" in names
        assert "OPENAI_API_KEY" not in names
        by_name = dict(findings)
        assert "display.tool_progress" in by_name["OPENCODON_TOOL_PROGRESS"]
    ...
    def _run_doctor_with_config(self, monkeypatch, tmp_path, *, config_yaml: str, env_text: str = ""):
        opencodon_home = tmp_path / ".opencodon"
        opencodon_home.mkdir(parents=True)
        (opencodon_home / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (opencodon_home / ".env").write_text(env_body, encoding="utf-8")
...
"""
        out, opencodon_home = self._run_doctor_with_config(monkeypatch, tmp_path, config_yaml=cfg)
        ...
        on_disk = (opencodon_home / "config.yaml").read_text(encoding="utf-8")
...
"""
        ...
        assert "Deprecated: OPENCODON_TOOL_PROGRESS" in out
...
"""
        ...
        assert "Deprecated: OPENCODON_TOOL_PROGRESS" not in out
    ...
    def test_report_does_not_count_as_blocking_issue(self, monkeypatch, tmp_path, capsys):
        ...
        findings = doctor_mod.report_deprecated_config_and_env(
            ...
            {"OPENCODON_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: OPENCODON_TOOL_PROGRESS_MODE" in out
