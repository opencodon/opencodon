...
def isolated_kanban_home(monkeypatch):
    """Spin up a fresh OPENCODON_HOME with a clean kanban DB."""
    test_home = tempfile.mkdtemp(prefix="kanban_default_assignee_test_")
    monkeypatch.setenv("OPENCODON_HOME", test_home)
    # Force-reimport so the fresh OPENCODON_HOME is picked up.
    for mod in list(sys.modules.keys()):
        if mod.startswith("opencodon_cli") or mod.startswith("opencodon_state") or mod == "opencodon_constants":
            del sys.modules[mod]
    from opencodon_cli import kanban_db
    yield kanban_db, test_home
    # Cleanup is best-effort; tempfile dir survives but pytest isolation
    # gives each test its own monkeypatched OPENCODON_HOME so no cross-test
    # contamination.
...
def test_dry_run_with_default_assignee_reports_without_mutating(isolated_kanban_home):
    ...
    `opencodon kanban dispatch --dry-run` see the routing decision before
    committing."""
    kb, _home = isolated_kanban_home
    with kb.connect_closing() as conn:
        kb.create_board(slug="default", name="Test")
...
def test_dispatch_result_has_auto_assigned_default_field():
    ...
    from opencodon_cli.kanban_db import DispatchResult
    r = DispatchResult()
