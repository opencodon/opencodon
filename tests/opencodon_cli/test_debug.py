...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from opencodon_cli.debug import _upload_paste_rs
        ...
        with patch("opencodon_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "opencodon_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from opencodon_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from opencodon_cli.debug import upload_to_pastebin

        with patch("opencodon_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("opencodon_cli.debug._upload_paste_rs",
             ...
             patch("opencodon_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, opencodon_home):
        from opencodon_cli.debug import _capture_log_snapshot
    ...
    def test_race_truncate_after_resolve_reports_empty(self, opencodon_home, monkeypatch):
        ...
        from opencodon_cli import debug
...
class TestCaptureLogSnapshotRedaction:
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from opencodon_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, opencodon_home):
        from opencodon_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_survives_sweep_failure(self, opencodon_home, capsys):
        ...
        with patch("opencodon_cli.dump.run_dump"), \
             patch(
                 "opencodon_cli.debug._sweep_expired_pastes",
    ...
    def test_share_keeps_report_and_full_log_on_same_snapshot(self, opencodon_home, capsys):
        ...
        from opencodon_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("opencodon_cli.dump.run_dump"), \
             patch("opencodon_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("opencodon_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        ...
        with patch("opencodon_cli.dump.run_dump"), \
             patch("opencodon_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("opencodon_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from opencodon_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from opencodon_cli.debug import delete_paste
        ...
        with patch("opencodon_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    def test_does_not_spawn_subprocess(self, opencodon_home):
        ...
        import ast
        import inspect
        from opencodon_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, opencodon_home):
        ...
        from opencodon_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_merges_with_existing_pending(self, opencodon_home):
        ...
        from opencodon_cli.debug import _schedule_auto_delete, _load_pending
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, opencodon_home):
        from opencodon_cli.debug import _sweep_expired_pastes
    ...
    def test_sweep_deletes_expired_entries(self, opencodon_home):
        from opencodon_cli.debug import (
        ...
        with patch("opencodon_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, opencodon_home):
        from opencodon_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("opencodon_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, opencodon_home):
        ...
        with patch(
            "opencodon_cli.debug.delete_paste",
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, opencodon_home):
        ...
        with patch("opencodon_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)
...
class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from opencodon_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("opencodon_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("opencodon_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, opencodon_home, capsys):
        ...
        with patch("opencodon_cli.dump.run_dump"), \
             ...
             patch("opencodon_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, opencodon_home, capsys):
        ...
        with patch("opencodon_cli.dump.run_dump"), \
             patch("opencodon_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("opencodon_cli.debug._schedule_auto_delete"):
            run_debug_share(args)
...
class TestBuildDebugShare:
    ...
    def test_returns_structured_urls(self, opencodon_home):
        from opencodon_cli.debug import build_debug_share, DebugShareResult
        ...
        with patch("opencodon_cli.dump.run_dump"), patch(
            "opencodon_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("opencodon_cli.debug._schedule_auto_delete"):
    ...
    def test_skips_missing_logs_without_failure(self, opencodon_home):
        ...
        with patch("opencodon_cli.dump.run_dump"), patch(
            "opencodon_cli.debug.upload_to_pastebin",
...
class TestCollectShareBundle:
    def test_returns_report_and_logs(self, opencodon_home):
        from opencodon_cli.debug import collect_share_bundle
    ...
    def test_build_debug_share_uses_collector(self, opencodon_home):
        ...
        from opencodon_cli.debug import build_debug_share, collect_share_bundle
...
class TestDebugSlashCommand:
    ...
    def _captured(self, cmd_original):
        ...
        with patch("opencodon_cli.debug.run_debug_share", _fake_run):
...
class TestShareConsentGate:
    ...
    def test_aborts_on_user_decline(self, opencodon_home, capsys, monkeypatch):
        ...
        with patch("opencodon_cli.dump.run_dump"), \
             patch("opencodon_cli.debug.upload_to_pastebin") as mock_upload:
            run_debug_share(self._args())
