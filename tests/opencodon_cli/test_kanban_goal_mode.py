...
2. Spawn: _default_spawn sets the OPENCODON_KANBAN_GOAL_MODE env vars only
...
def test_spawn_sets_goal_env_only_when_enabled(kanban_home, monkeypatch):
    ...
    env = captured["env"]
    assert env.get("OPENCODON_KANBAN_GOAL_MODE") == "1"
    assert env.get("OPENCODON_KANBAN_GOAL_MAX_TURNS") == "5"
...
def test_spawn_no_goal_env_for_plain_task(kanban_home, monkeypatch):
    ...
    kb._default_spawn(task, str(kanban_home))
    env = captured["env"]
    assert "OPENCODON_KANBAN_GOAL_MODE" not in env
    assert "OPENCODON_KANBAN_GOAL_MAX_TURNS" not in env
...
class TestCLIJudgeGate:
    ...
    def _run(self, monkeypatch, *, goal_mode=True, judge_available=True,
        ...
        from opencodon_cli.kanban import _cmd_complete
        ...
        monkeypatch.setattr("opencodon_cli.kanban.kb.get_task", lambda conn, tid: fake_task)
        monkeypatch.setattr("opencodon_cli.kanban.kb.complete_task", fake_complete_task)
        monkeypatch.setattr("opencodon_cli.kanban.kb.connect_closing", fake_connect_closing)
        monkeypatch.setattr("opencodon_cli.kanban._worker_run_id_for", lambda _: None)
