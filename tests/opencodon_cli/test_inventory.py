...
from opencodon_cli.inventory import (
    ConfigContext,
...
def test_build_models_payload_does_not_call_provider_model_ids():
    ...
    ctx = _empty_ctx()
    with _list_auth_returning(rows), \
         patch("opencodon_cli.models.provider_model_ids") as mock_pm:
...
def test_build_models_payload_forwards_refresh_flag():
    ...
    with patch("opencodon_cli.model_switch.list_authenticated_providers", side_effect=_capture):
...
def test_list_authenticated_providers_refresh_busts_cache():
    ...
    from opencodon_cli import model_switch

    with patch("opencodon_cli.models.clear_provider_models_cache") as clear:
