...
def test_unknown_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("opencodon_cli.config.get_managed_system", return_value=None), \
         patch("opencodon_cli.config.get_hermes_home", return_value=tmp_path):
...
def test_home_scoped_retired_stamp_falls_back_to_unknown(tmp_path, retired_method):
    ...
    with patch("opencodon_cli.config.get_managed_system", return_value=None), \
         patch("opencodon_cli.config.get_hermes_home", return_value=home):
        from opencodon_cli.config import detect_install_method
...
def test_stamp_install_method_writes_code_scoped(tmp_path):
    ...
    code.mkdir()
    home.mkdir()
    with patch("opencodon_cli.config.get_hermes_home", return_value=home):
