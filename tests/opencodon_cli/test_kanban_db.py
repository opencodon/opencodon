"""Tests for the Kanban DB layer (opencodon_cli.kanban_db)."""
...
def test_connect_honors_kanban_busy_timeout_env(kanban_home, monkeypatch):
    ...
    """
    monkeypatch.setenv("OPENCODON_KANBAN_BUSY_TIMEOUT_MS", "123456")
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("OPENCODON_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import opencodon_cli.kanban_db as _kb
...
def test_detect_crashed_workers_skips_freshly_claimed_tasks(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.delenv("OPENCODON_KANBAN_CRASH_GRACE_SECONDS", raising=False)
...
def test_detect_crashed_workers_grace_period_env_override(
    ...
    """OPENCODON_KANBAN_CRASH_GRACE_SECONDS env var adjusts the window."""
    import opencodon_cli.kanban_db as _kb

    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("OPENCODON_KANBAN_CRASH_GRACE_SECONDS", "5")
...
def test_resolve_crash_grace_seconds_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv("OPENCODON_KANBAN_CRASH_GRACE_SECONDS", bad_val)
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("OPENCODON_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
def test_respawn_guard_rate_limit_cooldown_zero_allows_immediately(
    ...
    monkeypatch.setenv("OPENCODON_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "0")
    now = 6_000_000
...
def test_resolve_rate_limit_cooldown_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv(
            "OPENCODON_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", bad_val
        )
        assert (
...
def test_scratch_workspace_created_under_opencodon_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
        assert task is not None
...
def test_dispatch_worktree_task_persists_materialized_workspace_and_branch(kanban_home, tmp_path, monkeypatch):
    ...
    import opencodon_cli.profiles as profiles
...
def test_cleanup_workspace_honors_workspaces_root_env_override(tmp_path, monkeypatch):
    """``OPENCODON_KANBAN_WORKSPACES_ROOT`` extends the managed-scratch set.
    ...
    monkeypatch.setenv("OPENCODON_KANBAN_WORKSPACES_ROOT", str(workspaces_override))
    kb.init_db()
...
class TestSharedBoardPaths:
    ...
    must anchor at the **shared root**, not the active profile's OPENCODON_HOME."""

    def _set_home(self, monkeypatch, tmp_path, opencodon_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.opencodon/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.opencodon/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.opencodon/kanban.db.
        default_home = tmp_path / ".opencodon"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # OPENCODON_HOME and confirm equality. This is the property the
    ...
    def test_docker_custom_opencodon_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
    ...
    def test_explicit_override_via_opencodon_kanban_home(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OPENCODON_HOME", str(profile_home))
        monkeypatch.setenv("OPENCODON_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OPENCODON_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # OPENCODON_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile OPENCODON_HOME and reads.
        monkeypatch.setenv("OPENCODON_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_opencodon_kanban_db_pin_beats_kanban_home(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OPENCODON_HOME", str(default_home))
        monkeypatch.setenv("OPENCODON_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("OPENCODON_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows OPENCODON_KANBAN_HOME -- the pins
        # are independent.
    ...
    def test_opencodon_kanban_workspaces_root_pin_beats_kanban_home(
        ...
        monkeypatch.setenv("OPENCODON_HOME", str(default_home))
        monkeypatch.setenv("OPENCODON_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("OPENCODON_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows OPENCODON_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OPENCODON_HOME", str(default_home))
        monkeypatch.setenv("OPENCODON_KANBAN_DB", "   ")
        monkeypatch.setenv("OPENCODON_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject OPENCODON_KANBAN_DB
        # and OPENCODON_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites OPENCODON_HOME.
        default_home = tmp_path / ".opencodon"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["OPENCODON_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["OPENCODON_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["OPENCODON_KANBAN_TASK"] == "t_dispatch_env"
        assert env["OPENCODON_KANBAN_BRANCH"] == "wt/t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see opencodon_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    with _patch("opencodon_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="opencodon_state"):
            conn = kb.connect()
...
def test_resolve_opencodon_argv_module_actually_runs():
    ...
    sufficient — if `opencodon_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m opencodon_cli.main --version`
    ...
    assert "opencodon v" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="opencodon_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_healthy_commit_no_exception(tmp_path):
    ...
    from opencodon_cli.kanban_db import connect, write_txn
    db = tmp_path / "test.db"
...
def test_write_txn_raises_on_truncated_file(tmp_path):
    ...
    with pytest.raises(sqlite3.DatabaseError, match="torn-extend|page count mismatch"):
        with unittest.mock.patch("opencodon_cli.kanban_db.os.path.getsize", side_effect=fake_getsize):
            with write_txn(conn) as c:
                c.execute(
                    "INSERT INTO tasks (id, title, assignee, status, priority, created_at) "
...
def test_write_txn_post_commit_check_fires_every_call(tmp_path):
    ...
    from opencodon_cli.kanban_db import connect, write_txn
    import opencodon_cli.kanban_db as kanban_db_module
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
    call_count = 0
...
def test_connect_sets_wal_autocheckpoint_100(tmp_path):
    ...
    from opencodon_cli.kanban_db import connect
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    import struct
    from opencodon_cli.kanban_db import connect, _check_file_length_invariant
...
def test_reap_worker_zombies_returns_count():
    ...
    with patch("opencodon_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("opencodon_cli.kanban_db._record_worker_exit"):
...
def test_reap_worker_zombies_noop_on_windows(monkeypatch):
    ...
    monkeypatch.setattr("opencodon_cli.kanban_db.os.name", "nt")
    with patch("opencodon_cli.kanban_db.os.waitpid") as mock_waitpid:
...
def test_reap_worker_zombies_noop_no_children():
    ...
    with patch("opencodon_cli.kanban_db.os.waitpid", side_effect=ChildProcessError):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_reap_worker_zombies_records_exit_status():
    ...
    with patch("opencodon_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch(
            "opencodon_cli.kanban_db._record_worker_exit",
...
def test_reap_worker_zombies_handles_waitpid_os_error():
    ...
    with patch("opencodon_cli.kanban_db.os.waitpid", side_effect=OSError("test error")):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_zombie_reaper_survives_all_boards_failing():
    ...
    for tick in range(5):
        ...
        with patch(
            "opencodon_cli.kanban_db.os.waitpid", side_effect=make_fake_waitpid(pids)
        ):
            with patch("opencodon_cli.kanban_db._record_worker_exit"):
                pids = kb.reap_worker_zombies()
...
def test_dispatch_once_still_reaps_via_extracted_fn(kanban_home):
    ...
    with patch("opencodon_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("opencodon_cli.kanban_db._record_worker_exit"):
            with patch("opencodon_cli.kanban_db.os.name", "posix"):
                pids = kb.reap_worker_zombies()
