...
def _refresh_bindings_against_live_module():
    ...
    up pointing at the *old* module object.  ``patch(\"opencodon_cli.main.X\")``
    ...
    live = sys.modules.get("opencodon_cli.main")
    if live is None:
        live = importlib.import_module("opencodon_cli.main")
...
class TestKillStaleDashboardPosix:
    ...
    def test_no_stale_processes_is_a_noop(self, capsys):
        with patch("opencodon_cli.main._find_stale_dashboard_pids", return_value=[]):
            _kill_stale_dashboard_processes()
    ...
    def test_sigterm_graceful_exit(self, capsys):
        ...
        with patch("opencodon_cli.main._find_stale_dashboard_pids",
    ...
    def test_update_path_restarts_managed_dashboard_instead_of_killing(self, capsys):
        ...
        with patch("subprocess.run", side_effect=fake_run), \
             patch("opencodon_cli.main._find_stale_dashboard_pids",
    ...
    def test_user_scope_restart_never_falls_back_to_system_or_sudo(self, capsys):
        ...
        with patch("subprocess.run", side_effect=fake_run), \
             patch("opencodon_cli.main._find_stale_dashboard_pids", return_value=[12345]) as find_pids, \
             patch("os.kill") as kill:
            _kill_stale_dashboard_processes(restart_managed=True)
    ...
    def test_user_scope_restart_failure_does_not_try_system_or_sudo(self):
        ...
        with patch("subprocess.run", side_effect=fake_run), \
             patch("opencodon_cli.main._find_stale_dashboard_pids") as find_pids, \
             patch("os.kill") as kill:
            _kill_stale_dashboard_processes(restart_managed=True)
