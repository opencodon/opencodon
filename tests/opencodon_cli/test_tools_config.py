...
def test_get_platform_tools_x_search_off_when_no_xai_credentials(monkeypatch):
    ...
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "opencodon_cli.tools_config._xai_credentials_present", lambda: False
    )
...
def test_apply_toolset_change_from_default_does_not_enable_default_off_toolsets():
    ...
    with patch("opencodon_cli.tools_config.save_config"):
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("opencodon_cli.tools_config._run_post_setup", lambda key: None)
...
def test_fresh_install_browser_default_is_free_local_not_paid_nous():
    ...
    """
    from opencodon_cli.tools_config import _detect_active_provider_index
...
def test_reconfigure_lists_enabled_web_without_existing_provider_config(monkeypatch):
    ...
    monkeypatch.setattr("opencodon_cli.tools_config._prompt_choice", fake_prompt_choice)
    monkeypatch.setattr(
        "opencodon_cli.tools_config._configure_tool_category_for_reconfig",
...
def test_configure_all_platforms_configures_selected_tool_missing_provider(monkeypatch):
    ...
    monkeypatch.setattr("opencodon_cli.tools_config._prompt_choice", choose_by_label)
    monkeypatch.setattr(
        "opencodon_cli.tools_config._prompt_toolset_checklist",
        lambda *args, **kwargs: {"web"},
    )
    monkeypatch.setattr(
        "opencodon_cli.tools_config._toolset_needs_configuration_prompt",
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from opencodon_cli.tools_config import PLATFORMS
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from opencodon_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from opencodon_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from opencodon_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        ...
        from opencodon_cli.tools_config import TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from opencodon_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("opencodon_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("opencodon_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("opencodon_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_discord_toolsets_not_available_on_other_platforms():
    ...
    from opencodon_cli.tools_config import _toolset_allowed_for_platform
...
def test_save_platform_tools_strips_restricted_toolsets():
    ...
    from opencodon_cli.tools_config import _save_platform_tools
    config = {}
...
def test_reconfigure_provider_runs_post_setup_for_env_var_providers(
    ...
    called = []
    monkeypatch.setattr("opencodon_cli.tools_config._run_post_setup", lambda key: called.append(key))
    monkeypatch.setattr("opencodon_cli.tools_config.get_env_value", lambda k: None)
    monkeypatch.setattr("opencodon_cli.tools_config._prompt", lambda *a, **kw: "")
    monkeypatch.setattr("opencodon_cli.tools_config.save_env_value", lambda k, v: None)
...
def test_apply_provider_selection_web_sets_backend():
    ...
    from opencodon_cli.tools_config import apply_provider_selection
...
def test_vision_picker_writes_provider_and_model(tmp_path, monkeypatch):
    ...
    seq = iter([1, 0, 1])
    with patch("opencodon_cli.model_switch.list_authenticated_providers",
...
def test_provider_readiness_env_vars_gate_keys(monkeypatch):
    ...
    monkeypatch.setattr("opencodon_cli.tools_config.get_env_value", lambda key: None)
    assert provider_readiness_status(provider, {}) == "needs_keys"

    monkeypatch.setattr("opencodon_cli.tools_config.get_env_value", lambda key: "sk-x")
...
def test_provider_readiness_local_install_rows_track_module_presence(monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon_cli.tools_config._module_installed", lambda name: False
    )
    assert provider_readiness_status(kitten, {}) == "needs_setup"
    assert provider_readiness_status(piper, {}) == "needs_setup"

    monkeypatch.setattr(
        "opencodon_cli.tools_config._module_installed", lambda name: True
    )
...
def test_post_setup_no_window_flags_zero_on_posix(monkeypatch):
    ...
    from opencodon_cli.tools_config import _post_setup_no_window_flags
