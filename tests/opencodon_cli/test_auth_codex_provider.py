...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("opencodon_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def _patch_httpx(monkeypatch, response):
    ...
    monkeypatch.setattr("opencodon_cli.auth.httpx.Client", _factory)
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from opencodon_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from opencodon_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr(
        "opencodon_cli.auth._codex_device_code_login",
        lambda: {
    ...
    monkeypatch.setattr("opencodon_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("opencodon_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
...
def _patch_httpx_post(monkeypatch, responses):
    ...
    monkeypatch.setattr("opencodon_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
