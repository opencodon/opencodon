...
class TestBuildWebUISkipsWhenFresh:
    ...
    def test_runs_npm_when_dist_missing(self, tmp_path):
        ...
        with patch("opencodon_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("opencodon_cli.main.subprocess.run", return_value=mock_cp) as mock_run, \
             patch("opencodon_cli.main._run_with_idle_timeout", return_value=build_ok) as mock_idle:
            result = _build_web_ui(web_dir)
    ...
    def test_npm_install_uses_utf8_replace_output_decoding(self, tmp_path):
        ...
        with patch("opencodon_cli.main.subprocess.run", return_value=mock_cp) as mock_run:
    ...
    def test_npm_install_fallback_forces_include_dev_and_no_save(self, tmp_path):
        ...
        with patch("opencodon_cli.main.subprocess.run", side_effect=[ci_fail, install_ok]) as mock_run:
            result = _run_npm_install_deterministic("/usr/bin/npm", web_dir)
    ...
    def test_npm_install_uses_workspace_web_scope(self, tmp_path):
        ...
        with patch("opencodon_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("opencodon_cli.main.subprocess.run", return_value=mock_cp) as mock_run, \
             patch("opencodon_cli.main._run_with_idle_timeout", return_value=build_ok):
            result = _build_web_ui(web_dir)
        assert result is True
    ...
    def test_web_install_omits_workspace_when_web_has_own_lockfile(
        ...
        with patch("opencodon_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("opencodon_cli.main.subprocess.run", return_value=install_cp) as mock_run, \
             patch("opencodon_cli.main._run_with_idle_timeout", return_value=build_cp):
            result = _build_web_ui(web_dir)
    ...
    def test_web_build_uses_idle_timeout_helper(self, tmp_path):
        ...
        build_cp = __import__("subprocess").CompletedProcess([], 0, stdout="", stderr="")
        with patch("opencodon_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("opencodon_cli.main.subprocess.run", return_value=install_cp), \
             patch("opencodon_cli.main._run_with_idle_timeout", return_value=build_cp) as mock_idle:
            result = _build_web_ui(web_dir)
...
class TestBuildWebUIRetryAndStaleFallback:
    ...
    def test_retries_build_once_on_failure(self, tmp_path):
        ...
        with patch("opencodon_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("opencodon_cli.main._time.sleep") as mock_sleep, \
             patch("opencodon_cli.main.subprocess.run", return_value=install_ok), \
             patch("opencodon_cli.main._run_with_idle_timeout",
    ...
    def test_falls_back_to_stale_dist_when_retry_also_fails(self, tmp_path, capsys):
        ...
        with patch("opencodon_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("opencodon_cli.main._time.sleep"), \
             patch("opencodon_cli.main.subprocess.run", return_value=install_ok), \
             patch("opencodon_cli.main._run_with_idle_timeout",
...
class TestBuildWebUIFlock:
    ...
    def test_contended_lock_with_dist_serves_stale_without_building(self, tmp_path):
        import fcntl
        from opencodon_cli.main import _build_web_ui as build
        ...
        try:
            ...
            with patch("opencodon_cli.main._do_build_web_ui") as mock_do:
    ...
    def test_uncontended_lock_builds_and_creates_lock_file(self, tmp_path):
        ...
        web_dir, _ = _make_web_dir(tmp_path)
        with patch("opencodon_cli.main._do_build_web_ui", return_value=True) as mock_do:
            result = build(web_dir)
