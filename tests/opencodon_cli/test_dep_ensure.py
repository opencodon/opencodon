...
def test_ensure_dependency_skips_when_present():
    ...
    with patch("opencodon_cli.dep_ensure.shutil") as mock_shutil:
...
def test_ensure_dependency_returns_false_when_missing_noninteractive():
    ...
    with patch("opencodon_cli.dep_ensure.shutil") as mock_shutil:
        ...
        with patch("opencodon_cli.dep_ensure._find_install_script", return_value=(None, None)):
            result = ensure_dependency("node", interactive=False)
...
def test_find_install_script_from_checkout(tmp_path):
    ...
    from opencodon_cli.dep_ensure import _find_install_script
    scripts_dir = tmp_path / "scripts"
    scripts_dir.mkdir()
    (scripts_dir / "install.sh").write_text("#!/bin/bash", encoding="utf-8")
    with patch("opencodon_cli.dep_ensure._IS_WINDOWS", False):
...
def test_find_install_script_prefers_ps1_on_windows(tmp_path):
    ...
    from opencodon_cli.dep_ensure import _find_install_script
    with patch("opencodon_cli.dep_ensure._IS_WINDOWS", True):
...
def test_has_system_browser_checks_windows_names():
    from opencodon_cli.dep_ensure import _has_system_browser
    with patch("opencodon_cli.dep_ensure._IS_WINDOWS", True), \
         patch("opencodon_cli.dep_ensure.shutil") as mock_shutil:
...
def test_has_opencodon_agent_browser_windows_path(tmp_path):
    ...
    from opencodon_cli.dep_ensure import _has_opencodon_agent_browser
    with patch("opencodon_cli.dep_ensure._IS_WINDOWS", True), \
...
def test_ensure_dependency_uses_powershell_on_windows(tmp_path):
    ...
    (scripts_dir / "install.ps1").write_text("# fake")
    with patch("opencodon_cli.dep_ensure._IS_WINDOWS", True), \
         patch("opencodon_cli.dep_ensure._DEP_CHECKS", {"node": lambda: False}), \
         patch("opencodon_cli.dep_ensure._find_install_script", return_value=(scripts_dir / "install.ps1", "powershell")), \
         patch("opencodon_cli.dep_ensure.shutil") as mock_shutil, \
