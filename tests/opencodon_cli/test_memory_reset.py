"""Tests for the `opencodon memory reset` CLI command.
...
- Profile-scoped reset (uses OPENCODON_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake OPENCODON_HOME with memory files."""
    opencodon_home = tmp_path / ".opencodon"
    memories = opencodon_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("OPENCODON_HOME", str(opencodon_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nopencodon repo is at ~/.opencodon/opencodon\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
    ...
    return opencodon_home, memories
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `opencodon memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        opencodon_home = tmp_path / ".opencodon"
        (opencodon_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("OPENCODON_HOME", str(opencodon_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's OPENCODON_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_opencodon_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
