...
class TestVerifyConsoleScriptsInstalled:
    def test_no_action_when_all_shims_present(self, temp_pyproject, fake_scripts_dir):
        ...
        with patch("opencodon_cli.main._is_windows", return_value=True), \
             ...
             patch("opencodon_cli.main._run_quarantined_install") as mock_install:
            from opencodon_cli.main import _verify_console_scripts_installed
    ...
    def test_skips_off_windows(self, temp_pyproject, fake_scripts_dir):
        with patch("opencodon_cli.main._is_windows", return_value=False), \
             patch("opencodon_cli.main._run_quarantined_install") as mock_install:
            from opencodon_cli.main import _verify_console_scripts_installed
    ...
    def test_primary_install_success_still_verifies_scripts(self):
        ...
        with patch("opencodon_cli.main._is_windows", return_value=False), \
             patch("opencodon_cli.main._run_quarantined_install") as mock_install, \
             patch("opencodon_cli.main._verify_console_scripts_installed") as mock_verify:
