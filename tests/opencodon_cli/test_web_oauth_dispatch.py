...
from opencodon_cli.web_server import _SESSION_TOKEN, app
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "opencodon_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "opencodon_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "opencodon_cli.web_server._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_oauth_provider_status_uses_profile_query(tmp_path, monkeypatch):
    ...
    fake_catalog = ({
        ...
        "name": "Fake OAuth",
        "flow": "pkce",
        "cli_command": "opencodon auth add fake-oauth",
...
def test_codex_dashboard_worker_persists_runtime_provider(tmp_path, monkeypatch):
    from opencodon_cli import web_server as ws
    from opencodon_cli.auth import get_active_provider
...
def test_codex_dashboard_start_rewords_device_authorization_error(monkeypatch):
    ...
    try:
        ...
        assert "opencodon auth" not in detail
    finally:
...
def test_minimax_dashboard_poller_accepts_absolute_ms_expired_in():
    ...
    try:
        with patch(
            "opencodon_cli.auth._minimax_poll_token",
            return_value={
                "status": "success",
                "access_token": "access",
            ...
            "opencodon_cli.auth._minimax_save_auth_state",
...
def test_anthropic_pkce_branch_still_works():
    ...
    with patch(
        "opencodon_cli.web_server._start_anthropic_pkce",
...
def test_accounts_offers_every_oauth_provider_from_catalog():
    ...
    `opencodon model` universe) must be offered by /api/providers/oauth. This keeps
    ...
    for d in provider_catalog():
        if d.tab == "accounts":
            assert d.slug in offered, (
                f"{d.slug} is an accounts-tab provider in `opencodon model` but is "
...
def test_oauth_catalog_marks_external_providers_not_disconnectable():
    """External CLI credentials are visible in Accounts but cannot be removed by opencodon."""
    resp = client.get("/api/providers/oauth", headers=HEADERS)
    assert resp.status_code == 200, resp.text
    providers = {p["id"]: p for p in resp.json()["providers"]}
...
def test_xai_dashboard_poller_seeds_single_entry_and_clears_suppression(tmp_path, monkeypatch):
    ...
    # Prior `opencodon auth remove xai-oauth` left the source suppressed.
...
def test_unknown_pkce_provider_rejected_cleanly():
    ...
    fake_entry = {
        ...
        "flow": "pkce",
        "cli_command": "opencodon auth add hypothetical-pkce-provider",
        "docs_url": "https://example.com",
        "status_fn": None,
    }
...
def test_status_falls_through_to_generic_dispatcher_for_catalog_only_provider():
    ...
    with patch("opencodon_cli.auth.get_auth_status", return_value=fake_status):
...
def test_status_unknown_provider_degrades_to_logged_out():
    ...
    with patch("opencodon_cli.auth.get_auth_status", return_value={"logged_in": False}):
