...
def test_register_calls_through_on_s6(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    mgr = _S6Manager()
    monkeypatch.setattr(
        "opencodon_cli.service_manager.get_service_manager", lambda: mgr,
    )
...
def test_register_swallows_no_backend_runtime_error(
    ...
    monkeypatch.setattr(
        "opencodon_cli.service_manager.get_service_manager", _no_backend,
    )
    # Should NOT raise
...
def test_register_silent_when_detect_throws(
    ...
    monkeypatch.setattr(
        "opencodon_cli.service_manager.detect_service_manager", _broken_detect,
    )
