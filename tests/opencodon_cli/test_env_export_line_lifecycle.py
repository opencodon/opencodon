...
``~/.opencodon/.env``. ``load_env()`` parses the export prefix (#6659), so every
...
def test_classic_pat_save_via_endpoint_succeeds(opencodon_home):
...
def test_remove_export_prefixed_token(opencodon_home):
    """DELETE must clear an ``export KEY=...`` line, not 404 on it."""
    _write_env_raw(opencodon_home, f"export GITHUB_TOKEN={OLD_PAT}\n")
...
def test_update_export_prefixed_token_does_not_duplicate(opencodon_home):
...
def test_export_line_with_comment_untouched(opencodon_home):
