...
from opencodon_cli.copilot_auth import _try_gh_cli_token
...
from opencodon_cli.auth import PROVIDER_REGISTRY as _REGISTRY
...
def _clear_provider_env(monkeypatch):
    ...
    monkeypatch.setattr("opencodon_cli.auth._load_auth_store", lambda: {})
...
class TestApiKeyProviderStatus:
    ...
    def test_copilot_status_uses_gh_cli_token(self, monkeypatch):
        monkeypatch.setattr("opencodon_cli.copilot_auth._try_gh_cli_token", lambda: "gho_gh_cli_token")
    ...
    def test_get_auth_status_dispatches_to_external_process(self, monkeypatch):
        monkeypatch.setattr("opencodon_cli.auth.shutil.which", lambda command: f"/opt/bin/{command}")
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_resolve_zai_with_key(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon_cli.auth.detect_zai_endpoint", lambda *a, **kw: None)
    ...
    def test_try_gh_cli_token_uses_homebrew_path_when_not_on_path(self, monkeypatch):
        monkeypatch.setattr("opencodon_cli.copilot_auth.shutil.which", lambda command: None)
        monkeypatch.setattr(
            "opencodon_cli.copilot_auth.os.path.isfile",
            lambda path: path == "/opt/homebrew/bin/gh",
        )
        monkeypatch.setattr(
            "opencodon_cli.copilot_auth.os.access",
        ...
        monkeypatch.setattr("opencodon_cli.copilot_auth.subprocess.run", _fake_run)
...
class TestRuntimeProviderResolution:
    ...
    def test_runtime_copilot_uses_responses_for_gpt_5_4(self, monkeypatch):
        monkeypatch.setattr("opencodon_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        monkeypatch.setattr(
            "opencodon_cli.runtime_provider._get_model_config",
...
class TestZaiEndpointAutoDetect:
    ...
    def test_probe_success_returns_detected_url(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "opencodon_cli.auth.detect_zai_endpoint",
            lambda *a, **kw: {
                "id": "coding-global",
    ...
    def test_env_override_skips_probe(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon_cli.auth.detect_zai_endpoint", _never_called)
        creds = resolve_api_key_provider_credentials("zai")
...
class TestKimiMoonshotModelListIsolation:
    ...
    def test_moonshot_list_excludes_coding_plan_only_models(self):
        from opencodon_cli.main import _PROVIDER_MODELS
...
class TestHuggingFaceModels:
    ...
    def test_model_lists_match(self):
        ...
        from opencodon_cli.main import _PROVIDER_MODELS as main_models
        from opencodon_cli.models import _PROVIDER_MODELS as models_models
...
class TestFetchDeepInfraModels:
    ...
    def test_returns_filtered_models_on_success(self, monkeypatch):
        ...
        from opencodon_cli.models import _fetch_deepinfra_models
...
class TestDeepInfraTagFiltering:
    ...
    def test_filters_by_surface_tag_and_handles_rollout_states(self, monkeypatch):
        ...
        import opencodon_cli.models as _m
    ...
    def test_returns_none_on_network_failure(self, monkeypatch):
        ...
        from opencodon_cli.models import _fetch_deepinfra_models_by_tag, _fetch_deepinfra_pricing
