...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hermes` should use `opencodon_cli.main`, not the legacy Fire wrapper."""
    ...
    def fake_main():
        called.append("opencodon_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "opencodon_cli.main", fake_main_module)
    ...
    assert called == ["opencodon_cli.main"]
