...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon_cli.auth._minimax_oauth_login",
        lambda **kwargs: {
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon_cli.auth._xai_oauth_device_code_login",
        lambda **kwargs: {
            "tokens": {
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from opencodon_cli.auth_commands import auth_reset_command
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from opencodon_cli.auth import _reset_config_provider
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from opencodon_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "opencodon_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("opencodon_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("opencodon_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("opencodon_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "opencodon_cli.auth_commands.time.time",
...
def test_seed_from_singletons_respects_qwen_suppression(tmp_path, monkeypatch):
    ...
    import opencodon_cli.auth as ha
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    with patch(
        ...
        "opencodon_cli.copilot_auth.get_copilot_api_token",
