...
import opencodon_cli.gateway as gateway
import opencodon_cli.gateway_windows as gateway_windows
import opencodon_cli.setup as setup
...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("opencodon_cli.config.get_opencodon_home", lambda: str(opencodon_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(base_pythonw), "-m", "opencodon_cli.main"]
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_opencodon_home(self, tmp_path, monkeypatch):
        ...
        home.mkdir()
        monkeypatch.setattr("opencodon_cli.config.get_opencodon_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("opencodon_cli.config.get_opencodon_home", lambda: missing)
...
def test_gateway_vbs_script_is_console_less(monkeypatch):
    ...
    assert "pythonw.exe" in content
    assert "opencodon_cli.main" in content
    assert "gateway run" in content
...
def test_stop_writes_planned_stop_marker_before_killing(monkeypatch):
    ...
    monkeypatch.setattr("opencodon_cli.gateway.kill_gateway_processes", fake_kill)
    monkeypatch.setattr("opencodon_cli.gateway._get_restart_drain_timeout", lambda: 5.0)
