...
Complements tests/opencodon_cli/test_kanban_db.py (schema + CAS atomicity)
and tests/opencodon_cli/test_kanban_cli.py (end-to-end run_slash).  The
...
def test_parse_duration_accepts_formats():
    from opencodon_cli.kanban import _parse_duration
...
def test_cli_bulk_complete_with_summary_rejects(kanban_home):
    ...
    # Note: opencodon_cli.main doesn't propagate sub-command exit codes
...
def _drive_worker_exit(conn, tid, fake_pid, raw_status):
    ...
    Deliberately resolves ``opencodon_cli.kanban_db`` fresh and uses that single
