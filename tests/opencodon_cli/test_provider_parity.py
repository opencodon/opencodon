...
the SAME provider universe as ``opencodon model`` (the CLI/TUI picker).
...
def test_every_opencodon_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `opencodon model` universe."""
    ...
    assert not missing, (
        "providers shown in `opencodon model` but not configurable in the desktop "
