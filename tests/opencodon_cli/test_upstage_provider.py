...
Regression guard for the bug where `opencodon model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`opencodon_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageResolver:
    ...
    def test_solar_alias_normalizes_to_upstage(self):
        from opencodon_cli.providers import normalize_provider, resolve_provider_full
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in OPENCODON_OVERLAYS
        overlay = OPENCODON_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `opencodon_cli/main.py` (cmd_model): read
