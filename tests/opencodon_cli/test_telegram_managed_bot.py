"""Tests for opencodon_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
class TestCreatePairing:
    def test_success(self):
        ...
        with patch(
            "opencodon_cli.telegram_managed_bot.httpx.post", return_value=mock_resp
        ) as post:
...
class TestPollForToken:
    ...
    def test_immediate_success(self):
        ...
        with patch(
            "opencodon_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("opencodon_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_setup_result_includes_owner_user_id(self):
        ...
        with patch("opencodon_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("opencodon_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_invalid_ready_token_returns_none(self):
        ...
        with patch("opencodon_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("opencodon_cli.telegram_managed_bot.time.sleep"):
                with patch(
                    "opencodon_cli.telegram_managed_bot.time.monotonic"
                ) as mock_time:
    ...
    def test_eventual_success(self):
        ...
        with patch("opencodon_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("opencodon_cli.telegram_managed_bot.time.sleep"):
                token = poll_for_token(
...
class TestSetupTelegramAuto:
    def test_setup_helper_exists(self):
        from opencodon_cli.setup import _setup_telegram_auto
    ...
    def test_setup_result_passes_profile_name_for_profile_home(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "opencodon_cli.telegram_managed_bot.auto_setup_telegram_bot_result",
