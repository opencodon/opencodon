...
def test_detect_service_manager_s6_keys_off_s6_running_not_is_container(
    ...
    monkeypatch.setattr(
        "opencodon_cli.service_manager._s6_running", lambda: True,
    )
...
def test_s6_running_true_when_comm_and_basedir_match(
    ...
    from opencodon_cli.service_manager import _s6_running
...
def test_systemd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "opencodon_cli.gateway.systemd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "opencodon_cli.gateway.systemd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "opencodon_cli.gateway.systemd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "opencodon_cli.gateway._probe_systemd_service_running",
...
def test_launchd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "opencodon_cli.gateway.launchd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "opencodon_cli.gateway.launchd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "opencodon_cli.gateway.launchd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "opencodon_cli.gateway._probe_launchd_service_running", lambda: False,
    )
    mgr = LaunchdServiceManager()
    mgr.start("ignored")
    mgr.stop("ignored")
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    import opencodon_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("opencodon_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
        "opencodon_cli.gateway.find_gateway_pids",
        lambda **kw: [12345],
    )
    mgr = WindowsServiceManager()
    mgr.start("ignored")
...
def test_get_service_manager_returns_correct_backend(
    ...
    monkeypatch.setattr(
        "opencodon_cli.service_manager.detect_service_manager", lambda: kind,
    )
...
def test_get_service_manager_raises_when_unsupported(
    ...
    monkeypatch.setattr(
        "opencodon_cli.service_manager.detect_service_manager", lambda: "none",
    )
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from opencodon_cli.service_manager import _seed_supervise_skeleton
...
def test_s6_register_staging_dir_is_dotfile_hidden_from_svscan(
    ...
    """
    import opencodon_cli.service_manager as sm
