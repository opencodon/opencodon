"""
Tests for timezone support (opencodon_time module + integration points).
...
def _reset_opencodon_time_cache():
    """Reset the opencodon_time module cache (replacement for removed reset_cache)."""
    opencodon_time._cached_tz = None
    opencodon_time._cached_tz_name = None
    opencodon_time._cache_resolved = False


# =========================================================================
# opencodon_time.now() — core helper
# =========================================================================

class TestOpencodonTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_opencodon_time_cache()

    def teardown_method(self):
        _reset_opencodon_time_cache()
        os.environ.pop("OPENCODON_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["OPENCODON_TIMEZONE"] = "Asia/Kolkata"
        result = opencodon_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["OPENCODON_TIMEZONE"] = "UTC"
        result = opencodon_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["OPENCODON_TIMEZONE"] = "America/New_York"
        result = opencodon_time.now()
        assert result.tzinfo is not None
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["OPENCODON_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="opencodon_time"):
            result = opencodon_time.now()
    ...
    def test_cache_invalidation(self):
        ...
        os.environ["OPENCODON_TIMEZONE"] = "UTC"
        _reset_opencodon_time_cache()
        r1 = opencodon_time.now()
        assert r1.utcoffset() == timedelta(0)

        os.environ["OPENCODON_TIMEZONE"] = "Asia/Kolkata"
        _reset_opencodon_time_cache()
        r2 = opencodon_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["OPENCODON_TIMEZONE"] = "Europe/London"
        tz = opencodon_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["OPENCODON_TIMEZONE"] = "Not/A/Timezone"
        tz = opencodon_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def _import_execute_code(self, monkeypatch):
        ...
        try:
            from opencodon.tools.code_execution_tool import execute_code
    ...
    def test_tz_injected_when_configured(self):
        """When OPENCODON_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) OPENCODON_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("OPENCODON_TIMEZONE=" + os.environ.get("OPENCODON_TIMEZONE", "NOT_SET"))'
        )
        with patch("opencodon.tools.model_tools.handle_function_call", side_effect=self._mock_handle):
        ...
        assert "OPENCODON_TIMEZONE=NOT_SET" in result["output"], (
            "OPENCODON_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When OPENCODON_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("OPENCODON_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_parse_schedule_duration_uses_tz_aware_now(self):
        ...
        os.environ["OPENCODON_TIMEZONE"] = "Asia/Kolkata"
        from opencodon.cron.jobs import parse_schedule
    ...
    def test_compute_next_run_tz_aware(self):
        ...
        os.environ["OPENCODON_TIMEZONE"] = "Asia/Kolkata"
        from opencodon.cron.jobs import compute_next_run
    ...
    def test_get_due_jobs_handles_naive_timestamps(self, tmp_path, monkeypatch):
        ...
        import opencodon.cron.jobs as jobs_module
        ...
        from opencodon.cron.jobs import create_job, load_jobs, save_jobs, get_due_jobs
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=opencodon_tz) which shifted
        absolute time when system-local tz != opencodon tz.  The fix interprets
        naive values as system-local wall time, then converts.
        """
        from opencodon.cron.jobs import _ensure_aware
    ...
    def test_ensure_aware_normalizes_aware_to_opencodon_tz(self):
        """Already-aware datetimes should be normalized to opencodon tz."""
        from opencodon.cron.jobs import _ensure_aware
        ...
        # Must be in opencodon tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of opencodon tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), opencodon is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when opencodon tz
        ...
        # Use a opencodon timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _opencodon_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["OPENCODON_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_opencodon_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of opencodon timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["OPENCODON_TIMEZONE"] = "US/Eastern"
        _reset_opencodon_time_cache()
