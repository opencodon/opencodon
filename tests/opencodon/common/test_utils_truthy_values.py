...
from opencodon.common.utils import env_var_enabled, is_truthy_value
...
def test_env_var_enabled_uses_shared_truthy_rules(monkeypatch):
    monkeypatch.setenv("OPENCODON_TEST_BOOL", "YeS")
    assert env_var_enabled("OPENCODON_TEST_BOOL") is True

    monkeypatch.setenv("OPENCODON_TEST_BOOL", "no")
    assert env_var_enabled("OPENCODON_TEST_BOOL") is False
