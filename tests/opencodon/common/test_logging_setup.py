"""Tests for opencodon_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
def _reset_logging_state():
    ...
    """
    opencodon_logging._logging_initialized = False
    ...
    opencodon_logging._reset_queued_handlers()
    ...
    opencodon_logging._install_session_record_factory()
    yield
    ...
    opencodon_logging._logging_initialized = False
    opencodon_logging.clear_session_context()


@pytest.fixture
def opencodon_home(tmp_path, monkeypatch):
    """Provide an isolated OPENCODON_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_opencodon_home from conftest,
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, opencodon_home):
        log_dir = opencodon_logging.setup_logging(opencodon_home=opencodon_home)
        assert log_dir == opencodon_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, opencodon_home):
        opencodon_logging.setup_logging(opencodon_home=opencodon_home)
        root = logging.getLogger()

        agent_handlers = [
            h for h in opencodon_logging.rotating_file_handlers()
    ...
    def test_creates_errors_log_handler(self, opencodon_home):
        opencodon_logging.setup_logging(opencodon_home=opencodon_home)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, opencodon_home):
        opencodon_logging.setup_logging(opencodon_home=opencodon_home)
        opencodon_logging.setup_logging(opencodon_home=opencodon_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, opencodon_home):
        opencodon_logging.setup_logging(opencodon_home=opencodon_home)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        opencodon_logging.setup_logging(opencodon_home=opencodon_home, force=True)
    ...
    def test_custom_log_level(self, opencodon_home):
        opencodon_logging.setup_logging(opencodon_home=opencodon_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, opencodon_home):
        opencodon_logging.setup_logging(
            opencodon_home=opencodon_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, opencodon_home):
        opencodon_logging.setup_logging(opencodon_home=opencodon_home)
    ...
    def test_writes_to_agent_log(self, opencodon_home):
        opencodon_logging.setup_logging(opencodon_home=opencodon_home)

        test_logger = logging.getLogger("test_opencodon_logging.write_test")
        test_logger.info("test message for agent.log")

        # Flush handlers
        opencodon_logging.flush_log_queue()

        agent_log = opencodon_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_warnings_appear_in_both_logs(self, opencodon_home):
        opencodon_logging.setup_logging(opencodon_home=opencodon_home)

        test_logger = logging.getLogger("test_opencodon_logging.warning_test")
        test_logger.warning("this is a warning")

        opencodon_logging.flush_log_queue()

        agent_log = opencodon_home / "logs" / "agent.log"
        errors_log = opencodon_home / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, opencodon_home):
        opencodon_logging.setup_logging(opencodon_home=opencodon_home)

        test_logger = logging.getLogger("test_opencodon_logging.info_test")
    ...
    def test_reads_config_yaml(self, opencodon_home):
    ...
    def test_explicit_params_override_config(self, opencodon_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (opencodon_home / "config.yaml").write_text(yaml.dump(config))

        opencodon_logging.setup_logging(opencodon_home=opencodon_home, log_level="WARNING")
    ...
    def test_record_factory_installed(self, opencodon_home):
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, opencodon_home):
    ...
    def test_gateway_log_not_created_in_cli_mode(self, opencodon_home):
        opencodon_logging.setup_logging(opencodon_home=opencodon_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_created_after_cli_init(self, opencodon_home):
        ...
        gw_log = opencodon_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "gateway connected after cli init" in gw_log.read_text()

    def test_gateway_log_created_after_cli_init_without_duplicate_handlers(self, opencodon_home):
    ...
    def test_gateway_log_receives_gateway_records(self, opencodon_home):
    ...
    def test_gateway_log_rejects_non_gateway_records(self, opencodon_home):
    ...
    def test_agent_log_still_receives_all(self, opencodon_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, opencodon_home):
    ...
    def test_gui_log_created_after_cli_init(self, opencodon_home):
        opencodon_logging.setup_logging(opencodon_home=opencodon_home, mode="cli")
        opencodon_logging.setup_logging(opencodon_home=opencodon_home, mode="gui")
    ...
    def test_gui_log_receives_only_gui_components(self, opencodon_home):
        ...
        logging.getLogger("opencodon.frontends.server.web_server").info("dashboard online")
        logging.getLogger("opencodon.frontends.tui.ws").info("ws connected")
        logging.getLogger("gateway.run").info("gateway event")

        opencodon_logging.flush_log_queue()

        gui_log = opencodon_home / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, opencodon_home):
        """When session context is set, log lines include [session_id]."""
        opencodon_logging.setup_logging(opencodon_home=opencodon_home)
        opencodon_logging.set_session_context("abc123")
    ...
    def test_no_session_tag_without_context(self, opencodon_home):
    ...
    def test_clear_session_context(self, opencodon_home):
        """After clearing, session tag disappears."""
        opencodon_logging.setup_logging(opencodon_home=opencodon_home)
        opencodon_logging.set_session_context("xyz789")
        opencodon_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, opencodon_home):
        ...
        def thread_a():
            opencodon_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            opencodon_logging.flush_log_queue()

        def thread_b():
            opencodon_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        opencodon_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        opencodon_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_nested_matching_prefix(self):
        ...
        f = opencodon_logging._ComponentFilter(
            opencodon_logging.COMPONENT_PREFIXES["gateway"]
        )
        record = logging.LogRecord(
...
class TestComponentPrefixes:
    ...
    def test_agent_prefix(self):
        prefixes = opencodon_logging.COMPONENT_PREFIXES["agent"]
    ...
    def test_cli_prefix(self):
        ...
        assert "opencodon_cli" in prefixes
    ...
    def test_gui_prefix(self):
        ...
        assert "opencodon.frontends.server.web_server" in prefixes
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, opencodon_home):
        opencodon_logging.setup_logging(opencodon_home=opencodon_home)
        opencodon_logging.setup_verbose_logging()
    ...
    def test_idempotent(self, opencodon_home):
        opencodon_logging.setup_logging(opencodon_home=opencodon_home)
        opencodon_logging.setup_verbose_logging()
        opencodon_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        opencodon_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = opencodon_logging._ComponentFilter(("test",))
        ...
        handlers = [h for h in opencodon_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)]
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        opencodon_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("opencodon.config.is_managed", return_value=True):
                opencodon_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
    ...
    def test_managed_mode_rollover_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("opencodon.config.is_managed", return_value=True):
                ...
                handler = next(
                    h for h in opencodon_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)
                )
                logger.info("a" * 256)
                opencodon_logging.flush_log_queue()
        finally:
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def _make_logger_and_handler(self, log_path: Path):
        ...
        handler = opencodon_logging._ManagedRotatingFileHandler(
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        with patch.object(opencodon_logging.sys, "platform", "win32"):
            assert opencodon_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("Cannot acquire lock after 20 attempts")
            )
            assert not opencodon_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("some other logging failure")
            )

        with patch.object(opencodon_logging.sys, "platform", "linux"):
            assert not opencodon_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("Cannot acquire lock after 20 attempts")
            )
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, opencodon_home):
        level, max_size, backup = opencodon_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, opencodon_home):
        import yaml
    ...
    def test_handles_missing_logging_section(self, opencodon_home):
        import yaml
        config = {"model": "test"}
        (opencodon_home / "config.yaml").write_text(yaml.dump(config))
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> opencodon_logging._ManagedRotatingFileHandler:
        handler = opencodon_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that opencodon_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        opencodon_logging.flush_log_queue()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        opencodon_logging.setup_logging(opencodon_home=opencodon_home, mode="gateway")
        gw_path = opencodon_home / "logs" / "gateway.log"
        rotated = opencodon_home / "logs" / "gateway.log.1"
...
class TestSafeStderr:
    ...
    def test_returns_stderr_on_utf8_system(self, monkeypatch):
        ...
        result = opencodon_logging._safe_stderr()
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, opencodon_home):
        opencodon_logging.setup_logging(opencodon_home=opencodon_home)
        root = logging.getLogger()
        ...
        assert any(
            ...
            for h in opencodon_logging.rotating_file_handlers()
        )

    def test_records_reach_file_through_queue(self, opencodon_home):
        opencodon_logging.setup_logging(opencodon_home=opencodon_home)
    ...
    def test_queue_preserves_per_handler_levels(self, opencodon_home):
        opencodon_logging.setup_logging(opencodon_home=opencodon_home)
