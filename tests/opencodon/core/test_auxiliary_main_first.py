...
class TestResolveAutoMainFirst:
    ...
    def test_openrouter_main_uses_main_model_for_aux(self, monkeypatch):
        ...
        with patch(
            "opencodon.core.auxiliary_client._read_main_provider",
    ...
    def test_moa_main_resolves_aux_to_aggregator(self, monkeypatch, tmp_path):
        ...
        with patch(
            ...
            "opencodon.core.auxiliary_client._is_provider_unhealthy", return_value=False
        ):
            mock_client = MagicMock()
    ...
    def test_nous_main_uses_main_model_for_aux(self, monkeypatch):
        ...
        with patch(
            "opencodon.core.auxiliary_client._read_main_provider", return_value="nous",
        ), patch(
            "opencodon.core.auxiliary_client._read_main_model",
    ...
    def test_non_aggregator_main_still_uses_main(self, monkeypatch):
        ...
        with patch(
            "opencodon.core.auxiliary_client._read_main_provider", return_value="deepseek",
        ), patch(
            "opencodon.core.auxiliary_client._read_main_model", return_value="deepseek-chat",
        ), patch(
            "opencodon.core.auxiliary_client.resolve_provider_client"
        ) as mock_resolve:
            mock_client = MagicMock()
    ...
    def test_main_unavailable_falls_through_to_chain(self, monkeypatch):
        ...
        with patch(
            "opencodon.core.auxiliary_client._read_main_provider", return_value="anthropic",
        ), patch(
            "opencodon.core.auxiliary_client._read_main_model", return_value="claude-opus",
        ), patch(
            "opencodon.core.auxiliary_client.resolve_provider_client",
            return_value=(None, None),  # main provider has no client
        ), patch(
            "opencodon.core.auxiliary_client._try_openrouter",
    ...
    def test_main_unavailable_uses_task_fallback_chain_before_builtin_chain(self):
        ...
        with patch(
            "opencodon.core.auxiliary_client._read_main_provider", return_value="nvidia",
        ), patch(
            "opencodon.core.auxiliary_client._read_main_model", return_value="qwen/qwen3.5-122b-a10b",
        ), patch(
            "opencodon.core.auxiliary_client.resolve_provider_client",
            return_value=(None, None),  # main provider has no client
        ), patch(
            ...
            "opencodon.core.auxiliary_client._try_main_fallback_chain",
    ...
    def test_no_main_config_uses_chain_directly(self):
        ...
        chain_client = MagicMock()
        with patch(
            "opencodon.core.auxiliary_client._read_main_provider", return_value="",
        ), patch(
            "opencodon.core.auxiliary_client._read_main_model", return_value="",
        ), patch(
            "opencodon.core.auxiliary_client._try_openrouter",
            return_value=(chain_client, "google/gemini-3-flash-preview"),
        ):
    ...
    def test_runtime_override_wins_over_config(self, monkeypatch):
        ...
        with patch(
            ...
            "opencodon.core.auxiliary_client._read_main_model", return_value="config-model",
        ), patch(
            "opencodon.core.auxiliary_client.resolve_provider_client"
        ) as mock_resolve:
    ...
    def test_resolve_provider_auto_returns_runtime_model_not_stale_config_default(self):
        ...
        with patch(
            ...
            "opencodon.core.auxiliary_client._resolve_auto",
...
class TestResolveVisionMainFirst:
    ...
    def test_openrouter_main_vision_uses_main_model(self, monkeypatch):
        ...
        with patch(
            "opencodon.core.auxiliary_client._read_main_provider", return_value="openrouter",
        ), patch(
            "opencodon.core.auxiliary_client._read_main_model",
            return_value="anthropic/claude-sonnet-4.6",
        ), patch(
    ...
    def test_exotic_provider_with_vision_override_preserved(self):
        ...
        with patch(
            "opencodon.core.auxiliary_client._read_main_provider", return_value="xiaomi",
        ), patch(
            "opencodon.core.auxiliary_client._read_main_model",
    ...
    def test_copilot_vision_sets_vision_header(self, monkeypatch):
        ...
        with patch(
            "opencodon.core.auxiliary_client._read_main_provider", return_value="copilot",
        ), patch(
            "opencodon.core.auxiliary_client._read_main_model", return_value="configured-copilot-model",
        ), patch(
            "opencodon.core.auxiliary_client._resolve_task_provider_model",
            return_value=("auto", None, None, None, None),
        ), patch(
            ...
            "opencodon.frontends.cli.copilot_auth.copilot_request_headers",
    ...
    def test_main_unavailable_vision_falls_through_to_aggregators(self):
        ...
        assert provider in {"openrouter", "deepinfra"}
    ...
    def test_explicit_provider_override_still_wins(self):
        ...
        # Explicit override → uses strict backend, NOT the main model path
...
class TestResolveVisionCustomProvider:
    ...
    def test_custom_main_forwards_runtime_endpoint(self, monkeypatch):
        ...
        with patch(
            "opencodon.core.auxiliary_client._read_main_provider", return_value="custom",
        ), patch(
            "opencodon.core.auxiliary_client._read_main_model", return_value="claude-opus-4-8",
        ), patch(
            "opencodon.core.auxiliary_client._resolve_task_provider_model",
            return_value=("auto", None, None, None, None),
        ), patch(
    ...
    def test_custom_main_no_runtime_falls_back_to_configured_endpoint(self, monkeypatch):
        ...
        with patch(
            ...
            "opencodon.core.auxiliary_client._resolve_custom_runtime",
...
def test_aggregator_providers_constant_removed():
    ...
    """
    import opencodon.core.auxiliary_client as aux_mod
