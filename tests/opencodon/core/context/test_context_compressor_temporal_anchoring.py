...
from opencodon.core.context.context_compressor import ContextCompressor, HISTORICAL_TASK_HEADING
...
def test_first_compaction_prompt_contains_dated_anchoring_rule():
    ...
    with patch.object(opencodon_time, "now", _fixed_now), patch(
        "opencodon.core.context.context_compressor.call_llm", return_value=_response("summary")
    ) as mock_call:
...
def test_iterative_update_prompt_also_contains_anchoring_rule():
    ...
    with patch.object(opencodon_time, "now", _fixed_now), patch(
        "opencodon.core.context.context_compressor.call_llm", return_value=_response("updated summary")
    ) as mock_call:
        compressor._generate_summary(_turns())
...
def test_clock_failure_omits_rule_but_compaction_still_runs():
    ...
    with patch.object(opencodon_time, "now", _boom), patch(
        "opencodon.core.context.context_compressor.call_llm", return_value=_response("summary")
    ) as mock_call:
...
def test_anchoring_rule_uses_date_from_opencodon_time_now():
    """The date is taken from opencodon_time.now(), which respects the user's TZ."""
    compressor = _compressor()
    fixed = datetime(2025, 12, 31, 23, 30, tzinfo=timezone.utc)
    with patch.object(opencodon_time, "now", lambda: fixed), patch(
        "opencodon.core.context.context_compressor.call_llm", return_value=_response("summary")
    ) as mock_call:
        compressor._generate_summary(_turns())
