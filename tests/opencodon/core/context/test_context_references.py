...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "opencodon Tests")
...
def test_parse_typed_references_ignores_emails_and_handles():
    from opencodon.core.context.context_references import parse_context_references
...
def test_folder_listing_falls_back_when_rg_is_blocked(sample_repo: Path):
    ...
    with patch("opencodon.core.context.context_references.subprocess.run", side_effect=blocked_rg):
        result = preprocess_context_references(
            "Review @folder:src/",
            cwd=sample_repo,
            context_length=100_000,
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_opencodon_paths(tmp_path: Path, monkeypatch):
    from opencodon.core.context.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("OPENCODON_HOME", str(tmp_path / ".opencodon"))

    opencodon_env = tmp_path / ".opencodon" / ".env"
    opencodon_env.parent.mkdir(parents=True)
    opencodon_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
...
async def test_canonical_guard_fails_closed_when_lookup_raises(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("opencodon.core.file_safety.get_read_block_error", _boom)
