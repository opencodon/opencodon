...
class TestCompress:
    ...
    def test_summary_failure_uses_deterministic_fallback_with_recovered_context(self):
        ...
        with (
            ...
            patch(
                "opencodon.core.context.context_compressor.call_llm",
    ...
    def test_threshold_below_window_at_minimum_ctx(self):
        ...
        actually fires."""
        from opencodon.core.context.context_compressor import MINIMUM_CONTEXT_LENGTH
    ...
    def test_minimum_ctx_model_can_actually_compress(self):
        ...
        at 100%."""
        with patch("opencodon.core.context.context_compressor.get_model_context_length", return_value=64000):
    ...
    def test_max_tokens_coercion_treats_non_int_as_no_reservation(self):
        ...
        with patch("opencodon.core.context.context_compressor.get_model_context_length", return_value=200000):
    ...
    def test_compress_terminal_sweep_strips_markers_even_if_a_copy_site_leaks(self, compressor):
        ...
        import opencodon.core.context.context_compressor as _cc
        ...
        with patch.object(_cc, "_fresh_compaction_message_copy", lambda m: m.copy()), \
             patch("opencodon.core.context.context_compressor.call_llm", side_effect=RuntimeError("no provider")):
            result = compressor.compress(msgs)
        assert len(result) < len(msgs)
        assert all("_db_persisted" not in msg for msg in result), (
...
class TestGenerateSummaryNoneContent:
    ...
    def test_none_content_does_not_crash(self):
        ...
        with patch("opencodon.core.context.context_compressor.call_llm", return_value=mock_response):
...
class TestNonStringContent:
    ...
    def test_empty_content_falls_back_to_main_model(self):
        ...
        with patch("opencodon.core.context.context_compressor.call_llm", return_value=mock_response) as mock_call:
            summary = c._generate_summary(messages)
...
class TestSummaryFailureCooldown:
    def test_summary_failure_enters_cooldown_and_skips_retry(self):
        ...
        with patch("opencodon.core.context.context_compressor.call_llm", side_effect=Exception("boom")) as mock_call:
...
class TestAuthFailureAborts:
    ...
    def test_400_out_of_extra_usage_aborts_instead_of_dropping_context(self):
        ...
        msgs = self._msgs(12)
        with patch("opencodon.core.context.context_compressor.call_llm", side_effect=err):
    ...
    def test_missing_provider_api_key_preserves_original_messages(self):
        ...
        err = RuntimeError(
            ...
            "to a different provider with opencodon model."
        )
        with patch(
            "opencodon.core.context.context_compressor.get_model_context_length", return_value=100000
        ):
            c = ContextCompressor(
                model="test",
                quiet_mode=True,
    ...
    def test_generate_summary_flags_auth_failure(self):
        with patch("opencodon.core.context.context_compressor.get_model_context_length", return_value=100000):
            c = ContextCompressor(model="test", quiet_mode=True)
        with patch("opencodon.core.context.context_compressor.call_llm", side_effect=self._auth_err(401)):
    ...
    def test_403_also_flags_auth_failure(self):
        with patch("opencodon.core.context.context_compressor.get_model_context_length", return_value=100000):
            c = ContextCompressor(model="test", quiet_mode=True)
        with patch("opencodon.core.context.context_compressor.call_llm", side_effect=self._auth_err(403)):
    ...
    def test_non_auth_failure_still_uses_fallback_path(self):
        ...
        msgs = self._msgs(12)
        with patch("opencodon.core.context.context_compressor.call_llm", side_effect=Exception("boom 500")):
            result = c.compress(msgs, current_tokens=999999, force=True)
        assert c._last_summary_auth_failure is False
        assert c._last_compress_aborted is False
...
class TestSummaryFallbackToMainModel:
    ...
    def test_json_decode_error_on_main_uses_short_cooldown(self):
        ...
        ), patch("opencodon.core.context.context_compressor.time.monotonic", return_value=1000.0):
            result = c._generate_summary(self._msgs())
...
class TestStreamingClosedFallback:
    ...
    def test_incomplete_chunked_read_falls_back_to_main(self):
        ...
        with patch(
            ...
            "opencodon.core.context.context_compressor._is_connection_error",
            return_value=True,
        ):
            result = c._generate_summary(self._msgs())
...
class TestSummaryFailureTrackingForGatewayWarning:
    ...
    def test_compress_records_fallback_and_dropped_count_on_summary_failure(self):
        ...
        with patch("opencodon.core.context.context_compressor.call_llm", side_effect=Exception("404 model not found")):
            result = c.compress(msgs)
    ...
    def test_summary_failure_fallback_preserves_tool_paths_and_redacts_secret_context(self):
        ...
        with patch("opencodon.core.context.context_compressor.call_llm", side_effect=Exception("timeout")):
            result = c.compress(msgs)
    ...
    def test_compress_clears_fallback_flag_on_subsequent_success(self):
        ...
        with patch("opencodon.core.context.context_compressor.call_llm", side_effect=Exception("boom")):
            c.compress(msgs)
        assert c._last_summary_fallback_used is True
...
class TestAbortOnSummaryFailure:
    ...
    def test_force_true_bypasses_persisted_session_cooldown(self, tmp_path):
        ...
        with patch("opencodon.core.context.context_compressor.call_llm", return_value=mock_response) as mock_llm:
            result = c.compress(msgs, current_tokens=999999, force=True)
...
class TestCompressWithClient:
    ...
    def test_merge_into_tail_end_marker_is_last(self):
        ...
        """
        from opencodon.core.context.context_compressor import _SUMMARY_END_MARKER
...
class TestSummaryTargetRatio:
    ...
    def test_tail_budget_scales_with_context(self):
        ...
        with patch("opencodon.core.context.context_compressor.get_model_context_length", return_value=200_000):
            c = ContextCompressor(model="test", quiet_mode=True, summary_target_ratio=0.40)
        # 200K < 512K → threshold floored at 75%: 150K * 0.40 ratio = 60K
        assert c.tail_token_budget == 60_000

        with patch("opencodon.core.context.context_compressor.get_model_context_length", return_value=1_000_000):
            c = ContextCompressor(model="test", quiet_mode=True, summary_target_ratio=0.40)
    ...
    def test_configured_threshold_used_at_512k_and_above(self):
        ...
        with patch("opencodon.core.context.context_compressor.get_model_context_length", return_value=512_000):
            c = ContextCompressor(model="test", quiet_mode=True)
...
class TestThresholdTokensCap:
    ...
    def test_cap_clamped_to_context_length(self):
        ...
        with patch("opencodon.core.context.context_compressor.get_model_context_length", return_value=64_000):
            comp = ContextCompressor(
                "model-a", threshold_percent=0.50, quiet_mode=True,
...
class TestTruncateToolCallArgsJson:
    ...
    def _helper(self):
        from opencodon.core.context.context_compressor import _truncate_tool_call_args_json
    ...
    def test_shrunken_args_remain_valid_json(self):
        ...
        original = _json.dumps({
            "path": "~/.opencodon/skills/shopping/browser-setup-notes.md",
        ...
        assert parsed["path"] == "~/.opencodon/skills/shopping/browser-setup-notes.md"
