...
class TestScanSkillCommands:
    ...
    def test_excludes_incompatible_platform(self, tmp_path):
        ...
        with (
            ...
            patch("opencodon.core.skills.skill_utils.sys") as mock_sys,
        ):
    ...
    def test_get_skill_commands_rescans_when_platform_scope_changes(self, tmp_path):
        ...
        """
        import opencodon.core.skills.skill_commands as sc_mod
        from opencodon.core.skills.skill_commands import get_skill_commands

        def _disabled_skills():
            platform = os.getenv("OPENCODON_PLATFORM")
        ...
        with (
            ...
            with patch.dict(os.environ, {"OPENCODON_PLATFORM": "telegram"}):
            ...
            with patch.dict(os.environ, {"OPENCODON_PLATFORM": "discord"}):
    ...
    def test_get_skill_commands_rescans_when_session_platform_changes(self, tmp_path):
        """``OPENCODON_SESSION_PLATFORM`` from the gateway session context must
        also trigger a rescan, not just ``OPENCODON_PLATFORM`` (#14536).

        Exercises the real ContextVar path: the gateway sets the active
        adapter via ``set_session_vars(platform=...)`` and the resolver
        reads it via ``get_session_env``. Setting ``OPENCODON_SESSION_PLATFORM``
    ...
    def test_get_skill_commands_rescans_when_leaving_platform_scope(self, tmp_path, monkeypatch):
        ...
        def _disabled_skills():
            if os.getenv("OPENCODON_PLATFORM") == "telegram":
                return {"telegram-only"}
            return set()
    ...
    def test_get_skill_commands_does_not_rescan_when_platform_unchanged(self, tmp_path):
        ...
        with (
            ...
            with patch(
                "opencodon.core.skills.skill_commands.scan_skill_commands",
    ...
    def test_slug_collision_warns(self, tmp_path, caplog):
        ...
        with patch("opencodon.tools.skills_tool.SKILLS_DIR", tmp_path):
            ...
            with caplog.at_level(_logging.WARNING, logger="opencodon.core.skills.skill_commands"):
                scan_skill_commands()
...
class TestBuildPreloadedSkillsPrompt:
    ...
    def test_skips_disabled_skill(self, tmp_path, monkeypatch):
        ...
        with patch("opencodon.tools.skills_tool.SKILLS_DIR", tmp_path):
            ...
            import opencodon.core.skills.skill_utils as su_module
            monkeypatch.setattr(
...
"""
    ...
    def test_returns_none_when_skill_load_fails(self, tmp_path):
        with patch("opencodon.tools.skills_tool.SKILLS_DIR", tmp_path):
            ...
            scan_skill_commands()
            with patch("opencodon.core.skills.skill_commands._load_skill_payload", return_value=None):
...
class TestTemplateVarSubstitution:
    """``${OPENCODON_SKILL_DIR}`` and ``${OPENCODON_SESSION_ID}`` in SKILL.md body
    ...
    def test_substitutes_skill_dir(self, tmp_path):
        with patch("opencodon.tools.skills_tool.SKILLS_DIR", tmp_path):
            skill_dir = _make_skill(
                tmp_path,
                "templated",
                body="Run: node ${OPENCODON_SKILL_DIR}/scripts/foo.js",
            )
            scan_skill_commands()
    ...
    def test_substitutes_session_id_when_available(self, tmp_path):
        with patch("opencodon.tools.skills_tool.SKILLS_DIR", tmp_path):
            _make_skill(
                tmp_path,
                "sess-templated",
                body="Session: ${OPENCODON_SESSION_ID}",
            )
            scan_skill_commands()
    ...
    def test_leaves_session_id_token_when_missing(self, tmp_path):
        ...
        assert "Session: ${OPENCODON_SESSION_ID}" in msg

    def test_disable_template_vars_via_config(self, tmp_path):
        with (
            patch("opencodon.tools.skills_tool.SKILLS_DIR", tmp_path),
            patch(
                "opencodon.core.skills.skill_commands._load_skills_config",
        ...
        assert "${OPENCODON_SKILL_DIR}/scripts/foo.js" in msg
...
class TestStackedSkillCommands:
    ...
    def test_split_consumes_leading_skill_tokens(self, tmp_path):
        from opencodon.core.skills.skill_commands import split_stacked_skill_commands
        with patch("opencodon.tools.skills_tool.SKILLS_DIR", tmp_path):
    ...
    def test_stacked_message_contains_all_bodies_and_instruction(self, tmp_path):
        from opencodon.core.skills.skill_commands import build_stacked_skill_invocation_message
        with patch("opencodon.tools.skills_tool.SKILLS_DIR", tmp_path):
            self._setup_three_skills(tmp_path)
            scan_skill_commands()
