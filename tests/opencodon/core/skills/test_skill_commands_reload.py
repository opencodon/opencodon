"""Tests for ``opencodon.core.skills.skill_commands.reload_skills``.
...
def opencodon_home(monkeypatch):
    """Isolate OPENCODON_HOME for ``reload_skills`` tests.
    ...
    import opencodon.core.skills.skill_commands as _sc
...
class TestReloadSkillsHelper:
    """``opencodon.core.skills.skill_commands.reload_skills``."""

    def test_returns_expected_keys(self, opencodon_home):
    ...
    def test_detects_newly_added_skill_with_description(self, opencodon_home):
        from opencodon.core.skills.skill_commands import reload_skills, get_skill_commands

        # Prime the cache so subsequent diff is meaningful
        get_skill_commands()

        _write_skill(opencodon_home / "skills", "demo", "a demo skill")
        result = reload_skills()
    ...
    def test_detects_removed_skill_carries_description(self, opencodon_home):
        from opencodon.core.skills.skill_commands import reload_skills

        skill_dir = _write_skill(opencodon_home / "skills", "demo", "soon to be gone")
    ...
    def test_description_passes_through_verbatim(self, opencodon_home):
        ...
        long_desc = "x" * 200
        _write_skill(opencodon_home / "skills", "longdesc", long_desc)

        result = reload_skills()
        assert len(result["added"]) == 1
        assert result["added"][0]["description"] == long_desc

    def test_unchanged_skills_appear_in_unchanged_list(self, opencodon_home):
        from opencodon.core.skills.skill_commands import reload_skills, get_skill_commands

        _write_skill(opencodon_home / "skills", "alpha")
        # Prime cache
        get_skill_commands()
    ...
    def test_does_not_invalidate_prompt_cache_snapshot(self, opencodon_home):
        ...
        """
        from opencodon.core.prompt.prompt_builder import _skills_prompt_snapshot_path
        from opencodon.core.skills.skill_commands import reload_skills
