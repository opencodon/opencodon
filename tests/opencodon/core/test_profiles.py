"""Comprehensive tests for opencodon.core.profiles module.
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("opencodon.core.profiles._cleanup_gateway_service"):
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("opencodon.core.profiles._cleanup_gateway_service"), \
             patch("opencodon.core.profiles.time.sleep"), \
             patch("opencodon.core.profiles.shutil.rmtree", side_effect=PermissionError("locked")):
    ...
    def test_stops_profile_bound_backends_before_removal(self, profile_env):
        ...
        with patch("opencodon.core.profiles._cleanup_gateway_service"), \
             patch("opencodon.core.profiles._profile_bound_backend_pids", return_value=[4242]) as pids, \
    ...
    def test_rmtree_retries_transient_enotempty_then_succeeds(self, profile_env):
        ...
        with patch("opencodon.core.profiles._cleanup_gateway_service"), \
             patch("opencodon.core.profiles._profile_bound_backend_pids", return_value=[]), \
             patch("opencodon.core.profiles.time.sleep"), \
             patch("opencodon.core.profiles.shutil.rmtree", side_effect=flaky_rmtree):
            delete_profile("coder", yes=True)
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        monkeypatch.setattr("opencodon.core.profiles.shutil.which", lambda name: "/opt/opencodon/bin/opencodon")
        from opencodon.core.profiles import create_wrapper_script
    ...
    def test_remove_finds_bat_on_windows(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "win32")
        from opencodon.core.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
    ...
    def test_remove_returns_false_when_absent(self, profile_env):
        from opencodon.core.profiles import remove_wrapper_script
...
class TestWrapperScriptSecurity:
    ...
    def test_legit_alias_stays_inside_wrapper_dir(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from opencodon.core.profiles import _get_wrapper_dir
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from opencodon.core.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_no_alias_returns_none(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from opencodon.core.profiles import find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr("sys.platform", "darwin")
        from opencodon.core.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        with patch("opencodon.core.profiles.check_alias_collision", return_value="skip"):
...
class TestInternalHelpers:
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        ...
        from opencodon.core.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OPENCODON_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from opencodon.core.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".opencodon"
