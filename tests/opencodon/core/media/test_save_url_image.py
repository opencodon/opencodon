...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate OPENCODON_HOME under tmp_path."""
    monkeypatch.setenv("OPENCODON_HOME", str(tmp_path / ".opencodon"))
    (tmp_path / ".opencodon").mkdir()

    # Force the constants/image cache helpers to re-read OPENCODON_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("opencodon_constants") or mod.startswith("opencodon.core.media.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_opencodon_home_cache(self, http_server):
        base, _ = http_server
        from opencodon.core.media.image_gen_provider import save_url_image

        path = save_url_image(f"{base}/image.png", prefix="xai_test")

        assert path.exists()
        assert path.read_bytes() == PNG_1PX
        # The cache directory must be under OPENCODON_HOME — gateway cleanup
    ...
    def test_oversize_raises_and_cleans_up(self, http_server, tmp_path):
        ...
        base, _ = http_server
        from opencodon.core.media.image_gen_provider import save_url_image, _images_cache_dir
