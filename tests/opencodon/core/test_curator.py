...
def curator_env(tmp_path, monkeypatch):
    """Isolated OPENCODON_HOME + freshly reloaded curator + skill_usage modules."""
    ...
    # save_state() resolves the state path from OPENCODON_HOME at write time, so a
    # straggler thread that outlives this test would write into whatever home
    # the *next* test has configured (or the default ~/.opencodon once monkeypatch
    ...
    # thread here, while OPENCODON_HOME is still pinned to this test's tmp home
...
def test_dry_run_does_not_advance_state(curator_env, monkeypatch):
    ...
    `opencodon curator status`. Fixes #18373.
    """
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
...
def test_run_review_consolidate_override_runs_llm(curator_env, monkeypatch):
    ...
    the LLM consolidation pass — mirrors `opencodon curator run --consolidate`."""
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
    _write_skill(skills_dir, "a")
...
# `opencodon model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_curator_slot_is_canonical_aux_task():
    ...
    # 2. opencodon_cli/main.py _AUX_TASKS — CLI picker
    aux_keys = {k for k, _name, _desc in _AUX_TASKS}
    assert "curator" in aux_keys, "curator missing from _AUX_TASKS (CLI picker)"

    # 3. opencodon_cli/web_server.py _AUX_TASK_SLOTS — REST API allowlist
    assert "curator" in _AUX_TASK_SLOTS, \
        "curator missing from _AUX_TASK_SLOTS (dashboard REST API)"

    # 4. The browser bundle's models page is checked at build time; the tsx
