...
import opencodon.core.display as display_module
...
class TestCuteToolMessagePreviewLength:
    ...
    def test_path_preview_uses_positive_configured_limit_not_default(self):
        set_tool_preview_max_len(80)
        path = "/tmp/opencodon-test-preview-length/deeply/nested/path/test-output.txt"
...
class TestEditDiffPreview:
    ...
    def test_render_edit_diff_with_delta_handles_renderer_errors(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.core.display._summarize_rendered_diff_sections", MagicMock(side_effect=RuntimeError("boom")))
...
class TestBuildToolLabel:
    ...
    def test_unknown_tool_falls_back_to_preview(self):
        from opencodon.core.display import build_tool_label, build_tool_preview
        args = {"some_arg": "value"}
    ...
    def test_every_known_verb_renders_without_error(self):
        from opencodon.core.display import build_tool_label, _TOOL_VERBS
...
class TestBuildStatusPhrase:
    ...
    def test_respects_friendly_labels_toggle(self):
        from opencodon.core.display import build_status_phrase, set_friendly_tool_labels
        set_friendly_tool_labels(False)
        try:
