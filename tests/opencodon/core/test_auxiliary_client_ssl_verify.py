...
``OPENCODON_CA_BUNDLE`` via ``agent.ssl_verify.resolve_httpx_verify``. Auxiliary calls
...
def test_resolve_aux_verify_uses_per_provider_ssl_ca_cert(clean_tls_env, monkeypatch):
    ...
    from opencodon.core import auxiliary_client

    # get_custom_provider_tls_settings is imported inside the function from
    # opencodon.config, so patch it at the source module.
    monkeypatch.setattr(
        cfg,
