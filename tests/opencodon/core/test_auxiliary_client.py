"""Tests for opencodon.core.auxiliary_client resolution chain, provider overrides, and model overrides."""
...
def _clean_env(monkeypatch):
    ...
    import opencodon.core.auxiliary_client as _aux_mod
...
def codex_auth_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.core.auxiliary_client._read_codex_access_token",
...
class TestAuxiliaryMaxTokensParam:
    def test_uses_max_completion_tokens_for_github_copilot_custom_base(self):
        with patch("opencodon.core.auxiliary_client._resolve_custom_runtime", return_value=("https://api.githubcopilot.com", "key", None)), \
    ...
    def test_uses_max_completion_tokens_for_github_copilot_custom_base_path(self):
        with patch("opencodon.core.auxiliary_client._resolve_custom_runtime", return_value=("https://api.githubcopilot.com/chat/completions", "key", None)), \
             patch.dict("os.environ", {}):
            assert auxiliary_max_tokens_param(2048) == {"max_completion_tokens": 2048}
...
class TestResolveTaskProviderModel:
    ...
    def test_explicit_provider_adopts_configured_task_endpoint(self):
        ...
        with patch("opencodon.core.auxiliary_client._get_auxiliary_task_config", return_value=task_config):
            resolved_provider, model, base_url, api_key, api_mode = _resolve_task_provider_model(
                task="vision",
                provider="custom",
                model="meta/llama-3.2-11b-vision-instruct",
...
class TestBuildCallKwargsMaxTokens:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("nous", "hermes-4", "https://inference.example.com/v1"),
    ...
    def test_omits_max_tokens_for_openai_compatible(self, provider, model, base_url):
        from opencodon.core.auxiliary_client import _build_call_kwargs
...
class TestReadCodexAccessToken:
    ...
    def test_pool_without_selected_entry_falls_back_to_auth_store(self, tmp_path, monkeypatch):
        ...
        with patch("opencodon.core.auxiliary_client._select_pool_entry", return_value=(True, None)), \
             patch("opencodon_cli.auth._read_codex_tokens", return_value={
    ...
    def test_missing_returns_none(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OPENCODON_HOME", str(opencodon_home))
        with patch("opencodon.core.auxiliary_client._select_pool_entry", return_value=(False, None)):
            result = _read_codex_access_token()
    ...
    def test_malformed_json_returns_none(self, tmp_path):
        ...
        with patch("opencodon.core.auxiliary_client.Path.home", return_value=tmp_path):
            result = _read_codex_access_token()
        assert result is None
...
class TestResolveXaiOAuthForAux:
    ...
    def test_pool_backed_credentials_honor_base_url_env_override(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OPENCODON_HOME", str(opencodon_home))
        monkeypatch.setenv("OPENCODON_XAI_BASE_URL", "https://example.x.ai/v1/")
...
class TestBuildCodexClient:
    def test_pool_without_selected_entry_falls_back_to_auth_store(self):
        with (
            patch("opencodon.core.auxiliary_client._select_pool_entry", return_value=(True, None)),
            patch("opencodon.core.auxiliary_client._read_codex_access_token", return_value="codex-auth-token"),
    ...
    def test_cached_codex_client_rebuilds_when_pool_entry_changes(self):
        ...
        with (
            patch("opencodon.core.auxiliary_client.load_pool", return_value=pool),
            patch("opencodon.core.auxiliary_client.OpenAI", side_effect=[client_a, client_b]) as mock_openai,
        ):
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    def test_empty_model_for_oauth_provider_falls_back_to_main_model(self):
        ...
        with (
            ...
            patch(
                "opencodon.core.auxiliary_client._get_aux_model_for_provider",
                return_value="",  # xai-oauth has no catalog default
            ),
            patch(
                "opencodon.core.auxiliary_client._build_xai_oauth_aux_client",
    ...
    def test_empty_model_for_codex_also_uses_main_model(self):
        ...
        with (
            ...
            patch(
                "opencodon.core.auxiliary_client._build_codex_client",
                return_value=(MagicMock(), "gpt-5.4"),
            ) as mock_build,
            patch(
                "opencodon.core.auxiliary_client._select_pool_entry",
    ...
    def test_explicit_model_takes_precedence_over_fallbacks(self):
        ...
        with (
            patch("opencodon.core.auxiliary_client._read_main_model") as mock_read_main,
            patch(
                "opencodon.core.auxiliary_client._get_aux_model_for_provider",
...
class TestExplicitProviderRouting:
    ...
    def test_explicit_openrouter_pool_exhausted_logs_precise_warning(self, monkeypatch, caplog):
        ...
        with patch("opencodon.core.auxiliary_client._select_pool_entry", return_value=(True, None)):
            with caplog.at_level(logging.WARNING, logger="opencodon.core.auxiliary_client"):
    ...
    def test_try_openrouter_pool_exhausted_no_env_marks_unhealthy(self, monkeypatch):
        ...
        monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
        with patch("opencodon.core.auxiliary_client._select_pool_entry", return_value=(True, None)), \
             patch("opencodon.core.auxiliary_client._mark_provider_unhealthy") as mock_mark, \
             patch("opencodon.core.auxiliary_client.OpenAI") as mock_openai:
            client, model = _try_openrouter()
...
class TestGetTextAuxiliaryClient:
    ...
    def test_codex_pool_entry_takes_priority_over_auth_store(self):
        ...
        with (
            ...
            patch("opencodon_cli.auth._read_codex_tokens", side_effect=AssertionError("legacy codex store should not run")),
        ):
            from opencodon.core.auxiliary_client import _build_codex_client

            client, model = _build_codex_client("gpt-5.4")

        from opencodon.core.auxiliary_client import CodexAuxiliaryClient
    ...
    def test_returns_none_when_nothing_available(self, monkeypatch):
        ...
        with patch.dict("os.environ", {}), \
             patch("opencodon.core.auxiliary_client._read_codex_access_token", return_value=None), \
             patch("opencodon.core.auxiliary_client._resolve_api_key_provider", return_value=(None, None)):
    ...
    def test_custom_endpoint_uses_codex_wrapper_when_runtime_requests_responses_api(self):
        with patch("opencodon.core.auxiliary_client._resolve_custom_runtime",
             ...
             patch.dict("os.environ", {}), \
             patch.dict("os.environ", {}), \
             patch("opencodon.core.auxiliary_client._read_main_model", return_value="gpt-5.3-codex"), \
             patch("opencodon.core.auxiliary_client.OpenAI") as mock_openai:
            client, model = get_text_auxiliary_client()
...
class TestVisionClientFallback:
    ...
    def test_anthropic_auxiliary_client_aggregates_stream_response(self):
        from opencodon.core.auxiliary_client import AnthropicAuxiliaryClient
...
class TestIsPaymentError:
    ...
    def test_404_free_tier_model_block_is_payment(self):
        exc = Exception(
            ...
            "Upgrade at https://idp.example.com or pick a free model."
        )
        exc.status_code = 404
        assert _is_payment_error(exc) is True
...
class TestGetProviderChain:
    ...
    def test_picks_up_patched_functions(self):
        ...
        with patch("opencodon.core.auxiliary_client._try_openrouter", sentinel):
            chain = _get_provider_chain()
...
class TestTryPaymentFallback:
    ...
    def _clear_unhealthy_cache(self):
        ...
        """
        from opencodon.core.auxiliary_client import _aux_unhealthy_until, _aux_unhealthy_logged_at
    ...
    def test_skips_failed_provider(self):
        ...
        with patch("opencodon.core.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("opencodon.core.auxiliary_client._try_custom_endpoint", return_value=(mock_client, "custom-model")), \
             patch("opencodon.core.auxiliary_client._read_main_provider", return_value="openrouter"):
    ...
    def test_returns_none_when_no_fallback(self):
        with patch("opencodon.core.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("opencodon.core.auxiliary_client._try_custom_endpoint", return_value=(None, None)), \
             patch("opencodon.core.auxiliary_client._resolve_api_key_provider", return_value=(None, None)), \
             patch("opencodon.core.auxiliary_client._read_main_provider", return_value="openrouter"):
    ...
    def test_codex_alias_maps_to_chain_label(self):
        ...
        mock_client = MagicMock()
        with patch("opencodon.core.auxiliary_client._try_openrouter", return_value=(mock_client, "or-model")), \
             patch("opencodon.core.auxiliary_client._read_main_provider", return_value="openai-codex"):
...
class TestCallLlmPaymentFallback:
    ...
    def test_non_payment_error_not_caught(self, monkeypatch):
        ...
        with patch("opencodon.core.auxiliary_client._get_cached_client",
                    return_value=(primary_client, "google/gemini-3-flash-preview")), \
             patch("opencodon.core.auxiliary_client._resolve_task_provider_model",
    ...
    def test_429_rate_limit_triggers_fallback(self, monkeypatch):
        ...
        with patch("opencodon.core.auxiliary_client._get_cached_client",
             ...
             patch("opencodon.core.auxiliary_client._try_payment_fallback",
    ...
    def test_401_auth_error_no_fallback_with_explicit_provider(self, monkeypatch):
        ...
        with patch("opencodon.core.auxiliary_client._get_cached_client",
             ...
             patch("opencodon.core.auxiliary_client._refresh_provider_credentials", return_value=False), \
             patch("opencodon.core.auxiliary_client._try_payment_fallback") as mock_fb:
...
class TestStaleFallbackCandidateSkip:
    ...
    def test_stale_anthropic_fallback_refreshes_and_retries(self, monkeypatch):
        ...
        with patch("opencodon.core.auxiliary_client._resolve_task_provider_model",
                   return_value=("auto", None, None, None, None)), \
             patch("opencodon.core.auxiliary_client._get_cached_client", side_effect=_cached_client), \
             patch("opencodon.core.auxiliary_client._try_configured_fallback_chain",
                   return_value=(None, None, "")), \
             patch("opencodon.core.auxiliary_client._try_main_fallback_chain",
                   return_value=(None, None, "")), \
             patch("opencodon.core.auxiliary_client._try_payment_fallback",
                   return_value=(stale_fb, "claude-haiku-4-5-20251001", "anthropic")), \
             patch("opencodon.core.auxiliary_client._refresh_provider_credentials",
    ...
    def test_unrefreshable_stale_candidate_is_skipped_to_next(self, monkeypatch):
        ...
        with patch("opencodon.core.auxiliary_client._resolve_task_provider_model",
                   return_value=("auto", None, None, None, None)), \
             patch("opencodon.core.auxiliary_client._get_cached_client",
...
class TestAuxiliaryFallbackLayering:
    ...
    def test_empty_choices_with_output_text_is_recovered_before_fallback(self, monkeypatch):
        ...
        with patch("opencodon.core.auxiliary_client._get_cached_client",
             ...
             patch("opencodon.core.auxiliary_client._try_configured_fallback_chain") as mock_chain:
            result = call_llm(
    ...
    def test_invalid_empty_choices_response_triggers_fallback(self, monkeypatch):
        ...
        with patch("opencodon.core.auxiliary_client._get_cached_client",
             ...
             patch("opencodon.core.auxiliary_client._try_main_agent_model_fallback") as mock_main:
            result = call_llm(
                task="title_generation",
                messages=[{"role": "user", "content": "hello"}],
            )
    ...
    async def test_async_invalid_empty_choices_response_triggers_fallback(self, monkeypatch):
        ...
        with patch("opencodon.core.auxiliary_client._get_cached_client",
             ...
             patch("opencodon.core.auxiliary_client._try_configured_fallback_chain",
                   return_value=(fallback_client, "gpt-5.4-mini", "fallback_chain[0](openai-codex)")) as mock_chain, \
             patch("opencodon.core.auxiliary_client._to_async_client",
    ...
    def test_auto_provider_uses_task_then_main_chain_before_builtin_chain(self, monkeypatch):
        ...
        with patch("opencodon.core.auxiliary_client._get_cached_client",
             ...
             patch("opencodon.core.auxiliary_client._try_payment_fallback") as mock_builtin_chain:
            result = call_llm(
                task="title_generation",
                messages=[{"role": "user", "content": "hello"}],
            )
    ...
    def test_explicit_provider_uses_configured_chain_first(self, monkeypatch, caplog):
        ...
        with patch("opencodon.core.auxiliary_client._get_cached_client",
             ...
             patch("opencodon.core.auxiliary_client._try_main_agent_model_fallback",
    ...
    def test_warning_emitted_when_all_fallbacks_exhausted(self, monkeypatch, caplog):
        ...
        with patch("opencodon.core.auxiliary_client._get_cached_client",
             ...
             patch("opencodon.core.auxiliary_client._try_main_agent_model_fallback",
                   return_value=(None, None, "")), \
             caplog.at_level("WARNING", logger="opencodon.core.auxiliary_client"):
    ...
    def test_fallback_entry_openai_codex_uses_oauth_pool_without_inline_key(self):
        ...
        from opencodon.core.auxiliary_client import _resolve_fallback_entry
        ...
        with patch("opencodon.core.auxiliary_client._select_pool_entry",
                   return_value=(True, pool_entry)), \
             patch("opencodon.core.auxiliary_client._read_codex_access_token",
                   side_effect=AssertionError("should use pool token")), \
             patch("opencodon.core.auxiliary_client.OpenAI", return_value=real_client) as mock_openai:
...
class TestTryMainAgentModelFallback:
    ...
    def test_returns_none_when_main_provider_is_auto(self):
        from opencodon.core.auxiliary_client import _try_main_agent_model_fallback
        with patch("opencodon.core.auxiliary_client._read_main_provider", return_value="auto"), \
             patch("opencodon.core.auxiliary_client._read_main_model", return_value="some-model"):
    ...
    def test_returns_none_when_failed_provider_equals_main(self):
        ...
        from opencodon.core.auxiliary_client import _try_main_agent_model_fallback
        with patch("opencodon.core.auxiliary_client._read_main_provider", return_value="openrouter"), \
             patch("opencodon.core.auxiliary_client._read_main_model", return_value="anthropic/claude-sonnet-4"):
    ...
    def test_resolves_main_provider_client(self):
        ...
        with patch("opencodon.core.auxiliary_client._read_main_provider", return_value="openrouter"), \
             patch("opencodon.core.auxiliary_client._read_main_model", return_value="anthropic/claude-sonnet-4"), \
             patch("opencodon.core.auxiliary_client._is_provider_unhealthy", return_value=False), \
    ...
    def test_skips_when_main_provider_is_unhealthy(self):
        ...
        with patch("opencodon.core.auxiliary_client._read_main_provider", return_value="openrouter"), \
             patch("opencodon.core.auxiliary_client._read_main_model", return_value="anthropic/claude-sonnet-4"), \
             patch("opencodon.core.auxiliary_client._is_provider_unhealthy", return_value=True):
            client, model, label = _try_main_agent_model_fallback("glm", task="vision")
        assert client is None
...
def test_resolve_api_key_provider_skips_unconfigured_anthropic(monkeypatch):
    ...
    from opencodon_cli.auth import ProviderConfig
    ...
    monkeypatch.setattr("opencodon_cli.auth.PROVIDER_REGISTRY", fake_registry)
    monkeypatch.setattr(
        "opencodon_cli.auth.is_provider_explicitly_configured",
        lambda pid: False,
    )

    from opencodon.core.auxiliary_client import _resolve_api_key_provider
...
class TestTransientTransportRetry:
    ...
    def _patches(self, client):
        return (
            ...
            patch(
                "opencodon.core.auxiliary_client._get_cached_client",
                return_value=(client, "some-model"),
            ),
            patch(
                "opencodon.core.auxiliary_client._validate_llm_response",
    ...
    def test_second_transient_failure_escalates_to_fallback(self):
        ...
        with (
            p1, p2, p3,
            patch("opencodon.core.auxiliary_client._transient_retry_count", return_value=1),
            patch("opencodon.core.auxiliary_client._TRANSIENT_RETRY_BACKOFF_BASE", 0.0),
            patch(
                "opencodon.core.auxiliary_client._try_configured_fallback_chain",
                return_value=(None, None, ""),
            ),
            patch(
                "opencodon.core.auxiliary_client._try_main_agent_model_fallback",
...
class TestIsTimeoutError:
    ...
    def test_timed_out_string(self):
        from opencodon.core.auxiliary_client import _is_timeout_error
...
class TestIsConnectionError:
    ...
    def test_connection_refused(self):
        from opencodon.core.auxiliary_client import _is_connection_error
...
class TestStaleBaseUrlWarning:
    ...
    def test_warns_when_openai_base_url_set_with_named_provider(self, monkeypatch, caplog):
        ...
        import opencodon.core.auxiliary_client as mod
        ...
        with patch("opencodon.core.auxiliary_client._read_main_provider", return_value="openrouter"), \
             patch("opencodon.core.auxiliary_client._read_main_model", return_value="google/gemini-flash"), \
             caplog.at_level(logging.WARNING, logger="opencodon.core.auxiliary_client"):
            _resolve_auto()
...
class TestAuxiliaryTaskExtraBody:
    ...
    def test_empty_reasoning_effort_is_noop(self):
        ...
        from opencodon.core.auxiliary_client import _get_task_extra_body
    ...
    def test_anthropic_aux_client_forwards_extra_body_reasoning(self):
        ...
        from opencodon.core.auxiliary_client import _AnthropicCompletionsAdapter
    ...
    def test_no_warning_when_provider_is_custom(self, monkeypatch, caplog):
        ...
        with patch("opencodon.core.auxiliary_client._read_main_provider", return_value="custom"), \
             patch("opencodon.core.auxiliary_client._read_main_model", return_value="llama3"), \
             patch("opencodon.core.auxiliary_client._resolve_custom_runtime",
    ...
    def test_no_warning_when_provider_is_named_custom(self, monkeypatch, caplog):
        ...
        with patch("opencodon.core.auxiliary_client._read_main_provider", return_value="custom:ollama-local"), \
             patch("opencodon.core.auxiliary_client._read_main_model", return_value="llama3"), \
             patch("opencodon.core.auxiliary_client.resolve_provider_client",
...
class TestAnthropicCompatImageConversion:
    ...
    def test_known_providers_detected(self):
        from opencodon.core.auxiliary_client import _is_anthropic_compat_endpoint
    ...
    def test_base64_image_converted(self):
        from opencodon.core.auxiliary_client import _convert_openai_images_to_anthropic
        messages = [{
            "role": "user",
            "content": [
...
class TestAuxiliaryAuthRefreshRetry:
    def test_call_llm_refreshes_codex_on_401_for_vision(self):
        ...
        with (
            ...
            patch("opencodon.core.auxiliary_client._refresh_provider_credentials", return_value=True) as mock_refresh,
        ):
            resp = call_llm(
                task="vision",
                provider="openai-codex",
    ...
    def test_call_llm_refreshes_codex_on_401_for_non_vision(self):
        ...
        with (
            patch("opencodon.core.auxiliary_client._resolve_task_provider_model", return_value=("openai-codex", "gpt-5.4", None, None, None)),
            patch("opencodon.core.auxiliary_client._get_cached_client", side_effect=[(stale_client, "gpt-5.4"), (fresh_client, "gpt-5.4")]),
            patch("opencodon.core.auxiliary_client._refresh_provider_credentials", return_value=True) as mock_refresh,
        ):
            resp = call_llm(
                task="compression",
    ...
    def test_call_llm_refreshes_copilot_when_auto_routes_to_copilot_on_401(self):
        ...
        with (
            patch("opencodon.core.auxiliary_client._resolve_task_provider_model", return_value=("auto", None, None, None, None)),
            patch("opencodon.core.auxiliary_client._get_cached_client", side_effect=[(stale_client, "gpt-5.5"), (fresh_client, "gpt-5.5")]) as mock_get_client,
            patch("opencodon.core.auxiliary_client._refresh_provider_credentials", return_value=True) as mock_refresh,
            patch("opencodon.core.auxiliary_client._evict_cached_clients") as mock_evict,
        ):
            resp = call_llm(
                task="title_generation",
                messages=[{"role": "user", "content": "hi"}],
    ...
    def test_call_llm_refreshes_anthropic_on_401_for_non_vision(self):
        ...
        with (
            patch("opencodon.core.auxiliary_client._resolve_task_provider_model", return_value=("anthropic", "claude-haiku-4-5-20251001", None, None, None)),
            patch("opencodon.core.auxiliary_client._get_cached_client", side_effect=[(stale_client, "claude-haiku-4-5-20251001"), (fresh_client, "claude-haiku-4-5-20251001")]),
            patch("opencodon.core.auxiliary_client._refresh_provider_credentials", return_value=True) as mock_refresh,
        ):
            resp = call_llm(
                task="compression",
...
class TestAuxiliaryPoolRotationRetry:
    def test_call_llm_rotates_explicit_codex_pool_on_429(self):
        ...
        with (
            ...
            patch("opencodon.core.auxiliary_client._refresh_provider_credentials", return_value=False),
            patch("opencodon.core.auxiliary_client.load_pool", return_value=pool),
            patch("opencodon.core.auxiliary_client._try_payment_fallback") as mock_fallback,
        ):
            resp = call_llm(
                task="compression",
                provider="openai-codex",
...
class TestCodexAdapterReasoningTranslation:
    ...
    def _build_adapter():
        ...
        from opencodon.core.auxiliary_client import _CodexCompletionsAdapter
        from types import SimpleNamespace
...
class TestVisionAutoSkipsKimiCoding:
    ...
    def test_kimi_coding_skipped_falls_through_to_openrouter(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "opencodon.core.auxiliary_client._read_main_provider", lambda: "kimi-coding",
        )
        monkeypatch.setattr(
            "opencodon.core.auxiliary_client._read_main_model", lambda: "kimi-code",
        )
        ...
        monkeypatch.setattr(
            "opencodon.core.auxiliary_client.resolve_provider_client", rpc_mock,
        )
        ...
        monkeypatch.setattr(
            "opencodon.core.auxiliary_client._resolve_strict_vision_backend",
            fake_strict,
        )
    ...
    def test_kimi_coding_cn_skipped_too(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "opencodon.core.auxiliary_client._read_main_provider", lambda: "kimi-coding-cn",
        )
        monkeypatch.setattr(
            "opencodon.core.auxiliary_client._read_main_model", lambda: "kimi-code",
        )
    ...
    def test_explicit_override_to_kimi_coding_still_honored(self, monkeypatch):
        ...
        """
        monkeypatch.setattr(
            "opencodon.core.auxiliary_client._read_main_provider", lambda: "openrouter",
        )
        fake_kimi_client = MagicMock(name="kimi_client")
        gcc_mock = MagicMock(return_value=(fake_kimi_client, "kimi-code"))
        monkeypatch.setattr(
            "opencodon.core.auxiliary_client._get_cached_client", gcc_mock,
        )
    ...
    def test_skip_set_covers_exactly_known_entries(self):
        ...
        from opencodon.core.auxiliary_client import _PROVIDERS_WITHOUT_VISION
...
class TestAuxiliaryClientPoisonedCacheEviction:
    ...
    def test_evict_cached_client_instance_handles_none_and_misses(self):
        from opencodon.core.auxiliary_client import _evict_cached_client_instance
    ...
    def test_call_llm_evicts_on_connection_error_with_explicit_provider(self):
        ...
        """
        from opencodon.core.auxiliary_client import _client_cache, _client_cache_lock
        ...
        try:
            with patch(
                ...
                "opencodon.core.auxiliary_client._try_payment_fallback",
                return_value=(None, None, ""),
            ):
...
class TestNvidiaBillingHeaders:
    ...
    def test_resolve_provider_client_cloud_adds_billing_origin_header(self, monkeypatch):
        ...
        with patch("opencodon.core.auxiliary_client.OpenAI", mock_openai):
            client, model = resolve_provider_client(
                provider="nvidia",
...
class TestAuxUnhealthyCache:
    ...
    def test_resolve_auto_skips_unhealthy_step2(self):
        ...
        _mark_provider_unhealthy("openrouter")
        with patch("opencodon.core.auxiliary_client._read_main_provider", return_value=""), \
             patch("opencodon.core.auxiliary_client._read_main_model", return_value=""), \
             patch("opencodon.core.auxiliary_client._try_openrouter") as or_try, \
             patch("opencodon.core.auxiliary_client._try_custom_endpoint", return_value=(custom_client, "custom-model")), \
             patch("opencodon.core.auxiliary_client._resolve_api_key_provider", return_value=(None, None)):
            client, model = _resolve_auto()
    ...
    def test_resolve_auto_skips_unhealthy_main_in_step1(self):
        ...
        _mark_provider_unhealthy("openrouter")
        with patch("opencodon.core.auxiliary_client._read_main_provider", return_value="openrouter"), \
             patch("opencodon.core.auxiliary_client._read_main_model", return_value="anthropic/claude-sonnet-4.6"), \
             patch("opencodon.core.auxiliary_client.resolve_provider_client") as step1, \
             patch("opencodon.core.auxiliary_client._try_openrouter") as or_try, \
             patch("opencodon.core.auxiliary_client._try_custom_endpoint", return_value=(custom_client, "n-model")), \
             patch("opencodon.core.auxiliary_client._resolve_api_key_provider", return_value=(None, None)):
            client, model = _resolve_auto()
    ...
    def test_payment_fallback_skips_unhealthy(self):
        ...
        with patch("opencodon.core.auxiliary_client._read_main_provider", return_value="openrouter"), \
             patch("opencodon.core.auxiliary_client._try_openrouter") as or_try, \
             patch("opencodon.core.auxiliary_client._try_custom_endpoint") as custom_try, \
             patch("opencodon.core.auxiliary_client._resolve_api_key_provider",
    ...
    def test_call_llm_marks_provider_unhealthy_on_402(self, monkeypatch):
        ...
        with patch("opencodon.core.auxiliary_client._get_cached_client",
             ...
             patch("opencodon.core.auxiliary_client._build_call_kwargs",
...
class TestAuxiliaryMaxTokensParam:
    ...
    def test_direct_openai_returns_max_completion_tokens(self):
        with (
            patch("opencodon.core.auxiliary_client._current_custom_base_url",
...
class TestCompressionFallbackContextFilter:
    ...
    def test_configured_chain_skips_too_small_candidate_for_compression(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "opencodon.core.auxiliary_client._get_auxiliary_task_config",
            lambda task: {"fallback_chain": entries} if task == "compression" else {},
        )

        with patch("opencodon.core.auxiliary_client._resolve_fallback_entry",
                   side_effect=fake_resolve), \
             patch("opencodon.core.auxiliary_client.get_model_context_length",
    ...
    def test_configured_chain_continues_after_skipping_too_small(self, monkeypatch):
        ...
        from opencodon.core.auxiliary_client import _try_configured_fallback_chain
    ...
    def test_main_chain_skips_too_small_candidate_for_compression(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "opencodon_cli.fallback_config.get_fallback_chain",
            lambda cfg: chain,
        )
        ...
        with patch("opencodon.core.auxiliary_client._resolve_fallback_entry",
             ...
             patch("opencodon.core.auxiliary_client.get_model_context_length",
                   side_effect=fake_ctx), \
             patch("opencodon.core.auxiliary_client._is_provider_unhealthy",
                   return_value=False):
