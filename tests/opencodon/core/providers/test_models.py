...
class TestModelIds:
    def test_returns_non_empty_list(self):
        with patch("opencodon.core.providers.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
            ids = model_ids()
...
class TestFetchOpenRouterModels:
    ...
    def test_falls_back_to_static_snapshot_on_fetch_failure(self, monkeypatch):
        ...
        with patch("opencodon.core.providers.model_catalog.get_curated_openrouter_models", return_value=None), \
             patch("opencodon.core.providers.models._urlopen_model_catalog_request", side_effect=OSError("boom")):
            models = fetch_openrouter_models(force_refresh=True)
    ...
    def test_filters_out_models_without_tool_support(self, monkeypatch):
        ...
        opencodon is tool-calling-first — surfacing a non-tool model leads to
        ...
        monkeypatch.setattr(_models_mod, "_openrouter_catalog_cache", None)
        with (
            patch("opencodon.core.providers.model_catalog.get_curated_openrouter_models", return_value=[]),
            patch("opencodon.core.providers.models._urlopen_model_catalog_request", return_value=_Resp()),
        ):
            models = fetch_openrouter_models(force_refresh=True)
...
class TestOpenRouterToolSupportHelper:
    ...
    def test_tools_in_supported_parameters(self):
        from opencodon.core.providers.models import _openrouter_model_supports_tools
...
class TestFindOpenrouterSlug:
    def test_exact_match(self):
        from opencodon.core.providers.models import _find_openrouter_slug
        with patch("opencodon.core.providers.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
...
class TestCodexSoftAcceptPlausibilityGate:
    ...
    def test_unrelated_name_rejected_on_openai_codex(self):
        from opencodon.core.providers.models import validate_requested_model
