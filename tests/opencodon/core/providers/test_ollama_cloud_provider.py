...
from opencodon.core.providers.model_metadata import _URL_TO_PROVIDER, _PROVIDER_PREFIXES
from opencodon.core.providers.models_dev import PROVIDER_TO_MODELS_DEV, list_agentic_models
...
class TestOllamaCloudModelCatalog:
    ...
    def test_provider_model_ids_returns_dynamic_models(self, tmp_path, monkeypatch):
        ...
        with patch("opencodon.core.providers.models.fetch_api_models", return_value=["qwen3.5:397b"]), \
             patch("opencodon.core.providers.models_dev.fetch_models_dev", return_value=mock_mdev):
...
class TestOllamaCloudMergedDiscovery:
    def test_merges_live_and_models_dev(self, tmp_path, monkeypatch):
        ...
        with patch("opencodon.core.providers.models.fetch_api_models", return_value=["qwen3.5:397b", "glm-5"]), \
             patch("opencodon.core.providers.models_dev.fetch_models_dev", return_value=mock_mdev):
    ...
    def test_falls_back_to_models_dev_without_api_key(self, tmp_path, monkeypatch):
        ...
        with patch("opencodon.core.providers.models_dev.fetch_models_dev", return_value=mock_mdev):
            result = fetch_ollama_cloud_models(force_refresh=True)
    ...
    def test_uses_disk_cache(self, tmp_path, monkeypatch):
        ...
        with patch("opencodon.core.providers.models.fetch_api_models", return_value=["model-a"]) as mock_api, \
    ...
    def test_stale_cache_used_on_total_failure(self, tmp_path, monkeypatch):
        ...
        from opencodon.core.providers.models import fetch_ollama_cloud_models, _save_ollama_cloud_cache
        ...
        with patch("opencodon.core.providers.models.fetch_api_models", return_value=None), \
             patch("opencodon.core.providers.models_dev.fetch_models_dev", return_value={}):
            result = fetch_ollama_cloud_models(force_refresh=True)
...
class TestOllamaCloudAgentInit:
    ...
    def test_ollama_cloud_agent_uses_chat_completions(self, monkeypatch):
        ...
        monkeypatch.setenv("OLLAMA_API_KEY", "test-key")
        with patch("opencodon.core.run_agent.OpenAI") as mock_openai:
...
class TestOllamaCloudProvidersNew:
    ...
    def test_alias_resolves(self):
        from opencodon.core.providers import normalize_provider as np
...
class TestOllamaCloudSuffixStripping:
    ...
    def test_no_duplicate_when_live_clean_and_mdev_suffixed(self, tmp_path, monkeypatch):
        ...
        with patch("opencodon.core.providers.models.fetch_api_models", return_value=["kimi-k2.6", "glm-5.1"]), \
             patch("opencodon.core.providers.models_dev.fetch_models_dev", return_value=mock_mdev):
            result = fetch_ollama_cloud_models(force_refresh=True)
    ...
    def test_strip_suffix_helper(self):
        ...
        from opencodon.core.providers.models import _strip_ollama_cloud_suffix
