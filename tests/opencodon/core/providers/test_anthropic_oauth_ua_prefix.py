...
class TestOAuthUserAgentPrefix:
    ...
    def test_build_anthropic_client_oauth_ua(self):
        ...
        mock_sdk = MagicMock()
        with patch("opencodon.core.providers.anthropic_adapter._get_anthropic_sdk", return_value=mock_sdk):
    ...
    def test_no_claude_cli_in_source(self):
        ...
        import inspect
        import opencodon.core.providers.anthropic_adapter as mod
    ...
    def test_token_exchange_ua_not_throttled(self):
        """run_opencodon_oauth_login_pure must NOT send a throttled token-endpoint UA.
        ...
        try:
            source = inspect.getsource(mod.run_opencodon_oauth_login_pure)
        except AttributeError:
            pytest.skip("run_opencodon_oauth_login_pure not found")
        ...
        assert "_OAUTH_TOKEN_USER_AGENT" in source, (
            "run_opencodon_oauth_login_pure should send the shared "
