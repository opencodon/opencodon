...
def test_build_native_request_preserves_thought_signature_on_tool_replay():
    from opencodon.core.providers.gemini_native_adapter import build_gemini_request
...
def test_parallel_tool_results_merge_into_one_user_content():
    ...
    from opencodon.core.providers.gemini_native_adapter import _build_gemini_contents
...
def test_translate_native_response_surfaces_reasoning_and_tool_calls():
    from opencodon.core.providers.gemini_native_adapter import translate_gemini_response
        ...
        "candidates": [
            {
                "content": {
                    "parts": [
                        ...
                        {"functionCall": {"name": "search", "args": {"q": "opencodon"}}},
                    ]
                },
                "finishReason": "STOP",
            }
    ...
    assert json.loads(choice.message.tool_calls[0].function.arguments) == {"q": "opencodon"}
...
def test_native_client_uses_x_goog_api_key_and_native_models_endpoint(monkeypatch):
    ...
    monkeypatch.setattr("opencodon.core.providers.gemini_native_adapter.httpx.Client", lambda *a, **k: DummyHTTP())
...
def test_bare_gemini_model_id_strips_only_self_prefix(model, expected):
    from opencodon.core.providers.gemini_native_adapter import bare_gemini_model_id
...
def test_native_http_error_keeps_status_and_retry_after():
    from opencodon.core.providers.gemini_native_adapter import gemini_http_error
...
async def test_async_native_client_streams_without_requiring_async_iterator_from_sync_client():
    from opencodon.core.providers.gemini_native_adapter import AsyncGeminiNativeClient
...
def test_stream_event_translation_emits_tool_call_delta_with_stable_index():
    from opencodon.core.providers.gemini_native_adapter import translate_stream_event
...
def test_max_tokens_none_defaults_to_gemini_output_ceiling():
    ...
    maxOutputTokens is absent, truncating tool calls mid-stream. opencodon passes
    ...
    """
    from opencodon.core.providers.gemini_native_adapter import (
...
def test_x_goog_api_client_header_is_set():
    ...
    assert "opencodon/" in headers["X-Goog-Api-Client"], (
        "opencodon not found in X-Goog-Api-Client header"
    )


def test_x_goog_api_client_header_format():
    """Header value should be 'opencodon/<version>' matching the package version."""
    from opencodon.core.providers.gemini_native_adapter import GeminiNativeClient, _OPENCODON_VERSION

    client = GeminiNativeClient(api_key="fake-key", model="gemini-2.0-flash")
    headers = client._headers()

    expected = f"opencodon/{_OPENCODON_VERSION}"
    assert headers["X-Goog-Api-Client"] == expected


def test_user_agent_contains_version():
    """User-Agent should include the opencodon version."""
    from opencodon.core.providers.gemini_native_adapter import GeminiNativeClient, _OPENCODON_VERSION

    client = GeminiNativeClient(api_key="fake-key", model="gemini-2.0-flash")
    headers = client._headers()

    assert f"opencodon/{_OPENCODON_VERSION}" in headers["User-Agent"]


def test_opencodon_version_is_valid():
    """_OPENCODON_VERSION should be a non-empty string."""
    from opencodon.core.providers.gemini_native_adapter import _OPENCODON_VERSION
