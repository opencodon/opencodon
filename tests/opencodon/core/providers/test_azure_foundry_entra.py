...
class TestResolveAzureFoundryRuntimeEntra:
    ...
    def test_entra_inherits_codex_responses_for_gpt5_family(self, fake_azure_identity):
        ...
        ``opencodon_cli/models.py::azure_foundry_model_api_mode``)."""
        from opencodon.core.providers.runtime_provider import _resolve_azure_foundry_runtime
        runtime = _resolve_azure_foundry_runtime(
            requested_provider="azure-foundry",
            model_cfg={
    ...
    def test_entra_propagates_scope_only(self, fake_azure_identity):
        """``model.entra.scope`` is the only opencodon-managed Azure SDK
...
class TestAzureFoundryAuthStatus:
    def test_entra_status_does_not_mint_token(self, monkeypatch, tmp_path):
        ...
        from opencodon.core.credentials import auth as _auth
        ...
        # missing would raise.
        monkeypatch.setattr(
            "opencodon.core.providers.azure_identity_adapter.has_azure_identity_installed",
            lambda: True,
        )
