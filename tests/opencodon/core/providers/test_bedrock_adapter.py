...
class TestResolveAwsAuthEnvVar:
    ...
    def test_prefers_bearer_token_over_access_keys_and_profile(self):
        from opencodon.core.providers.bedrock_adapter import resolve_aws_auth_env_var
        env = {
...
class TestNormalizeConverseResponse:
    ...
    def test_stop_reason_mapping(self):
        from opencodon.core.providers.bedrock_adapter import _converse_stop_reason_to_openai
...
class TestNormalizeConverseStreamEvents:
    ...
    def test_text_stream(self):
        from opencodon.core.providers.bedrock_adapter import normalize_converse_stream_events
        events = {"stream": [
...
class TestDiscoverBedrockModels:
    ...
    def test_discovers_foundation_models(self):
        from opencodon.core.providers.bedrock_adapter import discover_bedrock_models, reset_discovery_cache
        ...
        with patch("opencodon.core.providers.bedrock_adapter._get_bedrock_control_client", return_value=mock_client):
    ...
    def test_handles_api_error_gracefully(self):
        ...
        with patch("opencodon.core.providers.bedrock_adapter._get_bedrock_control_client", side_effect=Exception("No creds")):
            models = discover_bedrock_models("us-east-1")
...
class TestExtractProviderFromArn:
    def test_extracts_anthropic(self):
        from opencodon.core.providers.bedrock_adapter import _extract_provider_from_arn
...
class TestBedrockErrorClassification:
    ...
    def test_context_overflow_validation_exception(self):
        from opencodon.core.providers.bedrock_adapter import classify_bedrock_error
...
class TestBedrockContextLength:
    ...
    def test_claude_opus_4_8(self):
        from opencodon.core.providers.bedrock_adapter import get_bedrock_context_length
    ...
    def test_unknown_model_gets_default(self):
        from opencodon.core.providers.bedrock_adapter import get_bedrock_context_length, BEDROCK_DEFAULT_CONTEXT_LENGTH
    ...
    def test_no_region_skips_probe_uses_table(self):
        ...
        from opencodon.core.providers.bedrock_adapter import get_bedrock_context_length
        with patch("opencodon.core.providers.bedrock_adapter.probe_bedrock_context_length") as mock_probe:
...
class TestBedrockContextProbe:
    ...
    def test_probe_parses_real_window_from_error(self):
        from opencodon.core.providers.bedrock_adapter import probe_bedrock_context_length
        err = (
        ...
        with patch("opencodon.core.providers.bedrock_adapter._get_bedrock_runtime_client",
...
class TestModelSupportsToolUse:
    ...
    def test_claude_supports_tools(self):
        from opencodon.core.providers.bedrock_adapter import _model_supports_tool_use
...
class TestIsAnthropicBedrockModel:
    ...
    def test_us_claude_sonnet(self):
        from opencodon.core.providers.bedrock_adapter import is_anthropic_bedrock_model
...
class TestEmptyTextBlockFix:
    ...
    def test_none_content_gets_placeholder(self):
        from opencodon.core.providers.bedrock_adapter import _convert_content_to_converse, _EMPTY_TEXT_PLACEHOLDER
    ...
    def test_real_text_preserved(self):
        from opencodon.core.providers.bedrock_adapter import _convert_content_to_converse
...
class TestInvalidateRuntimeClient:
    ...
    def test_returns_false_when_region_not_cached(self):
        from opencodon.core.providers.bedrock_adapter import invalidate_runtime_client, reset_client_cache
        reset_client_cache()
...
class TestIsStaleConnectionError:
    ...
    def test_detects_botocore_connection_closed_error(self):
        ...
        from opencodon.core.providers.bedrock_adapter import is_stale_connection_error
...
class TestRequireBoto3VersionCheck:
    ...
    def test_raises_runtime_error_when_boto3_too_old(self):
        ...
        from opencodon.core.providers.bedrock_adapter import _require_boto3
