...
class TestEstimateRequestTokensRough:
    def test_caches_tools_estimate(self):
        ...
        with patch("opencodon.core.providers.model_metadata.json.dumps", wraps=__import__("json").dumps) as dumps:
...
class TestDefaultContextLengths:
    def test_k3_context_is_scoped_to_confirmed_coding_endpoint(self):
        ...
        """
        with patch("opencodon.core.providers.model_metadata.get_cached_context_length", return_value=None), \
             patch("opencodon.core.providers.model_metadata.fetch_model_metadata", return_value={}), \
             patch("opencodon.core.providers.model_metadata.fetch_endpoint_model_metadata", return_value={}), \
             patch("opencodon.core.providers.model_metadata._query_ollama_api_show", return_value=None), \
             patch("opencodon.core.providers.models_dev.lookup_models_dev_context", return_value=None):
            accepted_urls = (
    ...
    def test_grok_substring_matching(self):
        ...
        with mock_patch("opencodon.core.providers.model_metadata.fetch_model_metadata", return_value={}),              mock_patch("opencodon.core.providers.model_metadata.fetch_endpoint_model_metadata", return_value={}),              mock_patch("opencodon.core.providers.model_metadata.get_cached_context_length", return_value=None):
            cases = [
    ...
    def test_xai_oauth_grok_build_uses_xai_models_dev_context(self):
        ...
        with patch("opencodon.core.providers.model_metadata.get_cached_context_length", return_value=None), \
             patch("opencodon.core.providers.model_metadata._query_ollama_api_show", return_value=None), \
             patch("opencodon.core.providers.models_dev.fetch_models_dev", return_value=registry):
            assert get_model_context_length(
                "grok-build-0.1",
                provider="xai-oauth",
    ...
    def test_deepseek_v4_models_1m_context(self):
        ...
        with mock_patch("opencodon.core.providers.model_metadata.fetch_model_metadata", return_value={}), \
             mock_patch("opencodon.core.providers.model_metadata.fetch_endpoint_model_metadata", return_value={}), \
             mock_patch("opencodon.core.providers.model_metadata.get_cached_context_length", return_value=None):
            cases = [
                ("deepseek-v4-pro", 1_000_000),
                ("deepseek-v4-flash", 1_000_000),
    ...
    def test_openrouter_live_metadata_beats_hardcoded_catchall(self):
        ...
        with mock_patch("opencodon.core.providers.model_metadata.fetch_model_metadata", return_value=live), \
             mock_patch("opencodon.core.providers.model_metadata._query_ollama_api_show", return_value=None), \
             mock_patch("opencodon.core.providers.model_metadata.get_cached_context_length", return_value=None), \
             mock_patch("opencodon.core.providers.models_dev.lookup_models_dev_context", return_value=None):
...
class TestCodexOAuthContextLength:
    ...
    def test_fallback_table_used_without_token(self):
        ...
        with patch("opencodon.core.providers.model_metadata.get_cached_context_length", return_value=None), \
             patch("opencodon.core.providers.model_metadata.save_context_length"):
    ...
    def test_live_probe_overrides_fallback(self):
        ...
        with patch("opencodon.core.providers.model_metadata.requests.get", return_value=fake_response), \
             patch("opencodon.core.providers.model_metadata.get_cached_context_length", return_value=None), \
             patch("opencodon.core.providers.model_metadata.save_context_length"):
    ...
    def test_live_catalogue_cache_is_scoped_to_access_token(self):
        ...
        from opencodon.core.providers import model_metadata as mm
        from opencodon.core.providers.model_metadata import get_model_context_length
        ...
        with patch(
            "opencodon.core.providers.model_metadata.requests.get",
            side_effect=[first_response, second_response],
        ) as mock_get, patch("opencodon.core.providers.model_metadata.save_context_length") as mock_save:
    ...
    def test_non_codex_providers_unaffected(self):
        ...
        with patch("opencodon.core.providers.model_metadata.fetch_model_metadata", return_value={}), \
             patch("opencodon.core.providers.model_metadata.fetch_endpoint_model_metadata", return_value={}), \
             patch("opencodon.core.providers.model_metadata.get_cached_context_length", return_value=None), \
             patch("opencodon.core.providers.models_dev.lookup_models_dev_context", return_value=None):
            ctx = get_model_context_length(
    ...
    def test_live_codex_context_replaces_stale_cache_in_both_directions(
        ...
        with patch("opencodon.core.providers.model_metadata.requests.get", return_value=fake_response) as mock_get:
    ...
    def test_codex_fallback_is_not_persisted(self, tmp_path, monkeypatch):
        ...
        with patch("opencodon.core.providers.model_metadata.requests.get", return_value=fake_response), \
             patch("opencodon.core.providers.model_metadata.save_context_length") as mock_save:
            ctx = mm.get_model_context_length(
                model="gpt-5.6-terra",
                base_url=base_url,
                api_key="expired-token",
...
class TestGetModelContextLength:
    @patch("opencodon.core.providers.model_metadata.fetch_model_metadata")
    ...
    def test_cache_takes_priority_over_api(self, mock_fetch, tmp_path):
        ...
        with patch("opencodon.core.providers.model_metadata._get_context_cache_path", return_value=cache_file):
    ...
    @patch("opencodon.core.providers.model_metadata.fetch_model_metadata")
    @patch("opencodon.core.providers.model_metadata.fetch_endpoint_model_metadata")
    ...
    def test_custom_endpoint_falls_back_to_hardcoded_catalog(self, mock_fetch):
        ...
        with (
            ...
            patch(
                "opencodon.core.providers.model_metadata._query_local_context_length",
                return_value=None,
            ),
            patch(
                "opencodon.core.providers.model_metadata.is_local_endpoint",
                return_value=False,
            ),
        ):
...
class TestStripProviderPrefix:
    ...
    def test_ollama_model_tag_not_mangled_in_context_lookup(self, mock_fetch):
        ...
        """
        mock_fetch.return_value = {}
        with patch("opencodon.core.providers.model_metadata.fetch_endpoint_model_metadata") as mock_ep, \
             patch("opencodon.core.providers.model_metadata._is_custom_endpoint", return_value=True):
...
class TestFetchModelMetadata:
    ...
    def test_fresh_disk_cache_skips_network(self, tmp_path, monkeypatch):
        ...
        with patch("opencodon.core.providers.model_metadata.requests.get") as mock_get:
    ...
    def test_force_refresh_bypasses_fresh_disk_cache(self, tmp_path, monkeypatch):
        ...
        with patch("opencodon.core.providers.model_metadata.requests.get", return_value=mock_response) as mock_get:
    ...
    def test_network_success_writes_disk_cache(self, tmp_path, monkeypatch):
        ...
        with patch("opencodon.core.providers.model_metadata.requests.get", return_value=mock_response):
    ...
    def test_network_failure_falls_back_to_stale_disk_cache(self, tmp_path, monkeypatch):
        ...
        with patch("opencodon.core.providers.model_metadata.requests.get", side_effect=Exception("Network error")):
            result = fetch_model_metadata(force_refresh=True)

        assert result["stale/model"]["context_length"] == 50000

    @patch("opencodon.core.providers.model_metadata.requests.get")
...
class TestParseContextLimitFromError:
    ...
    def test_get_context_length_from_vllm_max_model_len_error(self):
        from opencodon.core.providers.model_metadata import get_context_length_from_provider_error
...
class TestGrok43StaleCacheGuard:
    ...
    def test_suggests_grok_4_3(self):
        from opencodon.core.providers.model_metadata import _model_name_suggests_grok_4_3
