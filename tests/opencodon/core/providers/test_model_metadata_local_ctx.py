...
def _clear_local_ctx_probe_cache():
    ...
    """
    import opencodon.core.providers.model_metadata as _mm
...
class TestQueryLocalContextLengthOllama:
    ...
    def test_ollama_model_info_context_length(self):
        ...
        from opencodon.core.providers.model_metadata import _query_local_context_length
    ...
    def test_ollama_num_ctx_wins_over_model_info(self):
        ...
        training max — using it would let opencodon grow conversations past
        the runtime limit and Ollama would silently truncate.

        Concrete example: opencodon-brain:qwen3-14b-ctx32k is a Modelfile
        derived from qwen3:14b with `num_ctx 32768`, but the underlying
        GGUF reports `qwen3.context_length: 40960` (training max). If
        opencodon used 40960 it would let the conversation grow past 32768
        ...
        with patch("opencodon.core.providers.model_metadata.detect_local_server_type", return_value="ollama"), \
            ...
            result = _query_local_context_length(
                "opencodon-brain:qwen3-14b-ctx32k", "http://100.77.243.5:11434/v1"
            )

        assert result == 32768, (
            f"Expected num_ctx (32768) to win over model_info (40960), got {result}. "
            "If opencodon uses the GGUF training max, conversations will silently truncate."
        )
...
class TestQueryLocalContextLengthVllm:
    ...
    def test_vllm_max_model_len(self):
        ...
        with patch("opencodon.core.providers.model_metadata.detect_local_server_type", return_value="vllm"), \
             patch("httpx.Client", return_value=client_mock):
...
class TestQueryLocalContextLengthModelsList:
    ...
    def test_models_list_max_model_len(self):
        ...
        with patch("opencodon.core.providers.model_metadata.detect_local_server_type", return_value=None), \
             patch("httpx.Client", return_value=client_mock):
...
class TestQueryLocalContextLengthLmStudio:
    ...
    def test_lmstudio_exact_key_match(self):
        ...
        with patch("opencodon.core.providers.model_metadata.detect_local_server_type", return_value="lm-studio"), \
             patch("httpx.Client", return_value=client_mock):
            result = _query_local_context_length(
...
class TestFetchEndpointModelMetadataLmStudio:
    ...
    def test_uses_native_models_endpoint_only(self):
        from opencodon.core.providers.model_metadata import fetch_endpoint_model_metadata
        ...
        with patch("opencodon.core.providers.model_metadata.detect_local_server_type", return_value="lm-studio"), \
             patch("opencodon.core.providers.model_metadata.requests.get", return_value=native_resp) as mock_get:
...
class TestGetModelContextLengthLocalFallback:
    ...
    def test_local_endpoint_unknown_model_queries_server(self):
        ...
        with patch("opencodon.core.providers.model_metadata.get_cached_context_length", return_value=None), \
             ...
             patch("opencodon.core.providers.model_metadata.is_local_endpoint", return_value=True), \
             patch("opencodon.core.providers.model_metadata._query_local_context_length", return_value=131072), \
    ...
    def test_local_endpoint_stale_cache_reconciled_from_live_probe(self):
        ...
        with patch("opencodon.core.providers.model_metadata.get_cached_context_length", return_value=131072), \
             patch("opencodon.core.providers.model_metadata.fetch_endpoint_model_metadata", return_value={}), \
             patch("opencodon.core.providers.model_metadata.fetch_model_metadata", return_value={}), \
             patch("opencodon.core.providers.model_metadata._query_ollama_api_show", return_value=None), \
             patch("opencodon.core.providers.model_metadata._is_custom_endpoint", return_value=False), \
             patch("opencodon.core.providers.model_metadata.is_local_endpoint", return_value=True), \
             patch("opencodon.core.providers.model_metadata._query_local_context_length", return_value=32768), \
             patch("opencodon.core.providers.model_metadata._invalidate_cached_context_length") as mock_invalidate, \
             patch("opencodon.core.providers.model_metadata.save_context_length") as mock_save:
    ...
    def test_local_endpoint_stale_cache_reconciled_to_valid_live_probe(self):
        ...
        with patch("opencodon.core.providers.model_metadata.get_cached_context_length", return_value=131072), \
             ...
             patch("opencodon.core.providers.model_metadata._is_custom_endpoint", return_value=False), \
             patch("opencodon.core.providers.model_metadata.is_local_endpoint", return_value=True), \
             patch("opencodon.core.providers.model_metadata._query_local_context_length", return_value=65536), \
             patch("opencodon.core.providers.model_metadata._invalidate_cached_context_length") as mock_invalidate, \
             patch("opencodon.core.providers.model_metadata.save_context_length") as mock_save:
            result = get_model_context_length(model, base, provider="custom")
    ...
    def test_local_endpoint_server_returns_none_falls_back_to_2m(self):
        ...
        from opencodon.core.providers.model_metadata import get_model_context_length, CONTEXT_PROBE_TIERS

        with patch("opencodon.core.providers.model_metadata.get_cached_context_length", return_value=None), \
             patch("opencodon.core.providers.model_metadata.fetch_endpoint_model_metadata", return_value={}), \
             patch("opencodon.core.providers.model_metadata.fetch_model_metadata", return_value={}), \
             patch("opencodon.core.providers.model_metadata.is_local_endpoint", return_value=True), \
             patch("opencodon.core.providers.model_metadata._query_local_context_length", return_value=None):
            result = get_model_context_length("omnicoder-9b", "http://localhost:11434/v1")
    ...
    def test_non_local_endpoint_does_not_query_local_server(self):
        ...
        with patch("opencodon.core.providers.model_metadata.get_cached_context_length", return_value=None), \
             patch("opencodon.core.providers.model_metadata.fetch_endpoint_model_metadata", return_value={}), \
             patch("opencodon.core.providers.model_metadata.fetch_model_metadata", return_value={}), \
             patch("opencodon.core.providers.model_metadata.is_local_endpoint", return_value=False), \
             patch("opencodon.core.providers.model_metadata._query_local_context_length") as mock_query:
    ...
    def test_cached_result_skips_local_query(self):
        ...
        with patch("opencodon.core.providers.model_metadata.get_cached_context_length", return_value=65536), \
             patch("opencodon.core.providers.model_metadata.is_local_endpoint", return_value=False), \
             patch("opencodon.core.providers.model_metadata._query_local_context_length") as mock_query:
            result = get_model_context_length(
...
class TestLocalContextProbeTTLCache:
    ...
    def test_second_call_within_ttl_does_not_reprobe(self):
        ...
        with patch("opencodon.core.providers.model_metadata.detect_local_server_type", return_value="ollama") as detect, \
             patch("httpx.Client", return_value=client_mock):
    ...
    def test_none_result_not_cached(self):
        ...
        with patch("opencodon.core.providers.model_metadata.detect_local_server_type", return_value=None) as detect, \
             patch("httpx.Client", return_value=client_mock):
            first = _query_local_context_length("m", "http://localhost:11434/v1")
