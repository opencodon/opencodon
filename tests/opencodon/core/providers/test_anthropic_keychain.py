...
class TestReadClaudeCodeCredentialsFromKeychain:
    ...
    def test_returns_none_on_linux(self):
        ...
        with patch("opencodon.core.providers.anthropic_adapter.platform.system", return_value="Linux"):
            assert _read_claude_code_credentials_from_keychain() is None

    def test_returns_none_on_windows(self):
        with patch("opencodon.core.providers.anthropic_adapter.platform.system", return_value="Windows"):
            assert _read_claude_code_credentials_from_keychain() is None
    ...
    def test_returns_none_when_security_command_not_found(self):
        ...
        with patch("opencodon.core.providers.anthropic_adapter.platform.system", return_value="Darwin"), \
             patch("opencodon.core.providers.anthropic_adapter.subprocess.run",
    ...
    def test_returns_none_on_nonzero_exit_code(self):
        ...
        with patch("opencodon.core.providers.anthropic_adapter.platform.system", return_value="Darwin"), \
             patch("opencodon.core.providers.anthropic_adapter.subprocess.run") as mock_run:
...
class TestRefreshOAuthTokenAdoptsFreshCredential:
    ...
    def test_adopts_already_refreshed_token_without_posting(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "opencodon.core.providers.anthropic_adapter.refresh_anthropic_oauth_pure",
    ...
    def test_falls_back_to_network_refresh_when_no_fresh_credential(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "opencodon.core.providers.anthropic_adapter.refresh_anthropic_oauth_pure", _fake_refresh
        )
        monkeypatch.setattr(
            "opencodon.core.providers.anthropic_adapter._write_claude_code_credentials",
            lambda *a, **k: None,
        )
