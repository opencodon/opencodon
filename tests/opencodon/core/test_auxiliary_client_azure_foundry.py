...
that delegates to :func:`opencodon.core.runtime_provider._resolve_azure_foundry_runtime`
...
class TestAuxAzureFoundryApiKey:
    def test_chat_completions_returns_plain_openai_client(self, monkeypatch, patch_load_config):
        from opencodon.core.auxiliary_client import _try_azure_foundry
    ...
    def test_codex_responses_wraps_in_codex_aux_client(self, monkeypatch, patch_load_config):
        from opencodon.core.auxiliary_client import _try_azure_foundry, CodexAuxiliaryClient
...
class TestResolveProviderClientAzureFoundry:
    ...
    def test_warns_and_returns_none_on_failure(
        ...
        clear warning pointing at ``opencodon doctor``."""
        import logging
        ...
        assert resolved is None
        assert any(
            "azure-foundry" in rec.message and "opencodon doctor" in rec.message
