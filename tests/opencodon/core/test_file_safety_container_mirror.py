...
``…/sandboxes/<backend>/<task>/home/.opencodon/…`` prefix. This covers the
complementary inner-container case: when file tools execute inside Docker,
the bind-mount strips that prefix and the guard sees plain ``/root/.opencodon/…``.
...
class TestClassifyContainerMirrorTarget:
    def test_returns_none_without_context(self):
        """No Docker context — /root/.opencodon/… must not be flagged."""
        from opencodon.core.file_safety import classify_container_mirror_target

        assert classify_container_mirror_target("/root/.opencodon/profiles/group1/SOUL.md") is None
    ...
    def test_catches_soul_md_with_context(self):
        ...
        result = classify_container_mirror_target(
            "/root/.opencodon/profiles/group1/SOUL.md",
            mirror_prefix="/root/.opencodon",
        )
        assert result is not None
        assert result["mirror_root"].replace("\\", "/").endswith("root/.opencodon")
    ...
    def test_catches_authoritative_profile_files(self, inner):
        ...
        result = classify_container_mirror_target(
            f"/root/.opencodon/{inner}",
            mirror_prefix="/root/.opencodon",
        )
        assert result is not None
        assert result["inner_path"] == inner

    def test_non_opencodon_path_not_flagged(self):
        """/root/workspace/… is not .opencodon state and must not be blocked."""
        from opencodon.core.file_safety import classify_container_mirror_target
...
class TestGetContainerMirrorWarning:
    def test_warning_names_inner_path_and_bypass(self):
        from opencodon.core.file_safety import get_container_mirror_warning
...
class TestOrthogonality:
    ...
    def test_inner_container_path_caught_by_context_guard(self):
        ...
        path = "/root/.opencodon/profiles/group1/SOUL.md"

        assert classify_container_mirror_target(path) is None  # no context
        assert classify_container_mirror_target(path, mirror_prefix="/root/.opencodon") is not None
