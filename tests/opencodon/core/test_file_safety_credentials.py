...
for the credential stores while leaving arbitrary ``OPENCODON_HOME`` files
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_opencodon_home_path()`` at a tmp dir for isolated checks."""
    import opencodon.core.file_safety as fs

    home = tmp_path / "opencodon_home"
    home.mkdir()
    monkeypatch.setattr(fs, "_opencodon_home_path", lambda: home)
    return home
...
def test_arbitrary_opencodon_home_file_not_blocked(fake_home):
    """Non-credential files inside OPENCODON_HOME stay readable."""
    from opencodon.core.file_safety import get_read_block_error
...
def test_path_traversal_resolves_to_blocked(fake_home, tmp_path):
    """A path that traverses through a sibling dir back into OPENCODON_HOME's
    ...
    sibling.mkdir()
    traversal = sibling / ".." / "opencodon_home" / "auth.json"
...
def test_symlink_to_auth_json_blocked(fake_home, tmp_path):
    """A symlink pointing at OPENCODON_HOME/auth.json from outside the home
...
def test_read_file_tool_blocks_relative_path_under_terminal_cwd(
    ...
    ``read_file_tool`` against ``TERMINAL_CWD == OPENCODON_HOME`` must still
...
def test_dotenv_blocked(fake_home):
    """.env in OPENCODON_HOME holds API keys — blocked."""
    from opencodon.core.file_safety import get_read_block_error
...
def test_identically_named_opencodon_files_outside_home_not_blocked(
    ...
    project.mkdir()
    # auth.json outside OPENCODON_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside OPENCODON_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, OPENCODON_HOME = <root>/profiles/<name>, but
