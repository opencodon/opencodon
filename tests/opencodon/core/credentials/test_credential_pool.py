...
def test_round_robin_strategy_rotates_priorities(tmp_path, monkeypatch):
    ...
    config_path = tmp_path / "opencodon" / "config.yaml"
...
def test_random_strategy_uses_random_choice(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("opencodon.core.credentials.credential_pool.random.choice", lambda entries: entries[-1])
...
def test_explicit_reset_timestamp_overrides_default_429_ttl(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.core.credentials.auth._import_codex_cli_tokens",
        lambda: None,
    )
    _write_auth_store(
        tmp_path,
...
def test_mark_exhausted_and_rotate_persists_status(tmp_path, monkeypatch):
    ...
    auth_payload = json.loads((tmp_path / "opencodon" / "auth.json").read_text())
...
def test_billing_rotation_marks_all_entries_sharing_failed_key(tmp_path, monkeypatch):
    ...
    from opencodon.core.credentials.credential_pool import load_pool, STATUS_EXHAUSTED
...
def test_unmatched_api_key_hint_rotates_without_benching_innocent_key(tmp_path, monkeypatch):
    ...
    from opencodon.core.credentials.credential_pool import load_pool, STATUS_DEAD, STATUS_EXHAUSTED
...
def test_token_invalidated_marks_credential_dead(tmp_path, monkeypatch):
    ...
    from opencodon.core.credentials.credential_pool import load_pool, STATUS_DEAD
...
def test_load_pool_does_not_persist_env_seeded_secret_value(tmp_path, monkeypatch):
    ...
    auth_text = (tmp_path / "opencodon" / "auth.json").read_text()
...
def test_load_pool_collapses_duplicate_env_rows_to_active_key(tmp_path, monkeypatch):
    ...
    persisted = json.loads((tmp_path / "opencodon" / "auth.json").read_text())
...
def test_credential_pool_never_selects_empty_borrowed_entry():
    from opencodon.core.credentials.credential_pool import CredentialPool, PooledCredential
...
def test_pooled_credential_to_dict_strips_borrowed_secret_fields():
    from opencodon.core.credentials.credential_pool import PooledCredential
...
def test_write_credential_pool_sanitizes_borrowed_payload_at_disk_boundary(tmp_path, monkeypatch):
    ...
    from opencodon.core.credentials.auth import write_credential_pool
    ...
    write_credential_pool("openrouter", [
        {
            ...
            "auth_type": "api_key",
            "priority": 0,
            "source": "systemd://opencodon/openrouter",
            "access_token": sentinel,
            "refresh_token": f"refresh-{sentinel}",
            "agent_key": f"agent-{sentinel}",
            "api_key": f"extra-{sentinel}",
    ...
    assert borrowed["source"] == "systemd://opencodon/openrouter"
...
def test_write_credential_pool_preserves_known_provider_owned_oauth_state(tmp_path, monkeypatch):
    ...
    persisted = json.loads(
        (tmp_path / "opencodon" / "auth.json").read_text()
...
def test_load_pool_prefers_dotenv_over_stale_os_environ(tmp_path, monkeypatch):
    ...
    from a parent shell) must NOT shadow the fresh key in ~/.opencodon/.env when
    ...
    # User edited ~/.opencodon/.env with the fresh key
    (opencodon_home / ".env").write_text(
...
def test_load_pool_falls_back_to_os_environ_when_dotenv_empty(tmp_path, monkeypatch):
    """When ~/.opencodon/.env does not define OPENROUTER_API_KEY (typical Docker /
    ...
    (opencodon_home / ".env").write_text("SOME_OTHER_VAR=unrelated\n")
...
def test_load_pool_removes_stale_file_backed_singleton_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.core.providers.anthropic_adapter.read_opencodon_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
...
def test_singleton_seed_does_not_clobber_manual_oauth_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("ANTHROPIC_TOKEN", raising=False)
    monkeypatch.delenv("CLAUDE_CODE_OAUTH_TOKEN", raising=False)
    monkeypatch.setattr("opencodon.core.credentials.auth.is_provider_explicitly_configured", lambda pid: True)
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
...
def test_load_pool_api_key_path_skips_oauth_autodiscovery(tmp_path, monkeypatch):
    ...
    When the user picks "Anthropic API key" at `opencodon setup`,
    ...
    + claude-cli user-agent).  Autodiscovered Claude Code / opencodon PKCE
    ...
    monkeypatch.setattr("opencodon.core.providers.anthropic_adapter.read_opencodon_oauth_credentials", _fake_pkce)
    monkeypatch.setattr("opencodon.core.providers.anthropic_adapter.read_claude_code_credentials", _fake_cc)
...
def test_load_pool_api_key_path_prunes_stale_oauth_entries(tmp_path, monkeypatch):
    ...
    `opencodon setup` would still have those OAuth entries dormant on disk.
    ...
    monkeypatch.setattr("opencodon.core.credentials.auth.is_provider_explicitly_configured", lambda pid: True)
    monkeypatch.setattr("opencodon.core.providers.anthropic_adapter.read_opencodon_oauth_credentials", lambda: None)
    monkeypatch.setattr("opencodon.core.providers.anthropic_adapter.read_claude_code_credentials", lambda: None)
...
def test_load_pool_oauth_path_still_autodiscovers(tmp_path, monkeypatch):
    ...
    OAuth path at `opencodon setup`.  When ANTHROPIC_TOKEN is set (and
...
def test_least_used_strategy_selects_lowest_count(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OPENCODON_HOME", str(tmp_path / "opencodon"))
    monkeypatch.setattr(
        "opencodon.core.credentials.credential_pool.get_pool_strategy",
        lambda _provider: "least_used",
    )
    monkeypatch.setattr(
        "opencodon.core.credentials.credential_pool._seed_from_singletons",
        lambda provider, entries: (False, set()),
    )
    monkeypatch.setattr(
        "opencodon.core.credentials.credential_pool._seed_from_env",
        lambda provider, entries: (False, set()),
    )
    _write_auth_store(
        tmp_path,
...
def test_custom_endpoint_pool_keyed_by_name(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.core.credentials.credential_pool._seed_custom_pool",
...
def test_list_custom_pool_providers(tmp_path, monkeypatch):
    ...
    from opencodon.core.credentials.credential_pool import list_custom_pool_providers
...
def test_load_pool_seeds_copilot_via_gh_auth_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.core.credentials.copilot_auth.resolve_copilot_token",
...
def test_load_pool_seeds_qwen_oauth_via_cli_tokens(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.core.credentials.auth.resolve_qwen_runtime_credentials",
        lambda **kw: {
            "provider": "qwen-oauth",
...
class TestLeastUsedStrategy:
    ...
    def test_request_count_increments(self):
        ...
        from opencodon.core.credentials.credential_pool import CredentialPool, PooledCredential, STRATEGY_LEAST_USED
        ...
        with _patch("opencodon.core.credentials.credential_pool.get_pool_strategy", return_value=STRATEGY_LEAST_USED):
...
def test_sync_codex_entry_from_auth_store_adopts_newer_tokens(tmp_path, monkeypatch):
    ...
    # Simulate `opencodon auth openai-codex` replacing the token pair on disk.
...
def test_codex_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    ...
    rate-limit reset the user ran `opencodon model` to reauth, but the pool
    ...
    # Simulate `opencodon model` / `opencodon auth` refreshing the tokens.
...
def test_is_terminal_xai_oauth_refresh_error():
    from opencodon.core.credentials.auth import AuthError, _is_terminal_xai_oauth_refresh_error
...
def test_xai_oauth_terminal_refresh_clears_auth_json_and_removes_pool_entries(
    ...
    from opencodon.core.credentials.credential_pool import PooledCredential, load_pool
...
def test_is_terminal_codex_oauth_refresh_error():
    from opencodon.core.credentials.auth import AuthError, _is_terminal_codex_oauth_refresh_error
...
def test_persist_preserves_concurrent_disk_only_entry(tmp_path, monkeypatch):
    ...
    final = json.loads((tmp_path / "opencodon" / "auth.json").read_text())
...
def _make_anthropic_claude_code_pool(tmp_path, monkeypatch, *, access_token, refresh_token, expires_at_ms=9_999_999_999_000):
    ...
    monkeypatch.delenv("CLAUDE_CODE_OAUTH_TOKEN", raising=False)
    _write_auth_store(tmp_path, {"version": 1, "credential_pool": {}})
    monkeypatch.setattr("opencodon.core.credentials.auth.is_provider_explicitly_configured", lambda pid: pid == "anthropic")
    monkeypatch.setattr(
        "opencodon.core.providers.anthropic_adapter.read_opencodon_oauth_credentials",
        lambda: None,
    )
