"""Tests for `_can_open_graphical_browser()` in opencodon.core.credentials.auth.

Guards the fix for the May 2026 report where `opencodon auth add xai-oauth`
...
from opencodon.core.credentials.auth import _can_open_graphical_browser
...
def _force_platform_linux(monkeypatch):
    monkeypatch.setattr("opencodon.core.credentials.auth.sys.platform", "linux")
...
def test_non_linux_with_gui_allows(monkeypatch):
    ...
    monkeypatch.setattr("opencodon.core.credentials.auth.sys.platform", "darwin")
