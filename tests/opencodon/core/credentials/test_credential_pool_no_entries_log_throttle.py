...
fails the Desktop backend readiness handshake ("Timed out connecting to opencodon
...
def test_empty_pool_logs_once_within_throttle_window(monkeypatch, caplog):
    clock = _FakeClock()
    monkeypatch.setattr("opencodon.core.credentials.credential_pool.time.monotonic", clock)

    pool = CredentialPool("test", [])

    with caplog.at_level(logging.INFO, logger="opencodon.core.credentials.credential_pool"):
        for _ in range(50):
