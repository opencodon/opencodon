...
def test_xai_oauth_poll_device_token_waits_until_authorized(monkeypatch):
    ...
    monkeypatch.setattr("opencodon.core.credentials.auth.time.sleep", lambda _: None)
...
def test_resolve_xai_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("opencodon.core.credentials.auth._refresh_xai_oauth_tokens", _fake_refresh)
...
# ``XAI_BASE_URL`` / ``OPENCODON_XAI_BASE_URL`` are a credential-leak vector
...
def _seed_xai_oauth_state(
    opencodon_home: Path, state: dict, *, active_provider: str = "xai-oauth"
) -> None:
    opencodon_home.mkdir(parents=True, exist_ok=True)
    auth_store = {
        "version": 1,
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    monkeypatch.setattr("opencodon.core.credentials.auth._refresh_xai_oauth_tokens", _terminal_refresh)
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure(
    ...
    monkeypatch.setattr("opencodon.core.credentials.auth._refresh_xai_oauth_tokens", _transient_refresh)
...
def test_xai_oauth_discovery_raises_typed_error_on_malformed_json(monkeypatch):
    ...
    from opencodon.core.credentials.auth import _xai_oauth_discovery
    ...
    monkeypatch.setattr(
        "opencodon.core.credentials.auth.httpx.get",
...
def test_xai_oauth_discovery_validates_endpoints(monkeypatch):
    ...
    monkeypatch.setattr("opencodon.core.credentials.auth.httpx.get", _fake_get)
    with pytest.raises(AuthError) as exc:
        _xai_oauth_discovery()
    assert exc.value.code == "xai_discovery_invalid"
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    ...
    # nothing to import.
    raw_after = json.loads((opencodon_home / "auth.json").read_text())
...
def test_login_xai_oauth_relogin_clears_suppression_and_reseeds(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.core.credentials.auth._update_config_for_provider",
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("opencodon.core.credentials.auth.refresh_xai_oauth_pure", _fake_refresh)
...
def test_runtime_provider_default_base_url_when_pool_entry_missing_url(tmp_path, monkeypatch):
    ...
    from opencodon.core.credentials.credential_pool import load_pool, AUTH_TYPE_OAUTH, PooledCredential
    import uuid
...
def test_pool_entry_needs_refresh_when_jwt_within_skew(tmp_path, monkeypatch):
    ...
    from opencodon.core.credentials.credential_pool import load_pool, AUTH_TYPE_OAUTH, PooledCredential
    from opencodon.core.credentials.auth import XAI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS
    import uuid
...
def test_pool_refresh_marks_entry_exhausted_on_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("opencodon.core.credentials.auth.refresh_xai_oauth_pure", _fake_refresh_fail)
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    ...
    (opencodon_home / "auth.json").write_text(json.dumps(raw))
