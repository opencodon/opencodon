...
from opencodon.core.credentials.auth import resolve_provider, AuthError


def _login(monkeypatch, provider_id):
    """Simulate a logged-in OAuth active_provider in auth.json."""
    monkeypatch.setattr("opencodon.core.credentials.auth._load_auth_store",
                        lambda: {"active_provider": provider_id})
    monkeypatch.setattr("opencodon.core.credentials.auth.get_auth_status",
...
def _no_aws(monkeypatch):
    ...
    monkeypatch.setattr("opencodon.core.providers.bedrock_adapter.has_aws_credentials", lambda: False)


def _clear_provider_env(monkeypatch):
    for var in ("OPENAI_API_KEY", "OPENROUTER_API_KEY", "GLM_API_KEY", "ZAI_API_KEY",
                "KIMI_API_KEY", "MINIMAX_API_KEY", "OPENCODON_INFERENCE_PROVIDER"):
...
class TestProviderPrecedence:
    ...
    def test_warns_on_silent_oauth_fallthrough(self, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="opencodon.core.credentials.auth"):
            assert resolve_provider("auto") == "anthropic"
    ...
    def test_openrouter_pool_beats_stale_oauth(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.core.credentials.credential_pool.load_pool", lambda name: _Pool())
        assert resolve_provider("auto") == "openrouter"
