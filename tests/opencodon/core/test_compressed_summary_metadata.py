...
def _make_compressor():
    with patch(
        "opencodon.core.context_compressor.get_model_context_length", return_value=8000
    ):
...
def _compress(cc, msgs):
    ...
    with patch("opencodon.core.context_compressor.call_llm", return_value=resp):
...
class TestClassifySummaryContent:
    ...
    def test_delimiter_without_summary_prefix_is_none(self):
        ...
        from opencodon.core.context_compressor import _MERGED_SUMMARY_DELIMITER
...
class TestClassifyAgreesWithPredicatesOnLiveEmissions:
    ...
    def _assert_agreement(message):
        from opencodon.core.context_compressor import is_compaction_summary_message
