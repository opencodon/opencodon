...
class TestCacheFileReadBlocking:
    ...
    def test_hub_index_cache_blocked(self, tmp_path):
        ...
        cache = opencodon_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("opencodon.core.file_safety._opencodon_home_path", return_value=opencodon_home):
    ...
    def test_hub_directory_blocked(self, tmp_path):
        ...
        opencodon_home = tmp_path / ".opencodon"
        hub = opencodon_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_opencodon_home(self, tmp_path):
        """The env basename guard does not depend on OPENCODON_HOME resolution."""
        opencodon_home = tmp_path / ".opencodon"
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        opencodon_home = tmp_path / ".opencodon"
        cache = opencodon_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
