"""Tests for provider-aware `/model` validation in opencodon.core.models."""
...
def _validate(model, provider="openrouter", api_models=FAKE_API_MODELS, **kw):
    ...
    with patch("opencodon.core.models.fetch_api_models", return_value=api_models), \
         patch("opencodon.core.models.probe_api_models", return_value=probe_payload):
...
class TestProviderModelIds:
    ...
    def test_copilot_prefers_live_catalog(self):
        with patch("opencodon.core.auth.resolve_api_key_provider_credentials", return_value={"api_key": "gh-token"}), \
             patch("opencodon.core.models._fetch_github_models", return_value=["gpt-5.4", "claude-sonnet-4.6"]):
    ...
    def test_anthropic_provider_uses_configured_base_url_for_live_catalog(self):
        ...
        with patch(
            ...
            "opencodon.core.models._urlopen_model_catalog_request",
            return_value=_Resp(),
        ) as mock_urlopen:
...
class TestFetchApiModels:
    ...
    def test_returns_none_on_network_error(self):
        with patch("opencodon.core.models._urlopen_model_catalog_request", side_effect=Exception("timeout")):
    ...
    def test_probe_api_models_tries_v1_fallback(self):
        ...
        with patch("opencodon.core.models._urlopen_model_catalog_request", side_effect=_fake_urlopen):
    ...
    def test_probe_api_models_uses_copilot_catalog(self):
        ...
        with patch("opencodon.core.models._urlopen_model_catalog_request", return_value=_Resp()) as mock_urlopen:
    ...
    def test_fetch_github_model_catalog_filters_non_chat_models(self):
        ...
        with patch("opencodon.core.models._urlopen_model_catalog_request", return_value=_Resp()):
...
class TestValidateApiFallback:
    ...
    def test_known_model_accepted_via_catalog_when_api_down(self):
        ...
        with patch(
            "opencodon.core.models.provider_model_ids",
    ...
    def test_unknown_provider_soft_accepted_when_api_down(self):
        ...
        with patch("opencodon.core.models.provider_model_ids", return_value=[]):
    ...
    def test_fetch_lmstudio_models_filters_embedding_type(self):
        ...
        with patch("opencodon.core.models._urlopen_model_catalog_request", return_value=mock_resp):
    ...
    def test_fetch_lmstudio_models_normalizes_native_api_base_url(self):
        ...
        with patch("opencodon.core.models._urlopen_model_catalog_request", return_value=mock_resp) as mock_urlopen:
    ...
    def test_fetch_lmstudio_models_raises_auth_error_on_401(self):
        ...
        with patch("opencodon.core.models._urlopen_model_catalog_request", side_effect=http_error):
...
class TestValidateCodexAutoCorrection:
    ...
    def test_missing_dash_auto_corrects(self):
        ...
        with patch("opencodon.core.models.provider_model_ids", return_value=codex_models):
...
class TestProbeApiModelsUserAgent:
    ...
    def test_probe_sends_client_context_to_gemini(self):
        from unittest.mock import patch
        from opencodon.core.models import _OPENCODON_VERSION
