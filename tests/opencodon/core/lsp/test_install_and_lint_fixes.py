...
2. ``opencodon lsp status`` surfaces a ``Backend warnings`` section when
...
from opencodon.core.lsp.install import INSTALL_RECIPES
...
def test_install_npm_passes_extras_to_npm_command(tmp_path, monkeypatch):
    ...
    from opencodon.core.lsp import install as install_mod
...
def test_install_npm_works_without_extras(tmp_path, monkeypatch):
    ...
    install_targets = [c for c in cmd if not c.startswith("-") and c not in {
        "install", "--prefix", str(install_mod.opencodon_lsp_bin_dir().parent),
        "/usr/bin/npm",
    }]
...
def test_existing_binary_finds_windows_wrapper_in_staging(tmp_path, monkeypatch):
    ...
    wrapper = install_mod.opencodon_lsp_bin_dir() / "pyright-langserver.cmd"
...
def test_install_pip_finds_windows_scripts_launcher(tmp_path, monkeypatch):
    ...
    def fake_run(cmd, **kwargs):
        scripts_dir = install_mod.opencodon_lsp_bin_dir().parent / "python-packages" / "Scripts"
    ...
    assert (install_mod.opencodon_lsp_bin_dir() / "fake-language-server.exe").exists()


# ---------------------------------------------------------------------------
# Fix 2: ``opencodon lsp status`` surfaces shellcheck-missing for bash
# ---------------------------------------------------------------------------
...
def test_backend_warnings_quiet_when_bash_not_installed(tmp_path, monkeypatch):
    """No bash → no warning."""
    monkeypatch.setenv("OPENCODON_HOME", str(tmp_path))
    from opencodon.core.lsp import cli as lsp_cli
