...
def test_existing_previous_summary_is_not_serialized_again_as_new_turn():
    ...
    with patch("opencodon.core.context_compressor.call_llm", return_value=_response("updated summary")) as mock_call:
...
def test_handoff_in_protected_head_is_replaced_not_duplicated():
    ...
    with patch("opencodon.core.context_compressor.call_llm", return_value=_response("UPDATED summary body")):
...
def test_resume_handoff_in_protected_head_is_not_preserved_as_fossil():
    ...
    with patch("opencodon.core.context_compressor.call_llm", return_value=_response("fresh summary")):
        result = compressor.compress(_messages_with_handoff(old_summary))
...
def test_resume_handoff_after_default_protected_head_decays_initial_turns():
    ...
    with patch("opencodon.core.context_compressor.call_llm", return_value=_response("fresh summary")) as mock_call:
