...
from opencodon.core.title_generator import (
    generate_title,
    auto_title_session,
    maybe_auto_title,
    _title_language,
...
class TestGenerateTitle:
    ...
    def test_returns_title_on_success(self):
        ...
        with patch("opencodon.core.title_generator.call_llm", return_value=mock_response):
    ...
    def test_default_prompt_matches_user_language(self):
        ...
        with patch("opencodon.core.title_generator.call_llm", return_value=mock_response) as llm:
    ...
    def test_configured_language_pins_prompt(self):
        ...
        with (
            patch("opencodon.core.title_generator.call_llm", return_value=mock_response) as llm,
            patch("opencodon.core.title_generator._title_language", return_value="Japanese"),
        ):
    ...
    def test_default_timeout_delegates_to_auxiliary_config(self):
        ...
        with patch("opencodon.core.title_generator.call_llm", side_effect=mock_call_llm):
    ...
    def test_returns_none_on_exception(self):
        with patch("opencodon.core.title_generator.call_llm", side_effect=RuntimeError("no provider")):
            assert generate_title("question", "answer") is None
    ...
    def test_invokes_failure_callback_on_exception(self):
        ...
        with patch("opencodon.core.title_generator.call_llm", side_effect=exc):
    ...
    def test_failure_callback_errors_are_swallowed(self):
        ...
        with patch("opencodon.core.title_generator.call_llm", side_effect=RuntimeError("nope")):
    ...
    def test_skips_when_title_generation_disabled(self):
        ...
        with (
            ...
            patch("opencodon.core.title_generator.call_llm") as mock_call_llm,
        ):
            assert generate_title("question", "answer") is None
...
class TestAutoTitleSession:
    ...
    def test_skips_if_title_exists(self):
        ...
        with patch("opencodon.core.title_generator.generate_title") as gen:
    ...
    def test_generates_and_sets_title(self):
        ...
        with patch("opencodon.core.title_generator.generate_title", return_value="New Title"):
            auto_title_session(db, "sess-1", "hi", "hello")
    ...
    def test_does_not_overwrite_title_set_immediately_before_conditional_write(
        ...
        with patch(
            "opencodon.core.title_generator.generate_title",
    ...
    def test_invokes_title_callback_after_setting_title(self):
        ...
        db.set_auto_title_if_empty.return_value = True
        seen = []
        with patch("opencodon.core.title_generator.generate_title", return_value="Readable Session"):
            auto_title_session(
                db,
                "sess-1",
                "hello",
    ...
    def test_skips_if_generation_fails(self):
        ...
        with patch("opencodon.core.title_generator.generate_title", return_value=None):
            auto_title_session(db, "sess-1", "hi", "hello")
    ...
    def test_never_raises_when_body_throws(self):
        ...
        with patch(
            "opencodon.core.title_generator._auto_title_session",
    ...
    def test_body_exception_routed_to_failure_callback(self):
        ...
        with patch("opencodon.core.title_generator._auto_title_session", side_effect=boom):
            auto_title_session(
                db,
                "sess-1",
                "hi",
...
class TestMaybeAutoTitle:
    ...
    def test_skips_if_not_first_exchange(self):
        ...
        with patch("opencodon.core.title_generator.auto_title_session") as mock_auto:
    ...
    def test_skips_when_title_generation_disabled(self):
        ...
        with (
            patch("opencodon.config.load_config_readonly", return_value=config),
            patch("opencodon.core.title_generator.auto_title_session") as mock_auto,
        ):
            maybe_auto_title(db, "sess-1", "hello", "hi there", history)
...
class TestAutoTitleDuplicateHandling:
    ...
    def test_swallows_value_error_without_lineage_support(self):
        ...
        with patch(
            "opencodon.core.title_generator.generate_title", return_value="Dup Title"
        ):
            auto_title_session(db, "sess-1", "hi", "hello")  # must not raise
    ...
    def test_manual_title_race_skips_without_callback(self):
        ...
        from opencodon.core.title_generator import _persist_session_title
        db = MagicMock()
...
class TestRuntimeValidator:
    ...
    def test_skips_when_validator_returns_false(self):
        with patch("opencodon.core.title_generator.call_llm") as mock_llm:
    ...
    def test_allows_when_validator_returns_true(self):
        ...
        with patch("opencodon.core.title_generator.call_llm", return_value=mock_response) as mock_llm:
            title = generate_title(
                "question", "answer",
