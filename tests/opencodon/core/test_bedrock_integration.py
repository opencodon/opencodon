...
Note: Tests that import ``opencodon.core.credentials.auth`` or ``opencodon.core.providers.runtime_provider``
...
class TestResolveProvider:
    ...
    def test_auto_detect_with_aws_credentials(self, monkeypatch):
        ...
        with patch("opencodon.core.credentials.auth._load_auth_store", return_value={}):
...
class TestRuntimeProvider:
    ...
    def test_bedrock_runtime_resolution(self, monkeypatch):
        ...
        with patch("opencodon.core.providers.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("opencodon.core.providers.runtime_provider._get_model_config", return_value={"provider": "bedrock"}):
    ...
    def test_bedrock_runtime_no_credentials_raises_on_auto_detect(self, monkeypatch):
        ...
        with patch("opencodon.core.providers.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("opencodon.core.providers.runtime_provider._get_model_config", return_value={"provider": "bedrock"}), \
             patch("opencodon.core.providers.runtime_provider.resolve_requested_provider", return_value="auto"), \
...
class TestProvidersModule:
    ...
    def test_bedrock_alias_in_providers(self):
        from opencodon.core.providers import ALIASES
    ...
    def test_bedrock_transport_mapping(self):
        from opencodon.core.providers import TRANSPORT_TO_API_MODE
    ...
    def test_label_override(self):
        from opencodon.core.providers import _LABEL_OVERRIDES
...
class TestBedrockModelIdDetection:
    ...
    def test_bare_bedrock_id_detected(self):
        from opencodon.core.providers.anthropic_adapter import _is_bedrock_model_id
...
class TestAuxiliaryClientBedrockResolution:
    ...
    def test_bedrock_returns_client_with_credentials(self, monkeypatch):
        ...
        with patch("opencodon.core.providers.anthropic_adapter.build_anthropic_bedrock_client",
    ...
    def test_bedrock_non_claude_model_uses_converse_client(self, monkeypatch):
        ...
        with patch("opencodon.core.providers.anthropic_adapter.build_anthropic_bedrock_client") as mock_build:
    ...
    def test_bedrock_converse_shim_normalizes_string_stop(self, monkeypatch):
        ...
        with patch("opencodon.core.providers.bedrock_adapter.call_converse") as mock_converse:
    ...
    def test_bedrock_converse_shim_stream_returns_complete_response(self, monkeypatch):
        ...
        client = BedrockAuxiliaryClient("us-east-1", "openai.gpt-oss-20b-1:0")
        sentinel = object()
        with patch("opencodon.core.providers.bedrock_adapter.call_converse", return_value=sentinel) as mock_converse:
