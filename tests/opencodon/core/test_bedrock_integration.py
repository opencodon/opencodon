...
Note: Tests that import ``opencodon_cli.auth`` or ``opencodon_cli.runtime_provider``
...
class TestResolveProvider:
    ...
    def test_auto_detect_with_aws_credentials(self, monkeypatch):
        ...
        with patch("opencodon.frontends.cli.auth._load_auth_store", return_value={}):
...
class TestRuntimeProvider:
    ...
    def test_bedrock_runtime_resolution(self, monkeypatch):
        ...
        with patch("opencodon.frontends.cli.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("opencodon.frontends.cli.runtime_provider._get_model_config", return_value={"provider": "bedrock"}):
    ...
    def test_bedrock_runtime_no_credentials_raises_on_auto_detect(self, monkeypatch):
        ...
        with patch("opencodon.frontends.cli.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("opencodon.frontends.cli.runtime_provider._get_model_config", return_value={"provider": "bedrock"}), \
             patch("opencodon.frontends.cli.runtime_provider.resolve_requested_provider", return_value="auto"), \
...
class TestProvidersModule:
    """Verify bedrock is wired into opencodon_cli/providers.py."""

    def test_bedrock_alias_in_providers(self):
        from opencodon.frontends.cli.providers import ALIASES
    ...
    def test_bedrock_transport_mapping(self):
        from opencodon.frontends.cli.providers import TRANSPORT_TO_API_MODE
    ...
    def test_label_override(self):
        from opencodon.frontends.cli.providers import _LABEL_OVERRIDES
...
class TestErrorClassifierBedrock:
    ...
    def test_throttling_in_rate_limit_patterns(self):
        from opencodon.core.error_classifier import _RATE_LIMIT_PATTERNS
        assert "throttlingexception" in _RATE_LIMIT_PATTERNS

    def test_context_overflow_patterns(self):
        from opencodon.core.error_classifier import _CONTEXT_OVERFLOW_PATTERNS
...
class TestPackaging:
    ...
    def _optional_dependencies():
        ...
        content = (Path(__file__).resolve().parents[3] / "pyproject.toml").read_text()
    ...
    def test_bedrock_is_not_eager_installed_by_all_extra(self):
        extras = self._optional_dependencies()
        assert "opencodon[bedrock]" not in extras["all"]
...
class TestBedrockModelIdDetection:
    ...
    def test_bare_bedrock_id_detected(self):
        from opencodon.core.anthropic_adapter import _is_bedrock_model_id
...
class TestAuxiliaryClientBedrockResolution:
    ...
    def test_bedrock_returns_client_with_credentials(self, monkeypatch):
        ...
        with patch("opencodon.core.anthropic_adapter.build_anthropic_bedrock_client",
                   return_value=mock_anthropic_bedrock):
            from opencodon.core.auxiliary_client import resolve_provider_client, AnthropicAuxiliaryClient
    ...
    def test_bedrock_async_mode(self, monkeypatch):
        ...
        with patch("opencodon.core.anthropic_adapter.build_anthropic_bedrock_client",
                   return_value=MagicMock()):
            from opencodon.core.auxiliary_client import resolve_provider_client, AsyncAnthropicAuxiliaryClient
    ...
    def test_bedrock_non_claude_model_uses_converse_client(self, monkeypatch):
        ...
        with patch("opencodon.core.anthropic_adapter.build_anthropic_bedrock_client") as mock_build:
    ...
    def test_bedrock_converse_shim_normalizes_string_stop(self, monkeypatch):
        ...
        from opencodon.core.auxiliary_client import BedrockAuxiliaryClient

        client = BedrockAuxiliaryClient("us-east-1", "openai.gpt-oss-20b-1:0")
        with patch("opencodon.core.bedrock_adapter.call_converse") as mock_converse:
    ...
    def test_bedrock_converse_shim_stream_returns_complete_response(self, monkeypatch):
        ...
        client = BedrockAuxiliaryClient("us-east-1", "openai.gpt-oss-20b-1:0")
        sentinel = object()
        with patch("opencodon.core.bedrock_adapter.call_converse", return_value=sentinel) as mock_converse:
