...
def test_random_strategy_uses_random_choice(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("opencodon.core.credential_pool.random.choice", lambda entries: entries[-1])
...
def test_explicit_reset_timestamp_overrides_default_429_ttl(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.frontends.cli.auth._import_codex_cli_tokens",
        lambda: None,
    )
    _write_auth_store(
        tmp_path,
...
def test_unmatched_api_key_hint_rotates_without_benching_innocent_key(tmp_path, monkeypatch):
    ...
    from opencodon.core.credential_pool import load_pool, STATUS_DEAD, STATUS_EXHAUSTED
...
def test_token_invalidated_marks_credential_dead(tmp_path, monkeypatch):
    ...
    from opencodon.core.credential_pool import load_pool, STATUS_DEAD
...
def test_credential_pool_never_selects_empty_borrowed_entry():
    from opencodon.core.credential_pool import CredentialPool, PooledCredential
...
def test_pooled_credential_to_dict_strips_borrowed_secret_fields():
    from opencodon.core.credential_pool import PooledCredential
...
def test_write_credential_pool_sanitizes_borrowed_payload_at_disk_boundary(tmp_path, monkeypatch):
    ...
    from opencodon.frontends.cli.auth import write_credential_pool
...
def test_load_pool_removes_stale_file_backed_singleton_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.core.anthropic_adapter.read_opencodon_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
...
def test_singleton_seed_does_not_clobber_manual_oauth_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("ANTHROPIC_TOKEN", raising=False)
    monkeypatch.delenv("CLAUDE_CODE_OAUTH_TOKEN", raising=False)
    monkeypatch.setattr("opencodon.frontends.cli.auth.is_provider_explicitly_configured", lambda pid: True)
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
...
def test_load_pool_api_key_path_skips_oauth_autodiscovery(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("opencodon.core.anthropic_adapter.read_opencodon_oauth_credentials", _fake_pkce)
    monkeypatch.setattr("opencodon.core.anthropic_adapter.read_claude_code_credentials", _fake_cc)
...
def test_load_pool_api_key_path_prunes_stale_oauth_entries(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("opencodon.frontends.cli.auth.is_provider_explicitly_configured", lambda pid: True)
    monkeypatch.setattr("opencodon.core.anthropic_adapter.read_opencodon_oauth_credentials", lambda: None)
...
def test_least_used_strategy_selects_lowest_count(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OPENCODON_HOME", str(tmp_path / "opencodon"))
    monkeypatch.setattr(
        "opencodon.core.credential_pool.get_pool_strategy",
    ...
    monkeypatch.setattr(
        "opencodon.core.credential_pool._seed_from_env",
        lambda provider, entries: (False, set()),
    )
    _write_auth_store(
        tmp_path,
...
def test_custom_endpoint_pool_keyed_by_name(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.core.credential_pool._seed_custom_pool",
...
def test_list_custom_pool_providers(tmp_path, monkeypatch):
    ...
    from opencodon.core.credential_pool import list_custom_pool_providers
...
class TestLeastUsedStrategy:
    ...
    def test_request_count_increments(self):
        ...
        from opencodon.core.credential_pool import CredentialPool, PooledCredential, STRATEGY_LEAST_USED
        ...
        with _patch("opencodon.core.credential_pool.get_pool_strategy", return_value=STRATEGY_LEAST_USED):
...
def test_is_terminal_xai_oauth_refresh_error():
    from opencodon.frontends.cli.auth import AuthError, _is_terminal_xai_oauth_refresh_error
...
def test_xai_oauth_terminal_refresh_clears_auth_json_and_removes_pool_entries(
    ...
    from opencodon.core.credential_pool import PooledCredential, load_pool
...
def test_is_terminal_codex_oauth_refresh_error():
    from opencodon.frontends.cli.auth import AuthError, _is_terminal_codex_oauth_refresh_error
...
def test_persist_preserves_concurrent_disk_only_entry(tmp_path, monkeypatch):
    ...
    from opencodon.core.credential_pool import load_pool
    from opencodon.frontends.cli.auth import read_credential_pool, write_credential_pool
...
def _make_anthropic_claude_code_pool(tmp_path, monkeypatch, *, access_token, refresh_token, expires_at_ms=9_999_999_999_000):
    ...
    monkeypatch.delenv("CLAUDE_CODE_OAUTH_TOKEN", raising=False)
    _write_auth_store(tmp_path, {"version": 1, "credential_pool": {}})
    monkeypatch.setattr("opencodon.frontends.cli.auth.is_provider_explicitly_configured", lambda pid: pid == "anthropic")
    monkeypatch.setattr(
        "opencodon.core.anthropic_adapter.read_opencodon_oauth_credentials",
        lambda: None,
    )
