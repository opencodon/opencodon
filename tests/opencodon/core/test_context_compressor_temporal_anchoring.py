...
from opencodon.core.context_compressor import ContextCompressor, HISTORICAL_TASK_HEADING
...
def test_first_compaction_prompt_contains_dated_anchoring_rule():
    ...
    with patch.object(opencodon_time, "now", _fixed_now), patch(
        "opencodon.core.context_compressor.call_llm", return_value=_response("summary")
    ) as mock_call:
...
def test_iterative_update_prompt_also_contains_anchoring_rule():
    ...
    with patch.object(opencodon_time, "now", _fixed_now), patch(
        "opencodon.core.context_compressor.call_llm", return_value=_response("updated summary")
    ) as mock_call:
        compressor._generate_summary(_turns())
