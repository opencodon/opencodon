...
def test_runtime_main_sync_happens_after_restore():
    ...
    calls = []
    with patch(
        "opencodon.core.auxiliary_client.set_runtime_main",
...
def test_preflight_skips_when_persisted_cooldown_survives_restart(tmp_path):
    ...
    with patch("opencodon.core.turn_context._should_run_preflight_estimate", return_value=True), \
         patch("opencodon.core.turn_context.estimate_request_tokens_rough", return_value=999_999):
        ctx = _build(agent)
