...
from opencodon.core.prompt.prompt_builder import DEFAULT_AGENT_IDENTITY
...
def agent():
    ...
    with (
        patch(
            "opencodon.core.run_agent.get_tool_definitions", return_value=_make_tool_defs("web_search")
        ),
...
def agent_with_memory_tool():
    ...
    with (
        patch(
            "opencodon.core.run_agent.get_tool_definitions",
...
def test_aiagent_reuses_existing_errors_log_handler():
    ...
    error_log_path = (run_agent._opencodon_home / "logs" / "errors.log").resolve()
...
class TestSaveSessionLogRedactsSecrets:
    ...
    def _ensure_redaction_enabled(self, monkeypatch):
        """Force redaction on regardless of host OPENCODON_REDACT_SECRETS state.
...
class TestInit:
    def test_anthropic_base_url_accepted(self):
        ...
        with (
            ...
            patch("opencodon.core.run_agent.check_toolset_requirements", return_value={}),
            patch("opencodon.core.providers.anthropic_adapter._anthropic_sdk") as mock_anthropic,
        ):
            agent = AIAgent(
                api_key="test-key-1234567890",
    ...
    def test_prompt_caching_native_anthropic(self):
        ...
        with (
            patch("opencodon.core.run_agent.get_tool_definitions", return_value=[]),
            patch("opencodon.core.run_agent.check_toolset_requirements", return_value={}),
            patch("opencodon.core.providers.anthropic_adapter._anthropic_sdk"),
        ):
            a = AIAgent(
                api_key="test-key-1234567890",
                base_url="https://api.anthropic.com/v1/",
    ...
    def test_model_max_tokens_from_config(self):
        ...
        with (
            patch("opencodon.core.run_agent.get_tool_definitions", return_value=_make_tool_defs("terminal")),
            patch("opencodon.core.run_agent.check_toolset_requirements", return_value={}),
            patch("opencodon.core.run_agent.OpenAI"),
            patch(
                "opencodon.config.load_config",
    ...
    def test_valid_tool_names_populated(self):
        ...
        with (
            patch("opencodon.core.run_agent.get_tool_definitions", return_value=tools),
            patch("opencodon.core.run_agent.check_toolset_requirements", return_value={}),
            patch("opencodon.core.run_agent.OpenAI"),
        ):
            a = AIAgent(
...
class TestInterrupt:
    def test_interrupt_sets_flag(self, agent):
        with patch("opencodon.core.run_agent._set_interrupt"):
            agent.interrupt()
...
class TestBuildSystemPrompt:
    ...
    def test_can_use_soul_identity_even_when_context_files_are_skipped(self):
        with (
            ...
            patch("opencodon.core.run_agent.check_toolset_requirements", return_value={}),
            patch("opencodon.core.run_agent.OpenAI"),
            patch("opencodon.core.run_agent.load_soul_md", return_value="SOUL IDENTITY"),
        ):
            agent = AIAgent(
                api_key="test-k...7890",
                base_url="https://openrouter.ai/api/v1",
    ...
    def test_memory_guidance_when_memory_tool_loaded(self, agent_with_memory_tool):
        from opencodon.core.prompt.prompt_builder import MEMORY_GUIDANCE
    ...
    def test_skills_prompt_derives_available_toolsets_from_loaded_tools(self):
        ...
        with (
            patch("opencodon.core.run_agent.get_tool_definitions", return_value=tools),
            patch(
                "opencodon.core.run_agent.check_toolset_requirements",
                side_effect=AssertionError("should not re-check toolset requirements"),
            ),
            patch("opencodon.core.run_agent.get_toolset_for_tool", create=True, side_effect=toolset_map.get),
            patch("opencodon.core.run_agent.build_skills_system_prompt", return_value="SKILLS_PROMPT") as mock_skills,
            patch("opencodon.core.run_agent.OpenAI"),
        ):
            agent = AIAgent(
                api_key="test-k...7890",
...
class TestToolUseEnforcementConfig:
    ...
    def test_auto_injects_for_gpt(self):
        from opencodon.core.prompt.prompt_builder import TOOL_USE_ENFORCEMENT_GUIDANCE
    ...
    def test_auto_injects_execution_guidance_for_grok(self):
        ...
        """
        from opencodon.core.prompt.prompt_builder import OPENAI_MODEL_EXECUTION_GUIDANCE
        agent = self._make_agent(model="x-ai/grok-4.3", tool_use_enforcement="auto")
        prompt = agent._build_system_prompt()
...
class TestTaskCompletionGuidance:
    ...
    def test_default_injects_for_claude(self):
        ...
        from opencodon.core.prompt.prompt_builder import TASK_COMPLETION_GUIDANCE
...
class TestEnvironmentProbeIntegration:
    ...
    def test_probe_appears_when_problem_detected(self, monkeypatch):
        ...
        from opencodon.tools import env_probe
...
class TestBuildApiKwargs:
    ...
    def test_reasoning_xhigh_preserved_for_copilot_when_supported(self, agent, monkeypatch):
        ...
        monkeypatch.setattr(
            "opencodon.core.providers.models.github_model_reasoning_efforts",
...
class TestExecuteToolCalls:
    def test_single_tool_executed(self, agent):
        ...
        messages = []
        with patch(
            "opencodon.core.run_agent.handle_function_call", return_value="search result"
        ) as mock_hfc:
    ...
    def test_sequential_memory_remove_notifies_provider_with_tool_result(self, agent):
        ...
        with patch("opencodon.tools.memory_tool.memory_tool", return_value=json.dumps({"success": True})):
    ...
    def test_keyboard_interrupt_emits_cancelled_post_tool_hook(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.plugins_runtime.invoke_hook", _capture_hook)
        monkeypatch.setattr("opencodon.plugins_runtime.has_hook", lambda name: True)

        with (
            patch("opencodon.core.run_agent.handle_function_call", side_effect=KeyboardInterrupt),
            patch("opencodon.core.run_agent._set_interrupt"),
    ...
    def test_invalid_json_args_are_rejected_without_dispatch(self, agent):
        ...
        mock_msg = _mock_assistant_msg(content="", tool_calls=[tc])
        messages = []
        with patch("opencodon.core.run_agent.handle_function_call", return_value="ok") as mock_hfc:
            agent._execute_tool_calls(mock_msg, messages, "task-1")
    ...
    def test_result_truncation_over_100k(self, agent, tmp_path, monkeypatch):
        ...
        with patch("opencodon.core.run_agent.handle_function_call", return_value=big_result):
            agent._execute_tool_calls(mock_msg, messages, "task-1")
    ...
    def test_quiet_tool_output_suppressed_when_progress_callback_present(self, agent):
        ...
        with patch("opencodon.core.run_agent.handle_function_call", return_value="search result"), \
    ...
    def test_run_conversation_suppresses_retry_noise_in_parseable_quiet_mode(self, agent):
        ...
        with patch("opencodon.core.run_agent.time.sleep", return_value=None):
...
class TestConcurrentToolExecution:
    ...
    def test_concurrent_executes_all_tools(self, agent):
        ...
        with patch("opencodon.core.run_agent.handle_function_call", side_effect=fake_handle):
    ...
    def test_concurrent_submit_shutdown_error_returns_tool_errors(self, agent):
        ...
        with patch("opencodon.tools.daemon_pool.DaemonThreadPoolExecutor", ShutdownExecutor):
            agent._execute_tool_calls_concurrent(mock_msg, messages, "task-1")
    ...
    def test_concurrent_timeout_returns_finished_tools_without_hanging(self, agent, monkeypatch):
        ...
        monkeypatch.setenv("OPENCODON_CONCURRENT_TOOL_TIMEOUT_S", "0.1")
    ...
    def test_concurrent_timeout_prefers_late_real_result_over_timeout_message(self, agent, monkeypatch):
        ...
        with patch("opencodon.core.tool_executor.concurrent.futures.wait", side_effect=fake_wait), \
             patch("opencodon.core.run_agent.handle_function_call", side_effect=lambda name, args, task_id, **k: f"real-{args.get('q')}"):
            agent._execute_tool_calls_concurrent(mock_msg, messages, "task-1")
    ...
    def test_invoke_tool_dispatches_to_handle_function_call(self, agent):
        ...
        with patch("opencodon.core.run_agent.handle_function_call", return_value="result") as mock_hfc:
    ...
    def test_sequential_tool_callbacks_fire_in_order(self, agent):
        ...
        with patch("opencodon.core.run_agent.handle_function_call", return_value='{"success": true}'):
            agent._execute_tool_calls_sequential(mock_msg, messages, "task-1")
    ...
    def test_sequential_browser_type_callbacks_redact_api_key(self, agent):
        ...
        with patch("opencodon.core.run_agent.handle_function_call", return_value='{"success": true, "typed": "sk-pro...EFGH"}'):
            agent._execute_tool_calls_sequential(mock_msg, messages, "task-1")
    ...
    def test_concurrent_tool_callbacks_fire_for_each_tool(self, agent):
        ...
        with patch("opencodon.core.run_agent.handle_function_call", side_effect=['{"id":1}', '{"id":2}']):
            agent._execute_tool_calls_concurrent(mock_msg, messages, "task-1")
    ...
    def test_invoke_tool_handles_agent_level_tools(self, agent):
        ...
        with patch("opencodon.tools.todo_tool.todo_tool", return_value='{"ok":true}') as mock_todo:
    ...
    def test_invoke_tool_agent_level_tool_emits_terminal_post_tool_hook(self, agent, monkeypatch):
        ...
        hook_calls = []
        monkeypatch.setattr(
            "opencodon.plugins_runtime.resolve_pre_tool_block",
    ...
    def test_invoke_tool_blocked_returns_error_and_skips_execution(self, agent, monkeypatch):
        ...
        with patch("opencodon.tools.todo_tool.todo_tool", side_effect=AssertionError("should not run")) as mock_todo:
            result = agent._invoke_tool("todo", {"todos": []}, "task-1")
    ...
    def test_invoke_tool_blocked_skips_handle_function_call(self, agent, monkeypatch):
        ...
        with patch("opencodon.core.run_agent.handle_function_call", side_effect=AssertionError("should not run")):
            result = agent._invoke_tool("web_search", {"q": "test"}, "task-1")
    ...
    def test_sequential_agent_level_tool_execution_middleware_wraps_inline_dispatch(self, agent, monkeypatch):
        ...
        monkeypatch.setattr(
            "opencodon.plugins_runtime.invoke_middleware",
    ...
    def test_concurrent_agent_level_tool_preserves_request_middleware_trace(self, agent, monkeypatch):
        ...
        with patch("opencodon.tools.todo_tool.todo_tool", return_value='{"ok":true}'):
            agent._execute_tool_calls_concurrent(mock_msg, messages, "task-1")
    ...
    def test_blocked_memory_tool_does_not_reset_counter(self, agent, monkeypatch):
        ...
        with patch("opencodon.tools.memory_tool.memory_tool", side_effect=AssertionError("should not run")):
    ...
    def test_invoke_tool_memory_failed_remove_skips_provider_notification(self, agent, monkeypatch):
        ...
        with patch(
            "opencodon.tools.memory_tool.memory_tool",
...
class TestPathsOverlap:
    ...
    def test_same_path_overlaps(self):
        from opencodon.core.run_agent import _paths_overlap
...
class TestParallelScopePathNormalization:
    def test_extract_parallel_scope_path_normalizes_relative_to_cwd(self, tmp_path, monkeypatch):
        from opencodon.core.run_agent import _extract_parallel_scope_path
    ...
    def test_extract_parallel_scope_path_treats_relative_and_absolute_same_file_as_same_scope(self, tmp_path, monkeypatch):
        from opencodon.core.run_agent import _extract_parallel_scope_path, _paths_overlap
    ...
    def test_should_parallelize_tool_batch_rejects_same_file_with_mixed_path_spellings(self, tmp_path, monkeypatch):
        from opencodon.core.run_agent import _should_parallelize_tool_batch
...
class TestMcpParallelToolBatch:
    ...
    def test_mcp_tools_parallel_when_server_opted_in(self):
        ...
        from opencodon.core.run_agent import _should_parallelize_tool_batch
        from opencodon.tools.mcp_tool import _mcp_tool_server_names, _parallel_safe_servers, _lock
        with _lock:
...
class TestRunConversation:
    ...
    def test_tool_calls_then_stop(self, agent):
        ...
        with (
            patch("opencodon.core.run_agent.handle_function_call", return_value="search result") as mock_handle_function_call,
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            patch("opencodon.core.run_agent.handle_function_call", return_value="search result"),
            patch(
                "opencodon.plugins_runtime.has_hook",
                side_effect=lambda name: name in {"pre_api_request", "post_api_request"},
            ),
            patch("opencodon.plugins_runtime.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_api_request_error_hook_skips_payload_work_without_listener(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.plugins_runtime.has_hook", lambda name: False)
        monkeypatch.setattr("opencodon.plugins_runtime.invoke_hook", _invoke_hook)
    ...
    def test_request_scoped_api_hooks_skip_payload_work_without_listeners(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.plugins_runtime.has_hook", _has_hook)
    ...
    def test_context_compression_triggered(self, agent):
        ...
        with (
            patch("opencodon.core.run_agent.handle_function_call", return_value="result"),
            patch.object(
    ...
    def test_length_with_tool_calls_returns_partial_without_executing_tools(self, agent):
        ...
        with (
            patch("opencodon.core.run_agent.handle_function_call") as mock_handle_function_call,
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_truncated_tool_call_retries_once_before_refusing(self, agent):
        ...
        with (
            patch("opencodon.core.run_agent.handle_function_call", return_value='{"success":true}') as mock_hfc,
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
...
class TestRetryExhaustion:
    ...
    def test_invalid_response_returns_error_not_crash(self, agent):
        ...
        with (
            ...
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
            patch("opencodon.core.run_agent.time", self._make_fast_time_mock()),
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "opencodon" in agent._cached_system_prompt
...
class TestSafeWriter:
    ...
    def test_write_delegates_normally(self):
        ...
        from opencodon.core.run_agent import _SafeWriter
...
class TestBuildApiKwargsAnthropicMaxTokens:
    ...
    def test_max_tokens_passed_to_anthropic(self, agent):
        ...
        with patch("opencodon.core.providers.anthropic_adapter.build_anthropic_kwargs") as mock_build:
...
class TestAnthropicImageFallback:
    def test_build_api_kwargs_converts_multimodal_user_image_to_text(self, agent):
        ...
        with (
            patch("opencodon.tools.vision_tools.vision_analyze_tool", new=AsyncMock(return_value=json.dumps({"success": True, "analysis": "A cat sitting on a chair."}))),
            patch("opencodon.core.providers.anthropic_adapter.build_anthropic_kwargs") as mock_build,
        ):
            mock_build.return_value = {"model": "claude-sonnet-4-20250514", "messages": [], "max_tokens": 4096}
    ...
    def test_build_api_kwargs_reuses_cached_image_analysis_for_duplicate_images(self, agent):
        ...
        with (
            patch("opencodon.tools.vision_tools.vision_analyze_tool", new=mock_vision),
            patch("opencodon.core.providers.anthropic_adapter.build_anthropic_kwargs") as mock_build,
        ):
            mock_build.return_value = {"model": "claude-sonnet-4-20250514", "messages": [], "max_tokens": 4096}
            agent._build_api_kwargs(api_messages)
...
class TestFallbackAnthropicProvider:
    ...
    def test_fallback_to_anthropic_sets_api_mode(self, agent):
        ...
        with (
            ...
            patch("opencodon.core.providers.anthropic_adapter.resolve_anthropic_token", return_value=None),
        ):
...
def test_aiagent_uses_copilot_acp_client():
    with (
        ...
        patch("opencodon.core.providers.copilot_acp_client.CopilotACPClient") as mock_acp_client,
    ):
...
class TestAnthropicCredentialRefresh:
    def test_try_refresh_anthropic_client_credentials_rebuilds_client(self):
        ...
        with (
            patch("opencodon.core.providers.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-oat01-fresh-token"),
            patch("opencodon.core.providers.anthropic_adapter.build_anthropic_client", return_value=new_client) as rebuild,
        ):
    ...
    def test_try_refresh_anthropic_client_credentials_returns_false_when_token_unchanged(self):
        ...
        with (
            patch("opencodon.core.providers.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-oat01-same-token"),
            patch("opencodon.core.providers.anthropic_adapter.build_anthropic_client") as rebuild,
        ):
...
class TestOAuthFlagAfterCredentialRefresh:
    ...
    def test_oauth_flag_updates_api_key_to_oauth(self, agent):
        ...
        with (
            patch("opencodon.core.providers.anthropic_adapter.resolve_anthropic_token",
                  return_value="sk-ant-setup-oauth-token"),
            patch("opencodon.core.providers.anthropic_adapter.build_anthropic_client",
...
class TestMemoryNudgeCounterPersistence:
    ...
    def test_counters_initialized_in_init(self):
        ...
        with patch("opencodon.core.run_agent.get_tool_definitions", return_value=[]):
            a = AIAgent(
