...
def test_substantive_tool_only_turn_invalidates_older_housekeeping_fallback():
    ...
    """
    with (
        patch("opencodon.core.run_agent.get_tool_definitions", return_value=_tool_defs("todo", "web_search")),
    ...
    with (
        patch("opencodon.core.run_agent.handle_function_call", return_value="ok"),
...
def test_housekeeping_only_turn_still_sets_fallback():
    ...
    """
    with (
        patch("opencodon.core.run_agent.get_tool_definitions", return_value=_tool_defs("memory")),
        patch("opencodon.core.run_agent.check_toolset_requirements", return_value={}),
        patch("opencodon.core.run_agent.OpenAI"),
    ):
        agent = AIAgent(
