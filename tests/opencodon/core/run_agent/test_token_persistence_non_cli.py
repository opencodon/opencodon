...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    opencodon_state = ModuleType("opencodon_state")
    opencodon_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "opencodon.state", opencodon_state)

    session_search_mod = ModuleType("opencodon.tools.session_search_tool")
    ...
    monkeypatch.setitem(sys.modules, "opencodon.tools.session_search_tool", session_search_mod)

    agent = _make_agent(None, platform="acp")
    result = json.loads(agent._invoke_tool("session_search", {"query": "opencodon"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "opencodon"
