...
# time (inside a per-test hermetic OPENCODON_HOME), but this module previously
...
# ~/.opencodon/config.yaml saw a different limit at import vs call and the
...
def _pin_max_concurrent_children(monkeypatch):
    monkeypatch.setattr(
        "opencodon.tools.delegate_tool._get_max_concurrent_children",
