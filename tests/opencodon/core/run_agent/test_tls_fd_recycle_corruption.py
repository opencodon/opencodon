...
``open()`` (e.g. another thread's SQLite file), and the worker's
...
def test_force_close_tcp_sockets_shutdown_only_no_close():
    ...
    FD-recycling race that corrupted a SQLite store (issue #29507) will
...
def test_agent_abort_request_openai_client_does_not_call_client_close(caplog):
    ...
    with caplog.at_level(logging.INFO, logger="opencodon.core.run_agent"):
