...
class TestStreamInterruptBeforeRetry:
    ...
    @patch("opencodon.core.run_agent.AIAgent._abort_request_openai_client")
    @patch("opencodon.core.run_agent.AIAgent._create_request_openai_client")
    @patch("opencodon.core.run_agent.AIAgent._close_request_openai_client")
    def test_stale_stream_attempt_cannot_emit_late_chunks_after_retry(
        ...
        from tests.opencodon.core.run_agent.test_streaming import (
            _make_stream_chunk,
            _make_tool_call_delta,
        )

        monkeypatch.setenv("OPENCODON_STREAM_STALE_TIMEOUT", "0.05")
