...
opencodon preserves the callable end-to-end so the SDK refreshes tokens
...
class TestCreateOpenAIClientCallable:
    ...
    def test_callable_api_key_passed_to_openai_constructor(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.core.run_agent.OpenAI", fake_openai)
...
class TestTruncateTokenCallable:
    def test_callable_returns_placeholder(self):
        ...
        ``"<function ...>"``."""
        from opencodon.frontends.server.web_server import _truncate_token
...
class TestCliEnsureRuntimeCredentialsCallable:
    ...
    ``OpencodonCLI`` instance) — the predicate change is the load-bearing
...
class TestInlinedDisplayMasks:
    ...
    def test_cli_show_config_handles_callable(self):
        """``cli.OpencodonCLI.show_config`` previously did
        ...
        assert '"Microsoft Entra ID"' in src, (
            "cli.OpencodonCLI.show_config must print the static "
    ...
    def test_mask_api_key_for_logs_handles_callable(self):
        ...
        assert (
            ...
            "opencodon.core.run_agent._mask_api_key_for_logs must short-circuit for "
