...
import pytest
from opencodon.core.providers.codex_responses_adapter import _chat_content_to_responses_parts, _chat_messages_to_responses_input, _normalize_codex_response, _preflight_codex_input_items
...
def _make_agent(monkeypatch, provider, api_mode="chat_completions", base_url="https://openrouter.ai/api/v1", model=None):
    monkeypatch.setattr("opencodon.core.run_agent.get_tool_definitions", lambda **kw: _tool_defs("web_search", "terminal"))
...
# ── Chat completions response handling (OpenRouter) ──────────────────────────
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_custom_endpoint_when_no_aggregator(self, monkeypatch):
        """Custom endpoint is used when no aggregator keys are available.
