"""Live Fireworks smoke test — exercises the opencodon runtime, not a raw SDK client.

Opt-in only:
    OPENCODON_LIVE_TESTS=1 FIREWORKS_API_KEY=fw_... \\
        pytest tests/run_agent/test_fireworks_live.py -q

Unlike a bare OpenAI() client pointed at the endpoint, this drives opencodon'
...
def test_opencodon_wires_fireworks_client():
...
def test_fireworks_basic_chat_through_runtime():
    """A single-turn completion via the opencodon-resolved client returns text."""
    client, model = _resolve_runtime_client()
