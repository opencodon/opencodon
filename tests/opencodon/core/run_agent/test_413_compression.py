...
class TestHTTP413Compression:
    ...
    def test_413_strips_vision_payloads_when_compression_cannot_reduce_messages(self, agent):
        ...
        still carries binary data; opencodon should evict the image payload and
...
class TestPreflightCompression:
    ...
    def test_compress_context_emits_lifecycle_status_before_work(self, agent):
        ...
        with (
            ...
            patch("opencodon.core.run_agent.estimate_request_tokens_rough", return_value=42),
        ):
...
class TestToolResultPreflightCompression:
    ...
    def test_large_tool_results_trigger_compression(self, agent):
        ...
        with (
            patch("opencodon.core.run_agent.handle_function_call", return_value=large_result),
            patch.object(agent, "_compress_context") as mock_compress,
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
    ...
    def test_mid_turn_retry_compares_fully_assembled_requests(self, agent):
        ...
        with (
            ...
            patch("opencodon.core.run_agent.handle_function_call", return_value="x" * 100_000),
            patch.object(
                agent,
                "_compress_context",
