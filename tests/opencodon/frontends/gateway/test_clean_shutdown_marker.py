...
When the gateway shuts down gracefully (opencodon update, gateway restart, /restart),
...
from opencodon.frontends.gateway.config import GatewayConfig, Platform
from opencodon.frontends.gateway.session import SessionSource, SessionStore
...
class TestCleanShutdownMarker:
    ...
    def test_marker_written_on_graceful_stop(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.frontends.gateway.run._opencodon_home", tmp_path)
        ...
        with patch("opencodon.frontends.gateway.run.GatewayRunner._drain_active_agents", new_callable=AsyncMock, return_value=([], False)), \
             patch("opencodon.frontends.gateway.run.GatewayRunner._finalize_shutdown_agents"), \
             patch("opencodon.frontends.gateway.run.GatewayRunner._update_runtime_status"), \
             patch("opencodon.frontends.gateway.status.remove_pid_file"), \
...
class TestResumePendingFreshnessGate:
    ...
    def test_stale_resume_pending_falls_through_to_reset(self, tmp_path, monkeypatch):
        ...
        from opencodon.frontends.gateway.config import SessionResetPolicy
        store = _make_store(
