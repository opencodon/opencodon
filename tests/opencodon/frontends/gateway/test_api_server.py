...
from opencodon.frontends.gateway.platforms.api_server import (
    APIServerAdapter,
    ResponseStore,
    _IdempotencyCache,
...
class TestCheckRequirements:
    ...
    @patch("opencodon.frontends.gateway.platforms.api_server.AIOHTTP_AVAILABLE", False)
...
class TestAdapterInit:
    ...
    def test_create_agent_forwards_runtime_config(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "opencodon.frontends.gateway.run._resolve_runtime_agent_kwargs",
            lambda: {
        ...
        monkeypatch.setattr("opencodon.frontends.gateway.run._resolve_gateway_model", lambda: "gpt-5.5")
        monkeypatch.setattr(
            "opencodon.frontends.gateway.run._load_gateway_config",
            lambda: {
        ...
        monkeypatch.setattr(
            "opencodon.frontends.gateway.run.GatewayRunner._load_reasoning_config",
            staticmethod(lambda: {"enabled": True, "effort": "xhigh"}),
        )
        monkeypatch.setattr("opencodon.frontends.gateway.run.GatewayRunner._load_fallback_model", staticmethod(lambda: None))
        monkeypatch.setattr("opencodon.frontends.cli.tools_config._get_platform_tools", lambda *_: set())
    ...
    def test_create_agent_refreshes_max_iterations_from_runtime_config(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.frontends.gateway.run._resolve_gateway_model", lambda: "gpt-5")
        monkeypatch.setattr("opencodon.frontends.gateway.run._load_gateway_config", lambda: {"agent": {"max_turns": 200}})
        monkeypatch.setattr(
            "opencodon.frontends.gateway.run.GatewayRunner._load_reasoning_config",
            staticmethod(lambda: {}),
        )
        monkeypatch.setattr("opencodon.frontends.gateway.run.GatewayRunner._load_fallback_model", staticmethod(lambda: None))
        monkeypatch.setattr("opencodon.frontends.gateway.run._current_max_iterations", lambda: 200)
        monkeypatch.setattr("opencodon.frontends.cli.tools_config._get_platform_tools", lambda *_: set())
    ...
    def test_create_agent_handles_fallback_model_kwarg_collision(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.frontends.gateway.run._resolve_gateway_model", lambda: "primary/model")
        monkeypatch.setattr("opencodon.frontends.gateway.run._load_gateway_config", lambda: {})
        monkeypatch.setattr(
            "opencodon.frontends.gateway.run.GatewayRunner._load_reasoning_config",
            staticmethod(lambda: {}),
        )
        monkeypatch.setattr("opencodon.frontends.gateway.run.GatewayRunner._load_fallback_model", staticmethod(lambda: None))
        monkeypatch.setattr("opencodon.frontends.gateway.run._current_max_iterations", lambda: 90)
        monkeypatch.setattr("opencodon.frontends.cli.tools_config._get_platform_tools", lambda *_: set())
...
class TestHealthEndpoint:
    ...
    async def test_health_returns_ok(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert data["platform"] == "opencodon"
    ...
    def test_health_version_prefers_runtime_source_over_stale_metadata(self):
        ...
        The health endpoint must report the running opencodon source version, not
        stale ``opencodon_agent-*.dist-info`` metadata from before a source update.
        """
        from opencodon.frontends.cli import __version__

        with patch("importlib.metadata.version", return_value="0.18.0"):
            assert _opencodon_version() == __version__
...
class TestHealthDetailedEndpoint:
    ...
    async def test_health_detailed_returns_ok(self, adapter):
        ...
        app = _create_app(adapter)
        with patch("opencodon.frontends.gateway.status.read_runtime_status", return_value={
    ...
    async def test_health_detailed_no_runtime_status(self, adapter):
        ...
        app = _create_app(adapter)
        with patch("opencodon.frontends.gateway.status.read_runtime_status", return_value=None):
            async with TestClient(TestServer(app)) as cli:
                resp = await cli.get("/health/detailed")
                assert resp.status == 200
                data = await resp.json()
    ...
    async def test_health_detailed_allows_authenticated_request(self, auth_adapter):
        ...
        with patch("opencodon.frontends.gateway.status.read_runtime_status", return_value={"gateway_state": "running"}):
            async with TestClient(TestServer(app)) as cli:
    ...
    async def test_health_detailed_reports_runtime_readiness(self, adapter):
        ...
        with patch("opencodon.frontends.gateway.status.read_runtime_status", return_value={"gateway_state": "running"}), \
             patch("opencodon.frontends.gateway.platforms.api_server.collect_runtime_readiness", return_value=expected):
            async with TestClient(TestServer(app)) as cli:
                resp = await cli.get("/health/detailed")
                assert resp.status == 200
                data = await resp.json()
    ...
    async def test_public_health_does_not_run_readiness_probes(self, adapter):
        app = _create_app(adapter)
        with patch("opencodon.frontends.gateway.platforms.api_server.collect_runtime_readiness") as probe:
            async with TestClient(TestServer(app)) as cli:
                resp = await cli.get("/health")
                assert resp.status == 200
...
class TestModelsEndpoint:
    @pytest.mark.asyncio
    async def test_models_returns_opencodon_agent(self, adapter):
        app = _create_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            ...
            assert data["data"][0]["id"] == "opencodon"
            assert data["data"][0]["owned_by"] == "opencodon"

    @pytest.mark.asyncio
    async def test_models_returns_profile_name(self):
        """When running under a named profile, /v1/models advertises the profile name."""
        with patch("opencodon.frontends.gateway.platforms.api_server.APIServerAdapter._resolve_model_name", return_value="lucas"):
            adapter = _make_adapter()
        app = _create_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.get("/v1/models")
...
class TestCapabilitiesEndpoint:
    ...
    async def test_capabilities_advertises_plugin_safe_contract(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert resp.status == 200
            data = await resp.json()
            assert data["object"] == "opencodon.api_server.capabilities"
            assert data["platform"] == "opencodon"
            assert data["model"] == "opencodon"
...
class TestToolsetsEndpoint:
    ...
    async def test_toolsets_returns_resolved_tools(self, adapter):
        ...
        with patch(
            "opencodon.frontends.cli.tools_config._get_effective_configurable_toolsets",
            return_value=fake_toolsets,
        ), patch(
            "opencodon.frontends.cli.tools_config._get_platform_tools",
...
class TestChatCompletionsEndpoint:
    ...
    async def test_stream_task_done_callback_enqueues_eos_for_chat_completions(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with (
                ...
                patch("opencodon.frontends.gateway.platforms.api_server.asyncio.ensure_future", side_effect=_fake_ensure_future),
    ...
    async def test_stream_sends_keepalive_during_quiet_tool_gap(self, adapter):
        ...
        import asyncio
        import opencodon.frontends.gateway.platforms.api_server as api_server_mod
    ...
    async def test_stream_includes_tool_progress(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", side_effect=_mock_run_agent):
                ...
                assert "event: opencodon.tool.progress" in body
    ...
    async def test_stream_emits_tool_lifecycle_with_call_id(self, adapter):
        ...
        ``tool.started``-style ``opencodon.tool.progress`` event; clients
        ...
        of the lifecycle pair on the same ``event: opencodon.tool.progress``
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            for i, line in enumerate(lines):
                if line.strip() != "event: opencodon.tool.progress":
                    continue
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp1 = await cli.post(
                    "/v1/responses",
                    json={"model": "opencodon", "input": "What is 1+1?"},
                )
    ...
    async def test_chained_rotation_propagates_effective_session_id(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_create_agent", return_value=mock_agent_1):
                resp1 = await cli.post(
                    "/v1/responses",
                    json={"model": "opencodon", "input": "Hello"},
                )
            assert resp1.status == 200
            data1 = await resp1.json()
...
class TestResponsesStreaming:
    ...
    async def test_stream_true_returns_responses_sse(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", side_effect=_mock_run_agent):
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "opencodon", "input": "hi", "stream": True},
                )
                assert resp.status == 200
                assert "text/event-stream" in resp.headers.get("Content-Type", "")
                body = await resp.text()
    ...
    async def test_stream_emits_function_call_and_output_items(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", side_effect=_mock_run_agent):
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "opencodon", "input": "read the file", "stream": True},
                )
                assert resp.status == 200
                body = await resp.text()
    ...
    async def test_streamed_response_is_stored_for_get(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", side_effect=_mock_run_agent):
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "opencodon", "input": "store this", "stream": True},
                )
                body = await resp.text()
                response_id = None
                for line in body.splitlines():
    ...
    async def test_stream_cancelled_persists_incomplete_snapshot(self, adapter):
        ...
        import opencodon.frontends.gateway.platforms.api_server as api_mod
        import queue as _q
    ...
    async def test_tool_calls_in_output(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "opencodon", "input": "What is 6*7?"},
                )
...
class TestChatCompletionsAgentIncomplete:
    ...
    async def test_truncation_with_partial_text_uses_length_finish_reason(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    json={"model": "opencodon", "messages": [{"role": "user", "content": "tell me everything"}]},
                )
            assert resp.status == 200
            data = await resp.json()
            assert data["choices"][0]["finish_reason"] == "length"
            assert data["choices"][0]["message"]["content"] == "Here is part one of the answer"
            assert data["opencodon"]["partial"] is True
            assert data["opencodon"]["completed"] is False
            assert data["opencodon"]["error_code"] == "output_truncated"
            assert resp.headers.get("X-opencodon-Completed") == "false"
            assert resp.headers.get("X-opencodon-Partial") == "true"
    ...
    async def test_hard_failure_redacts_secret_like_error_text(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    json={"model": "opencodon", "messages": [{"role": "user", "content": "hello"}]},
                )
            ...
            body = json.dumps(data)
            assert raw_secret not in body
            assert raw_secret not in resp.headers.get("X-opencodon-Error", "")
            assert "OPENAI_API_KEY=" in body
            assert data["error"]["opencodon"]["failed"] is True
    ...
    async def test_failure_with_no_text_returns_502_error_envelope(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    json={"model": "opencodon", "messages": [{"role": "user", "content": "x"}]},
                )
            ...
            assert data["error"]["opencodon"]["partial"] is True
            assert data["error"]["opencodon"]["failed"] is True
            assert resp.headers.get("X-opencodon-Completed") == "false"
    ...
    async def test_normal_completion_unchanged(self, adapter):
        ...
        and no opencodon extras (preserves the existing happy-path contract)."""
        mock_result = {
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    json={"model": "opencodon", "messages": [{"role": "user", "content": "hi"}]},
                )
            assert resp.status == 200
            data = await resp.json()
            assert data["choices"][0]["finish_reason"] == "stop"
            assert data["choices"][0]["message"]["content"] == "All good."
            assert "opencodon" not in data
            assert "X-opencodon-Completed" not in resp.headers
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    ...
                    json={"model": "opencodon", "messages": [{"role": "user", "content": "Continue"}]},
                )
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    ...
                    json={"model": "opencodon", "input": "hello", "store": False},
                )
            assert resp.status == 200
...
def _patch_create_agent_runtime(monkeypatch, captured: dict, fake_agent_cls):
    ...
    monkeypatch.setattr("opencodon.frontends.gateway.run._resolve_gateway_model", lambda: "global/model")
    monkeypatch.setattr("opencodon.frontends.gateway.run._load_gateway_config", lambda: {})
    monkeypatch.setattr(
        "opencodon.frontends.gateway.run.GatewayRunner._load_reasoning_config", staticmethod(lambda model="": {})
    )
    monkeypatch.setattr(
        "opencodon.frontends.gateway.run.GatewayRunner._load_fallback_model", staticmethod(lambda: None)
    )
    monkeypatch.setattr("opencodon.frontends.gateway.run._current_max_iterations", lambda: 90)
    monkeypatch.setattr("opencodon.frontends.cli.tools_config._get_platform_tools", lambda *_: set())
...
class TestModelRoutesParsing:
    ...
    def test_no_routes_configured(self):
        adapter = _make_routing_adapter({})
        assert adapter._resolve_route("opencodon") is None
...
class TestModelRoutesAgentCreation:
    ...
    def test_route_provider_resolves_provider_credentials(self, monkeypatch):
        ...
        _patch_create_agent_runtime(monkeypatch, captured, FakeAgent)
        monkeypatch.setattr(
            "opencodon.frontends.gateway.run._resolve_runtime_agent_kwargs_for_provider",
            lambda provider: {
                "provider": provider,
    ...
    def test_session_override_lookup_reads_gateway_runner(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.frontends.gateway.run._gateway_runner_ref", lambda: FakeRunner())
