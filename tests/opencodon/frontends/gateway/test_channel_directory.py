...
def _isolate_channel_aliases(tmp_path_factory):
    ...
    ~/.opencodon/channel_aliases.json never leaks into directory tests. Tests
    ...
    with patch("opencodon.frontends.gateway.channel_directory.CHANNEL_ALIASES_PATH", missing):
        yield
...
class TestLoadDirectory:
    def test_missing_file(self, tmp_path):
        with patch("opencodon.frontends.gateway.channel_directory.DIRECTORY_PATH", tmp_path / "nope.json"):
...
class TestBuildChannelDirectoryOffload:
    def test_discord_builder_runs_off_event_loop_thread(self, tmp_path):
        ...
        with patch("opencodon.frontends.gateway.channel_directory._build_discord", side_effect=fake_build_discord), \
             patch("opencodon.frontends.gateway.channel_directory.DIRECTORY_PATH", cache_file):
    ...
    def test_session_discovery_runs_off_event_loop_thread(self, tmp_path):
        ...
        with patch("opencodon.frontends.gateway.channel_directory._build_from_sessions", side_effect=fake_build_from_sessions), \
             patch("opencodon.frontends.gateway.channel_directory.DIRECTORY_PATH", cache_file):
    ...
    def test_plugin_session_discovery_runs_off_event_loop_thread(self, tmp_path):
        ...
        with patch("opencodon.frontends.gateway.channel_directory._build_from_sessions", side_effect=fake_build_from_sessions), \
             patch("opencodon.frontends.gateway.channel_directory.DIRECTORY_PATH", cache_file), \
             patch(
                 "opencodon.frontends.gateway.platform_registry.platform_registry.plugin_entries",
...
class TestResolveChannelName:
    def _setup(self, tmp_path, platforms):
        ...
        return patch("opencodon.frontends.gateway.channel_directory.DIRECTORY_PATH", cache_file)
...
class TestChannelAliases:
    ...
    def _setup_aliases(self, tmp_path, aliases):
        ...
        return patch("opencodon.frontends.gateway.channel_directory.CHANNEL_ALIASES_PATH", alias_file)
    ...
    def test_no_alias_file_is_noop(self, tmp_path):
        ...
        with patch("opencodon.frontends.gateway.channel_directory.DIRECTORY_PATH", cache_file), \
             patch("opencodon.frontends.gateway.channel_directory.CHANNEL_ALIASES_PATH", tmp_path / "nope.json"):
            result = load_directory()
    ...
    def test_corrupt_alias_file_is_ignored(self, tmp_path):
        ...
        with patch("opencodon.frontends.gateway.channel_directory.DIRECTORY_PATH", cache_file), \
             patch("opencodon.frontends.gateway.channel_directory.CHANNEL_ALIASES_PATH", bad):
            result = load_directory()
            assert result["platforms"]["whatsapp"][0]["name"] == "120363"
    ...
    def test_alias_persists_through_rebuild(self, tmp_path, monkeypatch):
        ...
        cache_file = tmp_path / "channel_directory.json"
        monkeypatch.setattr("opencodon.frontends.gateway.channel_directory._build_from_sessions",
    ...
    def test_apply_aliases_handles_malformed_map(self):
        ...
        with patch("opencodon.frontends.gateway.channel_directory._load_channel_aliases",
                   return_value={
                       "whatsapp": "not-a-dict",
                       "telegram": None,
