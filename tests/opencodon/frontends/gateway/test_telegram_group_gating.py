...
from opencodon.frontends.gateway.config import Platform, PlatformConfig, load_gateway_config
from opencodon.frontends.gateway.platforms.base import MessageType
...
def _mention_entity(text, mention="@opencodon_bot"):
...
def test_observed_group_context_uses_shared_source_and_prompt_for_later_mentions():
    async def _run():
        ...
        text = "@opencodon_bot what did Alice say?"
        msg = _group_message(
            text,
            from_user_id=222,
...
def test_observed_group_context_wraps_multimodal_current_message_without_mutating_parts():
    from opencodon.frontends.gateway.run import _wrap_current_message_with_observed_context
...
def test_observed_group_context_replays_normally_without_telegram_prompt():
    from opencodon.frontends.gateway.run import _build_gateway_agent_history
...
def test_observed_group_context_preserves_slash_command_text_for_dispatch():
    from opencodon.frontends.gateway.platforms.base import MessageEvent, MessageType, Platform, SessionSource
    ...
    event = MessageEvent(
        ...
        raw_message=_group_message(
            "/new@opencodon_bot",
            entities=[_bot_command_entity("/new@opencodon_bot", "/new@opencodon_bot")],
        ),
    )

    attributed = adapter._apply_telegram_group_observe_attribution(event)

    assert attributed.text == "/new@opencodon_bot"
...
def test_group_messages_can_require_direct_trigger_via_config():
    ...
    assert adapter._should_process_message(_group_message("hi @opencodon_bot", entities=[_mention_entity("hi @opencodon_bot")])) is True
    ...
    assert adapter._should_process_message(
        _group_message(
            "/status@opencodon_bot",
            entities=[_bot_command_entity("/status@opencodon_bot", "/status@opencodon_bot")],
        ),
        is_command=True,
    ) is True
...
def test_raw_bot_mention_fallback_does_not_match_email_or_substring():
    adapter = _make_adapter(require_mention=True, bot_username="opencodon_bot")

    assert adapter._should_process_message(_group_message("email ops@opencodon_bot.example")) is False
    assert adapter._should_process_message(_group_message("prefix@opencodon_bot hi")) is False
    assert adapter._should_process_message(_group_message("hi @opencodon_bot")) is True
...
def test_guest_mode_allows_only_direct_mentions_outside_allowed_chats():
    ...
    mentioned = _group_message(
        "hi @opencodon_bot",
        chat_id=-201,
        entities=[_mention_entity("hi @opencodon_bot")],
    )
...
def test_allowed_topics_drop_other_forum_topics_before_other_gates():
    ...
    assert adapter._should_process_message(
        _group_message("hi @opencodon_bot", chat_id=-100, thread_id=11, entities=[_mention_entity("hi @opencodon_bot")])
    ) is False
...
def test_config_bridges_telegram_group_settings(monkeypatch, tmp_path):
    ...
    # from a developer's real ~/.opencodon/.env, defeating the env-over-YAML bridge
...
def test_config_bridges_telegram_user_allowlists(monkeypatch, tmp_path):
    ...
    # ~/.opencodon/.env, which would defeat the env-over-YAML bridge here.
    tg_cfg = config.platforms.get(Platform.TELEGRAM)
    assert tg_cfg is not None
    assert tg_cfg.extra.get("group_allowed_chats") == ["-100"]
...
def test_text_reply_to_photo_caches_referenced_media(monkeypatch, tmp_path):
    async def _run():
        ...
        monkeypatch.setattr(
            "opencodon.frontends.gateway.platforms.base.cache_image_from_bytes",
...
def test_unmentioned_document_observed_with_cached_path(monkeypatch, tmp_path):
    async def _run():
        ...
        monkeypatch.setattr(
            "opencodon.frontends.gateway.platforms.base.cache_document_from_bytes",
...
def test_unmentioned_large_document_observed_without_download(monkeypatch):
    async def _run():
        ...
        monkeypatch.setattr("opencodon.frontends.gateway.platforms.base.cache_document_from_bytes", cache_doc)
