...
from opencodon.frontends.gateway.channel_directory import build_channel_directory
from opencodon.frontends.gateway.platforms.base import Platform
...
def test_connected_platform_still_uses_session_discovery(tmp_path):
    ...
    with patch(
        "opencodon.frontends.gateway.channel_directory._build_from_sessions",
        return_value={"channels": []},
    ) as mock_sessions, patch("opencodon.frontends.gateway.channel_directory.DIRECTORY_PATH", cache_file):
        directory = asyncio.run(build_channel_directory({Platform.TELEGRAM: object()}))
