...
class TestUsageAccountSection:
    ...
    async def test_usage_command_includes_account_section(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "opencodon.frontends.gateway.slash_commands.fetch_account_usage",
            lambda provider, base_url=None, api_key=None: object(),
        )
        monkeypatch.setattr(
            "opencodon.frontends.gateway.slash_commands.render_account_usage_lines",
    ...
    async def test_usage_command_uses_persisted_provider_when_agent_not_running(self, monkeypatch):
        ...
        async def _fake_to_thread(fn, *args, **kwargs):
            ...
            # (markdown-only) through to_thread — record every
            # call rather than last-wins so we can pick out the account fetch.
            calls.append({"args": args, "kwargs": kwargs})
            return fn(*args, **kwargs)

        monkeypatch.setattr("opencodon.frontends.gateway.run.asyncio.to_thread", _fake_to_thread)
        monkeypatch.setattr(
            "opencodon.frontends.gateway.slash_commands.fetch_account_usage",
            lambda provider, base_url=None, api_key=None: object(),
        )
