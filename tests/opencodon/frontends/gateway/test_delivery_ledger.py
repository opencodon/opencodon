...
def _fresh_db(tmp_path, monkeypatch):
    """Isolated state.db per test (autouse OPENCODON_HOME isolation already
    redirects get_opencodon_home; make the redirect explicit and per-test)."""
...
class TestGatewayRedeliverySweep:
    ...
    async def test_disabled_gate_short_circuits(self):
        ...
        with patch.object(dl, "ledger_enabled", return_value=False), patch(
            "opencodon.frontends.gateway.delivery_ledger.ledger_enabled", return_value=False
        ):
            n = await runner._redeliver_pending_obligations()
        assert n == 0
