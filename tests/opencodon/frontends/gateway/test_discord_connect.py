...
class FakeBot:
    def __init__(self, *, intents, proxy=None, allowed_mentions=None, **_):
        ...
        self.user = SimpleNamespace(id=999, name="opencodon")
        self._events = {}
        self.tree = FakeTree()
...
async def test_connect_only_requests_members_intent_when_needed(monkeypatch, allowed_users, expected_members_intent):
    ...
    monkeypatch.setattr("opencodon.frontends.gateway.status.acquire_scoped_lock", lambda scope, identity, metadata=None: (True, None))
    monkeypatch.setattr("opencodon.frontends.gateway.status.release_scoped_lock", lambda scope, identity: None)
...
async def test_connect_releases_token_lock_on_timeout(monkeypatch):
    ...
    monkeypatch.setattr("opencodon.frontends.gateway.status.acquire_scoped_lock", lambda scope, identity, metadata=None: (True, None))
    released = []
    monkeypatch.setattr("opencodon.frontends.gateway.status.release_scoped_lock", lambda scope, identity: released.append((scope, identity)))
...
async def test_safe_sync_slash_commands_only_mutates_diffs():
    ...
    desired_same = {
        "name": "status",
        "description": "Show opencodon session status",
        "type": 1,
        "options": [],
        "nsfw": False,
        "dm_permission": True,
...
async def test_post_connect_initialization_skips_same_fingerprint_after_success(tmp_path, monkeypatch):
    ...
    class _DesiredCommand:
        def to_dict(self, tree):
            return {
                "name": "status",
                "description": "Show opencodon status",
                "type": 1,
                "options": [],
            }
...
async def test_post_connect_initialization_reraises_non_rate_limit_exceptions(tmp_path, monkeypatch):
    ...
    class _DesiredCommand:
        def to_dict(self, tree):
            return {"name": "status", "description": "Show opencodon status", "type": 1, "options": []}
