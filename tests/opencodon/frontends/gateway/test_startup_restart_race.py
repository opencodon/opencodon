...
from opencodon.frontends.gateway.platforms.base import BasePlatformAdapter, SendResult
...
async def test_start_gateway_does_not_start_cron_after_aborted_startup(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("opencodon.frontends.gateway.status.acquire_gateway_runtime_lock", lambda: True)
    monkeypatch.setattr("opencodon.frontends.gateway.status.write_pid_file", lambda: None)
    monkeypatch.setattr("opencodon.frontends.gateway.status.remove_pid_file", lambda: None)
    monkeypatch.setattr("opencodon.frontends.gateway.status.release_gateway_runtime_lock", lambda: None)
    ...
    monkeypatch.setattr("opencodon.frontends.gateway.run.GatewayRunner", AbortedStartupRunner)
    monkeypatch.setattr("opencodon.frontends.gateway.run._start_cron_ticker", fail_if_cron_starts)
