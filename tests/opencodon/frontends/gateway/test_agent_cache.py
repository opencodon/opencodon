...
class TestAgentConfigSignature:
    ...
    def test_same_config_same_signature(self):
        ...
        sig1 = GatewayRunner._agent_config_signature("claude-sonnet-4", runtime, ["opencodon-telegram"], "")
        sig2 = GatewayRunner._agent_config_signature("claude-sonnet-4", runtime, ["opencodon-telegram"], "")
        assert sig1 == sig2
    ...
    def test_model_change_different_signature(self):
        ...
        runtime = {"api_key": "sk-test12345678", "base_url": "https://openrouter.ai/api/v1",
                    "provider": "openrouter"}
        sig1 = GatewayRunner._agent_config_signature("claude-sonnet-4", runtime, ["opencodon-telegram"], "")
        sig2 = GatewayRunner._agent_config_signature("claude-opus-4.6", runtime, ["opencodon-telegram"], "")
        assert sig1 != sig2
    ...
    def test_same_token_prefix_different_full_token_changes_signature(self):
        ...
        sig1 = GatewayRunner._agent_config_signature("gpt-5.3-codex", rt1, ["opencodon-telegram"], "")
        sig2 = GatewayRunner._agent_config_signature("gpt-5.3-codex", rt2, ["opencodon-telegram"], "")
        assert sig1 != sig2
    ...
    def test_provider_change_different_signature(self):
        ...
        sig1 = GatewayRunner._agent_config_signature("claude-sonnet-4", rt1, ["opencodon-telegram"], "")
        sig2 = GatewayRunner._agent_config_signature("claude-sonnet-4", rt2, ["opencodon-telegram"], "")
        assert sig1 != sig2
    ...
    def test_toolset_change_different_signature(self):
        ...
        sig1 = GatewayRunner._agent_config_signature("claude-sonnet-4", runtime, ["opencodon-telegram"], "")
        sig2 = GatewayRunner._agent_config_signature("claude-sonnet-4", runtime, ["opencodon-discord"], "")
        assert sig1 != sig2
...
class TestExtractCacheBustingConfig:
    ...
    def test_memory_provider_change_busts_signature(self):
        ...
        sig_a = GatewayRunner._extract_cache_busting_config({"memory": {"provider": "alpha"}})
        sig_b = GatewayRunner._extract_cache_busting_config({"memory": {"provider": "beta"}})

        assert sig_a["memory.provider"] == "alpha"
        assert sig_b["memory.provider"] == "beta"
        assert sig_a != sig_b
...
class TestAgentCacheLifecycle:
    ...
    def test_cache_hit_returns_same_agent(self):
        ...
        runtime = {"api_key": "test", "base_url": "https://openrouter.ai/api/v1",
                    "provider": "openrouter", "api_mode": "chat_completions"}
        sig = runner._agent_config_signature("anthropic/claude-sonnet-4", runtime, ["opencodon-telegram"], "")
    ...
    def test_cache_miss_on_model_change(self):
        ...
        old_sig = runner._agent_config_signature("anthropic/claude-sonnet-4", runtime, ["opencodon-telegram"], "")
        agent1 = AIAgent(
            model="anthropic/claude-sonnet-4", api_key="test",
            base_url="https://openrouter.ai/api/v1", provider="openrouter",
            max_iterations=5, quiet_mode=True, skip_context_files=True,
        ...
        new_sig = runner._agent_config_signature("anthropic/claude-opus-4.6", runtime, ["opencodon-telegram"], "")
...
class TestAgentCacheBoundedGrowth:
    ...
    def test_cap_evicts_lru_when_exceeded(self, monkeypatch):
        ...
        from opencodon.frontends.gateway import run as gw_run
    ...
    def test_is_session_finalizable_real_predicate(self, tmp_path):
        ...
        from opencodon.frontends.gateway.config import GatewayConfig, Platform, SessionResetPolicy
        ...
        with _patch("opencodon.frontends.gateway.session.SessionStore._ensure_loaded"):
...
class TestAgentCacheActiveSafety:
    ...
    def test_cap_leaves_cache_over_limit_if_all_active(self, monkeypatch, caplog):
        ...
        with caplog.at_level(_logging.WARNING, logger="opencodon.frontends.gateway.run"):
            with runner._agent_cache_lock:
                runner._enforce_agent_cache_cap()
...
class TestAgentCacheIdleResume:
    ...
    def test_release_clients_does_not_touch_process_registry(self, monkeypatch):
        ...
        from opencodon.tools import process_registry as _pr
        kill_all_calls: list = []
    ...
    def test_release_clients_does_not_touch_terminal_or_browser(self, monkeypatch):
        ...
        from opencodon.core.run_agent import AIAgent
        from opencodon.tools import terminal_tool as _tt
        from opencodon.tools import browser_tool as _bt
    ...
    def test_close_vs_release_full_teardown_difference(self, monkeypatch):
        ...
        """
        from opencodon.core.run_agent import AIAgent
        from opencodon.core import run_agent as _ra
...
class TestCachedAgentInactivityReset:
    ...
    def test_fresh_turn_resets_idle_clock(self):
        ...
        with patch("opencodon.frontends.gateway.run.time") as mock_time:
...
class TestAgentConfigSignatureUserId:
    ...
    A memory provider freezes the resolved runtime user identity at
    ...
    def test_signature_changes_with_user_id(self):
        ...
        sig_a = GatewayRunner._agent_config_signature(
            "claude-sonnet-4", runtime, ["opencodon-telegram"], "", user_id="7654321"
        )
        sig_b = GatewayRunner._agent_config_signature(
            "claude-sonnet-4", runtime, ["opencodon-telegram"], "", user_id="491827364"
        )
        assert sig_a != sig_b
    ...
    def test_signature_changes_with_user_id_alt(self):
        ...
        runtime = {"provider": "anthropic", "api_key": "k", "base_url": "", "api_mode": "chat_completions"}
        sig_a = GatewayRunner._agent_config_signature(
            "claude-sonnet-4", runtime, ["opencodon-telegram"], "",
