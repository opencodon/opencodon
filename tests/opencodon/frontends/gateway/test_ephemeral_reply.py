...
async def test_schedule_ephemeral_delete_calls_delete_after_ttl():
    ...
    # scheduler pumping.
    import opencodon.frontends.gateway.platforms.base as base_module
...
async def test_schedule_ephemeral_delete_swallows_errors():
    ...
    with patch("opencodon.frontends.gateway.platforms.base.asyncio.sleep", AsyncMock()):
        adapter._schedule_ephemeral_delete(
...
async def test_process_message_unwraps_ephemeral_before_send():
    ...
    with patch("opencodon.frontends.gateway.platforms.base.asyncio.sleep", _fake_sleep), patch.object(
...
async def test_process_message_ephemeral_reply_does_not_auto_upload_bare_paths(tmp_path):
    ...
    reply_text = f"Tip: opencodon chat --ignore-user-config skips {config_path}"
    ...
    session_key = "agent:main:telegram:private:42"
    with patch("opencodon.frontends.gateway.platforms.base.asyncio.sleep", AsyncMock()), patch.object(
        adapter, "_keep_typing", new=AsyncMock()
    ):
        await adapter._process_message_background(event, session_key)
