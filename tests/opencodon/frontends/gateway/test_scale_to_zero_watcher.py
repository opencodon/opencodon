...
def _arm_runner(monkeypatch, platform_states, *, enabled=True, wake_url="https://wake.example"):
    ...
    monkeypatch.setattr("opencodon.frontends.gateway.scale_to_zero.scale_to_zero_enabled", lambda *a, **k: enabled)
    monkeypatch.setattr("opencodon.frontends.gateway.relay.relay_wake_url", lambda: wake_url)
    return r
