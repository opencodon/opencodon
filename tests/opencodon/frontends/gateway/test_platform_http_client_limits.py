...
def _clear_env(monkeypatch):
    monkeypatch.delenv("OPENCODON_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", raising=False)
    monkeypatch.delenv("OPENCODON_GATEWAY_HTTPX_MAX_KEEPALIVE", raising=False)
...
def test_returns_none_when_httpx_unavailable(monkeypatch):
    ...
    import opencodon.frontends.gateway.platforms._http_client_limits as mod
...
def test_env_override_keepalive_expiry(monkeypatch):
    monkeypatch.setenv("OPENCODON_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", "7.5")
    from opencodon.frontends.gateway.platforms._http_client_limits import platform_httpx_limits
    limits = platform_httpx_limits()
...
def test_env_override_max_keepalive(monkeypatch):
    monkeypatch.setenv("OPENCODON_GATEWAY_HTTPX_MAX_KEEPALIVE", "25")
    from opencodon.frontends.gateway.platforms._http_client_limits import platform_httpx_limits
    limits = platform_httpx_limits()
...
def test_env_override_rejects_garbage(monkeypatch):
    ...
    monkeypatch.setenv("OPENCODON_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", "not-a-number")
    monkeypatch.setenv("OPENCODON_GATEWAY_HTTPX_MAX_KEEPALIVE", "-3")
    from opencodon.frontends.gateway.platforms._http_client_limits import platform_httpx_limits
    limits = platform_httpx_limits()
...
def test_helper_is_importable_from_every_platform_that_uses_it():
    ...
    import opencodon.frontends.gateway.platforms.whatsapp_cloud  # noqa: F401
