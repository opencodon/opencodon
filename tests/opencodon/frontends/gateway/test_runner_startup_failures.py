...
async def test_start_gateway_verbosity_imports_redacting_formatter(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("opencodon.frontends.gateway.run.GatewayRunner", _CleanExitRunner)
...
async def test_start_gateway_replace_force_uses_terminate_pid(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("opencodon.frontends.gateway.status.get_running_pid", _mock_get_running_pid)
    monkeypatch.setattr("opencodon.frontends.gateway.status.remove_pid_file", _mock_remove_pid_file)
    monkeypatch.setattr(
        "opencodon.frontends.gateway.status.release_all_scoped_locks",
        lambda **kwargs: 0,
    )
    ...
    def _mock_terminate_pid(pid, force=False):
        ...
        if force:
            _pid_state["alive"] = False
    monkeypatch.setattr("opencodon.frontends.gateway.status.terminate_pid", _mock_terminate_pid)
    monkeypatch.setattr(
        "opencodon.frontends.gateway.status._pid_exists", lambda pid: _pid_state["alive"]
    )
    monkeypatch.setattr("opencodon.frontends.gateway.run.os.getpid", lambda: 100)
    monkeypatch.setattr("opencodon.frontends.gateway.run.os.kill", lambda pid, sig: None)
...
async def test_start_gateway_replace_aborts_when_force_killed_pid_still_alive(
    ...
    monkeypatch.setattr("opencodon.frontends.gateway.status.get_running_pid", lambda: 42)
    monkeypatch.setattr("opencodon.frontends.gateway.status.remove_pid_file", _mock_remove_pid_file)
    monkeypatch.setattr(
        "opencodon.frontends.gateway.status.release_all_scoped_locks",
    ...
    monkeypatch.setattr(
        "opencodon.frontends.gateway.status.terminate_pid",
    ...
    monkeypatch.setattr("opencodon_logging.setup_logging", lambda opencodon_home, mode: tmp_path)
    monkeypatch.setattr("opencodon_logging._add_rotating_handler", lambda *args, **kwargs: None)
    monkeypatch.setattr("opencodon.frontends.gateway.run.GatewayRunner", _RunnerShouldNotStart)
...
async def test_start_gateway_replace_writes_takeover_marker_before_sigterm(
    ...
    def record_write_marker(target_pid: int) -> bool:
        ...
        from opencodon.frontends.gateway.status import _get_takeover_marker_path, _write_json_file
    ...
    monkeypatch.setattr("opencodon.frontends.gateway.status.write_takeover_marker", record_write_marker)
    monkeypatch.setattr("opencodon.frontends.gateway.status.terminate_pid", record_terminate)
    monkeypatch.setattr("opencodon.frontends.gateway.run.os.getpid", lambda: 100)
    # Simulate old process exiting on first check so we don't loop into force-kill
    monkeypatch.setattr(
        "opencodon.frontends.gateway.run.os.kill",
...
async def test_start_gateway_replace_clears_marker_on_permission_denied(
    ...
    monkeypatch.setattr("opencodon.frontends.gateway.status.get_running_pid", lambda: 42)
    monkeypatch.setattr("opencodon.frontends.gateway.status.write_takeover_marker", write_marker)
    monkeypatch.setattr("opencodon.frontends.gateway.status.terminate_pid", raise_permission)
    monkeypatch.setattr("opencodon.frontends.gateway.run.os.getpid", lambda: 100)
    monkeypatch.setattr("opencodon.tools.skills_sync.sync_skills", lambda quiet=True: None)
    monkeypatch.setattr("opencodon_logging.setup_logging", lambda opencodon_home, mode: tmp_path)
    monkeypatch.setattr("opencodon_logging._add_rotating_handler", lambda *args, **kwargs: None)
...
async def test_start_gateway_propagates_fatal_config_exit_code(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("opencodon_logging.setup_logging", lambda opencodon_home, mode: tmp_path)
    monkeypatch.setattr("opencodon_logging._add_rotating_handler", lambda *args, **kwargs: None)
    monkeypatch.setattr("opencodon.frontends.gateway.run.GatewayRunner", _FatalConfigRunner)
