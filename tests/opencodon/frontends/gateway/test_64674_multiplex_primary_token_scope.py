...
class TestLoadGatewayConfigForRunner:
    def test_unscoped_when_multiplex_off(self, tmp_path, monkeypatch):
        from opencodon.frontends.gateway import run as run_mod
    ...
    def test_scoped_reload_picks_up_default_profile_token(self, tmp_path, monkeypatch):
        ...
        from opencodon.frontends.gateway import run as run_mod
        import opencodon_constants as hc
        ...
        monkeypatch.delenv("TELEGRAM_BOT_TOKEN", raising=False)
        # Point both opencodon_constants and gateway.run at our temp home.
        monkeypatch.setattr(hc, "get_opencodon_home", lambda: home)
        monkeypatch.setattr(run_mod, "get_opencodon_home", lambda: home)
        monkeypatch.setattr(run_mod, "_opencodon_home", home)
...
class TestPlatformHasBotCredential:
    def test_telegram_empty_token_false(self):
        from opencodon.frontends.gateway.run import _platform_has_bot_credential
...
class TestReconnectDropsEmptyToken:
    ...
    async def test_empty_token_removed_from_queue(self):
        from opencodon.frontends.gateway.run import GatewayRunner, _platform_has_bot_credential
