...
async def test_safe_disconnect_times_out_and_continues(bare_runner, monkeypatch, caplog):
    ...
    monkeypatch.setenv("OPENCODON_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "0.001")
    adapter = MagicMock()
    ...
    with caplog.at_level(logging.WARNING, logger="opencodon.frontends.gateway.run"):
        await bare_runner._safe_adapter_disconnect(adapter, Platform.DISCORD)

    adapter.disconnect.assert_awaited_once()
    assert "Timed out after 0.0s while disconnecting discord adapter" in caplog.text
...
async def test_safe_disconnect_detaches_cancellation_swallowing_disconnect(
    ...
    operation = asyncio.create_task(
        bare_runner._safe_adapter_disconnect(adapter, Platform.DISCORD)
    )
    await started.wait()
