...
def _make_runner():
    ...
    from opencodon.frontends.gateway.hooks import HookRegistry
...
class TestHandleBackgroundCommand:
    ...
    async def test_valid_prompt_starts_task(self):
        ...
        with patch("opencodon.frontends.gateway.run.asyncio.create_task", side_effect=capture_task):
    ...
    async def test_prompt_truncated_in_preview(self):
        ...
        with patch("opencodon.frontends.gateway.run.asyncio.create_task", side_effect=lambda c, **kw: (c.close(), MagicMock())[1]):
...
class TestRunBackgroundTask:
    ...
    async def test_no_credentials_sends_error(self):
        ...
        with patch("opencodon.frontends.gateway.run._resolve_runtime_agent_kwargs", return_value={"api_key": None}):
            await runner._run_background_task("test prompt", source, "bg_test")
    ...
    async def test_successful_task_sends_result(self):
        ...
        with patch("opencodon.frontends.gateway.run._resolve_runtime_agent_kwargs", return_value={"api_key": "test-key"}), \
             patch("opencodon.frontends.gateway.run._load_gateway_config", return_value=checkpoint_config), \
    ...
    async def test_media_files_routed_by_type(self, monkeypatch):
        ...
        """
        from opencodon.frontends.gateway import run as gateway_run
    ...
    async def test_exception_sends_error_message(self):
        ...
        with patch("opencodon.frontends.gateway.run._resolve_runtime_agent_kwargs", side_effect=RuntimeError("boom")):
            await runner._run_background_task("test prompt", source, "bg_test")
