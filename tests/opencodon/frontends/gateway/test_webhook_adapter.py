...
from opencodon.frontends.gateway.platforms.webhook import (
    WebhookAdapter,
    _INSECURE_NO_AUTH,
...
class TestPayloadFilters:
    ...
    async def test_filter_accepts_nested_any_and_in_file(self, tmp_path, monkeypatch):
        """Nested any groups can match dynamic watchlists under OPENCODON_HOME."""
                        ...
                        "any": [
                            {
                                ...
                                "in_file": "~/.opencodon/data/watchlist.json",
                            },
                            {
    ...
    async def test_script_tilde_opencodon_path_resolves_to_active_profile_home(self, tmp_path, monkeypatch):
        ...
        routes = {
            "todoist": {
                "secret": _INSECURE_NO_AUTH,
                "script": "~/.opencodon/scripts/todoist_filter.py",
                "prompt": "Task: {body}",
            }
        }
        adapter = _make_adapter(routes=routes)
...
class TestHTTPHandling:
    ...
    async def test_connect_starts_server(self):
        ...
        with patch("opencodon.frontends.gateway.platforms.webhook.web.AppRunner") as MockRunner, \
             patch("opencodon.frontends.gateway.platforms.webhook.web.TCPSite") as MockSite:
...
class TestCheckRequirements:
    ...
    @patch("opencodon.frontends.gateway.platforms.webhook.AIOHTTP_AVAILABLE", False)
...
class TestInsecureNoAuthSafetyRail:
    ...
    def test_is_loopback_host_accepts(self, host):
        ...
        from opencodon.frontends.gateway.platforms.webhook import _is_loopback_host
...
class TestDualStackBind:
    ...
    def test_default_host_is_none_for_dual_stack(self):
        ...
        from opencodon.frontends.gateway.platforms.webhook import DEFAULT_HOST
