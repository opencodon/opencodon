...
def _patch_info(tmp_path, config_yaml, model, runtime):
    ...
    return (
        patch("opencodon.frontends.gateway.run._opencodon_home", tmp_path),
        patch("opencodon.frontends.gateway.run._resolve_gateway_model", return_value=model),
...
"""
    ...
    def test_runtime_resolution_failure_doesnt_crash(self, runner, tmp_path):
        ...
        with patch("opencodon.frontends.gateway.run._opencodon_home", tmp_path), \
             ...
             patch("opencodon.frontends.gateway.run._resolve_runtime_agent_kwargs", side_effect=RuntimeError("no creds")):
            info = runner._format_session_info()
        assert "4K" in info
        assert "config" in info
...
class TestResetNoticeSessionInfo:
    ...
    def test_multiplex_uses_profile_config(self, runner, tmp_path):
        ...
        with patch("opencodon.frontends.gateway.run._opencodon_home", base), \
             patch.object(GatewayRunner, "_resolve_profile_home_for_source", return_value=profile), \
             patch("opencodon.frontends.gateway.run._resolve_runtime_agent_kwargs", return_value=self._RUNTIME):
