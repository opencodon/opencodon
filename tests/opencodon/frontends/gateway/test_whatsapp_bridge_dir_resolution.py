...
def test_writable_install_returns_install_dir(tmp_path, monkeypatch):
    ...
    # Nothing mirrored into OPENCODON_HOME.
    assert not (opencodon_home / "scripts" / "whatsapp-bridge").exists()


def test_readonly_install_mirrors_to_opencodon_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable OPENCODON_HOME."""
    install_root = tmp_path / "install"
    install_bridge = install_root / "scripts" / "whatsapp-bridge"
    _seed_install_tree(install_bridge)
    ...
    expected = opencodon_home / "scripts" / "whatsapp-bridge"
...
def test_readonly_install_reuses_existing_mirror(tmp_path, monkeypatch):
    """If the OPENCODON_HOME mirror already exists, return it without re-copying."""
    install_root = tmp_path / "install"
    install_bridge = install_root / "scripts" / "whatsapp-bridge"
    _seed_install_tree(install_bridge)

    opencodon_home = tmp_path / "opencodon_home"
    mirror = opencodon_home / "scripts" / "whatsapp-bridge"
