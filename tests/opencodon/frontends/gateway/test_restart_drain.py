...
from opencodon.frontends.gateway.platforms.base import MessageEvent, MessageType
from opencodon.frontends.gateway.restart import DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
from opencodon.frontends.gateway.session import SessionEntry, build_session_key
from tests.opencodon.frontends.gateway.restart_test_helpers import make_restart_runner, make_restart_source
...
def test_load_busy_input_mode_prefers_env_then_config_then_default(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("OPENCODON_GATEWAY_BUSY_INPUT_MODE", raising=False)
    ...
    monkeypatch.setenv("OPENCODON_GATEWAY_BUSY_INPUT_MODE", "interrupt")
    assert gateway_run.GatewayRunner._load_busy_input_mode() == "interrupt"

    monkeypatch.setenv("OPENCODON_GATEWAY_BUSY_INPUT_MODE", "steer")
    assert gateway_run.GatewayRunner._load_busy_input_mode() == "steer"

    # Unknown values fall through to the safe default
    monkeypatch.setenv("OPENCODON_GATEWAY_BUSY_INPUT_MODE", "bogus")
    assert gateway_run.GatewayRunner._load_busy_input_mode() == "interrupt"


def test_load_busy_text_mode_follows_input_mode_and_honors_legacy(tmp_path, monkeypatch):
    monkeypatch.setattr(gateway_run, "_opencodon_home", tmp_path)
    monkeypatch.delenv("OPENCODON_GATEWAY_BUSY_TEXT_MODE", raising=False)
    monkeypatch.delenv("OPENCODON_GATEWAY_BUSY_INPUT_MODE", raising=False)
    ...
    monkeypatch.setenv("OPENCODON_GATEWAY_BUSY_TEXT_MODE", "queue")
    assert gateway_run.GatewayRunner._load_busy_text_mode() == "queue"

    # Bogus legacy value is ignored → falls through to busy_input_mode (interrupt).
    monkeypatch.setenv("OPENCODON_GATEWAY_BUSY_TEXT_MODE", "bogus")
    assert gateway_run.GatewayRunner._load_busy_text_mode() == "interrupt"
...
def test_load_restart_drain_timeout_prefers_env_then_config_then_default(
    ...
    monkeypatch.setenv("OPENCODON_RESTART_DRAIN_TIMEOUT", "7")
...
async def test_launch_detached_restart_command_uses_setsid(monkeypatch):
    ...
    monkeypatch.setattr(gateway_run, "_resolve_opencodon_bin", lambda: ["/usr/bin/opencodon"])
    ...
    # self-restart loop guard refuses to run `opencodon gateway restart`.
    assert kwargs["env"].get("_OPENCODON_GATEWAY") is None
...
async def test_windows_detached_restart_scrubs_gateway_marker(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway_run.sys, "platform", "win32")
    monkeypatch.setattr(gateway_run, "_resolve_opencodon_bin", lambda: ["opencodon"])
    monkeypatch.setattr(gateway_run.os, "getpid", lambda: 321)
    monkeypatch.setenv("_OPENCODON_GATEWAY", "1")
    monkeypatch.setenv("VIRTUAL_ENV", str(venv_dir))
    ...
    assert len(popen_calls) == 1
    cmd, kwargs = popen_calls[0]
    assert cmd[-3:] == ["opencodon", "gateway", "restart"]
    assert kwargs["env"].get("_OPENCODON_GATEWAY") is None
...
async def test_shutdown_notification_home_channel_suppressed_when_flag_disabled():
    ...
    from opencodon.frontends.gateway.config import HomeChannel, Platform
...
async def test_shutdown_notification_uses_persisted_origin_for_colon_ids():
    ...
    source.platform = gateway_run.Platform.DISCORD
    ...
    runner.adapters = {gateway_run.Platform.DISCORD: adapter}
...
async def test_drain_suppress_skips_home_channel_keeps_session_ping(tmp_path, monkeypatch):
    ...
    """
    from opencodon.frontends.gateway.config import HomeChannel, Platform
    import opencodon.frontends.gateway.drain_control as dc
