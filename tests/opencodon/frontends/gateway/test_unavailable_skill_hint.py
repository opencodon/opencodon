...
* ``disabled`` membership is checked by the declared name, because that
  is what :func:`opencodon_cli.skills_config.save_disabled_skills` stores.
"""
...
def tmp_skills(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Isolated skills dir + OPENCODON_HOME so the real user config is untouched."""
...
def test_frontmatter_slug_matched_even_when_dir_name_differs(
    ...
    # Config disables by declared name (matches what `opencodon skills config` writes).
    monkeypatch.setattr(
        "opencodon.frontends.gateway.run._get_disabled_skill_names",
    ...
    with patch(
        ...
        "opencodon.core.skills.skill_utils.get_all_skills_dirs",
    ...
    assert "opencodon skills config" in msg
...
def test_unknown_command_still_returns_none(
    ...
    with patch(
        ...
        "opencodon.core.skills.skill_utils.get_all_skills_dirs", return_value=[tmp_skills]
    ):
...
def test_optional_skill_uses_frontmatter_slug(
    ...
    monkeypatch.setattr(
        "opencodon_constants.get_optional_skills_dir",
    ...
    with patch(
        ...
        "opencodon.core.skills.skill_utils.get_all_skills_dirs", return_value=[empty_skills]
    ):
        msg = gateway_run._check_unavailable_skill("stable-diffusion-image-generation")
