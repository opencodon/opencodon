...
from opencodon.frontends.gateway import memory_monitor as mm
...
def test_log_memory_usage_emits_memory_line(caplog):
    caplog.set_level(logging.INFO, logger="opencodon.frontends.gateway.memory_monitor")
    mm.log_memory_usage()
...
def test_unavailable_rss_warns_and_does_not_start(caplog, monkeypatch):
    ...
    caplog.set_level(logging.WARNING, logger="opencodon.frontends.gateway.memory_monitor")
    started = mm.start_memory_monitoring(interval_seconds=3600.0)
