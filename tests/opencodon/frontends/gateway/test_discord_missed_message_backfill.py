...
from opencodon.frontends.gateway.platforms.base import MessageEvent, MessageType, ProcessingOutcome
...
def test_recovery_store_pins_profile_home_at_adapter_construction(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("OPENCODON_HOME", str(first_home))
    adapter = DiscordAdapter(PlatformConfig(enabled=True, token="one"))
    monkeypatch.setenv("OPENCODON_HOME", str(second_home))
