...
Task 2.2/2.3. Two layers:
  * drain_control.py — the presence-based marker contract (write/clear/read,
    OPENCODON_HOME-scoped, never-raises).
...
Q-B, exercises a real `opencodon gateway run`); these lock the unit contract.
"""
...
class TestMarkerContract:
    ...
    def test_path_respects_opencodon_home(self, home):
...
class TestInstantiationEpoch:
    ...
    def test_marker_from_prior_instantiation_reads_as_absent(self, home, monkeypatch):
        ...
        # container/VM instantiation survives on the durable OPENCODON_HOME volume
