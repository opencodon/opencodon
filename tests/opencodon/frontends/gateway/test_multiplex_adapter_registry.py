...
class TestCredentialFingerprint:
    ...
    def test_direct_token_takes_precedence_over_config(self):
        ...
        import hashlib
        expected = hashlib.sha256(b"opencodon-mux:from-direct").hexdigest()[:16]
        assert fp == expected
...
def _install_secondary_reconnect_context(monkeypatch, runner, adapter, scoped_homes=None):
    ...
    monkeypatch.setattr(
        "opencodon.frontends.cli.profiles.get_profile_dir", lambda name: Path("/profiles") / name
    )
    monkeypatch.setattr(
        "opencodon.frontends.gateway.config.load_gateway_config",
...
class TestSecondaryProfileConfigHandling:
    ...
    async def test_secondary_webhook_uses_degradable_error(self, monkeypatch):
        from opencodon.frontends.gateway.run import SecondaryPortBindingConfigError
        from opencodon.frontends.gateway.config import GatewayConfig, Platform, PlatformConfig
        ...
        monkeypatch.setattr(
            "opencodon.frontends.gateway.config.load_gateway_config", lambda: reviewer_cfg
        )
    ...
    async def test_secondary_reports_all_port_binding_platforms(self, monkeypatch):
        ...
        reviewer_cfg.platforms = {
            Platform.WHATSAPP_CLOUD: PlatformConfig(
                enabled=True,
                extra={"phone_number_id": "1", "access_token": "t"},
            ),
            Platform.WEBHOOK: PlatformConfig(enabled=True, extra={"port": 8644}),
        ...
        assert "whatsapp_cloud" in message
    ...
    async def test_multiplexer_skips_bad_profile_and_continues(self, monkeypatch, caplog):
        ...
        monkeypatch.setattr(
            "opencodon.frontends.cli.profiles.profiles_to_serve",
            lambda multiplex: [
        ...
        monkeypatch.setattr(
            "opencodon.frontends.cli.profiles.get_active_profile_name",
            lambda: "default",
        )
        monkeypatch.setattr(runner, "_start_one_profile_adapters", fake_start_one)
        monkeypatch.setattr(
            "opencodon.frontends.gateway.status.write_runtime_status",
            lambda **kwargs: None,
        )

        caplog.set_level(logging.WARNING, logger="opencodon.frontends.gateway.run")
    ...
    async def test_multiplexer_propagates_security_config_error(self, monkeypatch):
        from pathlib import Path
        from opencodon.frontends.gateway.config import GatewayConfig
        from opencodon.frontends.gateway.run import MultiplexConfigError
    ...
    async def test_open_policy_uses_fatal_config_error(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.frontends.gateway.config.load_gateway_config", lambda: unsafe_cfg)
    ...
    async def test_non_multiplex_profile_adapter_start_keeps_relay(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.frontends.gateway.config.load_gateway_config", lambda: profile_cfg)
    ...
    def test_port_binding_set_covers_known_listeners(self):
        from opencodon.frontends.gateway.run import _PORT_BINDING_PLATFORM_VALUES
