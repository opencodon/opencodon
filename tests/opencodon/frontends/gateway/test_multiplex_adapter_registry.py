...
def _install_secondary_reconnect_context(monkeypatch, runner, adapter, scoped_homes=None):
    ...
    monkeypatch.setattr(
        "opencodon.core.profiles.get_profile_dir", lambda name: Path("/profiles") / name
    )
    monkeypatch.setattr(
...
class TestSecondaryProfileConfigHandling:
    ...
    async def test_multiplexer_skips_bad_profile_and_continues(self, monkeypatch, caplog):
        ...
        monkeypatch.setattr(
            "opencodon.core.profiles.profiles_to_serve",
            lambda multiplex: [
        ...
        monkeypatch.setattr(
            "opencodon.core.profiles.get_active_profile_name",
            lambda: "default",
        )
