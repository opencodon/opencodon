...
(OPENCODON_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT) and always returns.
"""
...
async def test_teardown_bounds_hanging_disconnect(bare_runner, monkeypatch, caplog):
    ...
    monkeypatch.setenv("OPENCODON_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "0.01")
    adapter = MagicMock()
    ...
    with caplog.at_level(logging.WARNING, logger="opencodon.frontends.gateway.run"):
        await asyncio.wait_for(
            bare_runner._bounded_adapter_teardown(adapter, Platform.DISCORD),
            timeout=5.0,  # the helper itself must return well under this
        )

    adapter.disconnect.assert_awaited_once()
    assert "discord disconnect timed out" in caplog.text
...
async def test_teardown_bounds_hanging_cancel(bare_runner, monkeypatch, caplog):
    ...
    assert "discord background-task cancel timed out" in caplog.text
...
async def test_teardown_profile_suffix_in_logs(bare_runner, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="opencodon.frontends.gateway.run"):
...
async def test_teardown_timeout_zero_disables_bound(bare_runner, monkeypatch):
    ...
    monkeypatch.setenv("OPENCODON_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "0")
    adapter = MagicMock()
    adapter.cancel_background_tasks = AsyncMock(return_value=None)
    adapter.disconnect = AsyncMock(return_value=None)
