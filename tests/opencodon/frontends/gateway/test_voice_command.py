...
class TestSendVoiceReply:
    ...
    async def test_calls_tts_and_send_voice(self, runner):
        ...
        with patch("opencodon.tools.tts_tool.text_to_speech_tool", return_value=tts_result) as mock_tts, \
             patch("opencodon.tools.tts_tool._strip_markdown_for_tts", side_effect=lambda t: t), \
    ...
    async def test_auto_voice_reply_uses_thread_metadata_helper(self, runner):
        ...
        with patch("opencodon.tools.tts_tool.text_to_speech_tool", return_value=tts_result), \
             patch("opencodon.tools.tts_tool._strip_markdown_for_tts", side_effect=lambda t: t), \
             patch("os.path.isfile", return_value=True), \
             patch("os.unlink"), \
             patch("os.makedirs"):
    ...
    async def test_empty_text_after_strip_skips(self, runner):
        ...
        with patch("opencodon.tools.tts_tool.text_to_speech_tool") as mock_tts, \
             patch("opencodon.tools.tts_tool._strip_markdown_for_tts", return_value=""):
    ...
    async def test_exception_caught(self, runner):
        event = _make_event()
        with patch("opencodon.tools.tts_tool.text_to_speech_tool", side_effect=RuntimeError("boom")), \
             patch("opencodon.tools.tts_tool._strip_markdown_for_tts", side_effect=lambda t: t), \
             patch("os.makedirs"):
            # Should not raise
            await runner._send_voice_reply(event, "Hello")
...
class TestVoiceInHelp:
    ...
    def test_voice_is_known_command(self):
        ...
        from opencodon.frontends.cli.commands import GATEWAY_KNOWN_COMMANDS
...
class TestVoiceChannelCommands:
    ...
    async def test_join_success(self, runner):
        ...
        event.source.chat_name = "opencodon Server / #general"
        runner.adapters[event.source.platform] = mock_adapter
        result = await runner._handle_voice_channel_join(event)
    ...
    async def test_input_reuses_bound_source_metadata(self, runner):
        ...
        bound_source = SessionSource(
            chat_id="123",
            chat_name="opencodon Server / #general",
            chat_type="group",
            user_id="user1",
            user_name="user1",
        ...
        assert event.source.chat_name == "opencodon Server / #general"
...
class TestDiscordVoiceChannelMethods:
    ...
    async def test_process_voice_input_success(self):
        ...
        with patch("plugins.platforms.discord.adapter.VoiceReceiver.pcm_to_wav"), \
             patch("opencodon.tools.transcription_tools.transcribe_audio",
                   return_value={"success": True, "transcript": "Hello"}), \
             patch("opencodon.tools.voice_mode.is_whisper_hallucination", return_value=False):
    ...
    async def test_process_voice_input_hallucination_filtered(self):
        ...
        with patch("plugins.platforms.discord.adapter.VoiceReceiver.pcm_to_wav"), \
             ...
             patch("opencodon.tools.voice_mode.is_whisper_hallucination", return_value=True):
...
class TestStreamTtsToSpeaker:
    ...
    def test_none_sentinel_flushes_buffer(self):
        ...
        from opencodon.tools.tts_tool import stream_tts_to_speaker
        text_q = queue.Queue()
...
class TestVoiceTimeoutCleansRunnerState:
    ...
    def _make_discord_adapter():
        from plugins.platforms.discord.adapter import DiscordAdapter
        from opencodon.frontends.gateway.config import PlatformConfig, Platform
        config = PlatformConfig(enabled=True, extra={})
        config.token = "fake-token"
        adapter = object.__new__(DiscordAdapter)
        adapter.platform = Platform.DISCORD
...
class TestSendVoiceReplyCleanup:
    ...
    async def test_files_cleaned_on_send_exception(self, tmp_path):
        ...
        fake_audio = tmp_path / "opencodon_voice"
        fake_audio.mkdir()
        ...
        with patch("opencodon.frontends.gateway.run.asyncio.to_thread", new_callable=AsyncMock, return_value=tts_result), \
             patch("opencodon.tools.tts_tool._strip_markdown_for_tts", return_value="hello"), \
             patch("os.path.isfile", return_value=True), \
             patch("os.makedirs"):
            await runner._send_voice_reply(event, "Hello world")
...
class TestVoiceChannelAwareness:
    ...
    def _make_adapter(self):
        ...
        adapter._voice_receivers = {}
        adapter._client = MagicMock()
        adapter._client.user = SimpleNamespace(id=99999, name="OpencodonBot")
        return adapter
    ...
    def test_returns_info_with_members(self):
        ...
        bot_member = self._make_member(99999, "OpencodonBot", is_bot=True)
        ...
        assert "Bob" in names
        assert "OpencodonBot" not in names
...
class TestVoiceTTSPlayback:
    ...
    def _call_should_reply(self, runner, voice_mode, msg_type, response="Hello",
        ...
        from opencodon.frontends.gateway.platforms.base import MessageEvent, SessionSource
        from opencodon.frontends.gateway.config import Platform
