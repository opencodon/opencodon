...
async def test_run_agent_binds_api_session_context_for_tool_env(adapter, monkeypatch):
    ...
    monkeypatch.setenv("OPENCODON_SESSION_ID", "stale-session")
    observed = {}
    ...
    class FakeAgent:
        ...
        def run_conversation(self, user_message, conversation_history, task_id):
            ...
            observed["context_platform"] = get_session_env("OPENCODON_SESSION_PLATFORM")
            observed["context_session_key"] = get_session_env("OPENCODON_SESSION_KEY")
            observed["child_session_id"] = _make_run_env({}).get("OPENCODON_SESSION_ID")
...
async def test_session_crud_and_message_history(adapter, session_db):
    ...
    async with TestClient(TestServer(app)) as cli:
        ...
        session_db.append_message(session_id, "assistant", "hello from opencodon")
