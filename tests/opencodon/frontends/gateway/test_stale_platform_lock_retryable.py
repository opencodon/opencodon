...
``gateway run --replace`` only kills same-OPENCODON_HOME PID-file holders.
...
def test_explicit_replace_takeover_reacquires_lock_once(adapter):
    ...
    with patch("opencodon.frontends.gateway.status.acquire_scoped_lock", acquire), patch(
        "opencodon.frontends.gateway.status.take_over_scoped_lock_holder",
        return_value=4242,
