...
async def test_start_gateway_replace_reaps_old_gateway_children_posix(
    ...
    monkeypatch.setattr(
        "opencodon.frontends.gateway.status.release_all_scoped_locks", lambda **kwargs: 0
    )
    monkeypatch.setattr(
        "opencodon.frontends.gateway.status._snapshot_gateway_children",
        lambda pid: events.append(("snapshot", pid)) or kids,
    )
    monkeypatch.setattr(
        "opencodon.frontends.gateway.status.reap_gateway_children",
        lambda children, *, parent_pid, timeout=5.0: events.append(
            ("reap", parent_pid, children)
        )
        or len(children),
...
async def test_start_gateway_without_replace_never_touches_old_gateway(
    ...
    monkeypatch.setattr("opencodon.frontends.gateway.status._snapshot_gateway_children", snapshot)
    monkeypatch.setattr("opencodon.frontends.gateway.status.terminate_pid", terminate)
    monkeypatch.setattr("opencodon.frontends.gateway.status.reap_gateway_children", reap)
    monkeypatch.setattr("opencodon.frontends.gateway.run.os.getpid", lambda: 100)
