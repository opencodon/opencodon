...
from opencodon.frontends.gateway.config import Platform, StreamingConfig
...
class TestGetProxyUrl:
    ...
    def test_returns_none_when_not_configured(self, monkeypatch):
        ...
        runner = _make_runner()
        with patch("opencodon.frontends.gateway.run._load_gateway_config", return_value={}):
    ...
    def test_reads_from_config_yaml(self, monkeypatch):
        ...
        with patch("opencodon.frontends.gateway.run._load_gateway_config", return_value=cfg):
...
class TestRunAgentViaProxy:
    ...
    async def test_rejects_proxy_sse_without_line_boundary_after_buffer_cap(self, monkeypatch):
        monkeypatch.setenv("GATEWAY_PROXY_URL", "http://host:8642")
        monkeypatch.delenv("GATEWAY_PROXY_KEY", raising=False)
        monkeypatch.setattr("opencodon.frontends.gateway.run._GATEWAY_PROXY_SSE_BUFFER_MAX_CHARS", 16)
        runner = _make_runner()
        source = _make_source()
