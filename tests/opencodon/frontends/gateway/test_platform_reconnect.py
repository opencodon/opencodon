...
class TestStartupPlatformIsolation:
    ...
    async def test_start_continues_after_platform_connect_timeout(self, tmp_path):
        ...
        adapters = {
            ...
            Platform.SLACK: StubAdapter(platform=Platform.SLACK),
        }
        ...
        with patch("opencodon.frontends.gateway.status.write_runtime_status"):
                ...
                with patch("opencodon.config.load_config", return_value={}):
                    with patch("opencodon.core.shell_hooks.register_from_config"):
                        with patch(
                            ...
                            with patch(
                                "opencodon.frontends.gateway.channel_directory.build_channel_directory",
                                new=AsyncMock(return_value={"platforms": {}}),
                            ):
                                with patch("opencodon.frontends.gateway.run.asyncio.create_task", side_effect=fake_create_task):
                                    assert await runner.start() is True

        assert Platform.TELEGRAM in runner._failed_platforms
        assert Platform.SLACK in runner.adapters
    ...
    async def test_connect_adapter_timeout_raises_retryable_exception(self, monkeypatch):
        ...
        monkeypatch.setenv("OPENCODON_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "0.001")
...
class TestPlatformReconnectWatcher:
    ...
    async def test_reconnect_succeeds_on_retry(self):
        ...
        with patch.object(runner, "_create_adapter", return_value=succeed_adapter):
            with patch("opencodon.frontends.gateway.run.build_channel_directory", create=True):
...
class TestSpawnSupervised:
    ...
    async def test_exception_restart_bounded_by_ceiling(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.frontends.gateway.run.asyncio.sleep", _instant_sleep)
...
class TestFatalHandoffCancellationProof:
    ...
    async def test_stranded_retryable_platform_exits_for_supervisor_restart(self):
        ...
        runner.adapters[Platform.SLACK] = StubAdapter(platform=Platform.SLACK)
