...
def _make_runner():
    ...
    from opencodon.frontends.gateway.run import GatewayRunner, _AGENT_PENDING_SENTINEL
...
class TestBusySessionAck:
    ...
    async def test_telegram_grace_followups_respect_queue_fifo(self, monkeypatch):
        ...
        monkeypatch.setenv("OPENCODON_TELEGRAM_FOLLOWUP_GRACE_SECONDS", "3.0")
    ...
    async def test_queue_mode_suppresses_interrupt_and_updates_ack(self):
        ...
        with patch("opencodon.frontends.gateway.run.merge_pending_message_event"):
    ...
    async def test_steer_mode_calls_agent_steer_no_interrupt_no_queue(self, monkeypatch):
        ...
        import opencodon.frontends.gateway.run as _gr

        monkeypatch.delenv("OPENCODON_GATEWAY_BUSY_STEER_ACK_ENABLED", raising=False)
        ...
        with patch("opencodon.frontends.gateway.run.merge_pending_message_event") as mock_merge:
            await runner._handle_active_session_busy_message(event, sk)
    ...
    async def test_steer_ack_env_override_can_suppress_visible_ack(self, monkeypatch):
        ...
        monkeypatch.setenv("OPENCODON_GATEWAY_BUSY_STEER_ACK_ENABLED", "false")
        monkeypatch.setattr(
            _gr,
            "_load_gateway_config",
...
class TestBusySessionOnboardingHint:
    ...
    async def test_first_busy_ack_appends_interrupt_hint(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(_gr, "_opencodon_home", tmp_path)
        # mark_seen imports utils.atomic_yaml_write; make sure it resolves
        # against a writable dir by pointing _opencodon_home at tmp_path.
        monkeypatch.setattr(_gr, "_load_gateway_config", lambda: {})
