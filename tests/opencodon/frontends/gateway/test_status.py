...
class TestGatewayPidState:
    ...
    def test_get_running_pid_cleans_stale_record_from_dead_process(self, tmp_path, monkeypatch):
        ...
        pid_path.write_text(json.dumps({
            ...
            "argv": ["python", "-m", "opencodon.frontends.cli.main", "gateway", "run"],
            "start_time": 111,
        }))
    ...
    def test_get_running_pid_accepts_gateway_metadata_when_cmdline_unavailable(self, tmp_path, monkeypatch):
        ...
        pid_path.write_text(json.dumps({
            "pid": os.getpid(),
            "kind": "opencodon-gateway",
            "argv": ["python", "-m", "opencodon.frontends.cli.main", "gateway"],
            "start_time": 123,
        }))
    ...
    def test_get_running_pid_accepts_script_style_gateway_cmdline(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda pid: "/venv/bin/python /repo/opencodon_cli/main.py gateway run --replace",
        )
    ...
    def test_get_running_pid_accepts_no_supervisor_restart_runtime(self, tmp_path, monkeypatch):
        ...
        record = {
            "pid": os.getpid(),
            "kind": "opencodon-gateway",
            "argv": ["python", "-m", "opencodon.frontends.cli.main", "gateway", "restart"],
            "start_time": 123,
        }
        ...
        monkeypatch.setattr(
            status,
            "_read_process_cmdline",
            lambda pid: "python -m opencodon_cli.main gateway restart",
        )
...
class TestGatewayRuntimeStatus:
    ...
    def test_write_runtime_status_overwrites_stale_argv_on_restart(self, tmp_path, monkeypatch):
        ...
        state_path.write_text(json.dumps({
            ...
            "start_time": 1000.0,
            "kind": "opencodon-gateway",
            "argv": ["/old/path/opencodon", "gateway", "run"],
            "platforms": {},
            "updated_at": "2025-01-01T00:00:00Z",
        }))

        monkeypatch.setattr(status.sys, "argv", ["/new/path/opencodon", "gateway", "run"])
        monkeypatch.setattr(status, "_get_process_start_time", lambda pid: 2000)

        status.write_runtime_status(gateway_state="running")

        payload = status.read_runtime_status()
        assert payload["argv"] == ["/new/path/opencodon", "gateway", "run"]
        assert payload["pid"] == os.getpid()
    ...
    def test_runtime_status_running_pid_rejects_stale_record_for_supervisor_pid(self, monkeypatch):
        ...
        payload = {
            ...
            "gateway_state": "running",
            "kind": "opencodon-gateway",
            "argv": ["/opt/opencodon/.venv/bin/opencodon", "gateway", "run", "--replace"],
        }
    ...
    def test_runtime_status_running_pid_rejects_pid_reused_by_other_profile(self, monkeypatch):
        ...
        onto the live *default* gateway (``opencodon gateway run``).  The recorded
        ...
        monkeypatch.setattr(
            status, "_read_process_cmdline", lambda pid: "opencodon gateway run --replace"
        )
    ...
    def test_runtime_status_running_pid_accepts_matching_profile_cmdline(self, monkeypatch):
        ...
        for cmdline in (
            "opencodon -p coder gateway run --replace",
            "/opt/opencodon/.venv/bin/opencodon --profile coder gateway run --replace",
            "opencodon_home=/opt/data/profiles/coder opencodon gateway run --replace",
        ):
    ...
    def test_runtime_status_running_pid_default_profile_rejects_named_cmdline(self, monkeypatch):
        ...
        monkeypatch.setattr(status, "_get_process_start_time", lambda pid: None)
        monkeypatch.setattr(
            status, "_read_process_cmdline", lambda pid: "opencodon -p coder gateway run --replace"
        )
    ...
    def test_runtime_status_running_pid_default_profile_accepts_bare_cmdline(self, monkeypatch):
        """The default/root gateway (bare ``opencodon gateway run``) is reported
        running for the default profile."""
        payload = {
            "pid": 139,
            "gateway_state": "running",
...
class TestScopedLocks:
    ...
    def test_acquire_scoped_lock_replaces_reused_pid_even_with_matching_start_time(self, tmp_path, monkeypatch):
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": 123,
            "kind": "opencodon-gateway",
            "argv": ["/usr/bin/python", "-m", "opencodon.frontends.cli.main", "gateway", "run"],
        }))
...
class TestTakeoverMarker:
    ...
    def test_consume_accepts_legacy_marker_without_opencodon_home(self, tmp_path, monkeypatch):
        """Back-compat (#29092): markers written by older opencodon versions have no
...
class TestScopedLockTakeover:
    ...
    def test_verified_distinct_home_handoff_marks_target_before_sigterm(
        ...
        monkeypatch.setattr(
            status,
            "_read_process_cmdline",
            lambda _pid: "python -m opencodon_cli.main gateway run",
        )
        calls = []
...
class TestPlannedStopMarker:
    ...
    def test_consume_returns_true_on_windows_when_start_time_unavailable(
        ...
        it would always return False — and ``opencodon gateway stop`` would be
...
class TestReadProcessCmdlinePsFallback:
    ...
    def test_proc_cmdline_takes_priority_over_ps(self, monkeypatch):
        ...
        def fake_read_bytes(self):
            calls.append("proc")
            return b"python\x00opencodon_cli/main.py\x00gateway\x00"

        monkeypatch.setattr(status.Path, "read_bytes", fake_read_bytes)
        result = status._read_process_cmdline(12345)
        assert "opencodon_cli/main.py" in result
        assert calls == ["proc"]
    ...
    def test_ps_fallback_used_when_proc_returns_empty(self, monkeypatch):
        ...
        monkeypatch.setattr(
            status.subprocess, "run",
            lambda args, **kwargs: SimpleNamespace(returncode=0, stdout="python opencodon_cli/main.py gateway run\n"),
        )
        result = status._read_process_cmdline(12345)
        assert "opencodon_cli/main.py" in result
    ...
    def test_windows_skips_ps_fallback_and_uses_psutil(self, monkeypatch):
        ...
        class _Proc:
            ...
            def cmdline(self):
                return ["pythonw.exe", "-m", "opencodon.frontends.cli.main", "gateway", "run"]
        ...
        assert result == "pythonw.exe -m opencodon.frontends.cli.main gateway run"
        assert ps_calls == []
...
class TestLaunchdPlistRespawnGovernance:
    def test_plist_has_throttle_interval(self, tmp_path, monkeypatch):
        monkeypatch.setenv("OPENCODON_HOME", str(tmp_path))
        from opencodon.frontends.cli.gateway import generate_launchd_plist
