...
@pytest.mark.asyncio
@patch("opencodon.plugins_runtime.invoke_hook", side_effect=Exception("boom"))
...
async def test_idle_expiry_fires_finalize_hook(mock_invoke_hook):
    ...
    with patch("opencodon.frontends.gateway.run.asyncio.sleep", side_effect=_fast_sleep):
