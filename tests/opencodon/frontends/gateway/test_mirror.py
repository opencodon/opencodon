...
import opencodon.frontends.gateway.mirror as mirror_mod
from opencodon.frontends.gateway.mirror import (
    mirror_to_session,
    _find_session_id,
)
...
class TestMirrorToSession:
    def test_successful_mirror(self, tmp_path):
        ...
        with patch.object(mirror_mod, "_SESSIONS_DIR", sessions_dir), \
             patch.object(mirror_mod, "_SESSIONS_INDEX", index_file), \
             patch("opencodon.frontends.gateway.mirror._append_to_sqlite") as mock_sqlite:
    ...
    def test_error_returns_false(self, tmp_path):
        with patch("opencodon.frontends.gateway.mirror._find_session_id", side_effect=Exception("boom")):
...
class TestAppendToSqlite:
    def test_connection_is_closed_after_use(self, tmp_path):
        ...
        from opencodon.frontends.gateway.mirror import _append_to_sqlite
        mock_db = MagicMock()

        with patch("opencodon.state.SessionDB", return_value=mock_db):
