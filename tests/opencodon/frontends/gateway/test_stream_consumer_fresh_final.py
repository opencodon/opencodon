...
from opencodon.frontends.gateway.stream_consumer import GatewayStreamConsumer, StreamConsumerConfig


def _stale_ts() -> float:
    """A ``_message_created_ts`` guaranteed to read as older than any threshold.

    NOT ``0.0``. The staleness check is ``time.monotonic() - _message_created_ts``,
    and ``time.monotonic()`` is time since boot — so ``0.0`` means "as old as the
    machine's uptime", not "infinitely old". On a developer box that is days; on a
    freshly booted CI runner it can be under the 60s threshold these tests
    configure, and the fresh-final path silently does not trigger. Anchoring to
    the current clock makes the age deterministic on any host.
    """
