...
def _clear_component_auth_env(monkeypatch):
    ...
    with patch("opencodon.frontends.gateway.pairing.PairingStore", return_value=mock_store):
        yield
...
# ---------------------------------------------------------------------------
# Pairing store: users approved via ``opencodon pairing approve`` must be
...
def test_component_check_pairing_import_error_graceful(monkeypatch):
    ...
    with patch("opencodon.frontends.gateway.pairing.PairingStore", side_effect=ImportError("simulated")):
        interaction = _interaction(11111)
        assert _component_check_auth(interaction, set(), set()) is False
