...
from opencodon.frontends.gateway import delivery_ledger as dl
...
class TestProducerHook:
    ...
    async def test_disabled_gate_skips_recording_but_sends(self):
        adapter = _Adapter()
        with patch("opencodon.frontends.gateway.delivery_ledger.ledger_enabled", return_value=False):
            await _run(adapter, _event())
        assert adapter.sent == ["final answer"]
        assert _rows() == []
    ...
    async def test_ledger_crash_never_blocks_send(self):
        adapter = _Adapter()
        with patch(
            "opencodon.frontends.gateway.delivery_ledger.record_obligation",
