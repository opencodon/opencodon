...
class TestLoadSaveCache:
    def test_load_missing_file(self, tmp_path):
        with patch("opencodon.frontends.gateway.sticker_cache.CACHE_PATH", tmp_path / "nope.json"):
            assert _load_cache() == {}

    def test_load_corrupt_file(self, tmp_path):
        bad_file = tmp_path / "bad.json"
        bad_file.write_text("not json{{{")
        with patch("opencodon.frontends.gateway.sticker_cache.CACHE_PATH", bad_file):
            assert _load_cache() == {}

    def test_save_and_load_roundtrip(self, tmp_path):
        cache_file = tmp_path / "cache.json"
        data = {"abc123": {"description": "A cat", "emoji": "", "set_name": "", "cached_at": 1.0}}
        with patch("opencodon.frontends.gateway.sticker_cache.CACHE_PATH", cache_file):
            _save_cache(data)
            loaded = _load_cache()
        assert loaded == data
