...
from opencodon.frontends.gateway.config import GatewayConfig, HomeChannel, Platform, PlatformConfig
...
def _stable_discord_tools(monkeypatch):
    ...
    monkeypatch.setattr("opencodon.frontends.gateway.session._discord_tools_loaded", lambda: True)
...
class TestEphemeralChangeKeyParity:
    ...
    def test_discord_tools_gate_flip_changes_key(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.frontends.gateway.session._discord_tools_loaded", lambda: False)
