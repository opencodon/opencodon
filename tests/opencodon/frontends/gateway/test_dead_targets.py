...
from opencodon.frontends.gateway.delivery import DeliveryRouter, DeliveryTarget
from opencodon.frontends.gateway.dead_targets import DeadTargetRegistry
...
def isolate(tmp_path, monkeypatch):
    monkeypatch.setattr("opencodon.frontends.gateway.delivery.get_opencodon_home", lambda: tmp_path)
    monkeypatch.setattr("opencodon.frontends.gateway.dead_targets.get_opencodon_home", lambda: tmp_path)
    return tmp_path
...
class TestDeadTargetRegistry:
    ...
    def test_persists_across_instances(self, isolate):
        ...
        # New instance reads the same on-disk store under tmp OPENCODON_HOME.
...
class TestNotFoundBlastRadius:
    def test_is_chat_level_not_found_chat_level(self):
        from opencodon.frontends.gateway.platforms.base import is_chat_level_not_found
    ...
    def test_classify_dead_from_error_text_gates_not_found(self):
        from opencodon.frontends.gateway.delivery import _classify_dead_from_error_text
    ...
    def test_error_blob_is_shared_source_of_truth(self):
        ...
        from opencodon.frontends.gateway.platforms import base
