"""Tests for opencodon-api-server toolset and API server tool availability."""
...
class TestOpencodonApiServerToolset:
    """Tests for the opencodon-api-server toolset definition."""

    def test_toolset_exists(self):
        ts = get_toolset("opencodon-api-server")
        assert ts is not None

    def test_toolset_validates(self):
        assert validate_toolset("opencodon-api-server")

    def test_toolset_includes_web_tools(self):
        tools = resolve_toolset("opencodon-api-server")
...
class TestApiServerPlatformConfig:
    def test_platforms_dict_includes_api_server(self):
        ...
        assert PLATFORMS["api_server"]["default_toolset"] == "opencodon-api-server"
...
class TestApiServerAdapterToolset:
    @patch("opencodon.frontends.gateway.platforms.api_server.AIOHTTP_AVAILABLE", True)
    def test_create_agent_reads_config_toolsets(self):
        ...
        with patch("opencodon.frontends.gateway.run._resolve_runtime_agent_kwargs") as mock_kwargs, \
             patch("opencodon.frontends.gateway.run._resolve_gateway_model") as mock_model, \
             patch("opencodon.frontends.gateway.run._load_gateway_config") as mock_config, \
            ...
            # No platform_toolsets override — should fall back to opencodon-api-server default
