...
async def test_non_internal_event_without_user_triggers_pairing(monkeypatch, tmp_path):
    ...
    import opencodon.frontends.gateway.run as gateway_run
    import opencodon.frontends.gateway.pairing as pairing_mod

    monkeypatch.setattr(gateway_run, "_opencodon_home", tmp_path)
    # gateway.pairing.PAIRING_DIR is a module-level constant captured at
    # import time from whichever OPENCODON_HOME was set then. Per-test
    # OPENCODON_HOME redirection in conftest doesn't retroactively move it.
    ...
    # module-level dotenv in gateway/run.py from the real ~/.opencodon/.env).
