...
from opencodon.frontends.gateway.config import PlatformConfig, GatewayConfig, Platform, _apply_env_overrides, load_gateway_config
...
class TestTelegramYamlConfigLoading:
    ...
    def test_top_level_reply_to_mode_off(self, tmp_path, monkeypatch):
        ...
        opencodon_home = self._write_config(tmp_path, "telegram:\n  reply_to_mode: off\n")
    ...
    def test_extra_reply_to_mode_off(self, tmp_path, monkeypatch):
        ...
        opencodon_home = self._write_config(
            tmp_path, "telegram:\n  extra:\n    reply_to_mode: \"off\"\n"
        )
        monkeypatch.setenv("OPENCODON_HOME", str(opencodon_home))
        monkeypatch.delenv("TELEGRAM_REPLY_TO_MODE", raising=False)
...
class TestDMTopicFallbackReplyToMode:
    ...
    Regression tests for upstream#23994:
    reply_to_mode 'off' was ignored when sending via opencodon-created DM topic
