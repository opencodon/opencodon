...
def adapter():
    ...
    a._client = SimpleNamespace(user=SimpleNamespace(id=99999, name="OpencodonBot"), guilds=[])
    return a
...
def _stub_pairing_store(monkeypatch, approved_ids):
    ...
    import opencodon.frontends.gateway.pairing as pairing
...
async def test_unauthorized_attempt_notifies_telegram(adapter):
    from opencodon.frontends.gateway.session import Platform
...
def _capture_skill_registration(adapter, monkeypatch, entries):
    ...
    import opencodon.frontends.cli.commands as _hc
    monkeypatch.setattr(
