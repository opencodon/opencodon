...
from opencodon.frontends.gateway.run import GatewayRunner, _parse_session_key
...
class TestLoadBackgroundNotificationsMode:
    ...
    def test_defaults_to_all(self, monkeypatch, tmp_path):
        import opencodon.frontends.gateway.run as gw
        monkeypatch.setattr(gw, "_opencodon_home", tmp_path)
        monkeypatch.delenv("OPENCODON_BACKGROUND_NOTIFICATIONS", raising=False)
    ...
    def test_env_var_overrides_config(self, monkeypatch, tmp_path):
        ...
        import opencodon.frontends.gateway.run as gw
        monkeypatch.setattr(gw, "_opencodon_home", tmp_path)
        monkeypatch.setenv("OPENCODON_BACKGROUND_NOTIFICATIONS", "off")
