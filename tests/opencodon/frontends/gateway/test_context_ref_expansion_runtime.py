...
``OpencodonCLI``, which does carry ``self.model``/``self.base_url`` — see
...
from opencodon.core.context.context_references import ContextReferenceResult
...
async def test_at_reference_reaches_preprocessor_with_real_context_length(
    ...
    import opencodon.core.context.context_references as ctx_mod

    monkeypatch.setattr(ctx_mod, "preprocess_context_references_async", _fake_preprocess)

    caplog.set_level(logging.DEBUG, logger="opencodon.frontends.gateway.run")
...
async def test_at_reference_resolves_model_via_session_runtime(monkeypatch):
    ...
    import opencodon.core.providers.model_metadata as model_meta_mod
