...
def isolated_home(tmp_path, monkeypatch):
    ...
    _saved = {
        ...
        if k.startswith(("opencodon_cli", "gateway"))
    }
    ...
    def fresh_gateway():
        for mod in list(sys.modules.keys()):
            if mod.startswith(("opencodon_cli", "gateway")):
                del sys.modules[mod]
        return importlib.import_module("opencodon.frontends.gateway.run")
...
def test_lift_helper_accepts_alias_and_rejects_garbage(isolated_home):
    ...
    for mod in list(sys.modules.keys()):
        if mod.startswith("opencodon_cli"):
            del sys.modules[mod]
    rp = importlib.import_module("opencodon.core.runtime_provider")
