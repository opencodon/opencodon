...
from opencodon.frontends.acp import entry
...
def test_main_setup_runs_model_configuration(monkeypatch):
    ...
    monkeypatch.setattr("opencodon.frontends.cli.main.main", fake_opencodon_main)
...
def test_main_setup_offers_browser_install_when_tty(monkeypatch):
    ...
    monkeypatch.setattr("opencodon.frontends.cli.main.main", lambda: None)
...
def test_main_setup_browser_calls_ensure_dependency(monkeypatch):
    """`opencodon-acp --setup-browser` routes through dep_ensure.ensure_dependency."""
    calls = []
    ...
    monkeypatch.setattr("opencodon.frontends.cli.dep_ensure.ensure_dependency", fake_ensure)
