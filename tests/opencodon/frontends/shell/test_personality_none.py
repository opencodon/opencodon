...
class TestGatewayPersonalityNone:
    ...
    @pytest.mark.asyncio
    async def test_none_clears_ephemeral_prompt(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_default_clears_ephemeral_prompt(self, tmp_path, monkeypatch):
        runner = self._make_runner()
    ...
    async def test_empty_personality_list_uses_profile_display_path(self, tmp_path):
        ...
        with patch("opencodon.frontends.gateway.run._opencodon_home", tmp_path), \
             patch("opencodon_constants.display_opencodon_home", return_value="~/.opencodon/profiles/coder"):
            event = self._make_event("")
            result = await runner._handle_personality_command(event)

        assert result == "No personalities configured in `~/.opencodon/profiles/coder/config.yaml`"
...
class TestPersonalityDictFormat:
    ...
    def test_resolve_prompt_dict_no_tone_no_style(self):
        from opencodon.frontends.cli.shell import OpencodonCLI
        result = OpencodonCLI._resolve_personality_prompt({
    ...
    def test_resolve_prompt_string(self):
        from opencodon.frontends.cli.shell import OpencodonCLI
        result = OpencodonCLI._resolve_personality_prompt("You are helpful.")
        assert result == "You are helpful."
