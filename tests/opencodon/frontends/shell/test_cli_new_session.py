...
def _reset_session_id_context():
    from opencodon.frontends.gateway.session_context import _UNSET, _VAR_MAP
...
def test_new_command_rotates_opencodon_session_id_env_and_context(tmp_path):
    from opencodon.frontends.gateway.session_context import _VAR_MAP, get_session_env
    ...
    assert cli.session_id != old_session_id
    assert os.environ["OPENCODON_SESSION_ID"] == cli.session_id
    assert get_session_env("OPENCODON_SESSION_ID") == cli.session_id
