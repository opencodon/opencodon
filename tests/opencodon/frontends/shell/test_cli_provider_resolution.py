...
def test_opencodon_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}
    ...
    monkeypatch.setattr("opencodon.core.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("opencodon.core.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("opencodon.core.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("opencodon.core.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_model_flow_openrouter_clears_stale_custom_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.frontends.cli.main._prompt_api_key",
        lambda *args, **kwargs: ("sk-openrouter", False),
    )
    monkeypatch.setattr(
        "opencodon.core.models.model_ids",
        lambda **kwargs: ["anthropic/claude-sonnet-4.6"],
    )
    monkeypatch.setattr("opencodon.core.models.get_pricing_for_provider", lambda *a, **k: {})
    monkeypatch.setattr(
...
def test_model_flow_anthropic_clears_stale_custom_key_and_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("opencodon.core.auth.get_anthropic_key", lambda: "sk-ant-api03-test")
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "opencodon.frontends.cli.model_setup_flows._prompt_auth_credentials_choice",
        lambda title: "use",
    )
    monkeypatch.setattr(
        "opencodon.core.auth._prompt_model_selection",
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    ...
    monkeypatch.setattr("opencodon.core.auth.resolve_provider", _resolve_provider)
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    ...
    monkeypatch.setattr("opencodon.core.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("opencodon.core.auth.deactivate_provider", lambda: None)
    ...
    monkeypatch.setattr("opencodon.frontends.cli.secret_prompt.masked_secret_prompt", lambda _prompt="": next(answers))
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr("opencodon.core.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("opencodon.core.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr(
        "opencodon.core.models.probe_api_models",
        lambda api_key, base_url: {
    ...
    monkeypatch.setattr(
        "opencodon.frontends.cli.main._save_custom_provider",
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("opencodon.frontends.cli.secret_prompt.masked_secret_prompt", lambda _prompt="": "test-key")
...
def test_auto_provider_name_localhost():
    from opencodon.frontends.cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from opencodon.frontends.cli.main import _save_custom_provider
