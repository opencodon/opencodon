"""Behavior tests for opencodon_cli.inventory.
...
from opencodon.frontends.cli.inventory import (
    ConfigContext,
...
def test_build_models_payload_does_not_call_provider_model_ids():
    ...
    ctx = _empty_ctx()
    with _list_auth_returning(rows), \
         patch("opencodon.frontends.cli.models.provider_model_ids") as mock_pm:
...
def test_explicit_only_keeps_moa_when_raw_config_has_enabled_preset():
    ...
    with (
        ...
        patch("opencodon.frontends.cli.auth.is_provider_explicitly_configured", return_value=False),
    ):
        payload = build_models_payload(ctx, explicit_only=True)
...
def test_picker_hints_adds_warning_to_skeleton_rows():
    ...
    for row in skeleton_rows:
        ...
        # opencodon model fallback.
        assert (
            row["warning"].startswith("paste ")
            or row["warning"].startswith("run `opencodon model`")
        )
...
def test_payload_shape_compatible_with_modelpickerdialog_frontend():
    """The frontend model picker reads:
...
def test_build_models_payload_forwards_refresh_flag():
    ...
    with patch("opencodon.frontends.cli.model_switch.list_authenticated_providers", side_effect=_capture):
...
def test_list_authenticated_providers_refresh_busts_cache():
    ...
    with patch("opencodon.frontends.cli.models.clear_provider_models_cache") as clear:
