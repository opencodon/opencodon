...
def test_format_banner_version_label_is_our_version_only():
    from opencodon.frontends.cli import banner
...
def test_get_git_banner_state_falls_back_to_build_sha_when_no_repo():
    ...
    parent nor ``$OPENCODON_HOME/opencodon/`` is a git repo (the canonical
...
def test_get_git_banner_state_falls_back_when_live_git_returns_nothing(tmp_path):
    ...
    with patch("opencodon.frontends.cli.banner.subprocess.run", return_value=failed), \
