"""Tests for issue #26670 — concurrent opencodon.exe detection and improved
quarantine retry / reboot-deferred fallback during `opencodon update` on Windows.
...
# Tests in this module either exercise the REAL _detect_concurrent_opencodon_instances
# helper (and need the autouse stub in tests/opencodon_cli/conftest.py disabled),
...
def _make_proc(pid: int, exe: str, name: str = "opencodon.exe"):
...
def test_detect_concurrent_excludes_self_pid(_winp, tmp_path):
    ...
    procs = [_make_proc(my_pid, str(shim), "opencodon.exe")]
...
@patch.object(cli_main, "_is_windows", return_value=True)
def test_detect_concurrent_finds_other_opencodon_process(_winp, tmp_path):
    scripts_dir = tmp_path
    shim = scripts_dir / "opencodon.exe"
    shim.write_bytes(b"")
    ...
    procs = [
        _make_proc(other_pid, str(shim), "opencodon.exe"),
    ...
    assert result == [(other_pid, "opencodon.exe")]
...
def test_detect_concurrent_matches_case_insensitively(_winp, tmp_path):
    ...
    # Simulate the desktop spawning opencodon.EXE (uppercase ext) from same path
    upper = str(shim).replace("opencodon.exe", "OPENCODON.EXE")
    procs = [_make_proc(9999, upper, "OPENCODON.EXE")]
    fake_psutil = types.SimpleNamespace(process_iter=lambda attrs: iter(procs))
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_opencodon_instances(scripts_dir)

    assert result == [(9999, "OPENCODON.EXE")]
...
def test_detect_concurrent_is_noop_off_windows(_winp, tmp_path):
    ...
    assert cli_main._detect_concurrent_opencodon_instances(tmp_path) == []
...
def test_detect_concurrent_excludes_parent_chain(_winp, tmp_path):
    ...
    Simulates the real Windows topology: opencodon.exe launcher (PID L) spawns
    ...
    rows = [
        ...
        _make_proc(launcher_pid, str(shim), "opencodon.exe"),
    ]
...
@patch.object(cli_main, "_is_windows", return_value=True)
def test_detect_concurrent_still_finds_unrelated_other_opencodon(_winp, tmp_path):
    """A sibling opencodon.exe outside our ancestor chain must still be reported."""
    scripts_dir = tmp_path
    shim = scripts_dir / "opencodon.exe"
    shim.write_bytes(b"")
    me = os.getpid()
    launcher_pid = me + 100  # our .exe launcher (parent — must be excluded)
    sibling_pid = me + 200  # an UNRELATED opencodon.exe (must still be reported)

    rows = [
        _make_proc(me, str(shim), "python.exe"),
        _make_proc(launcher_pid, str(shim), "opencodon.exe"),
        _make_proc(sibling_pid, str(shim), "opencodon.exe"),
    ]
    fake_psutil = _fake_psutil_with_parent_chain(
        parent_chain=[launcher_pid],
        proc_iter_rows=rows,
    ...
    assert result == [(sibling_pid, "opencodon.exe")]
...
def test_detect_concurrent_parent_chain_walks_deep(_winp, tmp_path):
    ...
    rows = [
        _make_proc(me, str(shim), "python.exe"),
        _make_proc(parent_pid, str(shim), "opencodon.exe"),
        _make_proc(grandparent_pid, str(shim), "opencodon.exe"),
        _make_proc(greatgrandparent_pid, str(shim), "opencodon.exe"),
    ]
    fake_psutil = _fake_psutil_with_parent_chain(
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(_winp, tmp_path):
    ...
    assert result == [(launcher_pid, "opencodon.exe")]
...
def test_detect_concurrent_parent_walk_handles_stub_without_process(_winp, tmp_path):
    ...
    rows = [
        _make_proc(me, str(shim), "opencodon.exe"),
        _make_proc(other_pid, str(shim), "opencodon.exe"),
    ]
...
# ---------------------------------------------------------------------------
# _quarantine_running_opencodon_exe — retry + reboot-deferred fallback
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    shim = tmp_path / "opencodon.exe"
    shim.write_bytes(b"old")

    pairs = cli_main._quarantine_running_opencodon_exe(tmp_path)

    assert len(pairs) == 1
    orig, quarantine = pairs[0]
    assert orig == shim
    assert quarantine.name.startswith("opencodon.exe.old.")
...
def test_quarantine_retries_then_succeeds(_winp, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli_main, "_opencodon_exe_shims", lambda d: [shim])
...
def test_quarantine_actionable_warning_when_everything_fails(
    ...
    assert "opencodon Desktop" in captured or "gateway" in captured.lower()
...
def test_pause_windows_gateways_for_update_stops_profile_and_unmapped_pids(
    ...
    monkeypatch.setattr(
        ...
        lambda pid: ["pythonw.exe", "-m", "opencodon.frontends.cli.main", "gateway", "run"]
        if pid == 202
        else None,
    )
    ...
    assert token == {
        ...
        "unmapped": [
            {
                "pid": 202,
                "argv": ["pythonw.exe", "-m", "opencodon.frontends.cli.main", "gateway", "run"],
            }
        ],
    }
...
def test_resume_windows_gateways_after_update_respawns_unmapped_by_cmdline(
    ...
    scheduled_argv = ["pythonw.exe", "-m", "opencodon.frontends.cli.main", "gateway", "run"]
    token = {
        "resume_needed": True,
        "profiles": {},
        "unmapped_pids": [7560],
...
def test_cmd_update_aborts_on_concurrent_instance(_winp, tmp_path, capsys):
    """If another opencodon.exe is running, the update bails out before
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_opencodon_instances",
        return_value=[(4242, "opencodon.exe")],
    ), patch.object(
...
def test_cmd_update_force_bypasses_concurrent_check(_winp, tmp_path):
    ...
    detect = MagicMock(return_value=[(9, "opencodon.exe")])
    ...
    with patch.object(
        ...
        cli_main, "_detect_concurrent_opencodon_instances", detect
    ), patch.object(
