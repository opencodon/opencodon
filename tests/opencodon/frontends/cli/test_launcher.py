...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    ...
    launcher_path = Path(__file__).resolve().parents[4] / "bin" / "opencodon"
    called = []

    fake_main_module = types.ModuleType("opencodon.frontends.cli.main")

    def fake_main():
        called.append("opencodon.frontends.cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "opencodon.frontends.cli.main", fake_main_module)

    fake_cli_module = types.ModuleType("opencodon.frontends.cli.shell")
    ...
    assert called == ["opencodon.frontends.cli.main"]
