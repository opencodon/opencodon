...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<opencodon_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<opencodon_home>/plugins/``; we derive
    ``<opencodon_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <OPENCODON_HOME>/config.yaml.
        # Config is always read from OPENCODON_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        opencodon_home_str = os.environ.get("OPENCODON_HOME")
        if opencodon_home_str:
            opencodon_home = Path(opencodon_home_str)
        else:
            opencodon_home = base.parent
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.opencodon/plugins/ are discovered."""
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OPENCODON_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".opencodon" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under opencodon_plugins.<name>."""
        plugins_dir = tmp_path / "opencodon_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("OPENCODON_HOME", str(tmp_path / "opencodon_test"))

        # Clean up any prior namespace module
        sys.modules.pop("opencodon_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "opencodon_plugins.ns_plugin" in sys.modules
...
class TestPluginContext:
    ...
    def test_register_tool_override_blocked_via_delayed_callback(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            import sys as _sys
            mod = _sys.modules.get("opencodon_plugins.delayed_override_plugin")
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `opencodon plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        opencodon_home = tmp_path / "opencodon_test"
        plugins_dir = opencodon_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "opencodon_test" / "plugins"
        # Set OPENCODON_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginDebugLogging:
    """OPENCODON_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("OPENCODON_PLUGINS_DEBUG", raising=False)
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With OPENCODON_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("OPENCODON_PLUGINS_DEBUG", "1")
        from opencodon.frontends.cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from OPENCODON_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """OPENCODON_HOME at the root resolves to 'default'."""
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """OPENCODON_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".opencodon" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OPENCODON_HOME", str(prof))
    ...
    def test_works_without_cli_ref(self, tmp_path, monkeypatch):
        ...
        prof = tmp_path / ".opencodon" / "profiles" / "worker1"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OPENCODON_HOME", str(prof))
        ctx = self._ctx()
