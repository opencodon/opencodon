...
def test_tui_finds_bundled_entry_js(tmp_path):
    ...
    tui_dist = tmp_path / "opencodon_cli" / "tui_dist"
    tui_dist.mkdir(parents=True)
    entry = tui_dist / "entry.js"
    entry.write_text("// bundled TUI", encoding="utf-8")

    from opencodon.frontends.cli.main import _find_bundled_tui
    result = _find_bundled_tui(opencodon_cli_dir=tmp_path / "opencodon_cli")
