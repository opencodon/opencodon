...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.core.providers.anthropic_adapter.run_opencodon_oauth_login_pure",
        lambda: {
            "access_token": token,
            "refresh_token": "refresh-token",
...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.core.credentials.auth._minimax_oauth_login",
        lambda **kwargs: {
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.core.credentials.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("opencodon.core.credentials.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from opencodon.core.credentials.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from opencodon.core.credentials.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.core.credentials.auth._xai_oauth_device_code_login",
        lambda **kwargs: {
            "tokens": {
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from opencodon.frontends.cli.auth_commands import auth_reset_command
...
def test_logout_clears_stale_active_codex_without_provider_credentials(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI Codex." in out
    auth_payload = json.loads((opencodon_home / "auth.json").read_text())
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from opencodon.core.credentials.auth import _reset_config_provider
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from opencodon.frontends.cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "opencodon.frontends.cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("opencodon.frontends.cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("opencodon.frontends.cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("opencodon.frontends.cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "opencodon.frontends.cli.auth_commands.time.time",
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    (opencodon_home / "auth.json").write_text(json.dumps(auth_store))

    from types import SimpleNamespace
    from opencodon.frontends.cli.auth_commands import auth_remove_command
    auth_remove_command(SimpleNamespace(provider="anthropic", target="1"))

    updated = json.loads((opencodon_home / "auth.json").read_text())
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from opencodon.core.credentials.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("opencodon.core.credentials.auth._import_codex_cli_tokens", _fake_import)

    from opencodon.core.credentials.credential_pool import _seed_from_singletons
    ...
    after = json.loads((opencodon_home / "auth.json").read_text())
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.opencodon/.env (not the shell), the
    ...
    (opencodon_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (opencodon_home / ".env").read_text().strip() == ""
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    from opencodon.core.credentials.credential_pool import _seed_from_env
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import opencodon.core.credentials.copilot_auth as ca
...
def test_seed_from_singletons_respects_qwen_suppression(tmp_path, monkeypatch):
    ...
    import opencodon.core.credentials.auth as ha
...
def test_seed_from_singletons_respects_hermes_pkce_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (opencodon_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (opencodon_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
    ...
    import opencodon.core.providers.anthropic_adapter as aa
...
def test_seed_custom_pool_respects_config_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (opencodon_home / "config.yaml").write_text(yaml.dump({
        "model": {},
        "custom_providers": [
    ...
    from opencodon.core.credentials.credential_pool import _seed_custom_pool, get_custom_provider_pool_key
...
def test_credential_sources_registry_has_expected_steps():
    ...
    provider.
    """
    from opencodon.core.credentials.credential_sources import _REGISTRY
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    with patch(
        ...
        "opencodon.core.credentials.copilot_auth.get_copilot_api_token",
