...
``opencodon setup`` / ``opencodon model`` when an API key already exists in ``.env``.
...
def _run_prompt(existing_key, choice, new_key="", provider_id="", pconfig_name="deepseek"):
    ...
    with patch("builtins.input", return_value=choice), \
         patch("opencodon.frontends.cli.secret_prompt.masked_secret_prompt", return_value=new_key):
