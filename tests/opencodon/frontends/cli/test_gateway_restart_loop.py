...
Covers:
- Defense 1: gateway stop/restart refuse when _OPENCODON_GATEWAY=1
...
from opencodon.frontends.cli.cron import (
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl unload ~/Library/LaunchAgents/ai.opencodon.gateway.plist",
        "launchctl stop ai.opencodon.gateway",
        "systemctl restart opencodon-gateway",
        "systemctl stop opencodon-gateway.service",
        "systemctl start opencodon-gateway",
    ])
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.opencodon.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_OPENCODON_GATEWAY", raising=False)
        import opencodon.frontends.cli.gateway as gw
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _OPENCODON_GATEWAY=1.

    Issue #37453: systemctl --user restart opencodon-gateway runs as a child of the
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart opencodon-gateway", force=True
        ))
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart opencodon-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart opencodon-gateway"]
...
class TestLifecycleGuardModule:
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.opencodon.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    opencodon_cli.cron.cron_create. Enforcing at create_job covers both."""
...
class TestRestartLoopGuard:
    ...
    def _isolate_state(self, tmp_path, monkeypatch):
        monkeypatch.setenv("OPENCODON_HOME", str(tmp_path / ".opencodon"))
        (tmp_path / ".opencodon").mkdir(parents=True)
        import opencodon.frontends.gateway.restart_loop_guard as rlg
        rlg.clear()
