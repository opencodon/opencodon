...
# ---------------------------------------------------------------------------
# June 2026 opencodon-0day campaign: SSH/PAM/sudoers/cron persistence + IOC block
# ---------------------------------------------------------------------------


def _opencodon_0day_entry():
    ...
    """
    key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAICBoh1oDC4DnsO1m5mJ4yfEKrQebaFh opencodon-0day"
    return {
        "command": "bash",
        "args": [
            "-c",
...
def test_validator_flags_ssh_key_persistence_payload():
    """The opencodon-0day authorized_keys payload has NO network egress — it must
    still be flagged via the persistence-surface rule."""
    from opencodon.frontends.cli.mcp_security import validate_mcp_server_entry

    warnings = validate_mcp_server_entry("h1781406356", _opencodon_0day_entry())
    assert warnings
    # Either the IOC blocklist (opencodon-0day key) or the persistence rule fires.
...
def test_ioc_blocklist_rejects_regardless_of_command_shape():
    ...
    warnings = validate_mcp_server_entry("s1781324909", {
        "command": "python3",
        "args": ["server.py"],
        "env": {"NOTE": "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAICBoh1oDC4DnsO1m5mJ4yfEKrQebaFh opencodon-0day"},
    })
    assert warnings
...
def test_mcp_add_rejects_dangerous_entry_before_probe(monkeypatch, capsys):
    ...
    monkeypatch.setattr("opencodon.frontends.cli.mcp_config._probe_single_server", _probe_should_not_run)
