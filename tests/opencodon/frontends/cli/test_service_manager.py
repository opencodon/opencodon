"""Tests for opencodon_cli.service_manager — the abstract ServiceManager
...
def test_detect_service_manager_s6_keys_off_s6_running_not_is_container(
    ...
    monkeypatch.setattr(
        "opencodon.frontends.cli.service_manager._s6_running", lambda: True,
    )
...
def test_s6_running_true_when_comm_and_basedir_match(
    ...
    from opencodon.frontends.cli.service_manager import _s6_running
...
def test_systemd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "opencodon.frontends.cli.gateway.systemd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "opencodon.frontends.cli.gateway.systemd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "opencodon.frontends.cli.gateway.systemd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "opencodon.frontends.cli.gateway._probe_systemd_service_running",
...
def test_launchd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "opencodon.frontends.cli.gateway.launchd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "opencodon.frontends.cli.gateway.launchd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "opencodon.frontends.cli.gateway.launchd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "opencodon.frontends.cli.gateway._probe_launchd_service_running", lambda: False,
    )
    mgr = LaunchdServiceManager()
    mgr.start("ignored")
    mgr.stop("ignored")
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `opencodon_cli` package succeeds — gateway_windows is
    # imported lazily inside the wrapper and may not yet be loaded.
    import opencodon.frontends.cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("opencodon.frontends.cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
        "opencodon.frontends.cli.gateway.find_gateway_pids",
        lambda **kw: [12345],
    )
    mgr = WindowsServiceManager()
    mgr.start("ignored")
...
def test_get_service_manager_returns_correct_backend(
    ...
    monkeypatch.setattr(
        "opencodon.frontends.cli.service_manager.detect_service_manager", lambda: kind,
    )
...
def test_get_service_manager_raises_when_unsupported(
    ...
    monkeypatch.setattr(
        "opencodon.frontends.cli.service_manager.detect_service_manager", lambda: "none",
    )
...
def test_get_service_manager_returns_s6_instance(
    ...
    monkeypatch.setattr(
        "opencodon.frontends.cli.service_manager.detect_service_manager", lambda: "s6",
    )
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from opencodon.frontends.cli.service_manager import _seed_supervise_skeleton
...
def test_s6_register_staging_dir_is_dotfile_hidden_from_svscan(
    ...
    """
    import opencodon.frontends.cli.service_manager as sm
...
def test_lifecycle_raises_s6_command_error_on_subprocess_failure(
    ...
    import subprocess as _sp
    from opencodon.frontends.cli.service_manager import S6CommandError
...
def test_s6_stop_writes_planned_stop_marker(monkeypatch, s6_scandir):
    ...
    marked: list[int] = []
    monkeypatch.setattr(
        "opencodon.frontends.gateway.status.write_planned_stop_marker",
...
def test_s6_stop_tolerates_marker_write_failure(monkeypatch, s6_scandir):
    ...
    monkeypatch.setattr("opencodon.frontends.gateway.status.write_planned_stop_marker", _boom)
