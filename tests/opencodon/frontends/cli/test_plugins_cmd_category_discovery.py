...
class TestReadManifestInfo:
    def test_flat_plugin(self, tmp_path):
        from opencodon.frontends.cli.plugins_cmd import _read_manifest_info
...
class TestDiscoverAllPlugins:
    ...
    def test_flat_plugins_still_discovered(self, mock_user_dir, mock_bundled_dir, tmp_path):
        from opencodon.frontends.cli.plugins_cmd import _discover_all_plugins
...
class TestPluginStatus:
    def test_name_in_enabled(self):
        from opencodon.frontends.cli.plugins_cmd import _plugin_status
...
class TestFilterPluginEntries:
    def test_enabled_filter_uses_key(self):
        from opencodon.frontends.cli.plugins_cmd import _filter_plugin_entries
...
class TestCmdListJson:
    ...
    def test_json_status_uses_key(self, mock_user_dir, mock_bundled_dir, tmp_path, capsys):
        ...
        with patch("opencodon.frontends.cli.plugins_cmd._get_enabled_set", return_value={"web/tavily"}):
            args = MagicMock()
            args.json = True
            args.plain = False
            args.no_bundled = False
