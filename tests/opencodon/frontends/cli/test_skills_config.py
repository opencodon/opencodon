"""Tests for opencodon_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("opencodon.frontends.cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from opencodon.frontends.cli.skills_config import save_disabled_skills
        config = {}
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """OPENCODON_SESSION_PLATFORM should be used when OPENCODON_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_opencodon_platform_takes_precedence(self, tmp_path, monkeypatch):
        """OPENCODON_PLATFORM should win over OPENCODON_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from opencodon.frontends.cli.skills_config import _get_categories
        skills = [
