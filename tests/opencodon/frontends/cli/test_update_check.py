"""Tests for the update check mechanism in opencodon_cli.banner."""
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OPENCODON_HOME", str(tmp_path))
    with patch("opencodon.frontends.cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_official_ssh_origin_uses_https_probe(tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd == ["git", "remote", "get-url", "origin"]:
            return MagicMock(returncode=0, stdout="git@github.com:opencodon/opencodon.git\n")
    ...
    with patch("opencodon.frontends.cli.banner.subprocess.run", side_effect=fake_run):
...
def test_check_via_local_git_shallow_clone_behind_reports_no_count(tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        calls.append(cmd)
        if cmd == ["git", "remote", "get-url", "origin"]:
            return MagicMock(returncode=0, stdout="https://github.com/opencodon/opencodon.git\n")
...
def test_check_for_updates_docker_returns_none(tmp_path, monkeypatch):
    ...
    with patch("opencodon.config.detect_install_method", return_value="docker"), \
         patch("opencodon.frontends.cli.banner.subprocess.run") as mock_run:
        result = banner.check_for_updates()
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from opencodon.frontends.cli.main import _invalidate_update_cache
