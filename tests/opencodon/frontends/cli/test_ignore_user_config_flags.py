...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("OPENCODON_IGNORE_USER_CONFIG", "OPENCODON_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``OPENCODON_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<opencodon_home>/config.yaml`` is
    ...
    def _write_user_config(self, tmp_path, model_default):
        ...
        # config. With OPENCODON_IGNORE_USER_CONFIG=1, load_cli_config() falls
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._opencodon_home at tmp_path and return a fresh load_cli_config."""
        cli = importlib.import_module("opencodon.frontends.cli.shell")
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "test-vendor/ignore-user-config-sentinel")
        monkeypatch.setenv("OPENCODON_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``opencodon_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("OPENCODON_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("OPENCODON_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("OPENCODON_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("OPENCODON_IGNORE_RULES") == "1"
    ...
    def test_only_ignore_user_config(self, monkeypatch):
        ...
        assert os.environ.get("OPENCODON_IGNORE_USER_CONFIG") == "1"
        assert "OPENCODON_IGNORE_RULES" not in os.environ
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "OPENCODON_IGNORE_USER_CONFIG" not in os.environ
        assert "OPENCODON_IGNORE_RULES" not in os.environ
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        ...
        import inspect
        import opencodon.frontends.cli.main as hm
        src = inspect.getsource(hm)
        assert "OPENCODON_IGNORE_USER_CONFIG" in src
        assert "OPENCODON_IGNORE_RULES" in src
