...
def test_run_anthropic_oauth_flow_prefers_claude_code_credentials(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "opencodon.core.providers.anthropic_adapter.run_oauth_setup_token",
...
def test_run_anthropic_oauth_flow_manual_token_still_persists(tmp_path, monkeypatch, capsys):
    monkeypatch.setenv("OPENCODON_HOME", str(tmp_path))
    monkeypatch.setattr("opencodon.core.providers.anthropic_adapter.run_oauth_setup_token", lambda: None)
    monkeypatch.setattr("opencodon.core.providers.anthropic_adapter.read_claude_code_credentials", lambda: None)
    monkeypatch.setattr("opencodon.core.providers.anthropic_adapter.is_claude_code_token_valid", lambda creds: False)
    monkeypatch.setattr("builtins.input", lambda _prompt="": "sk-ant-oat01-manual-token")
    monkeypatch.setattr(
        "opencodon.frontends.cli.secret_prompt.masked_secret_prompt",
