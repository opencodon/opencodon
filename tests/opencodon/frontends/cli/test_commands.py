...
class TestSlackNativeSlashes:
    ...
    def test_opencodon_catchall_is_first(self):
        """``/opencodon`` must be reserved as the first slot so the legacy
        ``/opencodon <subcommand>`` form keeps working after we add new
        commands and hit the 50-slash cap."""
        slashes = slack_native_slashes()
        assert slashes[0][0] == "opencodon"
    ...
    def test_excludes_slack_reserved_commands(self):
        ...
        Users can still reach them via /opencodon <command>."""
        names = {n for n, _d, _h in slack_native_slashes()}
    ...
    def test_includes_aliases_as_first_class_slashes(self):
        ...
        aliases stay reachable via ``/opencodon <alias>``.
        """
        slashes = slack_native_slashes()
    ...
    def test_telegram_parity(self):
        ...
        # Commands deliberately routed through /opencodon <command> on Slack only
        # (Slack's 50-slash cap) are expected to be absent from native slashes.
        via_opencodon_norm = {_norm(n) for n in _SLACK_VIA_OPENCODON_ONLY}
        missing = (tg_norm - slack_norm) - reserved_norm - via_opencodon_norm
        assert not missing, (
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `opencodon slack manifest`)."""
...
class TestSubcommandCompletion:
    ...
    def test_tools_enable_completes_toolset_names(self, monkeypatch):
        ...
        from opencodon.frontends.cli import commands as commands_mod
        ...
        monkeypatch.setattr(
            "opencodon.frontends.cli.tools_config._get_plugin_toolset_keys",
            lambda: set(),
        )
    ...
    def _fake_gateway(self, monkeypatch, platforms):
        ...
        monkeypatch.setattr("opencodon.frontends.gateway.config.load_gateway_config", lambda: fake)
    ...
    def test_handoff_completion_swallows_config_errors(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.frontends.gateway.config.load_gateway_config", _boom)
...
class TestDiscordSkillCmdKeyDispatch:
    ...
    def test_long_skill_name_retains_cmd_key(self, tmp_path, monkeypatch):
        ...
        with patch("opencodon.core.skills.skill_commands.get_skill_commands", return_value=fake_cmds), \
             ...
             patch("opencodon.core.skills.skill_utils.get_external_skills_dirs", return_value=[]):
...
from opencodon.frontends.cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        ...
        from opencodon.frontends.cli import plugins as _plugins_mod
    ...
    def test_plugin_command_appears_in_slack_subcommand_map(self, monkeypatch):
        """/opencodon metricas must route through the Slack subcommand map."""
        self._patch_plugin_commands(monkeypatch, {
            "metricas": {
                "handler": lambda _a: "ok",
                "description": "Metrics",
