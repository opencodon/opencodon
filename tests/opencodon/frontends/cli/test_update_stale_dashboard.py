...
def _refresh_bindings_against_live_module():
    """Rebind module-level names to the *current* ``opencodon_cli.main``.

    Other tests in the suite (notably ``test_env_loader.py`` and
    ``test_skills_subparser.py``) reload or delete ``opencodon_cli.main`` from
    ``sys.modules``.  When that happens on the same xdist worker before we
    run, our top-of-file ``from opencodon_cli.main import ...`` bindings end
    up pointing at the *old* module object.  ``patch(\"opencodon.frontends.cli.main.X\")``
    ...
    live = sys.modules.get("opencodon.frontends.cli.main")
    if live is None:
        live = importlib.import_module("opencodon.frontends.cli.main")
...
class TestFindStaleDashboardPids:
    ...
    def test_matches_running_dashboard(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=_ps_line(12345, "python3 -m opencodon_cli.main dashboard --port 9119") + "\n",
                stderr="",
            )
    ...
    def test_unrelated_process_containing_word_dashboard_not_matched(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                ...
                stdout="\n".join([
                    _ps_line(12345, "python3 -m opencodon_cli.main dashboard --port 9119"),
                    _ps_line(22222, "python3 -m opencodon_cli.main chat -q 'rewrite my dashboard'"),
...
class TestKillStaleDashboardPosix:
    ...
    def test_no_stale_processes_is_a_noop(self, capsys):
        with patch("opencodon.frontends.cli.main._find_stale_dashboard_pids", return_value=[]):
            _kill_stale_dashboard_processes()
    ...
    def test_update_path_restarts_managed_dashboard_instead_of_killing(self, capsys):
        ...
        with patch("subprocess.run", side_effect=fake_run), \
             patch("opencodon.frontends.cli.main._find_stale_dashboard_pids",
    ...
    def test_user_scope_restart_never_falls_back_to_system_or_sudo(self, capsys):
        ...
        with patch("subprocess.run", side_effect=fake_run), \
             patch("opencodon.frontends.cli.main._find_stale_dashboard_pids", return_value=[12345]) as find_pids, \
             patch("os.kill") as kill:
            _kill_stale_dashboard_processes(restart_managed=True)
    ...
    def test_user_scope_restart_failure_does_not_try_system_or_sudo(self):
        ...
        with patch("subprocess.run", side_effect=fake_run), \
             patch("opencodon.frontends.cli.main._find_stale_dashboard_pids") as find_pids, \
             patch("os.kill") as kill:
            _kill_stale_dashboard_processes(restart_managed=True)
    ...
    def test_wmic_invoked_with_utf8_ignore_errors(self, monkeypatch):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "CommandLine=python -m opencodon_cli.main dashboard\n"
                    "ProcessId=12345\n"
                ),
                stderr="",
            )
