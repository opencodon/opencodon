"""Tests for opencodon_cli.gateway."""
...
def _install_fake_gateway_run(monkeypatch, start_gateway):
    module = ModuleType("opencodon.frontends.gateway.run")
    ...
    monkeypatch.setitem(sys.modules, "opencodon.frontends.gateway.run", module)
...
def test_gateway_run_subprocess_preserves_daemon_exit_codes(
    ...
    script = textwrap.dedent(
        ...
        fake_run = types.ModuleType("opencodon.frontends.gateway.run")
        fake_run.start_gateway = start_gateway
        setattr(fake_run, "_exit_after_graceful_shutdown", sys.exit)
        sys.modules["opencodon.frontends.gateway.run"] = fake_run
...
def test_run_gateway_refuses_existing_process_before_importing_gateway_run(monkeypatch, capsys):
    ...
    _install_fake_gateway_run(monkeypatch, fake_start_gateway)
    _clear_supervisor_markers(monkeypatch)
    monkeypatch.setattr("opencodon.frontends.gateway.status.get_running_pid", lambda: 17907)
...
def test_s6_runtime_snapshot_reports_supervised_service(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("opencodon.frontends.cli.service_manager.detect_service_manager", lambda: "s6")
    monkeypatch.setattr("opencodon.frontends.cli.service_manager.get_service_manager", lambda: FakeS6Manager())
...
def test_reap_unsupervised_orphans_sigterms_then_sigkills_survivor(monkeypatch):
    ...
    monkeypatch.setattr("opencodon.frontends.gateway.status.write_planned_stop_marker", lambda pid: True)
...
class TestWaitForGatewayExit:
    ...
    def test_returns_when_process_exits_gracefully(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.frontends.gateway.status.get_running_pid", mock_get_running_pid)
    ...
    def test_handles_process_already_gone_on_kill(self, monkeypatch):
        ...
        monkeypatch.setattr("time.monotonic", fake_monotonic)
        monkeypatch.setattr("time.sleep", lambda _: None)
        monkeypatch.setattr("opencodon.frontends.gateway.status.get_running_pid", lambda: 99)
        monkeypatch.setattr(gateway, "terminate_pid", mock_terminate)
...
class TestStopProfileGateway:
    def test_stop_profile_gateway_keeps_pid_file_when_process_still_running(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.frontends.gateway.status.get_running_pid", lambda: 12345)
        ...
        monkeypatch.setattr("time.sleep", lambda _: None)
        monkeypatch.setattr(
            "opencodon.frontends.gateway.status.remove_pid_file",
...
def test_module_has_logger():
    ...
    assert gateway.logger.name == "opencodon.frontends.cli.gateway"
