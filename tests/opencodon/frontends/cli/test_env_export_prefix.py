...
def test_has_any_provider_configured_with_export_prefix(tmp_path, monkeypatch):
    ...
    import opencodon.frontends.cli.main as hmain

    importlib.reload(hmain)
    # get_env_path() derives from OPENCODON_HOME (set above) → tmp_path/.env, so
    # no patching is needed. Re-clear os.environ provider keys that
    # load_opencodon_dotenv may have populated at import/reload time, forcing the
