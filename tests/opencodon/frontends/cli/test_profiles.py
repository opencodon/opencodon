"""Comprehensive tests for opencodon_cli.profiles module.
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("opencodon.frontends.cli.profiles._cleanup_gateway_service"):
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("opencodon.frontends.cli.profiles._cleanup_gateway_service"), \
             patch("opencodon.frontends.cli.profiles.time.sleep"), \
             patch("opencodon.frontends.cli.profiles.shutil.rmtree", side_effect=PermissionError("locked")):
    ...
    def test_stops_profile_bound_backends_before_removal(self, profile_env):
        ...
        with patch("opencodon.frontends.cli.profiles._cleanup_gateway_service"), \
             patch("opencodon.frontends.cli.profiles._profile_bound_backend_pids", return_value=[4242]) as pids, \
             patch("opencodon.frontends.gateway.status.terminate_pid") as terminate, \
    ...
    def test_rmtree_retries_transient_enotempty_then_succeeds(self, profile_env):
        ...
        with patch("opencodon.frontends.cli.profiles._cleanup_gateway_service"), \
             patch("opencodon.frontends.cli.profiles._profile_bound_backend_pids", return_value=[]), \
             patch("opencodon.frontends.cli.profiles.time.sleep"), \
             patch("opencodon.frontends.cli.profiles.shutil.rmtree", side_effect=flaky_rmtree):
            delete_profile("coder", yes=True)
    ...
    def test_backend_scan_only_matches_this_profile(self, profile_env, monkeypatch):
        ...
        procs = [
            ...
            FakeProc(101, ["python", "-m", "opencodon.frontends.cli.main", "--profile", "coder", "serve"]),
            # Interactive chat for coder → NOT a backend subcommand, skipped.
            FakeProc(102, ["python", "-m", "opencodon.frontends.cli.main", "--profile", "coder", "chat"]),
            # Backend for a different profile → skipped.
            FakeProc(103, ["python", "-m", "opencodon.frontends.cli.main", "--profile", "other", "serve"]),
            # This very process → skipped even if it matched.
            FakeProc(self_pid, ["python", "-m", "opencodon.frontends.cli.main", "--profile", "coder", "serve"]),
        ]
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        monkeypatch.setattr("opencodon.frontends.cli.profiles.shutil.which", lambda name: "/opt/opencodon/bin/opencodon")
        from opencodon.frontends.cli.profiles import create_wrapper_script
    ...
    def test_remove_finds_bat_on_windows(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "win32")
        from opencodon.frontends.cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
    ...
    def test_remove_returns_false_when_absent(self, profile_env):
        from opencodon.frontends.cli.profiles import remove_wrapper_script
...
class TestWrapperScriptSecurity:
    ...
    def test_legit_alias_stays_inside_wrapper_dir(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from opencodon.frontends.cli.profiles import _get_wrapper_dir
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from opencodon.frontends.cli.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_no_alias_returns_none(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from opencodon.frontends.cli.profiles import find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr("sys.platform", "darwin")
        from opencodon.frontends.cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        with patch("opencodon.frontends.cli.profiles.check_alias_collision", return_value="skip"):
...
class TestInternalHelpers:
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        ...
        from opencodon.frontends.cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OPENCODON_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from opencodon.frontends.cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".opencodon"

        with patch("opencodon.frontends.gateway.status.get_running_pid", return_value=99999) as mock_get_running_pid:
    ...
    def test_gateway_running_check_plain_pid(self, profile_env):
        ...
        with patch("opencodon.frontends.gateway.status.get_running_pid", return_value=None) as mock_get_running_pid:
    ...
    def test_gateway_running_check_falls_back_to_runtime_state(self, profile_env):
        ...
        """
        import os
        import opencodon.frontends.gateway.status as gw_status
        from opencodon.frontends.cli.profiles import _check_gateway_running
        ...
        with patch("opencodon.frontends.gateway.status.get_running_pid", return_value=None), patch(
            "opencodon.frontends.gateway.status._read_process_cmdline",
    ...
    def test_gateway_running_check_runtime_state_stopped(self, profile_env):
        ...
        with patch("opencodon.frontends.gateway.status.get_running_pid", return_value=None):
            assert _check_gateway_running(default_home) is False
    ...
    def test_gateway_running_check_rejects_pid_reused_by_other_profile(self, profile_env):
        ...
        with patch("opencodon.frontends.gateway.status.get_running_pid", return_value=None), patch(
            "opencodon.frontends.gateway.status._pid_exists", return_value=True
        ), patch("opencodon.frontends.gateway.status._get_process_start_time", return_value=None), patch(
            "opencodon.frontends.gateway.status._read_process_cmdline",
            return_value="opencodon gateway run --replace",
        ):
    ...
    def test_gateway_running_check_detects_matching_named_profile(self, profile_env):
        ...
        with patch("opencodon.frontends.gateway.status.get_running_pid", return_value=None), patch(
            "opencodon.frontends.gateway.status._pid_exists", return_value=True
        ), patch("opencodon.frontends.gateway.status._get_process_start_time", return_value=1000), patch(
            "opencodon.frontends.gateway.status._read_process_cmdline",
