...
from opencodon.frontends.cli.webhook import (
    webhook_command,
...
def _isolate(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.frontends.cli.webhook._is_webhook_enabled", lambda: True
    )
...
def test_webhook_base_url_maps_wildcard_hosts_to_localhost(monkeypatch, host):
    monkeypatch.setattr(
        "opencodon.frontends.cli.webhook._get_webhook_config",
...
class TestWebhookEnabledGate:
    def test_blocks_when_disabled(self, capsys, monkeypatch):
        monkeypatch.setattr("opencodon.frontends.cli.webhook._is_webhook_enabled", lambda: False)
    ...
    def test_real_check_disabled(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "opencodon.frontends.cli.webhook._is_webhook_enabled",
            lambda: bool({}.get("enabled")),
        )
        import opencodon.frontends.cli.webhook as wh_mod
