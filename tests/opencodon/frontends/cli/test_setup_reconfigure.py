...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("opencodon.frontends.cli.setup.ensure_opencodon_home", {}),
        ("opencodon.frontends.cli.setup.is_interactive_stdin", {"return_value": True}),
        ...
        ("opencodon.frontends.cli.auth.get_active_provider", {"return_value": "openrouter"}),
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("opencodon.frontends.cli.setup.load_config", {"return_value": {}}),
        ("opencodon.frontends.cli.setup.save_config", {}),
        ("opencodon.frontends.cli.auth.get_active_provider", {"return_value": None}),
        ("opencodon.frontends.cli.setup.get_env_value", {"return_value": None}),
    ]:
        stack.enter_context(patch(target, **kwargs))
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="opencodon.frontends.cli.setup.prompt_choice",
                quick="opencodon.frontends.cli.setup._run_quick_setup",
                model="opencodon.frontends.cli.setup.setup_model_provider",
                terminal="opencodon.frontends.cli.setup.setup_terminal_backend",
                agent="opencodon.frontends.cli.setup.setup_agent_settings",
                gateway="opencodon.frontends.cli.setup.setup_gateway",
                tools="opencodon.frontends.cli.setup.setup_tools",
            )
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("opencodon.frontends.cli.setup.prompt_choice", {"return_value": 0}),
                first="opencodon.frontends.cli.setup._run_first_time_quick_setup",
            )
            from opencodon.frontends.cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        import sys
        from opencodon.frontends.cli.main import main

        captured = {}
        monkeypatch.setattr(
            "opencodon.frontends.cli.setup.run_setup_wizard",
