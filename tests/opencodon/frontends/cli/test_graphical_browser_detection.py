"""Tests for `_can_open_graphical_browser()` in opencodon_cli.auth.
...
from opencodon.frontends.cli.auth import _can_open_graphical_browser
...
def _force_platform_linux(monkeypatch):
    monkeypatch.setattr("opencodon.frontends.cli.auth.sys.platform", "linux")
...
def test_non_linux_with_gui_allows(monkeypatch):
    ...
    monkeypatch.setattr("opencodon.frontends.cli.auth.sys.platform", "darwin")
