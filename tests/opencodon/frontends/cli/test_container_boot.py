"""Tests for opencodon_cli.container_boot — the cont-init.d-time
reconciliation that recreates per-profile gateway s6 service slots
from the persistent profiles directory.

These tests run against a fake $OPENCODON_HOME under tmp_path; no real
...
from opencodon.frontends.cli.container_boot import (
    ReconcileAction,
...
def _hermetic_container_argv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setattr(
        "opencodon.frontends.cli.container_boot._read_container_argv",
        lambda: (),
    )
...
def _make_profile(
    ...
    *,
    ...
    """Create a fake profile directory under opencodon_home/profiles/<name>/."""
    p = opencodon_home / "profiles" / name
...
def _seed_default_root(
    ...
    *,
    ...
    OPENCODON_HOME root (the implicit default profile)."""
    if state is not None:
        (opencodon_home / "gateway_state.json").write_text(json.dumps({
            "gateway_state": state, "timestamp": 1234567890,
        }))
    if with_pid:
        (opencodon_home / "gateway.pid").write_text(json.dumps(
            {"pid": 99999, "host": "old-container"},
        ))
        (opencodon_home / "processes.json").write_text("[]")
...
def test_reconcile_log_rotates_when_size_exceeded(
    ...
    from opencodon.frontends.cli import container_boot
...
def test_dry_run_makes_no_filesystem_changes(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        opencodon_home=tmp_path, scandir=scandir, dry_run=True,
    )
...
def test_missing_profiles_root_still_registers_default_slot(
    ...
    """When $OPENCODON_HOME/profiles doesn't exist (fresh install), the
...
def test_default_slot_always_registered_on_empty_home(tmp_path: Path) -> None:
    """Bare OPENCODON_HOME with nothing under it still produces a
...
def test_default_slot_autostarts_when_root_state_running(tmp_path: Path) -> None:
    """gateway_state.json at the OPENCODON_HOME root with state=running
...
def test_legacy_gateway_run_env_no_supervise_does_not_seed_s6_state(
    ...
    scandir = tmp_path / "run-service"; scandir.mkdir()
    monkeypatch.setenv("OPENCODON_GATEWAY_NO_SUPERVISE", "1")
...
def test_default_slot_cleans_up_stale_runtime_files_at_root(
    ...
    """gateway.pid and processes.json at the OPENCODON_HOME root (left
...
def test_is_dashboard_container_true_for_dashboard_argv(
    ...
    from opencodon.frontends.cli.container_boot import _is_dashboard_container
...
def test_main_ignores_removed_skip_reconcile_env_var(
    ...
    """The legacy OPENCODON_SKIP_PROFILE_RECONCILE flag is gone: setting it on a
    ...
    monkeypatch.setenv("OPENCODON_HOME", str(tmp_path))
    monkeypatch.setenv("S6_PROFILE_GATEWAY_SCANDIR", str(scandir))
    monkeypatch.setenv("OPENCODON_SKIP_PROFILE_RECONCILE", "1")
    monkeypatch.setattr(
        container_boot,
        "_read_container_argv",
        lambda: ("/init", "/opt/opencodon/docker/main-wrapper.sh", "gateway", "run"),
