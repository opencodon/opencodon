...
from opencodon.frontends.cli.main, these tests mock the delegation point and verify
...
from opencodon.frontends.cli.setup import _print_setup_summary, setup_model_provider
...
def _stub_tts(monkeypatch):
    monkeypatch.setattr("opencodon.frontends.cli.setup.prompt_choice", lambda q, c, d=0: (
        _maybe_keep_current_tts(q, c) if _maybe_keep_current_tts(q, c) is not None
        else d
    ))
    monkeypatch.setattr("opencodon.frontends.cli.setup.prompt_yes_no", lambda *a, **kw: False)
...
def test_setup_model_provider_preserves_auxiliary_choices_written_by_picker(tmp_path, monkeypatch):
    """Aux choices made inside opencodon setup must survive the wizard's final save."""
    ...
    monkeypatch.setattr("opencodon.frontends.cli.main.select_provider_and_model", fake_select)
...
def test_setup_copilot_acp_skips_same_provider_pool_step(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("opencodon.core.auth.get_active_provider", lambda: None)
...
def test_setup_summary_marks_anthropic_auth_as_vision_available(tmp_path, monkeypatch, capsys):
    ...
    assert "missing run 'opencodon setup' to configure" not in output
...
def test_setup_summary_local_browser_unavailable_without_chromium(
    ...
    real availability gate so the surface stays aligned with
    ...
    monkeypatch.setattr("opencodon.frontends.cli.tools_config._has_agent_browser", lambda: True)
