...
def test_openrouter_models_replaced_with_live_catalog(monkeypatch):
    ...
    monkeypatch.setattr(model_switch, "list_authenticated_providers",
                        lambda **kw: list(base))
    monkeypatch.setattr("opencodon.core.providers.models.fetch_openrouter_models",
...
def test_openrouter_falls_back_to_base_models_on_fetch_failure(monkeypatch):
    ...
    monkeypatch.setattr(model_switch, "list_authenticated_providers",
                        lambda **kw: list(base))
    monkeypatch.setattr("opencodon.core.providers.models.fetch_openrouter_models", _raise)
...
def test_include_moa_adds_virtual_provider_with_named_presets(monkeypatch):
    ...
    monkeypatch.setattr(model_switch, "list_authenticated_providers",
                        lambda **kw: list(base))
    monkeypatch.setattr("opencodon.config.load_config", lambda: moa_config)
    monkeypatch.setattr("opencodon.core.providers.models.fetch_openrouter_models",
                        lambda *a, **kw: pytest.fail("should not be called"))
...
def test_current_custom_endpoint_passthrough_marks_current_row(monkeypatch):
    ...
    monkeypatch.setattr("opencodon.core.providers.models_dev.PROVIDER_TO_MODELS_DEV", {})
...
def _stub_kimi_discovery(monkeypatch, *, canonical):
    ...
    """
    import opencodon.core.providers.models_dev as md
    import opencodon.core.providers.models as hm
