"""Tests for opencodon_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import opencodon.frontends.cli.gui_uninstall as gu


def _make_agent(opencodon_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = opencodon_home / "opencodon"
    (agent_root / "opencodon_cli").mkdir(parents=True)
    (agent_root / "opencodon_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(opencodon_home: Path) -> None:
    """Create the source-built GUI artifacts a `opencodon desktop` run produces."""
    desktop = opencodon_home / "opencodon" / "apps" / "desktop"
    ...
    (opencodon_home / "opencodon" / "node_modules").mkdir(parents=True)
    (opencodon_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(opencodon_home: Path) -> None:
    (opencodon_home / "config.yaml").write_text("x: 1\n")
    (opencodon_home / ".env").write_text("KEY=secret\n")
    (opencodon_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    opencodon_home = tmp_path / ".opencodon"
    opencodon_home.mkdir()
    assert gu.agent_is_installed(opencodon_home) is False
    _make_agent(opencodon_home)
    assert gu.agent_is_installed(opencodon_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    opencodon_home = tmp_path / ".opencodon"
    (opencodon_home / "opencodon" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(opencodon_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    opencodon_home = tmp_path / ".opencodon"
    _make_gui_build(opencodon_home)
    artifacts = gu.source_built_gui_artifacts(opencodon_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(opencodon_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(opencodon_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    opencodon_home = tmp_path / ".opencodon"
    agent_root = _make_agent(opencodon_home)
    _make_gui_build(opencodon_home)
    _make_user_data(opencodon_home)
    ...
    removed = gu.uninstall_gui(opencodon_home)
    ...
    assert not (opencodon_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "opencodon_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (opencodon_home / "config.yaml").exists()
    assert (opencodon_home / ".env").exists()
    assert (opencodon_home / "sessions").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    opencodon_home = tmp_path / ".opencodon"
    _make_agent(opencodon_home)
    userdata = tmp_path / "opencodon-userdata"
    userdata.mkdir()
    ...
    gu.uninstall_gui(opencodon_home)
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(opencodon_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(opencodon_home)
    ...
    assert summary["opencodon_home"] == str(opencodon_home)
    assert summary["platform"] == sys.platform


def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "opencodon"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "opencodon"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "opencodon"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "opencodon"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    OPENCODON_HOME with all the destructive externals stubbed out.
    """
    import opencodon.frontends.cli.uninstall as uninstall
    ...
    from opencodon.frontends.cli import gui_uninstall as gu_mod
...
def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole OPENCODON_HOME non-interactively."""
    import opencodon.frontends.cli.uninstall as uninstall

    opencodon_home = tmp_path / ".opencodon"
    (opencodon_home / "opencodon" / "opencodon_cli").mkdir(parents=True)
    (opencodon_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not opencodon_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m opencodon_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_opencodon_home", lambda: opencodon_home)
    ...
    assert not (desktop / "release").exists()
    assert not (opencodon_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "opencodon_cli").exists()
    assert (opencodon_home / "config.yaml").exists()
