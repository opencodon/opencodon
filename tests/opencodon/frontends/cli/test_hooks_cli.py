"""Tests for the ``opencodon hooks`` CLI subcommand."""
...
from opencodon.frontends.cli import hooks as hooks_cli
...
class TestHooksTest:
    def test_synthetic_payload_matches_production_shape(self, tmp_path):
        """`opencodon hooks test` must feed the script stdin in the same
        shape invoke_hook() would at runtime.  Prior to this fix,
        run_once bypassed _serialize_payload and the two paths diverged —
        scripts tested with `opencodon hooks test` saw different top-level
...
class TestHooksDoctor:
    ...
    def test_flags_mtime_drift(self, tmp_path, monkeypatch):
        ...
        from opencodon.core.shell_hooks import allowlist_path
    ...
    def test_unallowlisted_script_is_not_executed(self, tmp_path):
        """Regression for M4: `opencodon hooks doctor` used to run every
