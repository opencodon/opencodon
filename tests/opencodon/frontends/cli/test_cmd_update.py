...
from opencodon.frontends.cli.main import cmd_update, PROJECT_ROOT
...
class TestCmdUpdateNpmLockfileCache:
    @staticmethod
    def _cache_file(opencodon_root, project_root):
        cache_key = hashlib.sha256(str(project_root).encode()).hexdigest()[:12]
        return opencodon_root / f".npm_lock_hash_{cache_key}"

    def test_npm_lockfile_changed_no_cache(self, tmp_path, monkeypatch):
        from opencodon.frontends.cli import main as hm
    ...
    def test_package_json_only_edit_defeats_skip(self, tmp_path, monkeypatch):
        ...
        npm — lockfile unchanged. `opencodon update` must still install (the
    ...
    def test_update_uses_one_shared_npm_cache_across_profiles(
        ...
        shared_root = tmp_path / ".opencodon"
        named_profile = shared_root / "profiles" / "work"
        named_profile.mkdir(parents=True)

        monkeypatch.setattr(hm, "PROJECT_ROOT", checkout)
        monkeypatch.setattr(opencodon_constants.Path, "home", lambda: tmp_path)
        monkeypatch.setattr(
            opencodon_constants, "find_node_executable", lambda _name: "/usr/bin/npm"
        )
        ...
        with patch.object(
            ...
            monkeypatch.setenv("OPENCODON_HOME", str(shared_root))
            hm._update_node_dependencies()

            monkeypatch.setenv("OPENCODON_HOME", str(named_profile))
            hm._update_node_dependencies()
...
class TestCmdUpdateTermuxUvBootstrap:
    ...
    def test_termux_reuses_existing_path_uv_without_pip(self, mock_run, monkeypatch):
        ...
        monkeypatch.setattr(hm, "_is_termux_env", lambda env=None: True)
        # Production resolve_uv only checks $OPENCODON_HOME/bin/uv; model an empty
        # managed dir so the PATH probe is what surfaces the packaged uv.
        monkeypatch.setattr("opencodon.frontends.cli.managed_uv.resolve_uv", lambda: None)
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind opencodon/opencodon silently misses updates.
        """
        from opencodon.frontends.cli import main as hm
        ...
        with patch.object(
            hm,
            "_get_origin_url",
            return_value="https://github.com/example/opencodon.git",
    ...
    def test_update_refreshes_repo_and_tui_node_dependencies(
        ...
        # install covers all workspaces.  The apps/web ci call runs from the
        # workspace root too (an ancestor of web_dir) when the root lockfile
        # exists.
        #
        ...
        if len(npm_calls) > 2:
            # The apps/web install runs from the workspace root when the root
            # lockfile exists (npm workspaces hoist node_modules upward).
            assert npm_calls[2:] == [
                (
                    ["/usr/bin/npm", "ci", "--include=dev", "--workspace", "@opencodon/web", "--silent"],
                    PROJECT_ROOT,
                ),
            ]
        ...
        assert idle_kwargs["cwd"] == PROJECT_ROOT / "apps" / "web"
    ...
    def test_update_non_interactive_runs_config_reconcile(self, mock_args, capsys):
        ...
        ) as mock_reconcile, patch("opencodon.frontends.cli.main.sys") as mock_sys:
...
class TestCmdUpdateConfigPrompt:
    ...
    def test_missing_keys_are_listed_by_name_before_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "opencodon.config.reconcile_config",
            return_value={"env_added": [], "config_added": [], "warnings": []},
        ), patch("opencodon.frontends.cli.main.sys") as mock_sys:
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.opencodon"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.opencodon/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.opencodon/profiles/work"))
...
class TestCmdUpdateBranchFlag:
    """``opencodon update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``opencodon update --check --branch <name>`` honors the branch override.
...
class TestCmdUpdateZipBranchRefusal:
    """``opencodon update --branch=<non-main>`` must refuse on the ZIP fallback path.
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from opencodon.frontends.cli.main import _update_via_zip
...
class TestNodeRuntimeNpmResolution:
    ...
    def test_wsl_windows_only_npm_flags_skip(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr(opencodon_constants, "is_wsl", lambda: True)
        monkeypatch.setattr(
