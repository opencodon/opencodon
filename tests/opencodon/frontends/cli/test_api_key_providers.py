...
from opencodon.frontends.cli.copilot_auth import _try_gh_cli_token
...
from opencodon.frontends.cli.auth import PROVIDER_REGISTRY as _REGISTRY
...
def _clear_provider_env(monkeypatch):
    ...
    monkeypatch.setattr("opencodon.frontends.cli.auth._load_auth_store", lambda: {})
...
class TestApiKeyProviderStatus:
    ...
    def test_copilot_status_uses_gh_cli_token(self, monkeypatch):
        monkeypatch.setattr("opencodon.frontends.cli.copilot_auth._try_gh_cli_token", lambda: "gho_gh_cli_token")
    ...
    def test_copilot_acp_status_detects_local_cli(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.frontends.cli.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
    ...
    def test_get_auth_status_dispatches_to_external_process(self, monkeypatch):
        monkeypatch.setattr("opencodon.frontends.cli.auth.shutil.which", lambda command: f"/opt/bin/{command}")
    ...
    def test_non_api_key_provider(self):
        status = get_api_key_provider_status("openai-codex")
        assert status["configured"] is False
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_resolve_zai_with_key(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.frontends.cli.auth.detect_zai_endpoint", lambda *a, **kw: None)
    ...
    def test_resolve_copilot_with_gh_cli_fallback(self, monkeypatch):
        monkeypatch.setattr("opencodon.frontends.cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        creds = resolve_api_key_provider_credentials("copilot")
        assert creds["provider"] == "copilot"
    ...
    def test_try_gh_cli_token_uses_homebrew_path_when_not_on_path(self, monkeypatch):
        monkeypatch.setattr("opencodon.frontends.cli.copilot_auth.shutil.which", lambda command: None)
        monkeypatch.setattr(
            "opencodon.frontends.cli.copilot_auth.os.path.isfile",
            lambda path: path == "/opt/homebrew/bin/gh",
        )
        monkeypatch.setattr(
            "opencodon.frontends.cli.copilot_auth.os.access",
        ...
        monkeypatch.setattr("opencodon.frontends.cli.copilot_auth.subprocess.run", _fake_run)
    ...
    def test_resolve_invalid_provider_raises(self):
        with pytest.raises(AuthError):
            resolve_api_key_provider_credentials("openai-codex")
...
class TestRuntimeProviderResolution:
    ...
    def test_runtime_copilot_uses_responses_for_gpt_5_4(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "opencodon.frontends.cli.models.fetch_github_model_catalog",
...
class TestHasAnyProviderConfigured:
    ...
    def test_claude_code_creds_ignored_on_fresh_install(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(config_module, "get_env_path", lambda: opencodon_home / ".env")
        monkeypatch.setattr(config_module, "get_opencodon_home", lambda: opencodon_home)
        monkeypatch.setattr("opencodon.frontends.cli.copilot_auth.resolve_copilot_token", lambda: ("", ""))
        ...
        monkeypatch.setattr("opencodon.frontends.cli.auth.get_auth_status", lambda _pid: {})
...
class TestZaiEndpointAutoDetect:
    ...
    def test_probe_success_returns_detected_url(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "opencodon.frontends.cli.auth.detect_zai_endpoint",
            lambda *a, **kw: {
                "id": "coding-global",
    ...
    def test_env_override_skips_probe(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.frontends.cli.auth.detect_zai_endpoint", _never_called)
        creds = resolve_api_key_provider_credentials("zai")
...
class TestKimiMoonshotModelListIsolation:
    ...
    def test_moonshot_list_excludes_coding_plan_only_models(self):
        from opencodon.frontends.cli.main import _PROVIDER_MODELS
...
class TestHuggingFaceModels:
    ...
    def test_model_lists_match(self):
        ...
        from opencodon.frontends.cli.models import _PROVIDER_MODELS as models_models
    ...
    def test_provider_aliases_in_models_py(self):
        from opencodon.frontends.cli.models import _PROVIDER_ALIASES
        assert _PROVIDER_ALIASES.get("hf") == "huggingface"
        assert _PROVIDER_ALIASES.get("hugging-face") == "huggingface"

    def test_provider_label(self):
        from opencodon.frontends.cli.models import _PROVIDER_LABELS
...
class TestFetchDeepInfraModels:
    ...
    def test_returns_filtered_models_on_success(self, monkeypatch):
        ...
        from opencodon.frontends.cli.models import _fetch_deepinfra_models
...
class TestDeepInfraTagFiltering:
    ...
    def test_filters_by_surface_tag_and_handles_rollout_states(self, monkeypatch):
        ...
        import opencodon.frontends.cli.models as _m
    ...
    def test_returns_none_on_network_failure(self, monkeypatch):
        ...
        from opencodon.frontends.cli.models import _fetch_deepinfra_models_by_tag, _fetch_deepinfra_pricing
...
class TestDeepInfraProviderProfile:
    ...
    def test_profile_registered_with_alias_and_aux(self):
        ...
        from opencodon.frontends.cli.auth import PROVIDER_REGISTRY, resolve_provider
