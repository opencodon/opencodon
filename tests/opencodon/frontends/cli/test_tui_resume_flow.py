...
def main_mod(monkeypatch):
    import opencodon.frontends.cli.main as mod
...
def test_termux_fast_cli_launch_bare_defers_agent_startup(monkeypatch, main_mod):
    ...
    assert os.environ["OPENCODON_DEFER_AGENT_STARTUP"] == "1"
    assert os.environ["OPENCODON_FAST_STARTUP_BANNER"] == "1"
...
def test_termux_fast_cli_launch_oneshot_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setitem(
        sys.modules,
        "opencodon.frontends.cli.oneshot",
        types.SimpleNamespace(
...
def test_termux_bundled_skills_stamp_controls_sync(monkeypatch, tmp_path, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setattr(main_mod, "get_opencodon_home", lambda: tmp_path)
    ...
    monkeypatch.setenv("OPENCODON_TERMUX_FORCE_SKILLS_SYNC", "1")
    assert main_mod._termux_bundled_skills_sync_needed() is True
...
def test_run_and_exit_oneshot_hard_exits_when_oneshot_import_fails(
    ...
    def fake_import(name, globals=None, locals=None, fromlist=(), level=0):
        if name == "opencodon.frontends.cli.oneshot" and "run_oneshot" in (fromlist or ()):
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        """
        import opencodon.frontends.cli.oneshot as oneshot
        from opencodon.frontends.cli.main import _exit_after_oneshot
...
def test_main_oneshot_path_bypasses_late_atexit_abort():
    ...
    program = textwrap.dedent(
        ...
        sys.modules["opencodon.frontends.cli.oneshot"] = types.SimpleNamespace(
...
def test_oneshot_run_agent_closes_agent_after_chat(monkeypatch):
    import opencodon.frontends.cli.oneshot as oneshot_mod
...
def test_oneshot_exit_code_when_failed_without_response(monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.frontends.cli.oneshot._run_agent",
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from opencodon.frontends.cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    ...
    from opencodon.frontends.cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(
        sys.modules,
        "opencodon.frontends.cli.models",
        mod("opencodon.frontends.cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        sys.modules,
        "opencodon.frontends.cli.tools_config",
        mod("opencodon.frontends.cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["OPENCODON_TUI_ACTIVE_SESSION_FILE"])
...
def test_launch_tui_worktree_validates_relative_python_against_final_cwd(
    ...
    monkeypatch.setenv("OPENCODON_CWD", str(parent_cwd))
    ...
    assert captured["env"]["OPENCODON_CWD"] == str(worktree)
    assert captured["env"]["OPENCODON_PYTHON"] == str(relative_python)


def test_launch_tui_applies_terminal_backend_config(
    monkeypatch, main_mod, _isolate_opencodon_home
):
    captured = {}
...
def test_launch_tui_exit_code_42_relaunches_update(monkeypatch, main_mod):
    ...
    with patch("opencodon.frontends.cli.relaunch.relaunch") as mock_relaunch:
        with pytest.raises(SystemExit) as exc:
            main_mod._launch_tui()
...
def test_launch_tui_drops_stale_resume_env_without_resume_arg(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("OPENCODON_TUI_RESUME", "stale-missing-session")
    monkeypatch.setattr(
        main_mod,
        "_make_tui_argv",
        lambda tui_dir, tui_dev: (["node", "dist/entry.js"], Path(".")),
    ...
    assert "OPENCODON_TUI_RESUME" not in captured["env"]
...
def test_make_tui_argv_dev_prebuilds_opencodon_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("OPENCODON_TUI_DIR", raising=False)
