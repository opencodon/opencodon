"""ensure_opencodon_home is memoized per home path (perf: it runs on every
load_config), but a deleted home must still be recreated on the next call."""

import shutil

from opencodon.frontends.cli import config as cfg
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".opencodon"
    second = tmp_path / "b" / ".opencodon"

    monkeypatch.setenv("OPENCODON_HOME", str(first))
    cfg.ensure_opencodon_home()

    # Profile switch: OPENCODON_HOME moves → the new path is ensured too.
    monkeypatch.setenv("OPENCODON_HOME", str(second))
    cfg.ensure_opencodon_home()
