...
``acp_adapter/entry.py`` (via the shared ``opencodon_bootstrap.harden_import_path``
...
def test_sys_path_guard_runs_before_cli_import():
    ...
    ``opencodon_bootstrap.harden_import_path()`` call precedes ``import cli``."""
    src = (PROJECT_ROOT / "src" / "opencodon" / "frontends" / "tui" / "slash_worker.py").read_text()
    tree = ast.parse(src)
    ...
    for node in ast.walk(tree):
        ...
        is_cli_import = (
            isinstance(node, ast.Import) and any(a.name == "cli" for a in node.names)
        ) or (
            isinstance(node, ast.ImportFrom)
            and (node.module or "").startswith("opencodon.frontends.cli")
        )
        if is_cli_import:
    ...
    assert harden_call_line is not None, (
        "slash_worker.py must call opencodon_bootstrap.harden_import_path()"
    )
...
def test_guard_delegates_to_shared_helper_not_inline():
    ...
    assert '{"", "."}' not in src and "{'', '.'}" not in src, (
        "slash_worker.py should delegate to opencodon_bootstrap.harden_import_path, "
        "not re-implement the guard inline"
    )
    assert "opencodon_bootstrap.harden_import_path()" in src, (
        "slash_worker.py must call the shared opencodon_bootstrap.harden_import_path guard"
    )
