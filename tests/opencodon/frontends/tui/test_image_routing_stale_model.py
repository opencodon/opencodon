...
from opencodon.frontends.tui.server import _active_image_routing_identity
...
def test_missing_agent_identity_uses_runtime_fallback():
    ...
    with patch(
        "opencodon.core.auxiliary_client._read_main_provider", return_value="openai-codex"
    ), patch(
        "opencodon.core.auxiliary_client._read_main_model", return_value="gpt-5.5-codex"
    ):
        identity = _active_image_routing_identity(agent)
