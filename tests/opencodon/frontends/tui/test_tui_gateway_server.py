...
def test_voice_toggle_returns_configured_record_key(monkeypatch):
    ...
    # ``voice.toggle`` action=on mutates ``os.environ["OPENCODON_VOICE"]``
    ...
    # review on #19835).
    monkeypatch.setenv("OPENCODON_VOICE", "0")
...
def test_voice_record_start_handles_non_dict_voice_cfg(monkeypatch):
    ...
    monkeypatch.setenv("OPENCODON_VOICE", "1")
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("OPENCODON_VOICE", "1")
    # setenv (not delenv) — the handler writes OPENCODON_VOICE_TTS directly, and
    ...
    monkeypatch.setenv("OPENCODON_VOICE_TTS", "0")
...
def test_load_enabled_toolsets_prefers_tui_env(monkeypatch):
    monkeypatch.setenv("OPENCODON_TUI_TOOLSETS", "web, terminal, ,memory")
...
def test_load_enabled_toolsets_accepts_plugin_env_after_discovery(monkeypatch):
    monkeypatch.setenv("OPENCODON_TUI_TOOLSETS", "plugin_demo")
...
def test_load_enabled_toolsets_folds_project_into_focus_posture(monkeypatch):
    ...
    import opencodon.core.context.coding_context as cc
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    ...
    # Sorted: ["memory", "project", "science"]. `science` is auto-recovered
    # by _get_platform_tools (a non-configurable platform toolset in
    # opencodon-cli's universe); `project` is GUI-only, folded in by
...
def test_load_enabled_toolsets_all_env_means_all(monkeypatch):
    monkeypatch.setenv("OPENCODON_TUI_TOOLSETS", "all")
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("OPENCODON_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    ...
    assert server._load_enabled_toolsets() == ["web"]
    err = capsys.readouterr().err
    assert "ignoring unknown OPENCODON_TUI_TOOLSETS entries: nope" in err
    assert "ignoring disabled MCP servers" in err
    assert "mcp-off" in err
...
def test_resolve_model_uses_inference_model_env(monkeypatch):
    ...
    monkeypatch.setenv("OPENCODON_INFERENCE_MODEL", " anthropic/claude-sonnet-4.6\n")
...
def test_config_sync_config_wins_over_env_seed(monkeypatch):
    # Hosted instances set OPENCODON_INFERENCE_MODEL as a provision-time seed;
    # the per-turn sync must follow config.yaml edits, not stay pinned to it.
    monkeypatch.setenv("OPENCODON_INFERENCE_MODEL", "seed/model")
    monkeypatch.delenv("OPENCODON_MODEL", raising=False)
...
def test_config_sync_ignores_env_seed_without_config_model(monkeypatch):
    ...
    # permanently.
    monkeypatch.setenv("OPENCODON_MODEL", "one-shot/model")
    monkeypatch.setenv("OPENCODON_INFERENCE_MODEL", "one-shot/model")
    monkeypatch.setattr(
...
def test_config_model_target_never_reads_env(monkeypatch):
    monkeypatch.setenv("OPENCODON_MODEL", "seed/model")
    monkeypatch.setenv("OPENCODON_INFERENCE_MODEL", "seed/model")
...
def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.core.providers.models.detect_static_provider_for_model",
...
def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    monkeypatch.setenv("OPENCODON_MODEL", "sonnet")
    monkeypatch.delenv("OPENCODON_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("OPENCODON_INFERENCE_PROVIDER", raising=False)
    ...
    monkeypatch.setattr(
        "opencodon.core.providers.models.detect_static_provider_for_model", fake_detect
    )
...
def test_startup_runtime_resolves_short_alias_without_network(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "opencodon.core.providers.models.fetch_openrouter_models",
...
def test_config_set_yolo_global_scope_writes_approvals_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_opencodon_home", tmp_path)
...
def test_config_set_fast_updates_live_agent_session_scoped(monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.core.providers.models.resolve_fast_mode_overrides",
...
def test_config_set_yolo_process_scope_treats_false_like_env_as_disabled(monkeypatch):
    monkeypatch.setenv("OPENCODON_YOLO_MODE", "false")
    ...
    assert resp["result"]["value"] == "1"
    assert os.environ.get("OPENCODON_YOLO_MODE") == "1"
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    ...
    monkeypatch.delenv("OPENCODON_GATEWAY_SESSION", raising=False)
    monkeypatch.delenv("OPENCODON_INTERACTIVE", raising=False)

    server._enable_gateway_prompts()

    assert server.os.environ["OPENCODON_GATEWAY_SESSION"] == "1"
    assert server.os.environ["OPENCODON_EXEC_ASK"] == "1"
    assert server.os.environ["OPENCODON_INTERACTIVE"] == "1"
...
def test_config_set_model_explicit_provider_skips_broken_default_init(monkeypatch):
    ...
    monkeypatch.setattr("opencodon.core.providers.runtime_provider.resolve_runtime_provider", fake_runtime_provider)
...
def test_config_set_model_does_not_leak_inference_provider_env(monkeypatch):
    ...
    process; writing OPENCODON_INFERENCE_PROVIDER on a switch leaked the new
    ...
    try:
        ...
        assert os.environ["OPENCODON_INFERENCE_PROVIDER"] == "openrouter"
...
def test_config_set_model_records_per_session_override_not_env(monkeypatch):
    ...
    try:
        ...
        assert "OPENCODON_TUI_PROVIDER" not in os.environ
        assert "OPENCODON_INFERENCE_PROVIDER" not in os.environ
...
def test_config_set_model_switches_agent_without_touching_env(monkeypatch):
    ...
    a per-session override; it does NOT write OPENCODON_MODEL / OPENCODON_TUI_PROVIDER
    ...
    session = _session(agent=agent)
    server._sessions["sid"] = session
    monkeypatch.setenv("OPENCODON_TUI_PROVIDER", "openai-codex")
    monkeypatch.delenv("OPENCODON_MODEL", raising=False)
    monkeypatch.delenv("OPENCODON_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    try:
        ...
        assert os.environ["OPENCODON_TUI_PROVIDER"] == "openai-codex"
        assert "OPENCODON_MODEL" not in os.environ
        assert "OPENCODON_INFERENCE_MODEL" not in os.environ
    finally:
        server._sessions.clear()
...
def test_config_set_model_once_keeps_env_and_records_restore(monkeypatch):
    ...
    try:
        ...
        assert os.environ["OPENCODON_INFERENCE_PROVIDER"] == "openrouter"
        assert os.environ["OPENCODON_MODEL"] == "old/model"
    finally:
        server._sessions.clear()
...
def test_prompt_submit_expands_context_refs(monkeypatch):
    ...
    fake_ctx = types.ModuleType("opencodon.core.context.context_references")
    ...
    fake_meta = types.ModuleType("opencodon.core.providers.model_metadata")
    ...
    monkeypatch.setattr(server, "make_stream_renderer", lambda cols: None)
    monkeypatch.setattr(server, "render_message", lambda raw, cols: None)
    monkeypatch.setitem(sys.modules, "opencodon.core.context.context_references", fake_ctx)
    monkeypatch.setitem(sys.modules, "opencodon.core.providers.model_metadata", fake_meta)
...
def test_file_attach_uploads_remote_file_into_session_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".opencodon" / "desktop-attachments" / "report.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["path"] == str(stored)
        assert resp["result"]["ref_text"] == "@file:.opencodon/desktop-attachments/report.txt"
...
def test_file_attach_copies_gateway_visible_file_outside_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".opencodon" / "desktop-attachments" / "outside.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["ref_text"] == "@file:.opencodon/desktop-attachments/outside.txt"
...
def test_file_attach_uses_in_workspace_file_without_copying(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert not (workspace / ".opencodon" / "desktop-attachments").exists()
    finally:
        server._sessions.pop("sid", None)
...
def test_file_attach_quotes_ref_with_spaces(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert resp["result"]["attached"] is True
        assert resp["result"]["ref_text"] == "@file:`.opencodon/desktop-attachments/my exam schedule.csv`"
    finally:
        server._sessions.pop("sid", None)
...
def test_session_status_reads_live_gateway_agent(monkeypatch):
    ...
    assert "Opencodon TUI Status" in out
...
def test_skills_reload_runs_in_gateway_process(monkeypatch):
    import opencodon.core.skills.skill_commands as skill_commands
...
def test_get_db_degrades_cleanly_when_sessiondb_init_fails(monkeypatch):
    fake_mod = types.ModuleType("opencodon_state")
...
def test_session_delete_success_returns_deleted_id(monkeypatch):
    ...
    # too — not just the SQLite row.  The autouse _isolate_opencodon_home
    # fixture pins OPENCODON_HOME to a temp dir; the handler should append
    # /sessions to it.
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    with patch(
        ...
        # guarding against.
        with patch("opencodon.core.providers.models.provider_model_ids") as live_fetch:
...
def test_model_options_preserves_canonical_custom_row_after_agent_init(monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.core.providers.runtime_provider.canonical_custom_identity",
        canonical,
    )
    monkeypatch.setattr(
        "opencodon.frontends.cli.model_switch.list_authenticated_providers",
...
def test_model_save_key_uses_credential_lifecycle_and_picker_context(monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.core.credentials.auth.PROVIDER_REGISTRY",
        {
...
def test_verification_status_outside_workspace_is_not_applicable(monkeypatch, tmp_path):
    ...
    import opencodon.core.context.coding_context as coding_context
...
def test_reload_env_rpc_calls_opencodon_cli_reload_env(monkeypatch):
    """reload.env mirrors classic CLI's `/reload` — re-reads ~/.opencodon/.env
...
def test_tts_stream_begin_and_stop_lifecycle(monkeypatch):
    ...
    monkeypatch.setenv("OPENCODON_VOICE_TTS", "1")
    monkeypatch.setenv("OPENCODON_VOICE", "0")  # no barge-in monitor (no mic)
    playback_stops: list = []
