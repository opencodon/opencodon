...
from opencodon.frontends.tui.server import _finalize_session, _is_gateway_owned_source
...
class TestIsGatewayOwnedSource:
    ...
    def test_arbitrary_strings_are_not(self):
        assert _is_gateway_owned_source("opencodonbench-task-xyz") is False
...
class TestFinalizeSkipsGatewaySessions:
    @patch("opencodon.frontends.tui.server._get_db")
