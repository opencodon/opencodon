...
from opencodon.frontends.tui.compute_host import ComputeHost, _default_workers
from opencodon.frontends.tui.host_supervisor import (
    MUTATOR_ROUTE_TABLE,
    HostSupervisor,
    append_log_record,
)
...
def test_compute_host_workers_inherit_tui_pool_env_or_8(monkeypatch):
    monkeypatch.delenv("OPENCODON_TUI_RPC_POOL_WORKERS", raising=False)
    monkeypatch.delenv("OPENCODON_COMPUTE_HOST_WORKERS", raising=False)
    assert _default_workers() == 8

    monkeypatch.setenv("OPENCODON_TUI_RPC_POOL_WORKERS", "11")
    assert _default_workers() == 11

    # Dead-RC tombstone: malformed env falls back to 8, not the old except-branch 4.
    monkeypatch.setenv("OPENCODON_TUI_RPC_POOL_WORKERS", "not-an-int")
    assert _default_workers() == 8
...
def test_compute_host_compress_control_runs_identity_guard_in_host(monkeypatch):
    ...
    monkeypatch.setenv("OPENCODON_COMPUTE_HOST_CHILD", "1")
...
def test_supervisor_crash_emits_turn_error_and_respawns(tmp_path):
    ...
    script.write_text(
        """
import json, os, sys
print(json.dumps({'type':'hello','host_pid':os.getpid(),'boot_id':'boot-1','build_sha':'test','opencodon_home':os.environ.get('OPENCODON_HOME','')}), flush=True)
