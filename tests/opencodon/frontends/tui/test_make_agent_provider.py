...
def test_make_agent_passes_resolved_provider():
    ...
    with (
        patch("opencodon.frontends.tui.server._load_cfg", return_value=fake_cfg),
        patch("opencodon.frontends.tui.server._get_db", return_value=MagicMock()),
        patch("opencodon.frontends.tui.server._load_tool_progress_mode", return_value="compact"),
        patch("opencodon.frontends.tui.server._load_reasoning_config", return_value=None),
        patch("opencodon.frontends.tui.server._load_service_tier", return_value=None),
        patch("opencodon.frontends.tui.server._load_enabled_toolsets", return_value=None),
        patch(
        ...
        from opencodon.frontends.tui.server import _make_agent
...
def test_make_agent_honors_tui_launch_env_flags():
    ...
    with (
        patch.dict(
            os.environ,
            {
                "OPENCODON_TUI_MAX_TURNS": "7",
                "OPENCODON_TUI_CHECKPOINTS": "1",
                "OPENCODON_TUI_PASS_SESSION_ID": "1",
                "OPENCODON_IGNORE_RULES": "1",
            },
        ),
        patch("opencodon.frontends.tui.server._load_cfg", return_value=fake_cfg),
        patch("opencodon.frontends.tui.server._get_db", return_value=MagicMock()),
...
def test_probe_config_health_flags_null_sections():
    ...
    from opencodon.frontends.tui.server import _probe_config_health
...
def test_make_agent_tolerates_null_config_sections():
    """Bare `agent:` / `display:` keys in ~/.opencodon/config.yaml parse as
    ...
    with (
        patch("opencodon.frontends.tui.server._load_cfg", return_value=null_cfg),
        patch("opencodon.frontends.tui.server._get_db", return_value=MagicMock()),
        patch(
            "opencodon.frontends.cli.runtime_provider.resolve_runtime_provider",
            return_value=fake_runtime,
...
def test_make_agent_tolerates_null_personalities_with_active_personality():
    ...
    with (
        patch("opencodon.frontends.tui.server._load_cfg", return_value=cfg),
        patch("opencodon.frontends.tui.server._get_db", return_value=MagicMock()),
...
def test_apply_model_switch_does_not_leak_process_env():
    ...
    with (
        patch("opencodon.frontends.cli.model_switch.parse_model_flags",
              return_value=("glm-5.1", None, False, False, True)),
        patch("opencodon.frontends.cli.model_switch.resolve_persist_behavior",
              return_value=False),
        patch("opencodon.frontends.cli.model_switch.switch_model", return_value=_FakeResult()),
        patch("opencodon.frontends.tui.server._emit"),
        patch("opencodon.frontends.tui.server._restart_slash_worker"),
        patch("opencodon.frontends.tui.server._session_info", return_value={}),
        patch("opencodon.frontends.tui.server._persist_model_switch") as mock_persist,
    ):
