...
def server():
    with patch.dict("sys.modules", {
        "opencodon_constants": MagicMock(get_opencodon_home=MagicMock(return_value=__import__("pathlib").Path("/tmp/opencodon_test"))),
        "opencodon.config.env_loader": MagicMock(),
        "opencodon.frontends.cli.banner": MagicMock(),
        "opencodon.frontends.cli.banner": MagicMock(),
        "opencodon_state": MagicMock(),
        "opencodon.state": MagicMock(),
    }):
        import importlib
        mod = importlib.import_module("opencodon.frontends.tui.server")
        yield mod
...
def test_write_json_skips_flush_when_disable_flush_true(monkeypatch):
    ...
    transport_mod = importlib.import_module("opencodon.frontends.tui.transport")
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `OPENCODON_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("OPENCODON_TUI_GATEWAY_NO_FLUSH", "1")
    transport_mod = importlib.reload(importlib.import_module("opencodon.frontends.tui.transport"))

    try:
        assert transport_mod._DISABLE_FLUSH is True
    finally:
        # Restore the env-disabled state so other tests see the default.
        monkeypatch.delenv("OPENCODON_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_sync_session_key_after_compress_reanchors_active_session_lease(
    ...
    from opencodon.frontends.cli.active_sessions import (
...
def test_config_load_missing(server, tmp_path):
    server._opencodon_home = tmp_path
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    with patch("opencodon.core.skills.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
...
def test_slash_exec_routes_custom_skill_bundle_away_from_worker(server):
    ...
    with patch("opencodon.core.skills.skill_bundles.get_skill_bundles", return_value=fake_bundles), \
         patch(
             "opencodon.core.skills.skill_bundles.build_bundle_invocation_message",
...
def test_command_dispatch_builtin_queue_wins_over_colliding_bundle(server):
    ...
    with patch("opencodon.core.skills.skill_bundles.get_skill_bundles", return_value=fake_bundles), \
         patch("opencodon.core.skills.skill_bundles.build_bundle_invocation_message") as build_bundle:
        resp = server.handle_request({
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("opencodon.core.skills.skill_commands.scan_skill_commands", return_value=fake_skills), \
         patch("opencodon.core.skills.skill_commands.build_skill_invocation_message", return_value=fake_msg):
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
...
def test_command_dispatch_returns_custom_bundle_payload(server):
    ...
    with patch("opencodon.core.skills.skill_bundles.get_skill_bundles", return_value=fake_bundles), \
         ...
         patch("opencodon.core.skills.skill_commands.build_skill_invocation_message") as build_skill, \
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    ...
    import opencodon.frontends.cli.skin_engine as skin_engine

    (tmp_path / "skins").mkdir()
    (tmp_path / "skins" / "midnight.yaml").write_text(
        "name: midnight\ndescription: t\ncolors:\n  banner_title: '#00ffcc'\n  background: '#001010'\n"
    )
    monkeypatch.setattr(skin_engine, "get_opencodon_home", lambda: tmp_path)
