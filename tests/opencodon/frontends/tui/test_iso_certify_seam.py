...
seam: dead unless ``OPENCODON_ISO_CERTIFY_SYNTH_TURN=1``. These tests pin (a) the
...
from opencodon.frontends.tui.synthetic_turn import (
    SyntheticHeavyAgent,
...
def test_synth_seam_dead_when_env_unset(monkeypatch):
    monkeypatch.delenv("OPENCODON_ISO_CERTIFY_SYNTH_TURN", raising=False)
...
def test_synth_seam_armed_builds_agent(monkeypatch):
    monkeypatch.setenv("OPENCODON_ISO_CERTIFY_SYNTH_TURN", "1")
...
def test_synth_turn_non_json_prompt_uses_defaults(monkeypatch):
    monkeypatch.setenv("OPENCODON_ISO_CERTIFY_DURATION_S", "0.2")
