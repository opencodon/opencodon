    ...
    with patch.dict(
        "sys.modules",
        {
            "opencodon_constants": MagicMock(
                get_opencodon_home=MagicMock(return_value="/tmp/opencodon_test_compaction")
            ),
        ...
        yield importlib.import_module("opencodon.frontends.tui.server")
...
def test_compaction_lifecycle_is_retagged(server, monkeypatch):
    from opencodon.core.context.conversation_compression import COMPACTION_STATUS
...
def test_compaction_completion_status_is_preserved(server, monkeypatch):
    from opencodon.core.context.conversation_compression import COMPACTION_DONE_STATUS
