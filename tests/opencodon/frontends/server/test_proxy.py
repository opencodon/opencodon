"""Tests for the `opencodon proxy` subcommand and its upstream adapters."""
...
from opencodon.frontends.server.proxy.adapters import ADAPTERS, get_adapter
from opencodon.frontends.server.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from opencodon.frontends.server.proxy.adapters.xai import XAIGrokAdapter
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic OPENCODON_HOME."""
...
def test_xai_adapter_retry_refreshes_current_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("opencodon.core.credentials.auth.refresh_xai_oauth_pure", fake_refresh)
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("opencodon.core.credentials.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
from opencodon.frontends.server.proxy.server import create_app  # noqa: E402
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    monkeypatch.setenv("OPENCODON_HOME", str(tmp_path))
    from opencodon.frontends.server.proxy.cli import cmd_proxy_status
...
def test_cmd_proxy_providers_runs(capsys):
    from opencodon.frontends.server.proxy.cli import cmd_proxy_list_providers
...
def test_cmd_proxy_start_refuses_unknown_provider(capsys):
    from opencodon.frontends.server.proxy.cli import cmd_proxy_start
