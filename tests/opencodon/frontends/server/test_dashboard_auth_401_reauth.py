...
Verifies the contract documented in Phase 6 v2 of the plan:
  ...
  - ``set_session_cookies(refresh_token="")`` does NOT emit the
    ``opencodon_session_rt`` cookie (contract V1: no RT to persist).
...
from opencodon.frontends.server.dashboard_auth.base import ProviderError, RefreshExpiredError
from opencodon.frontends.server.dashboard_auth.cookies import (
    SESSION_AT_COOKIE,
...
class TestTransparentRefreshOnAccessTokenEviction:
    ...
    the browser deletes ``opencodon_session_at`` the instant the token lapses,
    while ``opencodon_session_rt`` lives for 30 days. From that moment the
    ...
    def _build_rt_only_app(self):
        ...
        """
        import time as _t
        from tests.opencodon.frontends.server.conftest_dashboard_auth import _sign
...
class TestAutoSsoRedirect:
    ...
    from opencodon.frontends.server.dashboard_auth.cookies import SSO_ATTEMPT_COOKIE
    ...
    def test_multiple_providers_render_chooser_not_auto_sso(self, gated_app):
        ...
        from tests.opencodon.frontends.server.conftest_dashboard_auth import StubAuthProvider
        from opencodon.frontends.server.dashboard_auth import register_provider
...
class TestNextSameOriginValidation:
    ...
    def test_safe_next_validator_accepts_same_origin(self):
        from opencodon.frontends.server.dashboard_auth.middleware import _safe_next_target
...
class TestValidatePostLoginTarget:
    ...
    def test_accepts_same_origin_paths(self):
        from opencodon.frontends.server.dashboard_auth.routes import _validate_post_login_target
...
class TestRenderLoginHtmlNext:
    ...
    def test_no_next_emits_plain_button(self):
        from opencodon.frontends.server.dashboard_auth.login_page import render_login_html
