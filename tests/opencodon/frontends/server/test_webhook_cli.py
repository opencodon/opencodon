"""Tests for opencodon_cli/webhook.py — webhook subscription CLI."""
...
from opencodon.frontends.server.webhook import (
    webhook_command,
...
def _isolate(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.frontends.server.webhook._is_webhook_enabled", lambda: True
    )
...
def test_webhook_base_url_maps_wildcard_hosts_to_localhost(monkeypatch, host):
    monkeypatch.setattr(
        "opencodon.frontends.server.webhook._get_webhook_config",
...
class TestWebhookEnabledGate:
    def test_blocks_when_disabled(self, capsys, monkeypatch):
        monkeypatch.setattr("opencodon.frontends.server.webhook._is_webhook_enabled", lambda: False)
        webhook_command(_make_args(webhook_action="subscribe", name="blocked"))
        out = capsys.readouterr().out
        assert "not enabled" in out.lower()
        assert "opencodon gateway setup" in out
        assert _load_subscriptions() == {}
    ...
    def test_real_check_disabled(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "opencodon.frontends.server.webhook._is_webhook_enabled",
            lambda: bool({}.get("enabled")),
        )
        import opencodon.frontends.server.webhook as wh_mod
