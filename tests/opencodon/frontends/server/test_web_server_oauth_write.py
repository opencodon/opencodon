...
from opencodon.frontends.server.web_server import _save_anthropic_oauth_creds
...
def oauth_file(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr('opencodon.core.providers.anthropic_adapter._get_opencodon_oauth_file', lambda: target)
    monkeypatch.setattr('opencodon.core.credentials.credential_pool.load_pool', lambda _provider: _DummyPool())
    return target
