"""Unit tests for opencodon.frontends.server.win_pty_bridge — ConPTY spawning + byte forwarding.

Windows-only counterpart to tests/opencodon_cli/test_pty_bridge.py.  Drives
...
``opencodon_cli/web_server.py`` ``/api/pty`` handler — so these tests are the
...
from opencodon.frontends.server.win_pty_bridge import PtyUnavailableError, WinPtyBridge
...
class TestWinPtyBridgeUnavailable:
    ...
    def test_bridge_class_is_importable(self):
        ...
        #     from opencodon.frontends.server.win_pty_bridge import WinPtyBridge, PtyUnavailableError
...
class TestClampDimension:
    ...
    def test_clamps_above_max(self):
        from opencodon.frontends.server.win_pty_bridge import _MAX_COLS, _MAX_ROWS, _clamp

        assert _clamp(131072, _MAX_COLS) == _MAX_COLS
        assert _clamp(131072, _MAX_ROWS) == _MAX_ROWS

    def test_floors_at_one(self):
        from opencodon.frontends.server.win_pty_bridge import _MAX_COLS, _clamp
