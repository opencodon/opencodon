...
from opencodon.frontends.server.dashboard_auth import clear_providers, list_providers
...
class TestBasicProviderLoadsAfterUnblock:
    def test_disabled_basic_blocks_registration(self, opencodon_home, monkeypatch):
    ...
    def test_unblock_then_rediscover_registers_provider(
        self, opencodon_home, monkeypatch,
    ):
        password_hash = basic_plugin.hash_password("hunter2")
        cfg = {
            "dashboard": {
