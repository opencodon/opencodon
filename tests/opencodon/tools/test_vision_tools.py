...
class TestValidateImageUrl:
    ...
    def test_valid_https_url(self):
        with patch("opencodon.tools.url_safety.socket.getaddrinfo", return_value=[
...
class TestHandleVisionAnalyze:
    ...
    def test_returns_awaitable(self):
        ...
        with patch(
            "opencodon.tools.vision_tools.vision_analyze_tool", new_callable=AsyncMock
        ) as mock_tool:
    ...
    async def test_prompt_contains_question(self):
        ...
        with (
            ...
            patch(
                "opencodon.tools.vision_tools._should_use_native_vision_fast_path",
                return_value=False,
            ),
        ):
            mock_tool.return_value = json.dumps({"result": "ok"})
...
class TestErrorLoggingExcInfo:
    ...
    async def test_download_failure_logs_exc_info(self, tmp_path, caplog):
        ...
        from opencodon.tools.vision_tools import _download_image

        with patch("opencodon.tools.vision_tools.httpx.AsyncClient") as mock_client_cls:
            ...
            with (
                caplog.at_level(logging.ERROR, logger="opencodon.tools.vision_tools"),
    ...
    async def test_analysis_error_logs_exc_info(self, caplog):
        ...
        with (
            patch("opencodon.tools.vision_tools._validate_image_url_async", new_callable=AsyncMock, return_value=True),
            patch(
                "opencodon.tools.vision_tools._download_image",
    ...
    async def test_cleanup_error_logs_exc_info(self, tmp_path, caplog):
        ...
        with (
            patch(
                "opencodon.tools.vision_tools._image_to_base64_data_url",
                return_value="data:image/jpeg;base64,abc",
            ),
            caplog.at_level(logging.WARNING, logger="opencodon.tools.vision_tools"),
        ):
            ...
            with (
                patch("opencodon.tools.vision_tools.async_call_llm", new_callable=AsyncMock, return_value=mock_response),
            ):
...
class TestVisionConfig:
    ...
    async def test_vision_uses_configured_temperature_and_timeout(self, tmp_path):
        ...
        with (
            ...
            patch(
                "opencodon.tools.vision_tools.async_call_llm",
                new_callable=AsyncMock,
...
class TestVisionSafetyGuards:
    ...
    async def test_local_non_image_file_rejected_before_llm_call(self, tmp_path):
        ...
        with patch("opencodon.tools.vision_tools.async_call_llm", new_callable=AsyncMock) as mock_llm:
    ...
    async def test_native_fast_path_local_env_file_blocked_via_read_guard(self, tmp_path):
        ...
        from opencodon.tools.vision_tools import _vision_analyze_native
    ...
    async def test_blocked_remote_url_short_circuits_before_download(self):
        ...
        with (
            patch("opencodon.tools.website_policy.check_website_access", return_value=blocked),
            patch("opencodon.tools.url_safety.is_safe_url", return_value=True),
            patch("opencodon.tools.vision_tools._download_image", new_callable=AsyncMock) as mock_download,
        ):
    ...
    async def test_download_blocks_redirected_final_url(self, tmp_path):
        ...
        with (
            patch("opencodon.tools.vision_tools.check_website_access", side_effect=fake_check),
            patch("opencodon.tools.vision_tools.httpx.AsyncClient") as mock_client_cls,
...
class TestBase64SizeLimit:
    ...
    async def test_oversized_image_rejected_before_api_call(self, tmp_path):
        ...
        with patch("opencodon.tools.vision_tools._MAX_BASE64_BYTES", 1000), \
             patch("opencodon.tools.vision_tools.async_call_llm", new_callable=AsyncMock) as mock_llm:
...
class TestResizeImageForVision:
    ...
    def test_no_pillow_returns_original(self, tmp_path):
        ...
        with patch("opencodon.tools.vision_tools._image_to_base64_data_url") as mock_b64:
...
class TestDownloadRetryClassification:
    ...
    def test_is_retryable_classification(self):
        from opencodon.tools.vision_tools import _is_retryable_download_error
    ...
    async def test_404_fails_fast_without_retry(self, tmp_path):
        ...
        with (
            patch("opencodon.tools.vision_tools.httpx.AsyncClient", return_value=mock_client),
            patch("opencodon.tools.vision_tools.check_website_access", return_value=None),
...
class TestVisionCpuBurstCap:
    ...
    def test_resolver_defaults_to_host_cpus_no_ceiling(self):
        from opencodon.tools import vision_tools as vt

        with (
            patch.dict(os.environ, {}, clear=False),
            patch("opencodon.tools.vision_tools._detect_host_cpus", return_value=64),
            patch("opencodon.config.load_config", side_effect=Exception),
        ):
            os.environ.pop("OPENCODON_VISION_MAX_CONCURRENCY", None)
    ...
    def test_resolver_respects_low_host_cpu_count(self):
        ...
        with (
            patch.dict(os.environ, {}, clear=False),
            patch("opencodon.tools.vision_tools._detect_host_cpus", return_value=2),
            patch("opencodon.config.load_config", side_effect=Exception),
        ):
            os.environ.pop("OPENCODON_VISION_MAX_CONCURRENCY", None)
    ...
    def test_resolver_env_override(self):
        ...
        with patch.dict(os.environ, {"OPENCODON_VISION_MAX_CONCURRENCY": "16"}):
    ...
    def test_resolver_rejects_sub_one_override(self):
        ...
        with (
            patch.dict(os.environ, {"OPENCODON_VISION_MAX_CONCURRENCY": "0"}),
            patch("opencodon.tools.vision_tools._detect_host_cpus", return_value=2),
            patch("opencodon.config.load_config", side_effect=Exception),
        ):
    ...
    def test_cpu_executor_is_dedicated_and_sized_to_workers(self):
        ...
        vt = importlib.import_module("opencodon.tools.vision_tools")
