...
class TestIsHeadedMode:
    ...
    def test_caching(self):
        from opencodon.tools.browser_tool import _is_headed_mode
        cfg = {"browser": {"headed": True}}
        with patch("opencodon.config.read_raw_config", return_value=cfg) as mock_read:
            assert _is_headed_mode() is True
            assert _is_headed_mode() is True
...
class TestCleanupTaskResourcesHeadedSkip:
    def test_headless_still_cleans_browser(self):
        ...
        with (
            patch("opencodon.tools.browser_tool._is_headed_mode", return_value=False),
            patch("opencodon.core.run_agent.cleanup_vm"),
            patch("opencodon.core.run_agent.cleanup_browser") as mock_cb,
            patch(
    ...
    def test_headed_skips_browser_cleanup(self):
        from opencodon.core.chat_completion_helpers import cleanup_task_resources
        with (
            patch("opencodon.tools.browser_tool._is_headed_mode", return_value=True),
            patch("opencodon.core.run_agent.cleanup_vm"),
            patch("opencodon.core.run_agent.cleanup_browser") as mock_cb,
            patch(
                "opencodon.core.chat_completion_helpers.is_persistent_env",
    ...
    def test_headed_env_var_fallback_when_import_fails(self):
        ...
        with (
            patch(
                "opencodon.tools.browser_tool._is_headed_mode",
    ...
    def test_headed_does_not_skip_vm_cleanup(self):
        ...
        with (
            patch("opencodon.tools.browser_tool._is_headed_mode", return_value=True),
            patch("opencodon.core.run_agent.cleanup_vm") as mock_vm,
            patch("opencodon.core.run_agent.cleanup_browser"),
            patch(
                "opencodon.core.chat_completion_helpers.is_persistent_env",
                return_value=False,
            ),
