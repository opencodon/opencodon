...
class TestFirecrawlClientConfig:
    ...
    def setup_method(self):
        ...
        import opencodon.tools.web_tools
        opencodon.tools.web_tools._firecrawl_client = None
        opencodon.tools.web_tools._firecrawl_client_config = None
        for key in (
            "FIRECRAWL_API_KEY",
            "FIRECRAWL_API_URL",
        ):
    ...
    def test_no_config_raises_with_helpful_message(self):
        ...
        with patch("opencodon.tools.web_tools.Firecrawl"):
            with patch("opencodon.tools.web_tools._ddgs_package_importable", return_value=False):
                from opencodon.tools.web_tools import _get_firecrawl_client
    ...
    def test_singleton_returns_same_instance(self):
        ...
        with patch.dict(os.environ, {"FIRECRAWL_API_KEY": "fc-test"}):
            with patch("opencodon.tools.web_tools.Firecrawl") as mock_fc:
                from opencodon.tools.web_tools import _get_firecrawl_client
    ...
    def test_constructor_failure_allows_retry(self):
        ...
        with patch.dict(os.environ, {"FIRECRAWL_API_KEY": "fc-test"}):
            with patch("opencodon.tools.web_tools.Firecrawl") as mock_fc:
                ...
                assert opencodon.tools.web_tools._firecrawl_client is None
...
class TestBackendSelection:
    ...
    ``opencodon tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
    ...
    def test_config_parallel(self):
        ...
        with patch("opencodon.tools.web_tools._load_web_config", return_value={"backend": "parallel"}):
            assert _get_backend() == "parallel"

    def test_config_exa(self):
        """web.backend=exa in config → 'exa' regardless of other keys."""
        from opencodon.tools.web_tools import _get_backend
        with patch("opencodon.tools.web_tools._load_web_config", return_value={"backend": "exa"}), \
    ...
    def test_config_firecrawl(self):
        ...
        from opencodon.tools.web_tools import _get_backend
        with patch("opencodon.tools.web_tools._load_web_config", return_value={"backend": "firecrawl"}), \
             patch.dict(os.environ, {"PARALLEL_API_KEY": "test-key"}):
    ...
    def test_config_tavily(self):
        ...
        from opencodon.tools.web_tools import _get_backend
        with patch("opencodon.tools.web_tools._load_web_config", return_value={"backend": "tavily"}):
    ...
    def test_fallback_parallel_only_key(self):
        ...
        from opencodon.tools.web_tools import _get_backend
        with patch("opencodon.tools.web_tools._load_web_config", return_value={}), \
             patch.dict(os.environ, {"PARALLEL_API_KEY": "test-key"}):
            assert _get_backend() == "parallel"
    ...
    def test_fallback_no_keys_defaults_to_firecrawl(self):
        ...
        from opencodon.tools.web_tools import _get_backend
        with patch("opencodon.tools.web_tools._load_web_config", return_value={}), \
             patch("opencodon.tools.web_tools._ddgs_package_importable", return_value=False):
            assert _get_backend() == "firecrawl"

    def test_invalid_config_falls_through_to_fallback(self):
        """web.backend=invalid → ignored, uses key-based fallback."""
        from opencodon.tools.web_tools import _get_backend
        with patch("opencodon.tools.web_tools._load_web_config", return_value={"backend": "nonexistent"}), \
             patch.dict(os.environ, {"PARALLEL_API_KEY": "test-key"}):
            assert _get_backend() == "parallel"
...
class TestParallelClientConfig:
    ...
    def setup_method(self):
        import opencodon.tools.web_tools
        opencodon.tools.web_tools._parallel_client = None
    ...
    def test_creates_client_with_key(self):
        ...
        with patch.dict(os.environ, {"PARALLEL_API_KEY": "test-key"}):
            from opencodon.tools.web_tools import _get_parallel_client
...
class TestWebSearchSchema:
    ...
    def test_schema_exposes_optional_limit(self):
        ...
        limit_schema = opencodon.tools.web_tools.WEB_SEARCH_SCHEMA["parameters"]["properties"]["limit"]

        assert limit_schema["type"] == "integer"
        assert limit_schema["minimum"] == 1
        assert limit_schema["maximum"] == 100
        assert limit_schema["default"] == 5
        assert "limit" not in opencodon.tools.web_tools.WEB_SEARCH_SCHEMA["parameters"]["required"]

    def test_registered_handler_passes_limit(self):
        import opencodon.tools.web_tools

        entry = opencodon.tools.web_tools.registry.get_entry("web_search")
        with patch("opencodon.tools.web_tools.web_search_tool", return_value='{"success": true}') as mock_search:
    ...
    def test_web_search_clamps_limit_before_backend_call(self):
        ...
        with patch("opencodon.tools.web_tools._get_search_backend", return_value="parallel"), \
             patch("opencodon.core.providers.web_search_registry.get_provider", return_value=fake_provider), \
             patch("opencodon.tools.interrupt.is_interrupted", return_value=False), \
             patch.object(opencodon.tools.web_tools._debug, "log_call"), \
             patch.object(opencodon.tools.web_tools._debug, "save"):
            result = json.loads(opencodon.tools.web_tools.web_search_tool("docs", limit=500))
...
class TestWebSearchErrorHandling:
    ...
    def test_search_error_response_does_not_expose_diagnostics(self):
        ...
        with patch("opencodon.tools.web_tools._get_search_backend", return_value="firecrawl"), \
             patch("opencodon.core.providers.web_search_registry.get_provider", return_value=fake_provider), \
             patch("opencodon.tools.interrupt.is_interrupted", return_value=False), \
             patch.object(opencodon.tools.web_tools._debug, "log_call") as mock_log_call, \
             patch.object(opencodon.tools.web_tools._debug, "save"):
            result = json.loads(opencodon.tools.web_tools.web_search_tool("test query", limit=3))
...
class TestCheckWebApiKey:
    ...
    def test_parallel_key_only(self):
        with patch.dict(os.environ, {"PARALLEL_API_KEY": "test-key"}):
            from opencodon.tools.web_tools import check_web_api_key
    ...
    def test_null_backend_value_does_not_crash(self):
        ...
        with patch("opencodon.tools.web_tools._load_web_config", return_value={"backend": None}):
            from opencodon.tools.web_tools import check_web_api_key
    ...
    def test_null_web_section_does_not_crash(self):
        ...
        with patch("opencodon.config.load_config", return_value={"web": None}):
            from opencodon.tools.web_tools import _load_web_config, check_web_api_key
...
def test_web_requires_env_includes_exa_key():
    from opencodon.tools.web_tools import _web_requires_env
...
class TestNonBuiltinProviderAvailability:
    ...
    def setup_method(self):
        ...
        for key in self._WEB_ENV_KEYS:
            os.environ.pop(key, None)
        from opencodon.core.providers.web_search_registry import _reset_for_tests, register_provider
        _reset_for_tests()
    ...
    def test_is_backend_available_delegates_to_registry(self):
        ...
        from opencodon.tools.web_tools import _is_backend_available
    ...
    def test_capability_backend_honors_custom_extract_provider(self):
        ...
        with patch("opencodon.tools.web_tools._ddgs_package_importable", return_value=False), \
             patch("opencodon.tools.web_tools._ddgs_package_importable", return_value=False), \
             patch("opencodon.tools.web_tools._load_web_config",
    ...
    def test_tool_registry_entries_not_filtered_out(self):
        ...
        with patch("opencodon.tools.web_tools._ddgs_package_importable", return_value=False), \
             patch("opencodon.tools.web_tools._ddgs_package_importable", return_value=False):
            import opencodon.tools.web_tools
            web_search_entry = opencodon.tools.web_tools.registry.get_entry("web_search")
            web_extract_entry = opencodon.tools.web_tools.registry.get_entry("web_extract")
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.opencodon/.env), not bare os.getenv. Parametrized over the four
