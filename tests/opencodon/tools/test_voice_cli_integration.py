...
def _make_voice_cli(**overrides):
    """Create a minimal OpencodonCLI with only voice-related attrs initialized.
...
class TestStreamingTTSActivation:
    ...
    def _gate() -> bool:
        ...
        try:
            from opencodon.tools.tts_tool import _import_sounddevice, check_tts_requirements
            _import_sounddevice()
    ...
    def test_activates_for_any_working_provider(self):
        ...
        with patch("opencodon.tools.tts_tool._import_sounddevice", return_value=MagicMock()), \
             patch("opencodon.tools.tts_tool.check_tts_requirements", return_value=True):
    ...
    def test_does_not_activate_when_provider_unavailable(self):
        ...
        with patch("opencodon.tools.tts_tool._import_sounddevice", return_value=MagicMock()), \
             patch("opencodon.tools.tts_tool.check_tts_requirements", return_value=False):
    ...
    def test_does_not_activate_when_sounddevice_missing(self):
        ...
        with patch("opencodon.tools.tts_tool._import_sounddevice", side_effect=OSError("no PortAudio")), \
             patch("opencodon.tools.tts_tool.check_tts_requirements", return_value=True):
            assert self._gate() is False

    def test_stale_boolean_imports_no_longer_exist(self):
        """Confirm _HAS_ELEVENLABS and _HAS_AUDIO are not in tts_tool module."""
        import opencodon.tools.tts_tool as tts_mod
...
class TestEnableVoiceModeReal:
    ...
    @patch("cli._cprint")
    @patch("opencodon.config.load_config", return_value={"voice": {}})
    @patch("opencodon.tools.voice_mode.check_voice_requirements",
           return_value={"available": True, "details": "OK"})
    @patch("opencodon.tools.voice_mode.detect_audio_environment",
    ...
    @patch("cli._cprint")
    @patch("opencodon.config.load_config", return_value={"voice": {"auto_tts": True}})
    @patch("opencodon.tools.voice_mode.check_voice_requirements",
           return_value={"available": True, "details": "OK"})
    @patch("opencodon.tools.voice_mode.detect_audio_environment",
           return_value={"available": True, "warnings": []})
    ...
    @patch("cli._cprint")
    @patch("opencodon.config.load_config", side_effect=Exception("broken config"))
    @patch("opencodon.tools.voice_mode.check_voice_requirements",
           return_value={"available": True, "details": "OK"})
    @patch("opencodon.tools.voice_mode.detect_audio_environment",
           return_value={"available": True, "warnings": []})
...
class TestVoiceBeepConfigReal:
    ...
    @patch("opencodon.config.load_config", return_value={"voice": {"beep_enabled": False}})
    ...
    @patch("opencodon.tools.voice_mode.play_beep")
    @patch("opencodon.tools.voice_mode.create_audio_recorder")
    @patch(
        "opencodon.tools.voice_mode.check_voice_requirements",
        return_value={
            "available": True,
...
class TestDisableVoiceModeReal:
    ...
    @patch("cli._cprint")
    @patch("opencodon.tools.voice_mode.stop_playback")
    ...
    @patch("cli._cprint")
    @patch("opencodon.tools.voice_mode.stop_playback", side_effect=RuntimeError("boom"))
...
class TestVoiceSpeakResponseReal:
    ...
    def test_early_return_when_tts_off(self, _cp):
        ...
        with patch("opencodon.tools.tts_tool.text_to_speech_tool") as mock_tts:
    ...
    @patch("opencodon.tools.voice_mode.play_audio_file")
    @patch("opencodon.tools.tts_tool.text_to_speech_tool", return_value='{"success": true}')
    ...
    @patch("cli._cprint")
    @patch("cli.os.makedirs")
    @patch("opencodon.tools.tts_tool.text_to_speech_tool", side_effect=RuntimeError("tts fail"))
...
class TestVoiceStopAndTranscribeReal:
    ...
    def test_guard_not_recording(self, _cp):
        ...
        with patch("opencodon.tools.voice_mode.transcribe_recording") as mock_tr:
    ...
    @patch("cli.os.unlink")
    @patch("cli.os.path.isfile", return_value=True)
    @patch("opencodon.config.load_config", return_value={"stt": {}})
    @patch("opencodon.tools.voice_mode.transcribe_recording",
    ...
    @patch("cli.os.unlink")
    @patch("cli.os.path.isfile", return_value=True)
    @patch("opencodon.config.load_config", return_value={"stt": {"model": "whisper-large-v3"}})
    @patch("opencodon.tools.voice_mode.transcribe_recording",
...
class TestVoiceBargeCaptureSubmit:
    ...
    def test_transcript_is_queued_and_wav_removed(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "opencodon.tools.voice_mode.transcribe_recording",
