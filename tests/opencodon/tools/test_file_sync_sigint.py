...
from opencodon.tools.environments.file_sync import FileSyncManager
...
def test_deferred_sigint_redelivered_via_raise_signal(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.tools.environments.file_sync.signal.raise_signal", raised.append
    )
    monkeypatch.setattr(
        "opencodon.tools.environments.file_sync.os.kill",
