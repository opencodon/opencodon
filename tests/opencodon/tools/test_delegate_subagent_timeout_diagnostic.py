...
``~/.opencodon/logs/subagent-timeout-<sid>-<ts>.log``. This gives users a
...
class TestDumpSubagentTimeoutDiagnostic:
    ...
    def test_writes_log_with_expected_sections(self, opencodon_home):
        from opencodon.tools.delegate_tool import _dump_subagent_timeout_diagnostic
        ...
        p = Path(path)
        assert p.is_file()
        # File lives under OPENCODON_HOME/logs/
        assert p.parent == opencodon_home / "logs"
    ...
    def test_truncates_very_long_goal(self, opencodon_home):
        from opencodon.tools.delegate_tool import _dump_subagent_timeout_diagnostic
        child = _StubChild()
        huge_goal = "x" * 5000
    ...
    def test_missing_worker_thread_is_handled(self, opencodon_home):
        from opencodon.tools.delegate_tool import _dump_subagent_timeout_diagnostic
        child = _StubChild()
        path = _dump_subagent_timeout_diagnostic(
            child=child,
    ...
    def test_exited_worker_thread_is_handled(self, opencodon_home):
        from opencodon.tools.delegate_tool import _dump_subagent_timeout_diagnostic
        child = _StubChild()
    ...
    def test_returns_none_on_unwritable_logs_dir(self, tmp_path, monkeypatch):
        # Point OPENCODON_HOME at an unwritable path so logs/ can't be created
        # (simulates permission-denied). Helper must not raise.
        from opencodon.tools.delegate_tool import _dump_subagent_timeout_diagnostic
        bogus = tmp_path / "does-not-exist" / ".opencodon"
        monkeypatch.setenv("OPENCODON_HOME", str(bogus))
        child = _StubChild()
...
class TestRunSingleChildTimeoutDump:
    ...
    def _invoke_with_short_timeout(self, child, monkeypatch):
        ...
        from opencodon.tools import delegate_tool
    ...
    def test_zero_api_calls_writes_dump_and_surfaces_path(self, opencodon_home, monkeypatch):
        ...
        assert dump_path.parent == opencodon_home / "logs"
    ...
    def test_nonzero_api_calls_skips_dump_and_uses_old_message(self, opencodon_home, monkeypatch):
