...
from opencodon.tools.skill_manager_tool import (
    _validate_name,
    _validate_category,
...
class TestSkillManageDispatcher:
    ...
    def test_create_from_background_review_marks_agent_created(self, tmp_path):
        ...
        from opencodon.tools.skill_provenance import set_current_write_origin, BACKGROUND_REVIEW
        ...
        try:
            ...
            from opencodon.tools.skill_provenance import reset_current_write_origin
    ...
    def test_background_review_delete_refuses_bundled_even_with_absorbed_into(self, tmp_path):
        from opencodon.tools.skill_provenance import (
            BACKGROUND_REVIEW,
        ...
        try:
            with _skill_dir(tmp_path), \
                 patch("opencodon.tools.skill_usage.is_protected_builtin", return_value=False), \
                 patch("opencodon.tools.skill_usage.is_hub_installed", return_value=False), \
                 patch("opencodon.tools.skill_usage.is_bundled",
...
class TestSecurityScanGate:
    ...
    def test_scan_noop_when_flag_off(self, tmp_path):
        ...
        from opencodon.tools.skill_manager_tool import _security_scan_skill

        with patch("opencodon.tools.skill_manager_tool._guard_agent_created_enabled", return_value=False), \
             patch("opencodon.tools.skill_manager_tool.scan_skill") as mock_scan:
    ...
    def test_scan_runs_when_flag_on(self, tmp_path):
        ...
        with patch("opencodon.tools.skill_manager_tool._guard_agent_created_enabled", return_value=True), \
             patch("opencodon.tools.skill_manager_tool.scan_skill", return_value=fake_result) as mock_scan:
            result = _security_scan_skill(tmp_path)
    ...
    def test_scan_blocks_dangerous_when_flag_on(self, tmp_path):
        ...
        with patch("opencodon.tools.skill_manager_tool._guard_agent_created_enabled", return_value=True), \
             patch("opencodon.tools.skill_manager_tool.scan_skill", return_value=fake_result):
            result = _security_scan_skill(tmp_path)
    ...
    def test_guard_flag_reads_config_default_false(self):
        ...
        from opencodon.tools.skill_manager_tool import _guard_agent_created_enabled

        with patch("opencodon.config.load_config", return_value={"skills": {}}):
...
class TestExternalSkillMutations:
    ...
    def test_background_review_refuses_to_patch_pinned_skill(self, tmp_path):
        ...
        with _skill_dir(tmp_path):
            ...
            token = set_current_write_origin(BACKGROUND_REVIEW)
            try:
                with patch("opencodon.tools.skill_usage.get_record", side_effect=_fake_get_record):
                    raw = skill_manage(
                        action="patch",
                        name="my-skill",
        ...
        with _skill_dir(tmp_path):
            ...
            try:
                ...
                with patch(
                    ...
                    "opencodon.tools.skill_usage.archive_skill", return_value=(True, "archived"),
                ):
                    raw = skill_manage(
                        action="delete",
                        name="agent-skill",
...
# come up. The user unpins via `opencodon curator unpin <name>` to delete.
# ---------------------------------------------------------------------------
...
class TestPinnedGuard:
    ...
    def _pin(name: str):
        ...
        return patch("opencodon.tools.skill_usage.get_record", side_effect=_fake_get_record)
    ...
    def test_delete_refuses_pinned(self, tmp_path):
        ...
        assert "opencodon curator unpin my-skill" in result["error"]
        # Skill still exists
        assert (tmp_path / "my-skill" / "SKILL.md").exists()
    ...
    def test_broken_sidecar_fails_open(self, tmp_path):
        ...
        with _skill_dir(tmp_path):
            _create_skill("my-skill", VALID_SKILL_CONTENT)
            with patch("opencodon.tools.skill_usage.get_record",
...
class TestCuratorConsolidationDeleteGuard:
    ...
    def test_dispatcher_preserves_usage_record_on_curator_archive(self, tmp_path, monkeypatch):
        ...
        # `opencodon curator restore` can bring it back.
