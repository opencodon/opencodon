...
silently discards them; opencodon now passes ``_make_logging_callback()``
...
class TestLoggingCallback:
    @pytest.mark.asyncio
    async def test_routes_to_opencodon_logger_with_server_tag(self, caplog):
        server = MCPServerTask("log_srv")
        callback = server._make_logging_callback()
        with caplog.at_level(logging.INFO, logger="opencodon.tools.mcp_tool"):
    ...
    async def test_includes_sub_logger_name(self, caplog):
        server = MCPServerTask("log_srv")
        callback = server._make_logging_callback()
        with caplog.at_level(logging.WARNING, logger="opencodon.tools.mcp_tool"):
    ...
    async def test_error_family_maps_to_error_level(self, caplog):
        server = MCPServerTask("log_srv")
        callback = server._make_logging_callback()
        with caplog.at_level(logging.ERROR, logger="opencodon.tools.mcp_tool"):
