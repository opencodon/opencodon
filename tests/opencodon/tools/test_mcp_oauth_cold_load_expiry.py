...
1. ``OpencodonTokenStorage.set_tokens`` persisted only relative ``expires_in``,
...
- ``OpencodonMCPOAuthProvider._initialize`` seeds ``context.token_expiry_time``
...
# ---------------------------------------------------------------------------
# OpencodonTokenStorage — absolute expiry persistence
# ---------------------------------------------------------------------------
...
class TestGetTokensReconstructsExpiresIn:
    ...
    def test_get_tokens_returns_zero_ttl_for_expired_token(
        ...
        monkeypatch.setenv("OPENCODON_HOME", str(tmp_path))
        from opencodon.tools.mcp_oauth import OpencodonTokenStorage, _get_token_dir
...
# ---------------------------------------------------------------------------
# OpencodonMCPOAuthProvider._initialize — seed token_expiry_time
# ---------------------------------------------------------------------------
...
async def test_initialize_seeds_token_expiry_time_from_stored_tokens(
    ...
    from opencodon.tools.mcp_oauth import OpencodonTokenStorage
    from opencodon.tools.mcp_oauth_manager import _OPENCODON_PROVIDER_CLS, reset_manager_for_tests
    ...
    provider = _OPENCODON_PROVIDER_CLS(
        server_name="srv",
...
async def test_initialize_skips_prefetch_when_no_tokens(tmp_path, monkeypatch):
    ...
    storage = OpencodonTokenStorage("srv")  # empty — no tokens on disk
    metadata = OAuthClientMetadata(
        redirect_uris=[AnyUrl("http://127.0.0.1:12345/callback")],
        client_name="opencodon",
    )
