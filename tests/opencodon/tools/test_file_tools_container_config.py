...
class TestFileToolsContainerConfig:
    def _run(self, env_config, task_id, task_env_overrides=None):
        ...
        with patch("opencodon.tools.terminal_tool._get_env_config", return_value=env_config), \
             patch("opencodon.tools.terminal_tool._task_env_overrides", task_env_overrides or {}), \
             patch("opencodon.tools.terminal_tool._active_environments", {}), \
             patch("opencodon.tools.terminal_tool._creation_locks", {}), \
             patch("opencodon.tools.terminal_tool._creation_locks_lock", __import__("threading").Lock()), \
             patch("opencodon.tools.terminal_tool._create_environment", side_effect=fake_create_env), \
             patch("opencodon.tools.terminal_tool._start_cleanup_thread"), \
             patch("opencodon.tools.terminal_tool._check_disk_usage_warning"), \
             patch("opencodon.tools.file_tools._file_ops_cache", {}), \
             patch("opencodon.tools.file_tools._file_ops_lock", __import__("threading").Lock()):
