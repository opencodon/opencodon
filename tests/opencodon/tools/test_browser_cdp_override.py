...
class TestResolveCdpOverride:
    def test_keeps_full_devtools_websocket_url(self):
        from opencodon.tools.browser_tool import _resolve_cdp_override
    ...
    def test_resolves_http_discovery_endpoint_to_websocket(self):
        ...
        with patch("opencodon.tools.browser_tool.requests.get", return_value=response) as mock_get:
    ...
    def test_falls_back_to_raw_url_when_discovery_fails(self):
        ...
        with patch("opencodon.tools.browser_tool.requests.get", side_effect=RuntimeError("boom")):
    ...
    def test_redacts_secret_query_params_in_success_log(self):
        ...
        with patch("opencodon.tools.browser_tool.requests.get", return_value=response), \
                patch("opencodon.tools.browser_tool.logger.info") as mock_info:
    ...
    def test_redacts_secret_query_params_in_failure_log(self):
        ...
        with patch("opencodon.tools.browser_tool.requests.get", side_effect=secret_error), \
                patch("opencodon.tools.browser_tool.logger.warning") as mock_warning:
            resolved = _resolve_cdp_override(raw)
    ...
    def test_normalizes_provider_returned_http_cdp_url_when_creating_session(self, monkeypatch):
        import opencodon.tools.browser_tool as browser_tool
...
class TestGetCdpOverride:
    ...
    def test_uses_config_browser_cdp_url_when_env_missing(self, monkeypatch):
        ...
        with patch("opencodon.config.read_raw_config", return_value={"browser": {"cdp_url": HTTP_URL}}), \
             patch("opencodon.tools.browser_tool.requests.get", return_value=response) as mock_get:
            resolved = browser_tool._get_cdp_override()
    ...
    def test_camofox_yields_to_config_cdp_override(self, monkeypatch):
        ...
        import opencodon.tools.browser_camofox as bc
...
class TestCreateCdpSession:
    ...
    def test_redacts_token_in_session_creation_log(self):
        from opencodon.tools.browser_tool import _create_cdp_session

        cdp_url_with_token = "wss://cdp.example/devtools/browser/abc?token=super-secret-token-999"

        with patch("opencodon.tools.browser_tool.logger.info") as mock_info:
...
class TestCDPSupervisorTimeoutRedaction:
    ...
    def _make_timed_out_supervisor(self, cdp_url: str):
        ...
        import threading
        from opencodon.tools.browser_supervisor import CDPSupervisor
...
class TestRedactCdpErrorText:
    ...
    def test_masks_query_token_in_exception(self):
        from opencodon.tools.browser_supervisor import _redact_cdp_error_text
