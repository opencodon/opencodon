...
def test_real_process_restart_restores_owned_completion_once(tmp_path):
    ...
    env = {**os.environ, "OPENCODON_HOME": str(tmp_path), "PYTHONPATH": repo}
    producer = r'''
import time
from opencodon.tools import async_delegation as ad
r = ad.dispatch_async_delegation(
...
def test_run_agent_dispatch_forces_background():
    """opencodon.core.run_agent._dispatch_delegate_task — the live model path — forces
    ...
    with patch("opencodon.tools.delegate_tool.delegate_task", _fake_delegate):
        agent = _FakeAgent()
