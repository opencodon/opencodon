...
def clean_env(monkeypatch):
    ...
    monkeypatch.delenv("OPENCODON_LOCAL_STT_COMMAND", raising=False)
    monkeypatch.delenv("OPENCODON_LOCAL_STT_LANGUAGE", raising=False)
...
class TestGetProviderGroq:
    ...
    def test_groq_when_key_set(self, monkeypatch):
        ...
        with patch("opencodon.tools.transcription_tools._HAS_OPENAI", True), \
             patch("opencodon.tools.transcription_tools._HAS_FASTER_WHISPER", False):
            from opencodon.tools.transcription_tools import _get_provider
    ...
    def test_groq_explicit_no_fallback(self, monkeypatch):
        ...
        monkeypatch.delenv("GROQ_API_KEY", raising=False)
        with patch("opencodon.tools.transcription_tools._HAS_FASTER_WHISPER", True):
            from opencodon.tools.transcription_tools import _get_provider
    ...
    def test_groq_nothing_available(self, monkeypatch):
        monkeypatch.delenv("GROQ_API_KEY", raising=False)
        monkeypatch.delenv("VOICE_TOOLS_OPENAI_KEY", raising=False)
        with patch("opencodon.tools.transcription_tools._HAS_FASTER_WHISPER", False), \
             patch("opencodon.tools.transcription_tools._HAS_OPENAI", False):
            from opencodon.tools.transcription_tools import _get_provider
            assert _get_provider({"provider": "groq"}) == "none"
...
class TestGetProviderFallbackPriority:
    ...
    def test_auto_detect_prefers_groq_over_openai(self, monkeypatch):
        ...
        with patch("opencodon.tools.transcription_tools._HAS_FASTER_WHISPER", False), \
             patch("opencodon.tools.transcription_tools._has_local_command", return_value=False), \
             patch("opencodon.tools.transcription_tools._HAS_OPENAI", True):
            from opencodon.tools.transcription_tools import _get_provider
...
class TestTranscribeGroq:
    def test_no_key(self, monkeypatch):
        monkeypatch.delenv("GROQ_API_KEY", raising=False)
        from opencodon.tools.transcription_tools import _transcribe_groq
    ...
    def test_openai_package_not_installed(self, monkeypatch):
        monkeypatch.setenv("GROQ_API_KEY", "gsk-test")
        with patch("opencodon.tools.transcription_tools._HAS_OPENAI", False):
            from opencodon.tools.transcription_tools import _transcribe_groq
            result = _transcribe_groq("/tmp/test.ogg", "whisper-large-v3-turbo")
        assert result["success"] is False
    ...
    def test_uses_groq_base_url(self, monkeypatch, sample_wav):
        ...
        with patch("opencodon.tools.transcription_tools._HAS_OPENAI", True), \
            ...
            from opencodon.tools.transcription_tools import _transcribe_groq, GROQ_BASE_URL
...
class TestTranscribeOpenAIExtended:
    def test_openai_package_not_installed(self, monkeypatch):
        monkeypatch.setenv("VOICE_TOOLS_OPENAI_KEY", "sk-test")
        with patch("opencodon.tools.transcription_tools._HAS_OPENAI", False):
            from opencodon.tools.transcription_tools import _transcribe_openai
    ...
    def test_uses_openai_base_url(self, monkeypatch, sample_wav):
        ...
        with patch("opencodon.tools.transcription_tools._HAS_OPENAI", True), \
             patch("openai.OpenAI", return_value=mock_client) as mock_openai_cls:
            from opencodon.tools.transcription_tools import _transcribe_openai, OPENAI_BASE_URL
...
class TestTranscribeLocalCommand:
    def test_auto_detects_local_whisper_binary(self, monkeypatch):
        monkeypatch.delenv("OPENCODON_LOCAL_STT_COMMAND", raising=False)
        monkeypatch.setattr("opencodon.tools.transcription_tools._find_whisper_binary", lambda: "/opt/homebrew/bin/whisper")

        from opencodon.tools.transcription_tools import _get_local_command_template
    ...
    def test_command_fallback_with_template(self, monkeypatch, sample_ogg, tmp_path):
        ...
        monkeypatch.setenv("OPENCODON_LOCAL_STT_LANGUAGE", "en")
        ...
        monkeypatch.setattr("opencodon.tools.transcription_tools.tempfile.TemporaryDirectory", fake_tempdir)
        monkeypatch.setattr("opencodon.tools.transcription_tools._find_ffmpeg_binary", lambda: "/opt/homebrew/bin/ffmpeg")
        monkeypatch.setattr("opencodon.tools.transcription_tools.subprocess.run", fake_run)

        from opencodon.tools.transcription_tools import _transcribe_local_command
...
class TestTranscribeLocalExtended:
    def test_model_reuse_on_second_call(self, tmp_path):
        ...
        with patch("opencodon.tools.transcription_tools._HAS_FASTER_WHISPER", True), \
             ...
             patch("opencodon.tools.transcription_tools._local_model", None), \
             patch("opencodon.tools.transcription_tools._local_model_name", None):
            from opencodon.tools.transcription_tools import _transcribe_local
...
class TestModelAutoCorrection:
    def test_groq_corrects_openai_model(self, monkeypatch, sample_wav):
        ...
        with patch("opencodon.tools.transcription_tools._HAS_OPENAI", True), \
             patch("openai.OpenAI", return_value=mock_client):
            from opencodon.tools.transcription_tools import _transcribe_groq, DEFAULT_GROQ_STT_MODEL
    ...
    def test_openai_corrects_groq_model(self, monkeypatch, sample_wav):
        ...
        with patch("opencodon.tools.transcription_tools._HAS_OPENAI", True), \
             patch("openai.OpenAI", return_value=mock_client):
            from opencodon.tools.transcription_tools import _transcribe_openai, DEFAULT_STT_MODEL
...
class TestLoadSttConfig:
    def test_returns_dict_when_import_fails(self):
        with patch("opencodon.tools.transcription_tools._load_stt_config") as mock_load:
            mock_load.return_value = {}
            from opencodon.tools.transcription_tools import _load_stt_config
    ...
    def test_real_load_returns_dict(self):
        ...
        with patch.dict("sys.modules", {"opencodon.config": None}):
            from opencodon.tools.transcription_tools import _load_stt_config
...
class TestValidateAudioFileEdgeCases:
    def test_directory_is_not_a_file(self, tmp_path):
        from opencodon.tools.transcription_tools import _validate_audio_file
    ...
    def test_all_supported_formats_accepted(self, tmp_path):
        from opencodon.tools.transcription_tools import _validate_audio_file, SUPPORTED_FORMATS
...
class TestTranscribeAudioDispatch:
    def test_dispatches_to_groq(self, sample_ogg):
        with patch("opencodon.tools.transcription_tools._load_stt_config", return_value={"provider": "groq"}), \
             patch("opencodon.tools.transcription_tools._get_provider", return_value="groq"), \
             patch("opencodon.tools.transcription_tools._transcribe_groq",
    ...
    def test_dispatches_to_local(self, sample_ogg):
        with patch("opencodon.tools.transcription_tools._load_stt_config", return_value={}), \
             patch("opencodon.tools.transcription_tools._get_provider", return_value="local"), \
             patch("opencodon.tools.transcription_tools._transcribe_local",
    ...
    def test_dispatches_to_openai(self, sample_ogg):
        with patch("opencodon.tools.transcription_tools._load_stt_config", return_value={"provider": "openai"}), \
             patch("opencodon.tools.transcription_tools._get_provider", return_value="openai"), \
             patch("opencodon.tools.transcription_tools._transcribe_openai",
    ...
    def test_no_provider_returns_error(self, sample_ogg):
        with patch("opencodon.tools.transcription_tools._load_stt_config", return_value={}), \
             patch("opencodon.tools.transcription_tools._get_provider", return_value="none"):
            from opencodon.tools.transcription_tools import transcribe_audio
            result = transcribe_audio(sample_ogg)
    ...
    def test_default_model_used_when_none(self, sample_ogg):
        with patch("opencodon.tools.transcription_tools._load_stt_config", return_value={}), \
             ...
             patch("opencodon.tools.transcription_tools._transcribe_groq",
                   return_value={"success": True, "transcript": "hi"}) as mock_groq:
            from opencodon.tools.transcription_tools import transcribe_audio, DEFAULT_GROQ_STT_MODEL
    ...
    def test_config_local_model_used(self, sample_ogg):
        ...
        with patch("opencodon.tools.transcription_tools._load_stt_config", return_value=config), \
             patch("opencodon.tools.transcription_tools._get_provider", return_value="local"), \
             patch("opencodon.tools.transcription_tools._transcribe_local",
                   return_value={"success": True, "transcript": "hi"}) as mock_local:
            from opencodon.tools.transcription_tools import transcribe_audio
...
class TestTranscribeMistral:
    def test_no_key(self, monkeypatch):
        monkeypatch.delenv("MISTRAL_API_KEY", raising=False)
        from opencodon.tools.transcription_tools import _transcribe_mistral
...
class TestGetProviderMistral:
    ...
    def test_mistral_when_key_and_sdk_available(self, monkeypatch):
        monkeypatch.setenv("MISTRAL_API_KEY", "test-key")
        with patch("opencodon.tools.transcription_tools._HAS_MISTRAL", True):
            from opencodon.tools.transcription_tools import _get_provider
    ...
    def test_mistral_explicit_no_sdk_returns_none(self, monkeypatch):
        ...
        monkeypatch.setenv("MISTRAL_API_KEY", "test-key")
        with patch("opencodon.tools.transcription_tools._HAS_MISTRAL", False):
            from opencodon.tools.transcription_tools import _get_provider
            assert _get_provider({"provider": "mistral"}) == "none"
    ...
    def test_auto_detect_mistral_after_openai(self, monkeypatch):
        ...
        with patch("opencodon.tools.transcription_tools._HAS_FASTER_WHISPER", False), \
             patch("opencodon.tools.transcription_tools._has_local_command", return_value=False), \
             patch("opencodon.tools.transcription_tools._HAS_OPENAI", False), \
             patch("opencodon.tools.transcription_tools._HAS_MISTRAL", True):
            from opencodon.tools.transcription_tools import _get_provider
    ...
    def test_auto_detect_skips_mistral_without_sdk(self, monkeypatch):
        ...
        with patch("opencodon.tools.transcription_tools._HAS_FASTER_WHISPER", False), \
             patch("opencodon.tools.transcription_tools._has_local_command", return_value=False), \
             patch("opencodon.tools.transcription_tools._HAS_OPENAI", False), \
             patch("opencodon.tools.transcription_tools._HAS_MISTRAL", False):
            from opencodon.tools.transcription_tools import _get_provider
...
class TestTranscribeAudioMistralDispatch:
    def test_dispatches_to_mistral(self, sample_ogg):
        with patch("opencodon.tools.transcription_tools._load_stt_config", return_value={"provider": "mistral"}), \
             patch("opencodon.tools.transcription_tools._get_provider", return_value="mistral"), \
             patch("opencodon.tools.transcription_tools._transcribe_mistral",
...
def mock_xai_http_module():
    ...
    fake_module = MagicMock()
    fake_module.opencodon_xai_user_agent = MagicMock(return_value="opencodon-xai/test")
    with patch.dict("sys.modules", {"opencodon.tools.xai_http": fake_module}):
        yield fake_module
...
class TestTranscribeXAI:
    def test_no_key(self, monkeypatch):
        ...
        from opencodon.tools.transcription_tools import _transcribe_xai
    ...
    def test_sends_language_and_format(self, monkeypatch, sample_ogg, mock_xai_http_module):
        ...
        monkeypatch.setenv("OPENCODON_LOCAL_STT_LANGUAGE", "fr")
...
class TestTranscribeAudioXAIDispatch:
    def test_dispatches_to_xai(self, sample_ogg):
        with patch("opencodon.tools.transcription_tools._load_stt_config", return_value={"provider": "xai"}), \
             patch("opencodon.tools.transcription_tools._get_provider", return_value="xai"), \
             patch("opencodon.tools.transcription_tools._transcribe_xai",
...
class TestTranscribeElevenLabs:
    def test_no_key(self, monkeypatch):
        monkeypatch.delenv("ELEVENLABS_API_KEY", raising=False)
        from opencodon.tools.transcription_tools import _transcribe_elevenlabs
...
class TestTranscribeAudioElevenLabsDispatch:
    def test_dispatches_to_elevenlabs(self, sample_ogg):
        with patch("opencodon.tools.transcription_tools._load_stt_config", return_value={"provider": "elevenlabs"}), \
             patch("opencodon.tools.transcription_tools._get_provider", return_value="elevenlabs"), \
             patch("opencodon.tools.transcription_tools._transcribe_elevenlabs",
...
class TestShellSafety:
    def test_auto_detected_template_is_shlex_safe(self, monkeypatch):
        ...
        monkeypatch.delenv("OPENCODON_LOCAL_STT_COMMAND", raising=False)
        monkeypatch.setattr(
            "opencodon.tools.transcription_tools._find_whisper_binary",
    ...
    def test_env_var_template_uses_shell_path(self, monkeypatch):
        """When OPENCODON_LOCAL_STT_COMMAND is set, use_shell should be True."""
        import os
        from opencodon.tools.transcription_tools import LOCAL_STT_COMMAND_ENV
