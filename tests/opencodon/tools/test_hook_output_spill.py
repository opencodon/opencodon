...
from opencodon.tools import hook_output_spill as hos
...
class SpillIfOversizedTests(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="opencodon-spill-test-")
    ...
    def test_session_id_with_path_separators_is_sanitised(self):
        ...
        self.assertFalse(Path("/etc/passwd-opencodon-test").exists())
    ...
    def test_default_directory_uses_opencodon_home(self):
        """When no directory override, spill under OPENCODON_HOME/hook_outputs."""
        test_home = tempfile.mkdtemp(prefix="opencodon-home-")
        try:
            with patch.dict(os.environ, {"OPENCODON_HOME": test_home}):
                # Also patch get_opencodon_home to the env var to mirror production.
            ...
            # ~/.opencodon/hook_outputs depending on get_opencodon_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.opencodon/hook_outputs/sess")),
            ]
