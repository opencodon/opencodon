...
def clean_env(monkeypatch):
    for key in ("MISTRAL_API_KEY", "OPENCODON_SESSION_PLATFORM"):
...
class TestGenerateMistralTts:
    def test_missing_api_key_raises_value_error(self, tmp_path, mock_mistral_module):
        from opencodon.tools.tts_tool import _generate_mistral_tts
    ...
    def test_default_voice_id_when_absent(
        ...
        from opencodon.tools.tts_tool import DEFAULT_MISTRAL_TTS_VOICE_ID, _generate_mistral_tts
    ...
    def test_default_model_used(self, tmp_path, mock_mistral_module, monkeypatch):
        from opencodon.tools.tts_tool import DEFAULT_MISTRAL_TTS_MODEL, _generate_mistral_tts
...
class TestTtsDispatcherMistral:
    def test_dispatcher_routes_to_mistral(
        ...
        with patch("opencodon.tools.tts_tool._load_tts_config", return_value={"provider": "mistral"}):
    ...
    def test_dispatcher_returns_error_when_sdk_not_installed(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MISTRAL_API_KEY", "test-key")
        with patch(
            "opencodon.tools.tts_tool._import_mistral_client", side_effect=ImportError("no module")
        ), patch("opencodon.tools.tts_tool._load_tts_config", return_value={"provider": "mistral"}):
            result = json.loads(
...
class TestCheckTtsRequirementsMistral:
    def test_mistral_sdk_and_key_returns_true(self, mock_mistral_module, monkeypatch):
        ...
        monkeypatch.setenv("MISTRAL_API_KEY", "test-key")
        with patch(
            "opencodon.tools.tts_tool._load_tts_config",
            return_value={"provider": "mistral"},
        ), patch("opencodon.tools.tts_tool._import_edge_tts", side_effect=ImportError), \
             patch("opencodon.tools.tts_tool._import_elevenlabs", side_effect=ImportError), \
             patch("opencodon.tools.tts_tool._import_openai_client", side_effect=ImportError), \
             patch("opencodon.tools.tts_tool._check_neutts_available", return_value=False):
            assert check_tts_requirements() is True

    def test_mistral_key_missing_returns_false(self, mock_mistral_module):
        from opencodon.tools.tts_tool import check_tts_requirements

        with patch("opencodon.tools.tts_tool._import_edge_tts", side_effect=ImportError), \
             patch("opencodon.tools.tts_tool._import_elevenlabs", side_effect=ImportError), \
             patch("opencodon.tools.tts_tool._import_openai_client", side_effect=ImportError), \
             patch("opencodon.tools.tts_tool._check_neutts_available", return_value=False), \
             patch("opencodon.tools.tts_tool._check_kittentts_available", return_value=False), \
             patch("opencodon.tools.tts_tool._check_piper_available", return_value=False), \
             patch("opencodon.tools.tts_tool._has_any_command_tts_provider", return_value=False):
