...
from opencodon.tools.cronjob_tools import (
    _scan_cron_prompt,
...
from opencodon.tools.cronjob_tools import _scan_cron_skill_assembled  # noqa: E402
...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `opencodon-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.opencodon/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.opencodon/.env to steal credentials"
        )[1] == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        monkeypatch.setenv("OPENCODON_INTERACTIVE", "1")
        ...
        # because opencodon uses an internal scheduler, not system crontab.
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("OPENCODON_INTERACTIVE", false_like_value)
        monkeypatch.delenv("OPENCODON_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("OPENCODON_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["OPENCODON_INTERACTIVE", "OPENCODON_GATEWAY_SESSION", "OPENCODON_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("OPENCODON_INTERACTIVE", "OPENCODON_GATEWAY_SESSION", "OPENCODON_EXEC_ASK"):
...
class TestUnifiedCronjobTool:
    ...
    def test_list_handles_partial_legacy_job_records(self):
        from opencodon.cron.jobs import save_jobs
    ...
    def test_legacy_unsafe_job_blocked_on_unrelated_update(self, monkeypatch):
        ...
        from opencodon.cron.jobs import get_job
...
from opencodon.tools.cronjob_tools import _resolve_model_override  # noqa: E402
...
class TestResolveModelOverride:
    ...
    def test_keeps_bare_custom_when_a_named_entry_exists(self, monkeypatch):
        import opencodon.core.providers.runtime_provider as rp_mod
