...
from opencodon.tools.file_tools import (
    PATCH_SCHEMA,
)


class TestReadFileHandler:
    @patch("opencodon.tools.file_tools._get_file_ops")
    def test_returns_file_content(self, mock_get):
        ...
        from opencodon.tools.file_tools import read_file_tool
...
class TestWriteFileHandler:
    ...
    def test_permission_error_returns_error_json_without_error_log(self, mock_get, caplog):
        ...
        from opencodon.tools.file_tools import write_file_tool
        with caplog.at_level(logging.DEBUG, logger="opencodon.tools.file_tools"):
    ...
    def test_unexpected_exception_still_logs_error(self, mock_get, caplog):
        ...
        from opencodon.tools.file_tools import write_file_tool
        with caplog.at_level(logging.ERROR, logger="opencodon.tools.file_tools"):
            result = json.loads(write_file_tool("/tmp/out.txt", "data"))
    ...
    def test_missing_content_key_returns_error(self):
        ...
        from opencodon.tools.file_tools import _handle_write_file
    ...
    def test_explicit_empty_content_is_allowed(self):
        ...
        with patch("opencodon.tools.file_tools._get_file_ops") as mock_get:
            mock_ops = MagicMock()
            result_obj = MagicMock()
...
class TestPatchHandler:
    ...
    def test_replace_mode_calls_patch_replace(self, mock_get):
        ...
        from opencodon.tools.file_tools import patch_tool
...
class TestSearchHandler:
    ...
    def test_search_calls_file_ops(self, mock_get):
        ...
        from opencodon.tools.file_tools import search_tool
...
class TestWindowsMsysPathResolution:
    ...
    def test_absolute_msys_path_normalized_before_windows_resolve(self, monkeypatch):
        import opencodon.tools.environments.local as local_mod
        import opencodon.tools.file_tools as file_tools
...
class TestSearchHints:
    ...
    def setup_method(self):
        ...
        from opencodon.tools.file_tools import _read_tracker
        _read_tracker.clear()
...
class TestSensitivePathCheck:
    ...
    def test_opencodon_config_blocked_for_write_file(self, tmp_path, monkeypatch):
        fake_config = tmp_path / "config.yaml"
        monkeypatch.setattr("opencodon.tools.file_tools._opencodon_config_resolved", str(fake_config))
        monkeypatch.setattr("opencodon.tools.file_tools._opencodon_config_resolved_loaded", True)

        from opencodon.tools.file_tools import write_file_tool
        result = json.loads(write_file_tool(str(fake_config), "approvals:\n  mode: off\n"))
        assert "error" in result
        assert "opencodon config" in result["error"]

    def test_opencodon_config_blocked_via_tilde_path(self, tmp_path, monkeypatch):
        fake_config = tmp_path / "config.yaml"
        monkeypatch.setattr("opencodon.tools.file_tools._opencodon_config_resolved", str(fake_config))
        monkeypatch.setattr("opencodon.tools.file_tools._opencodon_config_resolved_loaded", True)
    ...
    def test_opencodon_config_blocked_for_patch(self, tmp_path, monkeypatch):
        fake_config = tmp_path / "config.yaml"
    ...
    def test_system_path_still_blocked(self, monkeypatch):
        monkeypatch.setattr("opencodon.tools.file_tools._opencodon_config_resolved", "/some/other/path")
        monkeypatch.setattr("opencodon.tools.file_tools._opencodon_config_resolved_loaded", True)
    ...
    def test_normal_file_not_blocked(self, mock_get, monkeypatch):
        monkeypatch.setattr("opencodon.tools.file_tools._opencodon_config_resolved", "/home/user/.opencodon/config.yaml")
        monkeypatch.setattr("opencodon.tools.file_tools._opencodon_config_resolved_loaded", True)
        mock_ops = MagicMock()
        result_obj = MagicMock()
...
class TestSessionCwdSurvivesEnvRecreation:
    ...
    @patch("opencodon.tools.terminal_tool._active_environments", new_callable=dict)
    @patch("opencodon.tools.file_tools._file_ops_cache", new_callable=dict)
    @patch("opencodon.tools.terminal_tool._get_env_config")
    @patch("opencodon.tools.terminal_tool._create_environment")
    def test_recorded_cwd_used_for_recreated_env(
        ...
        from opencodon.tools.file_tools import _get_file_ops
