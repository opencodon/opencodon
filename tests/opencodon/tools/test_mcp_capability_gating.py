...
is non-None iff the server supports it. Before the capability gate, opencodon
...
class TestKeepaliveInterval:
    ...
    async def test_default_interval_when_unset(self):
        from opencodon.tools.mcp_tool import _DEFAULT_KEEPALIVE_INTERVAL
    ...
    async def test_interval_clamped_to_floor(self):
        from opencodon.tools.mcp_tool import _MIN_KEEPALIVE_INTERVAL
...
class TestMethodNotFoundDetection:
    ...
    def test_structural_code_match(self):
        from opencodon.tools.mcp_tool import _is_method_not_found_error
