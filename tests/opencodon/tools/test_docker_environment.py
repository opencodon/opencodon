...
def test_non_persistent_cleanup_removes_container(monkeypatch):
    ...
    docker rm so containers don't leak across opencodon processes.
...
def _make_execute_only_env(forward_env=None):
    ...
    env._snapshot_path = "/tmp/opencodon-snap-test123.sh"
    env._cwd_file = "/tmp/opencodon-cwd-test123.txt"
...
def test_init_env_args_uses_opencodon_dotenv_for_allowlisted_env(monkeypatch):
    ...
    monkeypatch.setattr(docker_env, "_load_opencodon_env_vars", lambda: {"DATABASE_URL": "value_from_dotenv"})
...
def test_init_env_args_prefers_shell_env_over_opencodon_dotenv(monkeypatch):
...
def test_init_env_args_uses_opencodon_dotenv_for_empty_shell_env(monkeypatch):
    ...
    monkeypatch.setattr(docker_env, "_load_opencodon_env_vars", lambda: {"MY_SECRET": "value_from_dotenv"})
...
def test_init_env_args_never_forwards_blank_secret(monkeypatch):
    ...
    monkeypatch.setenv("MY_SECRET", "")
    monkeypatch.setattr(docker_env, "_load_opencodon_env_vars", lambda: {})
...
def test_security_args_include_setuid_setgid_for_privdrop(monkeypatch):
    ...
    (e.g. via ``s6-setuidgid`` in the bundled opencodon image, or ``gosu``/``su``
...
def test_run_command_tags_opencodon_agent_label(monkeypatch):
    """Every container opencodon starts must carry the opencodon=1 label
    so the orphan reaper (and external operators) can identify them with a
    single ``docker ps --filter label=opencodon=1`` call. Regression test
    ...
    assert "opencodon=1" in labels, (
        f"opencodon=1 label missing; got labels: {sorted(labels)}"
    )
...
def test_run_command_tags_task_and_profile_labels(monkeypatch):
    ...
    returns ``"default"`` for the root opencodon home."""
    monkeypatch.setattr(docker_env, "find_docker", lambda: "/usr/bin/docker")
    ...
    labels = _labels_in_run_args(_run_args_from_calls(calls))
    assert "opencodon-task-id=task-42" in labels, (
        f"opencodon-task-id=task-42 missing; got: {sorted(labels)}"
    )
    assert "opencodon-profile=research-bot" in labels, (
        f"opencodon-profile=research-bot missing; got: {sorted(labels)}"
    )
...
def test_run_command_sanitizes_unsafe_task_id(monkeypatch):
    ...
    assert "opencodon-task-id=task_with_weird_chars" in labels, (
...
def test_labels_attribute_populated_after_init(monkeypatch):
    ...
    assert env._labels == {
        "opencodon": "1",
        "opencodon-task-id": "abc",
        "opencodon-profile": "default",
    }
...
def test_reuse_attaches_to_running_container_without_docker_run(monkeypatch):
    ...
    issue #20561 root cause: every opencodon process spawning a new container
...
def test_reuse_starts_stopped_container_before_attaching(monkeypatch):
    ...
    ``docker start`` before the new opencodon process uses it. Without this
...
def test_failed_docker_run_cleans_up_orphaned_container(monkeypatch):
    ...
    assert rm_cmd[3].startswith("opencodon-"), "should remove the container by its generated name"
...
def test_cleanup_with_persist_is_noop_for_container(monkeypatch):
    ...
    opencodon startup, not on graceful exit. Issue #20561 — the first iteration
...
def test_reap_orphan_removes_stale_exited_container(monkeypatch):
    ...
    SIGKILL'd opencodon processes leak containers permanently."""
...
def test_reap_orphan_spares_recently_exited_container(monkeypatch):
    ...
    container belongs to a opencodon process that just finished and may be
...
def test_reap_orphan_scopes_to_profile_filter_via_label(monkeypatch):
    """The reaper must pass ``--filter label=opencodon-profile=<profile>`` to
    ...
    assert "label=opencodon-profile=research-bot" in flat, (
        f"profile filter not applied to docker ps; got args: {ps_calls[0][0]}"
    )
    assert "label=opencodon=1" in flat, (
        f"opencodon label filter must also be applied; got: {ps_calls[0][0]}"
    )
    assert "status=exited" in flat, (
        "must filter to exited containers only — running containers may "
        "belong to a sibling opencodon process and must NEVER be reaped"
    )
...
def test_credential_mount_skipped_when_source_is_directory(monkeypatch, tmp_path, caplog):
    ...
    monkeypatch.setattr(
        "opencodon.tools.credential_files.get_credential_file_mounts",
        lambda: fake_mounts,
    )
    monkeypatch.setattr(
        "opencodon.tools.credential_files.get_skills_directory_mount",
        lambda: [],
    )
    monkeypatch.setattr(
        "opencodon.tools.credential_files.get_cache_directory_mounts",
        lambda: [],
    )
...
def test_image_uses_init_entrypoint_detects_s6_init(monkeypatch):
    ...
    monkeypatch.setattr(docker_env.subprocess, "run", _run)
    assert docker_env._image_uses_init_entrypoint("/usr/bin/docker", "opencodon:latest") is True
...
def test_s6_image_skips_docker_init_and_mounts_run_exec(monkeypatch):
    ...
    _make_dummy_env(image="opencodon:latest")
...
def test_is_container_gone_matches_removal_errors(monkeypatch):
    ...
    assert env._is_container_gone(
        "Error response from daemon: No such container: opencodon-abc123"
    )
...
def test_execute_recovers_from_out_of_band_removal(monkeypatch):
    ...
    outputs = iter([
        {"output": "Error response from daemon: No such container: opencodon-x", "returncode": 1},
