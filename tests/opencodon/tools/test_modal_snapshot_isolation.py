...
def _restore_tool_modules():
    original_opencodon_home = os.environ.get("OPENCODON_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "opencodon_cli"
    ...
    try:
        ...
        if original_opencodon_home is None:
            os.environ.pop("OPENCODON_HOME", None)
        else:
            os.environ["OPENCODON_HOME"] = original_opencodon_home
        _reset_modules(("tools", "opencodon_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    sys.modules["opencodon.config"] = types.SimpleNamespace(
    ...
    env_package = types.ModuleType("opencodon.tools.environments")
    env_package.__path__ = [str(TOOLS_DIR / "environments")]  # type: ignore[attr-defined]
    sys.modules["opencodon.tools.environments"] = env_package
    ...
    sys.modules["opencodon.tools.environments.base"] = types.SimpleNamespace(
    ...
    sys.modules["opencodon.tools.interrupt"] = types.SimpleNamespace(is_interrupted=lambda: False)
    sys.modules["opencodon.tools.credential_files"] = types.SimpleNamespace(
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="opencodon", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": opencodon_home / "modal_snapshots.json",
...
def test_modal_environment_migrates_legacy_snapshot_key_and_uses_snapshot_id(tmp_path):
    ...
    modal_module = _load_module("opencodon.tools.environments.modal", TOOLS_DIR / "environments" / "modal.py")
