...
from opencodon.tools.browser_camofox import (
    _drop_session,
    _get_session,
...
from opencodon.tools.browser_camofox_state import get_camofox_identity
...
def _enable_persistence():
    ...
    return patch("opencodon.tools.browser_camofox.load_config", return_value=config)


@pytest.fixture(autouse=True)
def _clear_session_state():
    import opencodon.tools.browser_camofox as mod
    yield
...
class TestManagedPersistenceToggle:
    def test_disabled_by_default(self):
        ...
        with patch("opencodon.tools.browser_camofox.load_config", return_value=config):
    ...
    def test_disabled_on_config_load_error(self):
        with patch("opencodon.tools.browser_camofox.load_config", side_effect=Exception("fail")):
            assert _managed_persistence_enabled() is False
...
class TestEphemeralMode:
    ...
    def test_session_gets_random_user_id(self, tmp_path, monkeypatch):
        ...
        assert session["user_id"].startswith("opencodon_")
...
class TestManagedPersistenceMode:
    """With managed_persistence: stable userId derived from opencodon profile."""
    ...
    def test_different_profiles_get_different_user_ids(self, tmp_path, monkeypatch):
        ...
        with _enable_persistence():
            monkeypatch.setenv("OPENCODON_HOME", str(tmp_path / "profile-a"))
            s1 = _get_session("task-1")
            uid_a = s1["user_id"]
            _drop_session("task-1")

            monkeypatch.setenv("OPENCODON_HOME", str(tmp_path / "profile-b"))
            s2 = _get_session("task-1")
    ...
    def test_navigate_uses_stable_identity(self, tmp_path, monkeypatch):
        ...
        with _enable_persistence(), \
             patch("opencodon.tools.browser_camofox.requests.post", side_effect=_capture_post):
    ...
    def test_navigate_reuses_identity_after_close(self, tmp_path, monkeypatch):
        ...
        with (
            _enable_persistence(),
            patch("opencodon.tools.browser_camofox.requests.post", side_effect=_capture_post),
            patch("opencodon.tools.browser_camofox.requests.delete", return_value=_mock_response()),
        ):
...
class TestConfiguredCamofoxIdentity:
    ...
    def test_env_identity_overrides_default_identity(self, tmp_path, monkeypatch):
        ...
        with patch("opencodon.tools.browser_camofox._get", return_value={"tabs": []}) as mock_get:
            session = _get_session("task-1")
    ...
    def test_adopts_existing_tab_matching_session_key(self, tmp_path, monkeypatch):
        ...
        with patch("opencodon.tools.browser_camofox._get", return_value=tabs):
            session = _get_session("task-1")
    ...
    def test_managed_persistence_can_opt_into_tab_adoption(self, tmp_path, monkeypatch):
        ...
        with (
            patch("opencodon.tools.browser_camofox.load_config", return_value=config),
            patch("opencodon.tools.browser_camofox._get", return_value={"tabs": [{"tabId": "tab-1"}]}),
        ):
            session = _get_session("task-1")
...
class TestVncUrlDiscovery:
    ...
    def test_vnc_url_from_health_port(self, monkeypatch):
        ...
        with patch("opencodon.tools.browser_camofox.requests.get", return_value=health_resp):
    ...
    def test_vnc_url_only_probed_once(self, monkeypatch):
        monkeypatch.setenv("CAMOFOX_URL", "http://localhost:9377")
        health_resp = _mock_response(json_data={"ok": True, "vncPort": 6080})
        with patch("opencodon.tools.browser_camofox.requests.get", return_value=health_resp) as mock_get:
            check_camofox_available()
            check_camofox_available()
    ...
    def test_navigate_includes_vnc_hint(self, tmp_path, monkeypatch):
        ...
        with patch("opencodon.tools.browser_camofox.requests.post", return_value=_mock_response(
...
class TestCamofoxSoftCleanup:
    ...
    def test_does_not_call_server_delete(self, tmp_path, monkeypatch):
        ...
        with (
            _enable_persistence(),
            patch("opencodon.tools.browser_camofox.requests.delete") as mock_delete,
        ):
            _get_session("task-1")
