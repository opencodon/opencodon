"""Integration tests for opencodon.tools.browser_supervisor.
...
Automated: skipped in CI unless ``OPENCODON_E2E_BROWSER=1`` is set.
"""
...
def chrome_cdp(request):
    ...
    profile = tempfile.mkdtemp(prefix="opencodon-supervisor-test-")
...
def supervisor_registry():
    ...
    from opencodon.tools.browser_supervisor import SUPERVISOR_REGISTRY
...
def test_auto_dismiss_policy(chrome_cdp, supervisor_registry):
    ...
    from opencodon.tools.browser_supervisor import DIALOG_POLICY_AUTO_DISMISS
...
def test_browser_dialog_tool_no_supervisor():
    ...
    from opencodon.tools.browser_dialog_tool import browser_dialog
...
def test_browser_cdp_frame_id_routes_via_supervisor(chrome_cdp, supervisor_registry, monkeypatch):
    ...
    import opencodon.tools.browser_supervisor as _bs
    with sv._state_lock:
    ...
    from opencodon.tools.browser_cdp_tool import browser_cdp
    result = browser_cdp(
