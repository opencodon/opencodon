...
@pytest.fixture
def isolated_opencodon_home(tmp_path, monkeypatch):
    """Point OPENCODON_HOME at a temp dir and clear known API key env vars.
...
def _write_env_file(home: Path, **kwargs) -> None:
    """Write key=value pairs to ~/.opencodon/.env."""
    lines = [f"{k}={v}" for k, v in kwargs.items()]
    (home / ".env").write_text("\n".join(lines) + "\n")


class TestCredentialPoolSeedsFromDotEnv:
    """_seed_from_env must read keys from ~/.opencodon/.env, not just os.environ.
    ...
    def test_openrouter_key_from_dotenv_only(self, isolated_opencodon_home):
    ...
    def test_dotenv_wins_over_stale_os_environ(self, isolated_opencodon_home, monkeypatch):
        """Regression for #20591: a fresh key rotated into ~/.opencodon/.env must
...
class TestAuthResolvesFromDotEnv:
    """_resolve_api_key_provider_secret must also read from ~/.opencodon/.env."""

    def test_key_from_dotenv_only(self, isolated_opencodon_home):
        """Key in .env but not os.environ → _resolve returns it with the env var source."""
        _write_env_file(isolated_opencodon_home, DEEPSEEK_API_KEY="sk-dotenv-resolve-789")
        assert "DEEPSEEK_API_KEY" not in os.environ

        from opencodon.core.credentials.auth import _resolve_api_key_provider_secret
    ...
    def test_dotenv_wins_over_stale_os_environ_on_resolve(
        self, isolated_opencodon_home, monkeypatch
    ):
        """Regression for #20591: when both ~/.opencodon/.env and os.environ define
        ...
        """
        _write_env_file(isolated_opencodon_home, DEEPSEEK_API_KEY="dotenv-fresh-deepseek")
    ...
    def test_get_anthropic_key_prefers_dotenv_over_stale_os_environ(
        ...
        prefer ~/.opencodon/.env over a stale shell export. This path resolves
        ...
        """
        _write_env_file(isolated_opencodon_home, ANTHROPIC_API_KEY="dotenv-fresh-anthropic")
...
class TestAuthCredentialPoolFallback:
    ...
    def test_credential_pool_fallback_structure(self, isolated_opencodon_home):
    ...
    def test_credential_pool_empty_returns_empty(self, isolated_opencodon_home):
    ...
    def test_env_var_takes_priority_over_pool(self, isolated_opencodon_home, monkeypatch):
        ...
        from opencodon.core.credentials.auth import _resolve_api_key_provider_secret
        with patch("opencodon.core.credentials.credential_pool.load_pool", return_value=mock_pool) as mp:
            key, source = _resolve_api_key_provider_secret(
                provider_id="deepseek",
                pconfig=_make_pconfig(),
            )
    ...
    def test_dotenv_takes_priority_over_pool(self, isolated_opencodon_home):
        """Key in .env beats credential pool — pool only fires when both env sources are empty."""
        _write_env_file(isolated_opencodon_home, DEEPSEEK_API_KEY="sk-dotenv-priority-xyz")
        assert "DEEPSEEK_API_KEY" not in os.environ
...
class TestAnthropicEnvAuthTypeClassification:
    ...
    def test_oauth_token_classified_as_oauth(self, isolated_opencodon_home):
        ...
        from opencodon.core.credentials.credential_pool import AUTH_TYPE_OAUTH
    ...
    def test_admin_key_classified_as_api_key(self, isolated_opencodon_home):
        ...
        """
        from opencodon.core.credentials.credential_pool import AUTH_TYPE_API_KEY
