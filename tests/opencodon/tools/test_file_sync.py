...
from opencodon.tools.environments.file_sync import FileSyncManager, _FORCE_SYNC_ENV, iter_sync_files
...
def _make_get_files(tmp_files, remote_base="/root/.opencodon"):
...
def _make_manager(tmp_files, remote_base="/root/.opencodon", upload=None, delete=None):
...
class TestRateLimiting:
    ...
    def test_failed_sync_does_not_suppress_next_retry(self, tmp_files, monkeypatch):
        ...
        """
        from opencodon.tools.environments import file_sync
...
class TestEdgeCases:
    ...
    def test_file_disappears_between_list_and_upload(self, tmp_path):
        ...
        upload = MagicMock()
        mgr = FileSyncManager(
            get_files_fn=lambda: [(str(f), "/root/.opencodon/ephemeral.txt")],
            upload_fn=upload,
            delete_fn=MagicMock(),
        )
...
class TestSyncBackSecurity:
    def test_sync_back_does_not_overwrite_uploaded_credential_files(self, tmp_path, monkeypatch):
            ...
            lambda: [
                {
                    ...
                    "container_path": "/root/.opencodon/credentials/token.json",
                }
            ],
        )
        monkeypatch.setattr(
            "opencodon.tools.credential_files.iter_skills_files",
            lambda container_base="/root/.opencodon": [
                {
                    "host_path": str(skill),
        ...
        monkeypatch.setattr(
            "opencodon.tools.credential_files.iter_cache_files",
            lambda container_base="/root/.opencodon": [],
        )
        ...
        def bulk_download(dest: Path) -> None:
            with tarfile.open(dest, "w") as tar:
                for name, data in {
                    "root/.opencodon/credentials/token.json": b"remote-token",
                    "root/.opencodon/skills/skill.py": b"remote-skill",
                }.items():
        ...
        mgr.sync(force=True)
        mgr.sync_back(opencodon_home=tmp_path)
