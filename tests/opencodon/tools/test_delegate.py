...
class TestStripBlockedTools(unittest.TestCase):
    ...
    def test_strip_set_derived_from_blocklist(self):
        ...
        """
        from opencodon.tools.delegate_tool import TOOLSETS, _strip_blocked_tools
    ...
    def test_mixed_composite_is_subtracted_at_child_assembly(self):
        ...
        ``opencodon-cli`` contains both allowed tools and every sensitive
        ...
        parent.enabled_toolsets = ["opencodon-cli"]
        parent.disabled_toolsets = ["browser"]

        with patch("opencodon.core.run_agent.AIAgent") as MockAgent:
    ...
    def test_orchestrator_composite_regains_only_delegate_task(self):
        ...
        with (
            ...
            patch("opencodon.tools.delegate_tool._get_orchestrator_enabled", return_value=True),
            patch("opencodon.tools.delegate_tool._get_max_spawn_depth", return_value=2),
        ):
            MockAgent.return_value = MagicMock()
            _build_child_agent(
                task_index=0,
...
class TestDelegateTask(unittest.TestCase):
    ...
    @patch("opencodon.tools.delegate_tool._run_single_child")
...
class TestToolNamePreservation(unittest.TestCase):
    ...
    def test_schema_never_exposes_acp_transport_fields(self):
        ...
        from opencodon.tools.delegate_tool import _build_dynamic_schema_overrides
...
class TestSubagentCostRollup(unittest.TestCase):
    ...
    def test_batch_children_costs_sum_into_parent(self):
        ...
        with patch("opencodon.tools.delegate_tool._run_single_child") as mock_run:
            mock_run.side_effect = [
                {
                    "task_index": 0,
                    "status": "completed",
...
class TestDelegationProviderIntegration(unittest.TestCase):
    ...
    @patch("opencodon.tools.delegate_tool._load_config")
    @patch("opencodon.tools.delegate_tool._resolve_delegation_credentials")
    ...
    def test_batch_mode_all_children_get_credentials(self, mock_creds, mock_cfg):
        ...
        with patch("opencodon.tools.delegate_tool._build_child_agent") as mock_build, \
             patch("opencodon.tools.delegate_tool._run_single_child") as mock_run:
            mock_child = MagicMock()
...
class TestChildCredentialPoolResolution(unittest.TestCase):
    ...
    def test_different_provider_loads_own_pool(self):
        ...
        with patch("opencodon.core.credentials.credential_pool.load_pool", return_value=mock_pool):
    ...
    def test_different_provider_load_failure_returns_none(self):
        ...
        with patch("opencodon.core.credentials.credential_pool.load_pool", side_effect=Exception("disk error")):
            result = _resolve_child_credential_pool("anthropic", parent)
    ...
    def test_custom_different_endpoint_does_not_inherit_parent_pool(self):
        ...
        with patch("opencodon.core.credentials.credential_pool.get_custom_provider_pool_key", side_effect=fake_key), \
             patch("opencodon.core.credentials.credential_pool.load_pool", return_value=child_pool) as load_mock:
    ...
    @patch("opencodon.tools.delegate_tool._load_config", return_value={})
    ...
    @patch(
        "opencodon.tools.delegate_tool._load_config",
...
class TestChildCredentialLeasing(unittest.TestCase):
    def test_run_single_child_acquires_and_releases_lease(self):
        from opencodon.tools.delegate_tool import _run_single_child
...
class TestDelegateHeartbeat(unittest.TestCase):
    ...
    def test_heartbeat_touches_parent_activity_during_child_run(self):
        ...
        with patch("opencodon.tools.delegate_tool._HEARTBEAT_INTERVAL", 0.05):
            _run_single_child(
                task_index=0,
                goal="Test heartbeat",
                child=child,
    ...
    def test_heartbeat_does_not_trip_idle_stale_while_inside_tool(self):
        ...
        with (
            patch("opencodon.tools.delegate_tool._HEARTBEAT_INTERVAL", 0.05),
            patch("opencodon.tools.delegate_tool._HEARTBEAT_STALE_CYCLES_IDLE", 2),
            patch("opencodon.tools.delegate_tool._HEARTBEAT_STALE_CYCLES_IN_TOOL", 40),
        ):
            _run_single_child(
                task_index=0,
...
class TestDelegationReasoningEffort(unittest.TestCase):
    ...
    @patch("opencodon.tools.delegate_tool._load_config")
    @patch("opencodon.core.run_agent.AIAgent")
...
class TestDispatchDelegateTask(unittest.TestCase):
    ...
    def test_model_acp_args_not_forwarded(self):
        ...
        parent = _make_mock_parent(depth=0)
        with patch("opencodon.tools.delegate_tool.delegate_task", fake_delegate_task):
...
class TestConcurrencyDefaults(unittest.TestCase):
    ...
    def test_load_config_prefers_active_persistent_config_over_cli_defaults(self):
        stale_cli = types.ModuleType("opencodon.frontends.cli.shell")
    ...
    def test_load_config_falls_back_to_cli_config_when_persistent_load_fails(self):
        fallback_cli = types.ModuleType("opencodon.frontends.cli.shell")
    ...
    def test_load_config_prefers_cli_config_when_user_config_ignored(self):
        ...
        ignoring_cli = types.ModuleType("opencodon.frontends.cli.shell")
...
class TestAsyncCapUnified(unittest.TestCase):
    ...
    def test_async_cap_follows_concurrent_children(self, mock_cfg):
        from opencodon.tools.delegate_tool import _get_max_async_children
...
class TestMaxSpawnDepth(unittest.TestCase):
    ...
    def test_max_spawn_depth_defaults_to_1(self, mock_cfg):
        from opencodon.tools.delegate_tool import _get_max_spawn_depth
    ...
    def test_max_spawn_depth_clamped_below_one(self, mock_cfg):
        import logging
        from opencodon.tools.delegate_tool import _get_max_spawn_depth
        with self.assertLogs("opencodon.tools.delegate_tool", level=logging.WARNING) as cm:
...
class TestOrchestratorRoleSchema(unittest.TestCase):
    ...
    def test_schema_has_role_top_level_and_per_task(self):
        from opencodon.tools.delegate_tool import DELEGATE_TASK_SCHEMA
        props = DELEGATE_TASK_SCHEMA["parameters"]["properties"]
...
class TestOrchestratorRoleBehavior(unittest.TestCase):
    ...
    def test_orchestrator_enabled_false_forces_leaf(self, mock_creds):
        ...
        parent = _make_mock_parent(depth=0)
        parent.enabled_toolsets = ["terminal", "delegation"]
        with patch("opencodon.tools.delegate_tool._load_config",
    ...
    def test_batch_mode_per_task_role_override(self, mock_cfg, mock_creds):
        ...
        with patch("opencodon.core.run_agent.AIAgent", side_effect=_factory):
            delegate_task(
                tasks=[
...
class TestOrchestratorEndToEnd(unittest.TestCase):
    ...
    def test_end_to_end_nested_orchestration(self, mock_cfg, mock_creds):
        ...
        with patch("opencodon.core.run_agent.AIAgent", side_effect=_factory) as MockAgent:
            delegate_task(
...
class TestSubagentApprovalCallback(unittest.TestCase):
    ...
    def test_auto_deny_returns_deny(self):
        from opencodon.tools.delegate_tool import _subagent_auto_deny
        self.assertEqual(
    ...
    def test_auto_approve_returns_once(self):
        from opencodon.tools.delegate_tool import _subagent_auto_approve
        self.assertEqual(
