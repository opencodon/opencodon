...
opencodon' existing ``cache_image_from_bytes`` so a ``MEDIA:<path>`` tag
...
def _png_bytes():
    ...
    opencodon' ``cache_image_from_bytes`` has a format-sniff guard that rejects
...
class TestMimeExtension:
    def test_maps_jpeg_variants_to_jpg(self):
        from opencodon.tools.mcp_tool import _mcp_image_extension_for_mime_type
