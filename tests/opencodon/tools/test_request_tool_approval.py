"""Tests for opencodon.tools.approval.request_tool_approval — the plugin pre_tool_call
...
def _isolate_approval_state(monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.tools.terminal_tool._get_approval_callback", lambda: None, raising=False
    )
    yield
...
class TestRequestToolApproval:
    ...
    def test_cron_deny_mode_blocks(self, monkeypatch):
        ...
        monkeypatch.setattr(approval, "env_var_enabled",
                            lambda v: v == "OPENCODON_CRON_SESSION")
