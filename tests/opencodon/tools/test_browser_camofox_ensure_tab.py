...
def test_ensure_tab_sends_list_item_id():
    ...
    from opencodon.tools import browser_camofox as mod

    fake_session = {
        "user_id": "opencodon_test123",
        "tab_id": None,
    ...
    with patch.object(mod, "_get_session", return_value=fake_session), \
         ...
         patch("opencodon.tools.browser_camofox.requests.post", return_value=mock_response) as mock_post:
    ...
    assert body["userId"] == "opencodon_test123"
...
def test_ensure_tab_skips_creation_when_tab_exists():
    ...
    with patch.object(mod, "_get_session", return_value=fake_session), \
         patch("opencodon.tools.browser_camofox.requests.post") as mock_post:
