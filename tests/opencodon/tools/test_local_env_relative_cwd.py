...
from opencodon.tools.environments.local import LocalEnvironment, _resolve_local_initial_cwd


def test_relative_initial_cwd_resolves_from_parent(tmp_path, monkeypatch):
    project = tmp_path / "opencodon"
    project.mkdir()
    monkeypatch.chdir(tmp_path)

    assert _resolve_local_initial_cwd("opencodon") == str(project)
...
def test_local_environment_does_not_cd_into_nested_matching_relative_cwd(tmp_path, monkeypatch):
    ...
    env = LocalEnvironment(cwd="opencodon", timeout=5)
    try:
    ...
    assert "cd: opencodon" not in result["output"]
