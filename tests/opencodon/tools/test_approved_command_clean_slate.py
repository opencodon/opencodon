...
from opencodon.tools.interrupt import (
    set_interrupt,
    is_interrupted,
...
def test_retry_backoff_does_not_clear_genuine_interrupt(monkeypatch):
    ...
    monkeypatch.setattr("opencodon.tools.terminal_tool.time.sleep", lambda *a, **k: None)
    set_interrupt(False)
...
def test_execute_code_approved_clears_stale_interrupt_bit(monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.tools.approval.check_execute_code_guard",
...
def test_execute_code_remote_clears_stale_bit(monkeypatch):
    ...
    from opencodon.tools import code_execution_tool as cet

    monkeypatch.setattr(
        "opencodon.tools.approval.check_execute_code_guard",
        lambda *a, **k: {"approved": True, "user_approved": True},
    )
    monkeypatch.setattr("opencodon.tools.terminal_tool._get_env_config", lambda *a, **k: {"env_type": "ssh"})
