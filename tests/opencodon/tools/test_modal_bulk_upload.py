...
from opencodon.tools.environments import modal as modal_env
...
class TestModalBulkUpload:
    ...
    def test_tar_archive_contains_all_files(self, monkeypatch, tmp_path):
        ...
        files = [
            (str(src_a), "/root/.opencodon/credentials/a.json"),
            (str(src_b), "/root/.opencodon/skills/b.py"),
        ]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            ...
            assert "root/.opencodon/credentials/a.json" in names
            assert "root/.opencodon/skills/b.py" in names

            # Verify content
            a_content = tar.extractfile("root/.opencodon/credentials/a.json").read()
            assert a_content == b"cred_content"
            b_content = tar.extractfile("root/.opencodon/skills/b.py").read()
    ...
    def test_mkdir_includes_all_parents(self, monkeypatch, tmp_path):
        ...
        files = [
            (str(src), "/root/.opencodon/credentials/f.txt"),
            (str(src), "/root/.opencodon/skills/deep/nested/f.txt"),
        ]
        ...
        cmd = exec_calls[0][2]
        assert "/root/.opencodon/credentials" in cmd
        assert "/root/.opencodon/skills/deep/nested" in cmd
    ...
    def test_single_exec_call(self, monkeypatch, tmp_path):
        ...
        for i in range(20):
            ...
            files.append((str(src), f"/root/.opencodon/cache/file_{i}.txt"))
    ...
    def test_bulk_upload_wired_in_filesyncmanager(self, monkeypatch):
        ...
        from opencodon.tools.environments.file_sync import iter_sync_files
        env._sync_manager = modal_env.FileSyncManager(
            get_files_fn=lambda: iter_sync_files("/root/.opencodon"),
    ...
    def test_timeout_set_to_120(self, monkeypatch, tmp_path):
        ...
        src = tmp_path / "f.txt"
        src.write_text("data")
        files = [(str(src), "/root/.opencodon/f.txt")]
    ...
    def test_stdin_chunked_for_large_payloads(self, monkeypatch, tmp_path):
        ...
        files = [(str(src), "/root/.opencodon/large.bin")]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            names = tar.getnames()
            assert "root/.opencodon/large.bin" in names
