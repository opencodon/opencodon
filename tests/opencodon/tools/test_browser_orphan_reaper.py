...
def fake_tmpdir(tmp_path):
    ...
    with patch("opencodon.tools.browser_tool._socket_safe_tmpdir", return_value=str(tmp_path)):
        yield tmp_path
...
def _make_socket_dir(tmpdir, session_name, pid=None, owner_pid=None):
    ...
    Args:
        ...
        owner_pid: owning opencodon PID to write to <session>.owner_pid
...
class TestReapOrphanedBrowserSessions:
    ...
    def test_no_socket_dirs_is_noop(self, fake_tmpdir):
        ...
        from opencodon.tools.browser_tool import _reap_orphaned_browser_sessions
    ...
    def test_orphaned_alive_daemon_is_killed(self, fake_tmpdir):
        ...
        with patch("opencodon.frontends.gateway.status._pid_exists", return_value=True), \
             patch("opencodon.tools.browser_tool._verify_reapable_browser_daemon", return_value=True), \
             patch("opencodon.tools.process_registry.ProcessRegistry._terminate_host_pid", side_effect=mock_terminate):
            _reap_orphaned_browser_sessions()
    ...
    def test_tracked_session_is_not_reaped(self, fake_tmpdir):
        ...
        with patch("opencodon.tools.process_registry.ProcessRegistry._terminate_host_pid", side_effect=mock_terminate):
            _reap_orphaned_browser_sessions()
    ...
    def test_non_opencodon_dirs_are_ignored(self, fake_tmpdir):
...
class TestOwnerPidCrossProcess:
    ...
    The owner_pid file records which opencodon process owns a daemon so that
    concurrent opencodon processes don't reap each other's active browser
    ...
    def test_dead_owner_triggers_reap(self, fake_tmpdir):
        ...
        with patch("opencodon.frontends.gateway.status._pid_exists",
    ...
    def test_run_browser_command_calls_write_owner_pid(
        ...
        with patch("opencodon.tools.browser_tool._socket_safe_tmpdir", return_value=str(fake_tmpdir)):
            try:
...
class TestReaperIdentityGuard:
    ...
    *this* session's agent-browser daemon, defeating planted pid files and
    ...
    def _run(self, fake_proc, socket_dir, session_name="h_sess123456",
        ...
        import psutil
        from opencodon.tools.browser_tool import _verify_reapable_browser_daemon
    ...
    def test_planted_pid_survives_full_reaper_path(self, fake_tmpdir):
        ...
        with patch("opencodon.frontends.gateway.status._pid_exists", return_value=True), \
             ...
             patch("opencodon.tools.process_registry.ProcessRegistry._terminate_host_pid",
