...
def _run_terminal(
    ...
    *,
    ...
    if invoke_hook is not _UNSET:
        monkeypatch.setattr("opencodon.plugins_runtime.invoke_hook", invoke_hook)
...
def test_large_process_output_is_bounded_before_sudo_and_plugin_hooks(
    ...
    limit = 10_000
    monkeypatch.setattr("opencodon.tools.tool_output_limits.get_max_bytes", lambda: limit)
    monkeypatch.setattr(
        terminal_tool_module, "_get_env_config", lambda: _make_env_config(tmp_path)
    )
    monkeypatch.setattr(terminal_tool_module, "_start_cleanup_thread", lambda: None)
    ...
    monkeypatch.setattr("opencodon.plugins_runtime.invoke_hook", _hook_spy)
