...
Older images made /opt/opencodon/apps/tui and /opt/opencodon/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV OPENCODON_TUI_DIR=/opt/opencodon/apps/tui" in text
    assert "cd apps/tui && npm run build" in text
    assert "chown -R opencodon:opencodon /opt/opencodon/apps/tui" not in text
    assert "chown -R opencodon:opencodon /opt/opencodon/node_modules" not in text
