...
from opencodon.tools import tool_output_limits as tol
...
class TestDefaults:
    ...
    def test_get_limits_returns_defaults_when_config_not_a_dict(self):
        ...
        with patch("opencodon.config.load_config", return_value="not a dict"):
            limits = tol.get_tool_output_limits()
    ...
    def test_get_limits_returns_defaults_when_load_config_raises(self):
        ...
        with patch("opencodon.config.load_config", side_effect=_boom):
            limits = tol.get_tool_output_limits()
...
class TestDefaultConfigHasSection:
    """The DEFAULT_CONFIG in opencodon.config must expose tool_output so
...
class TestIntegrationReadPagination:
    ...
    def test_pagination_limit_clamped_by_config_value(self):
        from opencodon.tools.file_operations import normalize_read_pagination
