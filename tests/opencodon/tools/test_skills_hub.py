...
class TestParseFrontmatterQuick:
    ...
    def test_nested_yaml(self):
        content = "---\nname: test\nmetadata:\n  opencodon:\n    tags: [a, b]\n---\n\nBody.\n"
        fm = GitHubSource._parse_frontmatter_quick(content)
        assert fm["metadata"]["opencodon"]["tags"] == ["a", "b"]
...
class TestSkillsShGroupings:
    ...
    def test_list_skills_stamps_category_from_sidecar(self):
        ...
        with patch.object(src, "_read_cache", return_value=None), \
             ...
             patch("opencodon.tools.skills_hub.httpx.get", return_value=resp):
...
class TestTrustLevelFor:
    ...
    def test_trusted_repo(self):
        ...
        from opencodon.tools.skills_guard import TRUSTED_REPOS
        if TRUSTED_REPOS:
    ...
    def test_nvidia_skills_tap_is_registered_and_trusted(self):
        ...
        # browseable/searchable through `opencodon skills browse` must also
    ...
    def test_browseable_trusted_repos_have_taps(self):
        ...
        for repo in TRUSTED_REPOS:
            assert repo in tap_repos, (
                ...
                "browsable via `opencodon skills browse`."
            )
...
class TestSkillsShSource:
    ...
    @patch("opencodon.tools.skills_hub._write_index_cache")
    @patch("opencodon.tools.skills_hub._read_index_cache", return_value=None)
    @patch("opencodon.tools.skills_hub.httpx.get")
...
class TestWellKnownSkillSource:
    ...
    def _allow_public_skill_fetches(self, monkeypatch):
        monkeypatch.setattr("opencodon.tools.skills_hub.is_safe_url", lambda _url: True)
        monkeypatch.setattr("opencodon.tools.skills_hub.check_website_access", lambda _url: None)
    ...
    @patch("opencodon.tools.skills_hub._write_index_cache")
    @patch("opencodon.tools.skills_hub._read_index_cache", return_value=None)
    @patch("opencodon.tools.skills_hub._ssrf_safe_http_get")
...
class TestUrlSource:
    ...
    @patch("opencodon.tools.skills_hub._ssrf_safe_http_get")
    @patch("opencodon.tools.skills_hub.check_website_access", return_value=None)
    @patch("opencodon.tools.skills_hub.is_safe_url", return_value=False)
    ...
    @patch("opencodon.tools.skills_hub._ssrf_safe_http_get")
    @patch("opencodon.tools.skills_hub.check_website_access", return_value=None)
    @patch("opencodon.tools.skills_hub.is_safe_url", side_effect=[True, False])
    ...
    @patch("opencodon.tools.skills_hub._ssrf_safe_http_get")
    @patch("opencodon.tools.skills_hub.check_website_access", return_value=None)
    @patch("opencodon.tools.skills_hub.is_safe_url", return_value=True)
    def test_fetch_blocks_connect_time_dns_rebind(self, _mock_safe, _mock_policy, mock_get):
        from opencodon.tools.url_safety import SSRFConnectionBlocked
...
class TestCheckForSkillUpdates:
    def test_bundle_content_hash_matches_installed_content_hash(self, tmp_path):
        from opencodon.tools.skills_guard import content_hash
...
class TestGithubProviderLabeling:
    def test_provider_for_known_taps_case_insensitive(self):
        from opencodon.tools.skills_hub import github_provider_for
...
def _make_index_source(skills):
    """Build a OpencodonIndexSource pre-loaded with a fixed skill list."""
    from opencodon.tools.skills_hub import OpencodonIndexSource
    src = OpencodonIndexSource(auth=GitHubAuth())
    src._index = {"skills": skills}
    src._loaded = True
    return src


class TestOpencodonIndexSearch:
...
class TestProviderFilter:
    def test_filter_results_by_provider_narrows_exactly(self):
        from opencodon.tools.skills_hub import _filter_results_by_provider
        results = [
    ...
    def test_provider_filter_values_match_tap_labels(self):
        from opencodon.tools.skills_hub import _PROVIDER_FILTER_VALUES, GITHUB_TAP_PROVIDERS
    ...
    def test_unified_search_provider_filter_keeps_index_source(self):
        ...
        src = MagicMock()
        src.source_id.return_value = "opencodon-index"
...
class TestAppendAuditLog:
    def test_creates_log_entry(self, tmp_path):
        ...
        with patch("opencodon.tools.skills_hub.AUDIT_LOG", log_file):
...
class TestQuarantineBundleBinaryAssets:
    def test_quarantine_bundle_writes_binary_files(self, tmp_path):
        import opencodon.tools.skills_hub as hub
...
class TestInstallPathSafety:
    ...
    def isolated_skills_dir(self, tmp_path, monkeypatch):
        ...
        skills_dir.mkdir()
        monkeypatch.setattr("opencodon.tools.skills_hub.SKILLS_DIR", skills_dir)
        return skills_dir
    ...
    def test_install_from_quarantine_rejects_symlinks(self, tmp_path):
        ...
        import opencodon.tools.skills_hub as hub
        from opencodon.tools.skills_guard import ScanResult
...
# ---------------------------------------------------------------------------
# _load_opencodon_index — centralized index fetch (Browse-hub landing / search)
# ---------------------------------------------------------------------------


class TestLoadOpencodonIndex:
    ...
    def _isolate_cache(monkeypatch, tmp_path):
        ...
        cache_file = tmp_path / "opencodon-index.json"
        monkeypatch.setattr(hub, "_opencodon_index_cache_file", lambda: cache_file)
        return cache_file
    ...
    def test_fetch_does_not_request_brotli(self, monkeypatch, tmp_path):
        ...
        data = hub._load_opencodon_index()
    ...
    def test_persistent_decoding_error_falls_back_to_stale_cache(
        ...
        old = time.time() - (hub.OPENCODON_INDEX_TTL + 100)
        import os
