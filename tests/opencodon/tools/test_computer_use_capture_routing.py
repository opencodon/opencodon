...
* ``opencodon_constants.get_opencodon_dir`` (cache path)
...
def tmp_cache_dir(tmp_path):
    """Override get_opencodon_dir so cache writes land under tmp_path."""
    ...
    with patch("opencodon_constants.get_opencodon_dir", _fake_get):
        yield cache_dir
...
class TestCaptureResponseRoutedToAuxVision:
    ...
    def test_som_capture_returns_text_with_vision_analysis(
        ...
        with patch.object(cu_tool, "_should_route_through_aux_vision",
                          return_value=True), \
             patch("opencodon.tools.model_tools._run_async", side_effect=_fake_run_async), \
             patch("opencodon.tools.vision_tools.vision_analyze_tool",
    ...
    def test_aux_route_creates_missing_cache_dir(self, tmp_path):
        ...
        with patch.object(cu_tool, "_should_route_through_aux_vision",
                          return_value=True), \
             patch("opencodon_constants.get_opencodon_dir", _fake_get), \
             patch("opencodon.tools.model_tools._run_async", side_effect=_fake_run_async), \
             patch("opencodon.tools.vision_tools.vision_analyze_tool",
                   new_callable=lambda: fake_vat):
            resp = cu_tool._capture_response(cap)
...
class TestRoutingDecisionWiring:
    ...
    def test_no_explicit_aux_and_vision_capable_main_keeps_multimodal(self):
        ...
        with patch("opencodon.core.auxiliary_client._read_main_provider",
             ...
             patch("opencodon.config.load_config", return_value=cfg), \
             patch("opencodon.tools.computer_use.vision_routing._lookup_supports_vision",
                   return_value=True), \
             patch("opencodon.tools.computer_use.vision_routing."
    ...
    def test_helper_decision_exception_is_swallowed(self):
        from opencodon.tools.computer_use import tool as cu_tool
        from opencodon.tools.computer_use import vision_routing as vr_mod
