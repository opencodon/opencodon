...
from opencodon.tools.delegate_tool import _expand_parent_toolsets
...
class TestExpandParentToolsets(unittest.TestCase):
    ...
    def test_composite_opencodon_cli_expands_web(self):
        """opencodon-cli includes web_search/web_extract → 'web' should be in expansion."""
        expanded = _expand_parent_toolsets({"opencodon-cli"})
        self.assertIn("web", expanded)
        self.assertIn("terminal", expanded)
        self.assertIn("browser", expanded)
        # Original composite is preserved
        self.assertIn("opencodon-cli", expanded)
    ...
    def test_intersection_with_expanded_composite(self):
        """End-to-end: requesting ['web'] from parent with ['opencodon-cli'] yields ['web']."""
        parent_toolsets = {"opencodon-cli"}
