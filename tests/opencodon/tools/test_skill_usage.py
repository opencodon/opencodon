...
def _bump_view_many(opencodon_home: str, skill_name: str, iterations: int) -> None:
    os.environ["OPENCODON_HOME"] = opencodon_home
    from opencodon.tools.skill_usage import bump_view
...
def skills_home(tmp_path, monkeypatch):
    """Isolated OPENCODON_HOME with a clean skills/ dir for each test.
    ...
    import importlib
    import opencodon.tools.skill_usage as mod
...
def test_save_and_load_roundtrip(skills_home):
    from opencodon.tools.skill_usage import load_usage, save_usage
...
def test_save_is_atomic_no_partial_tmp_files(skills_home):
    from opencodon.tools.skill_usage import save_usage, _usage_file
...
def test_get_record_missing_returns_empty_record(skills_home):
    from opencodon.tools.skill_usage import get_record
...
def test_get_record_backfills_missing_keys(skills_home):
    from opencodon.tools.skill_usage import get_record, save_usage
...
def test_load_usage_handles_corrupt_file(skills_home):
    from opencodon.tools.skill_usage import load_usage, _usage_file
...
def test_bump_view_increments_and_timestamps(skills_home):
    from opencodon.tools.skill_usage import bump_view, get_record
    bump_view("my-skill")
    bump_view("my-skill")
...
def test_bump_use_increments_and_timestamps(skills_home):
    from opencodon.tools.skill_usage import bump_use, get_record
    bump_use("my-skill")
    rec = get_record("my-skill")
...
def test_bump_patch_increments_and_timestamps(skills_home):
    from opencodon.tools.skill_usage import bump_patch, get_record
    bump_patch("my-skill")
    rec = get_record("my-skill")
...
def test_bump_on_empty_name_is_noop(skills_home):
    from opencodon.tools.skill_usage import bump_view, load_usage
    bump_view("")
    assert load_usage() == {}


def test_bumps_do_not_corrupt_other_skills(skills_home):
    from opencodon.tools.skill_usage import bump_view, bump_use, get_record
    bump_view("skill-a")
    bump_use("skill-b")
    bump_view("skill-a")
...
def test_set_state_active(skills_home):
    from opencodon.tools.skill_usage import set_state, get_record, STATE_ACTIVE
    set_state("x", STATE_ACTIVE)
    assert get_record("x")["state"] == "active"


def test_set_state_archived_records_timestamp(skills_home):
    from opencodon.tools.skill_usage import set_state, get_record, STATE_ARCHIVED
...
def test_set_state_invalid_is_noop(skills_home):
    from opencodon.tools.skill_usage import set_state, get_record
    set_state("x", "bogus")
...
def test_restoring_from_archive_clears_timestamp(skills_home):
    from opencodon.tools.skill_usage import set_state, get_record, STATE_ARCHIVED, STATE_ACTIVE
    set_state("x", STATE_ARCHIVED)
...
def test_set_pinned(skills_home):
    from opencodon.tools.skill_usage import set_pinned, get_record
    set_pinned("x", True)
...
def test_forget_removes_record(skills_home):
    from opencodon.tools.skill_usage import bump_view, forget, load_usage
    bump_view("x")
...
def test_agent_created_excludes_bundled(skills_home):
    from opencodon.tools.skill_usage import list_agent_created_skill_names, mark_agent_created
...
def test_is_agent_created(skills_home):
    from opencodon.tools.skill_usage import is_agent_created
    skills_dir = skills_home / "skills"
...
def test_archive_skill_moves_directory(skills_home):
    from opencodon.tools.skill_usage import archive_skill, get_record
    skills_dir = skills_home / "skills"
...
def test_archive_refuses_bundled_skill(skills_home):
    from opencodon.tools.skill_usage import archive_skill
    skills_dir = skills_home / "skills"
...
def test_restore_skill_moves_back(skills_home):
    from opencodon.tools.skill_usage import archive_skill, restore_skill, get_record
    skills_dir = skills_home / "skills"
...
def test_restore_skill_finds_nested_archive_subdir(skills_home):
    ...
    from opencodon.tools.skill_usage import restore_skill, get_record
    skills_dir = skills_home / "skills"
...
def test_restore_skill_finds_nested_timestamped_prefix(skills_home):
    ...
    from opencodon.tools.skill_usage import restore_skill
    skills_dir = skills_home / "skills"
...
def test_agent_created_report_includes_marked_skills_with_defaults(skills_home):
    from opencodon.tools.skill_usage import agent_created_report, bump_view, mark_agent_created
    skills_dir = skills_home / "skills"
...
def test_manual_skill_with_usage_is_not_curator_managed(skills_home):
    from opencodon.tools.skill_usage import agent_created_report, bump_view, list_agent_created_skill_names
    skills_dir = skills_home / "skills"
...
def test_agent_created_report_excludes_bundled_and_hub(skills_home):
    from opencodon.tools.skill_usage import agent_created_report, mark_agent_created
    skills_dir = skills_home / "skills"
...
def test_bump_use_tracks_hub_skill(skills_home):
    from opencodon.tools.skill_usage import bump_use, load_usage
    skills_dir = skills_home / "skills"
    _write_skill(skills_dir, "from-hub")
    hub = skills_dir / ".hub"
    hub.mkdir()
...
def test_set_state_no_op_for_bundled_skill(skills_home):
    ...
    from opencodon.tools.skill_usage import set_state, load_usage, STATE_ARCHIVED
    skills_dir = skills_home / "skills"
    (skills_dir / ".bundled_manifest").write_text(
        "locked:abc\n", encoding="utf-8",
    )
...
def test_restore_refuses_to_shadow_bundled_skill(skills_home):
    ...
    from opencodon.tools.skill_usage import archive_skill, restore_skill
    skills_dir = skills_home / "skills"
