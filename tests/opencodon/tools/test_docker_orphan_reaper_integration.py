...
def test_maybe_reap_runs_once_per_process(monkeypatch):
    ...
    with patch("opencodon.tools.environments.docker.reap_orphan_containers", _fake_reap):
...
def test_maybe_reap_respects_disable_flag(monkeypatch):
    ...
    hatch for operators running multiple opencodon processes in the same
    profile."""
    _reset_reaper_gate()
    call_count = {"reap": 0}
...
def test_maybe_reap_doubles_lifetime_for_max_age(monkeypatch):
    ...
    floor). Generous default — gives sibling opencodon processes ample grace
...
def test_maybe_reap_passes_current_profile_as_filter(monkeypatch):
    """The reaper must be scoped to the current opencodon profile — a research
    ...
    with patch("opencodon.tools.environments.docker.reap_orphan_containers", _fake_reap), \
         patch("opencodon.tools.environments.docker._get_active_profile_name", return_value="research-bot"):
        terminal_tool._maybe_reap_docker_orphans({"docker_orphan_reaper": True})
...
def test_maybe_reap_swallows_exceptions(monkeypatch):
    ...
    with patch("opencodon.tools.environments.docker.reap_orphan_containers", _exploding_reap):
        # Must not raise
        terminal_tool._maybe_reap_docker_orphans({"docker_orphan_reaper": True})
