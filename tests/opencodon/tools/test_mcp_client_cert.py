...
class TestResolveClientCert:
    def test_returns_none_when_unset(self):
        from opencodon.tools.mcp_tool import _resolve_client_cert
    ...
    def test_cert_forwarded_to_async_client(self, tmp_path):
        ...
        async def _drive():
            with patch("opencodon.tools.mcp_tool._MCP_HTTP_AVAILABLE", True), \
                 patch("opencodon.tools.mcp_tool._MCP_NEW_HTTP", True), \
                 patch("httpx.AsyncClient", DummyAsyncClient), \
                 patch("opencodon.tools.mcp_tool.streamable_http_client",
...
def patch_sse_client():
    ...
    with patch("opencodon.tools.mcp_tool.sse_client", new=fake_sse_client), \
         patch("opencodon.tools.mcp_tool.ClientSession", new=_FakeSession):
