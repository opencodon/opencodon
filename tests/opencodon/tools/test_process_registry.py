...
from opencodon.tools.environments.local import _OPENCODON_PROVIDER_ENV_FORCE_PREFIX
from opencodon.tools.process_registry import (
    ProcessRegistry,
    ProcessSession,
...
def test_write_stdin_uses_str_for_windows_pty(monkeypatch, registry):
    ...
    monkeypatch.setattr("opencodon.tools.process_registry._IS_WINDOWS", True)
...
def test_write_stdin_uses_bytes_for_posix_pty(monkeypatch, registry):
    ...
    session._pty = _FakePty()
    registry._running[session.id] = session
    monkeypatch.setattr("opencodon.tools.process_registry._IS_WINDOWS", False)
...
def test_close_terminal_tool_requires_process_id():
    ...
    from opencodon.tools.close_terminal_tool import close_terminal_tool
...
def test_close_terminal_tool_routes_to_registry(monkeypatch):
    ...
    import opencodon.tools.close_terminal_tool as ct
...
class TestOrphanedPipeReconciliation:
    ...
    `opencodon update` in Feishu spawned a background subprocess that restarted
...
class TestSpawnEnvSanitization:
    def test_spawn_local_strips_blocked_vars_from_background_env(self, registry):
        ...
        with patch.dict(os.environ, {
            ...
            patch("opencodon.tools.process_registry._find_shell", return_value="/bin/bash"), \
    ...
    def test_spawn_via_env_uses_backend_temp_dir_for_artifacts(self, registry):
        ...
        with patch("opencodon.tools.process_registry.threading.Thread", return_value=fake_thread), \
            patch.object(registry, "_write_checkpoint"):
            session = registry.spawn_via_env(env, "echo hello")

        bg_command = env.commands[0][0]
        assert session.pid == 4321
        assert "/data/data/com.termux/files/usr/tmp/opencodon_bg_" in bg_command
        assert ".exit" in bg_command
        assert "rc=$?;" in bg_command
        assert " > /tmp/opencodon_bg_" not in bg_command
        assert "cat /tmp/opencodon_bg_" not in bg_command
    ...
    def test_env_poller_quotes_temp_paths_with_spaces(self, registry):
        ...
        with patch("opencodon.tools.process_registry.time.sleep", return_value=None), \
            patch.object(registry, "_move_to_finished"):
            registry._env_poller_loop(
                session,
                env,
                "/path with spaces/opencodon_bg.log",
                "/path with spaces/opencodon_bg.pid",
                "/path with spaces/opencodon_bg.exit",
            )

        assert env.commands[0][0] == "cat '/path with spaces/opencodon_bg.log' 2>/dev/null"
        assert env.commands[1][0] == "kill -0 \"$(cat '/path with spaces/opencodon_bg.pid' 2>/dev/null)\" 2>/dev/null; echo $?"
        assert env.commands[2][0] == "cat '/path with spaces/opencodon_bg.exit' 2>/dev/null"
...
class TestPopenLeakOnSetupFailure:
    ...
    def test_popen_killed_when_thread_creation_fails(self, registry):
        ...
        with patch("opencodon.tools.process_registry._find_shell", return_value="/bin/bash"), \
...
class TestCheckpoint:
    def test_write_checkpoint(self, registry, tmp_path):
        with patch("opencodon.tools.process_registry.CHECKPOINT_PATH", tmp_path / "procs.json"):
            s = _make_session()
            registry._running[s.id] = s
    ...
    def test_recover_no_file(self, registry, tmp_path):
        with patch("opencodon.tools.process_registry.CHECKPOINT_PATH", tmp_path / "missing.json"):
    ...
    def test_recover_dead_pid(self, registry, tmp_path):
        ...
        with patch("opencodon.tools.process_registry.CHECKPOINT_PATH", checkpoint):
...
class TestKillProcess:
    ...
    def test_kill_detached_session_uses_host_pid(self, registry):
        ...
        try:
            ...
            with patch("opencodon.frontends.gateway.status._pid_exists", return_value=True), \
...
class TestProcessToolHandler:
    def test_list_action(self):
        from opencodon.tools.process_registry import _handle_process
...
class TestTerminateHostPidWindows:
    ...
    def test_windows_invokes_taskkill_with_tree_and_force_flags(self, monkeypatch):
        ...
        from opencodon.tools import process_registry as pr
