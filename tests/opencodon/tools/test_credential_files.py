...
def _clean_state():
    ...
    import opencodon.tools.credential_files as _cred_mod
    clear_credential_files()
...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        (opencodon_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(opencodon_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.opencodon/token.json"
    ...
    def test_string_entry(self, tmp_path):
        opencodon_home = tmp_path / ".opencodon"
        opencodon_home.mkdir()
        (opencodon_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        opencodon_home = tmp_path / ".opencodon"
        opencodon_home.mkdir()
        (opencodon_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.opencodon/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        opencodon_home = tmp_path / ".opencodon"
        (opencodon_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"OPENCODON_HOME": str(opencodon_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.opencodon")

        assert mounts[0]["container_path"] == "/home/user/.opencodon/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.opencodon/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.opencodon/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape OPENCODON_HOME."""
        monkeypatch.setenv("OPENCODON_HOME", str(tmp_path / ".opencodon"))
        (tmp_path / ".opencodon").mkdir()

        # Create a sensitive file one level above opencodon_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside opencodon_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside OPENCODON_HOME must still be registered."""
        opencodon_home = tmp_path / ".opencodon"
        opencodon_home.mkdir()
        monkeypatch.setenv("OPENCODON_HOME", str(opencodon_home))
        (opencodon_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_opencodon_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of OPENCODON_HOME must be allowed."""
        opencodon_home = tmp_path / ".opencodon"
        opencodon_home.mkdir()
        subdir = opencodon_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside OPENCODON_HOME pointing outside must be rejected."""
        opencodon_home = tmp_path / ".opencodon"
        opencodon_home.mkdir()
        monkeypatch.setenv("OPENCODON_HOME", str(opencodon_home))

        # Create a sensitive file outside opencodon_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside opencodon_home pointing outside
        symlink = opencodon_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes OPENCODON_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, opencodon_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape OPENCODON_HOME."""
        opencodon_home = tmp_path / ".opencodon"
        opencodon_home.mkdir()
        monkeypatch.setenv("OPENCODON_HOME", str(opencodon_home))
        ...
        sensitive.write_text("{}")
        self._write_config(opencodon_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(opencodon_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside OPENCODON_HOME via config must still mount."""
        opencodon_home = tmp_path / ".opencodon"
        opencodon_home.mkdir()
        monkeypatch.setenv("OPENCODON_HOME", str(opencodon_home))

        (opencodon_home / "oauth.json").write_text("{}")
        self._write_config(opencodon_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        opencodon_home = tmp_path / ".opencodon"
        opencodon_home.mkdir()
        (opencodon_home / "cache" / "documents").mkdir(parents=True)
        (opencodon_home / "cache" / "audio").mkdir(parents=True)
        (opencodon_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("OPENCODON_HOME", str(opencodon_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.opencodon/cache/documents" in paths
        assert "/root/.opencodon/cache/audio" in paths
        assert "/root/.opencodon/cache/videos" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.opencodon/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_opencodon_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        opencodon_home = tmp_path / ".opencodon"
        opencodon_home.mkdir()
        # Use legacy dir name with content — get_opencodon_dir prefers
        # populated old over new.
        legacy_doc = opencodon_home / "document_cache"
        legacy_img = opencodon_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(opencodon_home / "document_cache") in host_paths
        assert str(opencodon_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.opencodon/cache/documents" in container_paths
        assert "/root/.opencodon/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        opencodon_home = tmp_path / ".opencodon"
        img_dir = opencodon_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.opencodon/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.opencodon")
            == "/home/agent/.opencodon/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        opencodon_home = tmp_path / ".opencodon"
        (opencodon_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("OPENCODON_HOME", str(opencodon_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(opencodon_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        opencodon_home = tmp_path / ".opencodon"
        doc_dir = opencodon_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        opencodon_home = tmp_path / ".opencodon"
        ss_dir = opencodon_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.opencodon/cache/screenshots/session_abc/screen1.png"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — OPENCODON_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping OPENCODON_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"OPENCODON_HOME": str(home)}):
    ...
    def test_per_service_token_still_mounts(self, tmp_path):
        ...
        assert [m["container_path"] for m in mounts] == [
            "/root/.opencodon/google_token.json"
        ]
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.opencodon/google_token.json" in paths
        assert "/root/.opencodon/.env" not in paths
    ...
    def test_missing_guard_fails_closed_with_error_log(self, tmp_path, caplog):
        ...
        """
        import opencodon.tools.credential_files as cf

        home = self._home(tmp_path)
        with patch.dict(os.environ, {"OPENCODON_HOME": str(home)}), \
                patch.object(cf, "get_read_block_error", None):
            with caplog.at_level("ERROR", logger="opencodon.tools.credential_files"):
