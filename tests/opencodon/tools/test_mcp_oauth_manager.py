...
def test_manager_isolates_same_named_servers_by_profile_home(tmp_path, monkeypatch):
    ...
    from opencodon.tools.mcp_oauth_manager import MCPOAuthManager
...
def test_manager_explicit_home_removes_only_that_profiles_tokens(tmp_path):
    ...
    token = set_opencodon_home_override(profile_a)
    try:
        MCPOAuthManager().remove("shared", opencodon_home=profile_b)
    finally:
        reset_opencodon_home_override(token)
...
def test_opencodon_provider_subclass_exists():
    ...
    assert issubclass(_OPENCODON_PROVIDER_CLS, OAuthClientProvider)
...
async def test_disk_watch_invalidates_on_mtime_change(tmp_path, monkeypatch):
    ...
    """
    monkeypatch.setenv("OPENCODON_HOME", str(tmp_path))
    from opencodon.tools.mcp_oauth_manager import MCPOAuthManager, reset_manager_for_tests
...
async def test_handle_401_tracks_inflight_task_to_prevent_gc(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OPENCODON_HOME", str(tmp_path))
    from opencodon.tools.mcp_oauth_manager import MCPOAuthManager, _ProviderEntry
...
def test_manager_builds_opencodon_provider_subclass(tmp_path, monkeypatch):
    ...
    from opencodon.tools.mcp_oauth_manager import (
