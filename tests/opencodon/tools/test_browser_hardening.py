...
class TestCommandTimeoutCache:
    ...
    def test_default_is_30(self):
        from opencodon.tools.browser_tool import _get_command_timeout
...
class TestSessionInactivityTimeout:
    ...
    def test_default_matches_config_default(self, monkeypatch):
        ...
        from opencodon.tools.browser_tool import _get_session_inactivity_timeout
...
class TestHomebrewNodeDirsCache:
    ...
    def test_lru_cached(self):
        from opencodon.tools.browser_tool import _discover_homebrew_node_dirs
...
class TestUrlDecodedSecretCheck:
    ...
    def test_encoded_key_blocked_in_navigate(self):
        ...
        import urllib.parse
        from opencodon.tools.browser_tool import browser_navigate
        import json
...
class TestTruncateSnapshot:
    ...
    def test_short_snapshot_unchanged(self):
        from opencodon.tools.browser_tool import _truncate_snapshot
        short = '- heading "Example" [ref=e1]\n- link "More" [ref=e2]'
        assert _truncate_snapshot(short) == short

    def test_long_snapshot_truncated_at_line_boundary(self):
        from opencodon.tools.browser_tool import SNAPSHOT_SUMMARIZE_THRESHOLD, _truncate_snapshot
    ...
    def test_threshold_aligned_with_web_extract_budget(self):
        ...
        from opencodon.tools.browser_tool import SNAPSHOT_SUMMARIZE_THRESHOLD
        from opencodon.tools.web_tools import DEFAULT_EXTRACT_CHAR_LIMIT
    ...
    def test_truncation_survives_storage_failure(self):
        ...
        lines = [f"- line {i}" for i in range(100)]
        snapshot = "\n".join(lines)
        with patch("opencodon.tools.browser_tool._store_full_snapshot", return_value=None):
            result = _truncate_snapshot(snapshot, max_chars=200)
        assert "truncated" in result.lower()
    ...
    def test_stored_snapshot_is_secret_redacted(self):
        ...
        from pathlib import Path
        from opencodon.tools.browser_tool import _store_full_snapshot
    ...
    def test_extract_relevant_content_appends_stored_pointer(self):
        ...
        from opencodon.tools.browser_tool import _extract_relevant_content
        ...
        with patch("opencodon.tools.browser_tool.call_llm", return_value=mock_resp):
