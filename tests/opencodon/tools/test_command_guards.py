...
import opencodon.tools.tirith_security
...
_TIRITH_PATCH = "opencodon.tools.tirith_security.check_command_security"
...
def _clean_state():
    ...
    saved = {}
    for k in ("OPENCODON_INTERACTIVE", "OPENCODON_GATEWAY_SESSION", "OPENCODON_EXEC_ASK", "OPENCODON_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["OPENCODON_INTERACTIVE"] = "1"
        session_key = os.getenv("OPENCODON_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No OPENCODON_INTERACTIVE or OPENCODON_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
...
class TestCombinedWarnings:
    ...
    def test_combined_cli_always_persists_pattern_but_not_tirith(self, mock_tirith):
        ...
        assert result["approved"] is True
        session_key = os.getenv("OPENCODON_SESSION_KEY", "default")
        from opencodon.tools import approval as _mod
...
class TestTirithImportError:
    def test_import_error_allows(self):
        """When opencodon.tools.tirith_security can't be imported, treated as allow."""
        import sys
        # Temporarily remove the module and replace with something that raises
        original = sys.modules.get("opencodon.tools.tirith_security")
        sys.modules["opencodon.tools.tirith_security"] = None  # causes ImportError on from-import
        try:
            result = check_all_command_guards("echo hello", "local")
            assert result["approved"] is True
        finally:
            if original is not None:
                sys.modules["opencodon.tools.tirith_security"] = original
            else:
                sys.modules.pop("opencodon.tools.tirith_security", None)
...
class TestGatewayApprovalAllowPermanent:
    ...
    def _capture_gateway_payload(self, command, session_key):
        ...
        try:
            ...
            os.environ.pop("OPENCODON_GATEWAY_SESSION", None)
            os.environ.pop("OPENCODON_EXEC_ASK", None)
            os.environ.pop("OPENCODON_SESSION_KEY", None)
