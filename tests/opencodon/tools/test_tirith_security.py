...
import opencodon.tools.tirith_security as _tirith_mod
from opencodon.tools.tirith_security import check_command_security, ensure_installed
...
class TestExitCodeMapping:
    @patch("opencodon.tools.tirith_security.subprocess.run")
    @patch("opencodon.tools.tirith_security._load_security_config")
...
class TestPathExpansion:
    def test_tilde_expanded_in_resolve(self):
        ...
        from opencodon.tools.tirith_security import _resolve_tirith_path
        _tirith_mod._resolved_path = None
...
class TestEnsureInstalled:
    ...
    @patch("opencodon.tools.tirith_security.shutil.which", return_value="/usr/local/bin/tirith")
    @patch("opencodon.tools.tirith_security._load_security_config")
    ...
    def test_not_found_returns_none(self, mock_cfg):
        mock_cfg.return_value = {"tirith_enabled": True, "tirith_path": "tirith",
                                 "tirith_timeout": 5, "tirith_fail_open": True}
        _tirith_mod._resolved_path = None
        with patch("opencodon.tools.tirith_security.shutil.which", return_value=None), \
             patch("opencodon.tools.tirith_security._opencodon_bin_dir", return_value="/nonexistent"), \
             patch("opencodon.tools.tirith_security._is_install_failed_on_disk", return_value=False), \
             patch("opencodon.tools.tirith_security.threading.Thread") as MockThread:
...
class TestUnsupportedPlatform:
    ...
    def test_is_platform_supported_true_on_linux_x86_64(self):
        with patch("opencodon.tools.tirith_security.platform.system", return_value="Linux"), \
             patch("opencodon.tools.tirith_security.platform.machine", return_value="x86_64"):
            assert _tirith_mod.is_platform_supported() is True

    def test_is_platform_supported_true_on_darwin_arm64(self):
        with patch("opencodon.tools.tirith_security.platform.system", return_value="Darwin"), \
             patch("opencodon.tools.tirith_security.platform.machine", return_value="arm64"):
            assert _tirith_mod.is_platform_supported() is True

    def test_is_platform_supported_false_on_windows(self):
        with patch("opencodon.tools.tirith_security.platform.system", return_value="Windows"), \
             patch("opencodon.tools.tirith_security.platform.machine", return_value="AMD64"):
            assert _tirith_mod.is_platform_supported() is False

    def test_is_platform_supported_false_on_unknown_arch(self):
        with patch("opencodon.tools.tirith_security.platform.system", return_value="Linux"), \
             patch("opencodon.tools.tirith_security.platform.machine", return_value="riscv64"):
            assert _tirith_mod.is_platform_supported() is False
    ...
    def test_ensure_installed_unsupported_returns_none_no_thread(self, mock_cfg):
        ...
        mock_cfg.return_value = {"tirith_enabled": True, "tirith_path": "tirith",
                                 "tirith_timeout": 5, "tirith_fail_open": True}
        _tirith_mod._resolved_path = None
        with patch("opencodon.tools.tirith_security.is_platform_supported", return_value=False), \
             patch("opencodon.tools.tirith_security.threading.Thread") as MockThread, \
             patch("opencodon.tools.tirith_security._mark_install_failed") as mock_mark, \
             patch("opencodon.tools.tirith_security.shutil.which") as mock_which:
            result = ensure_installed()
            assert result is None
    ...
    def test_check_command_security_unsupported_allows_silently(self, mock_cfg):
        ...
        mock_cfg.return_value = {"tirith_enabled": True, "tirith_path": "tirith",
                                 "tirith_timeout": 5, "tirith_fail_open": True}
        with patch("opencodon.tools.tirith_security.is_platform_supported", return_value=False), \
             patch("opencodon.tools.tirith_security.subprocess.run") as mock_run, \
             patch("opencodon.tools.tirith_security._resolve_tirith_path") as mock_resolve:
            result = check_command_security("rm -rf /")
...
class TestFailedDownloadCaching:
    @patch("opencodon.tools.tirith_security._mark_install_failed")
    @patch("opencodon.tools.tirith_security._is_install_failed_on_disk", return_value=False)
    @patch("opencodon.tools.tirith_security._install_tirith", return_value=(None, "download_failed"))
    @patch("opencodon.tools.tirith_security.shutil.which", return_value=None)
    def test_failed_install_cached_no_retry(self, mock_which, mock_install,
        ...
        from opencodon.tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        _tirith_mod._resolved_path = None
...
class TestExplicitPathNoAutoDownload:
    @patch("opencodon.tools.tirith_security._install_tirith")
    @patch("opencodon.tools.tirith_security.shutil.which", return_value=None)
    ...
    @patch("opencodon.tools.tirith_security._mark_install_failed")
    @patch("opencodon.tools.tirith_security._is_install_failed_on_disk", return_value=False)
    @patch("opencodon.tools.tirith_security._install_tirith", return_value=("/auto/tirith", ""))
    @patch("opencodon.tools.tirith_security.shutil.which", return_value=None)
...
class TestCosignVerification:
    @patch("opencodon.tools.tirith_security.subprocess.run")
    @patch("opencodon.tools.tirith_security.shutil.which", return_value="/usr/bin/cosign")
    def test_cosign_pass(self, mock_which, mock_run):
        """cosign verify-blob exits 0 → returns True."""
        from opencodon.tools.tirith_security import _verify_cosign
    ...
    @patch("opencodon.tools.tirith_security._verify_cosign", return_value=False)
    @patch("opencodon.tools.tirith_security.shutil.which", return_value="/usr/local/bin/cosign")
    @patch("opencodon.tools.tirith_security._download_file")
    @patch("opencodon.tools.tirith_security._detect_target", return_value="aarch64-apple-darwin")
    def test_install_aborts_on_cosign_rejection(self, mock_target, mock_dl,
        ...
        from opencodon.tools.tirith_security import _install_tirith
        path, reason = _install_tirith()
        assert path is None
        assert reason == "cosign_verification_failed"

    @patch("opencodon.tools.tirith_security.tarfile.open")
    @patch("opencodon.tools.tirith_security._verify_checksum", return_value=True)
    @patch("opencodon.tools.tirith_security.shutil.which", return_value=None)
    @patch("opencodon.tools.tirith_security._download_file")
    @patch("opencodon.tools.tirith_security._detect_target", return_value="aarch64-apple-darwin")
    ...
    @patch("opencodon.tools.tirith_security.tarfile.open")
    @patch("opencodon.tools.tirith_security._verify_checksum", return_value=True)
    @patch("opencodon.tools.tirith_security._verify_cosign", return_value=None)
    @patch("opencodon.tools.tirith_security.shutil.which", return_value="/usr/local/bin/cosign")
    @patch("opencodon.tools.tirith_security._download_file")
    @patch("opencodon.tools.tirith_security._detect_target", return_value="aarch64-apple-darwin")
    ...
    @patch("opencodon.tools.tirith_security.tarfile.open")
    @patch("opencodon.tools.tirith_security._verify_checksum", return_value=True)
    @patch("opencodon.tools.tirith_security._verify_cosign", return_value=True)
    @patch("opencodon.tools.tirith_security.shutil.which", return_value="/usr/local/bin/cosign")
    @patch("opencodon.tools.tirith_security._download_file")
    @patch("opencodon.tools.tirith_security._detect_target", return_value="aarch64-apple-darwin")
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        with patch("opencodon.tools.tirith_security._download_file",
...
class TestBackgroundInstall:
    def test_ensure_installed_non_blocking(self):
        ...
        with patch("opencodon.tools.tirith_security._load_security_config",
    ...
    def test_ensure_installed_skips_on_disk_marker(self):
        ...
        with patch("opencodon.tools.tirith_security._load_security_config",
             ...
             patch("opencodon.tools.tirith_security.shutil.which", return_value=None), \
             patch("opencodon.tools.tirith_security._opencodon_bin_dir", return_value="/nonexistent"), \
             patch("opencodon.tools.tirith_security._read_failure_reason", return_value="download_failed"), \
             patch("opencodon.tools.tirith_security._is_install_failed_on_disk", return_value=True):
...
class TestDiskFailureMarker:
    def test_mark_and_check(self):
        ...
        with patch("opencodon.tools.tirith_security._failure_marker_path", return_value=marker):
            from opencodon.tools.tirith_security import (
    ...
    def test_expired_marker_ignored(self):
        ...
        marker = os.path.join(tmpdir, ".tirith-install-failed")
        with patch("opencodon.tools.tirith_security._failure_marker_path", return_value=marker):
            from opencodon.tools.tirith_security import _mark_install_failed, _is_install_failed_on_disk
            _mark_install_failed("download_failed")
    ...
    def test_cosign_missing_marker_clears_when_cosign_appears(self):
        ...
        with patch("opencodon.tools.tirith_security._failure_marker_path", return_value=marker):
            ...
            with patch("opencodon.tools.tirith_security.shutil.which", return_value="/usr/local/bin/cosign"):
                assert not _is_install_failed_on_disk()
    ...
    @patch("opencodon.tools.tirith_security._mark_install_failed")
    @patch("opencodon.tools.tirith_security._is_install_failed_on_disk", return_value=False)
    @patch("opencodon.tools.tirith_security._install_tirith", return_value=(None, "cosign_missing"))
    @patch("opencodon.tools.tirith_security.shutil.which", return_value=None)
    ...
    @patch("opencodon.tools.tirith_security._clear_install_failed")
    @patch("opencodon.tools.tirith_security._is_install_failed_on_disk", return_value=False)
    @patch("opencodon.tools.tirith_security._install_tirith", return_value=("/installed/tirith", ""))
    @patch("opencodon.tools.tirith_security.shutil.which", return_value=None)
    ...
    def test_sync_resolve_skips_install_on_disk_marker(self):
        ...
        with patch("opencodon.tools.tirith_security.shutil.which", return_value=None), \
             ...
             patch("opencodon.tools.tirith_security._read_failure_reason", return_value="download_failed"), \
             patch("opencodon.tools.tirith_security._is_install_failed_on_disk", return_value=True), \
             patch("opencodon.tools.tirith_security._install_tirith") as mock_install:
            _resolve_tirith_path("tirith")
            mock_install.assert_not_called()
            assert _tirith_mod._resolved_path is _INSTALL_FAILED
            assert _tirith_mod._install_failure_reason == "download_failed"
    ...
    def test_install_failed_still_checks_local_paths(self):
        ...
        with patch("opencodon.tools.tirith_security.shutil.which", return_value="/usr/local/bin/tirith"), \
             patch("opencodon.tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == "/usr/local/bin/tirith"
    ...
    def test_install_failed_recovers_from_opencodon_bin(self):
        """After _INSTALL_FAILED, manual install in OPENCODON_HOME/bin is picked up."""
        from opencodon.tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        opencodon_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(opencodon_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(opencodon_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("opencodon.tools.tirith_security.shutil.which", return_value=None), \
             patch("opencodon.tools.tirith_security._opencodon_bin_dir", return_value=tmpdir), \
             patch("opencodon.tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == opencodon_bin
            assert _tirith_mod._resolved_path == opencodon_bin
            mock_clear.assert_called_once()
    ...
    def test_cosign_missing_disk_marker_allows_retry(self):
        ...
        with patch("opencodon.tools.tirith_security.shutil.which", return_value=None), \
             patch("opencodon.tools.tirith_security._opencodon_bin_dir", return_value="/nonexistent"), \
             patch("opencodon.tools.tirith_security._is_install_failed_on_disk", return_value=False), \
             patch("opencodon.tools.tirith_security._install_tirith", return_value=("/new/tirith", "")) as mock_install, \
             patch("opencodon.tools.tirith_security._clear_install_failed"):
            result = _resolve_tirith_path("tirith")
    ...
    def test_in_memory_cosign_missing_retries_when_cosign_appears(self):
        ...
        with patch("opencodon.tools.tirith_security.shutil.which", side_effect=_which_side_effect), \
             patch("opencodon.tools.tirith_security._opencodon_bin_dir", return_value="/nonexistent"), \
             patch("opencodon.tools.tirith_security._is_install_failed_on_disk", return_value=False), \
             patch("opencodon.tools.tirith_security._install_tirith", return_value=("/new/tirith", "")) as mock_install, \
             patch("opencodon.tools.tirith_security._clear_install_failed"):
    ...
    def test_disk_marker_reason_preserved_in_memory(self):
        ...
        with patch("opencodon.tools.tirith_security.shutil.which", return_value=None), \
             patch("opencodon.tools.tirith_security._opencodon_bin_dir", return_value="/nonexistent"), \
             patch("opencodon.tools.tirith_security._read_failure_reason", return_value="cosign_missing"), \
             patch("opencodon.tools.tirith_security._is_install_failed_on_disk", return_value=True):
            _resolve_tirith_path("tirith")
            assert _tirith_mod._resolved_path is _INSTALL_FAILED
...
class TestOpencodonHomeIsolation:
    def test_opencodon_bin_dir_respects_opencodon_home(self):
        """_opencodon_bin_dir must use OPENCODON_HOME, not hardcoded ~/.opencodon."""
        from opencodon.tools.tirith_security import _opencodon_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"OPENCODON_HOME": tmpdir}):
            result = _opencodon_bin_dir()
    ...
    def test_failure_marker_respects_opencodon_home(self):
        ...
        from opencodon.tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"OPENCODON_HOME": "/custom/opencodon"}):
            result = _failure_marker_path()
        assert result == "/custom/opencodon/.tirith-install-failed"
    ...
    def test_conftest_isolation_prevents_real_home_writes(self):
        ...
        assert "opencodon_test" in opencodon_home, "Should point to test temp dir"

    def test_get_opencodon_home_fallback(self):
        """Without OPENCODON_HOME set, falls back to the active OS home."""
        from opencodon.tools.tirith_security import _get_opencodon_home
...
class TestSpawnWarningDedup:
    ...
    def test_repeated_spawn_failure_logs_once(self, mock_cfg, mock_run, caplog):
        ...
        with caplog.at_level("WARNING", logger="opencodon.tools.tirith_security"):
            for i in range(15):
                result = check_command_security("echo hi")
    ...
    def test_path_none_logs_once(self, mock_cfg, caplog):
        ...
        with patch(
            "opencodon.tools.tirith_security._resolve_tirith_path", return_value=None
        ):
            with caplog.at_level("WARNING", logger="opencodon.tools.tirith_security"):
                for _ in range(10):
...
class TestIsAppTldFinding:
    ...
    def setup_method(self):
        from opencodon.tools.tirith_security import _is_app_tld_finding
...
class TestMkdtempOSErrorNoSpace:
    ...
    def test_mkdtemp_oserror_returns_no_space(self):
        ...
        with patch("opencodon.tools.tirith_security.tempfile.mkdtemp",
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("opencodon.tools.tirith_security.tempfile.mkdtemp",
                   side_effect=OSError(28, "No space left on device")), \
             patch("opencodon.tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("opencodon.tools.tirith_security._opencodon_bin_dir",
                   return_value="/nonexistent"), \
             patch("opencodon.tools.tirith_security._is_install_failed_on_disk",
                   return_value=False), \
             patch("opencodon.tools.tirith_security._mark_install_failed") as mock_mark:
            result = _resolve_tirith_path("tirith")
            assert _tirith_mod._resolved_path is _INSTALL_FAILED
