...
class TestGetExecutionMode(unittest.TestCase):
    ...
    def test_empty_config_falls_back_to_default(self):
        with patch("opencodon.tools.code_execution_tool._load_config", return_value={}):
...
class TestExecuteCodeModeIntegration(unittest.TestCase):
    ...
    def test_strict_mode_runs_in_tmpdir(self):
        ...
        self.assertIn("opencodon_sandbox_", result["output"])
    ...
    def test_project_mode_interpreter_is_venv_python(self):
        ...
        # The opencodon venv is always active during tests, so this also
    ...
    def test_project_mode_can_still_import_opencodon_tools(self):
        """Regression: opencodon_tools still importable from non-tmpdir CWD.

        This is the PYTHONPATH fix — without it, switching to session CWD
        breaks `from opencodon_tools import terminal`.
        """
        import tempfile
        with tempfile.TemporaryDirectory() as td:
            code = (
    ...
    def test_strict_mode_can_still_import_opencodon_tools(self):
