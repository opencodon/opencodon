...
``~/.opencodon/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.opencodon/.env`` would be told
    ...
    def test_import_after_config_env_patch_uses_restored_dotenv_loader(self):
        """Importing STT while opencodon.config.get_env_value is patched must
        not freeze that temporary helper into this module forever.
        """
        import importlib
        import opencodon.config as config_mod
        from opencodon.tools import transcription_tools as tt
        ...
        try:
            with patch.object(tt, "_HAS_FASTER_WHISPER", False), \
                 ...
                 patch("opencodon.config.load_env",
    ...
    def test_xai_resolver_import_after_config_env_patch_uses_restored_dotenv_loader(self):
        ...
        try:
            with patch(
                ...
                "opencodon.core.auth.resolve_xai_oauth_runtime_credentials",
                return_value={},
            ), patch(
...
class TestEndToEndRegressionGuard:
    """End-to-end probe: patch ``opencodon.config.load_env`` to simulate
    ``~/.opencodon/.env`` carrying the key while ``os.environ`` does not.
    ...
    def test_xai_key_only_in_dotenv_before_fix(self, monkeypatch):
        ...
        with patch("opencodon.config.load_env",
                   return_value={"XAI_API_KEY": "dotenv-secret"}):
