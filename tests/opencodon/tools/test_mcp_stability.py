...
class TestMCPLoopExceptionHandler:
    ...
    def test_suppresses_event_loop_closed(self):
        from opencodon.tools.mcp_tool import _mcp_loop_exception_handler
        loop = MagicMock()
...
class TestStdioPidTracking:
    ...
    def test_snapshot_returns_set(self):
        from opencodon.tools.mcp_tool import _snapshot_child_pids
    ...
    def test_stdio_pids_starts_empty(self):
        from opencodon.tools.mcp_tool import _stdio_pids, _lock
        with _lock:
    ...
    def test_run_stdio_reaps_orphans_before_spawn(self):
        ...
        async def _run():
            ...
            with patch("opencodon.tools.mcp_tool._MCP_AVAILABLE", True), \
                 patch("opencodon.tools.mcp_tool._build_safe_env", return_value={}), \
                 patch("opencodon.tools.mcp_tool._resolve_stdio_command",
                       return_value=("echo", {})), \
                 patch("opencodon.tools.mcp_tool._write_stderr_log_header"), \
                 patch("opencodon.tools.mcp_tool._get_mcp_stderr_log",
                       return_value=None), \
                 patch("opencodon.tools.mcp_tool.check_package_for_malware",
                       return_value=None, create=True), \
                 patch("opencodon.tools.osv_check.check_package_for_malware",
                       return_value=None):
                # Patch stdio_client to raise so the test exits quickly
                cm = MagicMock()
                cm.__aenter__ = AsyncMock(side_effect=RuntimeError("test"))
                cm.__aexit__ = AsyncMock(return_value=False)
                with patch("opencodon.tools.mcp_tool.stdio_client", return_value=cm):
                    try:
...
class TestStdioPgroupReaping:
    ...
    def test_killpg_failure_falls_back_to_kill(self, monkeypatch):
        ...
        with patch(
            "opencodon.tools.mcp_tool.os.killpg",
...
class TestMCPReloadTimeout:
    ...
    def test_reload_timeout_does_not_block_forever(self, tmp_path, monkeypatch):
        ...
        # Create a mock OpencodonCLI-like object with the needed attributes
        class FakeCLI:
            _config_mtime = 0.0
            _config_mcp_servers = {}
...
class TestMCPInitialConnectionRetry:
    ...
    def test_initial_connect_retries_constant_exists(self):
        ...
        from opencodon.tools.mcp_tool import _MAX_INITIAL_CONNECT_RETRIES
