...
from opencodon.tools.website_policy import WebsitePolicyError, check_website_access, load_website_blocklist
...
def test_check_website_access_uses_dynamic_opencodon_home(monkeypatch, tmp_path):
    ...
    # Invalidate the module-level cache so the new OPENCODON_HOME is picked up.
    # A prior test may have cached a default policy (enabled=False) under the
    # old OPENCODON_HOME set by the autouse _isolate_opencodon_home fixture.
    from opencodon.tools.website_policy import invalidate_cache
    invalidate_cache()
...
class TestWebToolPolicy:
    ...
    async def test_web_extract_short_circuits_blocked_url(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.tools.interrupt.is_interrupted", lambda: False)
...
def test_check_website_access_fails_open_on_malformed_config(tmp_path, monkeypatch):
    ...
    # Simulate default path by pointing OPENCODON_HOME to tmp_path
    monkeypatch.setenv("OPENCODON_HOME", str(tmp_path))
    from opencodon.tools import website_policy
