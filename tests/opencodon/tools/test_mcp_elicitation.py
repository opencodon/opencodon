...
from opencodon.tools.mcp_tool import (  # noqa: E402
    ElicitationHandler,
...
class TestElicitationHandlerFormMode:
    def test_user_accepts_once_returns_accept(self):
        ...
        with patch("opencodon.tools.approval.request_elicitation_consent", return_value="accept"):
    ...
    def test_user_denies_returns_decline(self):
        ...
        with patch("opencodon.tools.approval.request_elicitation_consent", return_value="decline"):
            result = asyncio.run(handler(context=None, params=params))
    ...
    def test_cancel_propagates_through(self):
        ...
        with patch("opencodon.tools.approval.request_elicitation_consent", return_value="cancel"):
            result = asyncio.run(handler(context=None, params=params))
...
class TestElicitationHandlerFailureModes:
    def test_url_mode_is_declined_without_prompting(self):
        ...
        with patch(
            "opencodon.tools.approval.request_elicitation_consent",
    ...
    def test_timeout_returns_cancel(self, monkeypatch):
        ...
        with patch("opencodon.tools.approval.request_elicitation_consent", side_effect=stall):
            result = asyncio.run(handler(context=None, params=params))
...
class TestElicitationHandlerContextBridge:
    ...
    inherit the agent's contextvars (OPENCODON_SESSION_PLATFORM etc.). The
    ...
    def test_captured_context_is_replayed_in_consent_call(self):
        ...
        with patch("opencodon.tools.approval.request_elicitation_consent", side_effect=fake_consent):
            result = asyncio.run(handler(context=None, params=params))
    ...
    def test_missing_captured_context_falls_back_to_direct_call(self):
        ...
        don't set OPENCODON_SESSION_PLATFORM, would break."""
        handler = ElicitationHandler("pay", {"timeout": 5}, owner=None)
        params = _form_params()

        with patch("opencodon.tools.approval.request_elicitation_consent", return_value="accept") as m:
            result = asyncio.run(handler(context=None, params=params))
