...
from opencodon.tools.mcp_tool import MCPServerTask, _format_connect_error, _resolve_stdio_command, _MCP_AVAILABLE
...
def test_resolve_stdio_command_falls_back_to_opencodon_node_bin(tmp_path):
    ...
    with patch("opencodon.tools.mcp_tool.shutil.which", return_value=None), \
         patch.dict("os.environ", {"OPENCODON_HOME": str(tmp_path)}, clear=False):
...
def test_resolve_stdio_command_falls_back_to_usr_local_bin():
    """When ``npx`` isn't on the filtered PATH and isn't under ``$OPENCODON_HOME/node/bin``
    ...
    the upstream ``node:bookworm-slim`` image (which the opencodon Docker image
    ...
    # the other candidates ($OPENCODON_HOME/node/bin/npx and ~/.local/bin/npx)
    ...
    with patch("opencodon.tools.mcp_tool.shutil.which", return_value=None), \
         patch("opencodon.tools.mcp_tool.os.path.isfile", side_effect=_fake_isfile), \
         patch("opencodon.tools.mcp_tool.os.access", side_effect=_fake_access):
...
def test_resolve_stdio_command_respects_explicit_empty_path():
    ...
    with patch("opencodon.tools.mcp_tool.shutil.which", side_effect=_fake_which):
...
def test_run_stdio_uses_resolved_command_and_prepended_path(tmp_path):
    ...
    async def _test():
        with patch("opencodon.tools.mcp_tool.shutil.which", return_value=None), \
             patch.dict("os.environ", {"OPENCODON_HOME": str(tmp_path), "PATH": "/usr/bin", "HOME": str(tmp_path)}, clear=False), \
             patch("opencodon.tools.mcp_tool.StdioServerParameters") as mock_params, \
             patch("opencodon.tools.mcp_tool.stdio_client", return_value=mock_stdio_cm), \
             patch("opencodon.tools.mcp_tool.ClientSession", return_value=mock_session_cm):
...
def test_run_stdio_malware_check_does_not_block_event_loop():
    ...
    async def _test():
        with patch("opencodon.tools.osv_check.check_package_for_malware", side_effect=slow_check), \
             patch("opencodon.tools.mcp_tool.StdioServerParameters"), \
             patch("opencodon.tools.mcp_tool.stdio_client", return_value=mock_stdio_cm), \
             patch("opencodon.tools.mcp_tool.ClientSession", return_value=mock_session_cm):
            server = MCPServerTask("srv")
...
def test_run_stdio_malware_check_times_out_fail_open():
    ...
    async def _test():
        with patch("opencodon.tools.osv_check.check_package_for_malware", side_effect=hung_check), \
             patch("opencodon.tools.mcp_tool._OSV_MALWARE_CHECK_TIMEOUT_S", 0.2), \
             patch("opencodon.tools.mcp_tool.StdioServerParameters"), \
             patch("opencodon.tools.mcp_tool.stdio_client", return_value=mock_stdio_cm), \
             patch("opencodon.tools.mcp_tool.ClientSession", return_value=mock_session_cm):
            server = MCPServerTask("srv")
