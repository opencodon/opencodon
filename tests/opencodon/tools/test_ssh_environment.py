...
class TestBuildSSHCommand:
    ...
    def _mock_connection(self, monkeypatch):
        monkeypatch.setattr("opencodon.tools.environments.ssh.subprocess.run",
                            lambda *a, **k: subprocess.CompletedProcess([], 0))
        monkeypatch.setattr("opencodon.tools.environments.ssh.subprocess.Popen",
                            lambda *a, **k: MagicMock(stdout=iter([]),
                                                      stderr=iter([]),
                                                      stdin=MagicMock()))
        monkeypatch.setattr("opencodon.tools.environments.base.time.sleep", lambda _: None)
...
class TestControlSocketPath:
    ...
    def test_fits_under_macos_socket_limit_with_ipv6_host(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.tools.environments.ssh.tempfile.gettempdir",
                            lambda: fake_tmp)
...
class TestTerminalToolConfig:
    def test_ssh_persistent_default_true(self, monkeypatch):
        ...
        from opencodon.tools.terminal_tool import _get_env_config
...
class TestOneShotSSH:
    ...
    def test_state_does_not_persist(self):
        _run("export OPENCODON_ONESHOT_TEST=yes")
        r = _run("echo $OPENCODON_ONESHOT_TEST")
...
class TestPersistentSSH:
    ...
    def test_env_var_persists(self):
        _run("export OPENCODON_PERSIST_TEST=works")
        r = _run("echo $OPENCODON_PERSIST_TEST")
