...
from opencodon.tools.discord_tool import (
    DiscordAPIError,
    _ACTIONS,
    _ADMIN_ACTIONS,
    _CORE_ACTIONS,
...
class TestDiscordRequest:
    @patch("opencodon.tools.discord_tool.urllib.request.urlopen")
    ...
    def test_response_body_size_limit(self, mock_urlopen_fn, monkeypatch):
        monkeypatch.setattr("opencodon.tools.discord_tool._DISCORD_RESPONSE_BODY_MAX_BYTES", 8)
        mock_resp = MagicMock()
    ...
    def test_http_error_body_size_limit(self, mock_urlopen_fn, monkeypatch):
        monkeypatch.setattr("opencodon.tools.discord_tool._DISCORD_ERROR_BODY_MAX_BYTES", 8)
        http_error = urllib.error.HTTPError(
            url="https://discord.com/api/v10/test",
            code=403,
            msg="Forbidden",
...
class TestListGuilds:
    @patch("opencodon.tools.discord_tool._discord_request")
...
class TestNonBlockingCapabilityDetection:
    ...
    def test_memory_cache_hit_no_network(self):
        from opencodon.tools.discord_tool import _capability_cache, _detect_capabilities_nonblocking
        caps_in = {"has_members_intent": False, "has_message_content": True, "detected": True}
        _capability_cache["tok"] = caps_in
        with patch("opencodon.tools.discord_tool._discord_request") as mock_req:
            caps = _detect_capabilities_nonblocking("tok")
        assert caps == caps_in
        mock_req.assert_not_called()

    def test_cold_start_returns_permissive_default_immediately(self):
        from opencodon.tools.discord_tool import _detect_capabilities_nonblocking
        with patch("opencodon.tools.discord_tool._load_caps_from_disk", return_value=None), \
             patch("opencodon.tools.discord_tool.threading.Thread") as mock_thread:
            caps = _detect_capabilities_nonblocking("tok")
        assert caps["has_members_intent"] is True
        assert caps["has_message_content"] is True
        assert caps["detected"] is False
    ...
    def test_bg_detection_scheduled_once_per_token(self):
        ...
        with patch("opencodon.tools.discord_tool._load_caps_from_disk", return_value=None), \
            ...
            from opencodon.tools.discord_tool import _capability_cache
    ...
    def test_disk_cache_round_trip(self, tmp_path, monkeypatch):
        import opencodon.tools.discord_tool as dt
        monkeypatch.setattr(
    ...
    def test_schema_build_uses_nonblocking_path(self, monkeypatch):
        ...
        with patch("opencodon.tools.discord_tool._load_caps_from_disk", return_value=None), \
             patch("opencodon.tools.discord_tool.threading.Thread"), \
             patch("opencodon.tools.discord_tool._discord_request") as mock_req:
...
class TestConfigAllowlist:
    ...
    def _reset_tools_logger(self):
        ...
        ``opencodon.tools.discord_tool`` for every test that follows.  Reset here so
        ``caplog`` can capture warnings regardless of worker history.
        """
        import logging as _logging
        _prev_tools = _logging.getLogger("tools").level
        _prev_dt = _logging.getLogger("opencodon.tools.discord_tool").level
        _logging.getLogger("tools").setLevel(_logging.NOTSET)
        _logging.getLogger("opencodon.tools.discord_tool").setLevel(_logging.NOTSET)
        try:
            yield
        finally:
            _logging.getLogger("tools").setLevel(_prev_tools)
            _logging.getLogger("opencodon.tools.discord_tool").setLevel(_prev_dt)
    ...
    def test_config_load_failure_is_permissive(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.config.load_config", bad_load)
        assert _load_allowed_actions_config() is None
...
class TestModelToolsIntegration:
    def setup_method(self):
        _reset_capability_cache()
        from opencodon.tools.model_tools import _clear_tool_defs_cache
        from opencodon.tools.registry import invalidate_check_fn_cache
