...
def _reset_engine_cache():
    ...
    import opencodon.tools.browser_tool as bt
...
class TestGetBrowserEngine:
    ...
    def test_config_lightpanda(self):
        ...
        with patch("opencodon.config.read_raw_config", return_value=cfg):
    ...
    def test_caching(self):
        ...
        with patch("opencodon.config.read_raw_config", mock_read):
            assert _get_browser_engine() == "lightpanda"
            assert _get_browser_engine() == "lightpanda"
...
class TestShouldInjectEngine:
    ...
    def test_auto_never_injects(self):
        from opencodon.tools.browser_tool import _should_inject_engine
        assert _should_inject_engine("auto") is False

    def test_lightpanda_injects_in_local_mode(self):
        from opencodon.tools.browser_tool import _should_inject_engine
        with patch("opencodon.tools.browser_tool._is_camofox_mode", return_value=False), \
             patch("opencodon.tools.browser_tool._get_cdp_override", return_value=""), \
             patch("opencodon.tools.browser_tool._get_cloud_provider", return_value=None):
    ...
    def test_no_inject_in_camofox_mode(self):
        from opencodon.tools.browser_tool import _should_inject_engine
        with patch("opencodon.tools.browser_tool._is_camofox_mode", return_value=True):
            assert _should_inject_engine("lightpanda") is False

    def test_no_inject_with_cdp_override(self):
        from opencodon.tools.browser_tool import _should_inject_engine
        with patch("opencodon.tools.browser_tool._is_camofox_mode", return_value=False), \
             patch("opencodon.tools.browser_tool._get_cdp_override", return_value="ws://localhost:9222"):
            assert _should_inject_engine("lightpanda") is False
    ...
    def test_no_inject_with_cloud_provider(self):
        ...
        with patch("opencodon.tools.browser_tool._is_camofox_mode", return_value=False), \
             patch("opencodon.tools.browser_tool._get_cdp_override", return_value=""), \
             patch("opencodon.tools.browser_tool._get_cloud_provider", return_value=mock_provider):
            assert _should_inject_engine("lightpanda") is False
...
class TestNeedsLightpandaFallback:
    ...
    def test_non_lightpanda_never_falls_back(self):
        from opencodon.tools.browser_tool import _needs_lightpanda_fallback
    ...
    def test_failed_command_reason_is_user_visible(self):
        from opencodon.tools.browser_tool import _lightpanda_fallback_reason
        result = {"success": False, "error": "page.goto: Timeout"}
...
class TestLightpandaRequirements:
    ...
    def test_lightpanda_local_mode_does_not_require_chromium(self):
        ...
        with patch("opencodon.tools.browser_tool._is_camofox_mode", return_value=False), \
             patch("opencodon.tools.browser_tool._get_cdp_override", return_value=""), \
             patch("opencodon.tools.browser_tool._find_agent_browser", return_value="/usr/bin/agent-browser"), \
             patch("opencodon.tools.browser_tool._requires_real_termux_browser_install", return_value=False), \
             patch("opencodon.tools.browser_tool._get_cloud_provider", return_value=None), \
             patch("opencodon.tools.browser_tool._get_browser_engine", return_value="lightpanda"), \
             patch("opencodon.tools.browser_tool._chromium_installed", return_value=False):
    ...
    def test_chrome_local_mode_still_requires_chromium(self):
        ...
        with patch("opencodon.tools.browser_tool._is_camofox_mode", return_value=False), \
             ...
             patch("opencodon.tools.browser_tool._requires_real_termux_browser_install", return_value=False), \
             patch("opencodon.tools.browser_tool._get_cloud_provider", return_value=None), \
             patch("opencodon.tools.browser_tool._get_browser_engine", return_value="auto"), \
             patch("opencodon.tools.browser_tool._chromium_installed", return_value=False):
...
class TestLightpandaFallbackWarning:
    ...
    def test_fallback_result_gets_user_visible_warning(self):
        from opencodon.tools.browser_tool import _annotate_lightpanda_fallback
    ...
    def test_browser_navigate_surfaces_fallback_warning(self):
        ...
        with patch("opencodon.tools.browser_tool._is_local_backend", return_value=True), \
             patch("opencodon.tools.browser_tool._get_cloud_provider", return_value=None), \
             patch("opencodon.tools.browser_tool._get_session_info", return_value={
                 "session_name": "test", "_first_nav": False, "features": {"local": True, "proxies": True}
             }), \
             patch("opencodon.tools.browser_tool._run_browser_command", side_effect=[
                 result,
    ...
    def test_failed_fallback_warning_is_preserved_on_click_error(self):
        ...
        with patch("opencodon.tools.browser_tool._run_browser_command", return_value=result):
    ...
    def test_browser_vision_lightpanda_uses_chrome_capture_and_normal_call_llm_shape(self, tmp_path):
        ...
        with patch("opencodon.tools.browser_tool._get_browser_engine", return_value="lightpanda"), \
             patch("opencodon.tools.browser_tool._should_inject_engine", return_value=True), \
             patch("opencodon.tools.browser_tool._chrome_fallback_screenshot", return_value={
                 "success": True, "data": {"path": str(chrome_shot)}
             }), \
             patch("opencodon_constants.get_opencodon_dir", return_value=tmp_path), \
             patch("opencodon.tools.browser_tool.call_llm", side_effect=fake_call_llm):
    ...
    def test_browser_vision_lightpanda_response_has_structured_fallback(self, tmp_path):
        ...
        with patch("opencodon.tools.browser_tool._get_browser_engine", return_value="lightpanda"), \
             ...
             patch("opencodon_constants.get_opencodon_dir", return_value=tmp_path), \
             patch("opencodon.tools.browser_tool.call_llm", return_value=_Response()):
...
class TestEngineOverride:
    ...
    @patch("opencodon.tools.browser_tool._get_session_info")
    @patch("opencodon.tools.browser_tool._find_agent_browser", return_value="/usr/bin/agent-browser")
    @patch("opencodon.tools.browser_tool._is_local_mode", return_value=True)
    @patch("opencodon.tools.browser_tool._chromium_installed", return_value=True)
    @patch("opencodon.tools.browser_tool._get_cloud_provider", return_value=None)
    @patch("opencodon.tools.browser_tool._get_cdp_override", return_value="")
    ...
    def test_override_prevents_engine_injection(
        ...
        with patch("subprocess.Popen", side_effect=capture_popen), \
             ...
             patch("opencodon.tools.browser_tool._write_owner_pid"):
    ...
    def test_hybrid_local_sidecar_injects_engine_even_with_cloud_provider(self):
        ...
        with patch("opencodon.tools.browser_tool._get_session_info", return_value={"session_name": "local-sidecar"}), \
             patch("opencodon.tools.browser_tool._find_agent_browser", return_value="/usr/bin/agent-browser"), \
             patch("opencodon.tools.browser_tool._is_local_mode", return_value=False), \
             patch("opencodon.tools.browser_tool._chromium_installed", return_value=True), \
             patch("opencodon.tools.browser_tool._get_cloud_provider", return_value=mock_provider), \
             patch("opencodon.tools.browser_tool._get_cdp_override", return_value=""), \
             patch("opencodon.tools.browser_tool._is_camofox_mode", return_value=False), \
