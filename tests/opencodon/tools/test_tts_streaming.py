"""Tests for the provider-agnostic streaming TTS backend (opencodon.tools.tts_streaming)
...
def test_streamer_path_writes_pcm_to_output(monkeypatch):
    ...
    with patch("opencodon.tools.tts_streaming.resolve_streaming_provider", return_value=_Fake({}, {})), \
...
def test_sync_fallback_speaks_each_sentence(monkeypatch):
    ...
    monkeypatch.setitem(__import__("sys").modules, "opencodon.tools.voice_mode", fake_vm)
    ...
    with patch("opencodon.tools.tts_streaming.resolve_streaming_provider", return_value=None):
        tts_tool.stream_tts_to_speaker(q, stop, done)
