...
from opencodon.tools import delegation_live_log as dll
...
def test_writer_precreates_file_with_header():
    ...
    assert "opencodon subagent live transcript" in text
    assert "delegation: deleg_test1" in text
    assert "goal: do the thing" in text
    assert "kickoff" in text
    assert "some ctx" in text
    # Lives under the opencodon cache/delegation/live root, named task-<n>.log
...
def test_delegate_task_background_dispatch_includes_live_transcripts(monkeypatch):
    import opencodon.tools.delegate_tool as dt
    from opencodon.tools import async_delegation as ad
...
def test_delegate_task_proceeds_when_transcripts_unavailable(monkeypatch):
    ...
    import opencodon.tools.delegate_tool as dt
    from opencodon.tools import delegation_live_log as _dll
