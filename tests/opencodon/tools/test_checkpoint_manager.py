...
from opencodon.tools.checkpoint_manager import (
    CheckpointManager,
    _shadow_repo_path,
    _init_shadow_repo,
    _init_store,
...
def checkpoint_base(tmp_path):
    """Isolated checkpoint base — never writes to ~/.opencodon/."""
...
def mgr(work_dir, checkpoint_base, monkeypatch):
    monkeypatch.setattr("opencodon.tools.checkpoint_manager.CHECKPOINT_BASE", checkpoint_base)
...
class TestStoreInit:
    ...
    def test_bc_init_shadow_repo_shim(self, work_dir, checkpoint_base, monkeypatch):
        ...
        assert err is None
        assert (store / "HEAD").exists()
        assert (store / "OPENCODON_WORKDIR").exists()
...
class TestTakeCheckpoint:
    ...
    def test_multiple_projects_share_store(self, mgr, tmp_path):
        ...
        from opencodon.tools.checkpoint_manager import CHECKPOINT_BASE as BASE
...
class TestErrorResilience:
    ...
    def test_run_git_allows_expected_nonzero_without_error_log(
        ...
        with patch("opencodon.tools.checkpoint_manager.subprocess.run", return_value=completed):
            with caplog.at_level(logging.ERROR, logger="opencodon.tools.checkpoint_manager"):
    ...
    def test_run_git_missing_git_reports_git_not_found(
        ...
        monkeypatch.setattr("opencodon.tools.checkpoint_manager.subprocess.run", raise_missing_git)
        with caplog.at_level(logging.ERROR, logger="opencodon.tools.checkpoint_manager"):
            ok, _, stderr = _run_git(
    ...
    def test_checkpoint_failure_does_not_raise(self, mgr, work_dir, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.tools.checkpoint_manager._run_git", broken_run_git)
...
class TestPruneCheckpointsV2:
    ...
    def test_deletes_orphan_project_entry(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.tools.checkpoint_manager.CHECKPOINT_BASE", base)
...
def _seed_orphan_project(base: Path, tmp_path: Path, name: str) -> None:
    """Commit a project into the v2 store, then delete its workdir."""
    import shutil as _shutil
    work = tmp_path / name
    work.mkdir()
    (work / "f.py").write_text(name)
    m = CheckpointManager(enabled=True)
    m.ensure_checkpoint(str(work), name)
    _shutil.rmtree(work)


class TestMaybeAutoPruneCheckpoints:
    def test_first_call_prunes_and_writes_marker(self, tmp_path, monkeypatch):
        base = tmp_path / "checkpoints"
        monkeypatch.setattr("opencodon.tools.checkpoint_manager.CHECKPOINT_BASE", base)
        _seed_orphan_project(base, tmp_path, "gone")
    ...
    def test_second_call_within_interval_skips(self, tmp_path, monkeypatch):
        base = tmp_path / "checkpoints"
        monkeypatch.setattr("opencodon.tools.checkpoint_manager.CHECKPOINT_BASE", base)
        _seed_orphan_project(base, tmp_path, "gone")
        ...
        _seed_orphan_project(base, tmp_path, "also-gone")
        second = maybe_auto_prune_checkpoints(
            checkpoint_base=base, min_interval_hours=24,
        )
        assert second["skipped"] is True
        gone_hash = _project_hash(str(tmp_path / "also-gone"))
        assert (base / "store" / "projects" / f"{gone_hash}.json").exists()

    def test_corrupt_marker_treated_as_no_prior_run(self, tmp_path, monkeypatch):
        base = tmp_path / "checkpoints"
        monkeypatch.setattr("opencodon.tools.checkpoint_manager.CHECKPOINT_BASE", base)
        _seed_orphan_project(base, tmp_path, "gone")
