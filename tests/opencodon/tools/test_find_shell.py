...
from opencodon.tools.environments.local import _find_bash, _find_shell
...
class TestFindShellWindowsBehavior:
    ...
    def test_windows_ignores_shell_env(self):
        ...
        with patch("opencodon.tools.environments.local._IS_WINDOWS", True):
...
class TestFindBashSkipsBrokenCustomPath:
    """Stale OPENCODON_GIT_BASH_PATH must not brick Windows terminal startup."""
    ...
    def test_falls_through_to_portable_when_custom_fails_probe(self, tmp_path, monkeypatch):
        ...
        portable = tmp_path / "opencodon" / "git" / "bin" / "bash.exe"
        portable.parent.mkdir(parents=True)
        portable.write_text("", encoding="utf-8")

        monkeypatch.setenv("OPENCODON_GIT_BASH_PATH", str(broken))
...
class TestGitBashExternalProgramProbe:
    ...
    def test_aslr_failure_surfaces_targeted_windows_command(
        ...
        monkeypatch.setattr(local_mod, "_IS_WINDOWS", True)
        monkeypatch.setenv("OPENCODON_GIT_BASH_PATH", "")
        monkeypatch.setenv("LOCALAPPDATA", str(tmp_path))
        ...
        assert str(tmp_path / "opencodon" / "git") in message
