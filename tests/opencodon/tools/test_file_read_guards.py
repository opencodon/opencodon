...
class TestDevicePathBlocking(unittest.TestCase):
    ...
    def test_proc_fd_other_not_blocked(self):
        ...
        from opencodon.tools.file_tools import _is_blocked_device_path
...
class TestCharacterCountGuard(unittest.TestCase):
    ...
    @patch("opencodon.tools.file_tools._get_file_ops")
    @patch("opencodon.tools.file_tools._get_max_read_chars", return_value=1000)
    ...
    @patch("opencodon.tools.file_tools._get_file_ops")
    @patch("opencodon.tools.file_tools._get_max_read_chars", return_value=_DEFAULT_MAX_READ_CHARS)
...
class TestTruncateToCharBudget(unittest.TestCase):
    ...
    def _fn(self):
        from opencodon.tools.file_tools import _truncate_to_char_budget
...
class TestFileDedup(unittest.TestCase):
    ...
    def setUp(self):
        _read_tracker.clear()
        self._tmpdir = _make_safe_tempdir("opencodon-dedup-")
    ...
    def test_write_allows_large_file_that_quotes_status_text(self, mock_ops):
        ...
        opencodon' own docs / SKILL.md files may legitimately mention the dedup
...
class TestConfigOverride(unittest.TestCase):
    ...
    def setUp(self):
        ...
        import opencodon.tools.file_tools as _ft
    ...
    @patch("opencodon.tools.file_tools._get_file_ops")
    @patch("opencodon.config.load_config", return_value={"file_read_max_chars": 50})
    ...
    @patch("opencodon.tools.file_tools._get_file_ops")
    @patch("opencodon.config.load_config", return_value={"file_read_max_chars": 500_000})
...
class TestWriteInvalidatesDedup(unittest.TestCase):
    ...
    Regression test for upstream#13144
    """

    def setUp(self):
        _read_tracker.clear()
        self._tmpdir = _make_safe_tempdir("opencodon-write-dedup-")
