...
def _make_mock_server(name, session=None, tools=None):
    ...
    from opencodon.tools.mcp_tool import MCPServerTask
    ...
    def test_filters_gateway_children_by_argv_marker(self, monkeypatch):
        ...
        cmdlines = {
            101: [
                "/usr/bin/python3",
                "-m",
                "opencodon.frontends.tui.slash_worker",
                "--session-key",
                "abc",
            ],
            102: [
...
class TestLoadMCPConfig:
    def test_no_config_returns_empty(self):
        ...
        with patch("opencodon.config.load_config", return_value={"model": "test"}):
    ...
    def test_valid_config_parsed(self):
        ...
        with patch("opencodon.config.load_config", return_value={"mcp_servers": servers}):
            from opencodon.tools.mcp_tool import _load_mcp_config
            result = _load_mcp_config()
    ...
    def test_mcp_servers_not_dict_returns_empty(self):
        ...
        with patch("opencodon.config.load_config", return_value={"mcp_servers": "invalid"}):
            from opencodon.tools.mcp_tool import _load_mcp_config
            result = _load_mcp_config()
            assert result == {}
...
class TestLifecycleConfig:
    def test_get_lifecycle_seconds_accepts_top_level_and_nested_values(self):
        from opencodon.tools.mcp_tool import _get_lifecycle_seconds
...
class TestSchemaConversion:
    def test_converts_mcp_tool_to_opencodon_schema(self):
        from opencodon.tools.mcp_tool import _convert_mcp_schema
    ...
    def test_missing_type_on_object_is_coerced(self):
        ...
        from opencodon.tools.mcp_tool import _normalize_mcp_input_schema
...
class TestCheckFunction:
    def test_disconnected_returns_false(self):
        from opencodon.tools.mcp_tool import _make_check_fn, _servers
...
class TestRunOnMcpLoop:
    def test_scheduler_failure_closes_factory_coroutine(self):
        ...
        import gc
        import warnings
        import opencodon.tools.mcp_tool as mcp
...
class TestToolHandler:
    ...
    def _patch_mcp_loop(self, coro_side_effect=None):
        ...
        if coro_side_effect:
            return patch("opencodon.tools.mcp_tool._run_on_mcp_loop", side_effect=coro_side_effect)
        return patch("opencodon.tools.mcp_tool._run_on_mcp_loop", side_effect=fake_run)

    def test_successful_call(self):
        from opencodon.tools.mcp_tool import _make_tool_handler, _servers
    ...
    def test_interrupted_call_returns_interrupted_error(self):
        ...
        try:
            ...
            with patch(
                "opencodon.tools.mcp_tool._run_on_mcp_loop",
    ...
    def test_recycled_stdio_server_reconnects_lazily_on_tool_call(self):
        ...
        try:
            handler = _make_tool_handler("test_srv", "greet", 120)
            with patch("opencodon.tools.mcp_tool._request_lazy_reconnect", side_effect=fake_lazy_reconnect) as reconnect, \
...
class TestRunOnMCPLoopInterrupts:
    ...
    def test_interrupt_cancels_waiting_mcp_call(self):
        import opencodon.tools.mcp_tool as mcp_mod
        from opencodon.tools.interrupt import set_interrupt
...
class TestDiscoverAndRegister:
    def test_tools_registered_in_registry(self):
        ...
        from opencodon.tools.registry import ToolRegistry
        from opencodon.tools.mcp_tool import _discover_and_register_server, _servers, MCPServerTask
        ...
        with patch("opencodon.tools.mcp_tool._connect_server", side_effect=fake_connect), \
             patch("opencodon.tools.registry.registry", mock_registry):
...
class TestMCPServerTask:
    ...
    def _mock_stdio_and_session(self, session):
        ...
        return (
            patch("opencodon.tools.mcp_tool.stdio_client", return_value=mock_stdio_cm),
            patch("opencodon.tools.mcp_tool.ClientSession", return_value=mock_cs_cm),
    ...
    def test_start_connects_and_discovers_tools(self):
        ...
        async def _test():
            with patch("opencodon.tools.mcp_tool.StdioServerParameters"), p_stdio, p_cs:
    ...
    def test_empty_env_gets_safe_defaults(self):
        ...
        async def _test():
            with patch("opencodon.tools.mcp_tool.StdioServerParameters") as mock_params, \
                 p_stdio, p_cs, \
...
class TestToolsetInjection:
    def test_mcp_tools_resolve_through_server_aliases(self):
        ...
        with patch("opencodon.tools.mcp_tool._MCP_AVAILABLE", True), \
             patch("opencodon.tools.mcp_tool._servers", fresh_servers), \
             patch("opencodon.tools.mcp_tool._load_mcp_config", return_value=fake_config), \
             patch("opencodon.tools.mcp_tool._connect_server", side_effect=fake_connect), \
             patch("opencodon.tools.registry.registry", mock_registry):
    ...
    def test_server_toolset_skips_builtin_collision(self):
        ...
        fake_toolsets = {
            "opencodon-cli": {"tools": ["terminal"], "description": "CLI", "includes": []},
    ...
    def test_server_connection_failure_skipped(self):
        ...
        fake_toolsets = {
            "opencodon-cli": {"tools": [], "description": "CLI", "includes": []},
        }

        with patch("opencodon.tools.mcp_tool._MCP_AVAILABLE", True), \
             patch("opencodon.tools.mcp_tool._servers", fresh_servers), \
             patch("opencodon.tools.mcp_tool._load_mcp_config", return_value=fake_config), \
             patch("opencodon.tools.mcp_tool._connect_server", side_effect=flaky_connect), \
             patch("toolsets.TOOLSETS", fake_toolsets):
            from opencodon.tools.mcp_tool import discover_mcp_tools
            result = discover_mcp_tools()
    ...
    def test_partial_failure_retry_on_second_call(self):
        ...
        with patch("opencodon.tools.mcp_tool._MCP_AVAILABLE", True), \
            ...
            import opencodon.tools.mcp_tool as _mcp_mod
...
class TestGracefulFallback:
    def test_mcp_unavailable_returns_empty(self):
        ...
        with patch("opencodon.tools.mcp_tool._MCP_AVAILABLE", False):
            from opencodon.tools.mcp_tool import discover_mcp_tools
            result = discover_mcp_tools()
            assert result == []
    ...
    def test_no_servers_returns_empty(self):
        ...
        with patch("opencodon.tools.mcp_tool._MCP_AVAILABLE", True), \
             patch("opencodon.tools.mcp_tool._servers", {}), \
             patch("opencodon.tools.mcp_tool._load_mcp_config", return_value={}):
            from opencodon.tools.mcp_tool import discover_mcp_tools
            result = discover_mcp_tools()
            assert result == []
...
class TestShutdown:
    def test_no_servers_safe(self):
        ...
        from opencodon.tools.mcp_tool import shutdown_mcp_servers, _servers
    ...
    def test_shutdown_deregisters_registered_tools(self):
        ...
        import opencodon.tools.mcp_tool as mcp_mod
        from opencodon.tools.mcp_tool import MCPServerTask, shutdown_mcp_servers, _servers
...
class TestBuildSafeEnv:
    ...
    def test_only_safe_vars_passed(self):
        ...
        from opencodon.tools.mcp_tool import _build_safe_env
...
class TestSanitizeError:
    ...
    def test_strips_github_pat(self):
        from opencodon.tools.mcp_tool import _sanitize_error
...
class TestHTTPConfig:
    ...
    def test_http_unavailable_raises(self):
        ...
        async def _test():
            with patch("opencodon.tools.mcp_tool._MCP_HTTP_AVAILABLE", False):
    ...
    def test_http_seeds_initial_protocol_header(self):
        from opencodon.tools.mcp_tool import LATEST_PROTOCOL_VERSION, MCPServerTask
        ...
        async def _run(config, *, new_http):
            captured.clear()
            with patch("opencodon.tools.mcp_tool._MCP_HTTP_AVAILABLE", True), \
                 patch("opencodon.tools.mcp_tool._MCP_NEW_HTTP", new_http), \
                 patch("httpx.AsyncClient", DummyAsyncClient), \
                 patch("opencodon.tools.mcp_tool.streamable_http_client", return_value=DummyTransportCtx()), \
                 patch("opencodon.tools.mcp_tool.streamablehttp_client", side_effect=lambda url, **kwargs: DummyLegacyTransportCtx(**kwargs)), \
                 patch("opencodon.tools.mcp_tool.ClientSession", DummySession), \
...
class TestReconnection:
    ...
    def test_no_reconnect_on_initial_failure(self):
        ...
        """
        from opencodon.tools.mcp_tool import MCPServerTask, _MAX_INITIAL_CONNECT_RETRIES
    ...
    def test_initial_oauth_failure_does_not_retry(self):
        ...
        async def _test():
            ...
            with patch.object(MCPServerTask, "_run_stdio", patched_run_stdio), \
                 patch("opencodon.tools.mcp_tool._is_auth_error", return_value=True), \
...
class TestConfigurableTimeouts:
    ...
    def test_default_timeout(self):
        ...
        from opencodon.tools.mcp_tool import MCPServerTask, _DEFAULT_TOOL_TIMEOUT
    ...
    def test_timeout_passed_to_handler(self):
        ...
        try:
            ...
            with patch("opencodon.tools.mcp_tool._run_on_mcp_loop") as mock_run:
...
class TestUtilitySchemas:
    ...
    def test_builds_four_utility_schemas(self):
        from opencodon.tools.mcp_tool import _build_utility_schemas
...
class TestUtilityHandlers:
    ...
    def test_list_resources_success(self):
        from opencodon.tools.mcp_tool import _make_list_resources_handler, _servers
    ...
    def test_read_resource_success(self):
        from opencodon.tools.mcp_tool import _make_read_resource_handler, _servers
    ...
    def test_list_prompts_success(self):
        from opencodon.tools.mcp_tool import _make_list_prompts_handler, _servers
    ...
    def test_get_prompt_success(self):
        from opencodon.tools.mcp_tool import _make_get_prompt_handler, _servers
...
class TestMCPServerTaskSamplingIntegration:
    def test_sampling_handler_created_when_enabled(self):
        ...
        from opencodon.tools.mcp_tool import MCPServerTask, _MCP_SAMPLING_TYPES
...
class TestDiscoveryFailedCount:
    ...
    def test_failed_server_increments_failed_count(self):
        ...
        from opencodon.tools.mcp_tool import discover_mcp_tools, _servers, _ensure_mcp_loop
        ...
        with patch("opencodon.tools.mcp_tool._load_mcp_config", return_value=fake_config), \
             patch("opencodon.tools.mcp_tool._discover_and_register_server", side_effect=fake_register), \
             patch("opencodon.tools.mcp_tool._MCP_AVAILABLE", True), \
             patch("opencodon.tools.mcp_tool._existing_tool_names", return_value=["mcp__good_server__tool_a"]):
            _ensure_mcp_loop()

            # Capture the logger to verify failed_count in summary
            with patch("opencodon.tools.mcp_tool.logger") as mock_logger:
                discover_mcp_tools()
    ...
    def test_all_servers_fail_still_prints_summary(self):
        ...
        with patch("opencodon.tools.mcp_tool._load_mcp_config", return_value=fake_config), \
             patch("opencodon.tools.mcp_tool._discover_and_register_server", side_effect=always_fail), \
             patch("opencodon.tools.mcp_tool._MCP_AVAILABLE", True), \
             patch("opencodon.tools.mcp_tool._existing_tool_names", return_value=[]):
            _ensure_mcp_loop()
    ...
    def test_ok_servers_excludes_failures(self):
        ...
        with patch("opencodon.tools.mcp_tool._load_mcp_config", return_value=fake_config), \
             patch("opencodon.tools.mcp_tool._discover_and_register_server", side_effect=selective_register), \
             patch("opencodon.tools.mcp_tool._MCP_AVAILABLE", True), \
             patch("opencodon.tools.mcp_tool._existing_tool_names", return_value=["mcp__ok1__t", "mcp__ok2__t"]):
            _ensure_mcp_loop()
...
class TestMCPSelectiveToolLoading:
    ...
    def _run_discover(self, name, tool_names, config, session=None):
        from opencodon.tools.registry import ToolRegistry
        from opencodon.tools.mcp_tool import _discover_and_register_server, _servers
    ...
    def test_existing_tool_names_reflect_registered_subset(self):
        from opencodon.tools.mcp_tool import _existing_tool_names, _servers, _discover_and_register_server
        from opencodon.tools.registry import ToolRegistry
...
class TestRegistryCollisionWarning:
    ...
    def test_overwrite_same_toolset_no_warning(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="opencodon.tools.registry"):
            reg.register(name="my_tool", toolset="mcp-server", schema=schema, handler=handler)
...
class TestSanitizeMcpNameComponent:
    ...
    def test_hyphens_replaced(self):
        from opencodon.tools.mcp_tool import sanitize_mcp_name_component
    ...
    def test_slash_in_server_alias_resolution(self):
        ...
        with patch("opencodon.tools.registry.registry", reg):
...
class TestRegisterMcpServers:
    ...
    def test_skips_already_connected_servers(self):
        from opencodon.tools.mcp_tool import register_mcp_servers, _servers
        ...
        try:
            with patch("opencodon.tools.mcp_tool._MCP_AVAILABLE", True), \
                 patch("opencodon.tools.mcp_tool._existing_tool_names", return_value=["mcp__existing__tool"]):
    ...
    def test_connects_new_servers(self):
        from opencodon.tools.mcp_tool import register_mcp_servers, _servers, _ensure_mcp_loop
        ...
        with patch("opencodon.tools.mcp_tool._MCP_AVAILABLE", True), \
             patch("opencodon.tools.mcp_tool._discover_and_register_server", side_effect=fake_register), \
             patch("opencodon.tools.mcp_tool._existing_tool_names", return_value=["mcp__my_server__tool1"]):
            _ensure_mcp_loop()
    ...
    def test_logs_summary_on_success(self):
        ...
        with patch("opencodon.tools.mcp_tool._MCP_AVAILABLE", True), \
             patch("opencodon.tools.mcp_tool._discover_and_register_server", side_effect=fake_register), \
             patch("opencodon.tools.mcp_tool._existing_tool_names", return_value=["mcp__srv__t1", "mcp__srv__t2"]):
            _ensure_mcp_loop()
...
class TestMcpParallelToolCalls:
    ...
    def test_register_mcp_servers_tracks_parallel_flag(self):
        ...
        with patch("opencodon.tools.mcp_tool._MCP_AVAILABLE", True), \
             patch("opencodon.tools.mcp_tool._ensure_mcp_loop"), \
             patch("opencodon.tools.mcp_tool._run_on_mcp_loop"), \
             patch("opencodon.tools.mcp_tool._existing_tool_names", return_value=[]):
            register_mcp_servers(fake_config)
