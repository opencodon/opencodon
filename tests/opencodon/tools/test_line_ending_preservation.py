...
def opencodon_home(monkeypatch, tmp_path):
    """Isolate OPENCODON_HOME so the tests don't pollute the real config.
    ...
    try:
        from opencodon.tools.file_tools import clear_file_ops_cache, _read_tracker_lock, _read_tracker
        ...
        pass
    try:
        from opencodon.tools.terminal_tool import _active_environments, _env_lock
        with _env_lock:
...
class TestPatchCRLFPreservation:
    def test_patch_on_crlf_file_stays_pure_crlf(self, opencodon_home, tmp_path):
        """LLM sends LF old/new; file has CRLF.  Result must be all CRLF,
        no mixed endings."""
        from opencodon.tools.file_tools import _handle_patch
    ...
    def test_patch_on_lf_file_stays_lf(self, opencodon_home, tmp_path):
    ...
    def test_patch_multiline_replacement_on_crlf(self, opencodon_home, tmp_path):
...
class TestWriteFileCRLFPreservation:
    ...
    def test_new_file_written_as_is(self, opencodon_home, tmp_path):
    ...
    def test_overwrite_lf_file_stays_lf(self, opencodon_home, tmp_path):
...
class TestLineEndingHelpers:
    ...
    def test_detect_crlf(self):
        from opencodon.tools.file_operations import _detect_line_ending
    ...
    def test_normalize_to_lf_strips_cr(self):
        from opencodon.tools.file_operations import _normalize_line_endings
