...
import opencodon.tools.environments.docker as docker_env
...
def test_probe_returns_true_when_container_starts(monkeypatch):
    ...
    assert docker_env._cgroup_limits_available("opencodon:latest") is True
    ...
    assert "opencodon:latest" in captured["cmd"]
