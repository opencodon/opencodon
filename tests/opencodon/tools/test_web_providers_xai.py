...
class TestXAIProviderIsAvailable:
    ...
    auth-store lock. It runs on every ``opencodon tools`` repaint and at
    ...
    def test_available_via_auth_store(self, monkeypatch, tmp_path):
        """Cheap probe should detect xai-oauth tokens in ~/.opencodon/auth.json
...
class TestXAIBackendWiring:
    ...
    def test_is_backend_available_does_not_call_resolver(self, monkeypatch):
        ...
        dispatch and every `opencodon tools` repaint. It must not invoke the
        OAuth resolver (which can trigger a network refresh)."""
        from opencodon.tools import web_tools

        monkeypatch.setenv("XAI_API_KEY", "sk-xai-test")
        with patch(
            "opencodon.tools.xai_http.resolve_xai_http_credentials",
    ...
    def test_xai_not_in_legacy_backend_candidate_chain(self, monkeypatch):
        ...
        chain is for users who set env vars but never ran ``opencodon tools``,
...
class TestXAIProviderOAuthPath:
    ...
    credential pool (``opencodon auth`` sign-in) rather than an env-var key.
    ...
    def test_search_uses_oauth_bearer_token_and_base_url(self, monkeypatch, tmp_path):
        ...
        monkeypatch.delenv("XAI_API_KEY", raising=False)
        monkeypatch.setenv("OPENCODON_HOME", str(tmp_path))
        monkeypatch.setenv("OPENCODON_XAI_BASE_URL", "https://proxy.x.ai/v1/")
    ...
    def test_pool_only_direct_refresh_updates_main_runtime(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "opencodon.frontends.cli.auth.refresh_xai_oauth_pure",
            fake_refresh,
        )
