...
def _reset_backend():
    ...
    from opencodon.tools.computer_use.tool import reset_backend_for_tests
    reset_backend_for_tests()
    # Force the noop backend.
    with patch.dict(os.environ, {"OPENCODON_COMPUTER_USE_BACKEND": "noop"}, clear=False):
        yield
    reset_backend_for_tests()
...
def noop_backend():
    ...
    from opencodon.tools.computer_use.tool import _get_backend
    return _get_backend()
...
class TestSchema:
    def test_schema_is_universal_openai_function_format(self):
        from opencodon.tools.computer_use.schema import COMPUTER_USE_SCHEMA
    ...
    def test_schema_max_elements_documents_default_and_upper_bound(self):
        ...
        """
        from opencodon.tools.computer_use.schema import COMPUTER_USE_SCHEMA
        from opencodon.tools.computer_use.tool import (
...
class TestRegistration:
    def test_tool_registers_with_registry(self):
        ...
        import opencodon.tools.computer_use_tool  # noqa: F401
    ...
    def test_check_fn_true_on_linux_when_binary_present(self):
        ...
        with patch("opencodon.tools.computer_use.tool.sys.platform", "linux"), \
             patch("opencodon.tools.computer_use.cua_backend.cua_driver_binary_available", return_value=True):
            assert cu_tool.check_computer_use_requirements() is True

    def test_check_fn_false_on_linux_without_binary(self):
        from opencodon.tools.computer_use import tool as cu_tool
        with patch("opencodon.tools.computer_use.tool.sys.platform", "linux"), \
             patch("opencodon.tools.computer_use.cua_backend.cua_driver_binary_available", return_value=False):
            assert cu_tool.check_computer_use_requirements() is False

    def test_check_fn_false_on_unsupported_platform(self):
        from opencodon.tools.computer_use import tool as cu_tool
        with patch("opencodon.tools.computer_use.tool.sys.platform", "freebsd13"):
            assert cu_tool.check_computer_use_requirements() is False
    ...
    def test_check_fn_true_on_windows_when_binary_present(self):
        ...
        from opencodon.tools.computer_use import tool as cu_tool
        with patch("opencodon.tools.computer_use.tool.sys.platform", "win32"), \
             patch("opencodon.tools.computer_use.cua_backend.cua_driver_binary_available", return_value=True):
            assert cu_tool.check_computer_use_requirements() is True
    ...
    def test_check_fn_finds_user_local_cua_driver_when_path_omits_it(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("OPENCODON_CUA_DRIVER_CMD", raising=False)
        monkeypatch.setenv("HOME", str(tmp_path))
        monkeypatch.setenv("PATH", "/usr/bin:/bin:/usr/sbin:/sbin")

        with patch("opencodon.tools.computer_use.tool.sys.platform", "darwin"), \
    ...
    def test_cua_driver_cmd_env_override_is_resolved_dynamically(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OPENCODON_CUA_DRIVER_CMD", str(driver))
...
class TestDispatch:
    def test_missing_action_returns_error(self):
        from opencodon.tools.computer_use.tool import handle_computer_use
    ...
    def test_capture_after_skipped_when_action_failed(self, noop_backend):
        ...
        from opencodon.tools.computer_use.backend import ActionResult
        from opencodon.tools.computer_use.tool import handle_computer_use
...
class TestCaptureResponse:
    ...
    def test_capture_vision_mode_with_image_returns_multimodal_envelope(self):
        ...
        from opencodon.tools.computer_use.backend import CaptureResult
        from opencodon.tools.computer_use import tool as cu_tool
    ...
    def test_capture_tiny_image_returns_text_json(self):
        ...
        from opencodon.tools.computer_use.backend import CaptureResult, UIElement
        from opencodon.tools.computer_use import tool as cu_tool
...
class TestCuaCaptureImageDimensions:
    def test_png_dimensions_are_sniffed_from_image_bytes(self):
        from opencodon.tools.computer_use.cua_backend import _image_dimensions_from_bytes
...
class TestRunAgentMultimodalHelpers:
    def test_is_multimodal_tool_result(self):
        from opencodon.core.run_agent import _is_multimodal_tool_result
    ...
    def test_multimodal_text_summary_prefers_summary(self):
        from opencodon.core.run_agent import _multimodal_text_summary
    ...
    def test_append_subdir_hint_to_multimodal_appends_to_text_part(self):
        from opencodon.core.run_agent import _append_subdir_hint_to_multimodal
        env = {
            "_multimodal": True,
            "content": [
                {"type": "text", "text": "summary"},
    ...
    def test_trajectory_normalize_strips_images(self):
        from opencodon.core.run_agent import _trajectory_normalize_msg
        msg = {
            "role": "tool",
            "tool_call_id": "c1",
            "content": [
...
class TestElementLabelParsing:
    ...
    def test_classic_quoted_label_format(self):
        from opencodon.tools.computer_use.cua_backend import _parse_elements_from_tree
        tree = (
            '  - [14] AXButton "One"\n'
            '  - [15] AXButton "Two"\n'
            '  - [16] AXTextField ""\n'
...
class TestUpdateCheck:
    ...
    def _run_returning(stdout: str):
        fake = MagicMock()
        fake.stdout = stdout
        return patch("opencodon.tools.computer_use.cua_backend.subprocess.run", return_value=fake)
    ...
    def test_subprocess_failure_is_quiet(self):
        from opencodon.tools.computer_use import cua_backend
        with patch("opencodon.tools.computer_use.cua_backend.subprocess.run",
...
class TestLazyMcpInstall:
    ...
    def test_start_lazy_installs_mcp(self):
        ...
        with patch.object(cua_backend, "_maybe_nudge_update"), \
             patch("opencodon.tools.lazy_deps.ensure") as mock_ensure, \
    ...
    def test_start_propagates_feature_unavailable(self):
        ...
        with patch.object(cua_backend, "_maybe_nudge_update"), \
             patch("opencodon.tools.lazy_deps.ensure", side_effect=unavailable), \
             patch.object(cua_backend._CuaDriverSession, "start") as mock_sess_start:
...
class TestCaptureAfterAppContext:
    ...
    def test_capture_after_uses_last_app(self):
        ...
        from opencodon.tools.computer_use.backend import ActionResult, CaptureResult
        from opencodon.tools.computer_use import tool as cu_tool
...
def _make_cua_backend_with_windows(windows: List[Dict[str, Any]]):
    ...
    from opencodon.tools.computer_use.cua_backend import CuaDriverBackend
...
class TestCuaDriverSessionReconnect:
    ...
    def test_extract_tool_result_missing_mock_error_flag_is_not_error(self):
        from opencodon.tools.computer_use.cua_backend import _extract_tool_result
...
class TestCaptureAfterExactTarget:
    def test_followup_capture_reuses_exact_window_identity(self):
        from opencodon.tools.computer_use.backend import ActionResult, CaptureResult
        from opencodon.tools.computer_use.tool import _maybe_follow_capture
    ...
    def test_dispatch_capture_after_reuses_metadata_resolved_identity(self, monkeypatch):
        from opencodon.tools.computer_use import tool as computer_use_tool
...
class TestCuaEnvironmentScrubbing:
    ...
    def test_cua_session_sanitizes_provider_env_vars(self):
        ...
        from opencodon.tools.computer_use.cua_backend import _CuaDriverSession, _AsyncBridge
        import asyncio
        ...
        async def drive_lifecycle():
            ...
            with patch.dict(os.environ, test_env, clear=True), \
                 patch("opencodon.tools.computer_use.cua_backend.resolve_cua_driver_cmd",
                       return_value="cua-driver"), \
                 patch("opencodon.tools.computer_use.cua_backend._resolve_mcp_invocation",
...
class TestCuaCliFallbackResolution:
    def test_cli_fallback_uses_resolved_driver_under_thin_path(self):
        ...
        """
        from opencodon.tools.computer_use.cua_backend import _AsyncBridge, _CuaDriverSession
...
class TestZIndexSorting:
    ...
    def test_null_z_index_treated_as_lowest(self):
        ...
        from opencodon.tools.computer_use.cua_backend import _ingest_windows
...
class TestImageMimeTypePropagation:
    ...
    def test_capture_response_uses_explicit_mime_when_provided(self):
        from opencodon.tools.computer_use.backend import CaptureResult
        from opencodon.tools.computer_use.tool import _capture_response
...
class TestMcpInvocationResolution:
    ...
    def _no_overlay_off(self):
        ...
        with patch("opencodon.tools.computer_use.cua_backend._cua_no_overlay",
                   return_value=False):
            yield
    ...
    def test_falls_back_when_manifest_missing_command(self):
        ...
        resolved driver path (so OPENCODON_CUA_DRIVER_CMD still wins)."""
        from unittest.mock import patch
        from opencodon.tools.computer_use.cua_backend import _resolve_mcp_invocation
...
class TestStructuredElementsConsumption:
    ...
    def test_structured_parser_reads_frames(self):
        from opencodon.tools.computer_use.cua_backend import _parse_elements_from_structured
