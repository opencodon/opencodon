...
from opencodon.tools.code_execution_tool import (
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        with patch("opencodon.tools.code_execution_tool._load_config", return_value={"timeout": 30, "max_tool_calls": 5}), \
             patch("opencodon.tools.code_execution_tool._get_or_create_env", return_value=(env, "ssh")), \
             patch("opencodon.tools.code_execution_tool._ship_file_to_remote"), \
             patch("opencodon.tools.code_execution_tool.threading.Thread", return_value=fake_thread):
    ...
    def test_timezone_shell_quoted_in_remote_execution(self):
        ...
        with patch("opencodon.tools.code_execution_tool._load_config",
                   return_value={"timeout": 30, "max_tool_calls": 5}), \
             patch("opencodon.tools.code_execution_tool._get_or_create_env",
                   return_value=(env, "ssh")), \
             patch("opencodon.tools.code_execution_tool._ship_file_to_remote"), \
             patch("opencodon.tools.code_execution_tool.threading.Thread",
                   return_value=fake_thread), \
             patch.dict(os.environ, {"OPENCODON_TIMEZONE": malicious_tz}):
            result = json.loads(_execute_remote("print('hello')", "task-1", ["terminal"]))
...
class TestExecuteCode(unittest.TestCase):
    ...
    def _run(self, code, enabled_tools=None):
        ...
        with patch("opencodon.tools.code_execution_tool._rpc_server_loop") as mock_rpc:
            # Use real execution but mock the tool dispatcher
            pass
        # Actually run with full integration, mocking at the model_tools level
        with patch("opencodon.tools.model_tools.handle_function_call", side_effect=_mock_handle_function_call):
            result = execute_code(
                code=code,
                task_id="test-task",
    ...
    def test_repo_root_modules_are_importable(self):
        """Sandboxed scripts can import the canonical opencodon package."""
        result = self._run(
            'from opencodon.common import constants; print(constants.__file__)'
        )
        self.assertEqual(result["status"], "success")
...
'''
        ...
        with patch("opencodon.tools.model_tools.handle_function_call", side_effect=slow_mock):
            raw = execute_code(
                code=code,
...
"""
    ...
    def test_timeout_enforcement(self):
        ...
        with patch("opencodon.tools.model_tools.handle_function_call", side_effect=_mock_handle_function_call):
            ...
            with patch("opencodon.tools.code_execution_tool._load_config", return_value={"timeout": 2, "max_tool_calls": 50}):
...
class TestStubSchemaDrift(unittest.TestCase):
    ...
    def test_stubs_cover_all_schema_params(self):
        ...
        import re
        from opencodon.tools.code_execution_tool import _TOOL_STUBS

        # Import the registry and trigger tool registration
        from opencodon.tools.registry import registry
        import opencodon.tools.file_tools  # noqa: F401 - registers read_file, write_file, patch, search_files
        import opencodon.tools.web_tools  # noqa: F401 - registers web_search, web_extract
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def _get_child_env(self, extra_env=None):
        ...
        try:
            ...
            with patch("opencodon.tools.model_tools.handle_function_call", return_value='{}'), \
                 patch("opencodon.tools.code_execution_tool._load_config",
...
class TestExecuteCodeEdgeCases(unittest.TestCase):
    ...
    def test_windows_returns_error(self):
        ...
        with patch("opencodon.tools.code_execution_tool.SANDBOX_AVAILABLE", False):
    ...
    def test_none_enabled_tools_uses_all(self):
        ...
        with patch("opencodon.tools.model_tools.handle_function_call",
...
class TestLoadConfig(unittest.TestCase):
    def test_returns_empty_dict_when_cli_config_unavailable(self):
        from opencodon.tools.code_execution_tool import _load_config
...
'''
    ...
    def test_remote_large_output_gets_truncation_metadata(self):
        ...
        with patch("opencodon.tools.code_execution_tool._load_config", return_value={"timeout": 30, "max_tool_calls": 5}), \
             patch("opencodon.tools.code_execution_tool._get_or_create_env", return_value=(FakeEnv(), "ssh")), \
             patch("opencodon.tools.code_execution_tool._ship_file_to_remote"), \
             patch("opencodon.tools.code_execution_tool.threading.Thread", return_value=fake_thread):
...
class TestRpcTokenAuthorization(unittest.TestCase):
    ...
    request without the matching OPENCODON_RPC_TOKEN must be rejected before
    ...
    def _drive_server(self, rpc_token, requests):
        ...
        """
        from opencodon.tools.code_execution_tool import _rpc_server_loop
        ...
        def _run():
            with patch(
                "opencodon.tools.model_tools.handle_function_call",
