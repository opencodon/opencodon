...
class TestGetSubprocessHome:
    """Unit tests for opencodon_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(opencodon_constants, "is_container", lambda: False)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("OPENCODON_REAL_HOME", raising=False)
    ...
    def test_returns_none_when_opencodon_home_unset(self, monkeypatch):
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        opencodon_home = real_home / ".opencodon" / "profiles" / "coder"
        profile_home = opencodon_home / "home"
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OPENCODON_REAL_HOME", str(real_home))

        from opencodon_constants import get_subprocess_home, get_real_home
    ...
    def test_real_home_falls_back_to_os_account_when_home_is_profile(self, tmp_path, monkeypatch):
        ...
        from opencodon_constants import get_real_home

        assert get_real_home() != str(profile_home)

    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        self._container_mode(monkeypatch)
        base = tmp_path / ".opencodon" / "profiles"
        ...
        monkeypatch.setenv("OPENCODON_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("OPENCODON_HOME", str(base / "beta"))
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_opencodon_home()))
        ...
        try:
            assert get_opencodon_home() == profile
            release.set()
        ...
        assert get_opencodon_home() == root
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_no_injection_when_opencodon_home_unset(self, monkeypatch):
        monkeypatch.delenv("OPENCODON_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["OPENCODON_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from opencodon.core.profiles import _PROFILE_DIRS
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from opencodon.core.profiles import create_profile
...
class TestPythonProcessUnchanged:
    ...
    def test_path_home_unchanged_after_subprocess_home_resolved(
        ...
        assert sub_home in (None, str(opencodon_home / "home"), original_home)
