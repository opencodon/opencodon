...
def test_x_search_posts_responses_request(monkeypatch):
    ...
    assert captured["headers"]["User-Agent"] == f"opencodon-Agent/{__version__}"
...
def test_x_search_retries_read_timeout_then_succeeds(monkeypatch):
    ...
    monkeypatch.setenv("XAI_API_KEY", "xai-test-key")
    monkeypatch.setattr("requests.post", _fake_post)
    monkeypatch.setattr("opencodon.tools.x_search_tool.time.sleep", lambda *_: None)
...
def test_x_search_uses_xai_oauth_when_only_oauth_available(monkeypatch):
    ...
    from opencodon.tools.x_search_tool import check_x_search_requirements, x_search_tool
    ...
    monkeypatch.setattr(
        "opencodon.tools.x_search_tool.resolve_xai_http_credentials", _fake_resolve
    )
...
def test_x_search_returns_tool_error_when_no_credentials(monkeypatch):
    ...
    assert "opencodon auth add xai-oauth" in result
...
def test_x_search_check_fn_false_when_resolver_raises(monkeypatch):
    ...
    from opencodon.tools.registry import invalidate_check_fn_cache
    from opencodon.tools.x_search_tool import check_x_search_requirements
    ...
    monkeypatch.setattr(
        "opencodon.tools.x_search_tool.resolve_xai_http_credentials", _boom
    )
    invalidate_check_fn_cache()
...
def test_x_search_honors_config_model_and_timeout(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "opencodon.tools.x_search_tool._load_x_search_config",
...
def test_x_search_registered_in_registry_with_check_fn():
    ...
    import opencodon.tools.x_search_tool  # noqa: F401 — ensures registration runs
...
def test_x_search_rejects_future_from_date(monkeypatch):
    ...
    monkeypatch.setattr("opencodon.tools.x_search_tool.datetime", _FrozenDateTime)
