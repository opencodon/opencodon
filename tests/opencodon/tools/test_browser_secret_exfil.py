...
class TestBrowserSecretExfil:
    ...
    def test_cloud_blocks_opaque_sensitive_query_param(self):
        ...
        with patch("opencodon.tools.browser_tool._is_local_backend", return_value=False), \
             patch("opencodon.tools.browser_tool._navigation_session_key", return_value="default"), \
             patch("opencodon.tools.browser_tool._run_browser_command") as mock_run:
    ...
    def test_local_browser_allows_opaque_sensitive_query_param(self):
        ...
        with patch("opencodon.tools.browser_tool._run_browser_command", return_value=mock_result), \
             patch("opencodon.tools.browser_tool._get_session_info", return_value={"_first_nav": False}), \
             patch("opencodon.tools.browser_tool._is_local_backend", return_value=True):
            result = browser_navigate("https://example.com/callback?token=opaque-oauth-code")
    ...
    def test_allows_normal_url(self):
        ...
        mock_result = {"success": True, "data": {"title": "ok", "url": "https://github.com/opencodon/opencodon"}}
        with patch("opencodon.tools.browser_tool._run_browser_command", return_value=mock_result), \
             patch("opencodon.tools.browser_tool._get_session_info", return_value={"_first_nav": False}), \
             patch("opencodon.tools.browser_tool._is_local_backend", return_value=True):
            result = browser_navigate("https://github.com/opencodon/opencodon")
        parsed = json.loads(result)
    ...
    def test_normalizes_non_ascii_url_before_navigation(self):
        ...
        with patch("opencodon.tools.browser_tool._run_browser_command", side_effect=mock_run), \
             patch("opencodon.tools.browser_tool._get_session_info", return_value={"_first_nav": False}), \
             patch("opencodon.tools.browser_tool._is_local_backend", return_value=True):
...
class TestBrowserSnapshotRedaction:
    ...
    def test_extract_relevant_content_redacts_secrets(self):
        ...
        with patch("opencodon.tools.browser_tool.call_llm", mock_call_llm):
...
class TestBrowserSupervisorRedaction:
    ...
    def test_pending_and_recent_dialog_messages_redacted(self):
        from opencodon.tools.browser_supervisor import DialogRecord, PendingDialog, SupervisorSnapshot
