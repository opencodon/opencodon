...
def make_env(daytona_sdk, monkeypatch):
    ...
    monkeypatch.setattr("opencodon.tools.environments.base.is_interrupted", lambda: False)
    # Prevent skills/credential sync from consuming mock exec calls
    monkeypatch.setattr("opencodon.tools.credential_files.get_credential_file_mounts", lambda: [])
    monkeypatch.setattr("opencodon.tools.credential_files.get_skills_directory_mount", lambda **kw: None)
    monkeypatch.setattr("opencodon.tools.credential_files.iter_skills_files", lambda **kw: [])
    ...
    def _factory(
        ...
        **kwargs,
        ...
        from opencodon.tools.environments.daytona import DaytonaEnvironment
...
class TestPersistence:
    def test_persistent_resumes_via_get(self, make_env):
        ...
        env._mock_client.get.assert_called_once_with("opencodon-mytask")
    ...
    def test_persistent_resumes_legacy_via_list(self, make_env, daytona_sdk):
        ...
        env._mock_client.list.assert_called_once_with(
            labels={"opencodon_task_id": "mytask"}, limit=1)
        env._mock_client.create.assert_not_called()
    ...
    def test_persistent_creates_new_when_none_found(self, make_env, daytona_sdk):
        ...
        env._mock_client.get.assert_called_with("opencodon-mytask")
...
class TestInterrupt:
    def test_interrupt_stops_sandbox_and_returns_130(self, make_env, monkeypatch):
        ...
        monkeypatch.setattr(
            "opencodon.tools.environments.base.is_interrupted", lambda: True
        )
        try:
