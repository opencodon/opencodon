...
class TestFindAgentBrowser:
    ...
    def test_finds_in_current_path(self):
        ...
        with patch("shutil.which", return_value="/usr/local/bin/agent-browser"), \
             patch("opencodon.tools.browser_tool.agent_browser_runnable", return_value=True):
    ...
    def test_finds_in_homebrew_bin(self):
        ...
        with patch("shutil.which", side_effect=mock_which), \
             ...
             patch(
                 "opencodon.tools.browser_tool._discover_homebrew_node_dirs",
                 return_value=[],
             ):
...
class TestBrowserRequirements:
    def test_cdp_override_does_not_require_agent_browser_cli(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.tools.browser_tool._is_camofox_mode", lambda: False)
        monkeypatch.setattr("opencodon.tools.browser_tool._find_agent_browser", lambda: (_ for _ in ()).throw(FileNotFoundError("not found")))
    ...
    def test_termux_requires_real_agent_browser_install_not_npx_fallback(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.tools.browser_tool._is_camofox_mode", lambda: False)
        monkeypatch.setattr("opencodon.tools.browser_tool._get_cloud_provider", lambda: None)
        monkeypatch.setattr("opencodon.tools.browser_tool._find_agent_browser", lambda **_kw: "npx agent-browser")
...
class TestRunBrowserCommandPathConstruction:
    ...
    def test_subprocess_preserves_executable_path_with_spaces(self, tmp_path):
        ...
        with patch("opencodon.tools.browser_tool._find_agent_browser", return_value=browser_path), \
 patch("opencodon.tools.browser_tool._chromium_installed", return_value=True), \
             patch("opencodon.tools.browser_tool._get_session_info", return_value=fake_session), \
             patch("opencodon.tools.browser_tool._socket_safe_tmpdir", return_value=str(tmp_path)), \
             patch("opencodon.tools.browser_tool._discover_homebrew_node_dirs", return_value=[]), \
             patch("opencodon_constants.Path.home", return_value=tmp_path), \
    ...
    def test_subprocess_splits_npx_fallback_into_command_and_package(self, tmp_path):
        ...
        with patch("opencodon.tools.browser_tool._find_agent_browser", return_value="npx agent-browser"), \
 patch("opencodon.tools.browser_tool._chromium_installed", return_value=True), \
             patch("opencodon.tools.browser_tool._get_session_info", return_value=fake_session), \
             patch("opencodon.tools.browser_tool._socket_safe_tmpdir", return_value=str(tmp_path)), \
             patch("opencodon.tools.browser_tool._discover_homebrew_node_dirs", return_value=[]), \
    ...
    def test_subprocess_path_includes_homebrew_node_dirs(self, tmp_path):
        ...
        with patch("opencodon.tools.browser_tool._find_agent_browser", return_value="/usr/local/bin/agent-browser"), \
 patch("opencodon.tools.browser_tool._chromium_installed", return_value=True), \
             patch("opencodon.tools.browser_tool._get_session_info", return_value=fake_session), \
             patch("opencodon.tools.browser_tool._socket_safe_tmpdir", return_value=str(tmp_path)), \
             patch("opencodon.tools.browser_tool._discover_homebrew_node_dirs", return_value=fake_homebrew_dirs), \
             patch("os.path.isdir", side_effect=selective_isdir), \
             patch("subprocess.Popen", side_effect=capture_popen), \
             patch("os.open", return_value=99), \
             patch("os.close"), \
