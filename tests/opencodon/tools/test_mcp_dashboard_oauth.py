...
def test_mcp_oauth_helpers_use_dashboard_flow_without_loopback_port():
    from opencodon.tools.mcp_dashboard_oauth import DashboardOAuthFlow, dashboard_oauth_flow
...
def test_manager_build_allows_dashboard_flow_without_tty(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("opencodon.tools.mcp_oauth.sys.stdin.isatty", lambda: False)
    flow = DashboardOAuthFlow(
        flow_id="flow-5",
        server_name="reports",
        profile=None,
...
def test_reconnect_mcp_server_signals_live_task(monkeypatch):
    from opencodon.tools import mcp_tool
