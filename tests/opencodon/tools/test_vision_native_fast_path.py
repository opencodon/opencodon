...
class TestVisionAnalyzeNative:
    ...
    def test_oversized_image_resized_under_embed_cap(self, tmp_path):
        ...
        from opencodon.tools.vision_tools import _EMBED_TARGET_BYTES
...
class TestHandleVisionAnalyzeFastPath:
    ...
    def test_vision_capable_main_model_uses_fast_path(self, tmp_path, monkeypatch):
        ...
        from opencodon.core.auxiliary_client import set_runtime_main, clear_runtime_main
    ...
    def test_non_vision_main_model_falls_through_to_aux(self, tmp_path, monkeypatch):
        ...
        try:
            with patch("opencodon.tools.vision_tools.vision_analyze_tool", side_effect=_aux_sentinel):
                coro = _handle_vision_analyze({"image_url": str(img), "question": "?"})
                result = asyncio.get_event_loop().run_until_complete(coro)
        finally:
            clear_runtime_main()
    ...
    def test_supports_vision_override_bypasses_provider_allowlist(self, tmp_path):
        ...
        try:
            with patch(
                ...
                "opencodon.tools.vision_tools.vision_analyze_tool", side_effect=_aux_sentinel,
            ) as mock_aux:
                coro = _handle_vision_analyze({"image_url": str(img), "question": "?"})
                result = asyncio.get_event_loop().run_until_complete(coro)
        finally:
