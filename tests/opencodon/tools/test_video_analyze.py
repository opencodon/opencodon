...
class TestHandleVideoAnalyze:
    ...
    def test_returns_awaitable(self, tmp_path, monkeypatch):
        ...
        with patch("opencodon.tools.vision_tools.video_analyze_tool", new_callable=AsyncMock) as mock_tool:
...
class TestVideoAnalyzeTool:
    ...
    def test_local_file_success(self, tmp_path, monkeypatch):
        ...
        with patch("opencodon.tools.vision_tools.async_call_llm", new_callable=AsyncMock, return_value=mock_response):
            with patch("opencodon.tools.vision_tools.extract_content_or_reasoning", return_value="A short video showing a demo."):
    ...
    def test_video_too_large(self, tmp_path, monkeypatch):
        ...
        with patch("opencodon.tools.vision_tools._video_to_base64_data_url") as mock_encode:
    ...
    def test_interrupt_check(self, tmp_path):
        ...
        with patch("opencodon.tools.interrupt.is_interrupted", return_value=True):
            result = self._run(video_analyze_tool(str(video), "What?"))
    ...
    def test_empty_response_retries(self, tmp_path):
        ...
        with patch("opencodon.tools.vision_tools.async_call_llm", side_effect=fake_llm):
            with patch("opencodon.tools.vision_tools.extract_content_or_reasoning", side_effect=["", "Video analysis result."]):
                result = self._run(video_analyze_tool(str(video), "What?"))
    ...
    def test_file_scheme_stripped(self, tmp_path):
        ...
        with patch("opencodon.tools.vision_tools.async_call_llm", new_callable=AsyncMock, return_value=mock_response):
            with patch("opencodon.tools.vision_tools.extract_content_or_reasoning", return_value="OK"):
    ...
    def test_api_message_format(self, tmp_path):
        ...
        with patch("opencodon.tools.vision_tools.async_call_llm", side_effect=capture_llm):
            with patch("opencodon.tools.vision_tools.extract_content_or_reasoning", return_value="OK"):
...
class TestVideoToolsetRegistration:
    ...
    def test_not_in_core_tools(self):
        """video_analyze should NOT be in _OPENCODON_CORE_TOOLS (default disabled)."""
        from toolsets import _OPENCODON_CORE_TOOLS
        assert "video_analyze" not in _OPENCODON_CORE_TOOLS
