...
class TestBrowserConsole:
    ...
    def test_returns_console_messages_and_errors(self):
        from opencodon.tools.browser_tool import browser_console
        ...
        with patch("opencodon.tools.browser_tool._run_browser_command") as mock_cmd:
    ...
    def test_passes_clear_flag(self):
        ...
        with patch("opencodon.tools.browser_tool._run_browser_command", return_value=empty) as mock_cmd:
    ...
    def test_empty_console_and_errors(self):
        ...
        empty = {"success": True, "data": {"messages": [], "errors": []}}
        with patch("opencodon.tools.browser_tool._run_browser_command", return_value=empty):
            result = json.loads(browser_console(task_id="test"))
    ...
    def test_handles_failed_commands(self):
        ...
        with patch("opencodon.tools.browser_tool._run_browser_command", return_value=failed):
            result = json.loads(browser_console(task_id="test"))
    ...
    def test_redacts_secrets_from_eval_result(self):
        from opencodon.tools.browser_tool import _browser_eval

        fake_key = "ghp_" + "BROWSEREVALSECRET1234567890"
        with patch("opencodon.tools.browser_tool._last_session_key", return_value="test"), \
             patch("opencodon.tools.browser_tool._is_camofox_mode", return_value=False), \
             patch("opencodon.tools.browser_tool._run_browser_command", return_value={"success": True, "data": {"result": fake_key}}):
    ...
    def test_redacts_secrets_from_snapshot_output(self):
        from opencodon.tools.browser_tool import browser_snapshot
        ...
        with patch("opencodon.tools.browser_tool._last_session_key", return_value="test"), \
             patch("opencodon.tools.browser_tool._is_camofox_mode", return_value=False), \
             patch("opencodon.tools.browser_tool._run_browser_command", return_value=snapshot_response):
    ...
    def test_expression_allows_harmless_dom_inspection(self):
        ...
        with patch("opencodon.tools.browser_tool._allow_unsafe_browser_evaluate", return_value=False), \
             patch("opencodon.tools.browser_tool._browser_eval", return_value=json.dumps({"success": True, "result": "Example"})) as mock_eval:
    ...
    def test_expression_allows_risky_eval_by_default(self):
        ...
        with patch("opencodon.tools.browser_tool._browser_eval", return_value=json.dumps({"success": True, "result": "ok"})) as mock_eval:
    ...
    def test_expression_blocks_cookie_access_before_eval(self):
        ...
        with patch("opencodon.tools.browser_tool._restrict_browser_evaluate", return_value=True), \
             patch("opencodon.tools.browser_tool._allow_unsafe_browser_evaluate", return_value=False), \
             patch("opencodon.tools.browser_tool._browser_eval") as mock_eval:
    ...
    def test_expression_allows_string_literals_without_sensitive_tokens(self):
        ...
        with patch("opencodon.tools.browser_tool._restrict_browser_evaluate", return_value=True), \
             patch("opencodon.tools.browser_tool._allow_unsafe_browser_evaluate", return_value=False), \
             patch("opencodon.tools.browser_tool._browser_eval", return_value=json.dumps({"success": True, "result": True})) as mock_eval:
    ...
    def test_expression_config_opt_in_allows_risky_eval(self):
        ...
        with patch("opencodon.tools.browser_tool._restrict_browser_evaluate", return_value=True), \
             patch("opencodon.tools.browser_tool._allow_unsafe_browser_evaluate", return_value=True), \
             patch("opencodon.tools.browser_tool._browser_eval", return_value=json.dumps({"success": True, "result": "cookie=value"})) as mock_eval:
            result = json.loads(browser_console(expression="document.cookie", task_id="test"))
    ...
    def test_allow_unsafe_evaluate_reads_browser_config(self):
        from opencodon.tools.browser_tool import _allow_unsafe_browser_evaluate

        with patch("opencodon.config.read_raw_config", return_value={"browser": {"allow_unsafe_evaluate": "true"}}):
            assert _allow_unsafe_browser_evaluate() is True
        with patch("opencodon.config.read_raw_config", return_value={"browser": {"allow_unsafe_evaluate": False}}):
            assert _allow_unsafe_browser_evaluate() is False

    def test_restrict_evaluate_reads_browser_config(self):
        from opencodon.tools.browser_tool import _restrict_browser_evaluate

        with patch("opencodon.config.read_raw_config", return_value={"browser": {"restrict_evaluate": "true"}}):
            assert _restrict_browser_evaluate() is True
        with patch("opencodon.config.read_raw_config", return_value={"browser": {"restrict_evaluate": False}}):
            assert _restrict_browser_evaluate() is False
        # Default (key absent) is off — the denylist is opt-in.
        with patch("opencodon.config.read_raw_config", return_value={}):
            assert _restrict_browser_evaluate() is False
...
class TestBrowserConsoleSchema:
    ...
    def test_schema_in_browser_schemas(self):
        from opencodon.tools.browser_tool import BROWSER_TOOL_SCHEMAS
...
class TestBrowserConsoleToolsetWiring:
    ...
    def test_in_legacy_toolset_map(self):
        from opencodon.tools.model_tools import _LEGACY_TOOLSET_MAP
        assert "browser_console" in _LEGACY_TOOLSET_MAP["browser_tools"]

    def test_in_registry(self):
        from opencodon.tools.registry import registry
        from opencodon.tools import browser_tool  # noqa: F401
...
class TestBrowserVisionAnnotate:
    ...
    def test_annotate_false_no_flag(self):
        ...
        from opencodon.tools.browser_tool import browser_vision

        with (
            patch("opencodon.tools.browser_tool._run_browser_command") as mock_cmd,
            patch("opencodon.tools.browser_tool.call_llm") as mock_call_llm,
            patch("opencodon.tools.browser_tool._get_vision_model", return_value="test-model"),
        ):
...
class TestBrowserVisionConfig:
    ...
    def test_browser_vision_uses_configured_temperature_and_timeout(self, tmp_path):
        ...
        with (
            ...
            patch("opencodon.tools.browser_tool._cleanup_old_screenshots"),
            patch("opencodon.tools.browser_tool._run_browser_command", return_value={"success": True, "data": {"path": str(screenshot)}}),
            patch("opencodon.tools.browser_tool._get_vision_model", return_value="test-model"),
            patch("opencodon.config.load_config", return_value={"auxiliary": {"vision": {"temperature": 1, "timeout": 45}}}),
            patch("opencodon.tools.browser_tool.call_llm", return_value=mock_response) as mock_llm,
        ):
    ...
    def test_browser_vision_defaults_temperature_when_config_omits_it(self, tmp_path):
        ...
        with (
            ...
            patch("opencodon.tools.browser_tool._run_browser_command", return_value={"success": True, "data": {"path": str(screenshot)}}),
            patch("opencodon.tools.browser_tool._get_vision_model", return_value="test-model"),
            patch("opencodon.config.load_config", return_value={"auxiliary": {"vision": {}}}),
            patch("opencodon.tools.browser_tool.call_llm", return_value=mock_response) as mock_llm,
        ):
            result = json.loads(browser_vision("what is on the page?", task_id="test"))
    ...
    def test_browser_vision_native_fast_path_returns_multimodal(self, tmp_path):
        ...
        try:
            with (
                ...
                patch("opencodon.tools.browser_tool._cleanup_old_screenshots"),
                patch(
                    "opencodon.tools.browser_tool._run_browser_command",
                    return_value={
                        "success": True,
                ...
                patch("opencodon.tools.browser_tool._get_vision_model") as mock_get_vision_model,
                patch("opencodon.tools.browser_tool.call_llm") as mock_llm,
            ):
...
class TestRecordSessionsConfig:
    ...
    def test_maybe_start_recording_disabled(self):
        ...
        from opencodon.tools.browser_tool import _maybe_start_recording, _recording_sessions
    ...
    def test_maybe_stop_recording_noop_when_not_recording(self):
        ...
        from opencodon.tools.browser_tool import _maybe_stop_recording, _recording_sessions
