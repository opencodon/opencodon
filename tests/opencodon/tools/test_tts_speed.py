...
class TestEdgeTtsSpeed:
    def _run(self, tts_config, tmp_path):
        ...
        with patch("opencodon.tools.tts_tool._import_edge_tts", return_value=mock_edge):
            from opencodon.tools.tts_tool import _generate_edge_tts
...
class TestOpenaiTtsSpeed:
    def _run(self, tts_config, tmp_path, monkeypatch):
        ...
        with patch("opencodon.tools.tts_tool._import_openai_client", return_value=mock_cls), \
             patch("opencodon.tools.tts_tool._resolve_openai_audio_client_config",
                   return_value=("test-key", None, False)):
            from opencodon.tools.tts_tool import _generate_openai_tts
...
class TestMinimaxTtsT2aV2:
    ...
    def _run(self, tts_config, tmp_path, monkeypatch, response=None):
        ...
        with patch("requests.post", return_value=resp) as mock_post:
            from opencodon.tools.tts_tool import _generate_minimax_tts
