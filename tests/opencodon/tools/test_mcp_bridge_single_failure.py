...
class TestConnectCooldownHelpers:
    def test_failure_arms_exponential_backoff(self):
        now = 1000.0
        with patch("opencodon.tools.mcp_tool.time.monotonic", return_value=now):
    ...
    def test_cooldown_active_then_clears(self):
        ...
        with patch("opencodon.tools.mcp_tool.time.monotonic", return_value=later):
...
class TestRegisterMcpServersIsolation:
    ...
    def _run_with_mocked_connect(self, attempts):
        ...
        return patch("opencodon.tools.mcp_tool._connect_server", side_effect=fake_connect)
    ...
    def test_failing_server_skipped_on_second_pass(self):
        ...
        with self._run_with_mocked_connect(attempts), \
                patch("opencodon.tools.mcp_tool._register_server_tools", return_value=[]), \
                patch("opencodon.tools.mcp_tool._filter_suspicious_mcp_servers", side_effect=lambda x: x):
...
class TestShutdownClearsCooldownState:
    ...
    def test_fast_path_clears_cooldown_state(self):
        ...
        with patch("opencodon.tools.mcp_tool._stop_mcp_loop"):
