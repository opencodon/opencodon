...
class TestGating:
    def test_disabled_lazy_installs_skips(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.tools.lazy_deps._allow_lazy_installs", lambda: False)
...
class TestInstall:
    def test_success_installs_binary_only_and_rechecks(self, monkeypatch):
        monkeypatch.setattr(bt, "_running_in_docker", lambda: False)
        monkeypatch.setattr("opencodon.tools.lazy_deps._allow_lazy_installs", lambda: True)
