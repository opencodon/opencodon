...
import opencodon.tools.web_tools as wt
...
class TestCharLimitConfig:
    ...
    def test_config_override(self):
        with patch("opencodon.tools.web_tools._load_web_config", return_value={"extract_char_limit": 40000}):
            assert wt._get_extract_char_limit() == 40000

    def test_clamps_floor(self):
        with patch("opencodon.tools.web_tools._load_web_config", return_value={"extract_char_limit": 100}):
            assert wt._get_extract_char_limit() == 2000

    def test_bad_value_falls_back(self):
        with patch("opencodon.tools.web_tools._load_web_config", return_value={"extract_char_limit": "nope"}):
            assert wt._get_extract_char_limit() == wt.DEFAULT_EXTRACT_CHAR_LIMIT
