...
class TestReadWriteManifest:
    def test_read_missing_manifest(self, tmp_path):
        with patch(
            "opencodon.tools.skills_sync.MANIFEST_FILE",
    ...
    def test_write_and_read_roundtrip_v2(self, tmp_path):
        ...
        with patch("opencodon.tools.skills_sync.MANIFEST_FILE", manifest_file):
...
class TestRmtreeWritableScopeGuard:
    ...
    def test_refuses_root_path(self, tmp_path):
        ...
        from opencodon.tools.skills_sync import _rmtree_writable, SKILLS_DIR

        skills = tmp_path / "skills"
        skills.mkdir()
        with patch("opencodon.tools.skills_sync.SKILLS_DIR", skills):
            with pytest.raises(ValueError, match="refusing to rmtree"):
                _rmtree_writable(Path("/"))

    def test_refuses_opencodon_home_itself(self, tmp_path):
        """``~/.opencodon/`` itself is what the #48200 wipe destroyed."""
        from opencodon.tools.skills_sync import _rmtree_writable

        opencodon = tmp_path / "home"
        opencodon.mkdir()
        (opencodon / "skills").mkdir()
        with patch("opencodon.tools.skills_sync.SKILLS_DIR", opencodon / "skills"):
            with pytest.raises(ValueError, match="refusing to rmtree"):
                _rmtree_writable(opencodon)
...
class TestExternalDirsIndexing:
    ...
    def _patches(self, bundled, skills_dir, manifest_file):
        ...
        stack = ExitStack()
        stack.enter_context(patch("opencodon.tools.skills_sync._get_bundled_dir", return_value=bundled))
        stack.enter_context(patch("opencodon.tools.skills_sync._get_optional_dir", return_value=bundled.parent / "optional-skills"))
        stack.enter_context(patch("opencodon.tools.skills_sync.SKILLS_DIR", skills_dir))
        stack.enter_context(patch("opencodon.tools.skills_sync.MANIFEST_FILE", manifest_file))
        return stack
    ...
    def test_shadowed_skill_skipped_and_deferred(self, tmp_path):
        ...
        with self._patches(bundled, skills_dir, manifest_file):
            with patch("opencodon.core.skills.skill_utils.get_external_skills_dirs", return_value=[ext_dir]):
    ...
    def test_no_external_dirs_unchanged(self, tmp_path):
        ...
        with self._patches(bundled, skills_dir, manifest_file):
            with patch("opencodon.core.skills.skill_utils.get_external_skills_dirs", return_value=[]):
                result = sync_skills(quiet=True)
...
class TestSyncSkills:
    ...
    def test_suppressed_builtin_not_reseeded(self, tmp_path):
        ...
        with self._patches(bundled, skills_dir, manifest_file), \
                patch("opencodon.tools.skills_sync._read_suppressed_names", return_value={"old-skill"}):
            result = sync_skills(quiet=True)
    ...
    def test_collision_prints_reset_hint(self, tmp_path, capsys):
        ...
        assert "opencodon skills reset new-skill" in captured
    ...
    def test_backfills_official_optional_provenance_for_existing_identical_skill(self, tmp_path):
        ...
        with self._patches(bundled, skills_dir, manifest_file):
            with patch("opencodon.tools.skills_sync._get_optional_dir", return_value=optional):
                result = sync_skills(quiet=True)
    ...
    def test_nonexistent_bundled_dir(self, tmp_path):
        with patch("opencodon.tools.skills_sync._get_bundled_dir", return_value=tmp_path / "nope"):
            result = sync_skills(quiet=True)
        assert result == {
...
class TestGetBundledDir:
    def test_env_var_override(self, tmp_path, monkeypatch):
        """OPENCODON_BUNDLED_SKILLS env var overrides the default path resolution."""
        custom_dir = tmp_path / "custom_skills"
        custom_dir.mkdir()
        monkeypatch.setenv("OPENCODON_BUNDLED_SKILLS", str(custom_dir))
    ...
    def test_default_without_env_var(self, monkeypatch):
        ...
        monkeypatch.delenv("OPENCODON_BUNDLED_SKILLS", raising=False)
        result = _get_bundled_dir()
        assert result.name == "skills"

    def test_env_var_empty_string_ignored(self, monkeypatch):
        """Empty OPENCODON_BUNDLED_SKILLS should fall back to default."""
        monkeypatch.setenv("OPENCODON_BUNDLED_SKILLS", "")
        result = _get_bundled_dir()
        assert result.name == "skills"
...
class TestResetBundledSkill:
    ...
    def test_reset_restore_preserves_manifest_on_rmtree_failure(self, tmp_path):
        ...
        with self._patches(bundled, skills_dir, manifest_file), patch(
            "opencodon.tools.skills_sync._rmtree_writable", side_effect=_boom
        ):
            result = reset_bundled_skill("google-workspace", restore=True)
...
class TestNoBundledSkillsOptOut:
    ...
    This is what `opencodon profile create --no-skills` (named profiles) and the
    installer's `--no-skills` flag (default ~/.opencodon) rely on so bundled
    skills are never seeded at install time NOR re-injected by `opencodon update`.
    """
    ...
    def test_marker_skips_sync(self, tmp_path):
        ...
        (opencodon_home / ".no-bundled-skills").write_text("opted out\n")

        with patch("opencodon.tools.skills_sync._get_bundled_dir", return_value=bundled), \
             patch("opencodon.tools.skills_sync.SKILLS_DIR", skills_dir), \
             patch("opencodon.tools.skills_sync.MANIFEST_FILE", manifest_file), \
             patch("opencodon.tools.skills_sync.OPENCODON_HOME", opencodon_home):
            result = sync_skills(quiet=True)
    ...
    def test_no_marker_seeds_normally(self, tmp_path):
        ...
        with patch("opencodon.tools.skills_sync._get_bundled_dir", return_value=bundled), \
             patch("opencodon.tools.skills_sync._get_optional_dir", return_value=bundled.parent / "optional-skills"), \
             patch("opencodon.tools.skills_sync.SKILLS_DIR", skills_dir), \
             patch("opencodon.tools.skills_sync.MANIFEST_FILE", manifest_file), \
             patch("opencodon.tools.skills_sync.OPENCODON_HOME", opencodon_home):
            result = sync_skills(quiet=True)
...
class TestOptOutToggleAndRemove:
    """`opencodon skills opt-out/opt-in` core: marker toggle + safe removal."""
    ...
    def test_marker_toggle(self, tmp_path):
        ...
        home = tmp_path / "home"
        home.mkdir()
        with patch("opencodon.tools.skills_sync.OPENCODON_HOME", home):
    ...
    def test_remove_keeps_user_modified(self, tmp_path):
        ...
        with patch("opencodon.tools.skills_sync._get_bundled_dir", return_value=bundled), \
             ...
             patch("opencodon.tools.skills_sync.SKILLS_DIR", skills_dir), \
             patch("opencodon.tools.skills_sync.MANIFEST_FILE", manifest_file), \
             patch("opencodon.tools.skills_sync.OPENCODON_HOME", home):
            sync_skills(quiet=True)
            # User edits 'beta'
...
class TestUpdateBackupRecovery:
    ...
    def test_stale_backup_does_not_poison_failed_update(self, tmp_path):
        ...
        with self._patches(bundled, skills_dir, manifest_file), \
                patch("opencodon.tools.skills_sync.shutil.copytree", side_effect=_boom):
            sync_skills(quiet=True)
    ...
    def test_partial_copy_failure_restores_original(self, tmp_path):
        ...
        with self._patches(bundled, skills_dir, manifest_file), \
                patch("opencodon.tools.skills_sync.shutil.copytree", side_effect=_partial_then_fail):
            sync_skills(quiet=True)
