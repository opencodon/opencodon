...
`opencodon update` keeps (does not overwrite) bundled skills the user edited
...
def _patches(bundled, skills_dir, manifest_file):
    stack = ExitStack()
    stack.enter_context(
        patch("opencodon.tools.skills_sync._get_bundled_dir", return_value=bundled)
    )
    stack.enter_context(
        patch(
            "opencodon.tools.skills_sync._get_optional_dir",
