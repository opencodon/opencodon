...
from opencodon.tools.cronjob_tools import cronjob, _execute_job_now
...
class TestCronjobRunExecutesImmediately:
    def test_run_action_claims_and_fires_via_run_one_job(self):
        ...
        with patch("opencodon.tools.cronjob_tools.resolve_job_ref", return_value=dict(_JOB)), \
             patch("opencodon.tools.cronjob_tools.claim_job_for_fire", return_value=True) as m_claim, \
             patch("opencodon.cron.scheduler.run_one_job", return_value=True) as m_run, \
             patch("opencodon.tools.cronjob_tools.get_job", return_value=ran):
    ...
    def test_run_skips_when_claim_lost(self):
        ...
        with patch("opencodon.tools.cronjob_tools.resolve_job_ref", return_value=dict(_JOB)), \
             patch("opencodon.tools.cronjob_tools.claim_job_for_fire", return_value=False), \
             patch("opencodon.cron.scheduler.run_one_job") as m_run, \
             patch("opencodon.tools.cronjob_tools.get_job", return_value=dict(_JOB)):
            out = json.loads(cronjob(action="run", job_id="job-run-1"))
    ...
    def test_run_reports_failure_from_last_status(self):
        ...
        with patch("opencodon.tools.cronjob_tools.resolve_job_ref", return_value=dict(_JOB)), \
             patch("opencodon.tools.cronjob_tools.claim_job_for_fire", return_value=True), \
             patch("opencodon.cron.scheduler.run_one_job", return_value=True), \
             patch("opencodon.tools.cronjob_tools.get_job", return_value=failed):
            out = json.loads(cronjob(action="run", job_id="job-run-1"))
    ...
    def test_execute_job_now_bails_without_claim(self):
        ...
        with patch("opencodon.tools.cronjob_tools.claim_job_for_fire", return_value=False), \
             patch("opencodon.cron.scheduler.run_one_job") as m_run:
    ...
    def test_execute_job_now_marks_failure_on_exception(self):
        ...
        with patch("opencodon.tools.cronjob_tools.claim_job_for_fire", return_value=True), \
             patch("opencodon.cron.scheduler.run_one_job", side_effect=RuntimeError("boom")), \
             patch("opencodon.tools.cronjob_tools.mark_job_run") as m_mark, \
             patch("opencodon.tools.cronjob_tools.get_job", return_value=dict(_JOB)):
            res = _execute_job_now(dict(_JOB))
