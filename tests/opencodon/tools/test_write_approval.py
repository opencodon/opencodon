...
and the shared slash-command handlers (opencodon_cli/write_approval_commands.py).
...
def test_default_gate_is_off(opencodon_home):
...
def test_invalid_subsystem_is_off(opencodon_home):
    from opencodon.tools import write_approval as wa
...
def test_memory_gate_off_allows_write(opencodon_home):
...
def test_memory_gate_on_no_interactive_stages(opencodon_home):
...
def test_memory_gate_on_then_apply(opencodon_home):
    from opencodon.tools.memory_tool import memory_tool, MemoryStore, apply_memory_pending
    from opencodon.tools import write_approval as wa
    _set_approval("memory", True)
    store = MemoryStore(); store.load_from_disk()
...
def test_cli_memory_approve_without_live_agent_uses_fresh_store(opencodon_home, capsys):
...
def test_load_on_disk_store_honors_configured_char_limits(opencodon_home, monkeypatch):
...
def test_skill_gate_off_allows_create(opencodon_home):
    # Default (gate off) → skill is created normally, not staged.
    import importlib
    import opencodon.tools.skill_manager_tool as smt
...
def test_skill_gate_on_always_stages(opencodon_home):
    # Skills stage even in the foreground (too big to review inline).
    from opencodon.tools.skill_manager_tool import skill_manage
    from opencodon.tools import write_approval as wa
...
def test_skill_gate_on_then_apply_writes_file(opencodon_home):
    # SKILLS_DIR is resolved at import time, so reload the skill module under
    # this test's OPENCODON_HOME to exercise the real on-disk write path.
    import importlib
    import opencodon.tools.skill_manager_tool as smt
    importlib.reload(smt)
    from opencodon.tools import write_approval as wa
...
def test_skill_create_diff_is_full_content(opencodon_home):
    from opencodon.tools.skill_manager_tool import skill_manage
    from opencodon.tools import write_approval as wa
    _set_approval("skills", True)
...
def test_pending_store_roundtrip(opencodon_home):
    from opencodon.tools import write_approval as wa
...
def approval_callback_cleanup():
    yield
    from opencodon.tools.terminal_tool import set_approval_callback
    set_approval_callback(None)


def test_memory_inline_approve_writes(opencodon_home, approval_callback_cleanup):
    from opencodon.tools.memory_tool import memory_tool, MemoryStore
    from opencodon.tools.terminal_tool import set_approval_callback
    from opencodon.tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(opencodon_home, approval_callback_cleanup):
    from opencodon.tools.memory_tool import memory_tool, MemoryStore
    from opencodon.tools.terminal_tool import set_approval_callback
    from opencodon.tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_callback_error_stages(opencodon_home, approval_callback_cleanup):
...
def test_gateway_context_stages_not_prompts(opencodon_home, monkeypatch):
    ...
    from opencodon.tools import write_approval as wa
    _set_approval("memory", True)
    monkeypatch.setenv("OPENCODON_GATEWAY_SESSION", "1")
...
def test_skills_never_prompt_inline_even_with_callback(opencodon_home, approval_callback_cleanup):
...
def test_memory_invalid_params_rejected_before_staging(opencodon_home):
