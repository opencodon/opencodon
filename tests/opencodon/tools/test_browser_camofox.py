...
class TestCamofoxLoopbackRewrite:
    @patch("opencodon.tools.browser_camofox.load_config")
...
class TestCamofoxNavigate:
    @patch("opencodon.tools.browser_camofox.requests.post")
...
class TestCamofoxSnapshot:
    ...
    @patch("opencodon.tools.browser_camofox.requests.post")
    @patch("opencodon.tools.browser_camofox.requests.get")
...
class TestCamofoxInteractions:
    ...
    def test_type_redacts_api_key(self, mock_post, monkeypatch):
        monkeypatch.setenv("CAMOFOX_URL", "http://localhost:9377")
        monkeypatch.setenv("OPENCODON_REDACT_SECRETS", "true")
...
class TestCamofoxClose:
    @patch("opencodon.tools.browser_camofox.requests.delete")
    @patch("opencodon.tools.browser_camofox.requests.post")
...
class TestCamofoxVisionConfig:
    @patch("opencodon.tools.browser_camofox.requests.post")
    @patch("opencodon.tools.browser_camofox._get")
    @patch("opencodon.tools.browser_camofox._get_raw")
    def test_camofox_vision_uses_configured_temperature_and_timeout(self, mock_get_raw, mock_get, mock_post, monkeypatch):
        ...
        with (
            patch("opencodon.tools.browser_camofox.open", create=True) as mock_open,
            patch("opencodon.core.auxiliary_client.call_llm", return_value=mock_response) as mock_llm,
            patch("opencodon.tools.browser_camofox.load_config", return_value={"auxiliary": {"vision": {"temperature": 1, "timeout": 45}}}),
        ):
    ...
    def test_camofox_vision_defaults_temperature_when_config_omits_it(self, mock_get_raw, mock_get, mock_post, monkeypatch):
        ...
        with (
            patch("opencodon.tools.browser_camofox.open", create=True) as mock_open,
            patch("opencodon.core.auxiliary_client.call_llm", return_value=mock_response) as mock_llm,
            patch("opencodon.tools.browser_camofox.load_config", return_value={"auxiliary": {"vision": {}}}),
        ):
            mock_open.return_value.__enter__.return_value.read.return_value = b"fakepng"
...
class TestBrowserToolRouting:
    ...
    def test_browser_navigate_routes_to_camofox(self, mock_post, monkeypatch):
        ...
        with patch("opencodon.tools.browser_tool._is_safe_url", return_value=True):
    ...
    def test_check_requirements_passes_with_camofox(self, monkeypatch):
        monkeypatch.setenv("CAMOFOX_URL", "http://localhost:9377")
        from opencodon.tools.browser_tool import check_browser_requirements
