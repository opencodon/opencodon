...
try:
    ...
    _tt_mod = sys.modules["opencodon.tools.terminal_tool"]
except ImportError:
    pytest.skip("opencodon tools not importable (missing deps)", allow_module_level=True)
...
class TestCwdHandling:
    ...
    def test_home_path_replaced_for_modal(self, monkeypatch):
        ...
        monkeypatch.setenv("TERMINAL_CWD", "/home/dakota/github/opencodon")
    ...
    def test_docker_default_cwd_maps_current_directory_when_enabled(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.tools.terminal_tool.os.getcwd", lambda: "/home/user/project")
        monkeypatch.setenv("TERMINAL_ENV", "docker")
        monkeypatch.setenv("TERMINAL_DOCKER_MOUNT_CWD_TO_WORKSPACE", "true")
        monkeypatch.delenv("TERMINAL_CWD", raising=False)
        config = _tt_mod._get_env_config()
...
class TestModalEnvironmentDefaults:
    ...
    def test_default_cwd_is_root(self):
        ...
        from opencodon.tools.environments.modal import ModalEnvironment
        import inspect
...
class TestEnsurepipFix:
    ...
    def test_modal_environment_creates_image_with_setup_commands(self):
        ...
        try:
            from opencodon.tools.environments.modal import _resolve_modal_image
        except ImportError:
            pytest.skip("opencodon.tools.environments.modal not importable")
...
class TestDockerHostBindApproval:
    ...
    def test_should_skip_container_guards(self):
        ...
        import opencodon.tools.approval as A
    ...
    def _isolate_approval_state(monkeypatch):
        ...
        ``opencodon.tools.approval`` loads ``command_allowlist`` into module-level
        ``_permanent_approved`` at import time. This file imports
        ``opencodon.tools.terminal_tool`` at module level (collection time — BEFORE the
        hermetic OPENCODON_HOME fixture runs), so on a dev machine whose real
