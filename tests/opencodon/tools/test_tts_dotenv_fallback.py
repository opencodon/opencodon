...
``opencodon.config.get_env_value``) and not only from ``os.environ`` —
...
class TestDotenvFallbackPerProvider:
    ...
    ``opencodon.config.load_env`` integration. Before the fix, ``os.getenv``
    ...
    def test_xai_reads_dotenv_key(self, tmp_path):
        ...
        """
        from opencodon.tools import tts_tool
        from opencodon.tools import xai_http
...
class TestRegressionGuard:
    ...
    ``opencodon.config.load_env`` to simulate ``~/.opencodon/.env`` carrying the
    ...
    def test_import_after_config_env_patch_uses_restored_dotenv_loader(self, tmp_path, monkeypatch):
        """Importing TTS while opencodon.config.get_env_value is patched must
    ...
    def test_minimax_missing_when_only_in_dotenv_before_fix(self, tmp_path, monkeypatch):
        ...
        with patch(
            ...
            # os.environ is empty.
            from opencodon.config import get_env_value as live_get
