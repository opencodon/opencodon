...
def fake_opencodon(tmp_path, monkeypatch):
    """Build a two-profile opencodon layout and point OPENCODON_HOME at
    the opencodon-security profile (matching the original-incident shape).
    """
    ...
    monkeypatch.setenv("OPENCODON_HOME", str(sec_home))
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_opencodon):
        from opencodon.tools.file_tools import write_file_tool
        target = fake_opencodon["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_opencodon):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from opencodon.tools.file_tools import write_file_tool
        target = fake_opencodon["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "opencodon-security" in result["error"]
        # File untouched.
        assert target.read_text() == original

    def test_cross_profile_True_bypass(self, fake_opencodon):
    ...
    def test_non_opencodon_path_unaffected(self, fake_opencodon, tmp_path):
        from opencodon.tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_opencodon):
    ...
    def test_cross_profile_patch_bypass(self, fake_opencodon):
        from opencodon.tools.file_tools import patch_tool
        target = fake_opencodon["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_opencodon):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_opencodon, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_opencodon["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up OPENCODON_HOME (set in
        # the fixture). Skill_manager_tool computes SKILLS_DIR at import.
        import importlib
        import opencodon.tools.skill_manager_tool
        importlib.reload(opencodon.tools.skill_manager_tool)
        from opencodon.tools.skill_manager_tool import _skill_not_found_error

        err = _skill_not_found_error("default-only-skill")
        assert "not found in active profile 'opencodon-security'" in err
        assert "default" in err
        assert "cross_profile=True" in err

    def test_error_names_multiple_profiles(self, fake_opencodon, monkeypatch):
        """When the skill exists in TWO other profiles, both should be named."""
        self._make_skill_in_profile(fake_opencodon["root"], "everywhere-skill")
        self._make_skill_in_profile(fake_opencodon["coder_home"], "everywhere-skill")
...
class TestSystemPromptActiveProfile:
    ...
    def test_named_profile_line_in_prompt_text(self, fake_opencodon):
        ...
        assert "Active opencodon profile: default" in src
        assert "Active opencodon profile: {active_profile}" in src
