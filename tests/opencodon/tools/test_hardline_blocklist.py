...
def test_yolo_env_var_cannot_bypass_hardline(clean_session, monkeypatch):
    """OPENCODON_YOLO_MODE=1 must not bypass the hardline floor."""
...
def test_approvals_mode_off_cannot_bypass_hardline(clean_session, monkeypatch, tmp_path):
    ...
    # _get_approval_mode() reads from opencodon config; simplest path: monkeypatch the helper.
    import opencodon.tools.approval as approval_mod
