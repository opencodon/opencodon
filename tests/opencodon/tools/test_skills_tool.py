...
from opencodon.tools.skills_tool import (
...
class TestRequiredEnvironmentVariablesNormalization:
    ...
    def test_empty_env_file_value_is_treated_as_missing(self, monkeypatch):
        ...
        from opencodon.tools.skills_tool import _is_env_var_persisted
...
class TestGetCategoryFromPath:
    ...
    def test_outside_skills_dir(self, tmp_path):
        with patch("opencodon.tools.skills_tool.SKILLS_DIR", tmp_path / "skills"):
...
class TestFindAllSkills:
    ...
    def test_nonexistent_directory(self, tmp_path):
        with patch("opencodon.tools.skills_tool.SKILLS_DIR", tmp_path / "nope"):
            skills = _find_all_skills()
        assert skills == []
...
class TestSkillView:
    ...
    def test_skill_view_applies_template_vars(self, tmp_path):
        with (
            ...
            patch(
                "opencodon.core.skills.skill_preprocessing.load_skills_config",
    ...
    def test_view_disabled_skill_blocked(self, tmp_path):
        ...
        with (
            patch("opencodon.tools.skills_tool.SKILLS_DIR", tmp_path),
            patch(
                "opencodon.tools.skills_tool._is_skill_disabled",
                return_value=True,
            ),
        ):
