...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. opencodon.tools.code_execution_tool._scrub_child_env — broad OPENCODON_ prefix dropped,
...
from opencodon.tools import approval as A
...
def test_helper_propagates_contextvar_and_approval_callback():
    from opencodon.tools import terminal_tool as TT
...
def test_both_rpc_threads_use_propagation_helper():
    ...
    silently returns."""
    import inspect
    import opencodon.tools.code_execution_tool as cet
...
def gw_session(monkeypatch):
    """A clean gateway session: OPENCODON_GATEWAY_SESSION set, a bound session
...
def test_terminal_serializes_smart_deny_pending_capabilities(monkeypatch):
    from opencodon.tools import terminal_tool as terminal_module
...
def test_env_scrub_opencodon_allowlist_and_secret_blocks():
    from opencodon.tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "OPENCODON_HOME": "/h", "OPENCODON_PROFILE": "p",
...
def test_env_scrub_logs_dropped_opencodon_vars(caplog):
    """Dropping a non-allowlisted, non-secret OPENCODON_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset OPENCODON_* var."""
    import logging
    ...
    with caplog.at_level(logging.DEBUG, logger="opencodon.tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped OPENCODON_* vars."""
    import logging
