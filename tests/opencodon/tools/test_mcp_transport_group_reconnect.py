...
def _make_http_task(monkeypatch, transport_exc):
    ...
    monkeypatch.setattr("opencodon.tools.mcp_tool._MCP_HTTP_AVAILABLE", True, raising=False)
    monkeypatch.setattr("opencodon.tools.mcp_tool._MCP_NEW_HTTP", True, raising=False)
    monkeypatch.setattr(
        "opencodon.tools.mcp_tool.streamable_http_client",
        lambda url, http_client=None: _RaisingTransportCM(transport_exc),
        raising=False,
    )
    monkeypatch.setattr(
        "opencodon.tools.mcp_tool.ClientSession",
