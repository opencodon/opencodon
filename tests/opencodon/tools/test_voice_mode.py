"""Tests for opencodon.tools.voice_mode -- all mocked, no real microphone or API calls."""
...
def temp_voice_dir(tmp_path, monkeypatch):
    ...
    voice_dir = tmp_path / "opencodon_voice"
    voice_dir.mkdir()
    monkeypatch.setattr("opencodon.tools.voice_mode._TEMP_DIR", str(voice_dir))
    return voice_dir
...
def mock_sd(monkeypatch):
    ...
    monkeypatch.setattr("opencodon.tools.voice_mode._import_audio", _fake_import_audio)
    monkeypatch.setattr("opencodon.tools.voice_mode._audio_available", lambda: True)
    return mock
...
class TestPulseSocketReachable:
    def test_no_env_no_socket(self, monkeypatch):
        ...
        from opencodon.tools.voice_mode import _pulse_socket_reachable
...
class TestDetectAudioEnvironment:
    def test_clean_environment_is_available(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.tools.voice_mode._import_audio",
                            lambda: (MagicMock(), MagicMock()))
        monkeypatch.setattr("builtins.open", _non_wsl_proc_version(open))

        from opencodon.tools.voice_mode import detect_audio_environment
    ...
    def test_ssh_blocks_voice(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.tools.voice_mode._pulse_socket_reachable", lambda: False)
        monkeypatch.setattr("opencodon.tools.voice_mode._import_audio",
                            lambda: (MagicMock(), MagicMock()))
    ...
    def test_ssh_with_reachable_pulse_socket_allows_voice(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.tools.voice_mode._pulse_socket_reachable", lambda: True)
        monkeypatch.setattr("opencodon.tools.voice_mode._import_audio",
                            lambda: (MagicMock(), MagicMock()))
        monkeypatch.setattr("builtins.open", _non_wsl_proc_version(open))
    ...
    def test_termux_import_error_shows_termux_install_guidance(self, monkeypatch):
        ...
        monkeypatch.delenv("SSH_TTY", raising=False)
        monkeypatch.delenv("SSH_CONNECTION", raising=False)
        monkeypatch.setattr("opencodon.tools.voice_mode._import_audio", lambda: (_ for _ in ()).throw(ImportError("no audio libs")))
        monkeypatch.setattr("opencodon.tools.voice_mode._termux_microphone_command", lambda: None)
    ...
    def test_termux_api_package_without_android_app_blocks_voice(self, monkeypatch):
        ...
        monkeypatch.delenv("SSH_TTY", raising=False)
        monkeypatch.delenv("SSH_CONNECTION", raising=False)
        monkeypatch.setattr("opencodon.tools.voice_mode._termux_microphone_command", lambda: "/data/data/com.termux/files/usr/bin/termux-microphone-record")
        monkeypatch.setattr("opencodon.tools.voice_mode._termux_api_app_installed", lambda: False)
        monkeypatch.setattr("opencodon.tools.voice_mode._import_audio", lambda: (_ for _ in ()).throw(ImportError("no audio libs")))
    ...
    def test_docker_with_pulse_server_allows_voice(self, monkeypatch):
        ...
        monkeypatch.delenv("PIPEWIRE_REMOTE", raising=False)
        monkeypatch.setattr("opencodon_constants.is_container", lambda: True)
        monkeypatch.setattr("opencodon.tools.voice_mode._import_audio",
                            lambda: (MagicMock(), MagicMock()))
    ...
    def test_docker_with_pipewire_remote_and_no_devices_allows_voice(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.tools.voice_mode._import_audio", lambda: (sd, MagicMock()))
    ...
    def test_termux_api_microphone_allows_voice_without_sounddevice(self, monkeypatch):
        ...
        monkeypatch.delenv("SSH_TTY", raising=False)
        monkeypatch.delenv("SSH_CONNECTION", raising=False)
        monkeypatch.setattr("opencodon.tools.voice_mode.shutil.which", lambda cmd: "/data/data/com.termux/files/usr/bin/termux-microphone-record" if cmd == "termux-microphone-record" else None)
        monkeypatch.setattr("opencodon.tools.voice_mode._termux_api_app_installed", lambda: True)
        monkeypatch.setattr("opencodon.tools.voice_mode._import_audio", lambda: (_ for _ in ()).throw(ImportError("no audio libs")))
        monkeypatch.setattr("builtins.open", _non_wsl_proc_version(open))
...
class TestCheckVoiceRequirements:
    def test_termux_api_capture_counts_as_audio_available(self, monkeypatch):
        monkeypatch.setattr("opencodon.tools.voice_mode._audio_available", lambda: False)
        monkeypatch.setattr("opencodon.tools.voice_mode._termux_microphone_command", lambda: "/data/data/com.termux/files/usr/bin/termux-microphone-record")
        monkeypatch.setattr("opencodon.tools.voice_mode._termux_api_app_installed", lambda: True)
        monkeypatch.setattr("opencodon.tools.voice_mode.detect_audio_environment", lambda: {"available": True, "warnings": [], "notices": ["Termux:API microphone recording available"]})
        monkeypatch.setattr("opencodon.tools.transcription_tools._get_provider", lambda cfg: "openai")
    ...
    def test_all_requirements_met(self, monkeypatch):
        monkeypatch.setattr("opencodon.tools.voice_mode._audio_available", lambda: True)
        monkeypatch.setattr("opencodon.tools.voice_mode.detect_audio_environment",
    ...
    def test_missing_stt_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.tools.voice_mode.detect_audio_environment",
                            lambda: {"available": True, "warnings": []})
        monkeypatch.setattr("opencodon.tools.transcription_tools._get_provider", lambda cfg: "none")
...
class TestCreateAudioRecorder:
    def test_termux_uses_termux_audio_recorder_when_api_present(self, monkeypatch):
        ...
        from opencodon.tools.voice_mode import create_audio_recorder, TermuxAudioRecorder
    ...
    def test_termux_without_android_app_falls_back_to_audio_recorder(self, monkeypatch):
        ...
        from opencodon.tools.voice_mode import create_audio_recorder, AudioRecorder
        recorder = create_audio_recorder()
...
class TestTermuxAudioRecorder:
    def test_start_and_stop_use_termux_microphone_commands(self, monkeypatch, temp_voice_dir):
        ...
        monkeypatch.setattr("opencodon.tools.voice_mode._termux_microphone_command", lambda: "/data/data/com.termux/files/usr/bin/termux-microphone-record")
        monkeypatch.setattr("opencodon.tools.voice_mode._termux_api_app_installed", lambda: True)
        monkeypatch.setattr("opencodon.tools.voice_mode.time.strftime", lambda fmt: "20260409_120000")
        monkeypatch.setattr("opencodon.tools.voice_mode.subprocess.run", fake_run)

        from opencodon.tools.voice_mode import TermuxAudioRecorder
...
class TestAudioRecorder:
    def test_start_raises_without_audio_libs(self, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.tools.voice_mode._import_audio", _fail_import)

        from opencodon.tools.voice_mode import AudioRecorder
...
class TestAudioRecorderStop:
    ...
    def test_stop_writes_wav_file(self, mock_sd, temp_voice_dir):
        ...
        from opencodon.tools.voice_mode import AudioRecorder, SAMPLE_RATE
...
class TestTranscribeRecording:
    def test_delegates_to_transcribe_audio(self):
        ...
        with patch("opencodon.tools.transcription_tools.transcribe_audio", mock_transcribe):
    ...
    def test_oversized_wav_is_chunked_and_stitched(self, tmp_path, monkeypatch):
        ...
        temp_dir.mkdir()
        monkeypatch.setattr("opencodon.tools.voice_mode._TEMP_DIR", str(temp_dir))
        monkeypatch.setattr("opencodon.tools.transcription_tools.MAX_FILE_SIZE", 70 * 1024)
        ...
        with patch("opencodon.tools.transcription_tools.transcribe_audio", side_effect=fake_transcribe):
            from opencodon.tools.voice_mode import transcribe_recording
...
class TestPlayAudioFile:
    def test_play_wav_via_sounddevice(self, monkeypatch, sample_wav):
        ...
        monkeypatch.setattr("opencodon.tools.voice_mode._import_audio", _fake_import)
...
class TestCleanupTempRecordings:
    ...
    def test_nonexistent_dir_returns_zero(self, monkeypatch):
        monkeypatch.setattr("opencodon.tools.voice_mode._TEMP_DIR", "/nonexistent/dir")
...
class TestPlaybackInterrupt:
    ...
    def test_stop_playback_terminates_process(self):
        from opencodon.tools.voice_mode import stop_playback, _playback_lock
        import opencodon.tools.voice_mode as vm
...
class TestListenForSpeech:
    ...
    def _run(self, mock_sd, levels, should_stop=None, **kwargs):
        ...
        from opencodon.tools.voice_mode import listen_for_speech
    ...
    def test_returns_false_when_audio_unavailable(self, monkeypatch):
        monkeypatch.setattr("opencodon.tools.voice_mode._import_audio", MagicMock(side_effect=OSError("no audio")))
        from opencodon.tools.voice_mode import listen_for_speech
...
class TestListenForSpeechCapture:
    ...
    def _run(self, mock_sd, monkeypatch, levels, should_stop=None, **kwargs):
        ...
        written = {}
        monkeypatch.setattr(
            "opencodon.tools.voice_mode.AudioRecorder._write_wav",
