...
class TestResolveShellInitFiles:
    def test_auto_sources_bashrc_when_present(self, tmp_path, monkeypatch):
        ...
        with patch(
            "opencodon.tools.environments.local._read_terminal_shell_init_config",
            return_value=([], True),
        ):
...
class TestSnapshotEndToEnd:
    ...
    def test_exported_env_changes_persist_between_commands(self, tmp_path):
        ...
        try:
            first = env.execute(
                'export OPENCODON_SESSION_ENV_PROBE="sticky"; '
                'export PATH="/tmp/opencodon-session-bin:$PATH"; '
                'echo "first=$OPENCODON_SESSION_ENV_PROBE"'
            )
            second = env.execute(
                'echo "second=$OPENCODON_SESSION_ENV_PROBE"; echo "PATH=$PATH"'
            )
        finally:
            env.cleanup()
        ...
        assert "/tmp/opencodon-session-bin" in output
    ...
    def test_snapshot_picks_up_init_file_exports(self, tmp_path, monkeypatch):
        ...
        init_file.write_text(
            'export OPENCODON_SHELL_INIT_PROBE="probe-ok"\n'
        ...
        with patch(
            ...
            try:
                result = env.execute(
                    'echo "PROBE=$OPENCODON_SHELL_INIT_PROBE"; echo "PATH=$PATH"'
                )
            finally:
                env.cleanup()
