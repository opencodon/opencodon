...
class TestCronApprovalModeParsing:
    def test_default_is_deny(self):
        ...
        with mock_patch("opencodon.config.load_config", return_value={"approvals": {}}):
            assert _get_cron_approval_mode() == "deny"

    def test_explicit_deny(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("opencodon.config.load_config", return_value={"approvals": {"cron_mode": "deny"}}):
            assert _get_cron_approval_mode() == "deny"

    def test_explicit_approve(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("opencodon.config.load_config", return_value={"approvals": {"cron_mode": "approve"}}):
    ...
    def test_off_maps_to_approve(self):
        ...
        from unittest.mock import patch as mock_patch
        with mock_patch("opencodon.config.load_config", return_value={"approvals": {"cron_mode": "off"}}):
            assert _get_cron_approval_mode() == "approve"

    def test_allow_maps_to_approve(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("opencodon.config.load_config", return_value={"approvals": {"cron_mode": "allow"}}):
            assert _get_cron_approval_mode() == "approve"

    def test_yes_maps_to_approve(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("opencodon.config.load_config", return_value={"approvals": {"cron_mode": "yes"}}):
            assert _get_cron_approval_mode() == "approve"
    ...
    def test_unknown_value_defaults_to_deny(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("opencodon.config.load_config", return_value={"approvals": {"cron_mode": "maybe"}}):
            assert _get_cron_approval_mode() == "deny"

    def test_config_load_failure_defaults_to_deny(self):
        """If config loading fails entirely, default to deny (safe)."""
        from unittest.mock import patch as mock_patch
        with mock_patch("opencodon.config.load_config", side_effect=RuntimeError("config broken")):
            assert _get_cron_approval_mode() == "deny"

    def test_yaml_boolean_false_maps_to_deny(self):
        """YAML 1.1 parses bare 'off' as False. Ensure it maps to deny."""
        from unittest.mock import patch as mock_patch
        with mock_patch("opencodon.config.load_config", return_value={"approvals": {"cron_mode": False}}):
...
class TestCronDenyMode:
    """When OPENCODON_CRON_SESSION is set and cron_mode=deny, dangerous commands are blocked."""

    def test_dangerous_command_blocked_in_cron_deny_mode(self, monkeypatch):
        monkeypatch.setenv("OPENCODON_CRON_SESSION", "1")
        ...
        from unittest.mock import patch as mock_patch
        with mock_patch("opencodon.tools.approval._get_cron_approval_mode", return_value="deny"):
...
class TestCronApproveMode:
    """When OPENCODON_CRON_SESSION is set and cron_mode=approve, dangerous commands pass through."""
    ...
    def test_dangerous_command_allowed_in_cron_approve_mode(self, monkeypatch):
        ...
        from unittest.mock import patch as mock_patch
        with mock_patch("opencodon.tools.approval._get_cron_approval_mode", return_value="approve"):
            result = check_dangerous_command("rm -rf /tmp/stuff", "local")
            assert result["approved"]
...
class TestCronDenyModeAllGuards:
    ...
    def test_tirith_content_threat_blocked_in_cron_deny(self, monkeypatch):
        ...
        with (
            mock_patch("opencodon.tools.approval._get_cron_approval_mode", return_value="deny"),
            mock_patch("opencodon.tools.approval.detect_dangerous_command",
                       return_value=(False, None, None)),
            mock_patch("opencodon.tools.tirith_security.check_command_security",
    ...
    def test_tirith_import_error_fail_closed_blocks_in_cron_deny(self, monkeypatch):
        ...
        with (
            ...
            mock_patch("opencodon.tools.approval.detect_dangerous_command",
                       return_value=(False, None, None)),
            mock_patch("opencodon.config.load_config",
...
class TestCronModeInteractions:
    ...
    def test_yolo_overrides_cron_deny(self, monkeypatch):
        ...
        monkeypatch.setenv("OPENCODON_CRON_SESSION", "1")
        monkeypatch.setenv("OPENCODON_YOLO_MODE", "1")
        monkeypatch.delenv("OPENCODON_INTERACTIVE", raising=False)
        monkeypatch.delenv("OPENCODON_GATEWAY_SESSION", raising=False)

        # _YOLO_MODE_FROZEN is frozen at module import time (security: prevents
        # prompt injection from runtime-setting OPENCODON_YOLO_MODE). When the
        # test process imports opencodon.tools.approval BEFORE this test sets the env,
        ...
        # with OPENCODON_YOLO_MODE=1.
        from unittest.mock import patch as mock_patch
        import opencodon.tools.approval
        with (
            mock_patch.object(opencodon.tools.approval, "_YOLO_MODE_FROZEN", True),
            mock_patch("opencodon.tools.approval._get_cron_approval_mode", return_value="deny"),
        ):
...
class TestCronWithGatewayOrigin:
    ...
    cron/scheduler.py binds OPENCODON_SESSION_PLATFORM via contextvars for
