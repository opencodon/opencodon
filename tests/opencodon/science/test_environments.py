...
from opencodon.science import envmanager as em
...
def test_every_cell_records_the_identity_not_just_the_first(science_runtime, monkeypatch):
    ...
    """
    monkeypatch.setattr(
        "opencodon.science.runtime._lock_hash_of", lambda snapshot: "deadbeef" if snapshot else None
    )
...
def test_byte_match_under_a_matching_lock_is_verified(science_runtime, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.science.runtime._lock_hash_of", lambda snapshot: "lock-aaa" if snapshot else None
    )
    result = science_runtime.run_cell("s1", "save_artifact('stable', 'out.txt')")
    [artifact] = result["artifacts"]
...
def test_a_changed_environment_downgrades_to_reproduced(science_runtime, monkeypatch):
    ...
    monkeypatch.setattr(
        "opencodon.science.runtime._lock_hash_of",
...
def test_live_kernel_runs_inside_the_environment(db, tmp_path, live_env):
    ...
    from opencodon.science.kernels import SessionKernelManager, kernels_installed
