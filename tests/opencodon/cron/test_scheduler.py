...
from opencodon.cron.scheduler import _resolve_origin, _resolve_delivery_target, _deliver_result, _send_media_via_adapter, run_job, SILENT_MARKER, _build_job_prompt, _resolve_cron_enabled_toolsets, _merge_mcp_into_per_job_toolsets
...
class TestResolveDeliveryTarget:
    ...
    def test_list_form_multiple_platforms_normalized(self, monkeypatch):
        ...
        from opencodon.cron.scheduler import _resolve_delivery_targets
...
class TestRunJobSessionPersistence:
    ...
    def test_run_job_timeout_finalizes_original_session(self, tmp_path, monkeypatch):
        ...
        with self._run_job_patches(tmp_path) as (fake_db, mock_agent_cls), \
             patch(
                 "opencodon.cron.scheduler.concurrent.futures.wait",
    ...
    def _run_job_patches(self, tmp_path, extra=()):
        ...
        mock_agent.run_conversation.return_value = {"final_response": "ok"}
        base = [
            patch("opencodon.cron.scheduler._opencodon_home", tmp_path),
            patch("opencodon.cron.scheduler._resolve_origin", return_value=None),
            patch("opencodon.config.env_loader.load_opencodon_dotenv"),
            patch("opencodon.config.env_loader.reset_secret_source_cache"),
            patch("opencodon.state.SessionDB", return_value=fake_db),
            ...
            patch("opencodon.core.run_agent.AIAgent", return_value=mock_agent),
        ]
    ...
    def test_tick_skips_due_jobs_while_dispatch_is_paused(self, tmp_path):
        ...
        with patch("opencodon.cron.scheduler.get_due_jobs", return_value=[job]), patch(
            "opencodon.cron.scheduler.advance_next_run"
        ) as advance, patch("opencodon.cron.scheduler.run_one_job") as run_one:
    ...
    def test_tick_marks_empty_response_as_error(self, tmp_path):
        ...
        with patch("opencodon.cron.scheduler._opencodon_home", tmp_path), \
             patch("opencodon.cron.scheduler.get_due_jobs", return_value=[job]), \
             patch("opencodon.cron.scheduler.advance_next_run"), \
             patch("opencodon.cron.scheduler.mark_job_run") as mock_mark, \
             patch("opencodon.cron.scheduler.save_job_output", return_value="/tmp/out.md"), \
             patch("opencodon.cron.scheduler._resolve_origin", return_value=None), \
             patch("opencodon.cron.scheduler.run_job", return_value=(True, "output", "", None)):
            tick(verbose=False)
    ...
    def test_run_job_sets_auto_delivery_env_from_dotenv_home_channel(self, tmp_path, monkeypatch):
        ...
        with patch("opencodon.cron.scheduler._opencodon_home", tmp_path), \
             ...
             patch("opencodon.core.run_agent.AIAgent", FakeAgent):
            success, output, final_response, error = run_job(job)
    ...
    def test_run_job_heartbeats_oneshot_claim_in_both_wait_modes(
        ...
        with patch("opencodon.cron.scheduler._opencodon_home", tmp_path), \
             ...
             patch("opencodon.core.run_agent.AIAgent", FakeAgent), \
             patch("opencodon.cron.scheduler.concurrent.futures.ThreadPoolExecutor", return_value=fake_pool), \
             patch("opencodon.cron.scheduler.concurrent.futures.wait", side_effect=wait_results), \
             patch("opencodon.cron.scheduler.time.monotonic", side_effect=monotonic_ticks.__next__), \
             patch("opencodon.cron.scheduler.heartbeat_run_claim", return_value=True) as heartbeat:
...
class TestRunJobConfigLogging:
    ...
    def test_bad_config_yaml_is_logged(self, caplog, tmp_path):
        ...
        with patch("opencodon.cron.scheduler._opencodon_home", tmp_path), \
            ...
            with caplog.at_level(logging.WARNING, logger="opencodon.cron.scheduler"):
                run_job(job)
...
class TestRunJobModelResolution:
    ...
    def test_config_model_alias_key_resolves(self, tmp_path, monkeypatch):
        ...
        ``opencodon_cli/oneshot.py``, ``fallback_cmd.py`` and ``prompt_size.py``
...
class TestRunJobSkillBacked:
    def test_run_job_preserves_skill_env_passthrough_into_worker_thread(self, tmp_path):
        ...
        def _skill_view(name):
            assert name == "notion"
            from opencodon.tools.env_passthrough import register_env_passthrough

            register_env_passthrough(["NOTION_API_KEY"])
            return json.dumps({"success": True, "content": "# notion\nUse Notion."})

        def _run_conversation(prompt):
            from opencodon.tools.env_passthrough import get_all_passthrough
        ...
        with patch("opencodon.cron.scheduler._opencodon_home", tmp_path), \
             ...
             patch("opencodon.tools.skills_tool.skill_view", side_effect=_skill_view), \
             patch("opencodon.core.run_agent.AIAgent") as mock_agent_cls:
            mock_agent = MagicMock()
    ...
    def test_run_job_preserves_credential_file_passthrough_into_worker_thread(self, tmp_path):
        ...
        def _skill_view(name):
            ...
            from opencodon.tools.credential_files import register_credential_file

            register_credential_file("credentials/google_token.json")
            return json.dumps({"success": True, "content": "# google-workspace\nUse Google."})

        def _run_conversation(prompt):
            from opencodon.tools.credential_files import _get_registered
        ...
        with patch("opencodon.cron.scheduler._opencodon_home", tmp_path), \
             patch("opencodon.cron.scheduler._resolve_origin", return_value=None), \
             patch("opencodon.tools.credential_files._resolve_opencodon_home", return_value=tmp_path), \
             patch("opencodon.config.env_loader.load_opencodon_dotenv"), \
             patch("opencodon.config.env_loader.reset_secret_source_cache"), \
             patch("opencodon.state.SessionDB", return_value=fake_db), \
             patch(
    ...
    def test_run_job_loads_skill_and_disables_recursive_cron_tools(self, tmp_path):
        ...
        with patch("opencodon.cron.scheduler._opencodon_home", tmp_path), \
             ...
             patch("opencodon.tools.skills_tool.skill_view", return_value=json.dumps({"success": True, "content": "# Blogwatcher\nFollow this skill."})), \
             patch("opencodon.core.run_agent.AIAgent") as mock_agent_cls:
            mock_agent = MagicMock()
            mock_agent.run_conversation.return_value = {"final_response": "ok"}
            mock_agent_cls.return_value = mock_agent
    ...
    def test_run_job_loads_multiple_skills_in_order(self, tmp_path):
        ...
        with patch("opencodon.cron.scheduler._opencodon_home", tmp_path), \
             ...
             patch("opencodon.tools.skills_tool.skill_view", side_effect=_skill_view) as skill_view_mock, \
             patch("opencodon.core.run_agent.AIAgent") as mock_agent_cls:
            mock_agent = MagicMock()
            mock_agent.run_conversation.return_value = {"final_response": "ok"}
            mock_agent_cls.return_value = mock_agent
...
class TestSilentDelivery:
    ...
    def test_silent_response_suppresses_delivery(self, caplog):
        with patch("opencodon.cron.scheduler.get_due_jobs", return_value=[self._make_job()]), \
             patch("opencodon.cron.scheduler.run_job", return_value=(True, "# output", "[SILENT]", None)), \
             patch("opencodon.cron.scheduler.save_job_output", return_value="/tmp/out.md"), \
             patch("opencodon.cron.scheduler._deliver_result") as deliver_mock, \
             patch("opencodon.cron.scheduler.mark_job_run"):
            from opencodon.cron.scheduler import tick
            with caplog.at_level(logging.INFO, logger="opencodon.cron.scheduler"):
                tick(verbose=False)
    ...
    def test_silent_with_note_suppresses_delivery(self):
        with patch("opencodon.cron.scheduler.get_due_jobs", return_value=[self._make_job()]), \
             patch("opencodon.cron.scheduler.run_job", return_value=(True, "# output", "[SILENT] No changes detected", None)), \
             patch("opencodon.cron.scheduler.save_job_output", return_value="/tmp/out.md"), \
             patch("opencodon.cron.scheduler._deliver_result") as deliver_mock, \
             patch("opencodon.cron.scheduler.mark_job_run"):
            from opencodon.cron.scheduler import tick
    ...
    def test_silent_trailing_suppresses_delivery(self):
        ...
        with patch("opencodon.cron.scheduler.get_due_jobs", return_value=[self._make_job()]), \
             patch("opencodon.cron.scheduler.run_job", return_value=(True, "# output", response, None)), \
             patch("opencodon.cron.scheduler.save_job_output", return_value="/tmp/out.md"), \
             patch("opencodon.cron.scheduler._deliver_result") as deliver_mock, \
             patch("opencodon.cron.scheduler.mark_job_run"):
            from opencodon.cron.scheduler import tick
    ...
    def test_silent_is_case_insensitive(self):
        with patch("opencodon.cron.scheduler.get_due_jobs", return_value=[self._make_job()]), \
             patch("opencodon.cron.scheduler.run_job", return_value=(True, "# output", "[silent] nothing new", None)), \
             patch("opencodon.cron.scheduler.save_job_output", return_value="/tmp/out.md"), \
             patch("opencodon.cron.scheduler._deliver_result") as deliver_mock, \
             patch("opencodon.cron.scheduler.mark_job_run"):
            from opencodon.cron.scheduler import tick
    ...
    def test_bracketless_silent_variants_suppress(self):
        ...
        for marker in ("SILENT", "NO_REPLY", "NO REPLY", "no_reply"):
            with patch("opencodon.cron.scheduler.get_due_jobs", return_value=[self._make_job()]), \
                 patch("opencodon.cron.scheduler.run_job", return_value=(True, "# output", marker, None)), \
                 patch("opencodon.cron.scheduler.save_job_output", return_value="/tmp/out.md"), \
                 patch("opencodon.cron.scheduler._deliver_result") as deliver_mock, \
                 patch("opencodon.cron.scheduler.mark_job_run"):
                tick(verbose=False)
    ...
    def test_is_cron_silence_response_contract(self):
        ...
        from opencodon.cron.scheduler import _is_cron_silence_response as sil
    ...
    def test_failed_job_always_delivers(self):
        ...
        with patch("opencodon.cron.scheduler.get_due_jobs", return_value=[self._make_job()]), \
             patch("opencodon.cron.scheduler.run_job", return_value=(False, "# output", "", "some error")), \
             patch("opencodon.cron.scheduler.save_job_output", return_value="/tmp/out.md"), \
             patch("opencodon.cron.scheduler._deliver_result") as deliver_mock, \
             patch("opencodon.cron.scheduler.mark_job_run"):
            from opencodon.cron.scheduler import tick
    ...
    def test_output_saved_even_when_delivery_suppressed(self):
        with patch("opencodon.cron.scheduler.get_due_jobs", return_value=[self._make_job()]), \
             patch("opencodon.cron.scheduler.run_job", return_value=(True, "# full output", "[SILENT]", None)), \
             patch("opencodon.cron.scheduler.save_job_output") as save_mock, \
             patch("opencodon.cron.scheduler._deliver_result") as deliver_mock, \
             patch("opencodon.cron.scheduler.mark_job_run"):
    ...
    def test_whitespace_only_response_is_marked_failed_not_delivered(self):
        ...
        with patch("opencodon.cron.scheduler.get_due_jobs", return_value=[self._make_job()]), \
             patch("opencodon.cron.scheduler.run_job", return_value=(True, "# output", "   \n\t  ", None)), \
             patch("opencodon.cron.scheduler.save_job_output", return_value="/tmp/out.md"), \
             patch("opencodon.cron.scheduler._deliver_result") as deliver_mock, \
             patch("opencodon.cron.scheduler.mark_job_run") as mark_mock:
            from opencodon.cron.scheduler import tick
            tick(verbose=False)
...
class TestOneShotDispatchClaim:
    ...
    def test_claim_runs_before_run_job(self):
        order = []
        with patch("opencodon.cron.scheduler.get_due_jobs", return_value=[self._oneshot()]), \
             patch("opencodon.cron.scheduler.claim_dispatch", side_effect=lambda _id: order.append("claim") or True), \
             patch("opencodon.cron.scheduler.run_job", side_effect=lambda _j, **_kw: order.append("run") or (True, "# out", "ok", None)), \
             patch("opencodon.cron.scheduler.save_job_output", return_value="/tmp/out.md"), \
             patch("opencodon.cron.scheduler._deliver_result"), \
             patch("opencodon.cron.scheduler.mark_job_run"):
            from opencodon.cron.scheduler import tick
            tick(verbose=False)
    ...
    def test_refused_claim_skips_run_job(self):
        with patch("opencodon.cron.scheduler.get_due_jobs", return_value=[self._oneshot()]), \
             patch("opencodon.cron.scheduler.claim_dispatch", return_value=False), \
             patch("opencodon.cron.scheduler.run_job") as run_mock, \
             patch("opencodon.cron.scheduler.save_job_output"), \
             patch("opencodon.cron.scheduler._deliver_result") as deliver_mock, \
             patch("opencodon.cron.scheduler.mark_job_run") as mark_mock:
            from opencodon.cron.scheduler import tick
            tick(verbose=False)
...
class TestParseWakeGate:
    ...
    def test_empty_output_wakes(self):
        from opencodon.cron.scheduler import _parse_wake_gate
...
class TestRunJobWakeGate:
    ...
    def test_wake_false_skips_agent_and_returns_silent(self, caplog):
        ...
        suppressed."""
        from opencodon.cron.scheduler import SILENT_MARKER
        import opencodon.cron.scheduler as scheduler

        with patch.object(scheduler, "_run_job_script",
                          return_value=(True, '{"wakeAgent": false}')), \
             patch("opencodon.core.run_agent.AIAgent") as agent_cls:
    ...
    def test_wake_true_runs_agent_with_injected_output(self):
        ...
        with patch.object(scheduler, "_run_job_script",
             ...
             patch("opencodon.core.run_agent.AIAgent", return_value=agent) as agent_cls:
            success, doc, final, err = scheduler.run_job(self._make_job())
    ...
    def test_script_runs_only_once_on_wake(self):
        ...
        with patch.object(scheduler, "_run_job_script", side_effect=_script_stub), \
             patch("opencodon.core.run_agent.AIAgent", return_value=agent):
...
class TestBuildJobPromptMissingSkill:
    ...
    def test_missing_skill_does_not_raise(self):
        ...
        with patch("opencodon.tools.skills_tool.skill_view", side_effect=self._missing_skill_view):
    ...
    def test_valid_skill_loaded_alongside_missing(self):
        ...
        with patch("opencodon.tools.skills_tool.skill_view", side_effect=_mixed_skill_view):
...
class TestBuildJobPromptAbsoluteSkillPath:
    ...
    def test_absolute_skill_path_normalized_before_skill_view(self, tmp_path):
        ...
        with patch("opencodon.tools.skills_tool.SKILLS_DIR", skills_dir), \
             patch("opencodon.tools.skills_tool.skill_view", side_effect=_skill_view):
...
class TestBuildJobPromptBumpUse:
    ...
    def test_bump_use_called_for_loaded_skill(self):
        ...
        with patch("opencodon.tools.skills_tool.skill_view", side_effect=_skill_view), \
             patch("opencodon.tools.skill_usage.bump_use") as mock_bump:
    ...
    def test_bump_use_not_called_for_missing_skill(self):
        ...
        with patch("opencodon.tools.skills_tool.skill_view", side_effect=_missing_view), \
             patch("opencodon.tools.skill_usage.bump_use") as mock_bump:
    ...
    def test_bump_failure_does_not_break_prompt(self, caplog):
        ...
        with patch("opencodon.tools.skills_tool.skill_view", side_effect=_skill_view), \
             patch("opencodon.tools.skill_usage.bump_use", side_effect=RuntimeError("boom")), \
             caplog.at_level(logging.DEBUG, logger="opencodon.cron.scheduler"):
...
class TestParallelTick:
    ...
    def test_parallel_jobs_run_concurrently(self):
        ...
        with patch("opencodon.cron.scheduler.get_due_jobs", return_value=jobs), \
             patch("opencodon.cron.scheduler.advance_next_run"), \
             patch("opencodon.cron.scheduler.run_job", side_effect=mock_run_job), \
             patch("opencodon.cron.scheduler.save_job_output", return_value="/tmp/out.md"), \
             patch("opencodon.cron.scheduler._deliver_result", return_value=None), \
             patch("opencodon.cron.scheduler.mark_job_run"):
            from opencodon.cron.scheduler import tick
...
class TestHomeTargetEnvVarRegistry:
    ...
    def test_whatsapp_cloud_registered(self):
        ...
        from opencodon.cron.scheduler import _HOME_TARGET_ENV_VARS
...
class TestCronDeliveryMirror:
    ...
    def test_gate_default_off(self):
        from opencodon.cron.scheduler import _cron_mirror_delivery_enabled
    ...
    def test_target_matches_origin_exact(self):
        from opencodon.cron.scheduler import _target_matches_origin
    ...
    def test_open_thread_returns_id_on_thread_platform(self):
        ...
        from opencodon.cron.scheduler import _open_continuable_cron_thread
    ...
    def test_seed_thread_session_creates_session_and_mirrors(self):
        ...
        from opencodon.cron.scheduler import _seed_cron_thread_session
...
class TestSetCronSessionTitle:
    ...
    def test_sets_title_when_no_collision(self):
        from opencodon.cron.scheduler import _set_cron_session_title
        db = MagicMock()
