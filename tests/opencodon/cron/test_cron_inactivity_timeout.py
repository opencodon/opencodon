...
- Unlimited timeout (OPENCODON_CRON_TIMEOUT=0)
- Backward compat: OPENCODON_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """OPENCODON_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """OPENCODON_CRON_TIMEOUT env var is respected."""
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """OPENCODON_CRON_TIMEOUT=0 yields None (unlimited)."""
    ...
    def test_timeout_invalid_value_falls_back_to_default(self, monkeypatch):
        """OPENCODON_CRON_TIMEOUT=abc should fall back to 600s, not raise ValueError."""
        monkeypatch.setenv("OPENCODON_CRON_TIMEOUT", "abc")
        raw = os.getenv("OPENCODON_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
    ...
    def test_timeout_empty_string_uses_default(self, monkeypatch):
        """OPENCODON_CRON_TIMEOUT='' (empty) should use the 600s default."""
        monkeypatch.setenv("OPENCODON_CRON_TIMEOUT", "")
        raw = os.getenv("OPENCODON_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
        assert _cron_timeout == 600.0
...
class TestSysPathOrdering:
    ...
    def test_opencodon_time_importable(self):
        ...
        from opencodon.cron.scheduler import _opencodon_now
    ...
    def test_opencodon_constants_importable(self):
        ...
        assert callable(get_opencodon_home)
