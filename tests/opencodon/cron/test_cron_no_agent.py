...
@pytest.fixture
def opencodon_env(tmp_path, monkeypatch):
    ...
    import opencodon.cron.jobs
    importlib.reload(opencodon.cron.jobs)
    import opencodon.cron.scheduler
    importlib.reload(opencodon.cron.scheduler)
...
def test_create_job_no_agent_requires_script(opencodon_env):
...
def test_create_job_no_agent_stores_field(opencodon_env):
    from opencodon.cron.jobs import create_job

    script_path = opencodon_env / "scripts" / "watchdog.sh"
...
def test_create_job_default_is_not_no_agent(opencodon_env):
    from opencodon.cron.jobs import create_job
...
def test_update_job_roundtrips_no_agent_flag(opencodon_env):
    from opencodon.cron.jobs import create_job, update_job, get_job

    script_path = opencodon_env / "scripts" / "w.sh"
...
def test_cronjob_tool_create_no_agent_without_script_errors(opencodon_env):
...
def test_cronjob_tool_create_no_agent_with_script_succeeds(opencodon_env):
    from opencodon.tools.cronjob_tools import cronjob

    script_path = opencodon_env / "scripts" / "alert.sh"
    script_path.write_text("#!/bin/bash\necho alert\n")
...
def test_cronjob_tool_update_toggles_no_agent(opencodon_env):
    from opencodon.tools.cronjob_tools import cronjob
...
def test_cronjob_tool_update_no_agent_without_script_errors(opencodon_env):
...
def test_cronjob_tool_create_does_not_require_prompt_when_no_agent(opencodon_env):
...
def test_run_job_no_agent_success_returns_script_stdout(opencodon_env):
    """Happy path: script exits 0 with output, delivered verbatim."""
    from opencodon.cron.jobs import create_job
    from opencodon.cron.scheduler import run_job
...
def test_run_job_no_agent_empty_output_is_silent(opencodon_env):
    """Empty stdout → SILENT_MARKER, which suppresses delivery downstream."""
    from opencodon.cron.jobs import create_job
    from opencodon.cron.scheduler import run_job, SILENT_MARKER

    script_path = opencodon_env / "scripts" / "quiet.sh"
    script_path.write_text("#!/bin/bash\n# nothing to say\n")
...
def test_run_job_no_agent_wake_gate_is_silent(opencodon_env):
    """wakeAgent=false gate in stdout triggers a silent run."""
    from opencodon.cron.jobs import create_job
    from opencodon.cron.scheduler import run_job, SILENT_MARKER

    script_path = opencodon_env / "scripts" / "gated.sh"
...
def test_run_job_no_agent_script_failure_delivers_error(opencodon_env):
    """Non-zero exit → success=False, error alert is the delivered message."""
    from opencodon.cron.jobs import create_job
    from opencodon.cron.scheduler import run_job

    script_path = opencodon_env / "scripts" / "broken.sh"
    script_path.write_text("#!/bin/bash\necho oops >&2\nexit 3\n")
...
def test_run_job_no_agent_never_invokes_aiagent(opencodon_env):
    ...
    with patch("opencodon.core.run_agent.AIAgent") as ai_mock:
        from opencodon.cron.scheduler import run_job
...
def test_run_job_script_shell_script_runs_via_bash(opencodon_env):
    """.sh files should execute under /bin/bash even without a shebang line."""
    from opencodon.cron.scheduler import _run_job_script

    script_path = opencodon_env / "scripts" / "shelly.sh"
...
def test_run_job_script_bash_extension_also_runs_via_bash(opencodon_env):
    from opencodon.cron.scheduler import _run_job_script

    script_path = opencodon_env / "scripts" / "thing.bash"
...
def test_run_job_script_python_still_runs_via_python(opencodon_env):
    """Regression: .py files must keep running via sys.executable."""
    from opencodon.cron.scheduler import _run_job_script

    script_path = opencodon_env / "scripts" / "py.py"
...
def test_run_job_script_path_traversal_still_blocked(opencodon_env):
