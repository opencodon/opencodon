...
These exercise the real store against a temp OPENCODON_HOME (no mocks) per the
...
def temp_home(tmp_path, monkeypatch):
    """Isolated OPENCODON_HOME so jobs.json doesn't touch the real store."""
    monkeypatch.setenv("OPENCODON_HOME", str(tmp_path))
    # cron.jobs caches no home at import; get_opencodon_home() reads the env live.
    yield tmp_path
...
def test_claim_succeeds_once_then_blocks(temp_home):
    ...
    from opencodon.cron.jobs import create_job, claim_job_for_fire, get_job
...
def test_claim_oneshot_cannot_be_double_claimed(temp_home):
    ...
    from opencodon.cron.jobs import create_job, claim_job_for_fire
...
def test_claim_unknown_job_returns_false(temp_home):
    from opencodon.cron.jobs import claim_job_for_fire
...
def test_claim_paused_job_returns_false(temp_home):
    ...
    from opencodon.cron.jobs import create_job, claim_job_for_fire, pause_job
...
def test_mark_job_run_clears_claim(temp_home):
    ...
    from opencodon.cron.jobs import create_job, claim_job_for_fire, mark_job_run, get_job
