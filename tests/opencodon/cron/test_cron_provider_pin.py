...
resolve_runtime_provider against a temp OPENCODON_HOME) and the create_job
...
class TestCreateJobSnapshot:
    ...
    def _isolate_storage(monkeypatch):
        """Patch opencodon.cron.jobs storage so create_job never touches the real store."""
        import contextlib
    ...
    def test_pinned_job_skips_snapshot(self, monkeypatch):
        ...
        with patch("opencodon.core.providers.runtime_provider.resolve_runtime_provider", resolver):
            job = jobs.create_job(
    ...
    def test_unpinned_model_captures_model_snapshot(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "opencodon.cron.jobs.get_opencodon_home", lambda: tmp_path, raising=True
        )
        with patch(
            "opencodon.core.providers.runtime_provider.resolve_runtime_provider",
            return_value={"provider": "openrouter"},
...
def _run_with_current_provider_and_model(job, current_provider, current_model, tmp_path):
    ...
    fake_db = MagicMock()
    with patch("opencodon.cron.scheduler._opencodon_home", tmp_path), \
         patch("opencodon.cron.scheduler._get_opencodon_home", return_value=tmp_path), \
         patch("opencodon.cron.scheduler._resolve_origin", return_value=None), \
         patch("opencodon.config.env_loader.load_opencodon_dotenv"), \
         patch("opencodon.config.env_loader.reset_secret_source_cache"), \
         patch("opencodon.state.SessionDB", return_value=fake_db), \
