...
OPENCODON_HOME so the real suggestions.json is never touched.
"""
...
def store(tmp_path, monkeypatch):
    """A opencodon.cron.suggestions module bound to an isolated OPENCODON_HOME."""
    ...
    import opencodon.cron.suggestions as s
    importlib.reload(s)
    return s
...
class TestStore:
    ...
    def test_accept_creates_job_and_marks_accepted(self, store):
        ...
        with patch("opencodon.cron.jobs.create_job", fake_create_job):
    ...
    def test_clear_resolved_drops_accepted_only(self, store):
        ...
        with patch("opencodon.cron.jobs.create_job", lambda **k: {"id": "j"}):
...
class TestCatalog:
    def test_seed_registers_all_entries(self, store):
        from opencodon.cron.suggestion_catalog import CATALOG, seed_catalog_suggestions
    ...
    def test_monitor_entry_references_classifier_script(self):
        from opencodon.cron.suggestion_catalog import CATALOG, classify_items_script_path
        ...
        assert "opencodon.cron.scripts.classify_items" in monitor.job_spec["prompt"]
...
class TestBlueprintBridge:
    def test_blueprint_registers_suggestion(self, store):
        from opencodon.tools.blueprints import BlueprintSpec, register_blueprint_suggestion

        spec = BlueprintSpec(skill_name="morning-brief", schedule="0 8 * * *", deliver="telegram")
        with patch("opencodon.cron.suggestions.add_suggestion", store.add_suggestion):
    ...
    def test_blueprint_to_job_spec_matches_create_blueprint_job(self):
        from opencodon.tools.blueprints import BlueprintSpec, blueprint_to_job_spec
