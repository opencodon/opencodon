...
which is already bounded by OPENCODON_CRON_TIMEOUT — the worker thread submitted
...
timeout (OPENCODON_CRON_SESSION_DB_TIMEOUT, default 10s) so a hang there can
...
def _hanging_session_db(never_set: threading.Event):
    """Stand-in for opencodon_state.SessionDB() that blocks until released —
...
class TestSessionDbInitTimeout:
    def test_run_job_does_not_hang_when_sessiondb_init_wedges(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OPENCODON_CRON_SESSION_DB_TIMEOUT", "0.2")
    ...
    def test_invalid_timeout_env_falls_back_to_default(self, tmp_path, monkeypatch, caplog):
        """A malformed OPENCODON_CRON_SESSION_DB_TIMEOUT logs a warning and still
        bounds the call (mirrors OPENCODON_CRON_TIMEOUT's own fallback)."""
        monkeypatch.setenv("OPENCODON_CRON_SESSION_DB_TIMEOUT", "not-a-number")
        fake_db = MagicMock()
        ...
        assert any(
            "OPENCODON_CRON_SESSION_DB_TIMEOUT" in rec.message
    ...
    def test_timeout_resolved_from_config_yaml(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("OPENCODON_CRON_SESSION_DB_TIMEOUT", raising=False)
