...
class TestMarkRunningJobsInterrupted:
    def test_no_op_when_nothing_running(self):
        ...
        with patch("opencodon.cron.scheduler.mark_job_run") as mock_mark:
    ...
    def test_sets_interrupted_flag_for_consumption_by_run_one_job(self):
        ...
        with patch("opencodon.cron.scheduler.mark_job_run"):
    ...
    def test_one_job_marking_failure_does_not_block_the_others(self):
        ...
        with patch("opencodon.cron.scheduler.mark_job_run", side_effect=_side_effect):
            marked = sched.mark_running_jobs_interrupted("shutdown")
...
class TestRunOneJobHonoursInterruptedFlag:
    ...
    def test_success_path_skipped_when_interrupted(self):
        ...
        with patch("opencodon.cron.scheduler.claim_dispatch", return_value=True), \
             ...
             patch("opencodon.cron.scheduler._is_cron_silence_response", return_value=False), \
    ...
    def test_interrupted_job_delivers_failure_summary_not_raw_response(self):
        ...
        with patch("opencodon.cron.scheduler.claim_dispatch", return_value=True), \
             ...
             patch("opencodon.cron.scheduler.save_job_output", return_value="/tmp/out.md"), \
             patch(
                 "opencodon.cron.scheduler._summarize_cron_failure_for_delivery",
                 return_value="This run was interrupted.",
             ) as mock_summarize, \
             patch("opencodon.cron.scheduler._is_cron_silence_response", return_value=False), \
             patch("opencodon.cron.scheduler._deliver_result", return_value=None) as mock_deliver, \
