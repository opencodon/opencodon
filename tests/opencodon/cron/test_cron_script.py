...
- Path resolution (absolute, relative to OPENCODON_HOME/scripts/)
"""
...
class TestJobScriptField:
    ...
    def test_update_job_add_script(self, cron_env):
        from opencodon.cron.jobs import create_job, update_job
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit opencodon provider env (SECURITY.md §2.3)."""
        from opencodon.tools.environments.local import _OPENCODON_PROVIDER_ENV_BLOCKLIST
        from opencodon.cron.scheduler import _run_job_script

        # sorted() so the probed var is deterministic across runs
        # (frozenset iteration order varies with PYTHONHASHSEED).
        blocked_var = sorted(_OPENCODON_PROVIDER_ENV_BLOCKLIST)[0]
    ...
    def test_windows_uv_venv_python_script_bypasses_launcher(self, cron_env, tmp_path, monkeypatch):
        from opencodon.cron import scheduler as sched_mod
        from opencodon.cron.scheduler import _run_job_script
...
class TestBuildJobPromptWithScript:
    ...
    def test_script_output_injected(self, cron_env):
        from opencodon.cron.scheduler import _build_job_prompt
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.opencodon/scripts/ must be rejected."""
        from opencodon.cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("OPENCODON_SESSION_PLATFORM") is None
        assert os.environ.get("OPENCODON_SESSION_CHAT_ID") is None
        assert os.environ.get("OPENCODON_SESSION_CHAT_NAME") is None
