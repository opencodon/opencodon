...
When the gateway tears down (SIGTERM from ``opencodon update`` /
``opencodon gateway stop`` / systemd restart, or an OOM-kill), a cron tick can
...
class TestInterpreterShuttingDownHelper:
    def test_true_when_finalizing(self):
        from opencodon.cron.scheduler import _interpreter_shutting_down
...
class TestStandaloneDeliverySkipsDuringShutdown:
    ...
    def test_standalone_path_skips_without_scheduling(self):
        ...
        from opencodon.cron.scheduler import _deliver_result
