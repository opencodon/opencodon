...
profile's OPENCODON_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.opencodon/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active OPENCODON_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever OPENCODON_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    OPENCODON_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import opencodon_constants

    monkeypatch.setattr(
        opencodon_constants, "_get_platform_default_opencodon_home", lambda: root
    )
    monkeypatch.setenv("OPENCODON_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile OPENCODON_HOME (<root>/profiles/<name>), the cron store
    resolves to <profile>/cron, NOT the shared <root>/cron."""
    root = tmp_path / "opencodon_home"
...
def test_cron_execution_home_follows_active_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(scheduler, "_opencodon_home", None, raising=False)
    assert scheduler._get_opencodon_home().resolve() == profile_home.resolve()
    assert scheduler._get_opencodon_home().resolve() != root.resolve()


def test_cron_storage_unaffected_when_no_profile(tmp_path, monkeypatch):
    """With no profile (OPENCODON_HOME == root), the store is the root's cron dir
