...
CronScheduler plan, .opencodon/plans/cron-scheduler-provider-interface.md). They
...
No production code is exercised beyond the two ticker entry points:
  ...
  - opencodon_cli/web_server.py::_start_desktop_cron_ticker  (desktop fallback)

Both call `opencodon.cron.scheduler.tick(...)` on a loop and exit when their stop_event
is set. We patch `opencodon.cron.scheduler.tick` (both tickers import it locally as
...
def test_ticker_calls_tick_at_least_once_then_stops():
    ...
    with patch("opencodon.cron.scheduler.tick", side_effect=fake_tick):
...
def test_inprocess_provider_ticks_and_stops():
    """The built-in provider drives opencodon.cron.scheduler.tick(sync=False) on a loop
    ...
    with patch("opencodon.cron.scheduler.tick", side_effect=lambda *a, **k: calls.append(k) or 0):
        t = threading.Thread(
...
def test_resolve_defaults_to_builtin(monkeypatch):
    ...
    from opencodon.cron import scheduler_provider as sp
...
def test_ticker_survives_baseexception_from_tick():
    ...
    stop = threading.Event()
    prov = InProcessCronScheduler()
    with patch("opencodon.cron.scheduler.tick", side_effect=_boom), \
         patch("opencodon.cron.jobs.record_ticker_heartbeat"):
...
def test_ticker_records_heartbeat_each_iteration():
    ...
    stop = threading.Event()
    prov = InProcessCronScheduler()
    with patch("opencodon.cron.scheduler.tick", side_effect=lambda *a, **k: 0), \
         patch("opencodon.cron.jobs.record_ticker_heartbeat",
...
def test_failing_tick_records_liveness_but_not_success():
    ...
    stop = threading.Event()
    prov = InProcessCronScheduler()
    with patch("opencodon.cron.scheduler.tick", side_effect=RuntimeError("every tick fails")), \
         patch("opencodon.cron.jobs.record_ticker_heartbeat",
               side_effect=lambda success=False: beats.append(success)):
        t = threading.Thread(target=prov.start, args=(stop,), kwargs={"interval": 0}, daemon=True)
        t.start()
...
class TestGuardJobCredentialExfil:
    ...
    def test_validator_exception_with_base_url_fails_closed(self, monkeypatch):
        ...
        import pytest
        import opencodon.tools.cronjob_tools as ct
        from opencodon.cron.scheduler import _guard_job_credential_exfil
