...
class TestCronFilePermissions(unittest.TestCase):
    ...
    @patch("opencodon.cron.jobs.CRON_DIR")
    ...
    def test_ensure_dirs_sets_0700(self, mock_jobs_file, mock_output, mock_cron):
        ...
        with patch("opencodon.cron.jobs.CRON_DIR", cron_dir), \
             patch("opencodon.cron.jobs.OUTPUT_DIR", output_dir):
            from opencodon.cron.jobs import ensure_dirs
            ensure_dirs()
    ...
    def test_save_jobs_sets_0600(self, mock_jobs_file, mock_output, mock_cron):
        ...
        with patch("opencodon.cron.jobs.CRON_DIR", cron_dir), \
             patch("opencodon.cron.jobs.OUTPUT_DIR", output_dir), \
             patch("opencodon.cron.jobs.JOBS_FILE", jobs_file):
    ...
    def test_save_job_output_sets_0600(self):
        ...
        with patch("opencodon.cron.jobs.OUTPUT_DIR", output_dir), \
             patch("opencodon.cron.jobs.CRON_DIR", Path(self.tmpdir)), \
             patch("opencodon.cron.jobs.ensure_dirs"):
            output_dir.mkdir(parents=True, exist_ok=True)
            from opencodon.cron.jobs import save_job_output
