...
def test_cron_runs_cli_prints_execution_history(monkeypatch, tmp_path, capsys):
    ...
    from opencodon.frontends.cli.cron import cron_runs
...
def test_restart_marks_interrupted_execution_unknown_without_requeue(tmp_path):
    """Real temp-OPENCODON_HOME subprocess restart: in-flight is audit-only unknown."""
    home = tmp_path / "home"
    ...
    create = subprocess.run(
        [
            sys.executable,
            "-c",
            "from opencodon.cron.executions import create_execution, mark_execution_running; "
    ...
    recover = subprocess.run(
        [
            sys.executable,
            "-c",
            "import json; from opencodon.cron.executions import recover_interrupted_executions, list_executions; "
...
def test_provider_start_recovers_interrupted_records_before_tick(monkeypatch):
    import opencodon.cron.scheduler_provider as provider

    events = []
    stop = __import__("threading").Event()
    stop.set()
    monkeypatch.setattr(
        "opencodon.cron.executions.recover_interrupted_executions",
        lambda: events.append("recover") or 0,
        raising=False,
    )
    monkeypatch.setattr("opencodon.cron.jobs.record_ticker_heartbeat", lambda **_kwargs: events.append("heartbeat"))
