...
class TestParseSchedule:
    ...
    def test_naive_iso_anchors_to_configured_tz_not_server_local(self, monkeypatch):
        """A naive ISO timestamp must be interpreted in the CONFIGURED opencodon
        ...
        due-check compares against ``_opencodon_now()`` (configured zone), so the
        ...
        monkeypatch.setattr("opencodon.cron.jobs._opencodon_now", lambda: configured_now)
...
class TestNaiveScheduleTimezoneDivergence:
    ...
    become due even when the configured opencodon timezone differs from the
    ...
    def test_recent_past_oneshot_is_due_under_diverging_tz(self, tmp_cron_dir, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.cron.jobs._opencodon_now", lambda: now)
...
class TestResolveJobRef:
    ...
    def test_resolve_by_exact_id(self, tmp_cron_dir):
        from opencodon.cron.jobs import resolve_job_ref
    ...
    def test_trigger_by_name(self, tmp_cron_dir):
        from opencodon.cron.jobs import trigger_job
...
class TestMarkJobRun:
    ...
    def test_recurring_cron_not_disabled_when_croniter_missing(self, tmp_cron_dir, monkeypatch):
        ...
        # and this run.
        monkeypatch.setattr("opencodon.cron.jobs.HAS_CRONITER", False)
    ...
    def test_recurring_interval_not_disabled_when_next_run_is_none(self, tmp_cron_dir, monkeypatch):
        ...
        monkeypatch.setattr("opencodon.cron.jobs.compute_next_run", lambda *a, **kw: None)
...
class TestAdvanceNextRun:
    ...
    def test_advances_interval_job(self, tmp_cron_dir):
        ...
        updated = get_job(job["id"])
        from opencodon.cron.jobs import _ensure_aware, _opencodon_now
        new_next_dt = _ensure_aware(datetime.fromisoformat(updated["next_run_at"]))
        assert new_next_dt > _opencodon_now(), "next_run_at should be in the future after advance"
    ...
    def test_already_future_stays_future(self, tmp_cron_dir):
        ...
        from opencodon.cron.jobs import _ensure_aware, _opencodon_now
        new_next_dt = _ensure_aware(datetime.fromisoformat(updated["next_run_at"]))
        assert new_next_dt > _opencodon_now(), "next_run_at should remain in the future"
...
class TestGetDueJobs:
    ...
    def test_long_execution_does_not_perpetually_defer(self, tmp_cron_dir, monkeypatch):
        ...
        stale = (_opencodon_now() - timedelta(minutes=11)).isoformat()
        jobs[0]["next_run_at"] = stale
        jobs[0]["last_run_at"] = (_opencodon_now() - timedelta(minutes=1)).isoformat()
        save_jobs(jobs)
    ...
    def test_stale_repeat_limited_job_consumes_one_run_on_catchup(self, tmp_cron_dir, monkeypatch):
        ...
        from opencodon.cron.jobs import _opencodon_now
        job = create_job(prompt="Limited", schedule="every 5m", repeat=3)
        jobs = load_jobs()
        jobs[0]["next_run_at"] = (_opencodon_now() - timedelta(minutes=11)).isoformat()
        jobs[0]["last_run_at"] = (_opencodon_now() - timedelta(minutes=11)).isoformat()
        save_jobs(jobs)
    ...
    def test_one_shot_not_redispatched_while_running(self, tmp_cron_dir, monkeypatch):
        ...
        for gap in (28, 61, 130):
            monkeypatch.setattr("opencodon.cron.jobs._opencodon_now",
    ...
    def test_one_shot_run_claim_expires_after_ttl(self, tmp_cron_dir, monkeypatch):
        ...
        from opencodon.cron.jobs import _opencodon_now, _oneshot_run_claim_ttl_seconds
    ...
    def test_stale_maxed_oneshot_kept_while_running_in_this_process(
        ...
        """
        import opencodon.cron.scheduler as scheduler_mod
        from opencodon.cron.jobs import _opencodon_now, _oneshot_run_claim_ttl_seconds
        monkeypatch.delenv("OPENCODON_CRON_TIMEOUT", raising=False)
        ttl = _oneshot_run_claim_ttl_seconds()
        t0 = _opencodon_now()
    ...
    def test_cron_next_run_offset_migration_is_rescheduled_not_fired(self, tmp_cron_dir, monkeypatch):
        ...
        # A 21:00 cron was stored while opencodon/system local time was UTC+10.
...
class TestCronOutputRetention:
    ...
    def test_prune_keeps_newest_n(self, tmp_path):
        from opencodon.cron.jobs import _prune_job_output
        d = tmp_path / "job"
    ...
    def test_save_job_output_prunes_old_runs(self, tmp_cron_dir, monkeypatch):
        from opencodon.cron.jobs import save_job_output, _job_output_dir
        monkeypatch.setattr("opencodon.cron.jobs._cron_output_keep", lambda: 3)
        seq = iter(
            datetime(2026, 6, 25, 10, 0, 0, tzinfo=timezone.utc) + timedelta(seconds=i)
            for i in range(8)
        )
        monkeypatch.setattr("opencodon.cron.jobs._opencodon_now", lambda: next(seq))
        for _ in range(8):
    ...
    def test_cron_output_keep_reads_config(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "opencodon.config.load_config", lambda: {"cron": {"output_retention": 7}}
        )
    ...
    def test_cron_output_keep_defaults_on_bad_config(self, monkeypatch):
        import opencodon.cron.jobs as jobs
        monkeypatch.setattr(
            "opencodon.config.load_config", lambda: {"cron": {"output_retention": "oops"}}
        )
...
class TestLateEnvRepointScopesStore:
    """A OPENCODON_HOME set AFTER opencodon.cron.jobs import must scope the store even
...
class TestJobsJsonUtf8Bom:
    ...
    def test_load_jobs_accepts_utf8_bom(self, tmp_cron_dir):
        ...
        from opencodon.cron.jobs import JOBS_FILE, load_jobs
