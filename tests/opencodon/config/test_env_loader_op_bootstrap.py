...
1. ``load_opencodon_dotenv()`` auto-loads ``~/.opencodon/.op.env`` so the
...
import opencodon.core.credentials.credential_pool as credential_pool  # noqa: E402
...
def test_op_env_autoloads_bootstrap_token_in_cron_context(tmp_path, monkeypatch):
    ...
    env_loader.load_opencodon_dotenv(opencodon_home=home)
...
def _seed_openrouter_token(monkeypatch, dotenv_value, environ_value):
    ...
    monkeypatch.setattr(
        "opencodon.core.credentials.auth.is_source_suppressed", lambda _p, _s: False
    )
