"""
Tests for mcp_serve — opencodon MCP server.
...
def _isolate_opencodon_home(tmp_path, monkeypatch):
    """Redirect OPENCODON_HOME to a temp directory."""
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking opencodon_state schema."""
...
def fake_mcp_server(populated_sessions_dir, mock_session_db, monkeypatch):
    from opencodon.frontends import mcp as mcp_serve
...
class TestContentExtraction:
    def test_text(self):
        from opencodon.frontends.mcp import _extract_message_content
...
class TestAttachmentExtraction:
    def test_image_url_block(self):
        from opencodon.frontends.mcp import _extract_attachments
        msg = {"content": [
...
class TestEventBridge:
    def test_create(self):
        from opencodon.frontends.mcp import EventBridge
        b = EventBridge()
        assert b._cursor == 0
        assert b._queue == []

    def test_enqueue_and_poll(self):
        from opencodon.frontends.mcp import EventBridge, QueueEvent
        b = EventBridge()
    ...
    def test_queue_limit(self):
        from opencodon.frontends.mcp import EventBridge, QueueEvent, QUEUE_LIMIT
        b = EventBridge()
...
class TestE2EEventsPoll:
    ...
    def test_poll_with_events(self, mcp_server_e2e, _event_loop):
        from opencodon.frontends.mcp import QueueEvent
        server, bridge = mcp_server_e2e
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        # _poll_once reads <OPENCODON_HOME>/state.db for its mtime gate; the autouse
        # fixture points OPENCODON_HOME at tmp_path.
        db_path = tmp_path / "state.db"
    ...
    def test_poll_interval_is_200ms(self):
        ...
        from opencodon.frontends.mcp import POLL_INTERVAL
