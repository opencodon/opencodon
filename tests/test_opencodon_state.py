"""Tests for opencodon_state.py — SessionDB SQLite CRUD, FTS5 search, export."""
...
class TestTimestampPreservation:
    ...
    def test_branch_copy_roundtrip_preserves_timestamps(self, db):
        ...
        # opencodon_cli/cli_commands_mixin.py and tui_gateway/server.py.
        for msg in history:
            db.append_message(
                "child",
...
class TestDeleteEmptySessions:
    ...
    in ``opencodon_cli/web_server.py``.
    ...
    def test_optimize_fts_storage_transitions_to_v23(self, tmp_path):
        ...
        try:
            ...
            assert db.get_meta("fts_storage_version") == str(
                opencodon_state.FTS_STORAGE_VERSION
            )
            assert db._conn.execute(
                "SELECT version FROM schema_version"
...
class TestApplyWalProbe:
    ...
    def test_macos_checkpoint_fullsync_barrier_applied(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(opencodon_state.sys, "platform", "darwin")
    ...
    def test_checkpoint_fullsync_barrier_skipped_off_darwin(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(opencodon_state.sys, "platform", "linux")
...
def test_refresh_compression_lock_requires_holder_and_preserves_reclaimability(db, monkeypatch):
    ...
    monkeypatch.setattr(opencodon_state.time, "time", lambda: 1000.0)
    ...
    monkeypatch.setattr(opencodon_state.time, "time", lambda: 1005.0)
    ...
    monkeypatch.setattr(opencodon_state.time, "time", lambda: 1016.0)
