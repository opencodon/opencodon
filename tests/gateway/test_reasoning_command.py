...
class TestReasoningCommand:
    ...
    def test_resolve_session_reasoning_prefers_session_override(self, tmp_path, monkeypatch):
        opencodon_home = tmp_path / "opencodon"
        opencodon_home.mkdir()
        (opencodon_home / "config.yaml").write_text("agent:\n  reasoning_effort: low\n", encoding="utf-8")
    ...
    def test_run_agent_reloads_reasoning_config_per_message(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_run, "_opencodon_home", opencodon_home)
        monkeypatch.setattr(gateway_run, "_env_path", opencodon_home / ".env")
...
class TestLoadShowReasoningCoercion:
    ...
    def _load_with_config(self, tmp_path, monkeypatch, yaml_body: str) -> bool:
        opencodon_home = tmp_path / "opencodon"
        opencodon_home.mkdir()
        (opencodon_home / "config.yaml").write_text(yaml_body, encoding="utf-8")
        monkeypatch.setattr(gateway_run, "_opencodon_home", opencodon_home)
