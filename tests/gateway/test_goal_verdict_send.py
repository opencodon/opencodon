...
async def test_goal_verdict_done_sent_via_adapter_send(opencodon_home):
    ...
    with patch("opencodon_cli.goals.judge_goal", return_value=("done", "the feature shipped", False, None, False)):
...
async def test_goal_verdict_continue_enqueues_continuation(opencodon_home):
    ...
    with patch("opencodon_cli.goals.judge_goal", return_value=("continue", "still needs work", False, None, False)):
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
...
async def test_goal_verdict_budget_exhausted_sends_pause(opencodon_home):
    ...
    from opencodon_cli.goals import GoalManager, save_goal
    ...
    with patch("opencodon_cli.goals.judge_goal", return_value=("continue", "keep going", False, None, False)):
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
...
async def test_goal_verdict_survives_adapter_without_send(opencodon_home):
    ...
    with patch("opencodon_cli.goals.judge_goal", return_value=("done", "ok", False, None, False)):
        # must not raise
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
