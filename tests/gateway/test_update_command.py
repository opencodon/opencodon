...
class TestHandleUpdateCommand:
    ...
    async def test_no_hermes_binary(self, tmp_path):
        """Returns error when hermes is not on PATH and opencodon_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
    ...
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m opencodon_cli.main when hermes not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "opencodon_cli.main" in joined or "bash" in call_args[0]
    ...
    async def test_resolve_hermes_bin_fallback(self):
        ...
        assert result == [sys.executable, "-m", "opencodon_cli.main"]
