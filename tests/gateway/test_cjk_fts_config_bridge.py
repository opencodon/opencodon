...
def _write_home(tmp_path: Path, sessions_cfg: dict, env_text: str = "") -> Path:
    ...
    (opencodon_home / ".env").write_text(env_text, encoding="utf-8")
    return opencodon_home


def test_cjk_fts_bridged_from_config(tmp_path, monkeypatch):
    home = _write_home(tmp_path, {"cjk_fts": False})
    monkeypatch.setattr(gateway_run, "_opencodon_home", home)
    monkeypatch.setenv("OPENCODON_CJK_FTS", "1")
    gateway_run._reload_runtime_env_preserving_config_authority()
    assert os.environ["OPENCODON_CJK_FTS"] == "False"


def test_search_slow_ms_bridged_from_config(tmp_path, monkeypatch):
    home = _write_home(tmp_path, {"search_slow_ms": 250})
    monkeypatch.setattr(gateway_run, "_opencodon_home", home)
    monkeypatch.delenv("OPENCODON_SEARCH_SLOW_MS", raising=False)
    gateway_run._reload_runtime_env_preserving_config_authority()
    assert os.environ["OPENCODON_SEARCH_SLOW_MS"] == "250"
...
def test_env_survives_when_config_omits_search_knobs(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OPENCODON_SEARCH_SLOW_MS", "700")
    gateway_run._reload_runtime_env_preserving_config_authority()
    assert os.environ["OPENCODON_CJK_FTS"] == "0"
    assert os.environ["OPENCODON_SEARCH_SLOW_MS"] == "700"
...
def test_config_false_disables_cjk_semantics(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OPENCODON_CJK_FTS", "True")
    assert _cjk_fts_config_enabled()
    monkeypatch.delenv("OPENCODON_CJK_FTS", raising=False)
