...
def test_root_session_no_compression(db):
    ...
    assert prov["currentOpencodonSessionId"] == "root1"
    assert prov["rootOpencodonSessionId"] == "root1"
    assert prov["parentOpencodonSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_opencodon_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentOpencodonSessionId"] == "old"
    assert prov["rootOpencodonSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousOpencodonSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootOpencodonSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootOpencodonSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_opencodon_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousOpencodonSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"opencodon"}
    assert "sessionProvenance" in meta["opencodon"]
    assert meta["opencodon"]["sessionProvenance"]["currentOpencodonSessionId"] == "root1"
