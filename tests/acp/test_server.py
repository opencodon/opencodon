"""Tests for acp_adapter.server — OpencodonACPAgent ACP server."""
...
def agent(mock_manager):
    """OpencodonACPAgent backed by a mock session manager."""
    return OpencodonACPAgent(session_manager=mock_manager)
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "opencodon"
        assert resp.agent_info.version == OPENCODON_VERSION
        ...
        assert payloads == [
            {
                "args": ["--setup"],
                "description": (
                    "Open opencodon' interactive model/provider setup in a terminal. "
                    "Use this when opencodon has not been configured on this machine yet."
                ),
                "id": TERMINAL_SETUP_AUTH_METHOD_ID,
                "name": "Configure opencodon provider",
                "type": "terminal",
            }
        ]
...
class TestSessionOps:
    ...
    async def test_load_session_replays_persisted_history_to_client(self, agent):
        ...
        state.history = [
            ...
            {"role": "assistant", "content": "OpencodonACPAgent._ADVERTISED_COMMANDS controls them."},
            {
                "role": "assistant",
                "content": "",
                "tool_calls": [
        ...
        assert replay_calls[1].kwargs["update"].content.text.startswith("OpencodonACPAgent")
    ...
    async def test_load_session_flags_compaction_summary_on_replayed_user_chunk(self, agent):
        """A replayed compaction summary must carry _meta.opencodon.compactionSummary.
        ...
        assert user_chunks[0].field_meta == {"opencodon": {"compactionSummary": True}}
    ...
    async def test_load_session_flags_compaction_summary_on_replayed_assistant_chunk(self, agent):
        ...
        assert agent_chunks[0].field_meta == {"opencodon": {"compactionSummary": True}}
    ...
    async def test_load_session_flags_merged_tail_summary_as_contains_not_standalone(self, agent):
        ...
        assert user_chunks[0].field_meta == {
            "opencodon": {"containsCompactionSummary": True}
        }
...
class TestPrompt:
    ...
    async def test_prompt_propagates_opencodon_session_id_env(self, agent, monkeypatch):
        ...
        def mock_run(user_message, conversation_history=None, task_id=None, **kwargs):
            ...
            captured["env"] = os.environ.get("OPENCODON_SESSION_ID")
        ...
        assert captured["env"] == new_resp.session_id, (
            "OPENCODON_SESSION_ID must be set to the originating ACP session id "
        ...
        assert os.environ.get("OPENCODON_SESSION_ID") is None, (
            "OPENCODON_SESSION_ID must be restored after the agent call so "
            "a re-used executor thread doesn't leak the id into the next "
            "session's tools"
        )

    @pytest.mark.asyncio
    async def test_prompt_restores_prior_opencodon_session_id(self, agent, monkeypatch):
        """If the env already had OPENCODON_SESSION_ID set (e.g. nested
        agent loops), the prior value must be restored after the inner
        prompt completes — not popped, not left at the inner id."""
        monkeypatch.setenv("OPENCODON_SESSION_ID", "outer-sess")
        ...
        def mock_run(*args, **kwargs):
            captured["inner"] = os.environ.get("OPENCODON_SESSION_ID")
            return {"final_response": "ok", "messages": []}
        ...
        assert os.environ.get("OPENCODON_SESSION_ID") == "outer-sess"
...
class TestSlashCommands:
    ...
    def test_version(self, agent, mock_manager):
        ...
        assert OPENCODON_VERSION in result
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        state.agent.enabled_toolsets = ["opencodon-acp"]
    ...
    async def test_refreshes_agent_tool_surface(self, agent, mock_manager):
        ...
        mock_defs.assert_called_once_with(
            enabled_toolsets=["opencodon-acp", "mcp-srv"],
            disabled_toolsets=None,
            quiet_mode=True,
        )
        assert state.agent.enabled_toolsets == ["opencodon-acp", "mcp-srv"]
