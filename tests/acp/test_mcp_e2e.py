...
class TestMcpRegistrationE2E:
    ...
    async def test_session_with_mcp_servers_registers_tools(self, acp_agent, mock_manager):
        """new_session with mcpServers converts them to opencodon config and registers."""
        servers = [
            McpServerStdio(
                name="test-fs",
        ...
        with patch("opencodon.tools.mcp_tool.register_mcp_servers", side_effect=mock_register), \
             patch("opencodon.tools.model_tools.get_tool_definitions", return_value=fake_tools):
