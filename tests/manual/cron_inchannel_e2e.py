...
Uses a throwaway OPENCODON_HOME so it never touches ~/.opencodon. No real names.
"""
...
def _fresh_home():
    """Point OPENCODON_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_opencodon_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["OPENCODON_HOME"] = d
    return Path(d)
...
def main():
    ...
    print(f"OPENCODON_HOME (throwaway): {HOME}")
