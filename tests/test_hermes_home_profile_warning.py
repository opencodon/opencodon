...
profile is active, get_opencodon_home() should:
  1. STILL return ~/.opencodon (raising would brick 30+ module-level callers)
...
def fresh_constants(monkeypatch, tmp_path):
    """Import opencodon_constants fresh and reset the one-shot warn flag."""
    import importlib
...
class TestGetHermesHomeProfileWarning:
    def test_classic_mode_no_active_profile_no_warning(
        ...
        """Classic mode: no active_profile file → silent, returns ~/.opencodon."""
    ...
    def test_named_profile_unset_home_warns_once(
        ...
        assert err.count("OPENCODON_HOME fallback") == 1
        assert "'coder'" in err
        assert "#18594" in err

        # 3. One-shot: second and third calls don't re-warn
        fresh_constants.get_opencodon_home()
        fresh_constants.get_opencodon_home()
        err2 = capsys.readouterr().err
        assert "OPENCODON_HOME fallback" not in err2

    def test_opencodon_home_set_suppresses_warning(
        self, fresh_constants, tmp_path, capsys, monkeypatch
    ):
        """Even if active_profile is 'coder', setting OPENCODON_HOME suppresses warning."""
        profile_dir = tmp_path / ".opencodon" / "profiles" / "coder"
        profile_dir.mkdir(parents=True)
        (tmp_path / ".opencodon" / "active_profile").write_text("coder\n")
