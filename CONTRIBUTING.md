# Contributing to opencodon

Thank you for contributing to opencodon! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want opencodon to work everywhere.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo opencodon/opencodon "<your terms>"
  gh search prs --repo opencodon/opencodon --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/opencodon/opencodon/issues?q=) · [PRs (all states)](https://github.com/opencodon/opencodon/pulls?q=is%3Apr).
...
Related: #38284 covers the agent-side analog — opencodon itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every opencodon install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `opencodon skills browse` (labeled "official") and install it with `opencodon skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://github.com/opencodon/opencodon/discussions). Users can install it with `opencodon skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We do not accept memory providers into this repo.** No provider ships in-tree; `plugins/memory/` holds only the discovery layer and the shared config schema. If you want to add a memory backend, publish it as a **standalone plugin repo** that users install into `~/.opencodon/plugins/` (or via a pip entry point).
...
- Integrate with `opencodon memory setup` via `post_setup()` — no need to touch core code
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. opencodon ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.opencodon/plugins/`, project `.opencodon/plugins/`, or a pip entry point) — see [Build a opencodon Plugin](https://github.com/opencodon/opencodon/tree/main/docs)
- Register lifecycle hooks (`pre_tool_call`, `post_tool_call`, `pre_llm_call`, `post_llm_call`, `on_session_start`, `on_session_end`), tools (`ctx.register_tool`), and CLI subcommands (`ctx.register_cli_command`) through the surface we already expose — no core changes needed
- If your plugin needs a capability the framework doesn't expose, that's a feature request to **widen the generic plugin surface** (a new hook or `ctx` method) — never special-case your plugin in core
- Promote it in the [Nous Research Discord](https://github.com/opencodon/opencodon/discussions) `#plugins-skills-and-skins` channel so users can find and install it
...
The installer creates the opencodon venv, wires the `opencodon` command, stamps the
install method for `opencodon update`, and clones the full git project into
...
cd "${OPENCODON_HOME:-$HOME/.opencodon}/opencodon"
...
Use this only if you intentionally do not want opencodon' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `opencodon` entrypoint from this venv; running the
system `python3 -m opencodon_cli.main` can pick up unrelated system Python
packages.
...
uv venv ~/.opencodon/venvs/opencodon-dev --python 3.11
export VIRTUAL_ENV="$HOME/.opencodon/venvs/opencodon-dev"
...
```bash
# The standard installer already put `opencodon` on PATH.
opencodon doctor
opencodon chat -q "Hello"
```

If you used the manual clone fallback, run `./opencodon` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/opencodon" ~/.local/bin/opencodon
```
...
├── cli.py                    # OpencodonCLI class — interactive TUI, prompt_toolkit integration
...
│
├── opencodon_cli/               # CLI command implementations
...
The user may skip setup and keep loading the skill. opencodon only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `opencodon setup` or update `~/.opencodon/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native opencodon tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "opencodon" is the secondary collaborator. If the contributor's commit shows "opencodon" as author because they used opencodon to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing opencodon tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `opencodon --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

opencodon uses a data-driven skin system — no code changes needed to add a new skin.
...
Add to `_BUILTIN_SKINS` dict in `opencodon_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `opencodon_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

opencodon runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the opencodon wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `opencodon_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `opencodon_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `opencodon_cli/gateway_windows.py::
...
opencodon has terminal access. Security matters.
...
2. **Test manually**: Run `opencodon` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/opencodon/opencodon/issues)
- Include: OS, Python version, opencodon version (`opencodon version`), full error traceback
